...
------------------------------------------------
                UNIT      DIS.
 DESCRIPTION1    PRICE    PRICE    QTY    TOTAL
------------------------------------------------<br>
<?php
 for($i=0;$i<sizeof($bill_id);$i++){
 if($bi_discount[$i]!=0) $dis_print='@'.number_format(round($bi_discount[$i]/($bi_price[$i]+$bi_discount[$i])*100),2).'%'; else $dis_print='';
...
// print str_pad($bi_code[$i].' '.$dis_print,16,' ',STR_PAD_RIGHT).'  '.str_pad(number_format($bi_price[$i]+$bi_discount[$i]),5,' ',STR_PAD_LEFT).'  '.str_pad(number_format($bi_price[$i]),5,' ',STR_PAD_LEFT).'   '.str_pad(number_format($bi_qty[$i]),4,' ',STR_PAD_LEFT).'  '.str_pad(number_format(($bi_qty[$i]*$bi_price[$i]),2),7,' ',STR_PAD_LEFT).'<br>';
 print str_pad('[Dis '.$dis_print.']',16,' ',STR_PAD_RIGHT).'  '.str_pad(number_format($bi_price[$i]+$bi_discount[$i]),5,' ',STR_PAD_LEFT).'  '.str_pad(number_format($bi_price[$i]),5,' ',STR_PAD_LEFT).'   '.str_pad(number_format($bi_qty[$i]),4,' ',STR_PAD_LEFT).'  '.str_pad(number_format(($bi_qty[$i]*$bi_price[$i]),2),7,' ',STR_PAD_LEFT).'<br>';
 print '<br>';
}
print '----------------------------------------------<br>';
