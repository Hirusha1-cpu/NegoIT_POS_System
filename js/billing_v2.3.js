...
function setPayment($case) {
	...
	if(document.forms["payForm"]["amount_cash"].value != '') {
		var amount_cash = (document.forms["payForm"]["amount_cash"].value);
	}else{
		var amount_cash = 0; 
		document.forms["payForm"]["amount_cash"].value = 0;
	}
	...
	if(document.getElementById('amount_chque').value!=''){
		...
		var amount_chque=0; 
		document.getElementById('amount_chque').value=0; 
	}
	...
	// }else{ 
	// 	var amount_card=0; 
	// 	document.getElementById('amount_card').value=0; 
	// } -(amount_card * 10)
	...
	// document.getElementById('payment_card_div').innerHTML=thousands_separators(amount_card);
	...
	if((amount_cash!=0)&&(amount_chque!=0)){
		document.getElementById('payment_type').value=3; 
	}else if(amount_cash!=0){
		document.getElementById('payment_type').value=1; 
	}else if(amount_chque!=0) {
		document.getElementById('payment_type').value=2;
	}
	// else if (amount_card != 0) {
...
function deleteReturn(id){
 ...
 if (check== true)
	window.location = 'index.php?components=billing&action=delete_return&id='+id;
}
...
function validateQTY2(){
    ...
    if($w_price1=='') $count++;
    if($r_price1=='') $count++;
    if(($c_price1>500000)||($w_price1>500000)||($r_price1>500000)){
    	$count++;
    	$msg="Invalid Price";
    }
	for($i=1;$i<=10;$i++){
...
function apendPart(){
    ...
    if(document.getElementById('part').value=='') $count++;
    if(document.getElementById('qty').value=='') $count++;
    if ($count!=0) {
        alert($msg);
        return false;
