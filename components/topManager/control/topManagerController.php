...
switch ($_REQUEST['action']) {
   ...
   case "trans_report":
      ...
      getTransSalesman('all');
      getTransSubSystem('all');
      include_once 'components/trans/view/trans_report.php';
      break;
   ...
   case "quotation":
      ...
      if (isset($_COOKIE['district'])) {
         ...
         if (isset($_GET['cust'])) {
            if (validateQuotNo())
               header('Location: index.php?components=topmanager&action=new_quot&cust_id=' . $_GET['cust'] . '&validity=30');
         }
      }
      if (isMobile())
