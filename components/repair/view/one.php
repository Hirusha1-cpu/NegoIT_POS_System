...
<?php } ?>
	...
	function checkDrawer(){
	  var $part_name=document.getElementById('part').value;
	  if($part_name==''){
	  	alert('Please Enter the Part Name');
	  }else{
		  document.getElementById("div_dr").innerHTML=document.getElementById("loading").innerHTML;
		  var xhttp = new XMLHttpRequest();
		  xhttp.onreadystatechange = function() {
		    if (this.readyState == 4 && this.status == 200) {
			    var returntext=this.responseText;
			    if(returntext=='') $txt='NA'; else $txt=returntext;
			    document.getElementById("div_dr").innerHTML = $txt;
		     }
		  };
		  xhttp.open("GET", 'index.php?components=repair&action=get_part_drawer&part_name='+$part_name, true);
		  xhttp.send();
	  }
	}
...
<tr><td align="center"><?php 
	...
	<table align="center" style="font-family:'Calibri'"><tr><td valign="top">
		...
		<table style="font-size:12pt" width="100%">
			<tr><th colspan="6" style="background-color:#467898;color :white;">Spair Part Allocation</th></tr>
			<?php if($job_owner==$user_id){ ?>
			<!--<tr><th colspan="4"><input type="text" name="part" id="part" placeholder="Spair Part" /><input type="number" name="qty" id="qty" style="width:40px" placeholder="Qty" /><input type="submit" value="Add" /></th><th><input type="button" onclick="checkDrawer()" value="DR" /></th><th><div id="div_dr"></div></th></tr>-->
			<tr><th colspan="3">
			 <!--------------------------------->
				...
				<th><input type="number" name="qty" id="qty" style="width:40px" placeholder="Qty" /></th><th><input type="submit" value="Add" /></th></tr>
			<tr><th colspan="6"><hr /></th></tr>
			<?php } ?>
		</table>
		<table style="font-size:12pt" width="100%">
			<tr style="background-color:#467898;color :white;"><th>DR</th><th>Part</th><th>Qty</th><?php if($topuser) print '<th>Total</th>'; ?><th></th></tr>
			<?php 
			for($i=0;$i<sizeof($order_part_name);$i++){
				...
				print '<tr bgcolor="'.$color.'"><td style="padding-left:10px">'.$order_part_drawer[$i].'</td><td style="padding-left:10px">'.$order_part_name[$i].'</td><td align="right" style="padding-right:10px">'.$order_part_qty[$i].'</td>'.$show_total.'<td align="center">'.$removepart.'</td></tr>';
			} 
				if($topuser) $show_total='<td align="right" style="padding-right:10px"><strong>'.number_format($cogs).'</strong></td>'; else $show_total='';
				print '<tr bgcolor="DDDDDD"><td colspan="3" style="padding-left:10px"><strong>Job Parts Cost</strong></td>'.$show_total.'<td></td></tr>';
			?>
		</table>
		</form>
		<div id="cust_details" style="display:none">
		<br />
		<table bgcolor="#E5E5E5" style="font-size:10pt; font-family:'Calibri'">
		...
		<hr />
		<?php 
		if($systemid==4 && $action=='list_one'){
			print '<table width="100%" bgcolor="#EEEEEE"><tr><td style="font-size:16pt; color:navy" align="center">Pre Check for Repairs</td></tr></table>';
		if($bm_bocom_type==0){ ?>
		...
		<?php }else{
			print '<table width="100%" style="font-size:12pt; color:navy">';
			...
			print '<table width="100%" bgcolor="#EEEEEE"><tr><td style="font-size:16pt; color:navy" align="center">Post Check for Repairs</td></tr></table>';
		if($bm_bocom2!=''){
			print '<table width="100%" style="font-size:12pt; color:navy">';
	...
	<?php if($action=='list_one'){ ?>
		<form method="post" action="index.php?components=repair&action=add_repair_comment&id=<?php print $_GET['id']; ?>" >
		<input type="hidden" id="repcom_type" name="repcom_type" value="<?php if($topuser) print '2'; else print '1'; ?>" />
		<table width="100%" style="font-size:10pt" >
	...
	<form action="index.php?components=repair&action=finish&id=<?php print $_REQUEST['id']; ?>" method="post" id="form1" >
		...
		<tr><td style="background-color:#467898;color :white;"><strong>Invoice No</strong></td><td bgcolor="#EEEEEE"><?php print  str_pad($_REQUEST['id'], 7, "0", STR_PAD_LEFT); ?></td><td></td><td style="background-color:#467898;color :white;"><strong>Salesman</strong></td><td bgcolor="#EEEEEE"><?php print  ucfirst($bi_salesman); ?></td><td rowspan="2">
		<?php if($button=='Pick'){ ?>
			<div id="orderprocess1"><input type="button" value="Pick" style="height:50px; width:70px; background-color:green; font-weight:bold; color:white" onclick="orderPick()" /></div>
		<?php }else	if($button=='Repair'){ 
			if(($job_owner==$user_id)&&((!$gpm_exceed) ||(($total==$bm_force_accept)&&(!is_null($bm_force_accept))))){
		?>
			<div id="orderprocess1"><input type="button" value="Finish" style="height:50px; width:70px; background-color:green; font-weight:bold; color:white" onclick="orderProcess(1)" /></div>
			<div id="orderprocess2"><input type="button" value="Reject" style="height:50px; width:70px; background-color:#CC0000; font-weight:bold; color:white" onclick="orderProcess(2)" /></div>
		<?php }	if($topuser){
				print '<div id="orderprocess3"><input type="button" value="Unassign" style="height:50px; width:70px; background-color:orange; font-weight:bold; color:white" onclick="orderUnassign()" /></div>'; 
				}
			}else if($button=='Print'){ ?>
			<input type="button" value="Print" style="height:50px; width:70px; background-color:#0099FF; font-weight:bold; color:white" onclick="window.location = 'index.php?components=billing&action=finish_bill&id=<?php print $_REQUEST['id']; ?>'" />
		<?php } ?>
		</td></tr>
		<tr><td style="background-color:#467898;color :white;"><strong>Invoice Date</strong></td><td bgcolor="#EEEEEE"><?php print  $bi_date; ?></td><td></td><td style="background-color:#467898;color :white;"><strong>Customer</strong></td><td bgcolor="#EEEEEE"><a style="cursor:pointer; color:blue; text-decoration:underline" onclick="displayCust()"><?php print  ucfirst($bi_cust); ?></a></td></tr>
		...
		<tr><td colspan="5">
			<table>
			<tr style="background-color:#C0C0C0"><th>Item Description</th><th style="padding-left:20px; padding-right:20px">Item Qty</th><th style="padding-left:20px; padding-right:20px">Total Price</th><th></th></tr>
		...
		<tr><td colspan="5" height="50px"><div id="bi_comment" style="font-size:10; color:maroon; background-color:#FEEFE0"></div></td><td></td></tr>
		<tr><td colspan="5">
			<table width="100%">
			<tr style="background-color:#C0C0C0"><?php if(isset($_COOKIE['report'])){ ?><td style="padding-left:20px;">Seen By</td><?php } ?><td style="padding-left:20px;">Picked By</td><td style="padding-left:20px;">Repaired By</td><td style="padding-left:20px;">Deliverd By</td></tr>
			<tr style="background-color:#F0F0F0"><?php if(isset($_COOKIE['report'])){ ?><td style="padding-left:20px;"><?php print  ucfirst($bi_seen_by); ?></td><?php } ?><td style="padding-left:20px;"><?php print  ucfirst($bi_picked_by); ?></td><td style="padding-left:20px;"><?php print  ucfirst($bi_repaired_by); ?></td><td style="padding-left:20px;"><?php print  ucfirst($bi_deliverd_by); ?></td></tr>
			<tr style="background-color:#F0F0F0"><?php if(isset($_COOKIE['report'])){ ?><td style="padding-left:20px;"><?php print  $bi_seen_date.'<br/>'.$bi_seen_time; ?></td><?php } ?><td style="padding-left:20px;"><?php print  $bi_picked_date.'<br/>'.$bi_picked_time; ?></td><td style="padding-left:20px;"><?php print  $bi_repaired_date.'<br/>'.$bi_repaired_time; ?></td><td style="padding-left:20px;"><?php print  $bi_deliverd_date.'<br/>'.$bi_deliverd_time; ?></td></tr>
			</table>
		</td><td></td></tr>
		</table>
	</form>
