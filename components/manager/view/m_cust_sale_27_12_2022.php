...
<script type="text/javascript">
	...
	function paymentCorrelate(){
		var inv_no = [<?php for ($x=0;$x<sizeof($invoice_no);$x++){ print '"'.$invoice_no[$x].'",'; } ?>	];
	...
	<div class="w3-row">
		...
		<div class="w3-col">
			<form action="index.php" method="get" onsubmit="return validateDateRange2()" >
				...
				<tr><td style="background-color:#DDDDDD">Customer</td><td><input type="text" name="customer" id="tags1" value="<?php print $customer; ?>" onclick="this.value=''" /></td><th rowspan="5" align="right"><input type="submit" onclick="setCustID()" value="GET" style="width:80px; height:100px" /></th></tr>
				<tr><td style="background-color:#DDDDDD">From Date</td><td><input type="date" id="datefrom" name="datefrom" style="width:130px" value="<?php print $fromdate; ?>" /></td></tr>
				<tr><td style="background-color:#DDDDDD">To Date</td><td><input type="date" id="dateto" name="dateto" style="width:130px" value="<?php print $todate; ?>" /></td></tr>
				<tr><td style="background-color:#DDDDDD; color:maroon;"><input type="checkbox" id="correlate" onclick="paymentCorrelate()" /> Fully Paid Bills</td><td><div id="loading_correlate"></div></td></tr>
				<tr><td style="background-color:#DDDDDD">M Type</td><td style="color:red"><?php print ucfirst($cust_mtype); ?></td></tr>
				</table>
			</form>

			<br/>
			<?php if($customer!='') print '<div id="chart_div" style="width: 100%; height: 500px;"></div>'; ?>
			<br />
			...
			<table align="center" style="font-size:10pt">
				<tr>
					...
					<td>Cheque Postpone</td><td width="100px"></td>
					<td width="20px" height="20px" bgcolor="green"></td>
					<td>Postpone-Cleared</td>
				</tr>
			</table>
			
			<table align="center" bgcolor="#E5E5E5" height="100%" border="1" cellspacing="0" style="font-size:xx-small; overflow-x:auto;">
				<tr>
				...
				if(sizeof($invoice_no)>sizeof($payment_id)) $sizeofloop=sizeof($invoice_no);
				if(sizeof($invoice_no)<sizeof($payment_id)) $sizeofloop=sizeof($payment_id);

					for($j=0;$j<sizeof($all_dates);$j++){
						for($i=0;$i<sizeof($invoice_no);$i++){
								if($billed_date[$i]==$all_dates[$j]){
									$style=$style1=$style2='';
									if($billed_chq_postpone[$invoice_no[$i]]==1){ $style='style="color:white; background-color:#AA4477;"'; $style1='style="color:white;"'; $style2='color:white;'; }
									if($billed_chq_postpone[$invoice_no[$i]]>1){ $style='style="color:white; background-color:green;"'; $style1='style="color:white;"'; $style2='color:white;'; }
									if($billed_chq_return[$invoice_no[$i]]==1){ $style='style="color:white; background-color:red;"'; $style1='style="color:white;"'; $style2='color:white;'; }
									...
									$gr_date[]=$billed_date[$i]; $gr_credit[]=$credit_balance;
									print '<tr '.$style.'>
											<td align="center">
											...
											<td align="right" style="padding-right:10px;">
												<div id="col3_'.$invoice_no[$i].'">'.number_format($invoice_Total[$i]).'</div>
											</td>
											<td align="right" style="padding-right:10px;">
												<div id="col4_'.$invoice_no[$i].'">'.number_format($payment_cash[$invoice_no[$i]]).'</div>
											</td>
											<td  align="right" style="padding-right:10px;">
												<div id="col5_'.$invoice_no[$i].'">'.number_format($payment_card[$invoice_no[$i]]).'</div>
											</td>
											<td  align="right" style="padding-right:10px;">
												<div id="col6_'.$invoice_no[$i].'">'.number_format($payment_bank[$invoice_no[$i]]).'</div>
											</td>
											<td align="right" style="padding-right:10px;">
												<div id="col7_'.$invoice_no[$i].'">'.number_format($payment_chque[$invoice_no[$i]]).'</div>
											</td>
											<td  align="right" style="padding-right:10px;">
												<div id="col8_'.$invoice_no[$i].'">'.number_format($bill_credit).'</div>
											</td>
											<td  align="right" style="padding-right:10px;">
												<div id="col9_'.$invoice_no[$i].'">'.number_format($credit_balance).'</div>
											</td>
											<td align="center">
						...
						for($i=0;$i<sizeof($chqpayment_id);$i++){
							if($chqrtn_date[$i]==$all_dates[$j]){
								...
								if($chqpayment_invno[$i]==0){ $printid=$chqpayment_id[$i]; $url='index.php?components=billing&action=finish_payment&id='.$chqpayment_id[$i]; }else{ $printid=$chqpayment_invno[$i]; $url='index.php?components=billing&action=finish_bill&id='.$chqpayment_invno[$i]; }
								$gr_date[]=$chqrtn_date[$i]; $gr_credit[]=$credit_balance;
								print '<tr style="color:white; background-color:red;">
											...
											<td align="right" style="padding-right:10px;">0</td>
											<td align="right" style="padding-right:10px;">0</td>
											<td  align="right" style="padding-right:10px;">'.number_format($chqpayment_amount[$i]).'</td>
											<td></td>
											<td  align="right" style="padding-right:10px;">'.number_format($credit_balance).'</td>
						...
						for($i=0;$i<sizeof($payment_id);$i++){
								if($payment_date[$i]==$all_dates[$j]){
									...
									if($payment_chq_postpone[$i]==1){ $style='style="color:white; background-color:#AA4477;"'; $style1='style="color:white;"'; $style2='color:white;'; }
									if($payment_chq_postpone[$i]>1){ $style='style="color:white; background-color:green;"'; $style1='style="color:white;"'; $style2='color:white;'; }
									if($payment_chq_return[$i]==1){ $style='style="color:white; background-color:red;"'; $style1='style="color:white;"'; $style2='color:white;'; }
									if($payment_type[$i]=='Cash'){ $payment_cash2=$payment_amount[$i]; $payment_card2=0; $payment_bank2=0; $payment_chque2=0; $total_cash+=$payment_amount[$i]; }
									if($payment_type[$i]=='Card'){ $payment_cash2=0; $payment_card2=$payment_amount[$i]; $payment_bank2=0; $payment_chque2=0; $total_card+=$payment_amount[$i]; }
									if($payment_type[$i]=='Bank'){ $payment_cash2=0; $payment_card2=0; $payment_bank2=$payment_amount[$i]; $payment_chque2=0; $total_bank+=$payment_amount[$i]; }
									if($payment_type[$i]=='Chque'){ $payment_cash2=0; $payment_card2=0; $payment_bank2=0; $payment_chque2=$payment_amount[$i]; $total_chque+=$payment_amount[$i]; }
									$gr_date[]=$payment_date[$i]; $gr_credit[]=$credit_balance;
									print '<tr '.$style.' >
											...
											<td align="right" style="padding-right:10px;"></td>
											<td align="right" style="padding-right:10px;">'.number_format($payment_cash2).'</td>
											<td align="right" style="padding-right:10px;">'.number_format($payment_card2).'</td>
											<td align="right" style="padding-right:10px;">'.number_format($payment_bank2).'</td>
											<td  align="right" style="padding-right:10px;">'.number_format($payment_chque2).'</td>
											<td></td>
											<td  align="right" style="padding-right:10px;">'.number_format($credit_balance).'</td>
							...
							if(sizeof($all_dates)==($j+1)){
								print '<tr style="font-weight:bold; background-color:gray; color:white">
										...
										<td align="right" style="padding-right:10px;">'.number_format($invoice_sub_total).'</td>
										...
										<td align="right" style="padding-right:10px;">'.number_format($total_bank).'</td>
