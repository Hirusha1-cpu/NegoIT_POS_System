    ...
    */
    ...
    function changeStatus(selectElement, depositId) {
        ...
        if (selectedValue === "3") { // Rejected
            Swal.fire({
                ...
                inputValidator: (value) => {
                    ...
                    if (!/^[a-zA-Z0-9\s.,\-_]+$/.test(value)) {
                        return 'No special characters allowed! (Only letters, numbers, spaces, . , - _ )';
                    }
                    if (value.length < 2) {
                        return 'Please provide a more detailed reason (at least 2 characters).';
                    }
                    if (value.length > 255) {
                        return 'Reject reason is too long. Maximum 255 characters allowed.';
                    }
                }
            }).then((result) => {
                if (result.isConfirmed) {
                    // Send AJAX with reason
                    updateStatusAjax(depositId, 3, result.value, selectElement);
                } else {
            ...
            Swal.fire({
                title: 'Are you sure?',
                text: 'Do you want to change the status of this deposit as deposited?',
                icon: 'warning',
                showCancelButton: true,
                ...
                if (result.isConfirmed) {
                    updateStatusAjax(depositId, selectedValue, '', selectElement);
                } else {
                    selectElement.value = "";
                }
            });
    ...
    function updateStatusAjax(depositId, status, reason, selectElement) {
        fetch('index.php?components=<?php echo $components; ?>&action=change_payment_deposit_status_ajax', {
            method: 'POST',
            headers: {
...
<div id="rejectModal"
    ...
    <div style="background:#fff; padding:20px; border-radius:8px; min-width:300px; max-width:90vw;">
        ...
        <label for="rejectReason">Please enter the reason for rejection:</label><br>
        <textarea id="rejectReason" style="width:100%; height:80px;"></textarea><br>
        <button onclick="submitRejection()">Submit</button>
        <button onclick="closeRejectModal()">Cancel</button>
        <input type="hidden" id="rejectDepositId" value="">
        <input type="hidden" id="rejectSelectElementId" value="">
    </div>
</div>
...
<form action="index.php?components=<?php print $components; ?>&action=pending_payment_cash_deposits" method="post">
    <table border="0" width="900px" align="center" height="100%" cellspacing="0"
        ...
        <tr>
            <td colspan="7">
                <br> &nbsp;&nbsp;&nbsp;Note: This table shows cash payments collected by salesman, either transfer to
                you or bank (pending).
                <hr />
            </td>
        </tr>
