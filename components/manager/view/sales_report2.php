		...
		<tr>
							...
							<?php if ($_GET['components'] == 'manager') { ?>
								<select name="selection" id="selection" style="font-weight:bold" onchange="changeSelection()">
									<option value="customer" <?php if ($selection == 'customer')
										print 'selected="selected"'; ?>>Customer
									</option>
			...
			<td>
				<select id="sold_price_select" onchange="updateHiddenField()">
					<option value="all">--ALL--</option>
					<option value="sold_price" <?php if ((isset($_GET['group_by'])) && ($_GET['group_by'] == 'sold_price'))
						print "selected";
					else
						print ''; ?>>Sold Price</option>
				</select>
			</td>
			<td width="20px"></td>
				...
				<table align="center" bgcolor="#E5E5E5" height="100%" border="1" cellspacing="0" style="font-size:10pt">
					...
					if (isset($_REQUEST['category'])) {
						...
						for ($j = 0; $j < sizeof($item_des); $j++) {
							...
							$total += $item_price[$j] * $item_qty[$j];
							$item_total += $item_price[$j];
						...
						if ((isset($_REQUEST['group_by'])) && ($_REQUEST['group_by'] == 'sold_price')) {
							print '<tr style="font-weight:bold; background-color:gray; color:white">
								...
								<td align="right" style="padding-right:10px">' . number_format($item_total, $decimal) . '</td>
								<td align="right" style="padding-right:10px">' . number_format($total, $decimal) . '</td>
							</tr>';
						}
					}
					?>
