...
<div style="background-color:#EEEEEE; border-radius:10px">
	<form action="index.php" method="get" onsubmit="return validateDateRange2()" >
		...
		<table align="center" height="100%" cellspacing="0" style="font-size:10pt" >
			...
			<tr><td style="color:maroon"><input type="checkbox" id="correlate" onclick="paymentCorrelate()" /> Fully Paid Bills</td><td colspan="2"><div id="loading_correlate"></div></td><td colspan="2"></td><td style="font-size:9pt">Customer M Type</td><td style="color:red"><?php print ucfirst($cust_mtype); ?></td></tr>
		</table>
	</form>
</div>
...
<div id="print" >
	...
	<table align="center" bgcolor="#E5E5E5" height="100%" border="1" cellspacing="0" style="font-size:10pt; -webkit-print-color-adjust: exact;">
		<tr>
		...
		for($j=0;$j<sizeof($all_dates);$j++){
					...
					if($billed_date[$i]==$all_dates[$j]){
						...
						print '<tr '.$style.'>
								...
								<td align="right" style="padding-right:10px;">
									<div id="col5_'.$invoice_no[$i].'">'.number_format($invoice_Total[$i]).'</div>
								</td>
								<td align="right" style="padding-right:10px;">
									<div id="col6_'.$invoice_no[$i].'">'.number_format($payment_cash[$invoice_no[$i]]).'</div>
								</td>
								<td align="right" style="padding-right:10px;">
									<a href="#" style="text-decoration:none" title="'.$payment_card_tr[$invoice_no[$i]].'">
										<div id="col13_'.$invoice_no[$i].'">'.number_format($payment_card[$invoice_no[$i]]).'</div>
									</a>
								</td>
								<td align="right" style="padding-right:10px;">
									<a href="#" style="text-decoration:none" title="'.$payment_bank_tr[$invoice_no[$i]].'">
										<div id="col7_'.$invoice_no[$i].'">'.number_format($payment_bank[$invoice_no[$i]]).'</div>
									</a>
								</td>
								<td align="right" style="padding-right:10px;">
									<a href="#" style="text-decoration:none; '.$style2.'" title="'.$chq_details[$invoice_no[$i]].'">
										<div id="col8_'.$invoice_no[$i].'">'.number_format($payment_chque[$invoice_no[$i]]).'</div>
									</a>
								</td>
								<td  align="right" style="padding-right:10px;">
									<div id="col9_'.$invoice_no[$i].'">'.number_format($bill_credit).'</div>
								</td>
								<td  align="right" style="padding-right:10px;">
									<div id="col10_'.$invoice_no[$i].'">'.number_format($credit_balance).'</div>
								</td>
			...
			for($i=0;$i<sizeof($chqpayment_id);$i++){
				if($chqrtn_date[$i]==$all_dates[$j]){
					...
					if($chqpayment_invno[$i]==0){
						$printid=$chqpayment_id[$i]; $url='index.php?components=billing&action=finish_payment&id='.$chqpayment_id[$i]; 
					}else{ 
						$printid=$chqpayment_invno[$i]; $url='index.php?components=billing&action=finish_bill&id='.$chqpayment_invno[$i]; 
					}
					...
					print '<tr style="color:white; background-color:red;">
							...
							<td align="right" style="padding-right:10px;">0</td>
							<td  align="right" style="padding-right:10px;">
								<a href="#" style="text-decoration:none; color:white" title="'.$chqrtn_chq_details[$i].'">'.number_format($chqpayment_amount[$i]).'</a>
							</td>
							<td align="right" style="padding-right:10px;">0</td>
			...
			for($i=0;$i<sizeof($payment_id);$i++){
				if($payment_date[$i]==$all_dates[$j]){
					...
					if($payment_invoice[$i]==0) $payment_inv=''; else $payment_inv=str_pad($payment_invoice[$i], 7, "0", STR_PAD_LEFT);
					...
					print '<tr '.$style.'>
							...
							<td  align="right" style="padding-right:10px;">
								<a href="#" style="text-decoration:none;" title="'.$payment_card_details[$i].'">'.number_format($payment_card2).'</a>
							</td>
							<td  align="right" style="padding-right:10px;">
								<a href="#" style="text-decoration:none;" title="'.$bank_trans[$i].'">'.number_format($payment_bank2).'</a>
							</td>
							<td  align="right" style="padding-right:10px;">
								<a href="#" style="text-decoration:none; '.$style2.'" title="'.$payment_chq_details[$i].'">'.number_format($payment_chque2).'</a>
							</td>
							<td align="right" style="padding-right:10px;">0</td>
							<td  align="right" style="padding-right:10px;">'.number_format($credit_balance).'</td>
			...
			if(sizeof($all_dates)==($j+1)){
				print '<tr style="font-weight:bold; background-color:gray; color:white">
						...
						<td align="right" style="padding-right:10px;">
							<div id="div2">'.number_format($invoice_sub_total).'</div>
						</td>
						<td align="right" style="padding-right:10px;">
							<div id="div3">'.number_format($total_cash).'</div>
						</td>
						<td align="right" style="padding-right:10px;">
							<div id="div7">'.number_format($total_card).'</div>
						</td>
						<td align="right" style="padding-right:10px;">
							<div id="div4">'.number_format($total_bank).'</div>
						</td>
						<td align="right" style="padding-right:10px;">
							<div id="div5">'.number_format($total_chque).'</div>
						</td>
						<td align="right" style="padding-right:10px;">
							<div id="div6">'.number_format($total_credit).'</div>
						</td>
						<td colspan="3"></td>
					</tr>';
				print '</table>
