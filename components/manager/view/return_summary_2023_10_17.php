...
<div id="print">
	...
	<?php for($i=0;$i<sizeof($rtn_inv);$i++){
			...
			print '<tr><td align="center"><a href="index.php?components='.$bill_module.'&action=finish_return&id='.$rtn_inv[$i].'">'.str_pad($rtn_inv[$i], 7, "0", STR_PAD_LEFT).'</a></td><td align="center">'.$rtn_date[$i].'</td><td style="padding-left:10px; padding-right:10px"><a href="" style="text-decoration:none" title="'.$rtn_cust[$i].'">'.$cust.'</a></td><td>&nbsp;&nbsp;'.ucfirst($rtn_by[$i]).'&nbsp;&nbsp;</td></tr>';
	} ?>
	</table>
</div>
