...
<script type="text/javascript">
	...
	function clearChqueAjax($id){
		...
		console.log($bnk,$pydate,$btn,$components)
	}
</script>
<!--// Scripts -->
...
<div class="w3-container" style="margin-top:75px">
	...
	<div class="w3-row">
	  	...
	  	<div class="w3-col">
			<form action="index.php?components=<?php print $_GET['components']; ?>&action=chque" method="post" onsubmit="return validateDateRange()">
				<input type="hidden" id="components" name="components" value="<?php print $_GET['components']; ?>" />
				<table border="0" align="center" height="100%" cellspacing="0"  style="font-size:10pt; font-family:Calibri;     max-width: fit-content;overflow-x: auto;display: block;">
					<tr>
						<td align="center" bgcolor="#DDDDDD" width="50px" rowspan="2">
							Date Range<br><input type="checkbox" onchange="window.location = 'index.php?components=<?php print $_GET['components']; ?>&action=chque_range'" />
						</td>
						...
						<td rowspan="2"><input style="width:60px; height:40px" type="submit" value="GET"/></td>
					</tr>
					<tr>
						<td class="shipmentTB3"><strong>Shop/Store : </strong></td>
						<td>
							<select id="store" onchange="window.location = 'index.php?components=<?php print $_GET['components']; ?>&action=chque&dateto='+document.getElementById('dateto').value+'&st='+this.value">
					...
					<?php if(sizeof($py_id2)>0) print '<tr><td colspan="4><br>Note: Following shows List of cashed cheques<hr /></td></tr>'; ?>
				</table>
			</form>
		</div>
	  	<div class="w3-col">
			...
			<table align="center" bgcolor="#E5E5E5" height="100%" border="1" cellspacing="0" style="font-size:10pt; font-family:Calibri">
				...
				<tr>
					...
					<th align="right" style="padding-right:10px"><?php print number_format($chque_total); ?></th>
					<td colspan="2"></td>
				</tr>
			</table>
		</div>	
