		...
		<div class="w3-col">
			<table height="100%" cellspacing="0" style="font-family:Calibri" bgcolor="#F0F0F0" border="0">
				<tr>
					...
					<td rowspan="4">
						<?php if ($menu_components == 'manager' || $menu_components == 'topmanager') { ?>
							<?php if ($gps != '')
								print '<form id="map_form" method="post" action="index.php?components=' . $menu_components . '&action=show_map" target="_blank" style="cursor:pointer">
				...
				<tr>
					...
					<td width="100px" class="shipmentTB6" <?php if ($menu_components != 'manager' && $menu_components != 'topmanager') {
				...
				<table align="center" bgcolor="#E5E5E5" height="100%" border="1" cellspacing="0"
					...
					<tr>
						...
						<td colspan="<?php print $colspan; ?>"
							style="border:0; background-color:black; color:white; font-weight:bold">Invoiced
							Collection</td>
					</tr>
					<tr>
						<th width="60px">#</th>
					...
					for ($i = 0; $i < sizeof($invoice_no); $i++) {
						...
						if ($systemid == 17) {
							print '<td align="right" style="padding-right:10px;">' . number_format($invoice_total_wo_tax[$i], $decimal) . '</td>';
							print '<td align="right" style="padding-right:10px;">' . number_format($invoice_tax[$i], $decimal) . '</td>';
						}
						...
						if (sizeof($invoice_no) == ($i + 1)) {
							...
							if ($systemid == 17) {
								print '<td align="right" style="padding-right:10px;">' . number_format(array_sum($invoice_total_wo_tax), $decimal) . '</td>
								<td align="right" style="padding-right:10px;">' . number_format(array_sum($invoice_tax), $decimal) . '</td>';
							}
							print '<td align="right" style="padding-right:10px;">' . number_format($total_cash, $decimal) . '</td>
				...
				<table align="center" bgcolor="#E5E5E5" height="100%" border="1" cellspacing="0"
					...
					for ($i = 0; $i < sizeof($payment_id); $i++) {
						...
						print '<tr>
								...
								<td align="center">
									<a ' . (isset($payment_details[$i]) ? $payment_details[$i] : '') . '>' . $payment_type[$i] . '</a>
								</td>
					...
					foreach ($salesman_summary as $salesman => $data) {
						...
						<tr>
							...
							<td class="padding" align="right">
								<?php echo $data['Cash'] == 0 ? '' : number_format($data['Cash'], $decimal); ?>
							</td>
							<td class="padding" align="right">
								<?php echo $data['Card'] == 0 ? '' : number_format($data['Card'], $decimal); ?>
							</td>
							<td class="padding" align="right">
								<?php echo $data['Bank'] == 0 ? '' : number_format($data['Bank'], $decimal); ?>
							</td>
							<td class="padding" align="right">
