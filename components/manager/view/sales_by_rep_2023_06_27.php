...
<div style="background-color:#EEEEEF; border-radius: 5px; padding-left:10px; padding-right:10px">
	...
	</td><td width="50px"></td><td><strong>Category</strong></td><td>
	...
	</td><td width="50px"></td><td><strong>Store</strong></td><td>
...
<div id="printheader" style="display:none" >
	...
	<p style="font-size:14pt"><strong>Sales Quantity By Sales Rep</strong></p>
	<table><tr><td>
...
<div id="print">
	<table align="center" height="100%" border="1" cellspacing="0" style="font-family:Calibri">
		...
		?>
		<th class="shipmentTB4">Total Sale QTY</th><th class="shipmentTB4">Available Stock</th></tr>
		<?php
		$total_sub_qty=0;
		for($i=0;$i<sizeof($itm_id);$i++){
			$total_qty=0;
			$itm_id_tmp='';
			print '<tr><td class="shipmentTB3" style="background-color:#DDDDFE">'.$itm_desc[$i].'</td>';
			for($j=0;$j<sizeof($rep_id);$j++){
				$total_qty+=$itm_qty[$itm_id[$i]][$rep_id[$j]];
				$total_rep[$j]+=$itm_qty[$itm_id[$i]][$rep_id[$j]];
				$total_sub_qty+=$total_qty;
				print '<td align="right" class="shipmentTB3" style="background-color:#EEEEEE">'.number_format($itm_qty[$itm_id[$i]][$rep_id[$j]]).'</td>';
			}
			print '<td align="right" class="shipmentTB3" style="background-color:#FDDDDD">'.number_format($total_qty).'</td><td align="right" class="shipmentTB3" style="background-color:#FDDDDD">'.number_format($itm_stock[$i]).'</td></tr>';
		}
			print '<tr><td class="shipmentTB3" style="background-color:#DDDDFE"><strong>Total</strong></td>';
			for($j=0;$j<sizeof($rep_id);$j++){
				print '<td align="right" class="shipmentTB3" style="background-color:#DDDDDD"><strong>'.number_format($total_rep[$j]).'</strong></td>';
			}
			print '<td align="right" class="shipmentTB3" style="background-color:#FDDDDD"><strong>'.number_format($total_sub_qty).'</strong></td><td align="right" class="shipmentTB3" style="background-color:#FDDDDD"><strong>'.number_format(array_sum($itm_stock)).'</strong></td></tr>';
		?>
	</table>
</div>
<br />
