...
<div class="w3-row">
  ...
  <div class="w3-col">
	...
	<br />
		...
		<tr><td colspan="12" style="border:0; background-color:black; color:white; font-weight:bold"></td></tr>
		<tr bgcolor="#CCCCCC">
			<th width="50px">#</th>
		...
		if(($customer!='')||($store1!='')){
			for($j=0;$j<sizeof($item_des);$j++){
				print '<tr>
						...
						<td style="padding-left:30px">'.$item_des[$j].'</td>
						<td align="right" style="padding-right:30px">'.$item_qty[$j].'</td>
					</tr>';
			}
		}
		?>
		...
		<tr bgcolor="#CCCCCC">
			...
			<th width="100px">Sold/Total Items<br>In the category <?php print $customer_id; ?></th>
		</tr>
		<?php
		if($selection=='store') $cus_st='store1='.$store1; else $cus_st='customer='.$customer.'&customer_id='.$customer_id;
		if(($customer!='')||($store1!='')){
			for($i=0;$i<sizeof($cat2_id);$i++){
				if($cat2_sold_total[$i]!=0){ $color2 = "color:blue;"; } else { $color2 = "color:black;"; }
					...
					print '<tr>
								...
								<td style="padding-left:30px">
									<a style="text-decoration:none; color:'.$color2.'" '.$url.'>'.$cat2_name[$i].'</a>
								</td>
								<td align="right">'.$cat2_sold_total[$i].' / '.$cat2_total_count[$i].'</td>
						</tr>';
			}
		}
		?>
