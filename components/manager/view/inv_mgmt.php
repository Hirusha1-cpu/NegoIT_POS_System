...
<script type="text/javascript">
	...
	function changeRecoveryAgent($type){
		var check = confirm("Do you want to change Recovery Agent?");
	  	var $invoice_no = document.getElementById($type+'_hi').value;
		var $rg_new = document.getElementById('recoveryAgent').value;
		var $master_pw4 = document.getElementById('master_pw4').value;
	 	if(check==true){
			if($master_pw4!=''){
				...
				xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
				xmlhttp.send('id='+$invoice_no+'&rg_new='+$rg_new+'&master_pw4='+$master_pw4);
			}else{
				alert('Please fill the Date and Master Password');
			}
		}
...
<?php 	
	for($i=0;$i<sizeof($inv_pay_id);$i++){
		print '<tr style="background-color:#CCCCCC"><td align="center"><a href="index.php?components=billing&action=finish_payment&id='.$inv_pay_id[$i].'">'.str_pad($inv_pay_id[$i], 7, "0", STR_PAD_LEFT).'</a></td><td align="center">'.$inv_pay_date[$i].'</td><td align="center" style="color:'.$inv_pay_typec[$i].'">'.$inv_pay_typen[$i].'</td><td align="right">'.number_format($inv_pay_amount[$i]).'&nbsp;&nbsp;&nbsp;</td><td></td><td align="center">'.ucfirst($inv_pay_by[$i]).'</td></tr>';
	}
}
?>
</table>
