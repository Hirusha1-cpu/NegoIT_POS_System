...
<div style="background-color:#EEEEEE; border-radius:10px">
	<table align="center" height="100%" cellspacing="0" style="font-family:Calibri" bgcolor="#F0F0F0" border="0">
		<tr>
			...
			<td colspan="4">
				<form id="search_form" action="index.php" method="get">
					...
					<table>
						<tr>
							<td align="center" bgcolor="silver" style="color:white">&nbsp;Date Range&nbsp;<br><input type="checkbox"
									id="date_range" onchange="setDateRange('<?php print $date1; ?>','<?php print $date2; ?>')" <?php if (($date1 != '') && ($date2 != ''))
							...
							<td>
								<a onclick="document.getElementById('search_form').submit();" style="cursor:pointer"><img
										src="images/search.png" style="width:30px; vertical-align:middle" /></a>
							</td>
						</tr>
					</table>
				</form>
			...
			<td align="right" <?php if ($menu_components != 'manager' && $menu_components != 'topmanager') {
				print 'style=display:none'; ?> <?php } ?>><strong>Cash Back :</strong></td>
			<td <?php if ($menu_components != 'manager' && $menu_components != 'topmanager') {
				print 'style=display:none'; ?>
				<?php } ?>>
				<select id="cashback" <?php if ($menu_components != 'manager' && $menu_components != 'topmanager') {
					print 'style=display:none'; ?> <?php } ?>
					onchange="window.location = 'index.php?components=<?php print $menu_components; ?>&action=daily_sale&store='+document.getElementById('store0').value+'&group='+document.getElementById('group0').value+'&salesman='+document.getElementById('salesman0').value+'&processby='+document.getElementById('processby0').value+'&lock='+document.getElementById('lock').value+'&type='+document.getElementById('type').value+'&cashback='+document.getElementById('cashback').value+'&date1=<?php print $date1; ?>&date2=<?php print $date2; ?>'">
		...
		<tr>
			...
			<td>
				<select id="salesman0" <?php if (isset($userdisable))
					print $userdisable; ?>
					onchange="window.location = 'index.php?components=<?php print $menu_components; ?>&action=daily_sale&store='+document.getElementById('store0').value+'&group='+document.getElementById('group0').value+'&salesman='+document.getElementById('salesman0').value+'&processby='+document.getElementById('processby0').value+'&lock='+document.getElementById('lock').value+'&type='+document.getElementById('type').value+'&cashback='+document.getElementById('cashback').value+'&date1=<?php print $date1; ?>&date2=<?php print $date2; ?>'">
					<option value="all">--ALL--</option>
					<?php
...
<div id="print">
	<?php if ($type_req != 3 && $type_req != 5) { ?>
		<table align="center" bgcolor="#E5E5E5" height="100%" border="1" cellspacing="0" style="font-family:Calibri">
			<tr>
				...
				?>
				<td colspan="<?php print $colspan; ?>" style="border:0; background-color:black; color:white; font-weight:bold">
					Invoiced Collection
				</td>
			</tr>
			<tr>
				...
				<th width="120px">Total <?php if ($systemid == 17) {
					echo '(With Tax)';
				} ?></th>
				<?php if ($systemid == 17) { ?>
					<th width="130px">Total (Without Tax)</th>
			...
			for ($i = 0; $i < sizeof($invoice_no); $i++) {
				...
				if (isset($bi_discount[$i]) && $bi_discount[$i] > 0) {
				...
				if ($store0 != $billed_store[$i]) {
					...
					print '<tr>
								<td colspan="' . $colspan . '" style="padding-left:20px; font-weight:bold; background-color:#0066AA; color:white">' . $billed_store[$i] . '</td>
							</tr>';
				}
				...
				if ($systemid == 17) {
					print '<td align="right" class="padding">' . number_format($invoice_total_wo_tax[$i], $decimal) . '</td>';
					print '<td align="right" class="padding">' . number_format($invoice_tax[$i], $decimal) . '</td>';
				}
				print '<td align="right" class="padding">' . number_format($payment_cash[$invoice_no[$i]], $decimal) . '</td>
				...
				if (sizeof($invoice_no) == ($i + 1)) {
					...
					print '<td colspan="' . $colspan . '" align="right"  class="padding">Total</td>
								<td align="right" class="padding">' . number_format(array_sum($invoice_Total), $decimal) . '</td>';
					if ($systemid == 17) {
						print '<td align="right" class="padding">' . number_format(array_sum($invoice_total_wo_tax), $decimal) . '</td>
								<td align="right" class="padding">' . number_format(array_sum($invoice_tax), $decimal) . '</td>';
					}
					print '<td align="right" class="padding">' . number_format($total_cash, $decimal) . '</td>
	...
	<?php if ($type_req != 3 && $type_req != 5) { ?>
		<table align="center" bgcolor="#E5E5E5" height="100%" border="1" cellspacing="0" style="font-family:Calibri">
			...
			for ($i = 0; $i < sizeof($payment_id); $i++) {
				...
				print '<td align="right" class="padding"><a ' . (isset($payment_details[$i]) ? $payment_details[$i] : '') . ' >' . $pay_cash . '</a></td>';
				print '<td align="right" class="padding"><a ' . (isset($payment_details[$i]) ? $payment_details[$i] : '') . ' >' . $pay_card . '</a></td>';
				print '<td align="right" class="padding"><a ' . (isset($payment_details[$i]) ? $payment_details[$i] : '') . ' >' . $pay_bank . '</a></td>';
				print '<td align="right" class="padding"><a ' . (isset($payment_details[$i]) ? $payment_details[$i] : '') . ' >' . $pay_cheque . '</a></td>';
				print '<td align="right" class="padding"><a href="" style="text-decoration:none;">' . $payment_cheque_no[$i] . '</a></td>
	...
	<?php if ($type_req != 3 && $type_req != 5) { ?>
		<table align="center" bgcolor="#E5E5E5" height="100%" border="1" cellspacing="0" style="font-family:Calibri">
			<tr>
				<td colspan="7" style="border:0; background-color:black; color:white; font-weight:bold">
					Salesman Payment Collection Summary
				</td>
			</tr>
			<tr>
				<th width="60px">#</th>
			...
			$rownum = 1;
			foreach ($salesman_summary as $salesman => $data) {
				$grand_total['Cash'] += $data['Cash'];
				$grand_total['Card'] += $data['Card'];
				$grand_total['Bank'] += $data['Bank'];
				$grand_total['Cheque'] += $data['Cheque'];
				$grand_total['Total'] += $data['Total'];
				?>
				<tr>
					<td class="padding" align="center"><?php echo sprintf('%02d', $rownum++); ?></td>
					<td class="padding"><?php echo htmlspecialchars($salesman); ?></td>
					<td class="padding" align="right"><?php echo $data['Cash'] == 0 ? '' : number_format($data['Cash'], $decimal); ?>
					</td>
					<td class="padding" align="right"><?php echo $data['Card'] == 0 ? '' : number_format($data['Card'], $decimal); ?>
					</td>
					<td class="padding" align="right"><?php echo $data['Bank'] == 0 ? '' : number_format($data['Bank'], $decimal); ?>
					</td>
					<td class="padding" align="right">
						<?php echo $data['Cheque'] == 0 ? '' : number_format($data['Cheque'], $decimal); ?>
					</td>
					<td class="padding" align="right">
						<?php echo $data['Total'] == 0 ? '' : number_format($data['Total'], $decimal); ?>
					</td>
				</tr>
				<?php
			}
			?>
			<?php if (!empty($salesman_summary)) { ?>
				<tr style="font-weight:bold; background-color:gray; color:white">
					<td class="padding" align="right" colspan="2">Total</td>
					<td class="padding" align="right">
						<?php echo $grand_total['Cash'] == 0 ? '' : number_format($grand_total['Cash'], $decimal); ?>
					</td>
					<td class="padding" align="right">
						<?php echo $grand_total['Card'] == 0 ? '' : number_format($grand_total['Card'], $decimal); ?>
					</td>
					<td class="padding" align="right">
						<?php echo $grand_total['Bank'] == 0 ? '' : number_format($grand_total['Bank'], $decimal); ?>
					</td>
					<td class="padding" align="right">
						<?php echo $grand_total['Cheque'] == 0 ? '' : number_format($grand_total['Cheque'], $decimal); ?>
					</td>
					<td class="padding" align="right">
						<?php echo $grand_total['Total'] == 0 ? '' : number_format($grand_total['Total'], $decimal); ?>
					</td>
				</tr>
			<?php } ?>
		</table>
