...
<table align="center">
	<tr>
		<td valign="top">
			<?php
			if ($_GET['action'] == 'editcust') {
				if($systemid == 13){
					include_once  'components/manager/view/tpl/edit_cust_13.php';
				}else{
					include_once  'components/manager/view/tpl/edit_cust.php';
				}
			}
			else {
				if($systemid == 13){
					include_once  'components/manager/view/tpl/add_cust_13.php';
				}else{
			...
			<?php
				function getCustomerDocumentPath($systemid, $img_path, $index) {
					$base = 'images/customerdata/' . $systemid . '/' . str_pad($img_path, 8, "0", STR_PAD_LEFT) . '_' . $index;

					if (file_exists($base . '.jpg')) return $base . '.jpg';
					if (file_exists($base . '.jpeg')) return $base . '.jpeg';
					if (file_exists($base . '.pdf')) return $base . '.pdf';
			...
			if (($_GET['action'] == 'editcust') || ($_GET['action'] == 'searchcust')) {
				...
				if ($cu_image1 == 1) {
					$doc1 = getCustomerDocumentPath($systemid, $img_path, 1);
					if ($doc1 != '') {
						if (strtolower(pathinfo($doc1, PATHINFO_EXTENSION)) == 'pdf') {
							print '<div id="imageframe1" style="display:none"><iframe src="' . $doc1 . '?t=' . time() . '" style="width:450px; height:500px;"></iframe></div>';
						} else {
							print '<div id="imageframe1" style="display:none"><img src="' . $doc1 . '?t=' . time() . '" style="width:450px" /></div>';
						}
					}
				}

				if ($cu_image2 == 1) {
					$doc2 = getCustomerDocumentPath($systemid, $img_path, 2);
					if ($doc2 != '') {
						if (strtolower(pathinfo($doc2, PATHINFO_EXTENSION)) == 'pdf') {
							print '<div id="imageframe2" style="display:none"><iframe src="' . $doc2 . '?t=' . time() . '" style="width:450px; height:500px;"></iframe></div>';
						} else {
							print '<div id="imageframe2" style="display:none"><img src="' . $doc2 . '?t=' . time() . '" style="width:450px" /></div>';
						}
					}
				}

				if ($cu_image3 == 1) {
					$doc3 = getCustomerDocumentPath($systemid, $img_path, 3);
					if ($doc3 != '') {
						if (strtolower(pathinfo($doc3, PATHINFO_EXTENSION)) == 'pdf') {
							print '<div id="imageframe3" style="display:none"><iframe src="' . $doc3 . '?t=' . time() . '" style="width:450px; height:500px;"></iframe></div>';
						} else {
							print '<div id="imageframe3" style="display:none"><img src="' . $doc3 . '?t=' . time() . '" style="width:450px" /></div>';
						}
					}
				}

				if ($cu_image4 == 1) {
					$doc4 = getCustomerDocumentPath($systemid, $img_path, 4);
					if ($doc4 != '') {
						if (strtolower(pathinfo($doc4, PATHINFO_EXTENSION)) == 'pdf') {
							print '<div id="imageframe4" style="display:none"><iframe src="' . $doc4 . '?t=' . time() . '" style="width:450px; height:500px;"></iframe></div>';
						} else {
							print '<div id="imageframe4" style="display:none"><img src="' . $doc4 . '?t=' . time() . '" style="width:450px" /></div>';
						}
					}
				}
			}
			...
			if (($_GET['action'] == 'editcust') || ($_GET['action'] == 'searchcust')) {
				...
				if ($cu_image1 == 1) print '<div id="imageframe1" style="display:none"><img src="images/customerdata/' . $systemid . '/' . str_pad($img_path, 8, "0", STR_PAD_LEFT) . '_1.jpg?t=' . time() . '" style="width:450px" /></div>';
				if ($cu_image2 == 1) print '<div id="imageframe2" style="display:none"><img src="images/customerdata/' . $systemid . '/' . str_pad($img_path, 8, "0", STR_PAD_LEFT) . '_2.jpg?t=' . time() . '" style="width:450px" /></div>';
				if ($cu_image3 == 1) print '<div id="imageframe3" style="display:none"><img src="images/customerdata/' . $systemid . '/' . str_pad($img_path, 8, "0", STR_PAD_LEFT) . '_3.jpg?t=' . time() . '" style="width:450px" /></div>';
				if ($cu_image4 == 1) print '<div id="imageframe4" style="display:none"><img src="images/customerdata/' . $systemid . '/' . str_pad($img_path, 8, "0", STR_PAD_LEFT) . '_4.jpg?t=' . time() . '" style="width:450px" /></div>';
			}
			?> -->
			<br />
			<div style="background-color:#EEEEEF; border-radius: 15px; padding-left:10px; padding-right:10px"><br />
		...
		<td valign="top">
							...
							<form method="post" action="index.php?components=<?php print $_GET['components']; ?>&action=searchcust&show_map=no">
								<table align="center">
									<tr>
										<td>
											<div class="frmSearch">
												<input type="text" name="namesearch" id="search-cust" placeholder="<?php if($systemid == 13)  echo "Customer"; else echo "Shop"; ?> Name" style="width:150px" /> OR
												<div id="suggesstion-cust"></div>
											</div>
										</td>
										<td>
											<div class="frmSearch">
												<input type="text" name="nicknamesearch" id="search-nick" placeholder="Nickname" style="width:130px" />
												<div id="suggesstion-nick"></div>
											</div>
										</td>
										<td rowspan="2">
											<div id="div_search_btn"><input type="submit" value="Search" style="height:60px" /></div>
										</td>
									</tr>
									<tr>
										<td>
											<div class="frmSearch">
												<input type="text" name="mobsearch" id="search-mob" placeholder="Mobile Number" style="width:150px" /> OR
												<div id="suggesstion-mob"></div>
											</div>
										</td>
										<td>
											<div class="frmSearch">
												<input type="text" name="nicsearch" id="search-nic" placeholder="NIC" style="width:130px" />
			...
			<table align="center" style="font-size:10pt;">
				<tr style="font-size:12pt; background-color:#CCCCCC">
					<th>ID</th>
					<th><?php if($systemid == 13)  echo "Customer"; else echo "Shop"; ?> Name</th><?php if ($components != 'topmanager') print '<th>NIC</th>'; ?><th>Mobile</th>
				...
				for ($i = 0; $i < sizeof($cu_id); $i++) {
					if (($cu_status[$i] == 1) || ($cu_status[$i] == 2) || ($cu_status[$i] == 3)) {
						...
						if ($custstatus != $cu_status[$i])
							print '<tr style="background-color:#467898; color:white;"><td></td><td colspan="4" class="shipmentTB3">' . $stname . '</td></tr>';
						print '<tr style="background-color:#F1F1F1"><td ' . $color . ' class="shipmentTB3" align="right">' . $cu_id[$i] . '</td><td ' . $color . ' class="shipmentTB3"><a ' . $color . ' href="index.php?components=' . $_GET['components'] . '&action=editcust&id=' . $cu_id[$i] . '&show_map=no">' . $cu_name[$i] . '</a></td>' . $nic_td . '<td ' . $color . ' class="shipmentTB3">' . $cu_mobile[$i] . '</td><td ' . $color . ' class="shipmentTB3" align="right">' . number_format($cu_cr_limit[$i]) . '</td>' . $sys_td . '</tr>';
				...
				print '<tr style="background-color:#467898; color:white;"><td>' . $dis_button . '</td><td colspan="4" class="shipmentTB3">' . $stname . '</td></tr>';
				for ($i = 0; $i < sizeof($cu_id); $i++) {
					if ($cu_status[$i] == 0) {
						if ($components == 'topmanager') {
							$nic_td = '';
