...
?>
	...
	function changeSelection(){
		...
		if(selection=='customer'){
			document.getElementById('div_sel').innerHTML='<input type="text" name="customer" id="tags1" />';
			$( "#tags1" ).autocomplete({ source: availableTags1	});
		}
		if(selection=='store'){
			document.getElementById('div_sel').innerHTML='<input type="text" name="store1" id="tags2" />';
			$( "#tags2" ).autocomplete({ source: availableTags2	});
		}
	}
...
<form action="index.php" method="get" onsubmit="return validateDateRange()" >
	...
	<tr><td colspan="2" align="right">
			...
			<?php } ?>
			</td><td align="right"><div id="div_sel">
			<?php if($selection=='store')
	...
	<td width="20px"></td>
	<td width="70px" align="right"><strong>Category : </strong></td><td>
...
</td><td width="80px"></td><td valign="top" align="center">
	<table align="center" bgcolor="#E5E5E5" height="100%" border="1" cellspacing="0" style="font-size:10pt;">
	<tr><td colspan="11" style="border:0; background-color:black; color:white; font-weight:bold"></td></tr>
	<tr bgcolor="#CCCCCC"><th width="200px">Item Category</th><th width="100px">Sold/Total Items<br>In the category</th></tr>
	<?php
	if($selection=='store') $cus_st='store1='.$store1; else $cus_st='customer='.$customer.'&customer_id='.$customer_id;
	if(isset($_REQUEST['category'])){
		for($i=0;$i<sizeof($cat2_id);$i++){
			if($cat2_sold_total[$i]!=0){ $url='href="index.php?components='.$_GET['components'].'&action=sales_report3&selection='.$selection.'&category='.$cat2_id[$i].'&'.$cus_st.'&datefrom='.$fromdate.'&dateto='.$todate.'"'; }else{ $url=''; }
			print '<tr><td style="padding-left:30px"><a style="text-decoration:none" '.$url.'>'.$cat2_name[$i].'</a></td><td align="right" style="padding-right:30px">'.$cat2_sold_total[$i].' / '.$cat2_total_count[$i].'</td></tr>';
		}
	}
	?>
	</table>
