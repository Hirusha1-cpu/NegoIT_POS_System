...
<div style="background-color:#EEEEEE; border-radius:10px">
	...
	<tr><td width="50px"></td><td colspan="4">
	...
	<td align="right"><strong>Process By : </strong></td><td>
	...
	<td align="right"><strong>Cash Back :</strong></td><td>
	...
	</td><td width="80px" align="right"><strong>Group : </strong></td><td>
	...
	</td><td width="100px" align="right"><strong>Salesman : </strong></td><td>
	...
	</td><td width="100px" align="right"><strong>Bill Status : </strong></td><td align="right">
	...
	</td><td width="100px" align="right"><strong>Type : </strong></td><td align="right">
...
<div id="printheader" style="display:none" >
	...
	<table style="font-size:12pt" border="1" cellspacing="0">
		<tr><td style="background-color:#C0C0C0; padding-left:10px">Date</td><td style="background-color:#EEEEEE; padding-left:10px"><?php if($date2!='') print 'From '; print $date1; if($date2!='') print '<br />To '.$date2; ?></td></tr>
...
<?php if($type_req!=3 && $type_req!=5){ ?>
	...
	<tr><td colspan="10" style="border:0; background-color:black; color:white; font-weight:bold">Invoiced Collection</td></tr>
	<tr><th width="100px">Invoice No</th><th width="120px">Date/Time</th><th width="100px">Invoice Total</th><th width="100px">Cash</th><th width="100px">Card</th><th width="100px">Bank</th><th width="100px">Cheque</th><th width="100px">Credit</th><th width="100px">Salesman</th><th width="150px">Customer</th></tr>
<?php
$inv=0;
$store0='';
$total_cash=$total_bank=$total_chque=$total_credit=$total_card=0;
	...
	for($i=0;$i<sizeof($invoice_no);$i++){
		...
		if($store0!=$billed_store[$i]){
			print '<tr><td colspan="10" style="padding-left:20px; font-weight:bold; background-color:#0066AA; color:white">'.$billed_store[$i].'</td></tr>';
		}
			...
			print '<tr '.$color.'>
					<td align="center"><a '. $title.' href="index.php?components=billing&action=finish_bill&id='.$invoice_no[$i].'">'.str_pad($invoice_no[$i], 7, "0", STR_PAD_LEFT).'</a></td>
					...
					<td align="right" style="padding-right:10px;"><a href="#" style="text-decoration:none" title="'.$payment_card_tr[$invoice_no[$i]].'">'.number_format($payment_card[$invoice_no[$i]],$decimal).'</a></td>
					<td align="right" style="padding-right:10px;"><a href="#" style="text-decoration:none" title="'.$payment_bank_tr[$invoice_no[$i]].'">'.number_format($payment_bank[$invoice_no[$i]],$decimal).'</a></td>
					<td align="right" style="padding-right:10px;"><a href="#" style="text-decoration:none" title="'.$chq_details[$invoice_no[$i]].'">'.number_format($payment_chque[$invoice_no[$i]],$decimal).'</a></td>
...
<?php if($type_req!=3 && $type_req!=5){ ?>
	<table align="center" bgcolor="#E5E5E5" height="100%" border="1" cellspacing="0" style="font-family:Calibri">
	<tr><td colspan="8" style="border:0; background-color:black; color:white; font-weight:bold">Payment Collection</td></tr>
	<tr><th width="100px">Payment No</th><th width="120px">Date/Time</th><th width="100px">Cash</th><th width="100px">Card</th><th width="100px">Bank</th><th width="100px">Cheque</th><th width="100px">Salesman</th><th width="100px">Customer</th></tr>
<?php
...
$store0='';
	for($i=0;$i<sizeof($payment_id);$i++){
		...
		if($payment_type[$i]=='Cash'){ $pay_cash=number_format($payment_amount[$i],$decimal); $pay_total_cash+=$payment_amount[$i]; }
		if($payment_type[$i]=='Card'){ $pay_card=number_format($payment_amount[$i],$decimal); $pay_total_card+=$payment_amount[$i]; }
		if($payment_type[$i]=='Bank'){ $pay_bank=number_format($payment_amount[$i],$decimal); $pay_total_bank+=$payment_amount[$i]; }
		if($payment_type[$i]=='Chque'){ $pay_cheque=number_format($payment_amount[$i],$decimal); $pay_total_cheque+=$payment_amount[$i]; }
		
		if($store0!=$payment_store[$i]){
			print '<tr><td colspan="8" style="padding-left:20px; font-weight:bold; background-color:#0066AA; color:white">'.$payment_store[$i].'</td></tr>';
		}
			print '<tr><td align="center"><a href="index.php?components=billing&action=finish_payment&id='.$payment_id[$i].'">'.str_pad($payment_id[$i], 7, "0", STR_PAD_LEFT).'</a></td><td align="center" width="50px">'.$payment_time[$i].'</td>
			<td align="center"><a '.$payment_details[$i].' >'.$pay_cash.'</a></td>
			<td align="center"><a '.$payment_details[$i].' >'.$pay_card.'</a></td>
			<td align="center"><a '.$payment_details[$i].' >'.$pay_bank.'</a></td>
			<td align="center"><a '.$payment_details[$i].' >'.$pay_cheque.'</a></td>
		...
		if(sizeof($payment_id)==($i+1)){
			print '<tr style="font-weight:bold; background-color:gray; color:white"><td colspan="2" align="right"  style="padding-right:10px;" >Total</td>
			<td align="right" style="padding-right:10px;">'.number_format($pay_total_cash,$decimal).'</td>
			<td align="right" style="padding-right:10px;">'.number_format($pay_total_card,$decimal).'</td>
			<td align="right" style="padding-right:10px;">'.number_format($pay_total_bank,$decimal).'</td>
			<td align="right" style="padding-right:10px;">'.number_format($pay_total_cheque,$decimal).'</td>
			<td colspan="2"></td></tr>';
			print '<tr style="font-weight:bold; background-color:#BBBBBB; color:white"><td colspan="7" align="right"  style="padding-right:10px;" >Total Payments</td><td align="right" style="padding-right:10px;">'.number_format(($pay_total_cash+$pay_total_card+$pay_total_bank+$pay_total_cheque),$decimal).'</td>';
			print '</table><br /><table align="center" bgcolor="#E5E5E5" height="100%" border="1" cellspacing="0">';
		}
...
<?php if($type_req==3 || $type_req==''){ ?>
	...
	<tr><th width="100px">Return No</th><th width="100px" >Time</th><th width="150px" >Extra Pay</th><th width="100px">Salesman</th><th width="100px">Customer</th></tr>
<?php
$inv=$total_payment=0;
$store0='';
...
<?php if($type_req==5 || $type_req==''){ ?>
	...
	<tr><th width="100px">Job No</th><th width="200px" >Time</th><th width="150px" >Extra Pay</th><th width="100px">Salesman</th><th width="200px">Entity</th></tr>
<?php 
$inv=$total_payment=0;
$store0='';
