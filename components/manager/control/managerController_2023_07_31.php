...
if ($sub_system != 0 && $_REQUEST['action'] == 'shipment') header('Location: index.php?components=manager&action=daily_sale&store=all&group=all&salesman=all&lock=1&cashback=no&type=');
...
switch ($_REQUEST['action']) {
   ...
   case "quotation":
      ...
      if (isset($_COOKIE['district'])) {
         ...
         getCust(1, '1');
         if (isset($_GET['cust'])) {
            if (validateQuotNo()) header('Location: index.php?components=manager&action=new_quot&cust_id=' . $_GET['cust'] . '&validity=30');
         }
      }
