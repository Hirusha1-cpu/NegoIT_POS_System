...
function getOneCust($mode, $sub_system){
	...
	if (isset($_REQUEST['id'])) {
		...
		// Access the values
		if((isset($dataArray['designation'])) && ($dataArray['designation'] != '')) $designation = $dataArray['designation'];
		if((isset($dataArray['delivery_details_same'])) && ($dataArray['delivery_details_same'] != '')) $delivery_details_same = $dataArray['delivery_details_same'];
		if((isset($dataArray['goods_delivery_mobile'])) && ($dataArray['goods_delivery_mobile'] != '')) $goods_delivery_mobile = $dataArray['goods_delivery_mobile'];
		if((isset($dataArray['goods_delivery_address'])) && ($dataArray['goods_delivery_address'] != '')) $goods_delivery_address = $dataArray['goods_delivery_address'];
		if((isset($dataArray['goods_delivery_contact_person'])) && ($dataArray['goods_delivery_contact_person'] != '')) $goods_delivery_contact_person = $dataArray['goods_delivery_contact_person'];
	}
...
function addCust($systemid){
	...
	*/
	...
	if ($id_count > 0) {
		$out = false;
		if($systemid == 13) $message = 'Error: Customer Name already exist in the system!';
		else $message = 'Error: Name (Shop Name) already exist in the system!';
	...
	if(!validateMobileNo($mobile)){
		$out = false;
		if($systemid == 13) $message = 'Invalid contact number!';
	...
	if ($out) {
		...
		if ($result) {
			...
			if(isQuickBooksActive(1)){
				...
				try{
					$qb_result = QBCustomerAdd($cust_id); // Add new customer
				...
				if($qb_result['status'] == 'success'){
					...
					if(!$result1){
						$qb_msg = 'Quickbooks cust id update error.';
					}
				}
			}
			$out = true;
		...
		if ($file_upload == 0) {
			...
			if ($result) {
				...
				if(isQuickBooksActive(1)){
					...
					if($qb_cust_id){ // updated QB customer
					...
					}else{  // If customer not registered in QuickBooks
						try{
							$qb_result = QBCustomerAdd($cust_id);
							$qb_msg = $qb_result['message'];
						}catch (Exception $e) {
							$qb_msg = "<br>QuickBooks error: " . $e->getMessage();
							$qb_result['status'] = 'error';
						...
						if($qb_result['status'] == 'success'){
							$qb_cust_id = $qb_result['qb_cust_id'];
							$query1 = "UPDATE `cust` SET `qb_cust_id`='$qb_cust_id' WHERE `id`='$cust_id'";
							$result1 = mysqli_query($conn, $query1);
							if(!$result1){
								$qb_msg = 'Quickbooks customer id update error.';
							}
						}
					}
				}
...
function dailySale($store, $sub_system){
	...
	$payment_bank = $cu_list_arr = $invoice_no = $payment_id = $rtn_no = $wa_no = $item_type = $payment_cheque_date = $payment_cheque_no = array();
	$systemid = inf_systemid(1);
	...
	while ($row2 = mysqli_fetch_array($result2)) {
		...
		} else $payment_id = $payment_amount = $payment_type = $payment_salesman = $payment_cust = $payment_time = $payment_store = $payment_details = $payment_cheque_no = $payment_cheque_date = array();
	}
...
function getCustSale($systemid){
	global $customer, $decimal, $fromdate, $todate, $invoice_no, $invoice_Total, $billed_district, $billed_by, $billed_date, $billed_time, $billed_store, $payment_cash, $payment_card, $payment_chque, $payment_id, $payment_amount, $payment_type, $payment_salesman, $payment_date, $payment_time, $payment_store, $credit_balance, $payment_chq_return, $chqpayment_id, $chqpayment_amount, $chqpayment_salesman, $chqpayment_cust, $chqrtn_date, $chqrtn_time, $chqpayment_store, $chqpayment_invno, $chq_details, $payment_card_details, $payment_chq_details, $chqrtn_chq_details, $payment_bank, $payment_card_tr, $payment_bank_tr, $bank_trans, $payment_invoice, $payment_chq_postpone, $billed_chq_return, $billed_chq_postpone, $cust_mtype;
...
function setChqueStatus($status){
	...
	if($out){
		if(isQuickBooksActive(1)){
			if ($status == 1) {
				$query3="SELECT py.qb_payment_id FROM payment py WHERE py.id = '$id'";
				$row3=mysqli_fetch_row(mysqli_query($conn2,$query3));
				if(!empty($row3)){
					$qb_payment_id=$row3[0];
					if($qb_payment_id != ''){
						try{
							$qb_result = QBPaymentDelete($qb_payment_id);
							$qb_msg = $qb_result['message'];
							if($qb_result['status'] == 'success'){
								$query4="UPDATE `payment` SET `qb_status`=0 WHERE `qb_payment_id`='$qb_payment_id'";
								$result4=mysqli_query($conn,$query4);
								if(!$result4){
									$qb_msg = 'Quickbooks payment id status update error.';
								}
							}
						}catch (Exception $e) {
							$qb_msg = "<br>QuickBooks error: " . $e->getMessage();
...
function getTemporaryBills($sub_system){
	...
	WHERE bi.`bm_no`=bm.`bm_no` AND bm.`billed_by`=up.`id` AND bm.`cust`=cu.`id` AND bm.`store`=st.`id` AND bm.`status` NOT IN (0,7) $sub_system_qry GROUP BY bm.`bm_no` ORDER BY st.`name`, bi.`date` DESC";
...
function addShipPayment($case){
	...
	if(($out) && ($systemid==13)){ // restrict duplicate of ref no
...
function clearChque(){
	...
	if($out){
		if(isQuickBooksActive(1)){
			$query="SELECT py.invoice_no, cu.qb_cust_id, bm.qb_txnid, py.amount, py.cust, py.chque_no, py.chque_branch, bk.bank_code FROM payment py  LEFT JOIN cust cu ON py.cust = cu.id  LEFT JOIN bill_main bm ON bm.invoice_no = py.invoice_no LEFT JOIN bank bk ON py.chque_bank = bk.id WHERE py.id = '$id'";
			$row=mysqli_fetch_row(mysqli_query($conn2,$query));
			if(!empty($row)){
				$invoice_no=$row[0];
				$qb_cust_id=$row[1];
				...
				$amount=$row[3];
				$cust=$row[4];
				$cheque_no = $row[5] . '-' . $row[7] . '-' . str_pad($row[6], 3, "0", STR_PAD_LEFT);
				if($qb_cust_id != ''){ // only quickbooks registered users
					$out2 = false;
					$query3 = "SELECT `qb_id` FROM qb_payment_methods WHERE `name` = 'Check'";
					$result3 = mysqli_query($conn2, $query3);
					if ($result3 && mysqli_num_rows($result3) > 0) {
						$row3 = mysqli_fetch_assoc($result3);
						$qb_payment_method_id = $row3['qb_id'];

						$query4="SELECT `qb_account_id` FROM accounts WHERE id='$ac_id'";
						$result4 = mysqli_query($conn2, $query4);
						if (($result4) && (mysqli_num_rows($result4) > 0)) {
							$row4 = mysqli_fetch_row($result4);
							$qb_account_id = $row4[0];
							if($qb_account_id != ''){
								if((($invoice_no != '') || ($invoice_no != 0)) && ($qb_txnid != '')){ // cheque payment for invoice number
									$payment_array = array('qb_cust_id' => $qb_cust_id, 'qb_invoice_id'=> $qb_txnid, 'amount'=> $amount, 'total_amount' => $amount, 'qb_account_id'=> $qb_account_id,'qb_payment_method_id'=>$qb_payment_method_id, 'qb_payment_ref_number' => $cheque_no);
									$out2 = true;
								}else{ // cheque payment for customer (without invoice number))
									$query1="SELECT COUNT(bm.qb_txnid>0) FROM bill_main bm WHERE bm.cust = '$cust'";
									$row1=mysqli_fetch_row(mysqli_query($conn2,$query1));
									$bill_count = $row1[0];
									if($bill_count > 0){ // save data after at least one bill saved in quickbooks
										$payment_array = array('qb_cust_id' => $qb_cust_id, 'amount'=> $amount, 'total_amount' => $amount, 'qb_account_id'=> $qb_account_id,'qb_payment_method_id'=>$qb_payment_method_id, 'qb_payment_ref_number' => $cheque_no);
										$out2 = true;
									}
								}
								if($out2){
									try{
										$qb_cheque_payment_result = QBAddPayment($payment_array);
										$qb_msg = $qb_cheque_payment_result['message'];
										$qb_payment_id2 = $qb_cheque_payment_result['qb_payment_id'];
									}catch (Exception $e) {
										$qb_msg = "<br>QuickBooks error: " . $e->getMessage();
										$qb_cheque_payment_result['status'] = 'error';
									}
									if($qb_cheque_payment_result['status'] == 'success'){
										$qb_payment_id2 = $qb_cheque_payment_result['qb_payment_id'];
										$query6 = "UPDATE `payment` SET `qb_payment_id`='$qb_payment_id2', `qb_status`=1 WHERE id='$id'";
										$result6 = mysqli_query($conn, $query6);
										if(!$result6){
											$qb_msg = 'Quickbook payment id update error.';
										}
									}
								}
							}
