...
function addShipPayment($case){
		...
		if ((!$result1) || (!$result2)) {
			...
			if(isQuickBooksActive(1)){
				...
				$journal_entries_array[] = [
					...
					'amount' => $amount,
					'posting_type' => $cr_dr2,
					'account_id' => $qb_to_account_id,
					'account_name' => $qb_to_account_name,
					'account_type' => 'to'
				];
			}
		}
	...
	if($out){
		if (count($journal_entries_array) > 0) {
			try{
				$qb_result = QBAddJournalEntry($journal_entries_array);
				$qb_msg = $qb_result['message'];
			}catch (Exception $e) {
				$qb_msg = "<br>QuickBooks error: " . $e->getMessage();
				$qb_result['status'] = 'error';
...
function deleteShipPayment(){
	...
	$id = $_GET['id'];
	$pass = $_GET['pass'];
	$journal_id = $journal_entry_array = $qb_msg = '';
	$out = true;
	...
	if($out && isQuickBooksActive(1)){
		$result = mysqli_query($conn, "SELECT `qb_id` FROM journal_main WHERE journal_id='$journal_id'");
		$row = mysqli_fetch_assoc($result);
		$qb_id = $row['qb_id'];
