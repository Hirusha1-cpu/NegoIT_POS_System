...
function dailySale($store, $sub_system)
{
	...
	// --- Fetch Payment Details for bill_pay = 2 ---
	$query2 = "SELECT py.id, py.amount, py.payment_type, up.username, cu.name AS cust_name, py.payment_date, st.name AS store_name, py.chque_no, bk.bank_code,
	...
	LEFT JOIN bank bk ON py.chque_bank = bk.id
	LEFT JOIN accounts ac ON py.bank_trans = ac.id
	WHERE py.salesman=up.id AND py.cust=cu.id AND py.store=st.id AND cu.associated_group=gp.id AND py.bill_pay=2 AND py.`status`=0
	$sub_sys_pyqry $date_qry2 $pycashback_qry $storesearch $groupsearch $salesmansearch
	ORDER BY st.name, py.payment_date";
	$result2 = mysqli_query($conn2, $query2);
	while ($row2 = mysqli_fetch_array($result2, MYSQLI_ASSOC)) {
		if ($row2['id'] > 0) {
			...
			$payment_time[] = substr($row2['payment_date'], 0, 16);
