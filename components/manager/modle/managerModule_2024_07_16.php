...
function dailySale($store, $sub_system){
	...
	if ($type_req != 3 && $type_req != 5) {
		...
		$query = "SELECT
		bm.invoice_no,round(SUM(bi.qty*bi.unit_price),2),di.name,up.username,cu.name,bm.billed_timestamp,st.name,SUM(bi.discount),bm.`cust`,up.id,bm.gps_x,bm.gps_y
...
function getSalesReport2($sub_system){
	global $item_des, $item_qty, $selection, $customer, $customer_id, $store1, $fromdate, $todate, $category, $cat2_id, $cat2_name, $cat2_total_count, $cat2_sold_total, $salesman;
	$k = 0;
	$case = false;
	$cust_qry = $store_qry = $salesman_qry = '';
	...
	if ($case) {
		...
		if ($selection == 'customer'){
			$query = "SELECT itm.description,SUM(bi.qty) FROM bill_main bm, bill bi, inventory_items itm WHERE bm.invoice_no=bi.invoice_no AND bi.item=itm.id AND bm.`status` NOT IN (0,7) AND bm.`lock`=1 AND itm.description!='CHQUE' AND itm.description!='CREDIT' $cust_qry AND bm.`status` NOT IN (0,7) AND bm.exclude=0 AND date(bi.`date`) BETWEEN '$fromdate' AND '$todate' $cat_qry $sub_system_qry $salesman_qry GROUP BY bi.item ORDER BY itm.description";
		}
		if ($selection == 'store'){
			$query = "SELECT itm.description,SUM(bi.qty) FROM bill_main bm, bill bi, inventory_items itm, stores st WHERE bm.invoice_no=bi.invoice_no AND bi.item=itm.id AND bm.`store`=st.id AND bm.`status` NOT IN (0,7) AND bm.`lock`=1 AND itm.description!='CHQUE' AND itm.description!='CREDIT' $store_qry AND bm.`status` NOT IN (0,7) AND bm.exclude=0 AND date(bi.`date`) BETWEEN '$fromdate' AND '$todate' $cat_qry $sub_system_qry $salesman_qry GROUP BY bi.item ORDER BY itm.description";
		}
		$result = mysqli_query($conn2, $query);
		while ($row = mysqli_fetch_array($result)) {
			$item_des[] = $row[0];
