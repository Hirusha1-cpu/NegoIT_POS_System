...
{
	global $cu_id, $cu_id0, $cu_name, $cu_name0, $cu_nic, $cu_mobile, $cu_cr_limit, $cu_status, $cu_sub_sys, $conn2;
	$sub_sys_qry = '';
	...
	$cu_id = $cu_id0 = $cu_name = $cu_nic = $cu_mobile = $cu_cr_limit = $cu_status = $cu_sub_sys = array();
	include('config.php');
...
{
	global $cust_id, $cust_name, $cust_nic, $cust_mobile, $cust_asso_sman,$conn2;
	include('config.php');
	$cust_id = $cust_name = $cust_nic = $cust_mobile = $cust_asso_sman = [];
...
function getTown()
{
	global $town_id, $town_name, $town_default,$conn2;
	$town_id = $town_name = array();
	$town_default = '';
	include('config.php');
...
function getDistrictMGR($sub_system)
{
	global $dist_id_list, $dist_name_list;
	include('config.php');

	$dist_id_list = array();
	$dist_name_list = array();
	$q_d = "SELECT id, name FROM district ORDER BY name";
	$r_d = mysqli_query($conn2, $q_d);
	while ($rw = mysqli_fetch_array($r_d)) {
		$dist_id_list[] = $rw[0];
		$dist_name_list[] = $rw[1];
	}
}
...
{
	global $date, $store, $category, $itm_category, $itm_code, $itm_description, $sold_qty, $itm_store, $conn2;
	$category_qry = $store_qry = $user_query = '';
	$sub_system_qry = '';
	$itm_code = $itm_category = $itm_description = $sold_qty = $itm_store = array();
	include('config.php');
...
{
	global $map_api, $cu_id1, $cu_name1, $cu_dob1, $cu_nic1, $cu_mobile1, $cu_crlimit1, $cu_custname1, $cu_nickname1, $cu_shop_add1, $cu_shop_tel1,
		$cu_home_add1, $cu_home_tel1, $cu_email_add, $cu_email_alert, $cu_sms, $cu_status1, $cu_store, $cu_sa, $cu_group, $cu_town, $cu_image1,
		$cu_image2, $cu_image3, $cu_image4, $cu_cre_by, $cu_cre_time, $cu_app_by, $cu_app_time, $cu_sub_system, $refer_cust, $cu_gps_x, $cu_gps_y,
		$cu_master_cust, $forign_image, $cu_tax_no, $designation, $delivery_details_same, $goods_delivery_mobile, $goods_delivery_address, $goods_delivery_contact_person, $conn2;
	$note = '';
	...
	if (isset($_REQUEST['id'])) {
		...
		$query = "SELECT `id`,`name`,`nic`,`mobile`,`credit_limit`,`cust_name`,`nickname`,`shop_address`,`shop_tel`,`home_address`,`home_tel`,`email`,`email_enable`,sms,`status`,
		associated_store,associated_salesman,associated_group,associated_town,image1,image2,image3,image4,created_by,created_timestamp,approved_by,
		approved_timestamp,sub_system,master_cust,gps_x,gps_y,dob,tax_no,note
		FROM cust WHERE $mode1 $sub_sys_qry ORDER BY sub_system DESC";
		$result = mysqli_query($conn2, $query);
		...
		//-----------------------------Check for Customer Cross Reference--------------------------------------//
		if ($cu_master_cust != 0) {
...
function imageresize($systemid, $image1)
{
	...
	$dest_image1 = 'images/customerdata/' . $systemid . '/' . $image1;
	$image1 = 'images/customerdata/' . $systemid . '/uploads/' . $image1;
	...
	if (isset($org_img)) imagedestroy($org_img);
...
function addCust($systemid)
{
	...
	$shop_name = ucwords(strtolower($shop_name));
	$shop_name = preg_replace("/[^A-Za-z0-9\-+,. ]/", '', $shop_name);
	$shop_name = trim(ucwords($shop_name));
	...
	$custname = preg_replace("/[^A-Za-z0-9\-+,. ]/", '', $custname);
	$custname = mysqli_real_escape_string($conn, $custname);

	$custname0 = $custname;
	$dob = trim(mysqli_real_escape_string($conn, $_POST['dob']));  // added by nirmal 29_04_2022
	$nickname0 = trim(mysqli_real_escape_string($conn, ucwords(strtolower($_POST['nickname']))));
	$nickname = trim(mysqli_real_escape_string($conn, preg_replace("/[^A-Za-z0-9-+,. ]/", '', $nickname0)));
	$shop_address = trim(mysqli_real_escape_string($conn, $_POST['shop_address']));
	$shop_address = trim(ucwords($shop_address));
	$shop_tel = trim($_POST['shop_tel']);
	$home_address = trim(mysqli_real_escape_string($conn, $_POST['home_address']));
	$home_address = trim(ucwords($home_address));
	...
	*/
	...
	try {
		...
		if ($out) {
			...
			if (!$cust_id) {
				$out = false;
				$msg = 'Error: Customer inserting error';
				$jasonArray["cust_error"] = $msg;
				throw new Exception($msg);
			}
			...
			if ($result) {
				...
				if (isQuickBooksActive(1)) {
					try {
						$result1 = mysqli_query($conn, "SELECT `name` FROM `town` WHERE `id`='$cu_town'");
						if (!$result1) {
							$out = false;
							throw new Exception('Error : Failed to retrieve customer town');
						}
...
// update by nirmal 29_07_2025 (added pdf upload feature)
function addCustImage($systemid)
{
	...
	$id_next = $cust_id;
	error_log("upload_max_filesize = " . ini_get('upload_max_filesize') . PHP_EOL, 3, "/Users/hirushafernando/Documents/NEGOIT/march_5/code/billing/logfile.log");
error_log("post_max_size = " . ini_get('post_max_size') . PHP_EOL, 3, "/Users/hirushafernando/Documents/NEGOIT/march_5/code/billing/logfile.log");
error_log("max_file_uploads = " . ini_get('max_file_uploads') . PHP_EOL, 3, "/Users/hirushafernando/Documents/NEGOIT/march_5/code/billing/logfile.log");
	error_log("addCustImage called with cust_id: $cust_id", 3, "/Users/hirushafernando/Documents/NEGOIT/march_5/code/billing/logfile.log");
	error_log("POST data: " . print_r($_POST, true), 3, "/Users/hirushafernando/Documents/NEGOIT/march_5/code/billing/logfile.log");
	error_log("FILES data: " . print_r($_FILES, true), 3, "/Users/hirushafernando/Documents/NEGOIT/march_5/code/billing/logfile.log");
	...
	for ($i = 1; $i <= 4; $i++) {
		if (isset($_FILES["fileToUpload" . $i])) {
			if ($_FILES["fileToUpload" . $i]["name"] != '') {
				...
				$file_extension = strtolower(pathinfo($_FILES["fileToUpload" . $i]["name"], PATHINFO_EXTENSION));
				// Generate the file name with the correct extension
				$tmp_name = str_pad($id_next . '_' . $i, 10, "0", STR_PAD_LEFT) . '.' . $file_extension;
				...
				if (isset($_POST["submit"])) {
					if ($imageFileType != "pdf") { // Skip this check for PDFs
				...
				$maxFileSize = 15000000; // File size limit in bytes (10 MB)
				...
				if ($imageFileType != "jpg" && $imageFileType != "jpeg" && $imageFileType != "pdf") {
					$msg = "Error: Sorry, only JPG, JPEG, and PDF files are allowed.";
					$uploadOk = 0;
				}
				...
				if ($uploadOk == 0) {
					...
					if (move_uploaded_file($_FILES["fileToUpload" . $i]["tmp_name"], $destination_file)) {
						if ($imageFileType != "pdf") { // Skip resizing for PDFs
							...
							// For PDFs, move the file to the base folder
							// The rename() function is used to move the file from the uploads folder to the base folder.
							$base_folder_file = 'images/customerdata/' . $systemid . '/' . $tmp_name;
							if (rename($destination_file, $base_folder_file)) {
								if ($i == 1)
									$image1 = 1;
								if ($i == 2)
									$image2 = 1;
								...
								$msg = "Error: Failed to move PDF to the base folder.";
								$file_upload++;
							}
						}
					} else {
	...
	if ($file_upload == 0) {
		if ($imagesNotChoosed == 0 && $file_upload == 0) {
			...
			$result = mysqli_query($conn, $query);
			error_log("cust_id: $cust_id, image1: $image1, image2: $image2, image3: $image3, image4: $image4, query: $query", 3, "/Users/hirushafernando/Documents/NEGOIT/march_5/code/billing/logfile.log");
			if (!$result) error_log("Update failed: " . mysqli_error($conn), 3, "/Users/hirushafernando/Documents/NEGOIT/march_5/code/billing/logfile.log");
			if ($result) {
				return true;
			} else {
		...
		$file_extension1 = strtolower(pathinfo($_FILES["fileToUpload1"]["name"], PATHINFO_EXTENSION));
		$file_extension2 = strtolower(pathinfo($_FILES["fileToUpload2"]["name"], PATHINFO_EXTENSION));
		$file_extension3 = strtolower(pathinfo($_FILES["fileToUpload3"]["name"], PATHINFO_EXTENSION));
		$file_extension4 = strtolower(pathinfo($_FILES["fileToUpload4"]["name"], PATHINFO_EXTENSION));

		// Define file paths dynamically based on the file extension
		$file1_original = 'images/customerdata/' . $systemid . '/' . str_pad($id_next . '_1', 10, "0", STR_PAD_LEFT) . '.' . $file_extension1;
		$file1_upload = 'images/customerdata/' . $systemid . '/uploads/' . str_pad($id_next . '_1', 10, "0", STR_PAD_LEFT) . '.' . $file_extension1;

		$file2_original = 'images/customerdata/' . $systemid . '/' . str_pad($id_next . '_2', 10, "0", STR_PAD_LEFT) . '.' . $file_extension2;
		$file2_upload = 'images/customerdata/' . $systemid . '/uploads/' . str_pad($id_next . '_2', 10, "0", STR_PAD_LEFT) . '.' . $file_extension2;

		$file3_original = 'images/customerdata/' . $systemid . '/' . str_pad($id_next . '_3', 10, "0", STR_PAD_LEFT) . '.' . $file_extension3;
		$file3_upload = 'images/customerdata/' . $systemid . '/uploads/' . str_pad($id_next . '_3', 10, "0", STR_PAD_LEFT) . '.' . $file_extension3;

		$file4_original = 'images/customerdata/' . $systemid . '/' . str_pad($id_next . '_4', 10, "0", STR_PAD_LEFT) . '.' . $file_extension4;
		$file4_upload = 'images/customerdata/' . $systemid . '/uploads/' . str_pad($id_next . '_4', 10, "0", STR_PAD_LEFT) . '.' . $file_extension4;
...
// updated by nirmal (added pdf upload as cust images)
function updateCust()
{
	global $message, $conn;
	include('config.php');
	...
	$nikname = ucwords(strtolower($nikname));
	$nikname = preg_replace("/[^A-Za-z0-9\-+,. ']/", '', $nikname);
	$nikname = mysqli_real_escape_string($conn, $nikname);

	$shop_address = trim($_POST['shop_address']);
	$shop_address = preg_replace("/[^A-Za-z0-9\-+,. ']/", '', $shop_address);
	...
	$home_address = preg_replace("/[^A-Za-z0-9\-+,. ']/", '', $home_address);
	...
	$email_add = mysqli_real_escape_string($conn, $email_add);
		...
		if ($out) {
			...
			for ($i = 1; $i <= 4; $i++) {
				if ($_FILES["fileToUpload" . $i]["name"] != '') {
					...
					// Generate the file name with the correct extension
					$tmp_name = str_pad($cust_id . '_' . $i, 10, "0", STR_PAD_LEFT) . '.' . $file_extension;
					$destination_file = $target_dir . $tmp_name;
					$uploadOk = 1;

					// Check if image file is an actual image or fake image (skip for PDFs)
					if (isset($_POST["submit"])) {
						if ($file_extension != "pdf") { // Skip this check for PDFs
							$check = getimagesize($_FILES["fileToUpload" . $i]["tmp_name"]);
							if ($check !== false) {
								$uploadOk = 1;
							} else {
					...
					// Check file size
					$maxFileSize = 15000000; // File size limit in bytes (15 MB)
					$maxFileSizeMB = $maxFileSize / (1024 * 1024); // Convert bytes to MB
					if ($_FILES["fileToUpload" . $i]["size"] > $maxFileSize) {
						$msg = "Error: Your file is too large. Maximum allowed size is " . number_format($maxFileSizeMB, 2) . " MB.";
						$uploadOk = 0;
					...
					// Allow certain file formats
					if ($file_extension != "jpg" && $file_extension != "jpeg" && $file_extension != "pdf") {
						$msg = "Error: Only JPG, JPEG, and PDF files are allowed.";
						$uploadOk = 0;
						throw new Exception($msg);
					}
					...
					if ($uploadOk == 0) {
						...
						// if (move_uploaded_file($_FILES["fileToUpload" . $i]["tmp_name"], $destination_file)) {
						// 	if ($file_extension != "pdf") { // Skip resizing for PDFs
						// 		imageresize($systemid, $tmp_name);
						// 		unlink($destination_file);
						// 	} else {
						// 		// For PDFs, move the file to the base folder
						...
						if (move_uploaded_file($_FILES["fileToUpload" . $i]["tmp_name"], $destination_file)) {
							...
							if ($file_extension != "pdf") { // JPG / JPEG
								if (imageresize($systemid, $tmp_name)) {
									if ($i == 1) $image1 = 1;
									if ($i == 2) $image2 = 1;
									if ($i == 3) $image3 = 1;
...
{
	global $store_id, $store_name, $up_id, $up_name, $conn2;
...
{
	global $gps, $graph_user, $graph_total, $lock_req, $type_req, $date1, $date2, $invoice_no, $invoice_Total, $billed_district,
		$billed_by, $billed_cust, $billed_time, $billed_store, $payment_cash, $payment_chque, $payment_id, $payment_amount,
		$payment_type, $payment_salesman, $payment_cust, $payment_time, $payment_store, $chq_details, $payment_details,
		$bi_discount, $rtn_no, $rtn_time, $rtn_pay, $rtn_salesman, $rtn_store, $rtn_cust, $payment_bank, $payment_card,
		$payment_bank_tr, $payment_card_tr, $wa_no, $wa_time, $wa_pay, $wa_salesman, $wa_entity, $wa_store, $item_type, $payment_cheque_no, $payment_cheque_date,
		$salesman_summary, $invoice_tax, $invoice_total_wo_tax, $conn2;

	$lock_qry = $type_qry = $type_req = $groupsearch = $gps = '';
	// $payment_bank = $payment_bank_tr = $payment_cash = $payment_chque = $chq_details = $payment_card = $payment_card_tr = $cu_list_arr =
	// 	$invoice_no = $payment_id = $rtn_no = $wa_no = $item_type = $payment_cheque_date = $payment_cheque_no = $graph_user = $graph_total = $salesman_summary = array();
	$graph_user = $graph_total = $invoice_no = $invoice_Total = $billed_district = $billed_by = $billed_cust = $billed_time = $billed_store = $bi_discount =
		$payment_cash = $payment_chque = $payment_id = $payment_amount = $payment_type = $payment_salesman = $payment_cust = $payment_time = $payment_store =
		$chq_details = $payment_details = $payment_bank = $payment_card = $payment_bank_tr = $payment_card_tr = $wa_no = $wa_time = $wa_pay = $wa_salesman =
		$wa_entity = $wa_store = $item_type = $payment_cheque_no = $payment_cheque_date = $rtn_no = $rtn_time = $rtn_pay = $rtn_salesman = $rtn_store =
		$rtn_cust = $salesman_summary = $cu_list_arr = $invoice_tax = $invoice_total_wo_tax = array();
		...
		if (($group != '') && ($group != 'all')) {
			...
			if ($_REQUEST['components'] == 'marketing') {
				...
				if (!empty($gp_id)) {
					...
					$groupsearch = ""; // No group filter applied
				}
				// $groupsearch = "AND cu.associated_group IN ($gp_id)";
			}
		}
	...
	if ($type_req != 3 && $type_req != 5) {
		$query1 = "SELECT py.payment_type,py.amount,py.chque_no,bk.bank_code,py.chque_branch,py.chque_date,bk.name,py.invoice_no,py.bank_trans,py.card_no,ac.name AS account_name
		FROM payment py
		LEFT JOIN bank bk ON py.chque_bank=bk.id
		LEFT JOIN accounts ac ON py.bank_trans = ac.id
		WHERE py.bill_pay=1 AND py.`status`='0' $date_qry2";
		$result1 = mysqli_query($conn2, $query1);
		while ($row1 = mysqli_fetch_array($result1, MYSQLI_ASSOC)) {
			$invoice_no_id = $row1['invoice_no'];
			switch ($row1['payment_type']) {
				case 1:
					$payment_cash[$invoice_no_id] = (float) $row1['amount'];
					break;
				case 2:
					$payment_chque[$invoice_no_id] = (float) $row1['amount'];
					$chq_details[$invoice_no_id] = 'Chque No    : ' . $row1['chque_no'] . ' | ' . $row1['bank_code'] . ' | ' . $row1['chque_branch'] .
						'&#10;Bank             : ' . $row1['account_name'] .
						'&#10;Chque Date : ' . $row1['chque_date'];
					break;
				case 3:
					$payment_bank[$invoice_no_id] = (float) $row1['amount'];
					$payment_bank_tr[$invoice_no_id] = 'Bank    : ' . $row1['account_name'];
					break;
				case 4:
					$payment_card[$invoice_no_id] = (float) $row1['amount'];
					$card_no = "**** **** **** " . substr($row1['card_no'], 15);
					$payment_card_tr[$invoice_no_id] = 'Card    : ' . $card_no . ' | Bank : ' . $row1['account_name'];
					break;
			}
		}

		if ((isset($_REQUEST['components'])) && ($_GET['components'] == 'manager' || $_GET['components'] == 'topmanager')) {
			$query = "SELECT bm.invoice_no, SUM(bi.qty * bi.unit_price), di.name, up.username, cu.name, bm.billed_timestamp, st.name, SUM(bi.discount), bm.`cust`, up.id, bm.gps_x, bm.gps_y, bm.tax, (SUM(bi.qty * bi.unit_price) - bm.tax) AS total_wo_tax
			...
			// 13 indexes
		} elseif ((isset($_REQUEST['components'])) && ($_GET['components'] == 'marketing')) { // mkt
			$query = "SELECT bm.`invoice_no`, (bm.`invoice_+total` + bm.`invoice_-total`), di.name, up.username, cu.name, DATE(bm.billed_timestamp), st.name, bm.tax, ((bm.`invoice_+total` + bm.`invoice_-total`) - bm.tax) AS total_wo_tax
			FROM bill_main bm, userprofile up, district di, cust cu, stores st
			WHERE bm.store=st.id AND bm.`cust`=cu.id AND bm.billed_by=up.id AND bm.billed_district=di.id AND bm.`status` NOT IN (0,7)
			$lock_qry $type_qry $date_qry1 $groupsearch $storesearch $salesmansearch $processbysearch GROUP BY bm.invoice_no ORDER BY $order_by";
			// 8 indexes
		} else { // sup, bill, bill2
			$query = "SELECT bm.`invoice_no`, SUM(bi.qty*bi.unit_price), di.name, up.username, cu.name, DATE(bm.billed_timestamp), st.name, SUM(bi.discount), bm.tax, (SUM(bi.qty * bi.unit_price) - bm.tax) AS total_wo_tax
			FROM bill bi, bill_main bm, userprofile up, district di, cust cu, stores st
			WHERE bm.store=st.id AND bm.`cust`=cu.id AND bi.invoice_no=bm.invoice_no AND bm.billed_by=up.id AND bm.billed_district=di.id AND bm.`status` NOT IN (0,7)
			$lock_qry $type_qry $date_qry1 $groupsearch $storesearch $salesmansearch $processbysearch GROUP BY bm.invoice_no ORDER BY $order_by";
			// 8 indexes
		}
		...
		$result = mysqli_query($conn2, $query);
		if ($result === false) {
			die('Query failed: ' . mysqli_error($conn2) . '<br>SQL: ' . $query);
		}
		while ($row = mysqli_fetch_array($result)) {
			$invoice_no_id = $row[0];
			$invoice_no[] = $row[0];
			$invoice_Total[] = $row[1]; // with tax
			$billed_district[] = $row[2];
			$billed_by[] = $row[3];
			$billed_cust[] = $row[4];
			if ((isset($_REQUEST['components'])) && ($_GET['components'] == 'manager' || $_GET['components'] == 'topmanager')) {
				if (isTimeShow()) {
					$billed_time[] = substr($row[5], 0, 10) . " - " . substr($row[5], 11, 5);
				} else {
					$billed_time[] = substr($row[5], 0, 10);
				}
				$invoice_tax[] = $row[12]; // tax value
				$invoice_total_wo_tax[] = $row[13]; // total without tax
			} else { // bill2, marketing
				$billed_time[] = $row[5];
				$invoice_tax[] = $row[7]; // tax value
				$invoice_total_wo_tax[] = $row[8]; // total without tax
			}
			$billed_store[] = $row[6];
			if (isset($_REQUEST['components']) && $_REQUEST['components'] != 'marketing') { // bill2, manager, top mgr
				$bi_discount[] = $row[7];
			}
	...
	// Payments collection
	if (isset($_REQUEST['components']) && ($_REQUEST['components'] == 'manager') || ($_REQUEST['components'] == 'topmanager')) {
		$associate_group_query = '';
		$associate_group_query = "AND cu.associated_group=gp.id";

		$query2 = "SELECT py.id, py.amount, py.payment_type, up.username, cu.name AS cust_name, py.payment_date AS pay_date, st.name AS store_name, py.chque_no, bk.bank_code,
		py.chque_branch, py.chque_date, bk.name AS bank_name, py.`cust`, up.id AS up_id, py.gps_x, py.gps_y, py.bank_trans, py.card_no, ac.name AS account_name
		FROM userprofile up, cust cu, stores st, cust_group gp, payment py
		LEFT JOIN bank bk ON py.chque_bank = bk.id
		LEFT JOIN accounts ac ON py.bank_trans = ac.id
		WHERE py.salesman=up.id AND py.cust=cu.id AND py.store=st.id $associate_group_query AND py.bill_pay=2 AND py.`status`=0
		...
		while ($row2 = mysqli_fetch_array($result2, MYSQLI_ASSOC)) {
			if ($row2['id'] > 0) {
				$chq_details_tmp = 'Chque No    : ' . $row2['chque_no'] . ' | ' . $row2['bank_code'] . ' | ' . $row2['chque_branch'] .
					'&#10;Bank             : ' . $row2['bank_name'] .
					'&#10;Chque Date : ' . $row2['chque_date'];
				$payment_id[] = $row2['id'];
				$payment_amount[] = $row2['amount'];
				switch ($row2['payment_type']) {
					case 1:
						$payment_type[] = 'Cash';
					...
					case 3:
						$payment_type[] = 'Bank';
						$payment_details[] = 'href="#" style="text-decoration:none; color:#00AAAA;" title="Bank : ' . $row2['account_name'] . '"';
						break;
					case 4:
						$payment_type[] = 'Card';
						$card_no = "**** **** **** " . substr($row2['card_no'], 15);
						$payment_details[] = 'href="#" style="text-decoration:none; color:#CC3399;" title="Card : ' . $card_no . ' | Bank : ' . $row2['account_name'] . '"';
						break;
					default:
						$payment_details[] = '';
				}
				$payment_salesman[] = $row2['username'];
				$payment_cust[] = $row2['cust_name'];
				// Format time as "date - HH:MM"
				if (isTimeShow()) {
					$payment_time[] = substr($row2['pay_date'], 0, 10) . " - " . substr($row2['pay_date'], 11, 5);
				} else {
					$payment_time[] = substr($row2['pay_date'], 0, 10);
				}
				$payment_store[] = $row2['store_name'];
				$payment_cheque_no[] = $row2['chque_no'];
				$payment_cheque_date[] = $row2['chque_date'];
				if (array_search($row2['cust'], $cu_list_arr) === false) {
					if ($row2['gps_x'] != 0 && $row2['gps_y'] != 0) {
						$gps .= $row2['cust'] . ':' . $row2['up_id'] . ':' . $row2['gps_x'] . ':' . $row2['gps_y'] . ',';
					}
				}
			} else {
		...
		$query = "SELECT up.username AS salesman,py.payment_type,SUM(py.amount) AS total_amount
    FROM payment py
    JOIN userprofile up ON py.salesman = up.id
    JOIN cust cu ON py.cust = cu.id
    JOIN stores st ON py.store = st.id
    JOIN cust_group gp ON cu.associated_group = gp.id
    WHERE py.bill_pay = 2 AND py.`status` = 0 $sub_sys_pyqry $date_qry2 $storesearch $groupsearch $salesmansearch
    GROUP BY up.username, py.payment_type
    ORDER BY up.username";
		...
		while ($row = mysqli_fetch_assoc($result)) {
			...
			if (!isset($salesman_summary[$salesman])) {
			...
			switch ($type) {
				case 1:
					$salesman_summary[$salesman]['Cash'] += $amount;
					break;
				case 2:
					$salesman_summary[$salesman]['Cheque'] += $amount;
					break;
				case 3:
					$salesman_summary[$salesman]['Bank'] += $amount;
					break;
				case 4:
					$salesman_summary[$salesman]['Card'] += $amount;
					break;
			}
			$salesman_summary[$salesman]['Total'] += $amount;
		}
	} else { // sup, mkt, bill bill2
		// Build the query with meaningful aliases and explicit JOINs
		$query2 = "SELECT py.id, ROUND(py.amount) AS amount, py.payment_type, up.username AS salesman, cu.name AS cust_name, DATE(py.payment_date) AS pay_date, st.name AS store, py.chque_no, py.chque_date
				FROM payment py
				JOIN userprofile up ON py.salesman = up.id
				JOIN cust cu ON py.cust = cu.id
				JOIN stores st ON py.store = st.id
				WHERE py.bill_pay = 2 AND py.`status` = 0
				$date_qry2 $storesearch $groupsearch $salesmansearch
				ORDER BY st.name, py.payment_date";
		$result2 = mysqli_query($conn2, $query2);
		if ($result2) {
			while ($row2 = mysqli_fetch_assoc($result2)) {
				...
				if ($row2['id'] > 0) {
					...
					// Use switch to determine payment type
					switch ($row2['payment_type']) {
						case 1:
							$payment_type[] = 'Cash';
							break;
					...
					$payment_salesman[] = $row2['salesman'];
					$payment_cust[] = $row2['cust_name'];
					// Format time as "date - HH:MM"
					$payment_time[] = $row2['pay_date'];
					$payment_store[] = $row2['store'];
					$payment_cheque_no[] = $row2['chque_no'];
					$payment_cheque_date[] = $row2['chque_date'];
				}
			}
    ...
    JOIN cust cu ON py.cust = cu.id
    JOIN stores st ON py.store = st.id
    WHERE py.bill_pay = 2 AND py.`status` = 0 $date_qry2 $storesearch $groupsearch $salesmansearch
    GROUP BY up.username, py.payment_type
    ORDER BY up.username";
	...
	if ($type_req == 3 || $type_req == '') {
		if (isset($_REQUEST['components']) && (($_REQUEST['components'] == 'manager') || ($_REQUEST['components'] == 'topmanager'))) {
			// Use explicit JOINs and assign aliases for clarity.
			$query3 = "SELECT rm.invoice_no, TIME(rm.return_date) AS rtn_time, SUM(rt.extra_pay) AS rtn_pay, up.username AS rtn_salesman, st.name AS rtn_store, cu.name AS rtn_cust
				   FROM return_main rm
				   JOIN `return` rt ON rm.invoice_no = rt.invoice_no
				   JOIN stores st ON rm.store = st.id
				   JOIN cust cu ON rm.`cust` = cu.id
				   JOIN cust_group gp ON cu.associated_group = gp.id
				   JOIN userprofile up ON rm.return_by = up.id
				   WHERE rt.extra_pay != 0 AND rm.`status` = 2
					$sub_sys_rtqry $date_qry3 $storesearch $groupsearch $salesmansearch
				   GROUP BY rm.invoice_no
				   ORDER BY st.name";

			$result3 = mysqli_query($conn2, $query3);
			while ($row3 = mysqli_fetch_array($result3, MYSQLI_ASSOC)) {
				$rtn_no[] = $row3['invoice_no'];
				$rtn_time[] = $row3['rtn_time'];
				$rtn_pay[] = $row3['rtn_pay'];
				$rtn_salesman[] = $row3['rtn_salesman'];
				$rtn_store[] = $row3['rtn_store'];
				$rtn_cust[] = $row3['rtn_cust'];
			}
		} else {
			$query3 = "SELECT rm.invoice_no,time(rm.return_date),SUM(rt.extra_pay),up.username,st.name,cu.name
			FROM return_main rm, `return` rt, stores st, userprofile up, cust cu
			WHERE rm.invoice_no=rt.invoice_no AND rm.store=st.id AND rm.return_by=up.id AND rm.`cust`=cu.id AND rt.extra_pay!=0 AND rm.`status`=2
			$date_qry3 $storesearch $groupsearch $salesmansearch
	...
	if ($type_req == 5 || $type_req == '') {
		...
		while ($row3 = mysqli_fetch_array($result3)) {
			$wa_no[] = $row3[0];
			if (isTimeShow()) {
				$wa_time[] = substr($row3[1], 0, 10) . " - " . substr($row3[1], 10, 16);
			} else {
				$wa_time[] = substr($row3[1], 0, 10);
			}
			$wa_pay[] = $row3[2];
			$wa_salesman[] = $row3[3];
...
function dailySale2($store, $sub_system)
{
	...
	if ($type_req == 3 || $type_req == '') {
		...
		$query3 = "SELECT rm.invoice_no, 
                  time(rm.return_date), 
                  SUM(rt.extra_pay) as total_extra_pay,
                  up.username, 
                  st.name, 
                  cu.name 
           FROM return_main rm, 
                ...
                userprofile up, 
                cust cu 
           WHERE rm.invoice_no = rt.invoice_no 
             AND rm.store = st.id 
             AND cu.associated_group = gp.id 
             AND rm.return_by = up.id 
             AND rm.cust = cu.id 
             AND date(rm.return_date) = '$date' 
             AND rt.extra_pay != 0 
             AND rm.status = 2 
             $sub_sys_rtqry $storesearch $groupsearch $salesmansearch 
           GROUP BY rm.invoice_no, 
                    time(rm.return_date), 
                    up.username, 
                    st.name, 
                    cu.name 
           ORDER BY st.name, rt.invoice_no";
$result3 = mysqli_query($conn2, $query3);
		$result3 = mysqli_query($conn2, $query3);
		while ($row3 = mysqli_fetch_array($result3)) {
			if ($row3[2] != 0) {
...
function getCustSale($systemid)
{
	...
	if ($_REQUEST['components'] === 'bill2') {
		...
		$user_id = $_COOKIE['store'] ?? 0;
		if ($systemid == 24) {
			// $user_filter_payment = " AND py.salesman='$user_id' ";
			// $user_filter_bill = " AND bm.billed_by='$user_id' ";
			$user_filter_payment = " AND py.store='$user_id' ";
			$user_filter_bill = " AND bm.store='$user_id' ";
		}
	}
	...
	if (isset($_REQUEST['customer_id'])) {
		...
		//-------------Customer M Type----------------------//
		// Legacy code before change 16_02_2024
		// $cust_mtype = 'normal';
		...
		// Legacy code before change 16_02_2024
		$query = "SELECT SUM(bi.qty*bi.unit_price) FROM bill bi, bill_main bm WHERE bm.invoice_no=bi.invoice_no AND bm.`status` NOT IN (0,7) AND bm.exclude=0 AND bm.`lock`=1 AND bm.`cust`='$customer_id' AND date(bm.billed_timestamp) < '$fromdate' {$user_filter_bill}";
		...
		if ($result === false) {
			error_log("SQL Error: " . mysqli_error($conn2));
			$totalbill0 = 0; // Assign a default or error value
		} else {
			$row = mysqli_fetch_row($result);
			$totalbill0 = $row[0]; // Will be NULL if no rows found by SUM
		}

		// Legacy code before change 16_02_2024
		$query = "SELECT SUM(py.amount) FROM payment py, bill_main bm WHERE py.invoice_no=bm.invoice_no AND py.`status`=0 AND py.`cust`='$customer_id' AND py.bill_pay=1 AND date(bm.billed_timestamp)< '$fromdate' {$user_filter_payment}";
		...
		// Legacy code before change 16_02_2024
		$query = "SELECT SUM(py.amount) FROM payment py WHERE py.`status`=0 AND py.`cust`='$customer_id' AND py.bill_pay=2 AND date(py.payment_date) < '$fromdate' {$user_filter_payment}";
		// $query = "SELECT SUM(py.amount)
		// FROM payment py
		...
		// Legacy code before change 16_02_2024
		$query = "SELECT SUM(py.amount) FROM payment py WHERE py.`status`=0 AND py.chque_return=1 AND py.cust='$customer_id' AND date(py.chque_return_date) < '$fromdate' {$user_filter_payment}";
		// $query = "SELECT SUM(py.amount)
		// FROM payment py
...
function getChqueNo($sub_system, $postpone)
{
	...
	$date = date("Y-m-d", time() - 3 * 30 * 24 * 60 * 60);
	...
	$query = "SELECT py.chque_no,bk.bank_code,py.chque_branch,py.`id`
		FROM payment py, bank bk
		WHERE py.chque_bank=bk.id AND py.chque_no IS NOT NULL AND py.chque_return=0
		AND py.`status`='0' $sub_system_qry AND ((chque_date BETWEEN '$date' AND  '$today') $postpone_ary";
	$result = mysqli_query($conn2, $query);
	while ($row = mysqli_fetch_array($result)) {
		$py_chqnofull[] = '(' . $row[3] . ') ' . $row[0] . '-' . str_pad($row[1], 4, "0", STR_PAD_LEFT) . '-' . str_pad($row[2], 3, "0", STR_PAD_LEFT);
	}
}
	*/
...
function getChqueNo($sub_system, $postpone)
{
	...
	$previousMonthDate = date("Y-m-d", strtotime("-6 month"));
	$py_chqnofull = array();
	...
	// --- Your new conditional logic for py.chque_return ---
	$chque_return_filter = "";
	if ($postpone == 1) {
		$chque_return_filter = "AND py.chque_return = 0"; // Only non-returned cheques
	} else {
		$chque_return_filter = "AND py.chque_return IN (0,1)"; // Valid AND returned cheques
	}
	// --- End of new conditional logic ---

	$query = "SELECT py.chque_no, bk.bank_code, py.chque_branch, py.`id` FROM payment py, bank bk
            WHERE py.chque_bank = bk.id AND py.chque_no IS NOT NULL AND py.`status` = '0' $chque_return_filter $sub_system_qry
            AND (chque_date > '$previousMonthDate' $postpone_ary";

	$result = mysqli_query($conn2, $query);
	while ($row = mysqli_fetch_array($result)) {
		$py_chqnofull[] = '(' . $row[3] . ') ' . $row[0] . '-' .
			str_pad($row[1], 4, "0", STR_PAD_LEFT) . '-' .
			str_pad($row[2], 3, "0", STR_PAD_LEFT);
	}
}

// updated by nirmal 03_06_2025 (added filters)
function getReturnedChque($sub_system)
{
	global $chq0_id, $chq0_invno, $chq0_cuname, $chq0_amount, $chq0_no, $chq0_bank, $chq0_branch, $chq0_date, $chq0_paymentdate, $chq0_salesman, $chq0_returndate, $chq0_rtn_clear,
		$chq0_rtn_cle_date, $chq0_code, $salesman_filter, $chque_rtn_clear_filter, $return_clear_filter, $salesman_summary, $conn2;
	$salesman_filter = $chq0_salesman = $chq0_id = $chque_rtn_clear_filter = $salesman_summary = array();
	$filter_qry = ''; // Initialize the filter query for chque_rtn_clear
	include('config.php');

	// Populate the salesman_filter array
	$query_salesmen = "SELECT username FROM userprofile WHERE status = 0"; // Adjust the query as needed
	$result_salesmen = mysqli_query($conn2, $query_salesmen);

	if (!$result_salesmen) {
		die("SQL Error: " . mysqli_error($conn2));
	}

	while ($row = mysqli_fetch_assoc($result_salesmen)) {
		$salesman_filter[] = $row['username']; // Populate the array with valid salesmen
	}

	// Sub-system filter
	if ($sub_system != 'all') {
		$filter_qry = "AND py.`sub_system`='$sub_system'";
	}

	// Salesman filter
	if (isset($_GET['sm']) && $_GET['sm'] != '-all-') {
		$salesman_filter_value = $_GET['sm'];
		if (in_array($salesman_filter_value, $salesman_filter)) {
			$filter_qry .= " AND up.username='$salesman_filter_value'";
		} else {
			die("Invalid salesman value.");
		}
	}

	// Status filter
	$valid_statuses = ['0', '1', '2']; // Valid status values
	if (isset($_GET['return_clear'])) {
		$return_clear_filter = $_GET['return_clear'];

		// Check if the selected value is valid
		if ($return_clear_filter === '') {
			// "All" case: Include all statuses (0, 1, 2)
			$filter_qry .= " AND py.chque_rtn_clear IN (0, 1, 2)";
		} elseif (in_array($return_clear_filter, $valid_statuses)) {
			// Specific status selected
			$filter_qry .= " AND py.chque_rtn_clear='$return_clear_filter'";
		} else {
			// Invalid status value
			die("Invalid status value.");
		}
	} else {
		// Default behavior: Select only Pending (0)
		$filter_qry .= " AND py.chque_rtn_clear IN (0,1)";
	}
	...
	if (isset($_GET['start_date']) && isset($_GET['end_date']) && $_GET['start_date'] != '' && $_GET['end_date'] != '') {
		...
		// Use the validateDate function
		if (validateDate($start_date, 'Y-m-d') && validateDate($end_date, 'Y-m-d')) {
			if ($start_date <= $end_date) {
				$filter_qry .= " AND py.chque_return_date BETWEEN '$start_date' AND '$end_date'";
			} else {
	...
	$query1 = "SELECT py.id, py.invoice_no, cu.name, py.amount, py.chque_no, ba.name, ba.bank_code, py.chque_branch, py.chque_date, py.payment_date, up.username,
	date(py.chque_return_date), py.chque_rtn_clear, date(py.chque_rtn_cle_date)
	FROM payment py, bank ba, userprofile up, cust cu
	WHERE py.cust=cu.id AND py.salesman=up.id AND py.chque_bank=ba.id AND py.`status`=0 AND py.chque_return=1 $filter_qry
	...
	while ($row1 = mysqli_fetch_array($result1)) {
		...
		if ($row1[12] == 0) {
			...
			$chq0_rtn_clear[] = 'Deleted';
		} else {
			$chq0_rtn_clear[] = 'Unknown';
		}
		$chq0_rtn_cle_date[] = $row1[13];
		$chq0_code[] = $row1[4] . '-' . str_pad($row1[6], 4, "0", STR_PAD_LEFT) . '-' . str_pad($row1[7], 3, "0", STR_PAD_LEFT);
	}

	// Query for salesman summary
	$query_summary = "SELECT up.username, SUM(py.amount) AS total_amount
	FROM payment py, userprofile up
	WHERE py.salesman=up.id AND py.`status`=0 AND py.chque_return=1 $filter_qry
	GROUP BY up.username
	ORDER BY total_amount DESC";
	$result_summary = mysqli_query($conn2, $query_summary);
	if (!$result_summary) {
		die("SQL Error: " . mysqli_error($conn2));
	}
	while ($row_summary = mysqli_fetch_assoc($result_summary)) {
		$salesman_summary[] = [
			'salesman' => $row_summary['username'],
			'total_amount' => $row_summary['total_amount']
		];
	}

	// Create unique filters for salesman and chque_rtn_clear
	if (!empty($chq0_salesman)) {
		$salesman_filter = array_unique($chq0_salesman);
		$salesman_filter = array_values($salesman_filter);
	} else {
		$salesman_filter = array();
	}

	if (!empty($chq0_rtn_clear)) {
		$chque_rtn_clear_filter = array_unique($chq0_rtn_clear);
		$chque_rtn_clear_filter = array_values($chque_rtn_clear_filter);
	} else {
		$chque_rtn_clear_filter = array();
	}
}
...
function getChqueOne()
{
	...
	if (isset($_REQUEST['chque_no'])) {
		$py_chqnofull = trim($_REQUEST['chque_no']);
		$parsed_cheque_part = '';
		$paymentId = null; // Variable to store the ID if present in the first format

		if (preg_match('/^\(\s*(\d+)\s*\)\s*(.+)$/', $py_chqnofull, $matches)) {
			// Format is (ID) ChequeNo-BankCode-BranchCode
			$paymentId = (int) $matches[1]; // Capture the ID, cast to integer
			$parsed_cheque_part = trim($matches[2]); // Get the part after the parenthesis
		} else {
			// Format is just ChequeNo-BankCode-BranchCode or other non-(ID) format
			$parsed_cheque_part = $py_chqnofull;
		}

		if (empty($paymentId)) {
			exit('Error: Payment id is null');
		}

		// 2. Parse the ChequeNo-BankCode-BranchCode part by exploding on hyphen
		$parts = explode('-', $parsed_cheque_part);

		// 3. Validate the parsed parts
		if (count($parts) < 3) {
			exit('Invalid cheque string format: Missing parts after parsing.');
		}

		// 4. Assign parsed parts to variables
		$py_chqno = trim($parts[0]); // Cheque number
		$py_chqbank = trim($parts[1]); // Bank code
		$py_branch = trim($parts[2]); // Branch code

		// Assign the main cheque number to the global variable $chq_no
		$chq_no = $py_chqno; // Assign the parsed cheque number

		$conditions[] = "py.status = 0";
		// Conditionally add the payment filter
		if (isset($paymentId) && !empty($paymentId)) {
			$conditions[] = "py.id = '$paymentId'";
		}

		// Implode the conditions array to form the WHERE clause
...
function setChqueStatus($status)
{
	...
	$store = $_COOKIE['store'];
	$time_now = timeNow();
	$postpone_qry = $qb_msg = $qb_result = $reason = $salesman = $qb_cust_id = $storeQBID = '';
	include('config.php');
	$out = true;
		...
		try {
			...
			if ($out) {
				if (isQuickBooksActive(1)) {
					...
					$cust = $row[0];
					if (!$cust) {
						throw new Exception('Error: Failed to select customer');
					}
			...
			if ($out) {
				$query = "SELECT py.`chque_no`, bk.`bank_code`, py.`chque_branch` FROM payment py, bank bk WHERE py.`chque_bank` = bk.`id` AND py.`id`='$id'";
				$row = mysqli_fetch_row(mysqli_query($conn, $query));
				$chque_no = $row[0];
				$chque_bank = $row[1];
				$chque_branch = $row[2];
			...
			$query1 = "UPDATE `payment` SET `chque_return`='$status',`chque_return_date`='$time_now' $postpone_qry WHERE `id`='$id'";
			...
			if ($affected_rows1 === 0) {
				throw new Exception("No rows updated in payment table. Check conditions.");
			}
			$query2 = "UPDATE `payment_subsys` SET `status`='$status2',`chque_return_date`='$time_now' WHERE `id`='$id'";
			...
			if (isQuickBooksActive(1)) {
				...
				if ($status == 1) {
					$paymentQuery = "SELECT py.`invoice_no`, py.`amount`, c.`name`, py.`qb_id`, py.`chque_deposit_bank` FROM payment py, cust c
								WHERE c.`id` = py.`cust` AND py.`id` = '$id' AND py.`qb_status` IS NOT NULL";
					$paymentRow = mysqli_fetch_row(mysqli_query($conn, $paymentQuery));

					if (isSalesmanPaymentDepositActive()) {
						if ($paymentRow) {
							if ($reason == 7) { // bank return
								if (empty($paymentRow[4])) {
									$chequeIdentifier = $chque_no . '-' . $chque_bank . '-' . $chque_branch;
									throw new Exception(
										"Cheque '{$chequeIdentifier}' cannot be marked as returned because it has not been deposited yet. Please deposit it first."
									);
								}
								if ($paymentRow[4] != '') {  // Assuming chque_deposit_bank
									...
									$description = "[PAYMENT] - Payment No: $id, Cheque Return:Bank, Method: Cheque ($bank_name), Cheque Number $cheque_no";
									if ($invoice_no != 0) {
									...
									$journalEntryForCheque = buildJournalEntry(
										$conn,
										abs($amount),
										$debitAccountName,
										$creditAccountName,
									...
									if (isset($journalEntryForCheque['error'])) {
										...
										foreach ($journalEntryForCheque as $entry) {
											...
											$query = "INSERT INTO qb_queue (`batch_id`, `action`, `payment_id`, `posting_type`, `account_id`, `account_name`, `amount`, `description`, `created_at`, `class_id`, `department_id`, `entity_type`, `entity_id`)
														VALUES ('$batch_id','$action_name', '$id', '$posting_type', '$account_id', '$account_name', '$amount', '$description', '$time_now',
							...
							if ($reason == 8 || $reason == 9) { // 8 = Cash Receive, 9 = Issue New Cheque
								...
								$description = "[PAYMENT] - Payment No: $id, Cheque Return:Other, Method: Cheque, Cheque Number $cheque_no";
								if ($invoice_no != 0) {
									$description .= ", Invoice No: $invoice_no";
								}
								$description .= ", Customer : $custName";
...
function setChquePostpone($case)
{
	...
	if ($out) {
		$query = "SELECT COUNT(py.id) FROM payment py, bank bk WHERE py.id='$py_id'";
		...
		if ($row[0] == 0) {
			...
			$query = "SELECT py.chque_bank, bk.bank_code, py.chque_branch, py.chque_no FROM payment py, bank bk WHERE py.id='$py_id' AND py.chque_bank=bk.id";
			$row = mysqli_fetch_row(mysqli_query($conn, $query));
			if (!$row) {
				$message = 'Error: Cannot find the cheque bank and branch';
				$out = false;
			} else {
				$py_chqbank = $row[0];
				$py_chqbank_code = $row[1];
				$py_branch = $row[2];
				$py_chqno = $row[3];
				$py_chqnofull = '(' . $py_id . ') ' . $py_chqno . '-' .
					str_pad($py_chqbank_code, 4, "0", STR_PAD_LEFT) . '-' .
					str_pad($py_branch, 3, "0", STR_PAD_LEFT);
			}
		}
	}

	if ($master_pw3 == '') {
		$message = 'Error: Please fill date and password';
		$out = false;
	}
	...
	if ($out) {
		$query = "SELECT py.chque_postpone, py.chque_date, py.chque_date2 FROM payment py, bank bk WHERE py.chque_bank=bk.id AND py.id='$py_id'";
		$row = mysqli_fetch_row(mysqli_query($conn, $query));
		$chque_postpone = $row[0];
		$old_chque_date = $row[1];
		$old_chque_date2 = $row[2];
		if ($case == 'add') {
			$query = "UPDATE `payment` py, bank bk SET py.`chque_date2`='$old_chque_date',py.`chque_postpone`='1',py.`chque_date`='$postpone_date' WHERE py.chque_bank=bk.id AND py.id='$py_id'";
			$result = mysqli_query($conn, $query);
			if ($result) {
			...
			$query = "UPDATE `payment` py, bank bk SET py.`chque_date`='$postpone_date' WHERE py.chque_bank=bk.id AND py.id='$py_id'";
			$result = mysqli_query($conn, $query);
			if ($result) {
			...
			$query = "UPDATE `payment` py, bank bk SET py.`chque_date`='$old_chque_date2',py.`chque_postpone`='2',py.`chque_date2`='$old_chque_date' WHERE py.chque_bank=bk.id AND py.id='$py_id'";
			$result = mysqli_query($conn, $query);
			if ($result) {
...
{
	global $dev_id, $dev_name, $key_dev_name,$conn2;
	$key_dev_name = '';
	$dev_id = array();
	if ($sub_system == 'all')
		$sub_system_qry = '';
...
{
	global $store, $from_date, $to_date, $category, $sum_date, $sum_sale, $sum_cashback, $sum_totalsale, $conn2;
...
{
	global $selection, $customer, $customer_id, $store1, $category, $fromdate, $todate, $itm2_id, $itm2_name, $itm2_sold, $sold_itemid, $itm2_qty, $conn2;
	$sold_itemid = array();
	$k = 0;
	$case = false;
	if ($sub_system == 'all')
	...
	if ($case) {
		...
		include('config.php');
		if ($selection == 'customer')
			$query = "SELECT DISTINCT itm.id, itm.description FROM bill_main bm, bill bi, inventory_items itm WHERE bm.invoice_no=bi.invoice_no AND bi.item=itm.id AND bm.`status` NOT IN (0,7) AND bm.`lock`=1 AND itm.description!='CHQUE' AND itm.description!='CREDIT' AND bm.`cust`='$customer_id' AND bm.`status` NOT IN (0,7) AND bm.exclude=0 AND date(bi.`date`) BETWEEN '$fromdate' AND '$todate' AND itm.`category`='$category' $sub_system_qry ORDER BY itm.description";
		if ($selection == 'store')
			$query = "SELECT DISTINCT itm.id, itm.description FROM bill_main bm, bill bi, inventory_items itm, stores st WHERE bm.invoice_no=bi.invoice_no AND bi.item=itm.id AND bm.`store`=st.id AND bm.`status` NOT IN (0,7) AND bm.`lock`=1 AND itm.description!='CHQUE' AND itm.description!='CREDIT' AND st.name='$store1' AND bm.`status` NOT IN (0,7) AND bm.exclude=0 AND date(bi.`date`) BETWEEN '$fromdate' AND '$todate' AND itm.`category`='$category' $sub_system_qry ORDER BY itm.description";
		$result = mysqli_query($conn2, $query);
		while ($row = mysqli_fetch_array($result)) {
			$sold_itemid[] = $row[0];
		}
...
function getSalesReport4_OLD($sub_system)
{
	$user = $_COOKIE['user'];
	...
	if ($case) {
		...
		$query = "SELECT itc.name,st.name,up.username,cu.name,itm.description,bi.`comment`,bi.unit_price,date(bm.billed_timestamp),bi.`qty`
		FROM bill_main bm, bill bi, inventory_items itm, item_category itc, cust cu, stores st, userprofile up
		WHERE bm.billed_by=up.id AND bm.invoice_no=bi.invoice_no AND bi.item=itm.id AND itm.category=itc.id AND bm.`cust`=cu.id
		AND bm.`store`=st.id AND bm.`status` NOT IN (0,7) AND bm.`lock`=1
		AND itm.description!='CHQUE' AND itm.description!='CREDIT' $cust_st_qry  AND bm.`status`
		NOT IN (0,7) AND itm.unic=1 AND bm.exclude=0
		AND date(bi.`date`) BETWEEN '$fromdate' AND '$todate' $cat_qry $sub_system_qry
		ORDER BY itc.name,st.name,cu.name,bm.billed_timestamp,itm.description";
		$result = mysqli_query($conn2, $query);
		// while ($row = mysqli_fetch_array($result)) {
		// 	$item_category[] = $row[0];
		// 	$item_store[] = $row[1];
		...
		// 1. Fetch all rows
		while ($row = mysqli_fetch_array($result)) {
			// Store each row's data in an associative array.
			$rowData = array(
				'item_category' => $row[0],
				'item_store' => $row[1],
				'item_salesman' => ucwords($row[2]),
				'item_cust' => $row[3],
				'item_desc' => $row[4],
				'item_sn' => $row[5],  // comma-separated serial numbers
				'item_sold_price' => $row[6],
				'item_bm_date' => $row[7],
				'qty' => $row[8]   // quantity (+1 sale or -1 cash back)
			);

			// Explode the SN list (separated by commas) and update the global SN mapping.
			$snArray = explode(',', $rowData['item_sn']);
			foreach ($snArray as $sn) {
				...
				if (!isset($snTotals[$sn])) {
					$snTotals[$sn] = 0;
				}
				// Add +1 for positive qty, or subtract 1 if negative.
				// (Adjust this logic if a qty can be other than ±1.)
				if ($rowData['qty'] > 0) {
					$snTotals[$sn] += 1;
				} else {
					$snTotals[$sn] -= 1;
		...
		// Now rebuild your result arrays by filtering out canceled SNs.
		// That is, if a serial number's global total equals 0, it means a sale and its corresponding
		// cash back canceled out. We'll remove such SNs from the row.
		$item_category = array();
		$item_store = array();
		$item_salesman = array();
		$item_cust = array();
		$item_desc = array();
		$item_sn = array();
		$item_sold_price = array();
		$item_bm_date = array();

		foreach ($allRows as $rowData) {
			// Explode the comma-separated SN string, and then filter each SN.
			$snArray = explode(',', $rowData['item_sn']);
			$filteredSNs = array();
			foreach ($snArray as $sn) {
				$sn = trim($sn);
				...
				// Only include SNs whose net total is not zero.
				// (If you want only positive net sales, you could check for > 0.)
				if (isset($snTotals[$sn]) && $snTotals[$sn] != 0) {
					$filteredSNs[] = $sn;
				}
			}

			// Option: Skip this row entirely if all SNs cancel out.
			if (empty($filteredSNs)) {
				continue;
			}

			// Otherwise, store the row data with the filtered SN list.
			$item_category[] = $rowData['item_category'];
			$item_store[] = $rowData['item_store'];
			$item_salesman[] = $rowData['item_salesman'];
			$item_cust[] = $rowData['item_cust'];
			$item_desc[] = $rowData['item_desc'];
			// Join the remaining SNs back as a comma-separated string (or choose to keep as array)
			$item_sn[] = implode(",", $filteredSNs);
			$item_sold_price[] = $rowData['item_sold_price'];
			$item_bm_date[] = $rowData['item_bm_date'];
		}
...
// updated by nirmal 17_12_2025 (fixed serial number cancellation logic)
function getSalesReport4($sub_system)
{
	...
	if ($case) {
		...
		// 1. Initialize a blank array to hold all database rows.
		$allRows = array();

		// 2. Loop through the database results one row at a time.
		while ($row = mysqli_fetch_array($result)) {
			$allRows[] = array(
				'item_category' => $row[0],
				'item_store' => $row[1],
				'item_salesman' => ucwords($row[2]),
				'item_cust' => $row[3], // Customer Name
				'item_desc' => $row[4],
				'item_sn' => $row[5],
				'item_sold_price' => $row[6],
				'item_bm_date' => $row[7],
		...
		// 4. Create an array to keep score of Pluses and Minuses.
		// NEW: We track this per "SN + Customer".
		$pair_counts = array();

		// 5. Loop through every saved row.
		foreach ($allRows as $r) {
			$snArray = explode(',', $r['item_sn']);
			foreach ($snArray as $sn) {
				$sn = trim($sn);
				if ($sn === '')
					continue;
				...
				// --- NEW LOGIC START ---
				// Create a unique key that combines SN and Customer.
				// This ensures "John's Return" only cancels "John's Sale".
				$uniqueKey = $sn . '_' . $r['item_cust'];
				// -----------------------

				if (!isset($pair_counts[$uniqueKey])) {
					$pair_counts[$uniqueKey] = array('plus' => 0, 'minus' => 0);
				}

				if ($r['qty'] > 0) {
					$pair_counts[$uniqueKey]['plus']++;
				} else {
					$pair_counts[$uniqueKey]['minus']++;
				}
			}
		}

		// 14. Calculate Delete Quotas per Customer-SN Pair
		$pair_cancel_quota = array();
		foreach ($pair_counts as $key => $counts) {
			$pair_cancel_quota[$key] = min($counts['plus'], $counts['minus']);
		}

		// 17. Initialize final arrays
		$item_category = array();
		$item_store = array();
		$item_salesman = array();
		$item_cust = array();
		...
		// 18. Tracker for deletions
		$pair_deleted_so_far = array();

		// 19. Start looping through the rows again.
		foreach ($allRows as $rowData) {
			$snArray = explode(',', $rowData['item_sn']);
			$filteredSNs = array();
			...
			foreach ($snArray as $sn) {
				...
				// --- NEW LOGIC START ---
				// Re-create the key to find the specific quota for THIS customer
				$uniqueKey = $sn . '_' . $rowData['item_cust'];
				// -----------------------

				$quota = isset($pair_cancel_quota[$uniqueKey]) ? $pair_cancel_quota[$uniqueKey] : 0;
				...
				if (!isset($pair_deleted_so_far[$uniqueKey])) {
					$pair_deleted_so_far[$uniqueKey] = array('plus' => 0, 'minus' => 0);
				}

				$type = ($rowData['qty'] > 0) ? 'plus' : 'minus';

				// Check deletion against the CUSTOMER-SPECIFIC quota
				if ($pair_deleted_so_far[$uniqueKey][$type] < $quota) {
					$pair_deleted_so_far[$uniqueKey][$type]++;
					// Delete (Skip)
				} else {
					$filteredSNs[] = $sn;
				}
...
{
	global $fromdate, $todate, $cat_name, $cat_sale, $customer, $conn2, $conn;
...
function salesByRep($sub_system)
{
	...
	$sort_by = isset($_REQUEST['sort_by']) ? $_REQUEST['sort_by'] : 'qty_desc'; // Default sort
	switch ($sort_by) {
		case 'qty_desc':
			$order_by = "CASE WHEN total_qty = 0 THEN 2 ELSE 1 END, total_qty DESC";
			break;
		case 'qty_asc':
			$order_by = "CASE WHEN total_qty = 0 THEN 2 ELSE 1 END, total_qty ASC";
			break;
		case 'name_asc':
			$order_by = "up.username ASC";
			break;
		case 'name_desc':
			$order_by = "up.username DESC";
			break;
		default:
			$order_by = "CASE WHEN total_qty = 0 THEN 2 ELSE 1 END, total_qty DESC";
	}
	...
	if (!empty($_REQUEST['users'])) {
		// Use selected users
		$selected_users = array_map('intval', $_REQUEST['users']);
		$rep_id_list = implode(',', $selected_users);

		$query = "SELECT up.id, up.username FROM userprofile up WHERE up.id IN ($rep_id_list) AND up.`status`='0' $sub_system_qry_user";
		// echo "<!-- DEBUG: resp list 1 : $query -->";
	} else {
		$query = "SELECT up.id,up.username FROM userprofile up WHERE up.sales_rep='1' AND up.`status`='0' $sub_system_qry_user";
		// echo "<!-- DEBUG: resp list 2 : $query -->";
	}
	$result = mysqli_query($conn2, $query);
	while ($row = mysqli_fetch_array($result)) {
		$rep_id[] = $row[0];
		...
		// $rep_id_list .= $row[0] . ',';
		$total_rep[] = 0;
		// echo "<!-- DEBUG: rep added: {$row[0]} -->";
	}
	// Build the list AFTER the loop from the array
	$rep_id_list = implode(',', $rep_id);
	// echo "<!-- DEBUG: reps id list 2  : $rep_id_list -->";
	...
	if ($category != '') {
		...
		while ($row = mysqli_fetch_array($result)) {
			...
			$rep_id_list = empty($rep_id_list) ? '0' : $rep_id_list;
			...
			// DEBUGGING OUTPUT
			// echo "<!-- DEBUG: Item ID: $itm_id_tmp -->";
			// echo "<!-- DEBUG: Rep ID List: $rep_id_list -->";
			// echo "<!-- DEBUG: Query1: $query1 -->";
			...
			while ($row1 = mysqli_fetch_array($result1)) {
				...
				// echo "<!-- DEBUG: Rep {$row1[0]} sold: {$row1[1]} for item $itm_id_tmp -->";
				// $debug_count++;
			}
			// echo "<!-- DEBUG: Found $debug_count sales records for item $itm_id_tmp -->";
		}
	}
}
...
{
	global $sm1_id, $sm1_date, $sm1_invdate, $sm1_invdue, $sm1_inv, $sm1_sup, $sm1_amount, $ms2_des, $ms2_qty, $ms2_c_price, $ms2_unic, $sm3_soldprice, $sm3_soldqty, $acc_id, $acc_name, $sp_id, $sp_user, $sp_type, $sp_journal, $sp_pay_date, $sp_sys_date, $sp_ref, $sp_amount, $conn2;
	$ms2_des = $sp_id = array();
	if (isset($_GET['id'])) {
		$id = $_GET['id'];
		$unic_cal = unicCal();
...
function addShipPayment($case)
{
	...
	try {
		...
		if ($out) {
			...
			if (!$lastid_temp) {
				throw new Exception('Error: Shipment pay inseting error');
			}
		}
		if ($out) {
		...
		if ($out) {
			if (isQuickBooksActive(1)) {
				$qb_status = $vendorID = null;
				$shipmentQuery = "SELECT sm.`qb_id`, a.`qb_account_id`, s.`name`, sm.`qb_status`
                  FROM `shipment_main` sm
                  JOIN `supplier` s ON sm.`supplier` = s.`id`
                  JOIN `accounts` a ON s.`name` = a.`name`
                  WHERE sm.`id` = '$shipment_no'
                    AND sm.`status` = 0
                    AND sm.`qb_status` IS NOT NULL";

				// Execute the query
				$shipmentResult = mysqli_query($conn, $shipmentQuery);
				if (!$shipmentResult) {
					throw new Exception("Error: Failed to execute shipment query: " . mysqli_error($conn));
				}

				// Fetch the row and verify returned data
				$shipmentRow = mysqli_fetch_row($shipmentResult);
				if (!$shipmentRow || count($shipmentRow) < 4 || empty($shipmentRow[3])) {
					throw new Exception("Error: Invalid or missing shipment data for shipment_no $shipment_no");
				}

				$vendorID = $shipmentRow[1];
				$vendorName = $shipmentRow[2];
				$qb_status = $shipmentRow[3];
				...
				// Query for the first account details
				$result1 = mysqli_query($conn, "SELECT `qb_account_id`,`name` FROM accounts WHERE `id`='$from_account'");
				if (!$result1) {
					throw new Exception("Error: Failed to execute account query for 'from_account': " . mysqli_error($conn));
				}
				$row = mysqli_fetch_assoc($result1);
				if (!$row || empty($row['qb_account_id']) || empty($row['name'])) {
					throw new Exception("Error: No valid qb_account_id or name found for from_account id $from_account");
				}
				$qb_from_account_id = $row['qb_account_id'];
				$qb_from_account_name = $row['name'];

				// Query for the Accounts Payable account details
				$accountQuery2 = "SELECT `qb_account_id` FROM `accounts` WHERE `name` = 'Accounts Payable (A/P)' AND `status` = 1";
				$accountResult2 = mysqli_query($conn, $accountQuery2);
				if (!$accountResult2) {
					throw new Exception("Error: Failed to execute account query for Accounts Payable: " . mysqli_error($conn));
				}

				$accountRow = mysqli_fetch_row($accountResult2);
				if (!$accountRow || empty($accountRow[0])) {
					throw new Exception("Error: No valid qb_account_id found for Accounts Payable (A/P)");
				}
				$accountsPayableID = $accountRow[0];

				$amount = abs(floatval($amount));
				// $qb_status : shipment has to be pending or already recoded in qb to pay, other are null (before active qb shipments)
				if (($qb_status !== null && $qb_status != '') && ($vendorID !== null && $vendorID != '')) {
					$journal_entries = [
						[
							"posting_type" => "Debit",
						...
						[
							...
							"amount" => abs($amount),
							"description" => "[SHIPMENT PAY] Shipment No : $shipment_no, Ref: $ref",
							"class_id" => $classID,
							"department_id" => $departmentID
						]
					];
					...
					foreach ($journal_entries as $entry) {
						...
						$query = "INSERT INTO qb_queue (`batch_id`,`action`, `shipment_no`, `shipment_pay_id`, `posting_type`, `account_id`, `account_name`, `amount`, `description`, `created_at`, `class_id`, `department_id`, `entity_type`, `entity_id`, `entity_name`)
								VALUES ('$batch_id','$action_name', '$shipment_no', '$lastid_temp', '$posting_type', '$account_id', '$account_name', '$amount', '$description','$today',
									" . ($class_id !== null ? "'$class_id'" : "NULL") . ",
									" . ($department_id !== null ? "'$department_id'" : "NULL") . ",
									" . ($entity_type !== null ? "'$entity_type'" : "NULL") . ",
									" . ($entity_id !== null ? "'$entity_id'" : "NULL") . ",
					...
					$query2 = "UPDATE `shipment_pay` SET `qb_status`='0' WHERE `id`='$lastid_temp'";
					$result2 = mysqli_query($conn, $query2);
					if (!$result2) {
						$message = "MySQL Error while updating shipment_pay: " . mysqli_error($conn);
						throw new Exception($message);
					}
				}
			}
...
function deleteShipPayment()
{
	...
	$pass = $_GET['pass'];
	$store = $_COOKIE['store'];
	$journal_id = $qb_msg = $qb_status = $qb_result = '';
	$out = true;
	...
	try {
		...
		if (isQuickBooksActive(1)) {
			...
			$queryResult = mysqli_query($conn, "SELECT `qb_status` FROM shipment_pay WHERE `id`='$id'");
			if (!$queryResult) {
				throw new Exception("Query failed: " . mysqli_error($conn));
			}
			$rowData = mysqli_fetch_assoc($queryResult);
			$quickbooksStatus = $rowData['qb_status'];

			if ($quickbooksStatus !== null && $quickbooksStatus != '') {
				$shipmentQuery = "SELECT sm.`qb_id`, a.`qb_account_id`, s.`name` FROM shipment_main sm
							...
							WHERE sm.`id` = '$shipment_no'";
				$shipmentRowResult = mysqli_query($conn, $shipmentQuery);
				if (!$shipmentRowResult) {
					throw new Exception("Query failed: " . mysqli_error($conn));
				}
				$shipmentRow = mysqli_fetch_row($shipmentRowResult);
				$vendorID = $shipmentRow[1];
				$vendorName = $shipmentRow[2];
				...
				if (($vendorID != '') && ($qb_from_account_id != '')) {
					$journal_entries = [
						[
							"posting_type" => "Credit",
							"account_id" => $accountsPayableID,
					...
					foreach ($journal_entries as $entry) {
						...
						$query = "INSERT INTO qb_queue (`batch_id`,`action`, `shipment_no`, `shipment_pay_id`, `posting_type`, `account_id`, `account_name`, `amount`, `description`, `created_at`, `class_id`, `department_id`, `entity_type`, `entity_id`, `entity_name`)
							VALUES ('$batch_id','$action_name', '$shipment_no', '$id', '$posting_type', '$account_id', '$account_name', '$amount', '$description','$time_now',
								" . ($class_id !== null ? "'$class_id'" : "NULL") . ",
								" . ($department_id !== null ? "'$department_id'" : "NULL") . ",
								" . ($entity_type !== null ? "'$entity_type'" : "NULL") . ",
								" . ($entity_id !== null ? "'$entity_id'" : "NULL") . ",
					...
					$out = false;
					$qb_msg = "Quickbooks error: Debit account not registered in QB or vendor not registered in QB.";
					throw new Exception($qb_msg);
				}
			}
		}

		// SUM queries and UPDATE fully_paid
		$inventorySumQuery = "SELECT SUM(cost*added_qty) FROM inventory_shipment WHERE shipment_no='$shipment_no'";
		$inventorySumResult = mysqli_query($conn, $inventorySumQuery);
		if (!$inventorySumResult) {
			throw new Exception("Inventory sum query failed: " . mysqli_error($conn));
		}
		$inventorySumRow = mysqli_fetch_row($inventorySumResult);
		$ship_total = $inventorySumRow[0];

		$paymentSumQuery = "SELECT SUM(amount) FROM shipment_pay WHERE shipment_no='$shipment_no'";
		$paymentSumResult = mysqli_query($conn, $paymentSumQuery);
		if (!$paymentSumResult) {
			throw new Exception("Payment sum query failed: " . mysqli_error($conn));
		}
		$paymentSumRow = mysqli_fetch_row($paymentSumResult);
		$total_paid = $paymentSumRow[0];

		if ($ship_total != $total_paid) {
			$updateFullyPaidQuery = "UPDATE `shipment_main` SET `fully_paid`='0' WHERE `id`='$shipment_no'";
			$updateFullyPaidResult = mysqli_query($conn, $updateFullyPaidQuery);
			if (!$updateFullyPaidResult) {
				$updateErrorMessage = 'Error: Failed to update fully_paid status | 604';
		...
		$shipmentPayResult = mysqli_query($conn, "DELETE FROM shipment_pay WHERE `id`='$id'");
		if (!$shipmentPayResult) {
			$message = 'Error: Record cannot be deleted from shipment_pay | 603';
			throw new Exception($message);
		}

		$journalMainResult = mysqli_query($conn, "DELETE FROM journal_main WHERE `journal_id`='$journal_id'");
		if (!$journalMainResult) {
			$message = 'Record cannot be deleted from journal_main | 602';
			throw new Exception($message);
		}
...
function chquePendingFinalyze2()
{
	...
	$and_clause = " AND ((ct.`from` = '$user_id' AND ct.`status` IN(1,5) AND ct.`latest` = 1)
	OR
	(ct.`to` = '$user_id' AND ct.`status` IN(6) AND ct.`latest` = 1))";
	...
	AND py.`chque_return`=0 AND py.`payment_type`=2 AND py.`status`=0 AND py.`chque_clear`=0 AND date(py.chque_date)<='$todate' $storefilter $salesmanfilter ORDER BY py.chque_date,cu.name";
...
function getChqueData2($sub_system)
{
	...
	$and_clause = " ((ct.`from` = '$user_id' AND ct.`status` IN(1,5) AND ct.`latest` = 1)
	OR
	(ct.`to` = '$user_id' AND ct.`status` IN(6) AND ct.`latest` = 1)) AND";
	$where_cluase = " ct.`payment_id` = py.`id` AND py.`payment_date` > '$start_date' AND ";
	$from_clause = ", cheque_trans ct";
...
function clearChque()
{
	...
	try {
		...
		if ($out) {
			if (isQuickBooksActive(1)) {
				$query = "SELECT py.invoice_no, cu.qb_cust_id, py.amount, py.cust, py.chque_no, py.chque_branch, bk.bank_code, cu.`name`, s.`qb_id`
				FROM payment py
				LEFT JOIN cust cu ON py.cust = cu.id
				LEFT JOIN bill_main bm ON bm.invoice_no = py.invoice_no
				LEFT JOIN bank bk ON py.chque_bank = bk.id
				JOIN stores s ON s.id = py.store
				WHERE py.id = '$id' AND py.`qb_status` IS NOT NULL";
				$row = mysqli_fetch_row(mysqli_query($conn, $query));
				if (!empty($row)) {
					$invoice_no = $row[0];
					$amount = $row[2];
					...
					$creditAccountName = "Undeposited Funds";
					$description = "[PAYMENT] - Payment No: $id, Method: Cheque Payment ($bnkac_name), Check Number $cheque_no";
					if ($invoice_no != 0) {
						$description .= ", Invoice No: $invoice_no";
					}
					...
					if (isset($journalEntryForCheque['error'])) {
						...
						$action_name = 'clear_cheque_insert';
						foreach ($journalEntryForCheque as $entry) {
							$posting_type = mysqli_real_escape_string($conn, $entry['posting_type']);
							$account_id = mysqli_real_escape_string($conn, $entry['account_id']);
							$account_name = mysqli_real_escape_string($conn, $entry['account_name']);
	...
	if (isset($_POST['data'])) {
		try {
			...
			foreach ($depositData as $item) {
				...
				if ($bnkac_id != "") {
					$query = "SELECT id,name FROM accounts WHERE `id`='$bnkac_id'";
					$result = mysqli_query($conn, $query);
					$row = mysqli_fetch_assoc($result);
					$ac_id = isset($row['id']) ? $row['id'] : '';
					$bnkac_name = isset($row['name']) ? $row['name'] : '';
				}

				if (empty($ac_id)) {
					$out = false;
					$msg = "Error: Bank account not found for payment no : $id";
				...
				if ($out) {
					if (isQuickBooksActive(1)) {
						...
						LEFT JOIN bill_main bm ON bm.invoice_no = py.invoice_no
						JOIN stores s ON s.id = py.store
						LEFT JOIN bank bk ON py.chque_bank = bk.id WHERE py.id = '$id' AND py.`qb_status` IS NOT NULL";
						$row = mysqli_fetch_row(mysqli_query($conn, $query));
						if (!empty($row)) {
							$invoice_no = $row[0];
							$amount = $row[2];
							...
							if (isset($journalEntryForCheque['error'])) {
								...
								$action_name = 'clear_cheque_through_payment_deposit_insert';
								foreach ($journalEntryForCheque as $entry) {
									$posting_type = mysqli_real_escape_string($conn, $entry['posting_type']);
									$account_id = mysqli_real_escape_string($conn, $entry['account_id']);
									$account_name = mysqli_real_escape_string($conn, $entry['account_name']);
...
function getClearedChques()
{
	...
	$query = "SELECT date(py.payment_date),py.invoice_no,py.chque_no,bk1.bank_code,py.chque_branch,bk1.name,py.chque_date,cu.name,py.chque_deposit_date,bk2.name,up1.username,up2.username,py.amount,py.id
			...
			WHERE py.chque_bank = bk1.id AND py.chque_deposit_bank = bk2.id AND py.`cust` = cu.id AND py.salesman = up1.id AND py.chque_deposit_by = up2.id AND py.payment_type = '2' AND py.`status` = '0' AND py.chque_clear = '1' AND py.chque_return = '0'
			$sub_sys_qry $qry_year $qry_bnk $qry_date ORDER BY py.chque_date DESC";
	$result = mysqli_query($conn2, $query);
	while ($row = mysqli_fetch_array($result)) {
		$py_date[] = $row[0];
		$invoice[] = $row[1];
...
{
	global $type, $id, $inv_billed_by, $inv_type_id, $inv_store, $inv_sms, $inv_odr_date, $inv_inv_date, $inv_total, $status_out, $inv_status, $status_color, $inv_pay_id, $inv_pay_date, $inv_pay_typen, $inv_pay_typec, $inv_pay_amount, $inv_pay_by, $pay_date, $recovery_agent, $conn2;
	...
	if ($type == 'bill_no') {
		...
		$query1 = "SELECT py.id,date(py.payment_date),py.payment_type,py.amount,up.username FROM payment py, userprofile up WHERE py.salesman=up.id AND py.`status` IN(0,2) AND py.invoice_no='$id'";
		//		$row = mysqli_fetch_row(mysqli_query($conn2, $query));
		$result1 = mysqli_query($conn2, $query1);
		while ($row1 = mysqli_fetch_array($result1)) {
			$inv_pay_id[] = $row1[0] ?? '';
...
function deleteBillPayment()
{
	...
	try {
		...
		if (isset($_POST['id'])) {
			...
			throw new Exception("Error: Payment ID is null.");
		}
		if (isset($_POST['master_pw4'])) {
			$master_pw4 = $_POST['master_pw4'];
		} else {
			throw new Exception("Error: Master Password is null.");
		}

		if (isset($_COOKIE['user_id'])) {
			$delete_by = $_COOKIE['user_id'];
		} else {
		...
		if (!isset($_POST['id']) || !filter_var($_POST['id'], FILTER_VALIDATE_INT)) {
			throw new Exception("Error: Invalid or missing Payment ID.");
		}

		// Check if master password is set and not empty.
		if (!isset($_POST['master_pw4']) || empty(trim($_POST['master_pw4']))) {
			throw new Exception("Error: Master Password is required.");
		}

		// Check if user_id cookie is set and is a valid integer.
		if (
			!isset($_COOKIE['user_id']) || !filter_var($_COOKIE['user_id'], FILTER_VALIDATE_INT)
		) {
			throw new Exception("Error: User session is invalid or has expired.");
		}

		// --- 2. Sanitize All Inputs ---
		// This is the most critical step when not using prepared statements.
		$py_no = mysqli_real_escape_string($conn, $_POST['id']);
		$master_pw4 = mysqli_real_escape_string($conn, $_POST['master_pw4']);
		$delete_by = mysqli_real_escape_string($conn, $_COOKIE['user_id']);
		$time_now_safe = mysqli_real_escape_string($conn, $time_now);

		// --- 3. Verify Master Password ---
		$query_pw = "SELECT `value` FROM settings WHERE setting='master_pw4'";
		$result_pw = mysqli_query($conn, $query_pw);
		$row_pw = mysqli_fetch_row($result_pw);

		// Use hash_equals for a timing-attack-safe comparison.
		if (!$row_pw || ($row_pw[0] != $master_pw4)) {
			throw new Exception("Error: Invalid master password (04)");
		}

		// --- 4. Proper Transaction Handling ---
		mysqli_begin_transaction($conn);

		// --- 5. Verify Payment Exists and is Deletable ---
		$query_payment = "SELECT `bill_pay`, `status` FROM payment WHERE id='$py_no'";
		$result_payment = mysqli_query($conn, $query_payment);
		$payment_row = mysqli_fetch_assoc($result_payment);

		if (!$payment_row) {
			throw new Exception("Error: Payment with this ID does not exist.");
		}
		if ($payment_row["status"] == 1) {
			throw new Exception("Error: This payment has already been deleted.");
		}
		if ($payment_row["bill_pay"] != 1) {
			throw new Exception("Error: You can only delete bill payments.");
		}

		// --- 6. Perform Soft Delete ---
		// Corrected variable from `$user_id` to `$delete_by`.
		$query_update = "UPDATE `payment` SET `status`='1', `deleted_by`='$delete_by', `deleted_date`='$time_now_safe' WHERE `id`='$py_no'";
		$result_update = mysqli_query($conn, $query_update);

		if (!$result_update) {
			// If the query fails, throw an exception to trigger the rollback.
			throw new Exception("Error: Payment could not be deleted.");
		}

		if (isQuickBooksActive(1)) {
			$journalEntryForDeletedPayment = [];
			$paymentQuery = "SELECT p.`invoice_no`, p.`id`, p.`payment_type`, p.`qb_id`, c.`name`, c.`qb_cust_id`, p.`amount`, p.`chque_clear`, p.`chque_return`, p.`chque_deposit_bank`, p.`bank_trans`, s.`qb_id` AS `store_qb_id`
				FROM `payment` p, `cust` c, stores s
				WHERE c.`id` = p.`cust` AND s.`id` = p.`store` AND p.`id`='$py_no' AND p.`qb_status` IS NOT NULL AND p.`bill_pay` = 1";
					...
					if ($paymentRow['payment_type'] == 3) { // Bank
						...
						if (isSalesmanPaymentDepositActive()) {
							...
							if (mysqli_num_rows($depositLinkResult) > 0) {
								...
								$bankPaymentDetailsQuery = "SELECT p.`id`, p.`payment_type`, p.`qb_id`, c.`name` AS customer_name, c.`qb_cust_id`, p.`amount`, p.`chque_clear`, p.`chque_return`, p.`chque_deposit_bank`, p.`bank_trans` AS bank_account_id
																								FROM `payment` p JOIN `cust` c ON c.`id` = p.`cust` WHERE p.`id`='$payment_id'
								...
								if (mysqli_num_rows($bankPaymentDetailsResult) > 0) {
									...
									$bankAccountName = null; // Initialize
...
function adjustQty()
{
	...
	try {
		...
		if ($out && isQuickBooksActive(1)) {
			...
			$journalEntryForQtyAdjust = buildJournalEntry(
				$conn,
				abs($amount),
				$debitAccountName,
				$creditAccountName,
			...
			if (isset($journalEntryForQtyAdjust['error'])) {
				...
				$action_name = 'adjust_qty_insert';
				foreach ($journalEntryForQtyAdjust as $entry) {
					$posting_type = mysqli_real_escape_string($conn, $entry['posting_type']);
					$account_id = mysqli_real_escape_string($conn, $entry['account_id']);
					$account_name = mysqli_real_escape_string($conn, $entry['account_name']);
					$amount = mysqli_real_escape_string($conn, $entry['amount']);
					...
					$query = "INSERT INTO qb_queue (`batch_id`, `action`, `posting_type`, `account_id`, `account_name`, `amount`, `created_at`, `description`, `class_id`, `department_id`, `entity_type`, `entity_id`)
							VALUES ('$batch_id','$action_name', '$posting_type', '$account_id', '$account_name', '$amount', '$time_now', '$description',
								" . ($class_id !== null ? "'$class_id'" : "NULL") . ",
								" . ($department_id !== null ? "'$department_id'" : "NULL") . ",
								" . ($entity_type !== null ? "'$entity_type'" : "NULL") . ",
								" . ($entity_id !== null ? "'$entity_id'" : "NULL") . ")";
...
{
	global $item, $from_date, $to_date, $ie_date, $ie_item, $ie_old_qty, $ie_action_qty, $ie_user, $comment, $conn2;
	if (isset($_GET['item_id'])) {
		$item_id = $_GET['item_id'];

		$ie_date = array();
		include('config.php');
		$query1 = "SELECT ie.`datetime`,itm.description,ie.action_qty,up.username,ie.comment FROM inventory_edit ie, inventory_items itm, userprofile up WHERE ie.item=itm.id AND ie.edit_by=up.id AND date(ie.`datetime`) AND itm.id='$item_id' ORDER BY ie.id DESC LIMIT $count";
		$result1 = mysqli_query($conn2, $query1);
		while ($row1 = mysqli_fetch_array($result1)) {
			$ie_date[] = $row1[0];
			$ie_item[] = $row1[1];
...
function getDetailTaxReport($sub_system)
{
	global $from_date, $to_date, $total_tax, $invoice_no, $billed_time, $invoice_total, $tax_amount, $billed_cust, $billed_time, $billed_by, $group, $lock_req,
		$bills_total, $tax_amount_total, $gross_amount, $gross_amount_total, $conn2;
...
{
	global $cheque_name_id, $cheque_name, $conn;
...
// updated by nirmal 16_07_2025 (added reject reason)
function changePaymentDepositStatusAjax($sub_system)
{
	...
	try {
		...
		if (!isset($_POST['deposit_id'], $_POST['status'], $_POST['reason'])) {
			throw new Exception("Invalid input parameters (deposit_id, status or reject reason is missing).");
		}

		$deposit_id = mysqli_real_escape_string($conn, $_POST['deposit_id']);
		$status = mysqli_real_escape_string($conn, $_POST['status']);
		$reject_reason = mysqli_real_escape_string($conn, $_POST['reason']);

		if (strlen($reject_reason) > 255) {
			throw new Exception("Reject reason is too long. Maximum 255 characters allowed.");
		}
		...
		// Fetch deposit, linked payment, customer, and approved_by data in one query
		// Using LEFT JOINs where a linked record might not exist
		$query = "SELECT
							pd.`type`,             -- 0
							pd.`payment_id`,       -- 1 (Can be NULL if deposit doesn't link to a payment)
							pd.`transfer_source`,  -- 2
							up.`username`,         -- 3 (placed_by username)
							pd.`amount`,           -- 4 (deposit amount)
							pd.`bank_id`,          -- 5 (used for type 1 cash deposits)
							pd.`approved_by`,      -- 6
							p.`cust`,              -- 7 (customer ID from payment, Can be NULL)
							p.`amount`,            -- 8 (payment amount, Can be NULL)
							p.`qb_status`,         -- 9 (Can be NULL)
							p.`bank_trans`,        -- 10 (for type 2 bank payments, Can be NULL)
							p.`invoice_no`,        -- 11 (Can be NULL)
							p.`comment`,           -- 12 (Can be NULL)
							p.`bill_pay`,          -- 13 (Can be NULL)
							cu.`name`,              -- 14 (customer name, Can be NULL)
							cu.`qb_cust_id`,       -- 15 (customer QuickBooks ID)
							s.`qb_id`              -- 16 (store QuickBooks ID)
					FROM
							payment_deposit pd
					JOIN userprofile up ON pd.`placed_by` = up.`id`
					LEFT JOIN stores s ON up.`store` = s.`id`
					LEFT JOIN payment p ON pd.`payment_id` = p.`id`
					LEFT JOIN cust cu ON p.`cust` = cu.`id`
					WHERE
							pd.`id`='$deposit_id'
					LIMIT 1";
		...
		// Fetch the row (or null if no row found)
		$row = mysqli_fetch_row($result);

		// --- Check if the deposit record was found at all ---
		if (!$row) {
			throw new Exception("Deposit record with ID $deposit_id not found.");
		}

		// --- Assign variables from the fetched row based on column index ---
		$type = $row[0];
		$payment_id = $row[1];       // Can be NULL
		...
		// --- QuickBooks Logic ---
		// Only proceed if QuickBooks is active
		if (isQuickBooksActive(1)) {
			if ($storeQBID === null || $storeQBID === '') {
				throw new Exception("Error: Store QuickBooks ID not found");
			}
			...
			// --- Case 1: Cash Deposit to Bank (Type 1) ---
			if ($type == 1 && $transfer_source == 2 && $status == 2) {
				if ($bank_id === null || $bank_id === '') {
					throw new Exception("Bank ID is missing for cash deposit $deposit_id.");
				}

				$accountQuery = "SELECT `name` FROM `accounts` WHERE `id`='$bank_id'";
				$accountResult = mysqli_query($conn, $accountQuery);
				if ($accountResult === false) {
					throw new Exception("Database query failed fetching account name: " .
						mysqli_error($conn));
				}
				$accountRow = mysqli_fetch_assoc($accountResult);

				if (!$accountRow) {
					throw new Exception("Bank account with ID $bank_id not found for deposit $deposit_id.");
				}
				$bank_name = $accountRow['name'];
				...
				if (isset($journalEntryForBankDeposit['error'])) {
					...
					$action_name = "cash_on_hand_insert_through_payment_deposit_insert";
					$batch_id = generateBatchID();
					if (!$batch_id) {
						throw new Exception("Error: Failed to generate a valid batch ID.");
					}
					...
					if (!is_array($journalEntryForBankDeposit)) {
						throw new Exception("buildJournalEntry did not return a valid array for cash deposit $deposit_id.");
					}
					...
					foreach ($journalEntryForBankDeposit as $entry) {
						if (
							...
							throw new Exception("Invalid structure in journal entry array from buildJournalEntry for cash deposit $deposit_id.");
						}
			...
			// --- Case 2: Bank Payment Approval (Type 2) ---
			if ($type == 2 && $transfer_source == 2 && $status == 2 && $payment_id !== null && $qb_status !== null) {
				// Validate payment amount
				if (!is_numeric($payment_amount) || $payment_amount <= 0) {
					throw new Exception("Linked payment $payment_id has an invalid amount ($payment_amount) for QB processing.");
				}
				// Note: Adjust these validations depending on how qb_cust_id is populated.
				if ($cust === null || $qb_cust_id === null) {
					throw new Exception("Linked payment $payment_id is missing customer or QB Customer ID for processing.");
				}
				if ($tr_bank === null) {
					throw new Exception("Linked payment $payment_id is missing bank transfer account ID.");
				}

				// Fetch bank account name from linked payment's bank_trans ID
				$accountNameQuery = "SELECT `name` FROM accounts WHERE id='$tr_bank'";
				$accountNameResult = mysqli_query($conn, $accountNameQuery);
				if ($accountNameResult === false) {
					throw new Exception("Database query failed fetching payment account name: " . mysqli_error($conn));
				}
				$accountNameRow = mysqli_fetch_row($accountNameResult);

				if (!$accountNameRow) {
					throw new Exception("Bank account (ID $tr_bank) for payment $payment_id not found in accounts table.");
				}
				$debitAccountName = $accountNameRow[0];
				$creditAccountName = 'Accounts Receivable (A/R)'; // Standard AR account

				if ($bill_pay == 1) {
					$description = '[BILL PAYMENT] ';
				} else {
					$description = '[PAYMENT] - Payment No: ' . $payment_id;
				}
				$description .= ", Method: Bank Payment ($debitAccountName)";
				if (!empty($comment)) {
				...
				$description .= ", Customer: " . (!empty($custName) ? $custName : 'N/A');
				...
				if (is_array($result_array)) {
					$action_name = "bank_deposit_insert_through_payment_deposit_insert";
					$batch_id = generateBatchID();
					if (!$batch_id) {
						throw new Exception("Error: Failed to generate a valid batch ID.");
					}
					...
					if (!is_array($result_array)) {
						throw new Exception("processQBPayment did not return a valid array for bank payment $payment_id.");
					}

					foreach ($result_array as $entry) {
						if (
							!isset($entry['posting_type'], $entry['account_id'], $entry['account_name'], $entry['amount'], $entry['description'])
						) {
							throw new Exception("Invalid structure in journal entry array from processQBPayment for bank payment $payment_id.");
						}
						...
						if (!mysqli_query($conn, $insert_query)) {
							$message = "MySQL Error while inserting bank payment QB entry into qb_queue: " .
								mysqli_error($conn);
							throw new Exception($message);
						}
					}

					// After successfully queuing QB entries, update the linked payment status
					$updatePaymentQuery = "UPDATE `payment` SET `status` = 0 WHERE `id`='$payment_id'";
					$result1 = mysqli_query($conn, $updatePaymentQuery);

					if ($result1 === false) {
						$message = 'Failed to update linked payment status: ' . mysqli_error($conn);
						throw new Exception($message);
					}
					...
					$qb_msg = $result_array;
					throw new Exception("QB Payment Processing Error: " . $qb_msg);
				}
			}
		} else {
			// QuickBooks is NOT active
			// --- Case 2 (No QB): Bank Payment Approval (Type 2) ---
			if ($type == 2 && $transfer_source == 2 && $status == 2) {
				if ($payment_id !== null) {
					$updatePaymentQuery =
						"UPDATE `payment` SET `status` = 0 WHERE `id`='$payment_id'";
					$result1 = mysqli_query($conn, $updatePaymentQuery);

					if ($result1 === false) {
						$message = 'Failed to update linked payment status (QB inactive): ' .
							mysqli_error($conn);
						throw new Exception($message);
					}
					if (mysqli_affected_rows($conn) > 0) {
					...
					$message = 'Failed to find linked payment id';
					throw new Exception($message);
				}
			}
		}

		// --- Deposit Status Update Logic ---
		$updateDepositQuery = null;

		if ($status == 2) {
			// Approved
			if (empty($approved_by)) {
				$updateDepositQuery = "UPDATE payment_deposit SET `status` = '$status',`user_id` = '$user_id', `approved_by` = '$user_id' WHERE id = '$deposit_id'";
			} else {
				$updateDepositQuery = "UPDATE payment_deposit SET `status` = '$status',`user_id` = '$user_id' WHERE id = '$deposit_id'";
			}
		} else if ($status == 3) {
			// Rejected
			$updateDepositQuery = "UPDATE payment_deposit SET `status` = '$status', `transfer_note` = '$reject_reason', `user_id` = '$user_id' WHERE id = '$deposit_id'";
			$bank_payment_updated = false;
		} else {
			throw new Exception("Invalid status.");
		}

		if ($updateDepositQuery !== null) {
			if (!mysqli_query($conn, $updateDepositQuery)) {
				$message = 'Failed to update deposit status: ' . mysqli_error($conn);
				throw new Exception($message);
			}
		} else {
			throw new Exception("Warning: No deposit status update query generated for deposit ID: $deposit_id");
		}

		// Commit the transaction if all queries succeeded
		mysqli_commit($conn);

		// --- Send SMS when bank payment updated ---
		if ($bank_payment_updated === true) {
			if ($payment_id !== null && $cust !== null && $payment_amount !== null) {
				sendPaymentSMS($payment_id, $cust, $payment_amount);
			} else {
				$qb_msg .= " (Could not send SMS - missing payment details)";
			}
		}
		...
		echo json_encode([
			'success' => true,
			'message' => 'Status updated successfully. ' . $qb_msg
		]);
	} catch (Exception $e) {
		mysqli_rollback($conn);
		$message = $e->getMessage();
...
// update by nirmal 25_03_2025 (added rejected username to reviewd column)
function getBankDepositsReport($sub_system)
{
	global $payment_id, $invoice_no, $cust_name, $payment_amount, $bank_name, $comment, $payment_date, $payment_status, $payment_type, $payment_transfer_date, $payment_deposit_id,
		$payment_placed_by, $payment_reviewed_by, $fromdate, $todate, $payment_rejected_by, $payment_checked, $payment_note;
	$payment_id = $invoice_no = $cust_name = $payment_amount = $bank_name = $comment = $payment_date = $payment_status = $payment_type = $payment_transfer_date = $payment_deposit_id =
		$payment_placed_by = $payment_reviewed_by = $payment_rejected_by = $payment_checked = $payment_note = [];
	include('config.php');
	$user_id = $_COOKIE['user_id'];
	...
	$query = "SELECT py.`id`, py.`invoice_no`, cu.`name`, py.`amount`, ac.`name`, py.`comment`, py.`payment_date`, pd.`status`, pd.`type`, pd.`transfer_date`, pd.`id`,
	 u.`username`, up.`username`, uu.`username`, pd.`checked`, pd.`transfer_note`
	FROM payment_deposit pd
	JOIN payment py ON pd.`payment_id` = py.`id`
	JOIN cust cu ON py.`cust` = cu.`id`
	JOIN userprofile up ON pd.`placed_by` = up.`id`
	...
	LEFT JOIN userprofile uu ON pd.`user_id` = uu.id
	...
	while ($row = mysqli_fetch_array($result)) {
		...
		$payment_rejected_by[] = $row[13];
		$payment_checked[] = $row[14];
		$payment_note[] = $row[15];
	}
}

function updatePaymentAsChecked()
{
	include('config.php');
	...
	if (($systemid == 1) && ($_COOKIE['user_id'] == 4)) {
		...
		$query = "UPDATE payment_deposit SET checked = ? WHERE id = ?";
		$stmt = mysqli_prepare($conn, $query);
		mysqli_stmt_bind_param($stmt, "ii", $checked, $depositId);
		...
		echo json_encode(['success' => false, 'error' => "Unautherozied request"]);
	}

	mysqli_close($conn);
}

// update by nirmal 25_03_2025 (fixed received bank name show insetad name, (transfer to bank name))
function getCashDepositsReport($sub_system)
{
	global $payment_id, $payment_type, $payment_amount, $payment_received, $payment_source, $payment_note, $payment_date, $payment_status, $payment_placed_by, $payment_reviewed_by,
		$payment_image, $fromdate, $todate, $payment_reject_by;
	include('config.php');
	$payment_id = $payment_type = $payment_amount = $payment_received = $payment_source = $payment_note = $payment_date = $payment_status = $payment_placed_by = $payment_reviewed_by =
		$payment_image = $payment_reject_by = array();
	$user_id = $_COOKIE['user_id'];
	...
	$query = "SELECT pd.`id`, pd.`type`, pd.`amount`, pd.`transfer_date`, pd.`transfer_source`, pd.`transfer_note`, pd.`status`, up.`username`, uu.`username`, pd.`image`,u.`username`,
    CASE
        WHEN pd.`type` = 1 AND pd.`transfer_source` = 2 THEN a.name
        WHEN pd.`user_id` IS NOT NULL THEN u.username
        WHEN pd.`bank_id` IS NOT NULL THEN a.name
        ELSE 'Unknown'
    END AS bank_or_user_name
	...
	LEFT JOIN accounts a ON pd.`bank_id` = a.id
	WHERE pd.`type` = '1'
	$source_qry $status_qry $user_qry $reviewed_by_qry $bank_qry $date_qry
	ORDER BY pd.`transfer_date` DESC";
	...
	while ($row = mysqli_fetch_array($result)) {
		...
		$payment_image[] = $row[9];
		$payment_reject_by[] = $row[10];
		$payment_received[] = $row[11];
	}
}

// updated by nirmal 24_02_2025 (removed date filter)
function getCashOnHandReport($sub_system)
{
	global $amount_to_settle, $payment_out_trans, $user_id, $user_name, $fromdate, $todate, $payment_in_trans;
	$amount_to_settle = $payment_out_trans = $user_id = $user_name = $payment_in_trans = [];
	$payment_cash_total_sum = $payment_deposit_total_sum = $payment_deposit_to_user_sum = 0;
	$date_qry = '';
	include('config.php');
	$start_date = salesmanPaymentDepositStartDate();
	// 0=canceled, 1=pending, 2=accepted, 3=rejected, 4=banked
	...
	while ($row = mysqli_fetch_array($result)) {
		...
		$query1 = "SELECT SUM(amount) FROM payment WHERE `salesman` = '$row[0]' AND `status` = 0 AND `payment_type` = 1 AND payment_date > '$start_date'";
		$result1 = mysqli_query($conn2, $query1);
		$row1 = mysqli_fetch_row($result1);
		$payment_cash_total_sum = $row1[0];

		// accepted only
		$query2 = "SELECT SUM(amount) FROM payment_deposit WHERE `type` = 1 AND `placed_by` = '$row[0]' AND `status` IN(1,2)";
		$result2 = mysqli_query($conn2, $query2);
		$row2 = mysqli_fetch_row($result2);
		$payment_deposit_total_sum = $row2[0];

		// cash receive to user
		$query3 = "SELECT SUM(amount) FROM payment_deposit WHERE `type` = 1 AND `transfer_source` = 1 AND `user_id` = '$row[0]' AND `status` IN(2)";
		$result3 = mysqli_query($conn2, $query3);
		$row3 = mysqli_fetch_row($result3);
		$payment_deposit_to_user_sum = $row3[0];

		// payment out trans
		$query4 = "SELECT SUM(amount) FROM payment_deposit WHERE `type` = 1 AND `placed_by` = '$row[0]' AND `status` IN(1)";
		$result4 = mysqli_query($conn2, $query4);
		$row4 = mysqli_fetch_row($result4);
		$payment_out_trans[] = $row4[0];

		// payment in trans, not accepted yet
		$query5 = "SELECT SUM(amount) FROM payment_deposit WHERE `type` = 1 AND `transfer_source` = 1 AND `user_id` = '$row[0]' AND `status` IN(1)";
		$result5 = mysqli_query($conn2, $query5);
		$row5 = mysqli_fetch_row($result5);
		$payment_in_trans[] = $row5[0];
...
function getReturnAvailability($sub_system)
{
	global $return_availability, $customer, $conn2;
	include('config.php');

	if (isset($_REQUEST['customer_id'])) {
		$customer_id = $_REQUEST['customer_id'];

		$query = "SELECT c.name FROM cust c WHERE c.id = '$customer_id' LIMIT 1";
		$result = mysqli_query($conn2, $query);
		$row = mysqli_fetch_assoc($result);
		$customer = $row['name'];
		...
		$query = "SELECT
						...
						inv.code,
						inv.description,
						COALESCE(s.sold_pos_qty, 0)   AS sold_pos_qty,
						COALESCE(s.sold_neg_qty, 0)   AS sold_neg_qty,
						COALESCE(r.packed_qty,   0)   AS packed_qty,
						COALESCE(r.unpacked_qty, 0)   AS unpacked_qty,
						GREATEST(
							COALESCE(s.sold_pos_qty, 0)
							+ COALESCE(s.sold_neg_qty, 0)
							- COALESCE(r.packed_qty,   0)
							- COALESCE(r.unpacked_qty, 0),
							0
					...
					LEFT JOIN (
						SELECT
							bi.item                        AS item,
							SUM(IF(bi.qty > 0, bi.qty, 0)) AS sold_pos_qty,
							SUM(IF(bi.qty < 0, bi.qty, 0)) AS sold_neg_qty
						FROM bill_main AS bm
						JOIN bill      AS bi
							ON bm.invoice_no = bi.invoice_no
						WHERE bm.cust   = '$customer_id'
							AND bm.status != '0'
							AND bm.lock   = '1'
						GROUP BY bi.item
					) AS s
					...
					LEFT JOIN (
						SELECT
							rt.replace_item                  AS item,
							SUM(IF(rt.odr_packed = 1, rt.qty, 0)) AS packed_qty,
							SUM(IF(rt.odr_packed = 0, rt.qty, 0)) AS unpacked_qty
						FROM `return`     AS rt
						JOIN return_main AS rm
							ON rt.invoice_no = rm.invoice_no
						WHERE rm.cust   = '$customer_id'
							AND rm.status != '0'
						GROUP BY rt.replace_item
					) AS r
						ON inv.id = r.item
					WHERE s.item IS NOT NULL
						OR r.item IS NOT NULL
					ORDER BY inv.description;";
		$result = mysqli_query($conn2, $query);
		while ($row = mysqli_fetch_assoc($result)) {
			// $item_description[] = $row[1];
			// $item_sold_pos_qty[] = $row[2];
			// $item_sold_neg_qty[] = $row[3];
			// $item_packed_qty[] = $row[4];
			// $item_unpacked_qty[] = $row[5];
			// $item_available_to_return[] = $row[6];
			$return_availability[] = [
				'code' => $row['code'],
				'description' => $row['description'],
				'sold_positive_qty' => (int) $row['sold_pos_qty'],
				'sold_negative_qty' => (int) $row['sold_neg_qty'],
				'return_packed_qty' => (int) $row['packed_qty'],
				'return_unpacked_qty' => (int) $row['unpacked_qty'],
				'return_available_qty' => (int) $row['available_to_return'],
			];
		}
	}
}
...
function getItemListMGR($sub_system)
{
	...
	// Selecting distinct items that are active
	$query = "SELECT id, description FROM inventory_items WHERE status=1 $sub_sys_qry ORDER BY description";
	$result = mysqli_query($conn2, $query);
	$item_id_list = array();
	$item_name_list = array();
	while ($row = mysqli_fetch_array($result)) {
		$item_id_list[] = $row[0];
		$item_name_list[] = $row[1];
	}
}

function itemMonthlySales($sub_system)
{
	global $report_data, $month_headers, $cust_names, $item_req, $date1, $date2,
		$town_req, $salesman_req, $group_req, $grand_totals_month, $conn2;

	$report_data = array();
	$month_headers = array();
	$cust_names = array();
	$grand_totals_month = array();

	include('config.php');

	// 1. Inputs & Defaults
	$item_req = isset($_REQUEST['item_id']) ? $_REQUEST['item_id'] : '';
	$town_req = isset($_REQUEST['town']) ? $_REQUEST['town'] : 'all';
	$salesman_req = isset($_REQUEST['salesman']) ? $_REQUEST['salesman'] : 'all';
	$group_req = isset($_REQUEST['group']) ? $_REQUEST['group'] : 'all';

	// Date defaults (First day of this month, Last day of this month if not set)
	if (isset($_REQUEST['date1']) && isset($_REQUEST['date2'])) {
		$date1 = $_REQUEST['date1'];
		$date2 = $_REQUEST['date2'];
	} else {
	...
	// 2. Generate Month Headers (Logic to create columns: Jan, Feb, Mar...)
	...
	foreach ($period as $dt) {
		$key = $dt->format("Y-m"); // Key for data matching
		$label = $dt->format("M Y"); // Label for display
		$month_headers[$key] = $label;
		$grand_totals_month[$key] = 0; // Initialize totals
	}

	// 3. Build Query ONLY if an item is selected
	if ($item_req != '') {

		$filters = "";

		// Town Filter
		if ($town_req != 'all' && $town_req != '') {
			$filters .= " AND cu.associated_town = '$town_req'";
		}

		// Salesman Filter
		if ($salesman_req != 'all' && $salesman_req != '') {
			$filters .= " AND bm.billed_by = '$salesman_req'";
		}

		// Group Filter
		if ($group_req != 'all' && $group_req != '') {
			$filters .= " AND cu.associated_group = '$group_req'";
		}

		// Sub System
		if ($sub_system != 'all') {
			$filters .= " AND bm.sub_system = '$sub_system'";
		}

		// The Query
		// We group by Customer and Year-Month
		// $query = "SELECT
        //             cu.id AS cust_id,
        //             cu.name AS cust_name,
        //             DATE_FORMAT(bm.billed_timestamp, '%Y-%m') as sale_month,
        ...
        //             JOIN cust cu ON bm.cust = cu.id
        //             JOIN userprofile up ON bm.billed_by = up.id
        //             JOIN district di ON bm.billed_district = di.id
        //           WHERE
        //             bi.item = '$item_req'
        //             AND date(bm.billed_timestamp) BETWEEN '$date1' AND '$date2'
        //             AND bm.status NOT IN (0,7)
        //             AND bm.exclude = 0
        //             $filters
        //           GROUP BY
        //             cu.id, DATE_FORMAT(bm.billed_timestamp, '%Y-%m')
        //           ORDER BY
        //             cu.name ASC, bm.billed_timestamp ASC";
		$query = "SELECT
            cu.id AS cust_id,
            cu.name AS cust_name,
            DATE_FORMAT(bm.billed_timestamp, '%Y-%m') as sale_month,
          ...
          GROUP BY
            cu.id, DATE_FORMAT(bm.billed_timestamp, '%Y-%m')
          ORDER BY
            cu.name ASC, sale_month ASC";
		// -- Exclude deleted/cancelled (bm.status NOT IN (0,7))
		...
		while ($row = mysqli_fetch_assoc($result)) {
			...
			// Save Customer Name
			if (!isset($cust_names[$c_id])) {
				$cust_names[$c_id] = $row['cust_name'];
			}

			// Store Qty
			$report_data[$c_id][$month] = $qty;

			// Add to grand total
			if (isset($grand_totals_month[$month])) {
				$grand_totals_month[$month] += $qty;
			}
		}
	}
}
// newly added by hirusha using report module
