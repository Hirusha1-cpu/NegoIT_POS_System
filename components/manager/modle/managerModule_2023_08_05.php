...
function getCust($sub_system, $status){
	...
	else  $sub_sys_qry = "AND cu.`sub_system`='$sub_system'";
	...
	while ($row = mysqli_fetch_array($result)) {
		$cu_id[] = $row[0];
		if ($row[5] == 1) $cu_id0[] = $row[0];
		$cu_name[] = wordwrap($row[1], 25, "<br />\n");
		if ($row[5] == 1) $cu_name0[] = $row[1];
		$cu_nic[] = $row[2];
		$cu_mobile[] = $row[3];
		$cu_cr_limit[] = $row[4];
		$cu_status[] = $row[5];
...
function getSalesman($sub_system){
	...
	if ($sub_system == 'all') $sub_system_qry = '';
	else $sub_system_qry = "AND `sub_system`='$sub_system'";
	include('config.php');
...
function getCustGroups($sub_system){
	...
	if ($sub_system == 'all') $sub_system_qry = '';
	else $sub_system_qry = "WHERE `sub_system`='$sub_system'";
	$gp_id = array();
	include('config.php');
...
function getSoldQty($sub_system, $component){
	...
	if (isset($_GET['category'])){
		...
		if ($category!='all') $category_qry = "AND itc.id='$category'";
	}
	if (isset($_GET['store'])){
		$store = $_GET['store'];
		if ($store!='all') $store_qry = "AND st.id='$store'";
	}
...
function sms4($systemid, $cust_id){
	...
	if (($sms_cust == 1) && ($sms_balance > 0) && ($_SERVER['SERVER_NAME'] == inf_url_primary()) && ($approved_date == $date_now) && (strpos($mobile, "7") == 1)) {
		...
		if (trim($res[0]) == "OK") $mailstatus = true;
		else $mailstatus = false;
		//----------------------------------------------------------------//
		if ($mailstatus) {
			$sms_balance--;
...
function getOneCust($mode, $sub_system){
	...
	if (isset($_REQUEST['id'])) {
		...
		if ($cu_master_cust != 0) $refer_cust[] = $cu_master_cust;
...
function cust2Ajax($mode, $sub_system){
	...
	if (isset($_POST['val'])) {
		...
		include('config.php');
		$query = "SELECT id,name,nic,mobile,credit_limit,cust_name,nickname,shop_address,shop_tel,home_address,home_tel,email,email_enable,sms,`status`,associated_store,associated_salesman,associated_group,associated_town,image1,image2,image3,image4,created_by,created_timestamp,approved_by,approved_timestamp,sub_system,master_cust,gps_x,gps_y,dob FROM cust WHERE `name`='$cust2_name' $sub_sys_qry ORDER BY sub_system DESC";
		$result = mysqli_query($conn2, $query);
		while ($row = mysqli_fetch_array($result)) {
...
// added by E.S.P Nirmal 28_04_2022
function addCust($systemid){
	...
	if (isset($_POST['email_alert'])) $email_alert = 1;
	else $email_alert = 0;

	if (isset($_POST['sms'])) $sms0 = 1;
	else $sms0 = 0;

	if (isset($_POST['gps_x'])) $gps_x = $_POST['gps_x'];
	else $gps_x = 0;

	if (isset($_POST['gps_y'])) $gps_y = $_POST['gps_y'];
	else $gps_y = 0;
	...
	} else $approve_qry1 = $approve_qry2 = '';
	...
	/*
	if (strpos($mobile, '07') === 0) $sms = $sms0;
	else $sms = 0;
	*/
	#$systemid = inf_systemid(1);

	if ($status == 3) $msg = 'Custommer was created and the Approval is pending';
	else $msg = 'Customer was added successfully!';
	$id = '';
	$out = true;
	...
	if(!validateMobileNo($mobile)){
		$out = false;
		//$message = 'Invalid mobile number! Mobile number has ' . strlen($mobile) . ' letters';
		$message = 'Invalid mobile number!';
		$jasonArray["mobile"] = $message;
	}

	// added by E.S.P Nirmal 21_06_25
	...
	if($out){ // added by nirmal 29_04_2022
		if($dob != ''){
			...
			if(!($dt && $dt->format('Y-m-d') === $dob)){
				$out = false;
				$message = 'Date of Birth is not in yyyy-mm-dd format!';
			}
		}
	}
	if ($out) {
		if($dob != ''){  // added by nirmal 29_04_2022
			$query = "INSERT INTO `cust` (`associated_store`,`associated_salesman`,`associated_group`,`associated_town`,`name`,`nic`,`mobile`,`credit_limit`,`cust_name`,`nickname`,`shop_address`,`shop_tel`,`home_address`,`home_tel`,`email`,`email_enable`,`sms`,`gps_x`,`gps_y`,`created_by`,`created_timestamp`,`sub_system`,`master_cust`,`dob`,`status` $approve_qry1) VALUES ('$cu_store','$sm_id','$cu_group','$cu_town','$shop_name','$nic','$mobile','$cr_limit','$custname','$nickname','$shop_address','$shop_tel','$home_address','$home_tel','$email_add','$email_alert','$sms','$gps_x','$gps_y','$created_by','$created_timestamp','$sub_system','$master_cust','$dob','$status' $approve_qry2)";
		}else{
			$query = "INSERT INTO `cust` (`associated_store`,`associated_salesman`,`associated_group`,`associated_town`,`name`,`nic`,`mobile`,`credit_limit`,`cust_name`,`nickname`,`shop_address`,`shop_tel`,`home_address`,`home_tel`,`email`,`email_enable`,`sms`,`gps_x`,`gps_y`,`created_by`,`created_timestamp`,`sub_system`,`master_cust`,`status` $approve_qry1) VALUES ('$cu_store','$sm_id','$cu_group','$cu_town','$shop_name','$nic','$mobile','$cr_limit','$custname','$nickname','$shop_address','$shop_tel','$home_address','$home_tel','$email_add','$email_alert','$sms','$gps_x','$gps_y','$created_by','$created_timestamp','$sub_system','$master_cust','$status' $approve_qry2)";
		}
		$result = mysqli_query($conn, $query);
		...
		if ($result) {
			...
			if ($approve_qry2 != '') {
				if ($systemid == 1) sms4($systemid, $cust_id);
				if ($systemid == 2) sms4($systemid, $cust_id);
				if ($systemid == 14) sms4($systemid, $cust_id);
			}
			$out = true;
		...
		if (!$result) {
			$msg = 'Customer could not be created!';
			$msg = $query;
...
// added by E.S.P Nirmal 2021_06_11
...
// added by E.S.P Nirmal 2022_1_3
function addCustImage($systemid){
			...
			if ($_FILES["fileToUpload" . $i]["name"] != '') {
				...
				if ($uploadOk == 0) {
					...
					if (move_uploaded_file($_FILES["fileToUpload" . $i]["tmp_name"], $destination_file)) {
						...
						$message = 'Customer is created | ' . $msg;
						return false;
					}
				}
			}
	...
	if ($file_upload == 0) {
		...
		if ($imagesNotChoosed == 0 && $file_upload == 0) {
			...
			if (($upload1 == 1) && ($image1 == 1))	$imgqry1 = ",`image1`='1'";
			if (($upload2 == 1) && ($image2 == 1))	$imgqry2 = ",`image2`='1'";
			if (($upload3 == 1) && ($image3 == 1))	$imgqry3 = ",`image3`='1'";
			if (($upload4 == 1) && ($image4 == 1))	$imgqry4 = ",`image4`='1'";
			...
			if ($result) {
				...
				$message = 'Customer is created | ' .  'Error: Sorry, there was an error while uploading your file';
				return false;
			}
		}
	} else {
...
// update by nirmal 28_04_2022
function updateCust(){
	global $message;
	$cust_id = $_POST['cust_id'];
	$shop_name = ucwords($_POST['shop_name']);
	$shop_name = preg_replace("/[^A-Za-z0-9-+,. ]/", '', $shop_name);
	$nic = $_POST['nic'];
	$mobile = $_POST['mobile'];
	$cr_limit = $_POST['cr_limit'];
	...
	$cu_town = $_POST['cu_town'];
	$dob = $_POST['dob'];
	$custname0 = ucwords(strtolower($_POST['customer']));
	$custname = preg_replace("/[^A-Za-z0-9-+,. ]/", '', $custname0);
	$custname = ucwords($custname);
	$nikname0 = ucwords(strtolower($_POST['nikname']));
	$nikname = preg_replace("/[^A-Za-z0-9-+,. ]/", '', $nikname0);
	...
	if ($out) {
		...
		for ($i = 1; $i <= 4; $i++) {
			if ($_FILES["fileToUpload" . $i]["name"] != '') {
				...
				// Allow certain file formats
				if ($imageFileType != "jpg" && $imageFileType != "jpeg") {
					$msg = "Sorry, only JPEG files are allowed.";
					$uploadOk = 0;
				}
				// Check if $uploadOk is set to 0 by an error
				if ($uploadOk == 0) {
		...
		if ($file_upload == 0) {
			...
			if($_POST['dob'] == ''){
				$query = "UPDATE `cust` SET `name`='$shop_name',`nic`='$nic',`mobile`='$mobile',`credit_limit`='$cr_limit',`associated_store`='$cu_store',`associated_salesman`='$sm_id',`associated_group`='$cu_group',`associated_town`='$cu_town',`cust_name`='$custname',`nickname`='$nikname',`shop_address`='$shop_address',`shop_tel`='$shop_tel',`home_address`='$home_address',`home_tel`='$home_tel',`email`='$email_add',`email_enable`='$email_alert',`sms`='$sms',`sub_system`='$sub_system',`status`='$cust_type' $approved_qry $imgqry1 $imgqry2 $imgqry3 $imgqry4 WHERE `id`='$cust_id' ";
			}else{
				$query = "UPDATE `cust` SET `name`='$shop_name',`nic`='$nic',`mobile`='$mobile',`credit_limit`='$cr_limit',`associated_store`='$cu_store',`associated_salesman`='$sm_id',`associated_group`='$cu_group',`associated_town`='$cu_town',`cust_name`='$custname',`nickname`='$nikname',`shop_address`='$shop_address',`shop_tel`='$shop_tel',`home_address`='$home_address',`home_tel`='$home_tel',`email`='$email_add',`email_enable`='$email_alert',`sms`='$sms',`sub_system`='$sub_system',`dob`='$dob',`status`='$cust_type' $approved_qry $imgqry1 $imgqry2 $imgqry3 $imgqry4 WHERE `id`='$cust_id' ";
			}
			$result = mysqli_query($conn, $query);
			...
			if (($upload1 == 1) && ($image1 == 1)) unlink('images/customerdata/' . $systemid . '/' . str_pad($cust_id . '_1', 10, "0", STR_PAD_LEFT) . '.jpg');
			if (($upload2 == 1) && ($image2 == 1)) unlink('images/customerdata/' . $systemid . '/' . str_pad($cust_id . '_2', 10, "0", STR_PAD_LEFT) . '.jpg');
			if (($upload3 == 1) && ($image3 == 1)) unlink('images/customerdata/' . $systemid . '/' . str_pad($cust_id . '_3', 10, "0", STR_PAD_LEFT) . '.jpg');
			if (($upload4 == 1) && ($image4 == 1)) unlink('images/customerdata/' . $systemid . '/' . str_pad($cust_id . '_4', 10, "0", STR_PAD_LEFT) . '.jpg');
			if (($upload1 == 1) && ($image1 == 0)) unlink('images/customerdata/' . $systemid . '/uploads/' . str_pad($cust_id . '_1', 10, "0", STR_PAD_LEFT) . '.jpg');
			if (($upload2 == 1) && ($image2 == 0)) unlink('images/customerdata/' . $systemid . '/uploads/' . str_pad($cust_id . '_2', 10, "0", STR_PAD_LEFT) . '.jpg');
			if (($upload3 == 1) && ($image3 == 0)) unlink('images/customerdata/' . $systemid . '/uploads/' . str_pad($cust_id . '_3', 10, "0", STR_PAD_LEFT) . '.jpg');
			if (($upload4 == 1) && ($image4 == 0)) unlink('images/customerdata/' . $systemid . '/uploads/' . str_pad($cust_id . '_4', 10, "0", STR_PAD_LEFT) . '.jpg');
			$message = $msg;
			return false;
		}
	} else {
...
function deleteCust(){
	...
	if ($bill_count == 0) {
		...
		if ($result) {
			if ($image1 == 1) unlink('images/customerdata/' . $systemid . '/' . str_pad($id . '_1', 10, "0", STR_PAD_LEFT) . '.jpg');
			if ($image2 == 1) unlink('images/customerdata/' . $systemid . '/' . str_pad($id . '_2', 10, "0", STR_PAD_LEFT) . '.jpg');
			if ($image3 == 1) unlink('images/customerdata/' . $systemid . '/' . str_pad($id . '_3', 10, "0", STR_PAD_LEFT) . '.jpg');
			if ($image4 == 1) unlink('images/customerdata/' . $systemid . '/' . str_pad($id . '_4', 10, "0", STR_PAD_LEFT) . '.jpg');
...
function dailySale($store, $sub_system)
{
	...
	if ($store == 'all') $storesearch = '';
	else $storesearch = "AND st.id='" . $store . "'";
	if ($group == 'all') $groupsearch = '';
	else $groupsearch = "AND gp.id='" . $group . "'";
	if ($_REQUEST['salesman'] == 'all') $salesmansearch = '';
	else $salesmansearch = "AND up.`id`='" . $_REQUEST['salesman'] . "'";
	if ($_REQUEST['processby'] == 'all') $processbysearch = '';
	else $processbysearch = "AND bm.`packed_by`='" . $_REQUEST['processby'] . "'";
	...
	if (isset($_REQUEST['lock'])) {
		...
		if ($lock_req == 'all') {
			...
			if ($lock_req == 0) $lock_qry = "AND bm.`lock` IN (0,2)";
			else $lock_qry = "AND bm.`lock`=$lock_req";
		}
	} else {
		$lock_req = 1;
	...
	if ($type_req != 3 && $type_req != 5) {
		...
		$query = "SELECT bm.invoice_no,round(SUM(bi.qty*bi.unit_price),2),di.name,up.username,cu.name,bm.billed_timestamp,st.name,SUM(bi.discount),bm.`cust`,up.id,bm.gps_x,bm.gps_y FROM bill bi, bill_main bm, userprofile up, district di, cust cu, stores st, cust_group gp WHERE bm.store=st.id AND cu.`associated_group`=gp.id AND bm.`cust`=cu.id AND bi.invoice_no=bm.invoice_no AND bm.billed_by=up.id AND bm.billed_district=di.id AND bm.`status` NOT IN (0,7) AND bm.exclude=0 $sub_sys_bmqry $cashback_qry $lock_qry $type_qry $date_qry1 $storesearch $groupsearch $salesmansearch $processbysearch GROUP BY bm.invoice_no ORDER BY st.name, bm.billed_timestamp";
		$result = mysqli_query($conn2, $query);
		while ($row = mysqli_fetch_array($result)) {
			$invoice_no_id = $row[0];
			$invoice_no[] = $row[0];
			...
			if (isset($graph_total[$key])) $total = $graph_total[$key];
			else $total = 0;
			...
			if (!isset($payment_card_tr[$invoice_no_id])) $payment_card_tr[$invoice_no_id] = '';
	...
	$query2 = "SELECT py.id,round(py.amount),py.payment_type,up.username,cu.name,py.payment_date, st.name,py.chque_no,bk.bank_code,py.chque_branch,py.chque_date,bk.name,py.`cust`,up.id,py.gps_x,py.gps_y,py.bank_trans,py.card_no FROM userprofile up, cust cu, stores st, cust_group gp, payment py LEFT JOIN bank bk ON py.chque_bank=bk.id WHERE py.salesman=up.id AND py.cust=cu.id AND py.store=st.id AND cu.`associated_group`=gp.id AND py.bill_pay=2 AND py.`status`=0 $sub_sys_pyqry $date_qry2 $pycashback_qry $storesearch $groupsearch $salesmansearch ORDER BY st.name, py.payment_date";
	$result2 = mysqli_query($conn2, $query2);
	while ($row2 = mysqli_fetch_array($result2)) {
		if ($row2[0] > 0) {
	...
	if ($type_req == 5 || $type_req == '') {
		$query3 = "SELECT wa.id,wa.cust_pay_date,wa.cust_pay_amount,up.username,st.name FROM warranty wa, userprofile up, stores st WHERE wa.store=st.id AND wa.cust_pay_by=up.id AND wa.`status`!=0 AND wa.cust_pay_amount!=0 $sub_sys_waqry $date_qry4 $storesearch $salesmansearch";
		$result3 = mysqli_query($conn2, $query3);
		while ($row3 = mysqli_fetch_array($result3)) {
			$wa_no[] = $row3[0];
			$wa_time[] = $row3[1];
		...
		$query3 = "SELECT wa.id,wa.suplier_pay_date,wa.suplier_pay,up.username,st.name FROM warranty wa, userprofile up, stores st WHERE wa.store=st.id AND wa.suplier_pay_by=up.id AND wa.`status`!=0 AND wa.suplier_pay!=0 $sub_sys_waqry $date_qry5 $storesearch $salesmansearch";
		$result3 = mysqli_query($conn2, $query3);
		while ($row3 = mysqli_fetch_array($result3)) {
			$wa_no[] = $row3[0];
			$wa_time[] = $row3[1];
...
function dailySale2($store, $sub_system)
{
	...
	while ($row2 = mysqli_fetch_array($result2)) {
		if ($row2[0] > 0) {
			...
			$payment_time[] = $row2[5];
			$payment_store[] = $row2[6];
		} else $payment_id = $payment_amount = $payment_type = $payment_salesman = $payment_cust = $payment_time = $payment_store = $payment_chq_details = array();
	}
...
{
	global $customer, $decimal, $fromdate, $todate, $invoice_no, $invoice_Total, $billed_district, $billed_by,
		$billed_date, $billed_time, $billed_store, $payment_cash, $payment_card, $payment_chque, $payment_id,
		$payment_amount, $payment_type, $payment_salesman, $payment_date, $payment_time, $payment_store,
		$credit_balance, $payment_chq_return, $chqpayment_id, $chqpayment_amount, $chqpayment_salesman,
		$chqpayment_cust, $chqrtn_date, $chqrtn_time, $chqpayment_store, $chqpayment_invno, $chq_details, $payment_card_details,
		$payment_chq_details, $chqrtn_chq_details, $payment_bank, $payment_card_tr, $payment_bank_tr,
		$bank_trans, $payment_invoice, $payment_chq_postpone, $billed_chq_return, $billed_chq_postpone, $cust_mtype;

	$invoice_no = $payment_id = $invoice_Total = $payment_cash = $payment_card = $payment_chque =
		$billed_date = $payment_date = $chqrtn_date = $chqpayment_id = $payment_invoice = $payment_card_details = $payment_chq_details = array();

	$acc_arr = $py_inv_no = $py_type = $py_amount = $py_chq_no = $py_bnk_code = $py_chq_br = $py_chq_date =
		$py_bnk_name = $py_bnk_trans = $py_chq_rtn = $py_chq_ppone = $py_card_no = array();

	if ($systemid == 10 || $systemid == 13 || $systemid == 14) {
	...
	if (isset($_REQUEST['customer_id'])) {
		...
		if ($master_cust > 0) {
			...
			$row = mysqli_fetch_row(mysqli_query($conn, $query));
			if ($row[0] > 0)	$cust_mtype = 'primary';
			else $cust_mtype = 'normal';
		}
		//-------------Customer M Type END-------------------//
		$acc_arr[''] = '';
		...
		while ($row = mysqli_fetch_array($result)) {
			...
			//	$key=array_search($invoice_no_id,$py_inv_no);
			//	if($key>-1){
...
function getUnvisited($sub_system)
{
	...
	if (isset($_POST['from_date']) && isset($_POST['to_date'])) {
		...
		if ($_GET['components'] == 'billing') $from_date = date("Y-m-d", time() - (60 * 60 * 24 * 14));
		else
...
{
	global $chq0_id, $chq0_invno, $chq0_cuname, $chq0_amount, $chq0_no, $chq0_bank, $chq0_branch, $chq0_date, $chq0_paymentdate, $chq0_salesman, $chq0_returndate, $chq0_rtn_clear, $chq0_rtn_cle_date, $chq0_code, $salesman_filter;
	if ($sub_system == 'all') {
		$sub_system_qry = "";
	} else {
		$sub_system_qry = "AND py.`sub_system`='$sub_system'";
	...
	include('config.php');
	$query1 = "SELECT py.id,py.invoice_no,cu.name,py.amount,py.chque_no,ba.name,ba.bank_code,py.chque_branch,py.chque_date,py.payment_date,up.username,date(py.chque_return_date),py.chque_rtn_clear,date(py.chque_rtn_cle_date) FROM payment py, bank ba, userprofile up, cust cu WHERE py.cust=cu.id AND py.salesman=up.id AND py.chque_bank=ba.id AND py.`status`=0 AND py.chque_return=1 AND py.chque_rtn_clear IN (0,1) $sub_system_qry ORDER BY py.chque_rtn_clear,py.chque_return_date DESC";
	$result1 = mysqli_query($conn2, $query1);
	while ($row1 = mysqli_fetch_array($result1)) {
		$chq0_id[] = $row1[0];
		if ($row1[1] != 0) $chq0_invno[] = str_pad($row1[1], 7, "0", STR_PAD_LEFT);
		else $chq0_invno[] = '';
		$chq0_cuname[] = $row1[2];
		$chq0_amount[] = $row1[3];
		$chq0_no[] = $row1[4];
		...
		if ($row1[12] == 0) $chq0_rtn_clear[] = 'Pending';
		else $chq0_rtn_clear[] = 'Cleared';
...
{
	global $chq_no, $chq_id, $chq_invno, $chq_cuname, $chq_amount, $chq_bank, $chq_bank_code, $chq_branch, $chq_date, $chq_paymentdate, $chq_salesman, $chq_return, $chq_postpone, $chq_date2;
	$chq_salesman = array();
	if (isset($_REQUEST['chque_no'])) {
		...
		$chq_no = $py_chqno;
		include('config.php');
		$query = "SELECT py.id,py.invoice_no,cu.name,py.amount,ba.name,ba.bank_code,py.chque_branch,py.chque_date,py.payment_date,up.username,py.chque_return,py.chque_postpone,py.chque_date2 FROM payment py, bank ba, userprofile up, cust cu WHERE py.cust=cu.id AND py.salesman=up.id AND py.chque_bank=ba.id AND py.`status`=0 AND py.chque_no='$py_chqno' AND ba.bank_code='$py_chqbank' AND py.chque_branch='$py_branch'";
		$result = mysqli_query($conn, $query);
		while ($row = mysqli_fetch_array($result)) {
			$chq_id = $row[0];
			if ($row[1] != 0) $chq_invno[] = str_pad($row[1], 7, "0", STR_PAD_LEFT);
			else $chq_invno[] = '';
			$chq_cuname[] = $row[2];
			$chq_amount[] = $row[3];
			$chq_bank = $row[4];
...
function getPostponedChque($sub_system)
{
	...
	while ($row1 = mysqli_fetch_array($result1)) {
		...
		$chq0_salesman[] = $row1[10];
		$chq0_date[] = $row1[11];
		if ($row1[12] == 1) $chq0_postpone[] = 'Postponed';
		if ($row1[12] == 2) $chq0_postpone[] = 'Postpone-Clear';
		$chq0_code[] = $row1[4] . '-' . str_pad($row1[6], 4, "0", STR_PAD_LEFT) . '-' . str_pad($row1[7], 3, "0", STR_PAD_LEFT);
	}
	$salesman_filter = array_unique($chq0_salesman);
	$salesman_filter = array_values($salesman_filter);
...
function setChquePostpone($case)
{
	...
	$py_branch = substr($py_chqnofull, $dash2 + 1, $dash3 - $dash2 - 1);
	if ($master_pw3 == '') {
		$message = 'Please fill Date and Passowrd';
		$out = false;
	}
	...
	if ($out) {
		...
		$row = mysqli_fetch_row(mysqli_query($conn, $query));
		if ($row[0] == 0) {
			$message = 'No Cheque Found. Plaese contact support';
			$out = false;
		}
	}
	if ($out) {
		...
		if ($case == 'add') {
			...
			if ($result) {
				...
				$message = 'Error: Cannot Calear the Postpone Status';
				$out = false;
			}
		} else {
...
function getReturnSummary()
{
	...
	else $to_date = date("Y-m-d", time());
	if ($sm != 'all') $sm_qry = "AND rm.return_by='$sm'";
	if ($cu != '') $sm_qry = "AND rm.cust='$cu'";
	include('config.php');
...
function getReturn($display)
{
	...
	$dis_id = array();
	$today = date("Y-m-d", time());
	if ($display == 'unpacked') $packed_qry = "AND rt.odr_packed='0'";
	if (isset($_GET['from_date'])) $from_date = $_GET['from_date'];
	else $from_date = date("Y-m-d", time() - (60 * 60 * 24 * 30));
	if (isset($_GET['to_date'])) $to_date = $_GET['to_date'];
	else $to_date = date("Y-m-d", time());
	...
	while ($row = mysqli_fetch_array($result)) {
		...
		$rtn_cust[] = $row[6];
		$dis_date[] = $row[7];
		if ($row[8] != '') $dis_by[] = $userarr[$row[8]];
		else $dis_by[] = '';
		if ($row[9] == 0) $rtn_st[] = 'Pending';
		else $rtn_st[] = 'Processed';
		if ($cu != '') $cust = $row[6];
	}
}
...
function getDisposal()
{
	...
	while ($row = mysqli_fetch_array($result)) {
		...
		$dis_store[] = $row[3];
		$dis_date[] = $row[4];
		if ($today == $row[4]) $permission[] = true;
...
{
	global $invoice_no, $billed_by, $billed_cust, $date, $time, $store, $lock;
	if ($sub_system == 'all') $sub_system_qry = '';
	else $sub_system_qry = "AND bm.sub_system='$sub_system'";
	$invoice_no = array();
	include('config.php');
	$query = "SELECT DISTINCT bm.invoice_no,up.username,cu.name,DATE(bm.billed_timestamp),TIME(bm.billed_timestamp),st.name,bm.`lock` FROM bill bi ,bill_main bm, userprofile up, cust cu, stores st WHERE bi.invoice_no=bm.invoice_no AND bm.billed_by=up.id AND bm.`cust`=cu.id AND bm.store=st.id AND bm.`lock`!=1 AND bm.`status` NOT IN (0,7) $sub_system_qry ORDER BY st.name, bm.billed_timestamp";
	$result = mysqli_query($conn2, $query);
	while ($row = mysqli_fetch_array($result)) {
		$invoice_no[] = $row[0];
		$billed_by[] = $row[1];
...
function getTemporaryBills($sub_system){
	...
	WHERE bi.`bm_no`=bm.`bm_no` AND bm.`billed_by`=up.`id` AND bm.`cust`=cu.`id` AND bm.`store`=st.`id` AND bm.`status` NOT IN (0,7) $sub_system_qry ORDER BY st.`name`, bi.`date` DESC";
...
{
	global $item_des, $item_qty, $selection, $customer, $customer_id, $store1, $fromdate, $todate, $category, $cat2_id, $cat2_name, $cat2_total_count, $cat2_sold_total;
	$k = 0;
	$case = false;
	$cust_qry = $store_qry = '';
	$sold_catid = $item_des = array();
	if ($sub_system == 'all') $sub_system_qry = '';
	else $sub_system_qry = "AND bm.`sub_system`='$sub_system'";
	if (isset($_REQUEST['selection'])) {
		$selection = $_REQUEST['selection'];
		if ($selection == 'customer') if (isset($_REQUEST['customer_id'])) $case = true;
		if ($selection == 'store') if (isset($_REQUEST['store1'])) $case = true;
	}
	if ($case) {
		if ($selection == 'customer') {
			$customer_id = $_REQUEST['customer_id'];
			if ($customer_id != '') $cust_qry = "AND bm.`cust`='$customer_id'";
		}
		if ($selection == 'store') {
			$store1 = $_REQUEST['store1'];
			if ($store1 != '') $store_qry = "AND st.name='$store1'";
		}
		...
		if ($selection == 'customer') $query = "SELECT itm.description,SUM(bi.qty) FROM bill_main bm, bill bi, inventory_items itm WHERE bm.invoice_no=bi.invoice_no AND bi.item=itm.id AND bm.`status` NOT IN (0,7) AND bm.`lock`=1 AND itm.description!='CHQUE' AND itm.description!='CREDIT' $cust_qry AND bm.`status` NOT IN (0,7) AND bm.exclude=0 AND date(bi.`date`) BETWEEN '$fromdate' AND '$todate' $cat_qry $sub_system_qry GROUP BY bi.item ORDER BY itm.description";
		if ($selection == 'store') $query = "SELECT itm.description,SUM(bi.qty) FROM bill_main bm, bill bi, inventory_items itm, stores st WHERE bm.invoice_no=bi.invoice_no AND bi.item=itm.id AND bm.`store`=st.id AND bm.`status` NOT IN (0,7) AND bm.`lock`=1 AND itm.description!='CHQUE' AND itm.description!='CREDIT' $store_qry AND bm.`status` NOT IN (0,7) AND bm.exclude=0 AND date(bi.`date`) BETWEEN '$fromdate' AND '$todate' $cat_qry $sub_system_qry GROUP BY bi.item ORDER BY itm.description";
		$result = mysqli_query($conn2, $query);
		while ($row = mysqli_fetch_array($result)) {
			$item_des[] = $row[0];
			$item_qty[] = $row[1];
		...
		if ($selection == 'customer') $query = "SELECT itm.category, COUNT(DISTINCT itm.description) FROM bill_main bm, bill bi, inventory_items itm, cust cu WHERE bm.invoice_no=bi.invoice_no AND bi.item=itm.id AND bm.`cust`=cu.id AND bm.`status` NOT IN (0,7) AND bm.`lock`=1 AND itm.description!='CHQUE' AND itm.description!='CREDIT' $cust_qry AND bm.`status` NOT IN (0,7) AND bm.exclude=0 AND date(bi.`date`) BETWEEN '$fromdate' AND '$todate' $cat_qry $sub_system_qry GROUP BY itm.category";
		if ($selection == 'store') $query = "SELECT itm.category, COUNT(DISTINCT itm.description) FROM bill_main bm, bill bi, inventory_items itm, stores st WHERE bm.invoice_no=bi.invoice_no AND bi.item=itm.id AND bm.`store`=st.id AND bm.`status` NOT IN (0,7) AND bm.`lock`=1 AND itm.description!='CHQUE' AND itm.description!='CREDIT' $store_qry AND bm.`status` NOT IN (0,7) AND bm.exclude=0 AND date(bi.`date`) BETWEEN '$fromdate' AND '$todate' $cat_qry $sub_system_qry GROUP BY itm.category";
		$result = mysqli_query($conn2, $query);
		while ($row = mysqli_fetch_array($result)) {
			$sold_catid[] = $row[0];
...
function getSalesSummary($sub_system)
{
	...
	if ($sub_system != 'all') $sub_system_qry = "AND bm.`sub_system`='$sub_system'";
	$today = date("Y-m-d", time());
	if (isset($_GET['from_date'])) $from_date = $_GET['from_date'];
	else $from_date = date("Y-m-d", time() - (60 * 60 * 24 * 30));
	if (isset($_GET['to_date'])) $to_date = $_GET['to_date'];
	...
	if ((isset($_GET['from_date'])) && (isset($_GET['to_date']))) {
		$category = $_GET['category'];
		if ($category != 'all') $cat_qry = "AND itm.category='$category'";
		$store = $_GET['store'];
		if ($store != 'all') $st_qry = "AND bm.store='$store'";
...
{
	global $selection, $customer_id, $store1, $category, $fromdate, $todate, $itm2_id, $itm2_name, $itm2_sold;
	$k = 0;
	$case = false;
	if ($sub_system == 'all') $sub_system_qry = '';
	else $sub_system_qry = "AND bm.`sub_system`='$sub_system'";
	if (isset($_REQUEST['selection'])) {
		$selection = $_REQUEST['selection'];
		if ($selection == 'customer') if (isset($_REQUEST['customer'])) $case = true;
		if ($selection == 'store') if (isset($_REQUEST['store1'])) $case = true;
	}
	if ($case) {
		if ($selection == 'customer') $customer_id = $_REQUEST['customer_id'];
		if ($selection == 'store') $store1 = $_REQUEST['store1'];
		$category = $_REQUEST['category'];
		$fromdate = $_REQUEST['datefrom'];
		$todate = $_REQUEST['dateto'];
		include('config.php');
		if ($selection == 'customer') $query = "SELECT DISTINCT itm.id FROM bill_main bm, bill bi, inventory_items itm WHERE bm.invoice_no=bi.invoice_no AND bi.item=itm.id AND bm.`status` NOT IN (0,7) AND bm.`lock`=1 AND itm.description!='CHQUE' AND itm.description!='CREDIT' AND bm.`cust`='$customer_id' AND bm.`status` NOT IN (0,7) AND bm.exclude=0 AND date(bi.`date`) BETWEEN '$fromdate' AND '$todate' AND itm.`category`='$category' $sub_system_qry ORDER BY itm.description";
		if ($selection == 'store') $query = "SELECT DISTINCT itm.id FROM bill_main bm, bill bi, inventory_items itm, stores st WHERE bm.invoice_no=bi.invoice_no AND bi.item=itm.id AND bm.`store`=st.id AND bm.`status` NOT IN (0,7) AND bm.`lock`=1 AND itm.description!='CHQUE' AND itm.description!='CREDIT' AND st.name='$store1' AND bm.`status` NOT IN (0,7) AND bm.exclude=0 AND date(bi.`date`) BETWEEN '$fromdate' AND '$todate' AND itm.`category`='$category' $sub_system_qry ORDER BY itm.description";
		$result = mysqli_query($conn2, $query);
		while ($row = mysqli_fetch_array($result)) {
			$sold_itemid[] = $row[0];
		}
...
{
	global $fromdate, $todate, $cat_name, $cat_sale;
	if (isset($_REQUEST['datefrom'])) $fromdate = $_REQUEST['datefrom'];
	else $fromdate = date("Y-m-d", time() - 2592000);
	if (isset($_REQUEST['dateto'])) $todate = $_REQUEST['dateto'];
	else $todate = dateNow();
	if ($sub_system == 'all') $sub_system_qry = '';
	else $sub_system_qry = "AND bm.`sub_system`='$sub_system'";
	$cat_name = $cat_sale = array();
	include('config.php');
	$query = "SELECT itc.name,SUM(bi.qty * bi.unit_price) FROM bill_main bm, bill bi, inventory_items itm, item_category itc WHERE bm.invoice_no=bi.invoice_no AND bi.item=itm.id AND itm.category=itc.id AND bm.`status` NOT IN (0,7) AND bm.`lock`=1 $sub_system_qry AND date(bm.billed_timestamp) BETWEEN '$fromdate' AND '$todate' GROUP BY itc.id";
	$result = mysqli_query($conn2, $query);
	while ($row = mysqli_fetch_array($result)) {
		$cat_name[] = $row[0];
		$cat_sale[] = $row[1];
...
function salesByRep($sub_system)
{
	...
	if (isset($_REQUEST['store'])) $store = $_REQUEST['store'];
	else $store = '';
	if ($sub_system == 'all') $sub_system_qry = '';
	else $sub_system_qry = "AND bm.`sub_system`='$sub_system'";
	if ($store == '') {
	...
	include('config.php');
	$query = "SELECT up.id,up.username FROM userprofile up WHERE up.sales_rep='1' AND up.`status`='0'";
	$result = mysqli_query($conn2, $query);
	while ($row = mysqli_fetch_array($result)) {
		$rep_id[] = $row[0];
		$rep_name[] = $row[1];
	...
	if ($category != '') {
		...
		/*
		$query="SELECT itm.id,itm.description,SUM(bi.qty),bm.billed_by FROM bill_main bm, bill bi LEFT JOIN inventory_items itm ON bi.item=itm.id WHERE bm.invoice_no=bi.invoice_no AND itm.category='$category' AND bm.`lock`='1' AND bm.`status`>='3' AND itm.`status`='1' AND bm.billed_by IN ($rep_id_list) AND (date(bm.`billed_timestamp`) BETWEEN '$fromdate' AND '$todate') $sub_system_qry $store_qry GROUP BY itm.id,bm.billed_by ORDER BY itm.description ";
		$result=mysqli_query($conn2,$query);
		while($row=mysqli_fetch_array($result)){
			if($itm_id_tmp!=$row[0]){
				$itm_id0=$row[0];
				...
				$query1="SELECT SUM(qty) FROM inventory_qty WHERE item='$itm_id0' $itqst_qry";
				$row1=mysqli_fetch_row(mysqli_query($conn2,$query1));
				$query1="SELECT SUM(qty) FROM inventory_new WHERE item='$itm_id0' $itnst_qry";
				$row2=mysqli_fetch_row(mysqli_query($conn2,$query1));
				$itm_stock[]=$row1[0]+$row2[0];

			}
			for($i=0;$i<sizeof($rep_id);$i++){
				if(!isset($itm_qty[$row[0]][$rep_id[$i]])){
					$itm_qty[$row[0]][$rep_id[$i]]=0;
				}
				if($rep_id[$i]==$row[3]){
					$itm_qty[$row[0]][$rep_id[$i]]=$row[2];
				}
			}
			$itm_id_tmp=$row[0];
		}
...
function getRepairIncome($sub_system)
{
	...
	$inf_company = inf_company(1);
	if (isset($_GET['datefrom'])) $fromdate = $_GET['datefrom'];
	else $fromdate = date("Y-m-d", time() - 2592000);
	if (isset($_GET['dateto'])) $todate = $_GET['dateto'];
	else $todate = dateNow();
	if ($sub_system == 'all') $sub_system_qry = '';
	else $sub_system_qry = "AND bm.`sub_system`='$sub_system'";
...
function getRepairIncomeOne($sub_system)
{
	...
	if ($type == 'tech') $user_qry = "AND bm.shipped_by='$user'";
	if ($type == 'delivery') $user_qry = "AND bm.deliverd_by='$user'";
...
function getUnicItems()
{
	...
	if (isset($_GET['search_unic'])) {
		...
		if ($_GET['store'] == 'all') $storeqry = '';
		else $storeqry = "AND itq.location='$store'";
	}
	...
	for ($i = 0; $i < sizeof($status_arr); $i++) {
		if ($status != $status_arr[$i]) {
			...
			while ($row = mysqli_fetch_array($result)) {
				...
				if ($status == 4) $return_invoice_no[] = $row[6];
				else $return_invoice_no[] = 0;
			}
		}
	}
...
function snLookupList()
{
	...
	if ((isset($_POST['item_desc'])) || (isset($_POST['item_sn']))) {
		...
		if ($item_sn != '') $qry_sn = "AND itu.sn='$item_sn'";
		else $qry_sn = "AND itm.description='$item_desc'";
		if ($store == 'all') $qry_store = "";
		else $qry_store = "AND itq.location='$store'";
		...
		while ($row = mysqli_fetch_array($result)) {
			...
			if (($i > $row[4]) && (!$unic_cal)) $c_price = $w_price = $r_price = 'New Stock Price Available';
...
function snLookupPriceList()
{
	...
	if ((isset($_POST['max_price'])) || (isset($_POST['min_price']))) {
		...
		if ($key_word == '') $qry_keyword = "";
		else $qry_keyword = "AND itm.description  LIKE '%$key_word%'";
		if ($unic_cal) {
		...
		while ($row = mysqli_fetch_array($result)) {
			...
			$itm_desc = $row[3];
			$store = $row[4];
			if (($row[5] == "") || ($row[5] == 0)) $min_profit = 'No Price Tag';
			else $min_profit = $row[5] + $c_price;
...
function getShipmentList()
{
	...
	if (isset($_POST['ship_no'])) {
		...
		if ($filter_shipno != '') $qry_shipno = "AND sm.id='$filter_shipno'";
		else $qry_shipno = '';
	} else {
		$filter_shipno = '';
		$qry_shipno = '';
	...
	if (isset($_POST['inv_date'])) {
		...
		if ($filter_invdate != '') $qry_invdate = "AND sm.invoice_date='$filter_invdate'";
		else $qry_invdate = '';
	} else {
		$filter_invdate = '';
		$qry_invdate = '';
	...
	if (isset($_POST['inv_no'])) {
		...
		if ($filter_invno != '') $qry_invno = "AND sm.invoice_no LIKE '%$filter_invno%'";
		else $qry_invno = '';
	} else {
		$filter_invno = '';
		$qry_invno = '';
	...
	if (isset($_POST['inv_sup'])) {
		...
		if ($filter_invsup != '') $qry_invsup = "AND su.name LIKE '%$filter_invsup%'";
		else $qry_invsup = '';
	} else {
		$filter_invsup = '';
		$qry_invsup = '';
	...
	$sm_id = array();
	include('config.php');
	$query = "SELECT sm.id,sm.invoice_date,sm.invoice_no,su.name,sm.fully_paid FROM shipment_main sm, supplier su WHERE sm.`supplier`=su.id AND sm.`status`!='3' $qry_shipno $qry_invdate $qry_invno $qry_invsup ORDER BY sm.id DESC LIMIT 50";
	$result = mysqli_query($conn2, $query);
	while ($row = mysqli_fetch_array($result)) {
		$sm_id[] = $row[0];
		$sm_inv_date[] = $row[1];
	...
	if (isset($_GET['id'])) {
		...
		while ($row = mysqli_fetch_array($result)) {
			...
			if (($unic_cal) && ($row[4] == 1)) {
				...
				$row1 = mysqli_fetch_row(mysqli_query($conn2, $query1));
				$ms2_qty[] = $row1[0];
				if ($row1[0] != 0) $ms2_c_price[] = $row1[1] / $row1[0];
				else $ms2_c_price[] = 0;
			} else {
				$ms2_qty[] = $row[2];
				$ms2_c_price[] = $row[3];
...
function addShipPayment($case)
{
	...
	if ($ship_total == $total_paid) $fully_paid = 1;
	else $fully_paid = 0;
	...
	if ($out) {
		...
		if ((!$result1) || (!$result2)) {
			$out = false;
			$message = $word . ' Cannot Be Added | 503';
		}
	}
	if ($out) {
...
function deleteShipPayment()
{
	...
	if ($out) {
		...
		if (!$result) {
			$out = false;
			$message = 'Record Cannot be Deleted | 601';
		}
	}
	if ($out) {
		...
		if (!$result) {
			$out = false;
			$message = 'Record Cannot be Deleted | 603';
		}
	}
	if ($out) {
...
{
	global $todate, $py_id2, $customer2, $payment_amount2, $chq_date2, $payment_date2, $payment_salesman2, $payment_store2, $cheque_no2;
	$sub_system = $_COOKIE['sub_system'];
	$py_id2 = $chq_date = array();
	include('config.php');
	...
	$query2 = "SELECT py.id,cu.name,round(py.amount),date(py.chque_date),date(py.payment_date),up.username,st.name,py.chque_no,bk.bank_code,py.chque_branch FROM payment py, userprofile up, cust cu, stores st, bank bk WHERE py.salesman=up.id AND py.cust=cu.id AND cu.associated_store=st.id AND py.chque_bank=bk.id AND py.`chque_return`=0 AND py.`payment_type`=2 AND py.`status`=0 AND py.`chque_clear`=0 AND py.`sub_system`='$sub_system' AND date(py.chque_date)<='$todate' $storefilter $salesmanfilter ORDER BY py.chque_date,cu.name";
	$result2 = mysqli_query($conn2, $query2);
	while ($row2 = mysqli_fetch_array($result2)) {
		$py_id2[] = $row2[0];
		$customer2[] = $row2[1];
...
function getChqueData($sub_system){
	...
	$payment_date2, $payment_salesman2, $payment_store2, $cheque_no2, $payment_sub_sys;
	$py_id2 = $chq_date = array();
	...
	if (isset($_GET['bnk'])) {
		if ($_GET['bnk'] != '') {
			if($_GET['bnk'] == 1)$dipfilter = 'AND py.chque_deposit_bank is not null';
			else if($_GET['bnk'] == 0)$dipfilter = 'AND py.chque_deposit_bank is null';
			else $dipfilter = '';
		} else $dipfilter = '';
	} else $dipfilter = '';
	...
	if (isset($_GET['dateto'])) {
		$chque_total = 0;
		$query2 = "SELECT cu.name,round(py.amount),date(py.chque_date),date(py.payment_date),up.username,st.name,py.`sub_system`,py.`chque_no`,bk.`bank_code`,py.`chque_branch`,py.`invoice_no` FROM
		payment py, userprofile up, cust cu, stores st, bank bk WHERE
		py.salesman=up.id AND py.cust=cu.id AND cu.associated_store=st.id AND py.chque_bank=bk.id AND py.`chque_return`=0 AND py.`payment_type`=2 AND py.`status`=0 $dipfilter
	...
	$query2 = "SELECT py.id,cu.name,round(py.amount),date(py.chque_date),date(py.payment_date),up.username,st.name,py.chque_no,bk.bank_code,py.chque_branch
	FROM payment py, userprofile up, cust cu, stores st, bank bk
	WHERE py.salesman=up.id AND py.cust=cu.id AND cu.associated_store=st.id AND py.chque_bank=bk.id AND
...
function clearChque(){
	...
	if ($out) {
		...
		if (!$result) {
			$out = false;
			$msg = "Error: DB Cound not be Updated";
		}
	}
...
function getClearedChques(){
	...
	$chque_bnk_name, $chque_date, $cust, $deposit_date, $deposit_bnk, $salesman, $deposit_by, $amount;
	$sub_system = $_COOKIE['sub_system'];
	$qry_bnk = $qry_date = $qry_year =$filter_year= '';
	$py_date=array();

	if (isset($_REQUEST['year'])) $filter_year = $_REQUEST['year'];
	if ($filter_year != '') $qry_year = "AND py.chque_date LIKE '$filter_year%'";

	if ((isset($_REQUEST['from_date'])) && (isset($_REQUEST['to_date']))) {
		$from_date = $_REQUEST['from_date'];
		$to_date = $_REQUEST['to_date'];
		if (($from_date != '') && ($to_date != '')) $qry_date = " AND py.`chque_deposit_date` BETWEEN '$from_date' AND '$to_date'";
	}

	if ((isset($_REQUEST['bnk']))) {
		$bnk = $_REQUEST['bnk'];
		if ($bnk != '') $qry_bnk = "AND bk2.id ='$bnk'";
	}

	include('config.php');
	$query = "SELECT date(py.payment_date),py.invoice_no,py.chque_no,bk1.bank_code,py.chque_branch,bk1.name,py.chque_date,cu.name,py.chque_deposit_date,bk2.name,up1.username,up2.username,py.amount
			FROM payment py, cust cu, bank bk1, accounts bk2, userprofile up1 , userprofile up2
			WHERE py.chque_bank=bk1.id AND py.chque_deposit_bank=bk2.id AND py.`cust`=cu.id AND py.salesman=up1.id AND py.chque_deposit_by=up2.id AND py.payment_type='2' AND py.`status`='0' AND py.chque_clear='1' AND py.`sub_system`='$sub_system' $qry_year
...
function getChqueRange($sub_system){
	...
	if ((isset($_REQUEST['from_date'])) && (isset($_REQUEST['to_date']))) {
		...
		while ($row = mysqli_fetch_array($result)) {
			...
			$cust[] = $row[6];
			$deposit_date[] = $row[7];
			if ($row[8] != '') $deposit_bnk[] = $acc_name[$row[8]];
			else $deposit_bnk[] = '';
			$salesman[] = $up_name[$row[9]];
			if ($row[10] != '') $deposit_by[] = $up_name[$row[10]];
			else $deposit_by[] = '';
			$amount[] = $row[11];
			$chque_clear[] = $row[12];
			$chque_return[] = $row[13];
...
function addPayment($sub_system)
{
	...
	$result = mysqli_query($conn, $query);
	if ($result) $out = true;
	else $msg = 'The Payment could not be Submited !';
	...
	if ($out) {
		if (($payment_type == 2) && ($cheque_py != '')) {
			...
			$result1 = mysqli_query($conn, $query1);
			if ($result1) $out = true;
			else $msg = 'Error: Cust payment table could not be updated. Please contact NegoIT !';
		}
	}
...
function deletePayment($sub_system)
{
	...
	if (($pay_sub_system == $sub_system) && ($py_status == 1)) {
		...
		$result1 = mysqli_query($conn, $query1);
		if ($result1) $out = true;
		else $msg = 'Error: The Payment Could Not Be Deleted';
...
function getQtyMgmt()
{
	...
	if (isset($_GET['item_id'])) {
		...
		if ($unic == 0 && $pr_sr == 1) $approve_edit = true;
	}
}
...
function adjustQty()
{
		...
		if ($tag == 'inv') {
			...
			if ($inn_qty > 0) {
				if (($qty + $qty_adj) > 0) {
					...
					$message = 'New Inventory Available; Therefore, you cannot set 0 for Inventory Qty';
				}
			} else if ($inn_qty == 0) {
				if (($qty + $qty_adj) >= 0) {
					$query1 = "UPDATE `inventory_qty` SET `qty`=qty+$qty_adj WHERE `id`='$id'";
					...
					$message = 'Invalid Quantity in New Inventory (-)';
				}
			}
		} else if ($tag == 'inn') {
	...
	if ($out) {
		...
		if ($result2) {
			...
			$out = false;
			$message = 'Inventory Edit Log Could not be Created';
		}
	}
...
function getQtyAudit($count)
{
	...
	if (isset($_GET['item_id'])) {
		...
		while ($row1 = mysqli_fetch_array($result1)) {
			...
			$ie_user[] = $row1[3];
			if (strlen($row1[4]) > 20) $comment[] = substr($row1[4], 0, 20) . '..';
			else $comment[] = $row1[4];
		}
	}
}
...
function pendingQuot($sub_system)
{
	...
	include('config.php');
	$query = "SELECT qm.id,qm.quo_timestamp,qm.validity,st.name,cu.name,SUM(qi.qty * qi.unit_price),qm.created_by,qm.`cust` FROM quotation_main qm, quotation qi, cust cu, stores st WHERE qm.id=qi.quot_no AND qm.`cust`=cu.id AND qm.store=st.id AND qm.`status`=2 AND qm.sub_system='$sub_system' GROUP BY qm.id";
	$result = mysqli_query($conn2, $query);
	while ($row = mysqli_fetch_array($result)) {
		$qm_id[] = $row[0];
...
function getTaxReport($sub_system){
	global $from_date, $to_date, $total_tax;
	...
	$query="SELECT round(SUM(bm.`tax`),'$decimal') FROM bill_main bm WHERE  bm.`status` NOT IN (0,7) $sub_system_qry AND date(bm.`billed_timestamp`) BETWEEN '$from_date' AND '$to_date'";
	$result=mysqli_query($conn2,$query);
	$row=mysqli_fetch_row($result);
	$total_tax=$row[0];
}
...
function getDetailTaxReport($sub_system){
	global $from_date, $to_date, $total_tax, $invoice_no, $billed_time, $invoice_total, $tax_amount, $billed_cust, $billed_time, $billed_by,
	...
	$query="SELECT bm.`invoice_no`, round(SUM(bm.`invoice_+total` + bm.`invoice_-total`),'$decimal'), round(SUM(bm.`tax`),'$decimal'), up.`username`,cu.`name`, date(bm.`billed_timestamp`), time(bm.`billed_timestamp`)
	FROM bill_main bm, userprofile up, cust cu
	WHERE bm.`billed_by`=up.`id` AND bm.`cust`=cu.`id` AND bm.`status` NOT IN (0,7) AND date(bm.`billed_timestamp`) BETWEEN '$from_date' AND '$to_date' GROUP BY bm.invoice_no ORDER BY bm.`billed_timestamp` DESC";

	$result=mysqli_query($conn2,$query);
	while($row=mysqli_fetch_array($result)){
		$invoice_no[] = $row[0];
		$gross_amount[] = $row[1];
		$gross_amount_total += $row[1];
		$invoice_total[] = $row[1] + $row[2];
		$bills_total += $row[1] + $row[2];
		$tax_amount[] = $row[2];
