						...
						if ($paymentRow) {
							if ($reason == 7) { // bank return
								...
								if ($paymentRow[4] != '') {  // Assuming chque_deposit_bank
									...
									$journalEntryForCheque = buildJournalEntry($conn, $amount, $debitAccountName, $creditAccountName, $description, $debitEntityType, $debitEntityID, $creditEntityType, $creditEntityID);
	...
	try {
		...
		if (isQuickBooksActive(1)) {
			...
			if ($type == 2 && $transfer_source == 2 && $status == 2 && $payment_id !== null && $qb_status !== null) {
				...
				// Process QB payment
				$result_array = processQBPayment($creditAccountName, $debitAccountName, $conn, $payment_amount, $qb_cust_id, $description);
