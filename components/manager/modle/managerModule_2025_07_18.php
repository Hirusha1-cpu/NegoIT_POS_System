...
function dailySale($store, $sub_system)
{
	...
	if ($type_req != 3 && $type_req != 5) {
		...
		if ((isset($_REQUEST['components'])) && ($_GET['components'] == 'manager' || $_GET['components'] == 'topmanager')) {
			$query = "SELECT bm.invoice_no, SUM(bi.qty * bi.unit_price), di.name, up.username, cu.name, bm.billed_timestamp, st.name, SUM(bi.discount), bm.`cust`, up.id, bm.gps_x, bm.gps_y
			...
			$query = "SELECT bm.`invoice_no`, (bm.`invoice_+total` + bm.`invoice_-total`), di.name, up.username, cu.name, DATE(bm.billed_timestamp), st.name
			...
			$query = "SELECT bm.`invoice_no`,SUM(bi.qty*bi.unit_price),di.name,up.username,cu.name,DATE(bm.billed_timestamp),st.name,SUM(bi.discount)
...
function changePaymentDepositStatusAjax($sub_system)
{
	...
	if (!isset($_POST['deposit_id'], $_POST['status'])) {
		// Return error response for missing parameters
		echo json_encode([
			'success' => false,
			'message' => 'Invalid input parameters (deposit_id or status missing).'
		]);
...
{
	global $payment_id, $invoice_no, $cust_name, $payment_amount, $bank_name, $comment, $payment_date, $payment_status, $payment_type, $payment_transfer_date, $payment_deposit_id,
	$payment_placed_by, $payment_reviewed_by, $fromdate, $todate, $payment_rejected_by, $payment_checked;
	$payment_id = $invoice_no = $cust_name = $payment_amount = $bank_name = $comment = $payment_date = $payment_status = $payment_type = $payment_transfer_date = $payment_deposit_id =
		$payment_placed_by = $payment_reviewed_by = $payment_rejected_by = $payment_checked = [];
	include('config.php');
	$user_id = $_COOKIE['user_id'];
	...
	$query = "SELECT py.`id`, py.`invoice_no`, cu.`name`, py.`amount`, ac.`name`, py.`comment`, py.`payment_date`, pd.`status`, pd.`type`, pd.`transfer_date`, pd.`id`,
	 u.`username`, up.`username`, uu.`username`, pd.`checked`
	FROM payment_deposit pd
	JOIN payment py ON pd.`payment_id` = py.`id`
	JOIN cust cu ON py.`cust` = cu.`id`
	JOIN userprofile up ON pd.`placed_by` = up.`id`
