...
function addCust($systemid){
	...
	*/
	...
	if ($status == 3) $msg = 'Customer was created and the Approval is pending';
	...
	if(!validateMobileNo($mobile)){
		...
		if($systemid == 13){
			...
			$message = 'Invalid mobile number! (number must be start with 05 and length must be 10 digits)';
		}else{
...
function setChqueStatus($status){
	...
	if($out && (isQuickBooksActive(1))){
		...
		if($qb_cust_id == ''){
			$message = 'Error : This user is not registered in QB';
			$out=false;
		}
	}

	if($out && (isSalesmanPaymentDepositActive())){
							...
							if ($paymentRow[3] != '') {  // Assuming qb_id
								if ($paymentRow[4] != '') {  // Assuming chque_deposit_bank
									if($reason == 7){ // bank return
										...
										$description = "[PAYMENT] - CHECK RETURN:BANK, Method: Cheque Payment ($bank_name), Check Number $cheque_no";
										if ($invoice_no != 0) {
										...
										if (isset($journalEntryForCheque['error'])) {
											...
											throw new Exception("QuickBooks error: " . $qbResult['error']);
										} else {
											try {
									...
									if($reason == 8 || $reason == 9){
										...
										$description = "[PAYMENT] - CHECK RETURN:OTHER, Method: Cheque Payment ($bank_name), Check Number $cheque_no";
										if ($invoice_no != 0) {
											$description .= ", Invoice No: $invoice_no";
										}
										$description .= ", Customer : $custName";
	...
	if(($out) && (count($journal_entry_array)>0)){
		if(isQuickBooksActive(1)){
			if($qb_id != ''){
				...
				$qb_msg = "Quickbooks error: The shipment is missing a corresponding journal entry.";
			}
		}
	}
	$message = $message.' '.$qb_msg;
...
function chquePendingFinalyze(){
	...
	if(isSalesmanPaymentDepositActive()){
		...
		$where_cluase = " ct.`payment_id` = py.`id` AND ";
...
function clearChque2(){
    ...
    if (isset($_POST['data'])) {
        ...
        // Initialize an array to hold responses for each cheque
        $responses = [];
        ...
        foreach ($depositData as $item) {
            ...
            if (empty($ac_id)) {
                ...
                continue; // Skip to the next item if bank account not found
            }
            ...
            if ($out) {
                ...
                if (mysqli_query($conn, $query)) {
					...
					$out = false;
                    $responses[] = ['success' => false, 'message' => 'Failed to update status: ' . mysqli_error($conn), 'chequeId' => $id];
                }
            }
			...
			if($out){
				if(isQuickBooksActive(1)){
					...
					if(!empty($row)){
						...
						if (isset($journalEntryForCheque['error'])) {
							$qb_msg = $journalEntryForCheque['error'];
							$responses[] = ['success' => false, 'message' => $qb_msg, 'chequeId' => $id];
						}else{
							try {
								$journal_entry_result = QBAddJournalEntry($journalEntryForCheque);
							} catch (Exception $e) {
...
function changePaymentDepositStatusAjax($sub_system) {
    ...
    if (isset($_POST['deposit_id']) && isset($_POST['status'])) {
		...
		if ((isQuickBooksActive(1))) {
			$query = "SELECT pd.`type`, pd.`payment_id`, pd.`transfer_source`, up.`username`, pd.`amount` FROM payment_deposit pd, userprofile up WHERE pd.`placed_by` = up.`id` AND pd.`id`='$deposit_id'";
			$row = mysqli_fetch_row(mysqli_query($conn2, $query));
			$type = $row[0];
			$payment_id = $row[1];
...
function getPendingBankDeposits($sub_system){
	...
	WHERE pd.`status` = '1' AND pd.`type` = '2' $bank_qry $date_qry $salesman_qry ORDER BY pd.`transfer_date` DESC";
...
function getBankDepositsReport($sub_system){
	...
	WHERE pd.`type` = '2' $status_qry $bank_qry $date_qry $salesman_qry $reviewed_by_qry ORDER BY pd.`transfer_date` DESC";
...
function getCashOnHandReport($sub_system){
	...
	while ($row = mysqli_fetch_array($result)) {
		...
		$query1 = "SELECT SUM(amount) FROM payment WHERE `salesman` = '$row[0]' AND `status` = 0 AND `payment_type` = 1 $date_qry1";
		$result1 = mysqli_query($conn2, $query1);
...
function getPendingChequeTransfers($sub_system){
	...
	WHERE ct.`to` = $user_id $status_qry $salesman_qry $from_qry $date_qry AND ct.`latest` = 1 ORDER BY ct.id DESC";
	$result = mysqli_query($conn, $query);
	while ($row = mysqli_fetch_array($result)) {
		$trans_id[] = $row[0];
		$payment_id[] = $row[1];
			...
			if ($result) {
				...
				if ($from <= 0) {
					...
					try {
						...
						// Ensure autocommit is re-enabled
...
function getChequeApprovedByUser($sub_system){
    ...
    JOIN userprofile up ON ct.from = up.id
    JOIN userprofile upp ON p.salesman = upp.id
	WHERE (ct.`from` = $user_id AND ct.`status` IN(1,5,10) AND ct.`latest` = 1 $from_qry) OR (ct.to = $user_id AND ct.status = 6 AND ct.latest = 1 $from_qry2) $date_qry ORDER BY ct.id DESC"; //1:accept, 5=accept-trans, 6=reject-trans
...
function addChequeTransferToUserAjax($sub_system){
	...
	if($out){
		try {
			...
			// Ensure transaction is ended and connection is not left in a bad state
			mysqli_autocommit($conn, true);
		}
	}
...
function getApprovedChequeTransfers($sub_system){
    ...
    JOIN userprofile up ON ct.from = up.id
    JOIN userprofile upp ON p.salesman = upp.id
	WHERE ct.`from` = $user_id AND ct.`status` IN(1,5) AND ct.`latest` = 1 ORDER BY ct.id DESC"; // 1: accept, 5: accept-trans
	$result = mysqli_query($conn, $query);
	while ($row = mysqli_fetch_array($result)) {
		$trans_id[] = $row[0];
		$payment_id[] = $row[1];
...
function getChequeTransSummery($sub_system){
	...
	// WHERE ct.`latest` = 1 $date_qry ORDER BY ct.id DESC";
    ...
    JOIN userprofile up ON p.salesman = up.id
    JOIN userprofile uup ON ct.from = uup.id
	WHERE ct.`latest` = 1 $inhand_qry $date_qry ORDER BY ct.id DESC";
...
function getChequeOnHandReport($sub_system){
    ...
    JOIN userprofile up ON p.salesman = up.id
	WHERE ct.`latest` = 1 AND ct.`status` IN(1,2,5) $user_qry $date_qry ORDER BY ct.id DESC";
...
function getSalesmanChequeOnHandReport($sub_system){
	...
	WHERE p.payment_type = 2 AND p.status = 0 AND p.chque_return != 1 AND ct.payment_id IS NULL $user_qry_1 $date_qry_1 ORDER BY p.id DESC";
...
function getTransReturnMarkedCheques($sub_system){
	...
	LEFT JOIN userprofile up ON ct.from = up.id
	JOIN userprofile upp ON p.salesman = upp.id
	WHERE (ct.from = $user_id AND ct.status IN(7,8,9,10,13) AND ct.latest = 1)
	OR (ct.to = $user_id AND ct.status = 13 AND ct.latest = 1)
	ORDER BY ct.id ASC"; // 7=bank-return, 8=cash-receive, 9=issue-new-cheque, 10=modify-cheque, 13=return-reject
