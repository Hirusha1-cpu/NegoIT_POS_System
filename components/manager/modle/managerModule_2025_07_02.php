...
function dailySale($store, $sub_system)
{
	...
	$payment_bank = $payment_bank_tr = $payment_cash = $payment_chque = $chq_details = $payment_card = $payment_card_tr = $cu_list_arr =
		$invoice_no = $payment_id = $rtn_no = $wa_no = $item_type = $payment_cheque_date = $payment_cheque_no = $graph_user = $graph_total = array();
...
function getChqueOne()
{
	...
	if (isset($_REQUEST['chque_no'])) {
		...
		if (isset($paymentId) && !empty($paymentId)) {
			...
			$conditions[] = "py.chque_no = '$py_chqno'";
			$conditions[] = "ba.bank_code = '$py_chqbank'";
			$conditions[] = "py.chque_branch = '$py_branch'";
		}
						...
						if ($paymentRow) {
							if ($reason == 7) { // bank return
								...
								if ($paymentRow[4] != '') {  // Assuming chque_deposit_bank
									...
									$journalEntryForCheque = buildJournalEntry($conn, abs($amount), $debitAccountName, $creditAccountName, $description, $debitEntityType, $debitEntityID, $creditEntityType, $creditEntityID);
									if (isset($journalEntryForCheque['error'])) {
										...
										foreach ($journalEntryForCheque as $entry) {
											...
											$query = "INSERT INTO qb_queue (`batch_id`, `action`, `payment_id`, `posting_type`, `account_id`, `account_name`, `amount`, `description`, `created_at`, `entity_type`, `entity_id`)
			...
			if (isQuickBooksActive(1)) {
				...
				if (($qb_status !== null && $qb_status != '') && ($vendorID !== null && $vendorID != '')) {
					...
					foreach ($journal_entries as $entry) {
						...
						$query = "INSERT INTO qb_queue (`batch_id`,`action`, `shipment_no`, `shipment_pay_id`, `posting_type`, `account_id`, `account_name`, `amount`, `description`, `created_at`, `entity_type`, `entity_id`, `entity_name`)
...
function clearChque()
{
	...
	try {
		...
		if ($out) {
			if (isQuickBooksActive(1)) {
				$query = "SELECT py.invoice_no, cu.qb_cust_id, py.amount, py.cust, py.chque_no, py.chque_branch, bk.bank_code, cu.`name`
				FROM payment py
...
function deleteBillPayment()
{
	...
	try {
		...
		if (isQuickBooksActive(1)) {
			...
			$paymentQuery = "SELECT p.`invoice_no`, p.`id`, p.`payment_type`, p.`qb_id`, c.`name`, c.`qb_cust_id`, p.`amount`, p.`chque_clear`, p.`chque_return`, p.`chque_deposit_bank`, p.`bank_trans`
				FROM `payment` p, `cust` c
				WHERE c.`id` = p.`cust` AND p.`id`='$py_no' AND p.`qb_status` IS NOT NULL AND p.`bill_pay` = 1";
			...
			if ($paymentResult && mysqli_num_rows($paymentResult) > 0) {
				while ($paymentRow = mysqli_fetch_assoc($paymentResult)) {
					...
					if ($paymentRow['payment_type'] == 1) { // Cash
						...
						$journalEntryForDeletedPayment = buildJournalEntry($conn, abs($amount), $debitAccountName, $creditAccountName, $description, $debitEntityType, $debitEntityID, $creditEntityType, $creditEntityID);
...
function adjustQty()
{
	...
	try {
		...
		if ($out && isQuickBooksActive(1)) {
			...
			$journalEntryForQtyAdjust = buildJournalEntry($conn, abs($amount), $debitAccountName, $creditAccountName, $description, $debitEntityType, $debitEntityID, $creditEntityType, $creditEntityID);
			if (isset($journalEntryForQtyAdjust['error'])) {
				...
				foreach ($journalEntryForQtyAdjust as $entry) {
					...
					$query = "INSERT INTO qb_queue (`batch_id`, `action`, `posting_type`, `account_id`, `account_name`, `amount`, `created_at`, `description`, `entity_type`, `entity_id`)
	...
	try {
		...
		if (isQuickBooksActive(1)) {
			...
			if ($type == 2 && $transfer_source == 2 && $status == 2 && $payment_id !== null && $qb_status !== null) {
				...
				// Process QB payment
				$result_array = processQBPayment($creditAccountName, $debitAccountName, $conn, abs($payment_amount), $qb_cust_id, $description);
