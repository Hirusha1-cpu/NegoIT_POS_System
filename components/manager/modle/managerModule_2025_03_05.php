...
function dailySale($store, $sub_system)
{
	...
	if (isset($_REQUEST['components']) && ($_REQUEST['components'] == 'manager') || ($_REQUEST['components'] == 'topmanager')) {
		...
		$query2 = "SELECT py.id, py.amount, py.payment_type, up.username, cu.name AS cust_name, DATE(py.payment_date) AS pay_date, st.name AS store_name, py.chque_no, bk.bank_code,
		...
		$query2 = "SELECT py.id, ROUND(py.amount) AS amount, py.payment_type, up.username AS salesman, cu.name AS cust_name, DATE(py.payment_date) AS pay_date, TIME(py.payment_date) AS pay_time, st.name AS store, py.chque_no, py.chque_date
				FROM payment py
		...
		if ($result2) {
			while ($row2 = mysqli_fetch_assoc($result2)) {
				...
				if ($row2['id'] > 0) {
					...
					$payment_time[] = $row2['pay_date'] . ' - ' . substr($row2['pay_time'], 0, 5);
