	...
	if (isset($_REQUEST['components']) && ($_REQUEST['components'] == 'manager') || ($_REQUEST['components'] == 'topmanager')) {
		...
		while ($row2 = mysqli_fetch_array($result2, MYSQLI_ASSOC)) {
			if ($row2['id'] > 0) {
				...
				$payment_time[] = substr($row2['pay_date'], 0, 16);
	...
	if ($out) {
		try {
			...
			if ($out) {
				...
				if (isQuickBooksActive(1)) {
					if ($status == 1) {
						$paymentQuery = "SELECT py.invoice_no, py.amount, c.name, py.qb_id, py.chque_deposit_bank, py.qb_status FROM payment py, cust c
								WHERE c.id = py.cust AND py.id = '$id'";
						$paymentRow = mysqli_fetch_row(mysqli_query($conn2, $paymentQuery));

						if ($paymentRow) {
							$qb_status = $paymentRow[5];

							if ($qb_status !== null && $qb_status != '') {  // Assuming qb_id
...
function getSalesReport4($sub_system)
{
	...
	if ($case) {
		...
		WHERE bm.billed_by=up.id AND bm.invoice_no=bi.invoice_no AND bi.item=itm.id AND itm.category=itc.id AND bm.`cust`=cu.id AND bm.`store`=st.id AND bm.`status` NOT IN (0,7) AND bm.`lock`=1 AND itm.description!='CHQUE' AND itm.description!='CREDIT' $cust_st_qry  AND bm.`status` NOT IN (0,7) AND itm.unic=1 AND bm.exclude=0 AND date(bi.`date`) BETWEEN '$fromdate' AND '$todate' $cat_qry $sub_system_qry
	...
	try {
		...
		if ($out) {
			if (isQuickBooksActive(1)) {
				...
				$shipmentQuery = "SELECT  sm.`qb_id`, a.`qb_account_id`, s.`name`, sm.`qb_status` FROM `shipment_main` sm
								JOIN `supplier` s ON sm.`supplier` = s.`id` JOIN `accounts` a ON s.`name` = a.`name`
								WHERE sm.`id` = '$shipment_no' AND sm.`status` = 0";
				$shipmentRow = mysqli_fetch_row(mysqli_query($conn, $shipmentQuery));
				...
				$accountResult2 = mysqli_fetch_row(mysqli_query($conn, $accountQuery2));
	...
	if (isset($_POST['data'])) {
		try {
			...
			foreach ($depositData as $item) {
				...
				if ($bnkac_id != "") {
					$query = "SELECT id FROM accounts WHERE `id`='$bnkac_id'";
					$result = mysqli_query($conn2, $query);
					$row = mysqli_fetch_assoc($result);
		...
		if ($out && isQuickBooksActive(1)) {
			...
			if (isset($journalEntryForQtyAdjust['error'])) {
				...
				foreach ($journalEntryForCheque as $entry) {
					...
					$query = "INSERT INTO qb_queue (`batch_id`, `action`, `posting_type`, `account_id`, `account_name`, `amount`, `description`, `entity_type`, `entity_id`)
							VALUES ('$batch_id','$action_name', '$posting_type', '$account_id', '$account_name', '$amount', '$description',
								" . ($entity_type !== null ? "'$entity_type'" : "NULL") . ",
								" . ($entity_id !== null ? "'$entity_id'" : "NULL") . ")";
					$result = mysqli_query($conn, $query);
					if (!$result) {
				...
				if ($type == 1 && $transfer_source == 2 && $status == 2) { // cash deposit to bank
					...
					if (isset($journalEntryForBankDeposit['error'])) {
						...
						foreach ($journalEntryForBankDeposit as $entry) {
							...
							$query = "INSERT INTO qb_queue (`batch_id`,`action`, `payment_id`, `posting_type`, `account_id`, `account_name`, `amount`, `description`, `entity_type`, `entity_id`, `entity_name`)
									VALUES ('$batch_id','$action_name', '$payment_id', '$posting_type', '$account_id', '$account_name', '$amount', '$description',
										" . ($entity_type !== null ? "'$entity_type'" : "NULL") . ",
										" . ($entity_id !== null ? "'$entity_id'" : "NULL") . ",
										" . ($entity_name !== null ? "'$entity_name'" : "NULL") . ")";
							if (!mysqli_query($conn, $query)) {
