...
function getTaxReport($sub_system){
	...
	$query="SELECT SUM(bm.`tax`) FROM bill_main bm WHERE  bm.`status` NOT IN (0,7) AND date(bm.`billed_timestamp`) BETWEEN '$from_date' AND '$to_date'";
	$result=mysqli_query($conn2,$query);
	$row=mysqli_fetch_row($result);
	$total_tax=$row[0];
}
