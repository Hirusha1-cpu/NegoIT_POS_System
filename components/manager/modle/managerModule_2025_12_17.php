...
function getOneCust($mode, $sub_system)
{
	...
	$cu_master_cust, $forign_image, $cu_tax_no, $designation, $delivery_details_same, $goods_delivery_mobile, $goods_delivery_address, $goods_delivery_contact_person;
	$note = '';
...
function dailySale($store, $sub_system)
{
	...
	$salesman_summary, $invoice_tax, $invoice_total_wo_tax;
	...
	if ($type_req != 3 && $type_req != 5) {
		...
		while ($row1 = mysqli_fetch_array($result1, MYSQLI_ASSOC)) {
			...
			switch ($row1['payment_type']) {
				case 1:
					$payment_cash[$invoice_no_id] = (double) $row1['amount'];
					break;
				case 2:
					$payment_chque[$invoice_no_id] = (double) $row1['amount'];
					...
					break;
				case 3:
					$payment_bank[$invoice_no_id] = (double) $row1['amount'];
					$payment_bank_tr[$invoice_no_id] = 'Bank    : ' . $row1['account_name'];
					break;
				case 4:
					$payment_card[$invoice_no_id] = (double) $row1['amount'];
...
function getSalesReport41($sub_system)
{
	$user = $_COOKIE['user'];
...
function getSalesReport4($sub_system)
{
	...
	if ($case) {
		...
		while ($row = mysqli_fetch_array($result)) {
			...
			// 3. Save the current row into our $allRows array.
			// We access data using column indexes ($row[0], $row[1] etc.)
			$allRows[] = array(
				'item_category' => $row[0],
				'item_store' => $row[1],
				'item_salesman' => ucwords($row[2]), // Capitalize names
				'item_cust' => $row[3],
				'item_desc' => $row[4],
				'item_sn' => $row[5],          // The Serial Number(s)
				'item_sold_price' => $row[6],
				'item_bm_date' => $row[7],
				'qty' => $row[8]           // +1 (Sale) or -1 (Return)
			);
		}

		// 4. Create an array to keep score of Pluses and Minuses for each SN.
		$sn_counts = array();

		// 5. Loop through every saved row.
		foreach ($allRows as $r) {

			// 6. Split the SN string by commas.
			// (Usually just one SN, but handles cases like "SN1,SN2")
			$snArray = explode(',', $r['item_sn']);

			// 7. Loop through each SN found in this specific row.
			foreach ($snArray as $sn) {

				// 8. Remove whitespace (spaces) around the SN. Important!
				$sn = trim($sn);

				// 9. If SN is empty after trimming, skip it.
				if ($sn === '')
					continue;

				// 10. If this is the first time we see this SN, set counts to 0.
				if (!isset($sn_counts[$sn])) {
					$sn_counts[$sn] = array('plus' => 0, 'minus' => 0);
				}

				// 11. Check the Quantity (+1 or -1)
				if ($r['qty'] > 0) {
					// 12. If sale, add 1 to the "plus" counter.
					$sn_counts[$sn]['plus']++;
				} else {
					// 13. If return, add 1 to the "minus" counter.
					$sn_counts[$sn]['minus']++;
				}
			}
		}

		// 14. Create an array to store the "Delete Limit" for each SN.
		$sn_cancel_quota = array();

		// 15. Loop through our counts from Phase 2.
		foreach ($sn_counts as $sn => $counts) {

			// 16. The Logic: compare total Sales vs total Returns.
			// If Sales=2 and Returns=1, min is 1. We must delete 1 pair.
			$sn_cancel_quota[$sn] = min($counts['plus'], $counts['minus']);
		}

		// 17. Initialize final arrays for the Excel report.
		$item_category = array();
		$item_store = array();
		$item_salesman = array();
		$item_cust = array();
		...
		// 18. This array tracks how many we have deleted SO FAR while looping.
		$sn_deleted_so_far = array();

		// 19. Start looping through the rows again.
		foreach ($allRows as $rowData) {

			// 20. Get SNs from this row.
			$snArray = explode(',', $rowData['item_sn']);

			// 21. Create a temporary list for SNs we decide to KEEP in this row.
			$filteredSNs = array();

			// 22. Loop through SNs in this row.
			foreach ($snArray as $sn) {
				$sn = trim($sn);
				if ($sn === '')
					continue;

				// 23. Get the "Delete Quota" calculated in Phase 3.
				// If not found, quota is 0.
				$quota = isset($sn_cancel_quota[$sn]) ? $sn_cancel_quota[$sn] : 0;

				// 24. Optimization: If Quota is 0, we don't need to delete anything.
				// Add to filtered list and skip logic.
				if ($quota == 0) {
					$filteredSNs[] = $sn;
					continue;
				}

				// 25. Initialize the "deleted so far" tracker for this SN if new.
				if (!isset($sn_deleted_so_far[$sn])) {
					$sn_deleted_so_far[$sn] = array('plus' => 0, 'minus' => 0);
				}

				// 26. Check if this specific row is a Plus or Minus.
				$type = ($rowData['qty'] > 0) ? 'plus' : 'minus';

				// 27. THE CORE DECISION:
				// Have we deleted enough of this type yet?
				// If "Deleted So Far" is LESS THAN "Quota", we must delete this one.
				if ($sn_deleted_so_far[$sn][$type] < $quota) {

					// 28. Mark it as deleted (increment the tracker).
					$sn_deleted_so_far[$sn][$type]++;

					// 29. IMPORTANT: We do NOT add it to $filteredSNs.
					// This effectively removes it from the report.

				} else {
					// 30. We have already deleted enough.
					// This is a "Net Sale" or "Net Return". Keep it.
					$filteredSNs[] = $sn;
				}
			}

			// 31. If $filteredSNs is empty, it means every item in this row was cancelled.
			// Skip the whole row (don't add to Excel arrays).
			if (empty($filteredSNs)) {
				continue;
			}

			// 32. If we are here, the row (or parts of it) survived!
			// Add data to the final arrays for the Excel file.
			$item_category[] = $rowData['item_category'];
			$item_store[] = $rowData['item_store'];
			$item_salesman[] = $rowData['item_salesman'];
			$item_cust[] = $rowData['item_cust'];
			...
			// 33. Join the surviving SNs back into a string (e.g., "SN1, SN3")
			$item_sn[] = implode(",", $filteredSNs);
	...
	/*
			Here is the trace of the code execution using **Sample Data**.

		### The Sample Data
		We have 4 rows coming from the database.
		*   **Row 1:** Item: Phone A, SN: `A100`, Qty: `+1` (Sale)
		*   **Row 2:** Item: Phone A, SN: `A100`, Qty: `-1` (Return)
		*   **Row 3:** Item: Phone A, SN: `A100`, Qty: `+1` (Sale)
		*   **Row 4:** Item: Phone B, SN: `B200`, Qty: `+1` (Sale)

		---

		### Phase 1: Store Data (The `while` loop)
		*Code reads the DB and fills `$allRows` array.*

		**Result in Memory (`$allRows`):**
		*   `[0]` => {SN: `A100`, Qty: `1`}
		*   `[1]` => {SN: `A100`, Qty: `-1`}
		*   `[2]` => {SN: `A100`, Qty: `1`}
		*   `[3]` => {SN: `B200`, Qty: `1`}
		...
		### Phase 2: Counting (The `foreach` loop)
		*Code loops through `$allRows` to calculate totals.*

		1.  **Processing Row 0 (A100, +1):**
				*   `$sn_counts['A100']['plus']` increases to **1**.
		2.  **Processing Row 1 (A100, -1):**
				*   `$sn_counts['A100']['minus']` increases to **1**.
		3.  **Processing Row 2 (A100, +1):**
				*   `$sn_counts['A100']['plus']` increases to **2**.
		4.  **Processing Row 3 (B200, +1):**
				*   `$sn_counts['B200']['plus']` increases to **1**.

		**Result in Memory (`$sn_counts`):**
		*   `A100`: { plus: **2**, minus: **1** }
		*   `B200`: { plus: **1**, minus: **0** }

		---

		### Phase 3: Setting Quotas (The `min` calculation)
		*Code determines how many to delete based on the "Min" rule.*

		1.  **Check A100:**
				*   Plus: 2, Minus: 1.
				*   `min(2, 1) = 1`.
				*   **Quota:** We must delete **1 Plus** and **1 Minus**.
		2.  **Check B200:**
				*   Plus: 1, Minus: 0.
				*   `min(1, 0) = 0`.
				*   **Quota:** 0 (Don't delete anything).

		**Result in Memory (`$sn_cancel_quota`):**
		*   `A100`: **1**
		*   `B200`: **0**

		---

		### Phase 4: Filtering (The Final Loop)
		*Code walks through the rows one last time to decide: **KEEP** or **DELETE**.*

		**Init:** `$sn_deleted_so_far` is empty.

		#### Row 1 Execution (A100, +1)
		1.  **Get SN:** `A100`.
		2.  **Get Quota:** `1`.
		3.  **Get Type:** `plus` (since qty is 1).
		4.  **Logic Check:**
				*   Have we deleted enough Pluses for A100?
				*   Current deleted count: **0**.
				*   Is `0 < 1` (Quota)? **YES**.
		5.  **Action:**
				*   Increment deleted count: `$sn_deleted_so_far['A100']['plus']` becomes **1**.
				*   **SKIP** adding to `$filteredSNs`. (Row is effectively **DELETED**).

		#### Row 2 Execution (A100, -1)
		1.  **Get SN:** `A100`.
		2.  **Get Quota:** `1`.
		3.  **Get Type:** `minus` (since qty is -1).
		4.  **Logic Check:**
				*   Have we deleted enough Minuses for A100?
				*   Current deleted count: **0**.
				*   Is `0 < 1` (Quota)? **YES**.
		5.  **Action:**
				*   Increment deleted count: `$sn_deleted_so_far['A100']['minus']` becomes **1**.
				*   **SKIP** adding to `$filteredSNs`. (Row is effectively **DELETED**).

		#### Row 3 Execution (A100, +1)
		1.  **Get SN:** `A100`.
		2.  **Get Quota:** `1`.
		3.  **Get Type:** `plus`.
		4.  **Logic Check:**
				*   Have we deleted enough Pluses for A100?
				*   Current deleted count: **1** (from Row 1).
				*   Is `1 < 1` (Quota)? **NO**.
		5.  **Action:**
				*   We have reached the kill limit. We cannot delete anymore.
				*   **ADD** `A100` to `$filteredSNs`. (Row is **KEPT**).

		#### Row 4 Execution (B200, +1)
		1.  **Get SN:** `B200`.
		2.  **Get Quota:** `0`.
		3.  **Logic Check:**
				*   `Quota == 0`? **YES**.
		4.  **Action:**
				*   **ADD** `B200` to `$filteredSNs`. (Row is **KEPT**).

		---

		### Final Result (Sent to Excel)
		The array `$filteredSNs` (and thus your report) now contains:
		1.  **Row 3:** A100 (The valid sale)
		2.  **Row 4:** B200 (The valid sale)

		Rows 1 and 2 were successfully removed because they cancelled each other out.
	*/
}
