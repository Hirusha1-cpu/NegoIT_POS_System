...
function dailySale($store, $sub_system){
	...
	$payment_bank = $payment_bank_tr = $payment_cash = $payment_chque = $chq_details = $payment_card = $payment_card_tr = $cu_list_arr = $invoice_no = $payment_id = $rtn_no = $wa_no = $item_type = $payment_cheque_date = $payment_cheque_no = array();
	$systemid = inf_systemid(1);
	...
	if ($type_req != 3 && $type_req != 5) {
		...
		while ($row1 = mysqli_fetch_array($result1)) {
			...
			if ($row1[0] == 1)	$payment_cash[$invoice_no_id] = (double)$row1[1];
			if ($row1[0] == 2) {
		...
		while ($row = mysqli_fetch_array($result)) {
			...
			if (!isset($payment_cash[$invoice_no_id])) $payment_cash[$invoice_no_id] = 0.0;
			if (!isset($payment_chque[$invoice_no_id])) $payment_chque[$invoice_no_id] = 0.0;
			if (!isset($payment_card[$invoice_no_id])) $payment_card[$invoice_no_id] = 0.0;
			if (!isset($payment_bank[$invoice_no_id])) $payment_bank[$invoice_no_id] = 0.0;
	...
	if ($out) {
		...
		if ((!$result1) || (!$result2)) {
			...
			if (isQuickBooksActive(1)) {
				...
				$result1 = mysqli_query($conn2, "SELECT `qb_account_id`,`name` FROM accounts WHERE `id`='$to_account'");
				$row = mysqli_fetch_assoc($result1);
				$qb_to_account_id = $row['qb_account_id'];
				$amount = abs(floatval($amount));
				$vendor_id = $qb_to_account_id;

				// Step 1.1: Retrieve total cost of shipment/bill
				$queryTotal = "SELECT ins.id, ins.added_qty, ins.cost, itm.unic, sm.qb_bill_id FROM
							   inventory_shipment ins JOIN inventory_items itm ON itm.id = ins.inv_item JOIN shipment_main sm ON sm.id = ins.shipment_no
							   WHERE ins.shipment_no = '$shipment_no'";
				$resultTotal = mysqli_query($conn2, $queryTotal);

				$total_cost = 0;
				$qb_db_bill_id = '';
				$bill_result = '';

				while ($row2 = mysqli_fetch_assoc($resultTotal)) {
					$ins_id = $row2['id'];
					$unic = $row2['unic'];
					$quantity = $row2['added_qty'];
					$cost = $row2['cost'];
					$qb_db_bill_id = $row2['qb_bill_id'];

					if ($unic == 1 && $unic_cal) {
						$query1 = "SELECT COUNT(id), SUM(c_price) FROM inventory_unic_item WHERE inv_ship_id = '$ins_id' AND `status` != '2'";
						$row1 = mysqli_fetch_row(mysqli_query($conn2, $query1));
						$quantity = $row1[0];
						$cost = ($row1[0] != 0) ? $row1[1] / $row1[0] : 0;
					}
					$total_cost += $quantity * $cost;
				}

				// Step 2: Create the Bill if not created before payment is made (first time payment)
				if (($total_cost > 0) && ($qb_db_bill_id == '')) {
					$bill_array = array(
						'amount' => $total_cost,
						'from_account_id' => $qb_from_account_id,
						'vendor_id' => $vendor_id
					);

					try {
						$bill_result = QBAddBill($bill_array);
						$qb_msg = $bill_result['message'];
					} catch (Exception $e) {
						$qb_msg = "<br>QuickBooks error: " . $e->getMessage();
						$bill_result['status'] = 'error';
					}

					// Step 2.1: Update shipment_main with qb_bill_id after successful bill creation
					if (($qb_db_bill_id == '') && ($bill_result['status'] == 'success')) {
						$qb_bill_id = $bill_result['qb_bill_id'];
						$query3 = "UPDATE `shipment_main` SET `qb_bill_id`='$qb_bill_id' WHERE `id`='$shipment_no'";
						$result3 = mysqli_query($conn, $query3);
						if (!$result3) {
							$qb_msg .= ' QuickBooks column update error.';
						}
					}
				}

				// Step 3: Determine the correct bill ID and make the payment
				$qb_bill_id = ($qb_db_bill_id == '') ? $bill_result['qb_bill_id'] : $qb_db_bill_id;

				if (($qb_db_bill_id == '') && ($bill_result['status'] == 'success') || ($qb_db_bill_id != '')) {
					$bill_payment_array = array(
						'amount' => $amount,
						'bank_account_id' => $qb_from_account_id,
						'vendor_id' => $vendor_id,
						'bill_id' => $qb_bill_id,
						'payment_method' => 'CreditCard', // Change if necessary
						'ref' => substr($ref, 0, 21) // max allow 21 chars
					);

					try {
						$bill_payment_result = QBAddBillPayment($bill_payment_array);
						$qb_msg = $bill_payment_result['message'];

						if ($bill_payment_result['status'] == 'success') {
							$qb_bill_payment_id = $bill_payment_result['qb_bill_payment_id'];
							if ($qb_bill_payment_id != '') {
								$query3 = "UPDATE `shipment_pay` SET `qb_bill_payment_id`='$qb_bill_payment_id' WHERE `id`='$lastid_temp'";
								$result3 = mysqli_query($conn, $query3);
								if (!$result3) {
									$qb_msg .= ' QuickBooks column update error.';
								}
						...
						$qb_msg = "<br>QuickBooks error: " . $e->getMessage();
						$bill_payment_result['status'] = 'error';
					}
				}
			}
		}
...
function deleteShipPayment(){
	...
	$id = $_GET['id'];
	$pass = $_GET['pass'];
	$journal_id = $bill_payment_array = $qb_msg = '';
	$out = true;
	...
	if($out && isQuickBooksActive(1)){
		$result = mysqli_query($conn, "SELECT `qb_bill_payment_id` FROM shipment_pay WHERE id='$id'");
		$row = mysqli_fetch_assoc($result);
		$qb_id = $row['qb_bill_payment_id'];
		$bill_payment_array = array('bill_payment_id' => $qb_id);
	}
	if ($out) {
	...
	if(($out) && (count($bill_payment_array)>0)){
		if(isQuickBooksActive(1)){
			try{
				$qb_result = QBDeleteBillPayment($bill_payment_array);
				$qb_msg = $qb_result['message'];
			}catch (Exception $e) {
				$qb_msg = "<br>QuickBooks error: " . $e->getMessage();
				$qb_result['status'] = 'error';
...
// update by nirmal 13_06_2024 (show all payment to top manager without consider sub system)
