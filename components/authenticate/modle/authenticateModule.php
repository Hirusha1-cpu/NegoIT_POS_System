...
// update by nirmal 02_02_2024 to get mapped inventory id
// updated by nirmal 07_03_2024 force checking enable for all users
function login(){
	...
	$store_name=$store_shopname=$user=$mapped_inventory='';
	...
	if(($out) && ($authentication>0)){
		$query="SELECT up.`id`,up.`username`,pe.`function`,fn.`name`,up.`store`,st.`retail`,st.`name`,st.`shop_name`,up.`direct_mkt`,st.`sub_system`,up.`mapped_inventory` FROM permission pe, `function` fn, `userprofile` up LEFT JOIN stores st ON up.`store`=st.`id` WHERE fn.`id`=pe.`function` AND pe.`user`=up.`id` AND up.`username`='$user' AND fn.`status`=1 AND up.`status`='0'";
		$result=mysqli_query($conn,$query);
		while($row=mysqli_fetch_array($result)){
			$id=$row[0];
			$user=$row[1];
	...
	if(($id>0) && ($out)){
		...
		$_SESSION["mapped_inventory"] = $mapped_inventory;
		...
		$row = mysqli_fetch_assoc($result);
		setcookie("default_discount",$row['value'], time()+3600*10);

		$result = mysqli_query($conn,"SELECT `value` FROM settings WHERE setting='decimal'");
		$row = mysqli_fetch_assoc($result);
		setcookie("decimal",$row['value'], time()+3600*10);
		...
		if($numResults > 0){
			...
			if (setcookie("rsaid", $dev_key, time() + 86400 * $diff)) {
				...
				if(!$result1){
					$message = 'Device was registered un-successful!';
					$out = false;
				}
			} else {
...
function logout(){
	...
	setcookie("cus_details_on_billing",'', time()-3600*10);
	setcookie("cust_dob_on_manager",'', time()-3600*10);
	setcookie("default_discount",'', time()-3600*10);
	setcookie("decimal",'', time()-3600*10);
	session_unset();
	session_destroy();
	if(isset($_GET['type'])){
