...
function salesReport(){
	global $invoice_no,$time,$invoice_total,$invoice_profit,$date1,$date2,$item_req,$type_req,$salesman_req,$store_req,$cust_req,$invoice_type,$color,$total_discount,$invo_salesman,$cust,$store,$graph_user,$graph_total,$bi_discount,$rtn_no,$rtn_time,$rtn_pay,$rtn_salesman,$rtn_store,$rtn_cust,$group,$invoice_cost,$cat_id,$cat_name,$cat_select,$sub_system0,$hp;
	$sub_system0=$group=$store_req=$cust_req=$cust_qry=$store_qry=$salesman_req=$salesman_qry=$item_req=$item_qry=$type_req=$pr_sr=$type_qry=$hp_qry=$group_qry=$sub_system_qry=$cat_list='';
	$cat_id=$invoice_no=$invoice_total=$invoice_profit=$invo_salesman=$invoice_type=$cust=$store=$graph_user=$graph_total=$rtn_pay=$rtn_no=array();
	$total=0;
	$tags_qry=$usr='';
	$k=0;
	$j= -1;
	...
	if(isset($_REQUEST['filter_item'])){ $item_req=$_REQUEST['filter_item']; if($item_req!='') $item_qry="itm.description='$item_req' AND"; }
	if(isset($_REQUEST['filter_salesman'])){ $salesman_req=$_REQUEST['filter_salesman']; if($salesman_req!='') $salesman_qry="up.username='$salesman_req' AND"; }
	if(isset($_REQUEST['filter_store'])){ $store_req=$_REQUEST['filter_store']; if($store_req!='') $store_qry="st.name='$store_req' AND"; }
...
function exportExcelProfitReport(){
	global $message, $invoice_no, $date, $invoice_total, $invoice_profit, $invoice_type, $cust, $invoice_cost, $payments, $hp, $from_date, $to_date, $advance_payment, $hp, $group, $sub_system0;
	$user = $_COOKIE['user'];
...
function getEditQtyAudit(){
	...
	$ie_date=array();
	include('config.php');
	$query1="SELECT ie.`datetime`,itm.description,st.name,ie.item_cost,ie.old_qty,ie.action_qty,up.username,ie.`comment` FROM inventory_edit ie, inventory_items itm, userprofile up, stores st WHERE ie.item=itm.id AND ie.edit_by=up.id AND ie.store=st.id AND date(ie.`datetime`) BETWEEN '$from_date' AND '$to_date' $qry_itm";
	$result1=mysqli_query($conn2,$query1);
	while($row1=mysqli_fetch_array($result1)){
		$ie_date[]=$row1[0];
		$ie_item[]=$row1[1];
