...
<script type="text/javascript">
	function ackDelete1($id) {
	  ...
	  xhttp.open("GET", 'index.php?components=<?php print $components; ?>&action=ackdeleted1&id='+$id, true);
	  xhttp.send();
	}
	...
	function ackDelete2($id) {
	  ...
	  xhttp.open("GET", 'index.php?components=<?php print $components; ?>&action=ackdeleted2&id='+$id, true);
	  xhttp.send();
	}
...
<tr><td align="center">This report shows last 100 records of deleted "Bill Invoices", "Payment Invoices" And "Return Invoices"</td></tr>
...
<br />
	...
	<tr><th width="100px">Invoice No</th><th width="100px">Associated Payment<br>Invoice No</th><th width="100px">Date</th><th width="150px">Invoice Price</th><?php if($components=='report') print '<th width="150px">Invoice Profit</th>'; ?><th width="150px">Deleted By</th><th>Ack</th></tr>
<?php
$inv=0;
	for($i=0;$i<sizeof($bi_inv_no);$i++){
		...
		if($components=='report') $profit_txt='<td align="right">'.number_format($bi_inv_profit[$i]).'&nbsp;&nbsp;</td>'; else $profit_txt='';
			print '<tr><td align="center"><a href="index.php?components=billing&action=finish_bill&id='.$bi_inv_no[$i].'">'.str_pad($bi_inv_no[$i], 7, "0", STR_PAD_LEFT).'</a></td><td align="center"><a href="index.php?components=billing&action=finish_payment&id='.$bi_asso_pay[$i].'">'.$bi_asso_pay1.'</a></td><td align="center" width="50px">'.$bi_date[$i].'</td><td align="right">'.number_format($bi_inv_total[$i]).'&nbsp;&nbsp;</td>'.$profit_txt.'<td align="center">'.ucfirst($bi_deleted_by[$i]).'</td><td align="center"><div id="ack_div1_'.$bi_inv_no[$i].'">'.$ackdeleteBtn1.'</div></td></tr>';
	}
?>	
	</table>
	<br>
	...
	<tr><th width="100px">Invoice No</th><th width="100px">Date</th><th width="150px">Amount</th><th width="150px">Deleted By</th><th>Ack</th></tr>
	<?php
		$inv=0;
