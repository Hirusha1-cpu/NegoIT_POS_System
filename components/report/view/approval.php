...
?></td></tr></table>
	...
	<?php for($i=0;$i<sizeof($ship_id);$i++){
		if(($i%2)==0) $color='#FAFAFA'; else $color='#EEEEEE';
		print '<tr bgcolor="'.$color.'" ><td class="shipmentTB3"><a href="index.php?components=inventory&action=one_shipment&shipment_no='.$ship_id[$i].'" style="text-decoration:none">'.str_pad($ship_id[$i], 7, "0", STR_PAD_LEFT).'</a></td><td class="shipmentTB3" align="right">'.$ship_date[$i].'</td><td class="shipmentTB3" align="right">'.ucfirst($ship_submit_by[$i]).'</td><td class="shipmentTB3"><input type="button" value="Approve" onclick="setShipmentStatus('."'$ship_id[$i]','2'".')" />&nbsp;&nbsp;&nbsp;<input type="button" value="Reject" onclick="setShipmentStatus('."'$ship_id[$i]','0'".')" /></td></tr>';
	} ?>
	</table>
