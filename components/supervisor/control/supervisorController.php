...
switch ($_REQUEST['action']) {
   ...
   case "daily_sale":
      ...
      if ($systemid == 1 || ($systemid == 13 && $_COOKIE['user_id'] == 22)) {
         ...
         $storedisable = 'disabled';
         $userdisable = '';
         dailySale($_COOKIE['store'], $sub_system);
      }
      getFilter($sub_system);
   ...
   case "quotation":
      ...
      if (isset($_COOKIE['district'])) {
         ...
         if (isset($_GET['cust'])) {
            if (validateQuotNo())
               header('Location: index.php?components=supervisor&action=new_quot&cust_id=' . $_GET['cust'] . '&att=');
         }
      }
      if (isMobile())
   ...
   case "update_qo_main_comment":
      ...
      if (updateQuotMainComment()) {
         header('Location: index.php?components=supervisor&action=qo_finish&id=' . $id . '&message=' . $message . '&re=success');
      } else {
         header('Location: index.php?components=supervisor&action=qo_finish&id=' . $id . '&message=' . $message . '&re=fail');
      }
      break;
