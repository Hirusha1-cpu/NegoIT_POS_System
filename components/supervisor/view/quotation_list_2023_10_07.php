...
<script type="text/javascript">
	...
	function completeCheck($case){
		...
		if((document.getElementById("qo_check").checked)&&((qo_no.length)>0)){
			...
			for(var i=0;i<qo_no.length;i++){
					document.getElementById('col1_'+qo_no[i]).style.background = '';
					document.getElementById('col2_'+qo_no[i]).style.background = '';
					document.getElementById('col3_'+qo_no[i]).style.background = '';
					document.getElementById('col4_'+qo_no[i]).style.background = '';
					document.getElementById('col5_'+qo_no[i]).style.background = '';
					document.getElementById('col6_'+qo_no[i]).style.background = '';
					document.getElementById('col7_'+qo_no[i]).style.background = '';
					document.getElementById('col8_'+qo_no[i]).style.background = '';
			}
		}
	}
	
	function completeCheckPHP($case,qo_no,last_qo){
	  if($case==1){ var color='#00FF00'; var txt='yes'; }
	  ...
	  xhttp.onreadystatechange = function() {
	    ...
	    var returntext=this.responseText;
			...
			if($values[0]==txt){
				document.getElementById('col1_'+qo_no).style.background = color;
				document.getElementById('col2_'+qo_no).style.background = color;
				document.getElementById('col3_'+qo_no).style.background = color;
				document.getElementById('col4_'+qo_no).style.background = color;
				document.getElementById('col5_'+qo_no).style.background = color;
				document.getElementById('col6_'+qo_no).style.background = color;
				document.getElementById('col7_'+qo_no).style.background = color;
				document.getElementById('col8_'+qo_no).style.background = color;
			}
...
<div id="loading" style="display:none"><img src="images/loading.gif" style="width:30px" /></div>
	...
	<tr><td></td><td colspan="7" align="left">
		...
		<input type="text" style="width:110px" name="search1" id="search1" placeholder="Quotation Number" /> <a onclick="submitQForm2()" style="cursor:pointer"><img src="images/search.png" style="width:30px; vertical-align:middle" /></a>
		</form>
	</td><td colspan="3" style="color:maroon">
		<?php 
		...
		?>
	</td><td><div id="loading_qocheck"></div></td></tr>
	</table>
<!-- ----------------------------------------------------------------------------------------------------------------------------------------------- -->
<table align="center"><tr><td width="10px" bgcolor="orange"></td><td> - Quantity available in SUM of all stores | </td>
	<?php if($_GET['status']==8) print '<td width="10px" bgcolor="#EE6666"></td><td> - Partially Filled Quotations </td>';
		else print '<td width="10px" bgcolor="#00FF00"></td><td> - Fully Filled Quotations </td>';
	?>
</tr></table>
...
<div id="print">
	...
	<tr bgcolor="#E5E5E5"><th>Quotation No</th><th width="100px">Created Date</th><th width="100px">Store</th><th width="200px">Customer</th><th width="200px">Heading</th><th width="100px">Salesman</th><th width="100px">Amount</th><th width="80px">Completed %</th><th width="100px">Status</th></tr>
<?php
	for($i=0;$i<sizeof($qm_id);$i++){
		...
		print '<tr bgcolor="'.$row_color.'"><td align="center"><div id="col1_'.$qm_id[$i].'"><a href="index.php?components='.$components.'&action='.$action0.'&id='.$qm_id[$i].'" title="Name: '.$qm_custname[$i].'
Mobile: '.$qm_mobile[$i].'
Tel: '.$qm_tel[$i].'">'.str_pad($qm_id[$i], 7, "0", STR_PAD_LEFT).'</a></div></td><td align="center"><div id="col2_'.$qm_id[$i].'"><a style="cursor:pointer; color:blue" title="Time: '.substr($qm_created_date[$i],11,5).'" >'.substr($qm_created_date[$i],0,10).'</a></div></td><td class="shipmentTB3"><div id="col3_'.$qm_id[$i].'">'.$qm_store[$i].'</div></td><td class="shipmentTB3"><div id="col4_'.$qm_id[$i].'">'.$qm_cust[$i].'</div></td><td class="shipmentTB3"><div id="col5_'.$qm_id[$i].'"><a title="'.$qm_heading[$i].'" style="cursor:pointer; color:blue">'.$qm_heading0.'</a></div></td><td class="shipmentTB3"><div id="col6_'.$qm_id[$i].'">'.ucfirst($qm_created_by[$i]).'</div></td><td class="shipmentTB3" align="right"><div id="col7_'.$qm_id[$i].'">'.number_format($qm_amount[$i]).'</div></td><td align="right" class="shipmentTB3" ><div id="col8_'.$qm_id[$i].'" ></div></td><td align="center" style="color:'.$qm_status_color[$i].'" bgcolor="grey">'.$qm_st.'</td></tr>';
	}
?>	
	</table>
</div>	
