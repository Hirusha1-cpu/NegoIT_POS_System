...
<script type="text/javascript">
	...
	<?php if(($systemid == 13) && ($components != 'to')){ ?>
		function updateItemUnitPrice($id){
			var price=document.getElementById("poitemid_unit_price"+$id).value;
			var salesman=document.getElementById('salesman').value;
			var cust=document.getElementById('cust').value;
		...
		function updateQoDiscount(id){
			var salesman=document.getElementById('salesman').value;
			var cust=document.getElementById('cust').value;
			var discount=document.getElementById('discount_for_qo').value;
			if((discount != 0) && (discount > 0)){
				var check= confirm('Are you sure you want to apply a discount of '+discount+' to this quotation?');
				if(check== true){
					window.location = 'index.php?components=<?php print $components; ?>&action=qo_update_discount&id='+id+'&discount='+discount+'&s='+salesman+'&cust='+cust;
				}
			}else{
				alert('Discount value cannot be zero or a negative value');
	        	return false;
			}
		}

		function removeQoDiscount(id){
			var salesman=document.getElementById('salesman').value;
			var cust=document.getElementById('cust').value;
			var check= confirm('Are you sure you want to remove the discount of this quotation?');
			var discount=document.getElementById('discount_for_qo').value;
			if(check== true){
				window.location = 'index.php?components=<?php print $components; ?>&action=qo_remove_discount&id='+id+'&s='+salesman+'&cust='+cust;
			}
		}
	<?php } ?>
	...
	function updateItemComment(qi_id) {
		var comment = document.getElementById('comment_' + qi_id).value;
		var statusElement = document.getElementById('update_status_' + qi_id);
		statusElement.innerHTML=document.getElementById('loading').innerHTML;
		var xhr = new XMLHttpRequest();
		xhr.open('POST', 'index.php?components=<?php print $components; ?>&action=qo_update_comment&qi_id='+qi_id, true);
		...
		xhr.send('qi_id=' + qi_id + '&comment=' + encodeURIComponent(comment));
	}
</script>
...
<table align="center">
	<tr>
		<td valign="top">
			<table width="100%" style="border-radius: 15px; margin:0 auto; background-color:#EEEEEF; border-radius: 15px; padding-left:10px; padding-right:10px;" >
				<tr>
					<td>
						<h1 style="color:#0158C2;">Quotation</h1>
					</td>
					<td align="right">
						<select name="district" id="district" onchange="setDistrict('<?php print $components; ?>')" <?php if($static_district!=0) print 'disabled="disabled"'; ?> >
			...
			<?php if($current_district!=''){
				if(isset($_REQUEST['id'])) print '<form id="billingForm" action="index.php?components='.$components.'&action=apend_quot" onsubmit="return validateQuotation()" method="post" >';
				else print '<form id="billingForm" action="index.php?components='.$components.'&action=new_quot" method="post" >';
				?>
					...
					<table align="center" bgcolor="<?php print $main_tale_color; ?>" style="font-size:10pt; border-radius: 15px;">
						<tr>
							<td colspan="5">
								<?php
		...
		<td valign="top">
			<!-- ------------------Item List----------------------- -->
			<div style="background-color:#D1D8F1; border-radius: 5px; padding-left:20px; padding-right:20px; height:500px;">
				<br />
				<?php if(isset($_REQUEST['id'])) { ?>
					<span style="font-weight:bold; font-size:12pt;">
						Quotation No: <?php print str_pad($_REQUEST['id'], 7, "0", STR_PAD_LEFT); ?>
					</span>
					<br /><br /><br />
					<table align="center" bgcolor="#E5E5E5" style="border-radius: 5px;" >
						<thead>
							...
							<?php if(($systemid == 13) && ($components != 'to')) { ?> <th align="center">Unit Price</th> <?php } ?>
							<th align="center">QTY</th>
							 <?php if(($systemid == 13) && ($components != 'to')) { ?><th align="center">Total</th> <?php } ?>
						</thead>
						<?php
							for($i=0;$i<sizeof($qo_itm_des);$i++){
								$update_button='<input type="Button" value="Update"  onclick="updatePoi('.$qi_id[$i].')" />';
								$unit_price_update_button = '';
								$td_unit_price = '';
								$td_total = '';
								$total_tr='';
								$comment_update_button = '';
								if(($systemid == 13) && ($components != 'to')){
									$unit_price_update_button='<input type="Button" value="Update"  onclick="updateItemUnitPrice('.$qi_id[$i].')" />';
									$comment_update_button='<input type="Button" value="Update Comment"  onclick="updateItemComment('.$qi_id[$i].')" />';
									$td_unit_price='<td width="140px">
											<input style="width:70px; text-align: right;" type="number" id="poitemid_unit_price'.$qi_id[$i].'" value="'.$qo_itm_uprice[$i].'"/>'.$unit_price_update_button.'
										</td>';
									$td_total='<td align="right" width="80px;"><input style="width:80px; text-align: right;" type="text" id="poitemid_unit_price'.$qi_id[$i].'" value="'.number_format($qo_itm_uprice[$i]*$qo_itm_qty[$i],$decimal).'" disabled/></td>';
								}
								$remove_button='<input type="Button" value="x"  onclick="removePoi('.$qi_id[$i].')" style="background-color:maroon; color:white"/>';
								print '<tr style="font-size:12pt;height: 20px;">
										<td width="20px" style="color:blue">
											<strong><span>'.($i+1).'</span></strong>
										</td>
										<td style="padding-right:5px;"><span>'.$qo_itm_des[$i].'</span></td>
										'.$td_unit_price.'
										<td align="center" width="160px;">
											<input style="width:60px; text-align: right;" type="number" id="poitemid'.$qi_id[$i].'" value="'.$qo_itm_qty[$i].'" />'.$update_button.' '.$remove_button.'
										</td>
										'.$td_total.'
									</tr>
									<tr style="font-size:12pt;height: 20px;">
										<td width="20px">
										</td>
										<td colspan="2" align="left">
											<textarea style="width:100%; 18px" id="comment_'.$qi_id[$i].'">'.$qo_comment[$i].'</textarea>
										</td>
										<td align="right" width="160px;">
											'.$comment_update_button.'
										</td>
										<td id="update_status_'.$qi_id[$i].'" with="100px" style="font-weight:bold;" align="right"></td>
									</tr>';
							}
							if($components != 'to'){
								print '
									<tr style="font-size:12pt; font-weight:600;">
										<td colspan="2" style="padding-top: 10px;">Total Amount</td>
										<td width="50px" style="padding-top: 10px;"></td>
										<td align="right" colspan="2" style="padding-top: 20px;">'.number_format($total,$decimal).'</td>
									</tr>';
							}
						?>
					</table>
					<?php if(($systemid == 13) && ($components != 'to') && ($total > 0)){ ?>
						<br>
						<hr>
						<table style="font-weight:bold; font-size:12pt; width:100%;">
							<?php
								if(isset($qo_discount) && ($qo_discount > 0) && ($total > 0)){
									print '<tr style="font-size:12pt; font-weight:600;">
										<td colspan="1" style="padding: 10px 0px;">Sub Total Amount</td>
										<td width="50px" style="padding: 10px 0px;"></td>
										<td align="right" colspan="1" style="padding: 10px 0px;">'.number_format(($total-$qo_discount),$decimal).'</td>
									</tr>';
								}
							?>
							<tr>
								<td align="left">Discount for Quotation (<?php print $currency; ?>)</td>
								<td align="right" width="70px;">
									<input type="number" name="discount_for_qo" id="discount_for_qo" value="<?php if(isset($qo_discount)) print $qo_discount; ?>" placeholder="" style="width:60px; text-align: right;"></td>
								<td align="right" style="width:100px;">
									<input type="Button" value="Update"  onclick="updateQoDiscount(<?php print $_GET['id']; ?>)" />
									<input type="Button" value="x"  onclick="removeQoDiscount(<?php print $_GET['id']; ?>)" style="background-color:maroon; color:white"/>
								</td>
							</tr>
						</table>
					<?php } ?>
				<?php }else{ ?>
					...
					<form id="searchquo" action="index.php?components=<?php print $components; ?>&action=search_quot" method="post">
						<table align="center" height="100%">
							<tr>
								<td style="font-size:12pt;">
									<input type="text" style="width:110px" name="search1" id="search1" placeholder="Search Quotation" /><input type="Submit" value="Search" />
								</td>
							</tr>
						</table>
					</form>
