...
if ($storedisable != '') {
	if ($_GET['store'] != $global_store_id) {
		print '<script>window.location = ' . "'index.php?components=$components&action=sale&store=$global_store_id&salesman=all&processby=all&lock=1&type='" . ';</script>';
	}
}
if ($userdisable != '') {
	if ($_GET['salesman'] != $global_user_id) {
		print 'aaa';
		print '<script type="text/javascript">window.location = ' . "'index.php?components=$components&action=sale&store=$global_store_id&salesman=$global_user_id&processby=all&lock=1&type='" . ';</script>';
	}
}
...
<table align="center" height="100%" cellspacing="0" border="0"
	style="font-family:Calibri; font-size:10pt; border-radius: 15px; padding-left:10px; padding-right:10px"
	bgcolor="#F0F0F0" border="0">
	<tr>
		<td colspan="8">
			<table style="font-family:'Courier New', Courier, monospace">
				<tr>
					<td colspan="3">Report by Invoice Date</td>
				</tr>
				<tr>
					...
					<td>
						<div id="div_submit"><a onclick="getSalesReport()" style="cursor:pointer"><img src="images/search.png"
									style="width:30px; vertical-align:middle" /></a></div>
					</td>
				</tr>
			</table>
		</td>
			...
			<select id="store0" <?php print $storedisable; ?> onchange="getSalesReport()">
				...
				for ($i = 0; $i < sizeof($store_id); $i++) {
					if ($store_id[$i] == $_GET['store']) {
						$select = 'selected="selected"';
						$selectedstore = ucfirst($store_name[$i]);
					} else
						$select = '';
				...
				<select id="group0" onchange="getSalesReport()">
					...
					for ($i = 0; $i < sizeof($gp_id); $i++) {
						if ($gp_id[$i] == $_GET['group']) {
							$select = 'selected="selected"';
							$selectedgroup = ucfirst($gp_name[$i]);
						} else
							$select = '';
		...
		<td>
			<select id="lock" onchange="getSalesReport()">
				<option value="1" <?php if ($lock_req == 1)
					print 'selected="selected"'; ?>>Lock</option>
				<option value="0" <?php if ($lock_req == 0)
					print 'selected="selected"'; ?>>Unlock</option>
				<option value="all" <?php if ($lock_req == 'all')
					print 'selected="selected"'; ?>>--ALL--</option>
			</select>
		</td>
	...
	<table style="font-size:12pt" border="1" cellspacing="0">
		<?php if (($date1 != '') && ($date2 != '')) { ?>
			<tr>
				...
				<td style="background-color:#EEEEEE; padding-left:10px">&nbsp;&nbsp;<?php print $date1; ?>&nbsp;&nbsp;</td>
			</tr>
			<tr>
				<td style="background-color:#C0C0C0; padding-left:10px">&nbsp;&nbsp;To &nbsp;&nbsp;&nbsp;</td>
				<td style="background-color:#EEEEEE; padding-left:10px">&nbsp;&nbsp;<?php print $date2; ?>&nbsp;&nbsp;</td>
			</tr>
		<?php } else { ?>
			<tr>
		...
		<?php } ?>
		<tr>
			<td style="background-color:#C0C0C0; padding-left:10px">&nbsp;&nbsp;Store &nbsp;&nbsp;&nbsp;</td>
			<td style="background-color:#EEEEEE; padding-left:10px">
				&nbsp;&nbsp;<?php print $selectedstore; ?>&nbsp;&nbsp;</td>
		</tr>
		<tr>
			<td style="background-color:#C0C0C0; padding-left:10px">&nbsp;&nbsp;Group &nbsp;&nbsp;&nbsp;</td>
			<td style="background-color:#EEEEEE; padding-left:10px">
				&nbsp;&nbsp;<?php print $selectedgroup; ?>&nbsp;&nbsp;</td>
		</tr>
		<tr>
			<td style="background-color:#C0C0C0; padding-left:10px">&nbsp;&nbsp;Salesman &nbsp;&nbsp;&nbsp;</td>
			<td style="background-color:#EEEEEE; padding-left:10px">
				&nbsp;&nbsp;<?php print $selectedsalesman; ?>&nbsp;&nbsp;</td>
		</tr>
	</table><br />
	<p>Note: This report shows the total Sales of Supervisor's Group </p>
	<hr>
</div>
...
<div id="print">
	<?php if ($type_req != 3 && $type_req != 5) { ?>
		<table align="center" bgcolor="#E5E5E5" height="100%" border="1" cellspacing="0" style="font-family:Calibri">
			...
			<tr>
				...
				<?php if ($tax_rate > 0): ?>
					<th width="80px">My Sale</th>
					<th width="80px">Tax Price</th>
				<?php endif; ?>
			...
			for ($i = 0; $i < sizeof($invoice_no); $i++) {
				...
				if ($store0 != $billed_store[$i]) {
					...
					print '<tr>
						<td colspan="13" style="padding-left:20px; font-weight:bold; background-color:#0066AA; color:white">' . $billed_store[$i] . '</td>
					</tr>';
				}
				...
				if ($tax_rate > 0) {
					$netSale = $invoice_Total[$i] / (1 + ($tax_rate / 100));
					$taxAmount = $invoice_Total[$i] - $netSale; // Calculate the tax amount
					print '<td align="right" style="padding-right:10px;">' . number_format($netSale, $decimal) . '</td>';
					print '<td align="right" style="padding-right:10px;">' . number_format($taxAmount, $decimal) . '</td>';
				}
				print '<td align="right" style="padding-right:10px;">' . number_format($payment_cash[$invoice_no[$i]], $decimal) . '</td>
				...
				if (sizeof($invoice_no) == ($i + 1)) {
					$colspan = $tax_rate > 0 ? 6 : 4;
					print '<tr style="font-weight:bold; background-color:gray; color:white">
						<td colspan="' . $colspan . '" align="right" style="padding-right:10px;">Total</td>
	...
	<?php if ($type_req != 3 && $type_req != 5) { ?>
		<table align="center" bgcolor="#E5E5E5" height="100%" border="1" cellspacing="0" style="font-family:Calibri">
			<tr>
				<td colspan="12" style="border:0; background-color:black; color:white; font-weight:bold">Payment Collection
				</td>
			</tr>
			<tr>
				<th width="50px">#</th>
				...
				<th width="80px">Chque No</th>
				<th width="120px">Chque Date</th>
				<th width="120px">Salesman</th>
				<th width="300px">Customer</th>
			</tr>
			<?php
			...
			for ($i = 0; $i < sizeof($payment_id); $i++) {
				...
				if ($payment_type[$i] == 'Cash')
					$py_cash = $payment_amount[$i];
				if ($payment_type[$i] == 'Card')
					$py_card = $payment_amount[$i];
				if ($payment_type[$i] == 'Bank')
					$py_bank = $payment_amount[$i];
				if ($payment_type[$i] == 'Chque')
					$py_chque = $payment_amount[$i];
				...
				if ($store0 != $payment_store[$i]) {
					$total_payment = 0;
					print '<tr>
						<td colspan="12" style="padding-left:20px; font-weight:bold; background-color:#0066AA; color:white">' . $payment_store[$i] . '</td>
					</tr>';
				}
				print '<tr>
					<td align="center">
					...
					<td align="right" class="shipmentTB3"><a href="" style="text-decoration:none;">' . $payment_cheque_no[$i] . '</a></td>
					<td align="center" class="shipmentTB3"><a href="" style="text-decoration:none;">' . $payment_cheque_date[$i] . '</a></td>
				...
				<td align="right" class="shipmentTB3">' . number_format($py_total_chque, $decimal) . '</td>';
					print '<td colspan="4" align="right" class="shipmentTB3">' . number_format($total_payment, $decimal) . '</td></tr>';
					print '</table><br /><table align="center" bgcolor="#E5E5E5" height="100%" border="1" cellspacing="0">';
				}
