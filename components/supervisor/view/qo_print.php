...
<table align="center" style="font-family:Calibri">
					...
					<tr>
						<td>
							<?php if ($edit_note0 == '')
								...
								<form
	action="index.php?components=<?php print $components; ?>&action=<?php print $action0; ?>&id=<?php print $view_id; ?>"
	...
	if ($edit_note0 == '') {
		...
		print '<input type="hidden" name="comid" value="' . $view_comid . '" />';
		print '<div id="notesend"><input type="submit" value="Update" /> <input type="button" value="Cancel" onclick="window.location = \'index.php?components=supervisor&action=qo_finish&id=' . $view_id . '\'" /></div>';
	}
	?>
</form>
							<!-- <form
			...
			<div>
				<table cellspacing="0">
					<tr>
						<td style="font-weight:bold; color:#467898">Quotation Comment</td>
					</tr>
					<tr>
						<td>
							<table border="0" bordercolor="#C0C0C0" cellspacing="0" width="300px">
								<tr>
									<td>
										<form action="index.php?components=<?php print $components; ?>&action=update_qo_main_comment"
											method="POST">
											<textarea name="qo_main_comment" style="width: 100%;"
												id="qo_main_comment"><?php print html_entity_decode($main_comment); ?></textarea>
											<br />
											<input type="hidden" name="qo_main_no"
												value="<?php print isset($_GET['id']) ? $_GET['id'] : ''; ?>" />
			...
			<table align="center" style="font-size:11pt">
				...
				<tr>
						...
						if ($tm_template == 1 || $tm_template == 2) {
							...
							if ($systemid == 13) { // techneeds tax invoice
								if (isset($is_quotation_sent_with_tax) && $is_quotation_sent_with_tax) {
									$show_excluded_param = (true) ? '&show_excluded=1' : '&show_excluded=0';
									print '<iframe id="quot_iframe_3" width="' . $page_width . 'px" height="' . $page_height . 'px" src="components/supervisor/view/tpl/quotation_print3_2.php?id=' . $_GET['id'] . '&action=' . $action . '&sub_components=' . $components . $show_excluded_param . '"></iframe>';
								} else {
					...
					<td valign="top" align="center">
						<?php
						// if ($qm_status <= 4 && $systemid == 13 && (!$qo_price_inc_tax != 1)) {
						if ($qm_status <= 4 && $systemid == 13) { ?>
							<!-- techneeds tax invoice -->
							<div class="checkbox-container prtbutton3">
								<input style="color:black" type="checkbox" id="is_quotation_sent_with_tax" <?php echo ($is_quotation_sent_with_tax) ? 'checked' : ''; ?>>
								<label for="is_quotation_sent_with_tax" style="color:black">Is Quotation Sent With Tax?</label>
							</div>
							<br>
						<?php } ?>
...
<script>
	<?php if ($systemid == 13) {
		...
		document.getElementById("show_excluded").addEventListener("change", function () {
			const id = '<?php echo $qo_id; ?>';
			var checkbox = document.getElementById("show_excluded");
			if (checkbox.checked) {
				window.location = 'index.php?components=<?php echo $components; ?>&action=quotation_sent_with_tax_format&flag=1&id=' + id;
			}
		});
	<?php } ?>
</script>