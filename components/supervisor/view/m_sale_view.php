...
if ($storedisable != '') {
	if ($_GET['store'] != $global_store_id) {
		print '<script>window.location = ' . "'index.php?components=$components&action=sale&store=$global_store_id&salesman=all&processby=all&lock=1&type='" . ';</sctript>';
	}
}
if ($userdisable != '') {
	if ($_GET['salesman'] != $global_user_id) {
		print 'aaa';
		print '<script type="text/javascript">window.location = ' . "'index.php?components=$components&action=sale&store=$global_store_id&salesman=$global_user_id&processby=all&lock=1&type='" . ';</sctript>';
	}
}
		...
		<div class="w3-col">
			...
			<div class="table-responsive">
				<table align="center" height="100%" cellspacing="0" width="98%" style="font-size:x-small;">
						...
						<td colspan="4">
							<table style="font-family:'Courier New', Courier, monospace">
								<tr>
									...
									<td>
										<div id="div_submit"><a onclick="getSalesReport()" style="cursor:pointer"><img
													src="images/search.png" style="width:30px; vertical-align:middle" /></a></div>
									</td>
								</tr>
							</table>
						</td>
					...
					if ($components == 'supervisor') {
						?>
						<tr>
							<td width="100px"><strong>Group : </strong></td>
							<td width="150px">
					...
					<tr>
						<td width="100px"><strong>Salesman : </strong></td>
						<td>
					...
					<table bgcolor="#E5E5E5" height="100%" border="1" cellspacing="0" style="font-size:x-small;" align="center">
						<tr>
							...
							<td colspan="<?php echo $colspan; ?>"
								style="border:0; background-color:black; color:white; font-weight:bold">Invoiced Collection</td>
						</tr>
						<tr>
							<th width="50px">#</th>
						...
						for ($i = 0; $i < sizeof($invoice_no); $i++) {
							...
							print '<td align="right" style="padding-right:10px;">' . number_format($payment_cash[$invoice_no[$i]], $decimal) . '</td>
											...
											<td  align="right" style="padding-right:10px;">' . number_format($bill_credit, $decimal) . '</td><td align="center">' . ucfirst($billed_by[$i]) . '</td>
			...
			<?php if ($type_req != 3 && $type_req != 5) { ?>
				<div class="table-responsive">
					<table bgcolor="#E5E5E5" height="100%" border="1" cellspacing="0" style="font-size:x-small;" align="center">
						<tr>
							<td colspan="8" style="border:0; background-color:black; color:white; font-weight:bold">Payment Collection
							</td>
						</tr>
						<tr>
							<th width="50px">#</th>
						...
						for ($i = 0; $i < sizeof($payment_id); $i++) {
							if ($store0 != $payment_store[$i]) {
								$total_payment = 0;
								print '<tr>
											<td colspan="8" style="padding-left:20px; font-weight:bold; background-color:#0066AA; color:white">' . $payment_store[$i] . '</td>
										</tr>';
							}
							print '<tr>
										<td align="center">
