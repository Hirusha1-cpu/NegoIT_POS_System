	...
	<div class="w3-row">
		...
		<div class="w3-col">
			<form action="index.php?components=<?php print $components; ?>&action=credit" method="post">
				<table width="100%" height="100%" cellspacing="0" border="0" style="font-size:10pt; border-radius:10px"
					bgcolor="#EFEFEF">
					<tr>
					...
					<?php if ($_GET['components'] == 'report' || $_GET['components'] == 'supervisor' || $components == 'manager' || $components == 'topmanager' || (($systemid == 24) && $components == 'bill2')) { ?>
						<tr>
					...
					<tr>
						...
						<td>
							<input type="date" name="as_of" id="as_of" value="<?php print $as_of; ?>" onchange="filter()" />
						</td>
					</tr>
					<tr>
				...
				<table align="center">
					<tr>
						...
						for ($j = 0; $j < sizeof(array_reverse($days)); $j++) {
							$check = '';
							if ((sizeof($days) >= 2) && ($j === sizeof($days) - 2) || ($j === sizeof($days) - 1))
								$check = 'checked="checked"';
							print '<td class="shipmentTB3" bgcolor="#F5F5F5">' . $days[$j] . '+ <input type="checkbox" id="f' . ($j + 1) . '" ' . $check . ' onclick="showHideCols(' . ($j + 1) . ')" /></td>';
						}
						?>
						<td class="shipmentTB3" bgcolor="#F5F5F5">Now <input type="checkbox" id="f7" checked="checked"
								onclick="showHideCols(7)"></td>
					</tr>
				</table>
			</div>

			<table id="credittable" align="center" height="100%" border="1" cellspacing="0"
				...
				print '<tr>
					<th align="right" style="padding-right:10px; padding-left:10px">Total Amount</th>';
				for ($k = 0; $k < sizeof($days); $k++) {
					$day = $days[$k];
