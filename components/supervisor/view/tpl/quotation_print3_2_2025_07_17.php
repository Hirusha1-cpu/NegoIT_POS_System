		...
		<tr>
			<td rowspan="2" style="font-family:Calibri; font-size:11pt; vertical-align:top">
				...
				<table style="font-size:12pt;" cellspacing="0">
                    ...
                    <?php if($trn_no != ''){ ?>
                    <tr>
                        <td><?php if($systemid == 13) print 'VAT Reg. No. '; else print 'TRN No '; ?></td>
				...
				'<tr style="font-size:10pt; vertical-align:top" height="20px">
					...
					if(($components != 'to')){
						print '<td width="50px" style="border-bottom:0; border-top:0;" align="right">
								<div id="uprice_'.$i.'" style="padding: 2px 5px;">'.number_format($qi_uprice[$i],$decimal).'</div>
								</td>
								<td align="right" style="border-bottom:0; border-top:0;">
									<div id="tprice_'.$i.'" style="padding: 2px 5px;">'.number_format($qi_qty[$i]*$qi_uprice[$i],$decimal).'</div>
							</td>';
					}
			print '</tr>';
			...
			if($qo_discount > 0){
				print '<tr
				    ...
				    <td colspan="4" align="right" height="20px" style="padding-right:5px; border-right:1; border-bottom:0; border-top:0;">Discount
				        '.round(((($qo_discount)*100)/($total-$total_discount)),2).'% &nbsp;&nbsp; '.$currency.' </td>
				    <td align="right" style="border-bottom:0;">'.number_format($qo_discount,$decimal).'&nbsp;&nbsp;</td>
				</tr>';
			}

			// Payable amount excluding VAT
			print '<tr style="font-family:Arial; font-size:10pt; font-weight:600; color:white; background-color:black; -webkit-print-color-adjust: exact;">
				<td colspan="4" align="right" height="20px" style="padding-right:5px; border-right:1; border-bottom:0; border-top:0;">
					Payable Amount Excluding VAT &nbsp;&nbsp; '.$currency.'
				</td>
				<td align="right" style="border-bottom:0;">'.number_format((($total-$qo_discount)-$tax_added_value),$decimal).'&nbsp;&nbsp;</td>
			</tr>';

			// vat
            if(($tax != '') && ($tax != 0)) {
				print '<tr style="font-family:Arial; font-size:10pt; font-weight:600; color:white; background-color:black; -webkit-print-color-adjust: exact;"><td colspan="4" align="right" height="20px" style="padding-right:5px; border-right:1; border-bottom:0; border-top:0;">VAT '.$tax.'% &nbsp;&nbsp; '.$currency.' </td><td align="right" style="border-bottom:0;">'.number_format($tax_added_value,$decimal).'&nbsp;&nbsp;</td></tr>';
			}

			// Grand total
			print '<tr style="font-family:Arial; font-size:10pt; font-weight:600; color:white; background-color:black; -webkit-print-color-adjust: exact;">
					<td colspan="4" align="right" height="20px" style="padding-right:5px; border-right:1; border-bottom:0; border-top:0;">Grand Total &nbsp;&nbsp; '.$currency.' </td>
					<td align="right" style="border-bottom:0;">
						<div id="total">'.number_format(($total-$qo_discount),$decimal).'&nbsp;&nbsp;</div>
					</td>
				</tr>';
		}
...
<table cellspacing="0"  style="font-size:11pt; font-family:Calibri" >
	<?php
		function convertMonthsToYearsMonths($qo_warranty) {
			...
			if ($qo_warranty <= 12) {
				return "$qo_warranty Month(s)";
			}

			$years = floor($qo_warranty / 12);
