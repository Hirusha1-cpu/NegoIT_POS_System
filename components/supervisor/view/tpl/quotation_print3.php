...
<?php if (($systemid == 13 && $sub_system == 0) || ($systemid != 13)) { ?>
	<table width="100%">
					...
					<tr>
						<td width="50px">
							...
							if (($components != 'to')) {
								...
								if (!empty($qo_comment)) {
									print '<div style="text-decoration:none; color:black; margin-top:2px; display:block; padding-top: 2px; width: max-content; max-width: 400px;">' . $qo_comment . '</div>';
								}
							} else {
		...
		<tr>
			<td></td>
			<td align="right"
				style="font-family:Calibri; font-size:12pt; <?php (!empty($qo_comment)) ? print '' : print 'padding-top: 34px;' ?> ">
				<table cellspacing="">
					<tr>
						<td>Date</td>
						<td> : </td>
