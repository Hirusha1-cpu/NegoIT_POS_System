...
if(!isset($_GET['s'])||$current_district=='') print '<input type="hidden" id="salesman" value="'.$_COOKIE['user_id'].'" />';
?>
...
<tr><td><h1 style="color:#0158C2;">Quotation</h1></td>
<td align="right">
...
<input type="hidden" name="unic_item" value="1" />
	...
	<tr><td width="50px"></td><td style="font-size:12pt">Comment</td><td colspan="2"><textarea name="comment" ></textarea></td><td width="50px"></td></tr>
	<tr><td colspan="2" align="right"><br /><div id="addtobill"><?php if(sizeof($qo_itm_des)<$apend_limit) print '<input type="submit" value="Add to Quot" style="width:100px; height:30px" onclick="cashBack()" />'; ?></div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	<?php if(sizeof($qo_itm_des)>0) { ?>
		</td><td colspan="3" align="left"><div id="finalyze_div"><input type="Button" value="Finalyze" style="width:100px; height:70px" onclick="movetoTerms(<?php print $_GET['id']; ?>)" /></div>
	<?php } ?>
	<br /><br /></td></tr>
	<?php } ?>
	</table>
	...
	<table align="center" bgcolor="#E5E5E5" style="border-radius: 5px;" >
		<?php
			for($i=0;$i<sizeof($qo_itm_des);$i++){
				...
				print '<tr style="font-size:12pt"><td width="30px" style="color:blue"><strong>'.($i+1).'</strong></td><td>'.$qo_itm_des[$i].'</td><td width="50px"></td><td align="right"><input style="width:50px; type="text" id="poitemid'.$qi_id[$i].'" value="'.$qo_itm_qty[$i].'" /> '.$update_button.' '.$remove_button.' </td><td width="80px" align="right">'.number_format(($qo_itm_uprice[$i]*$qo_itm_qty[$i]),2).'</td></tr>';
			}
				print '<tr style="font-size:12pt; font-weight:900;"><td colspan="2">Total Amount</td><td width="50px"></td><td align="right" colspan="2">'.number_format($total,2).'</td></tr>';
		?>
	</table>
<?php }else{ ?>
<br />
	...
	<tr><td style="font-size:12pt;"><input type="text" style="width:110px" name="search1" id="search1" placeholder="Search Quotation" /><input type="Submit" value="Search" /></td></tr>
	</table>
	</form>
<?php } ?>
</div>
