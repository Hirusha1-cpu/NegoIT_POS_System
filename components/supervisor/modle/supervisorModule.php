...
{
	global $cu_id, $cu_id0, $cu_name, $cu_name0, $cu_nic, $cu_mobile, $cu_status, $cu_sub_sys,$conn2;
	include('config.php');
...
{
	global $store_id, $store_name, $up_id, $up_name, $gp_id, $gp_name, $town_id, $town_name, $item_id, $item_desc, $conn2;
	...
	if ($_GET['components'] == 'marketing') {
		...
		$query = "SELECT id, `name` FROM cust_group $sub_system_qry2 ORDER BY `name`";
	}
	$result = mysqli_query($conn2, $query);
	while ($row = mysqli_fetch_array($result)) {
		$gp_id[] = $row[0];
...
// update by nirmal 03_06_2024 (03_06_2024 change invoice list order by to invoice no to systemid == 25)
// update by nirmal 10_07_2024 (added cheque no, cheque date to payment query)
// update by nirmal 15_10_2024 (added if conditions before run loop of queries prevent null queries)
...
function dailySale($store)
{
	...
	$invoice_no = $invoice_Total = $billed_district = $billed_by = $billed_cust = $billed_time = $billed_store = $bi_discount = $payment_cash = $payment_chque =
		$payment_card = $payment_bank = $payment_id = $payment_amount = $payment_type = $payment_salesman = $payment_cust = $payment_time =
		$payment_store = $payment_cheque_no = $payment_cheque_date = $rtn_no = $rtn_time = $rtn_pay = $rtn_salesman = $rtn_store = $rtn_cust = $wa_no =
		$wa_time = $wa_pay = $wa_salesman = $wa_entity = $wa_store = $graph_user = $graph_total = $item_type = $invoice_tax = $invoice_total_wo_tax = array();
	...
	if ($type_req != 3 && $type_req != 5) {
		if ($_GET['components'] == 'marketing') {
			$query = "SELECT bm.`invoice_no`, (bm.`invoice_+total` + bm.`invoice_-total`) AS total_with_tax, di.name, up.username, cu.name, date(bm.billed_timestamp), time(bm.billed_timestamp), st.name,
			bm.tax, (SUM(bi.qty*bi.unit_price) - bm.tax) AS total_wo_tax
			...
			$lock_qry $type_qry $date_qry1 $group_qry $storesearch $salesmansearch $processbysearch GROUP BY bm.invoice_no ORDER BY $order_by";
		} else {
			$query = "SELECT bm.`invoice_no`, SUM(bi.qty*bi.unit_price) AS total_with_tax, di.name, up.username, cu.name, date(bm.billed_timestamp), time(bm.billed_timestamp),
			st.name, SUM(bi.discount), bm.tax, (SUM(bi.qty*bi.unit_price) - bm.tax) AS total_wo_tax
		...
		if ($result) {
			while ($row = mysqli_fetch_array($result)) {
				...
				$invoice_tax[] = $row[9]; // tax amount only
				$invoice_total_wo_tax[] = $row[10]; // total without tax
	...
	// Payments
	$query2 = "SELECT py.id,round(py.amount),py.payment_type,up.username,cu.name,date(py.payment_date),time(py.payment_date), st.name, py.chque_no, py.chque_date
	FROM payment py, userprofile up, cust cu, stores st
	WHERE py.salesman=up.id AND py.cust=cu.id AND py.store=st.id AND py.bill_pay=2 AND py.`status`=0 $date_qry2 $storesearch $group_qry $salesmansearch ORDER BY st.name, py.payment_date";
	...
	if ($result2) {
		while ($row2 = mysqli_fetch_array($result2)) {
			if ($row2[0] > 0) {
				...
				$payment_cheque_no[] = $row2[8];
				$payment_cheque_date[] = $row2[9];
			} else
				$payment_id = $payment_amount = $payment_type = $payment_salesman = $payment_cust = $payment_time = $payment_store = array();
		}
	}
	...
	if ($type_req == 3 || $type_req == '') {
		...
		$date_qry3 $storesearch $group_qry $salesmansearch
	...
	if ($type_req == 5 || $type_req == '') {
		...
		WHERE wa.store=st.id AND wa.cust_pay_by=up.id AND wa.`status`!=0 AND wa.cust_pay_amount!=0 $date_qry4 $storesearch $salesmansearch";
		$result3 = mysqli_query($conn2, $query3);
		if ($result3) {
			while ($row3 = mysqli_fetch_array($result3)) {
				$wa_no[] = $row3[0];
		...
		FROM warranty wa, userprofile up, stores st
		 WHERE wa.store=st.id AND wa.suplier_pay_by=up.id AND wa.`status`!=0 AND wa.suplier_pay!=0 $date_qry5 $storesearch $salesmansearch";
		$result3 = mysqli_query($conn2, $query3);
		if ($result3) {
			while ($row3 = mysqli_fetch_array($result3)) {
				$wa_no[] = $row3[0];
...
// 	include('config.php');
// 	$systemid = inf_systemid(1);
// 	$store_payment_filter = '';

// 	if ($sub_system == 'all')
// 		$sub_system_qry = '';
// 	else
// 		$sub_system_qry = "AND cu.`sub_system`='$sub_system'";
// 	if ($_REQUEST['components'] == 'report' || $_REQUEST['components'] == 'topmanager') {
// 		$storefilter2 = '';
// 	} else {
// 		$storefilter2 = '';
...
// 	} else
// 		$townfilter = '';
// 	if (isset($_GET['up'])) { // salesman filter disable for system id 24 because salesman are store in that system
// 		if ($_GET['up'] != '') {
// 			$sm_id = $_GET['up'];
// 			if ($systemid == 24) {
// 				if ($_REQUEST['components'] == 'bill2' || $_REQUEST['components'] == 'supervisor') {
// 					$store = $_COOKIE['store'];
// 					$salemanfilter = "AND bm.store='$store'";
// 					$store_payment_filter = "AND py.store='$store'";
// 				}
// 				if ($_REQUEST['components'] == 'report' || $_REQUEST['components'] == 'manager' || $_REQUEST['components'] == 'topmanager') {
// 					$query = "SELECT `store` FROM userprofile WHERE id=$sm_id";
// 					$row = mysqli_fetch_row(mysqli_query($conn2, $query));
// 					$salesman_store = $row[0];
// 					$salemanfilter = "AND bm.store='$salesman_store'";
// 					$store_payment_filter = "AND py.store='$salesman_store'";
// 				}
// 			} else {
// 				$salemanfilter = "AND cu.associated_salesman='$sm_id'";
// 			}
...
// 		// new code start
// 		// ${'cashback_bm_cust' . $day} = array();
// 		// ${'cashback_bm_total' . $day} = array();
// 		// new code end
// 	}
...
// 			}
// 		} else {
// 			if (!validateGroupPermission($user_id, $gp_id))
// 				$out = false;
// 		}
// 	}
...
// 		}
// 		// This query aggregates the total invoice amounts for each customer, which is a crucial part of calculating the customer's credit balance
// 		// if customer doesn't have invoice but have payment
// 		if ($systemid == 24) {
// 			$query = "SELECT cu.id, cu.name, COALESCE(SUM(bm.`invoice_+total`), 0) + COALESCE(SUM(bm.`invoice_-total`), 0) AS total_invoice
// 					FROM cust cu
// 					LEFT JOIN bill_main bm ON bm.`cust` = cu.id AND bm.exclude = 0 AND bm.`status` NOT IN ($exclude_status) AND bm.`lock` = 1 AND date(bm.billed_timestamp) <= '$as_of' $salemanfilter
// 					LEFT JOIN ( SELECT py.`cust`, SUM(py.amount) AS amount FROM payment py WHERE py.status = 0 $store_payment_filter AND py.chque_return = 0 AND date(py.payment_date) <= '$as_of' GROUP BY py.`cust`) py ON py.cust = cu.id
// 					WHERE cu.`status` != 0 $sub_system_qry $storefilter $townfilter $groupfilter
// 					GROUP BY cu.id, cu.name
// 					ORDER BY cu.name";
// 		} else {
// 			$query = "SELECT cu.id,cu.name,SUM(bm.`invoice_+total`)+SUM(bm.`invoice_-total`)
// 			FROM bill_main bm, cust cu WHERE bm.`cust`=cu.id AND bm.exclude=0 AND bm.`status` NOT IN ($exclude_status) AND bm.`lock`=1 AND cu.`status`!=0 AND date(bm.billed_timestamp)<='$as_of'
// 			$sub_system_qry $storefilter $townfilter $groupfilter $salemanfilter GROUP BY bm.`cust` ORDER BY cu.name";
// 		}
// 		$result = mysqli_query($conn2, $query);
// 		while ($row = mysqli_fetch_array($result)) {
// 			$cust_id[] = $row[0];
...
// 		//This query aggregates the total payment amounts for each customer, which is essential for calculating the customer's credit balance by offsetting their total invoices with their payments.
// 		// It excludes returned cheques and inactive payments and considers only payments up to a certain date
// 		$query = "SELECT py.`cust`,SUM(py.amount) FROM payment py WHERE py.status=0 AND py.chque_return=0 AND date(py.payment_date)<='$as_of' GROUP BY py.`cust`";
// 		if ($systemid == 24) {
// 			$query = "SELECT py.`cust`,SUM(py.amount) FROM payment py WHERE py.status=0 AND py.chque_return=0 AND date(py.payment_date)<='$as_of' $store_payment_filter GROUP BY py.`cust`";
// 		}
// 		$result = mysqli_query($conn2, $query);
// 		while ($row = mysqli_fetch_array($result)) {
// 			$py_cust_id[] = $row[0];
...
// 		// --- start legacy code ---
// 		// This query aggregates the total cashback amounts for each customer. Cashback amounts can affect the final balance calculations by reducing the total invoice amounts.
// 		// $query = "SELECT bm.`cust`,SUM(bm.`invoice_-total`) FROM bill_main bm WHERE bm.`status` NOT IN ($exclude_status) AND bm.exclude=0 AND bm.`lock`=1 AND date(bm.billed_timestamp)<='$as_of' GROUP BY bm.`cust`";
// 		// if ($systemid == 24) {
// 		// 	$query = "SELECT bm.`cust`,SUM(bm.`invoice_-total`) FROM bill_main bm WHERE bm.`status` NOT IN ($exclude_status) AND bm.exclude=0 AND bm.`lock`=1 AND date(bm.billed_timestamp)<='$as_of' $salemanfilter GROUP BY bm.`cust`";
// 		// }
// 		// $result = mysqli_query($conn2, $query);
// 		// while ($row = mysqli_fetch_array($result)) {
// 		// 	$cashback_bm_cust[] = $row[0];
...
// 		foreach ($days as $day) {
// 			$bkdate = ${'backdate' . $day};
// 			$query = "SELECT bm.`cust`,SUM(bm.`invoice_+total`) FROM bill_main bm WHERE bm.`status` NOT IN ($exclude_status) AND bm.exclude=0 AND bm.`lock`=1 AND date(bm.billed_timestamp)<='$bkdate' GROUP BY bm.`cust`";
// 			if ($systemid == 24) {
// 				$query = "SELECT bm.`cust`,SUM(bm.`invoice_+total`) FROM bill_main bm WHERE bm.`status` NOT IN ($exclude_status) AND bm.exclude=0 AND bm.`lock`=1 AND date(bm.billed_timestamp)<='$bkdate' $salemanfilter GROUP BY bm.`cust`";
// 			}
// 			$result = mysqli_query($conn2, $query);
// 			while ($row = mysqli_fetch_array($result)) {
// 				${'bm_cust' . $day}[] = $row[0];
...
// 			// new code start
// 			// Add this new query for day-specific cashback
// 			$query = "SELECT bm.`cust`,SUM(bm.`invoice_-total`) FROM bill_main bm WHERE bm.`status` NOT IN ($exclude_status) AND bm.exclude=0 AND bm.`lock`=1 AND date(bm.billed_timestamp)<='$bkdate' GROUP BY bm.`cust`";
// 			if ($systemid == 24) {
// 				$query = "SELECT bm.`cust`,SUM(bm.`invoice_-total`) FROM bill_main bm WHERE bm.`status` NOT IN ($exclude_status) AND bm.exclude=0 AND bm.`lock`=1 AND date(bm.billed_timestamp)<='$bkdate' $salemanfilter GROUP BY bm.`cust`";
// 			}
// 			$result = mysqli_query($conn2, $query);
// 			while ($row = mysqli_fetch_array($result)) {
// 				${'cashback_bm_cust' . $day}[] = $row[0];
// 				${'cashback_bm_total' . $day}[] = $row[1];
// 			}
// 			// new code end
...
// 			if ($systemid == 24) {
// 				$query = "SELECT py.`cust`,SUM(py.amount) FROM payment py WHERE py.status=0 AND py.chque_return=0 AND date(py.payment_date) <= '$bkdate' $store_payment_filter GROUP BY py.`cust`";
// 			}
// 			$result = mysqli_query($conn2, $query);
// 			while ($row = mysqli_fetch_array($result)) {
...
// 			if ($systemid == 24) {
// 				$query = "SELECT py.`cust`,SUM(py.amount) FROM payment py WHERE py.status=0 AND py.chque_return=0 AND py.bill_pay=2 AND date(py.payment_date) > '$bkdate' AND date(py.payment_date)<='$as_of' $store_payment_filter GROUP BY py.`cust`";
// 			}
// 			$result = mysqli_query($conn2, $query);
// 			while ($row = mysqli_fetch_array($result)) {
...
// 			$query = "SELECT py.`cust`,SUM(py.amount)-SUM(bm.`invoice_+total` + bm.`invoice_-total`) FROM bill_main bm, payment py WHERE bm.invoice_no=py.invoice_no AND py.`status`=0 AND bm.`status` NOT IN ($exclude_status) AND py.chque_return=0 AND py.bill_pay=1 AND py.amount>(bm.`invoice_+total` + bm.`invoice_-total`) AND date(py.payment_date) > '$bkdate' AND date(py.payment_date)<='$as_of' GROUP BY py.`cust`";
// 			if ($systemid == 24) {
// 				$query = "SELECT py.`cust`,SUM(py.amount)-SUM(bm.`invoice_+total` + bm.`invoice_-total`) FROM bill_main bm, payment py WHERE bm.invoice_no=py.invoice_no AND py.`status`=0 AND bm.`status` NOT IN ($exclude_status) AND py.chque_return=0 AND py.bill_pay=1 AND py.amount>(bm.`invoice_+total` + bm.`invoice_-total`) AND date(py.payment_date) > '$bkdate' AND date(py.payment_date)<='$as_of' $store_payment_filter GROUP BY py.`cust`";
// 			}
// 			$result = mysqli_query($conn2, $query);
...
// 		// The loop iterates over each customer ID stored in the $cust_id array
// 		for ($i = 0; $i < sizeof($cust_id); $i++) {
// 			$cust_id0 = $cust_id[$i];

// 			// Find the corresponding cashback amount for the current customer
// 			$key = array_search($cust_id0, $cashback_bm_cust); // Finds the index of the current customer ID ($cust_id0) in the $cashback_bm_cust array.
// 			if ($key > -1)
// 				$return = $cashback_bm_total[$key];
// 			else
// 				$return = 0; // If the customer ID is found ($key > -1), $return is set to the corresponding cashback total from $cashback_bm_total.

// 			//upto 0 days//
// 			// Find the total payments up to the current date (0 days) for the current customer
// 			$key = array_search($cust_id0, $py_cust_id); // Finds the index of the current customer ID ($cust_id0) in the $py_cust_id array
// 			if ($key > -1)
// 				$pay0 = $py_total[$key];
// 			else
// 				$pay0 = 0; // If the customer ID is found ($key > -1), $pay0 is set to the corresponding payment total from $py_total

// 			// legacy code
// 			// $cust_cr_balance0[] = $bm_total[$i] - $pay0;
// 			// $credit_total0 += $bm_total[$i] - $pay0;

// 			// This is same above code, 🔝
// 			// Calculate the customer’s credit balance up to the current date (0 days)
...
// 				if ($key > -1)
// 					${'bill' . $day} = ${'bm_total' . $day}[$key];
// 				else
// 					${'bill' . $day} = 0;
// 				$key = array_search($cust_id0, ${'py_upto_cust' . $day});
// 				if ($key > -1)
// 					${'pay' . $day . 'upto'} = ${'py_upto_total' . $day}[$key];
// 				else
// 					${'pay' . $day . 'upto'} = 0;
// 				$key = array_search($cust_id0, ${'py_after_cust' . $day});
// 				if ($key > -1)
// 					${'pay' . $day . 'after'} = ${'py_after_total' . $day}[$key];
// 				else
// 					${'pay' . $day . 'after'} = 0;
// 				$key = array_search($cust_id0, ${'overpy_after_cust' . $day});
// 				if ($key > -1)
// 					${'overpay' . $day . 'after'} = ${'overpy_after_total' . $day}[$key];
// 				else
// 					${'overpay' . $day . 'after'} = 0;

// 				// --- new code start ---
// 				// Look up day-specific cashback
// 				$key = array_search($cust_id0, ${'cashback_bm_cust' . $day});
// 				if ($key > -1)
// 					${'return' . $day} = ${'cashback_bm_total' . $day}[$key];
// 				else
// 					${'return' . $day} = 0;

// 				// Use day-specific cashback in balance calculation
// 				${'balance' . $day} = ${'bill' . $day} + ${'return' . $day} - ${'pay' . $day . 'upto'} - ${'pay' . $day . 'after'} - ${'overpay' . $day . 'after'};
// 				// --- new code end ---
...
function getCreditData($sub_system, $specific_cust_id = null)
{
	...
	$date2 = date_create($datenow);
	if (!$date1 || !$date2) {
		die("Invalid date format for as_of or date now.");
	}
	$diff = date_diff($date1, $date2);
	$diff1 = $diff->format("%R");
	$diff2 = $diff->format("%a");
	...
	// Create a variable for the extra customer condition
	$py_cust_filter = "";
	if ($specific_cust_id !== null) {
		$py_cust_filter = " AND py.cust = " . (int) $specific_cust_id;
	}

	// Create a variable for the extra customer condition
	$bm_cust_filter = "";
	if ($specific_cust_id !== null) {
		$bm_cust_filter = " AND bm.cust = " . (int) $specific_cust_id;
	}
	...
	if ($out) {
		...
		if ($systemid == 24) {
			$query = "SELECT cu.id, cu.name, COALESCE(SUM(bm.`invoice_+total`), 0) + COALESCE(SUM(bm.`invoice_-total`), 0) AS total_invoice
					...
					LEFT JOIN bill_main bm ON bm.`cust` = cu.id AND bm.exclude = 0 AND bm.`status` NOT IN ($exclude_status) AND bm.`lock` = 1 AND date(bm.billed_timestamp) <= '$as_of' $salemanfilter
					LEFT JOIN ( SELECT py.`cust`, SUM(py.amount) AS amount FROM payment py WHERE py.status = 0 $store_payment_filter AND py.chque_return = 0 AND date(py.payment_date) <= '$as_of' GROUP BY py.`cust`) py ON py.cust = cu.id
					WHERE cu.`status` != 0 $sub_system_qry $storefilter $townfilter $groupfilter $bm_cust_filter
					GROUP BY cu.id, cu.name
					ORDER BY cu.name";
		} else {
			/*
			...
			*/
			$query = "SELECT cu.id,cu.name,SUM(bm.`invoice_+total`)+SUM(bm.`invoice_-total`)
			FROM bill_main bm, cust cu WHERE bm.`cust`=cu.id AND bm.exclude=0 AND bm.`status` NOT IN ($exclude_status) AND bm.`lock`=1 AND cu.`status`!=0 AND date(COALESCE(bm.due_date, bm.billed_timestamp))<='$as_of'
			$sub_system_qry $storefilter $townfilter $groupfilter $salemanfilter $bm_cust_filter GROUP BY bm.`cust` ORDER BY cu.name";
		}
		$result = mysqli_query($conn2, $query);
		while ($row = mysqli_fetch_array($result)) {
			$cust_id[] = $row[0];
		...
		//This query aggregates the total payment amounts for each customer, which is essential for calculating the customer's credit balance by offsetting their total invoices with their payments.
		// It excludes returned cheques and inactive payments and considers only payments up to a certain date
		$query = "SELECT py.`cust`,SUM(py.amount) FROM payment py WHERE py.status=0 AND py.chque_return=0 AND date(py.payment_date)<='$as_of' $py_cust_filter GROUP BY py.`cust`";
		if ($systemid == 24) {
			$query = "SELECT py.`cust`,SUM(py.amount) FROM payment py WHERE py.status=0 AND py.chque_return=0 AND date(py.payment_date)<='$as_of' $store_payment_filter $py_cust_filter GROUP BY py.`cust`";
		}
		$result = mysqli_query($conn2, $query);
		while ($row = mysqli_fetch_array($result)) {
			$py_cust_id[] = $row[0];
		...
		// original code
		// $query = "SELECT bm.`cust`,SUM(bm.`invoice_-total`) FROM bill_main bm WHERE bm.`status` NOT IN ($exclude_status) AND bm.exclude=0 AND bm.`lock`=1 AND date(bm.billed_timestamp)<='$as_of' GROUP BY bm.`cust`";
		$query = "SELECT bm.`cust`,SUM(bm.`invoice_-total`) FROM bill_main bm WHERE bm.`status` NOT IN ($exclude_status) AND bm.exclude=0 AND bm.`lock`=1 AND date(COALESCE(bm.due_date, bm.billed_timestamp))<='$as_of' $bm_cust_filter GROUP BY bm.`cust`";
		if ($systemid == 24) {
			$query = "SELECT bm.`cust`,SUM(bm.`invoice_-total`) FROM bill_main bm WHERE bm.`status` NOT IN ($exclude_status) AND bm.exclude=0 AND bm.`lock`=1 AND date(bm.billed_timestamp)<='$as_of' $bm_cust_filter $salemanfilter GROUP BY bm.`cust`";
		}
		$result = mysqli_query($conn2, $query);
		while ($row = mysqli_fetch_array($result)) {
			$cashback_bm_cust[] = $row[0];
		...
		foreach ($days as $day) {
			...
			// original code
			// $query = "SELECT bm.`cust`,SUM(bm.`invoice_+total`) FROM bill_main bm WHERE bm.`status` NOT IN ($exclude_status) AND bm.exclude=0 AND bm.`lock`=1 AND date(bm.billed_timestamp)<='$bkdate' GROUP BY bm.`cust`";
			$query = "SELECT bm.`cust`,SUM(bm.`invoice_+total`) FROM bill_main bm WHERE bm.`status` NOT IN ($exclude_status) AND bm.exclude=0 AND bm.`lock`=1 AND date(COALESCE(bm.due_date, bm.billed_timestamp))<='$bkdate' $bm_cust_filter GROUP BY bm.`cust`";
			if ($systemid == 24) {
				$query = "SELECT bm.`cust`,SUM(bm.`invoice_+total`) FROM bill_main bm WHERE bm.`status` NOT IN ($exclude_status) AND bm.exclude=0 AND bm.`lock`=1 AND date(bm.billed_timestamp)<='$bkdate' $salemanfilter $bm_cust_filter GROUP BY bm.`cust`";
			}
			$result = mysqli_query($conn2, $query);
			while ($row = mysqli_fetch_array($result)) {
				${'bm_cust' . $day}[] = $row[0];
			...
			$query = "SELECT py.`cust`,SUM(py.amount) FROM payment py WHERE py.status=0 AND py.chque_return=0 AND date(py.payment_date) <= '$bkdate' $py_cust_filter GROUP BY py.`cust`";
			if ($systemid == 24) {
				$query = "SELECT py.`cust`,SUM(py.amount) FROM payment py WHERE py.status=0 AND py.chque_return=0 AND date(py.payment_date) <= '$bkdate' $store_payment_filter $py_cust_filter GROUP BY py.`cust`";
			}
			$result = mysqli_query($conn2, $query);
			while ($row = mysqli_fetch_array($result)) {
				${'py_upto_cust' . $day}[] = $row[0];
			...
			$query = "SELECT py.`cust`,SUM(py.amount) FROM payment py WHERE py.status=0 AND py.chque_return=0 AND py.bill_pay=2 AND date(py.payment_date) > '$bkdate' AND date(py.payment_date)<='$as_of' $py_cust_filter GROUP BY py.`cust`";
			if ($systemid == 24) {
				$query = "SELECT py.`cust`,SUM(py.amount) FROM payment py WHERE py.status=0 AND py.chque_return=0 AND py.bill_pay=2 AND date(py.payment_date) > '$bkdate' AND date(py.payment_date)<='$as_of' $store_payment_filter $py_cust_filter GROUP BY py.`cust`";
			}
			$result = mysqli_query($conn2, $query);
			while ($row = mysqli_fetch_array($result)) {
				${'py_after_cust' . $day}[] = $row[0];
			...
			//---------------------Overpay for invoice after database days-----------------------------------//
			$query = "SELECT py.`cust`,SUM(py.amount)-SUM(bm.`invoice_+total` + bm.`invoice_-total`) FROM bill_main bm, payment py WHERE bm.invoice_no=py.invoice_no AND py.`status`=0 AND bm.`status` NOT IN ($exclude_status) AND py.chque_return=0 AND py.bill_pay=1 AND py.amount>(bm.`invoice_+total` + bm.`invoice_-total`) AND date(py.payment_date) > '$bkdate' AND date(py.payment_date)<='$as_of' $bm_cust_filter GROUP BY py.`cust`";
			if ($systemid == 24) {
				$query = "SELECT py.`cust`,SUM(py.amount)-SUM(bm.`invoice_+total` + bm.`invoice_-total`) FROM bill_main bm, payment py WHERE bm.invoice_no=py.invoice_no AND py.`status`=0 AND bm.`status` NOT IN ($exclude_status) AND py.chque_return=0 AND py.bill_pay=1 AND py.amount>(bm.`invoice_+total` + bm.`invoice_-total`) AND date(py.payment_date) > '$bkdate' AND date(py.payment_date)<='$as_of' $store_payment_filter $bm_cust_filter GROUP BY py.`cust`";
			}
			$result = mysqli_query($conn2, $query);
		...
		if (isset($_GET['save_cust_credit_balance'])) {
			$as_of_esc = mysqli_real_escape_string($conn, $as_of);
			// (Optional) clear out old rows for this as_of date
			mysqli_query($conn, "DELETE FROM cust_credit_balances WHERE as_of = '$as_of_esc'");

			for ($i = 0; $i < count($cust_id); $i++) {
				$cid = (int) $cust_id[$i];
				$upto_now = (float) $cust_cr_balance0[$i];

				// // 7+ days balance
				// $balance7plus = isset(${'cust_cr_balance7'}[$i])
				// 	? (float) ${'cust_cr_balance7'}[$i]
				// 	: 0.0;

				// // 14+ days balance
				// $balance14plus = isset(${'cust_cr_balance14'}[$i])
				// 	? (float) ${'cust_cr_balance14'}[$i]
				// 	: 0.0;

				// // 30+ days balance
				// $balance30plus = isset(${'cust_cr_balance30'}[$i])
				// 	? (float) ${'cust_cr_balance30'}[$i]
				// 	: 0.0;

				// // 45+ days balance
				// $balance45plus = isset(${'cust_cr_balance45'}[$i])
				// 	? (float) ${'cust_cr_balance45'}[$i]
				// 	: 0.0;

				// // 60+ days balance
				// $balance60plus = isset(${'cust_cr_balance60'}[$i])
				// 	? (float) ${'cust_cr_balance60'}[$i]
				// 	: 0.0;

				// // 90+ days balance
				// $balance90plus = isset(${'cust_cr_balance90'}[$i])
				// 	? (float) ${'cust_cr_balance90'}[$i]
				// 	: 0.0;

				// $sql = "INSERT INTO cust_credit_balances
				//         (cust_id, as_of, upto_now, balance7plus, balance14plus, balance30plus, balance45plus, balance60plus, balance90plus)
				//     VALUES
				//         ($cid, '$as_of_esc', $upto_now, $balance7plus, $balance14plus, $balance30plus, $balance45plus, $balance60plus, $balance90plus)
				//     ON DUPLICATE KEY UPDATE
				//         upto_now = VALUES(upto_now),
				//         balance7plus = VALUES(balance7plus),
				//         balance14plus = VALUES(balance14plus),
				//         balance30plus = VALUES(balance30plus),
				//         balance45plus = VALUES(balance45plus),
				//         balance60plus = VALUES(balance60plus),
				//         balance90plus = VALUES(balance90plus)
				// ";

				$sql = "INSERT INTO cust_credit_balances (cust_id, as_of, upto_now)
            VALUES ($cid, '$as_of_esc', $upto_now)
            ON DUPLICATE KEY UPDATE upto_now = VALUES(upto_now)";
				if (!mysqli_query($conn, $sql)) {
					error_log("Error saving credit balance for customer $cid: " . mysqli_error($conn));
				}
			}
		}
	}
	// --- NEW PART: Return array for specific customer ---
	if ($specific_cust_id !== null) {
		$index = array_search((int) $specific_cust_id, $cust_id);
		if ($index !== false) {
			$return_array = array(
				'customer_id' => $cust_id[$index],
				'customer_name' => $cust_name[$index],
				'current_balance_0' => $cust_cr_balance0[$index],
				'credit_total_0' => $credit_total0
			);
			foreach ($days as $day) {
				$return_array["balance_$day"] = isset(${'cust_cr_balance' . $day}[$index]) ? ${'cust_cr_balance' . $day}[$index] : 0;
			}
			return $return_array;
		} else {
			return array('error' => 'Customer data not found.');
		}
	}
}

function getCreditReportBasedOnItemCategory($sub_system)
{
	global $message, $decimal, $as_of, $cust_id, $cust_name, $cust_cr_balance0, $credit_total0, $days, $up_to_now, $category, $days, $customers, $results;
	$bm_cust_id = $bm_cust_name = $bm_total = $py_cust_id = $py_total = $days = $up_to_now = $category = $cust_name = $cust_id = array();
	$cashback_bm_cust = $cashback_bm_total = $bm_category = array();
	$days = [];
	$customers = [];
	$return = '';
	$datenow = dateNow();
	...
	$days = array_reverse($days); // Reverse to handle in desired order
}

function getCategoryName($id)
{
	include('config.php');
	$query = "SELECT `name` FROM item_category WHERE id=$id";
	$row = mysqli_fetch_row(mysqli_query($conn2, $query));
	return $row[0];
}
...
function updateQuotComment()
{
	...
	$query = "UPDATE quotation SET `comment` = '$comment' WHERE `id`='$qi_id'";
	$result = mysqli_query($conn, $query);
	if (!$result) {
...
// update by nirmal 09_07_2025 (added validation for cookies)
...
function newQuot($cust0)
{
	...
	if (!isset($_COOKIE['district'])) {
		$message = 'District not set or invalid!';
		$out = false;
	} else {
		$district = $_COOKIE['district'];
	}
	if (!isset($_COOKIE['user_id'])) {
		$message = 'Salesman not set or invalid!';
		$out = false;
	} else {
		$salesman = $_COOKIE['user_id'];
	}
	if (!isset($_COOKIE['store'])) {
		$message = 'Store not set or invalid!';
		$out = false;
	} else {
		$store = $_COOKIE['store'];
	}
	if (!isset($_COOKIE['sub_system'])) {
		$message = 'Sub system not set or invalid!';
		$out = false;
	} else {
	...
	if ($out) {
		...
		// $result = mysqli_query($conn, "SELECT price_inc_tax FROM stores st WHERE st.id='$mapped_inventory'");
		// if ($result) {
		// 	$row = mysqli_fetch_assoc($result);
		// 	if ($row && isset($row['price_inc_tax'])) {
		// 		$tax_display_format = $row['price_inc_tax'];
		// 	}
		// }

		// if ($tax_display_format != 0) {
		// 	$is_quotation_sent_with_tax = 1;
		// }
		...
		if ($cust != '') {
			...
			// $query = "INSERT INTO `quotation_main` (`id`,`billed_district`,`cust`,`created_by`,`quo_timestamp`,`type`,`v`,`store`,`mapped_inventory`,`sub_system`,`status`,`is_quotation_sent_with_tax`,`tax_display_format`)
			// VALUES ('$quot_no','$district','$cust','$salesman','$time_now','$qo_type','1','$store','$mapped_inventory','$sub_system','1',$is_quotation_sent_with_tax,'$tax_display_format')";

			$query = "INSERT INTO `quotation_main` (`id`,`billed_district`,`cust`,`created_by`,`quo_timestamp`,`type`,`v`,`store`,`mapped_inventory`,`sub_system`,`status`,`is_quotation_sent_with_tax`)
			VALUES ('$quot_no','$district','$cust','$salesman','$time_now','$qo_type','1','$store','$mapped_inventory','$sub_system','1',$is_quotation_sent_with_tax)";
			$result3 = mysqli_query($conn, $query);
			if (!$result3) {
				$out = false;
				$message = 'Error: Quotation could not be created - ' . mysqli_error($conn);
			}
		} else {
...
function addTaxForQuotationMain($quot_no)
{
	...
	$existingItemQuery = "SELECT `qty`, `unit_price` FROM `quotation` WHERE `quot_no`='$quot_no'";
	$existingItemResult = mysqli_query($conn, $existingItemQuery);

	if ($existingItemResult) {
		$total = 0;
		while ($row = mysqli_fetch_assoc($existingItemResult)) {
		...
		$query = "UPDATE quotation_main SET `tax` = '$tax_added_value' WHERE `id`='$quot_no'";
		$result = mysqli_query($conn, $query);
		if (!$result) {
			$out = false;
			$message = 'Error: Failed to update the quotation main tax value';
		}
	} else {
		$out = false;
		$message = 'Error: Failed to fetch the quotation items.';
	}
	return array('out' => $out, 'message' => $message);
}

// update by nirmal 21_11_2023
// update by nirmal 11_01_2024 (fixed error log bug, discount undefined), 20_02_2024 (tax calculation)
...
function apendQuot()
{
	...
	if ($out && isset($_REQUEST['discount'])) {
		$discount0 = (double) $_REQUEST['discount'];
	}
	...
	if ($out) {
		if ($discount === 'error') {
			$out = false;
			$message = 'Error: Discount exceeds max allowed';
		} else {
	...
	if ($out) {
		$result1 = mysqli_query($conn, "SELECT qm.created_by FROM quotation_main qm WHERE qm.id='$quot_no'");
		$row1 = mysqli_fetch_assoc($result1);
		$qm_created_by = $row1['created_by'];

		if (($approver) || ($user == $qm_created_by)) {
			...
			if (mysqli_num_rows($existingItemResult) > 0) {
				...
				$encoded_comment = htmlspecialchars($comment, ENT_QUOTES);
				$query = "INSERT INTO `quotation` (`quot_no`,`item`,`qty`,`unit_price`,`cost`,`discount`,`comment`) VALUES ('$quot_no','$itemid','$qty','$price','$cost','$discount','$encoded_comment')";
				$result = mysqli_query($conn, $query);
				if (!$result) {
					$out = false;
					$message = 'Error: Item could not be added to the quotation';
				}
			}
		} else {
			$out = false;
	...
	// tax calculation
	if ($out) {
		$returnValues = addTaxForQuotationMain($quot_no);
		$out = $returnValues['out'];
		if (!$out) {
...
// update by nirmal 09_10_2023, 20_02_2024 tax calculation
function removeQuot()
{
	global $message, $quot_no, $salesman, $cust;
	$quot_itm_id = $_GET['id'];
...
// update by nirmal 09_10_2023, 20_02_2024 tax calculations
function updateQuot()
{
	global $message, $quot_no, $salesman, $cust;
	$quot_itm_id = $_GET['id'];
...
// added by nirmal 24_10_2023, 20_02_2024 tax calculations
function updateQuotItemUnitPrice()
{
	global $message, $quot_no, $salesman, $cust;
	$quot_itm_id = $_GET['id'];
	...
	$discount_value = 0;
	$out = true;
	$message = 'Item unit price updated successfully';
	...
	include('config.php');
	$result = mysqli_query($conn, "SELECT qm.id,qm.created_by,qm.`status`,qm.cust,qi.item FROM quotation_main qm, quotation qi WHERE qm.id=qi.quot_no AND qi.id='$quot_itm_id'");
	$row = mysqli_fetch_assoc($result);
	$quot_no = $row['id'];
	$qm_created_by = $row['created_by'];
	$cust = $row['cust'];
	...
	if (($unit_price < 0) || ($unit_price == 0)) {
		$out = false;
		$message = 'Item unit price cannot be negetive or zero';
	}
	...
	if ($out) {
		...
		$query = "SELECT min_w_rate,max_w_rate,max_r_rate,pr_sr,default_price FROM inventory_items WHERE `id`='$itemid'";
		$row = mysqli_fetch_row(mysqli_query($conn, $query));
		$min_w_rate = $row[0];
		$max_w_rate = $row[1];
		$max_r_rate = $row[2];
		...
		$query = "SELECT w_price,r_price FROM inventory_qty WHERE `item`='$itemid' AND `location` = '$store'";
		$row = mysqli_fetch_row(mysqli_query($conn, $query));
		$w_price = $row[0];
		$r_price = $row[1];
		...
		if ($pr_sr == 1) {
			if ($cust_type == 1) {
				$discount_value = $w_price - $unit_price;
				if ((($w_price * $max_w_rate) / 100) >= $discount_value) {
					if ((($w_price * $min_w_rate) / 100) <= $discount_value) {
						$discount = $discount_value;
					} else {
						$out = false;
						$message = 'Error: Minimum discount rate exceeded';
					}
				} else {
					$out = false;
					$message = 'Error: Maximum discount rate exceeded';
				}
			}
			if ($cust_type == 2) {
				$discount_value = $w_price - $unit_price;
				if ((($r_price * $max_r_rate) / 100) >= $discount_value) {
					$discount = $discount_value;
				} else {
					$out = false;
					$message = 'Error: Maximum discount rate exceeded';
			...
			$discount = $default_price - $unit_price;
		}
		if ($out) {
			if (($approver) && ($qm_status == 2)) {
				// Update pending state qty (manager, top manager)
				if ($cust_type == 1) {
					if ($unit_price <= $w_price) {
						$query = "UPDATE `quotation` SET `unit_price`='$unit_price', `discount`='$discount' WHERE `id`='$quot_itm_id'";
						$result = mysqli_query($conn, $query);
						if (!$result) {
							$out = false;
							$message = 'Error: Item unit price could not be updated';
						}
					} else {
						$out = false;
						$message = 'Error: Item maximum unit price exceeded';
					}
				}
				if ($cust_type == 2) {
					if ($unit_price <= $r_price) {
						$query = "UPDATE `quotation` SET `unit_price`='$unit_price', `discount`='$discount' WHERE `id`='$quot_itm_id'";
						$result = mysqli_query($conn, $query);
						if (!$result) {
							$out = false;
...
// update by nirmal 2024_06_24 when discount update tax calculate according to discount
function updateQuotDiscount()
{
	global $message, $quot_no, $salesman, $cust;
	$quot_no = $_GET['id'];
	...
	$out = true;
	$message = 'Quotation discount price updated successfully';

	if (($discount < 0) || ($discount == 0)) {
		$out = false;
		$message = 'Quotation discount price cannot be negetive or zero';
	}

	include('config.php');
	$result = mysqli_query($conn, "SELECT qm.created_by,qm.cust,qm.`status` FROM quotation_main qm, quotation qi WHERE qm.id=qi.quot_no AND qm.id='$quot_no'");
	$row = mysqli_fetch_assoc($result);
	$qm_created_by = $row['created_by'];
	$cust = $row['cust'];
	$qm_status = $row['status'];
	...
	if ($out) {
		if (($approver) && ($qm_status == 2)) {
			// Update pending state qty (manager, top manager)
			$query = "UPDATE `quotation_main` SET `discount`='$discount' WHERE `id`='$quot_no'";
			$result = mysqli_query($conn, $query);
			if (!$result) {
				$out = false;
				$message = 'Error: Quotation discount price could not be updated';
			}
		} else {
			if (($salesman == $qm_created_by) && ($qm_status == 1)) {
				$query = "UPDATE `quotation_main` SET `discount`='$discount' WHERE `id`='$quot_no'";
		...
		if ($out) {
			...
			if ($existingItemResult) {
				...
				if ($total > 0) {
					...
					$tax_added_value = $sub_total * $tax_rate / (100 + $tax_rate);
...
function removeQuotDiscount()
{
	...
	$out = true;
	$message = 'The quotation discount price removed successfully';

	include('config.php');
	$result = mysqli_query($conn, "SELECT qm.created_by,qm.cust,qm.`status`,qm.discount FROM quotation_main qm, quotation qi WHERE qm.id=qi.quot_no AND qm.id='$quot_no'");
	$row = mysqli_fetch_assoc($result);
	$qm_created_by = $row['created_by'];
	$cust = $row['cust'];
	$qm_status = $row['status'];
	...
	if ($out) {
		if (($approver) && ($qm_status == 2)) {
			// Update pending state qty (manager, top manager)
			$query = "UPDATE `quotation_main` SET `discount`='' WHERE `id`='$quot_no'";
			$result = mysqli_query($conn, $query);
			if (!$result) {
				$out = false;
				$message = 'Error: The quotation discount price could not be removed.';
			}
		} else {
			if (($salesman == $qm_created_by) && ($qm_status == 1)) {
				$query = "UPDATE `quotation_main` SET `discount`='' WHERE `id`='$quot_no'";
...
function getQOCustomerAddress()
{
	...
	$query = "SELECT qm.cust_address FROM quotation_main qm WHERE qm.id='$qo_number'";
	$row = mysqli_fetch_row(mysqli_query($conn, $query));
	$cust_address = $row[0];

	if (empty($cust_address)) {
		$query = "SELECT cu.shop_address FROM cust cu, quotation_main qm WHERE qm.cust = cu.`id` AND qm.id='$qo_number'";
		$row = mysqli_fetch_row(mysqli_query($conn, $query));
		$cust_address = $row[0];
		if (!$cust_address) {
			$cust_address = 'Address not found';
		}
	}
	return $cust_address;
}
...
// added by nirmal 17_02_2025
function setQuotCustomerAddress()
{
	include('config.php');
	...
	// Retrieve the address and quotation id from the decoded data
	$address = $data['address'];
	$quotation_id = $data['quotation_id'];

	// Prepare the UPDATE statement using mysqli
	$stmt = $conn->prepare("UPDATE quotation_main SET cust_address = ? WHERE id = ?");
	if (!$stmt) {
		echo json_encode(['status' => 'error', 'message' => 'Prepare failed: ' . $conn->error]);
		return;
	}

	// Bind parameters. "s" denotes string for address and "i" denotes integer for quotation_id.
	$stmt->bind_param("si", $address, $quotation_id);
...
function setQuotTerms()
{
	...
	$user_id = $_COOKIE['user_id'];
	$encoded_qo_note = htmlspecialchars($qo_note, ENT_QUOTES);
	...
	if ($approver || $quo_status == 1) {
		// $query1="UPDATE `quotation_main` SET `heading`='$qo_heading',`att`='$qo_attention',`warranty`='$qo_warranty',`validity`='$qo_validity',`leadtime`='$qo_leadtime',`terms1`='$qo_terms1',`terms2`='$qo_terms2',`note`='$encoded_qo_note',`address`='$qo_address'  WHERE id='$quot_no'";
		$query1 = "UPDATE `quotation_main` SET `heading`='$qo_heading',`att`='$qo_attention',`warranty`='$qo_warranty',`validity`='$qo_validity',`leadtime`='$qo_leadtime',`terms1`='$qo_terms1',`terms2`='$qo_terms2',`note`='$encoded_qo_note',`address`='$qo_address', `created_by`='$user_id'  WHERE id='$quot_no'";
...
function setQuotStatus($new_status)
{
	...
	if (isset($_COOKIE['manager']) || isset($_COOKIE['top_manager']) || isset($_COOKIE['report']))
		...
		$approver = false;
	if ($salesman == $qm_created_by)
		$qo_owner = true;
	else
		$qo_owner = false;

	/* 1. Auto Approve 			*/
	if ($qm_status == 1 && $new_status == 2 && $approver) {
		$query1 = "UPDATE `quotation_main` SET `status`='3',`quo_timestamp`='$datetime',`approved_by`='$salesman',`approve_timestamp`='$datetime' WHERE id='$quot_no'";
	} else
		/* 2. Submit for Approval 	*/
		if ($qm_status == 1 && $new_status == 2 && $qo_owner)
			$query1 = "UPDATE `quotation_main` SET `status`='2',`quo_timestamp`='$datetime' WHERE id='$quot_no'";
		else
			/* 2. Edited item Approval 	*/
			if ($qm_status == 2 && $new_status == 2 && $approver)
				$query1 = "UPDATE `quotation_main` SET `status`='2',`quo_timestamp`='$datetime' WHERE id='$quot_no'";
			else
				/* 3. Approve the Quot	 	*/
				if ($qm_status == 2 && $new_status == 3 && $approver)
					$query1 = "UPDATE `quotation_main` SET `status`='3',`approved_by`='$salesman',`approve_timestamp`='$datetime' WHERE id='$quot_no'";
				else
					/* 4. Reject the Quot	 	*/
					if ($qm_status == 2 && $new_status == 4 && $approver)
						$query1 = "UPDATE `quotation_main` SET `status`='4',`approved_by`='$salesman',`approve_timestamp`='$datetime' WHERE id='$quot_no'";
					else
						/* 5. Delete the Quot	 	*/
						if (($qm_status == 1 || $qm_status == 2 || $qm_status == 3) && $new_status == 0 && $qo_owner)
							$query1 = "UPDATE `quotation_main` SET `status`='0',`deleted_by`='$salesman',`deleted_timestamp`='$datetime' WHERE id='$quot_no'";
						else
							/* 6. Send to Cust	 		*/
							if ($qm_status == 3 && $new_status == 5)
								$query1 = "UPDATE `quotation_main` SET `status`='5',`submited_by`='$salesman',`submited_timestamp`='$datetime' WHERE id='$quot_no'";
							else
								/* 7. Customer Accepted		*/
								if ($qm_status == 5 && $new_status == 6)
									$query1 = "UPDATE `quotation_main` SET `status`='6',`custacc_by`='$salesman',`custacc_timestamp`='" . $_GET['accdate'] . "',`cust_po`='" . $_GET['custpo'] . "' WHERE id='$quot_no'";
								else
									/* 8. Customer Accepted		*/
									if ($qm_status == 5 && $new_status == 7)
										$query1 = "UPDATE `quotation_main` SET `status`='7',`custacc_by`='$salesman',`custacc_timestamp`='$datenow',`rejected_com`='" . $_GET['custreject'] . "' WHERE id='$quot_no'";
									else
										/* 9. Order Completed		*/
										if ($qm_status == 6 && $new_status == 8)
											$query1 = "UPDATE `quotation_main` SET `status`='8',`completed_by`='$salesman',`completed_timestamp`='$datetime' WHERE id='$quot_no'";
										else {
											$out = false;
	...
	if ($out) {
		...
		if (!$result) {
			$out = false;
			$message = 'Error: Payment update was failed';
		}
	}
	return $out;
}
...
// {
// 	global $authorize, $approver, $qm_status, $status, $color, $bm_no, $qm_cust, $qm_created_by, $conn2;
// 	$quot_no = $_GET['id'];
// 	$salesman = $_COOKIE['user_id'];
// 	$datenow = dateNow();
// 	$bm_no = array();
...
// 	if (($salesman == $qm_created_by) && ($datenow == $qm_created_date))
// 		$authorize = true;
// 	else
// 		$authorize = false;
// 	if (isset($_COOKIE['manager']) || isset($_COOKIE['top_manager']) || isset($_COOKIE['report']))
...
function qoPermission()
{
	...
	$result = mysqli_query($conn2, "SELECT qm.created_by, DATE(qm.quo_timestamp) AS qdate, qm.cust, qm.status 
	...
	$query1 = "SELECT bm.invoice_no 
		FROM bill_main bm, bill bi 
		WHERE bm.invoice_no=bi.invoice_no 
		AND bm.status NOT IN (0,7) 
		AND bm.quotation_no='$quot_no' 
...
function QuotDiscountEstimate()
{
	global $total1, $qi_item_code1, $qi_qty1, $qi_uprice1, $qi_discount1, $sub_total1;
	$quot_no = $_REQUEST['id'];
	include('config.php');

	$query2 = "SELECT `tax` FROM quotation_main WHERE `id`='$quot_no'";
	$row2 = mysqli_fetch_row(mysqli_query($conn2, $query2));
	$tax_added_value1 = $row2[0];

	$query = "SELECT itm.code,qi.qty,qi.unit_price,qi.discount FROM quotation qi, inventory_items itm WHERE qi.item=itm.id AND qi.quot_no='$quot_no' AND qi.discount > 0";
	$result = mysqli_query($conn2, $query);
	...
	$sub_total1 = $total1 - $tax_added_value1;
}

// update by nirmal 03_11_2023
// update by nirmal 11_01_2024 (added get to show temp billed qo bill numbers), bug fixed for same at 19_01_2024
// update by nirmal 19_02_2024 (get tax value and subtotal with tax)
// updated ny nirmal 17_02_2025 (get customer address either from quotation main or cust table)
// update by nirmal 24_11_2025 (added qo main comment)

function generateQuot($case)
{
	global $quot_no, $qo_date, $qo_time, $qo_salesman, $qo_cust_id, $qo_cust_name, $qo_cust_nic, $qo_cust_mob, $qo_cust_address, $cu_details,
	$qo_st_name, $qo_st_add, $qo_st_tel, $qo_att, $qo_warranty, $qo_validity, $qo_leadtime, $qo_terms, $qo_note, $qo_image,
	$qo_image_hei, $qi_item_des, $qi_item_code, $qi_qty, $qi_uprice, $qi_discount, $qi_comment, $qi_filled_qty, $total,
	$systemid, $web, $email, $qo_heading, $qo_v, $qi_invoiced_qty, $qi_tmp_invoiced_qty, $qo_discount, $qi_item_unit_type,
	$qi_tmp_invoice_no, $tax_added_value, $sub_total, $trn_no, $tax, $qo_price_inc_tax, $qo_comment, $conn2;
	$quot_no = $_REQUEST['id'];
	$total = 0;
	$qi_item_des = $qi_item_code = $qi_qty = $qi_uprice = $qi_discount = $qi_item_id = $qi_invoiced_qty = $qi_item_unit_type = $qi_tmp_invoice_no = array();
	...
	$store = $_COOKIE['store'];
	$result = mysqli_query($conn2, "SELECT `email` FROM stores WHERE `id`='$store'");
	$row = mysqli_fetch_assoc($result);
	$email = $row['email'];
	...
	$result = mysqli_query($conn2, "SELECT value FROM settings WHERE setting='trn_no'");
	$row = mysqli_fetch_assoc($result);
	$trn_no = $row['value'];

	$result = mysqli_query($conn2, "SELECT cust_address FROM quotation_main WHERE id='$quot_no'");
	$row = mysqli_fetch_assoc($result);
	...
	// $query = "SELECT date(qm.`quo_timestamp`), time(qm.`quo_timestamp`), $user_name_column, cu.`id`, cu.`name`, cu.nic, cu.mobile,
	// cu.shop_address, st.shop_name, st.address, st.tel, qm.att, qm.warranty, qm.validity, qm.leadtime, qm.terms1,
	// qm.terms2, qm.note, qm.image, qm.image_height, qm.address, qm.heading, qm.v, qm.discount, st.`price_inc_tax`
	// FROM quotation_main qm, cust cu, stores st, userprofile up
	// WHERE qm.`cust`=cu.id AND qm.created_by=up.id AND qm.mapped_inventory=st.id AND qm.id='$quot_no'";

	$query = "SELECT date(qm.`quo_timestamp`), time(qm.`quo_timestamp`), $user_name_column, cu.`id`, cu.`name`, cu.nic, cu.mobile,
	cu.shop_address, st.shop_name, st.address, st.tel, qm.att, qm.warranty, qm.validity, qm.leadtime, qm.terms1,
	qm.terms2, qm.note, qm.image, qm.image_height, qm.address, qm.heading, qm.v, qm.discount, qm.comment
	FROM quotation_main qm, cust cu, stores st, userprofile up
	WHERE qm.`cust`=cu.id AND qm.created_by=up.id AND qm.mapped_inventory=st.id AND qm.id='$quot_no'";
	...
	if (empty($cust_address)) {
		...
		$qo_cust_address = $cust_address; // Ensure we set $qo_cust_address if $cust_address is not empty
	}
	$qo_st_name = $row[8];
	$qo_st_add = $row[9];
	$qo_st_tel = $row[10];
	...
	$query = "SELECT itm.description,itm.code,qi.qty,qi.unit_price,qi.discount,qi.`comment`,qi.item,itm.`unit`
	FROM quotation qi, inventory_items itm WHERE qi.item=itm.id AND qi.quot_no='$quot_no'";
	$result = mysqli_query($conn2, $query);
	...
	while ($row = mysqli_fetch_array($result)) {
		...
		if ($row[7] != '') {
			$query1 = "SELECT `type` FROM unit_types WHERE `id`=$row[7]";
			$row1 = mysqli_fetch_row(mysqli_query($conn2, $query1));
			$qi_item_unit_type[] = $row1[0];
		} else {
			$qi_item_unit_type[] = '';
		}
		...
		// temp billed quotation bills
		$query4 = "SELECT bm.bm_no FROM bill_main_tmp bm, bill_tmp bi WHERE bm.bm_no=bi.bm_no AND bm.`status` NOT IN (0,6,7) AND bm.quotation_no='$quot_no' AND bi.item='$qi_item' AND bi.`qty` > 0";
		$result4 = mysqli_query($conn2, $query4);
		while ($row4 = mysqli_fetch_array($result4)) {
			$qi_tmp_invoice_no[] = $row4[0];
		}
	}
...
function checkItemAvailability($qo_number)
{
	...
	include('config.php');
	$query = "SELECT qo.qty,SUM(itq.qty),SUM(itn.qty)
	FROM inventory_qty itq, quotation qo
	LEFT JOIN inventory_new itn ON qo.item=itn.item
	WHERE qo.item=itq.item AND qo.quot_no='$qo_number' GROUP BY qo.item, qo.qty";
	$result = mysqli_query($conn2, $query);
	while ($row = mysqli_fetch_array($result)) {
		if ($row[0] > ($row[1] + $row[2]))
			$qty_avalability = false;
	}
...
function getQuotList($sub_system){
	global $from_date,$to_date,$cust,$item0,$store,$salesman,$status,$qm_id,$qm_created_by,$qm_created_date,$qm_validity,$qm_store,$qm_cust,$qm_heading,$qm_amount,$qm_status,
	$qm_status_name,$qm_status_color,$qm_qty_avalability,$qm_rejected_com,$qm_custname,$qm_mobile,$qm_tel,$qm_inv_check;
	...
	if($cust!=''){
		$qry_cust="AND cu.name='$cust'";
	}

	if($item!=''){
		$qry_item="AND qi.item='$item'";
	}
	...
	if($sub_system!='all'){
		$qry_subsys="AND qm.`sub_system`='$sub_system'";
	}
	...
	if($compnents == 'to'){
		...
		$salesman=$_GET['sm'];
		if($salesman!=''){
			$qry_salesman="AND qm.created_by='$salesman'";
		}
	}

	if($status!='all'){
		$qry_status="AND qm.`status`='$status'";
	}
	...
	$query="SELECT qm.id,up.username,qm.quo_timestamp,qm.validity,st.name,cu.name,qm.heading,SUM(qi.qty * qi.unit_price),qm.`status`,qm.rejected_com,cu.cust_name,cu.mobile,cu.shop_tel,
	qm.discount
	FROM quotation_main qm, quotation qi, cust cu, stores st, userprofile up
	WHERE qm.id=qi.quot_no AND qm.`cust`=cu.id AND qm.created_by=up.id AND qm.store=st.id AND qm.`status`!=0 AND date(qm.quo_timestamp) BETWEEN '$from_date' AND '$to_date'
	$qry_subsys $qry_cust $qry_item $qry_store $qry_salesman $qry_status
	GROUP BY qm.id";
	$result=mysqli_query($conn2,$query);
	while($row=mysqli_fetch_array($result)){
		$qm_id[]=$row[0];
		...
		$qm_cust[]=$row[5];
		$qm_heading[]=$row[6];
		$qm_amount[]=$row[7]-$row[13];
		$qm_status[]=$row[8];
		$qm_rejected_com[]=$row[9];
		$qm_status_name[]=qoStatus('name',$row[8]);
		$qm_status_color[]=qoStatus('color',$row[8]);
		if($row[8]==6){
			$qm_qty_avalability[]=checkItemAvailability($row[0]);
		} else{
			$qm_qty_avalability[]=false;
		}
		$qm_custname[]=$row[10];
		$qm_mobile[]=$row[11];
		$qm_tel[]=$row[12];
...
function getQuotList($sub_system)
{
	global $from_date, $to_date, $cust, $item0, $store1, $salesman, $status, $qm_id, $qm_created_by, $qm_created_date, $qm_validity, $qm_store, $qm_cust, $qm_heading, $qm_amount, $qm_status,
	$qm_status_name, $qm_status_color, $qm_qty_avalability, $qm_rejected_com, $qm_custname, $qm_mobile, $qm_tel, $qm_inv_check, $conn2;
	...
	if ($compnents == 'to') {
		$store1 = $_COOKIE['store'];
		$qry_store = "AND st.id='$store1'";
	} else {
		$store1 = $_GET['st'];
		if ($store1 != '') {
			$qry_store = "AND st.id='$store1'";
	...
	if (($store1 == '') && ($_GET['components'] != 'topmanager')) {
		$store1_list = groupStores();
		$qry_store = "AND st.id IN (" . $store1_list . ")";
	}
...
function qoCompleteCheck()
{
	...
	if ($total_qo_item != 0)
		$completed = round(($total_bi_item / $total_qo_item) * 100);
...
// update by nirmal 2024_06_26 (show hide tax quotation template according to quotation sent as taxed or not)
// updated by nirmal 24_11_2025 (added qo comment variable to show update it later)
// function qoDetails()
// {
// 	global $main_district, $main_store, $main_refinv, $main_type, $main_sub_system, $main_created_date, $main_created_by,
// 	$main_approved_date, $main_approved_by, $main_submited_date, $main_submited_by, $main_deleted_date, $main_deleted_by,
// 	$main_custacc_date, $main_custacc_by, $main_reject_comm, $main_completed_date, $main_completed_by, $main_cust_po,
// 	$main_image, $main_image_hei, $qitem_list, $qqty_list, $is_quotation_sent_with_tax, $tax_display_format, $main_comment,$conn2;
// 	include('config.php');
// 	$quot_no = $_GET['id'] ?? '';
...
// 	$query = "SELECT di.`name`, st1.`name`, st2.`name`, qm.`type`, ss.name, qm.quo_timestamp, qm.`created_by`, qm.approve_timestamp, qm.approved_by,
// 	 qm.submited_timestamp, qm.submited_by, qm.deleted_timestamp, qm.deleted_by, qm.custacc_timestamp, qm.custacc_by,
// 	 qm.rejected_com, qm.completed_timestamp, qm.completed_by, qm.cust_po, qm.`image`, qm.`image_height`, qm.`is_quotation_sent_with_tax`, qm.`comment`
// 	FROM quotation_main qm, cust cu, stores st1, stores st2, district di, sub_system ss
// 	WHERE qm.`cust`=cu.id AND qm.mapped_inventory=st2.id AND qm.store=st1.id AND qm.billed_district=di.id AND qm.`sub_system`=ss.id AND qm.id='$quot_no'";
// 	$row = mysqli_fetch_row(mysqli_query($conn2, $query));
// 	$main_district = $row[0];
// 	$main_store = $row[1];
// 	$main_refinv = $row[2];
...
// 	$is_quotation_sent_with_tax = $row[21];
// 	$main_comment = $row[22];
// 	// $tax_display_format = $row[22];
...
function qoDetails()
{
	...
	$query = "SELECT di.`name`, st1.`name`, st2.`name`, qm.`type`, ss.name, qm.quo_timestamp, qm.`created_by`, qm.approve_timestamp, qm.approved_by,
		...
		qm.rejected_com, qm.completed_timestamp, qm.completed_by, qm.cust_po, qm.`image`, qm.`image_height`, qm.`is_quotation_sent_with_tax`, qm.`comment`
		FROM quotation_main qm
		LEFT JOIN cust cu ON qm.`cust` = cu.id
		LEFT JOIN stores st1 ON qm.store = st1.id
		LEFT JOIN stores st2 ON qm.mapped_inventory = st2.id
		LEFT JOIN district di ON qm.billed_district = di.id
		LEFT JOIN sub_system ss ON qm.`sub_system` = ss.id
		WHERE qm.id = '$quot_no'";
	...
	$main_store = $row[1] ?? '';
	$main_refinv = $row[2] ?? '';
	$main_type = isset($row[3]) ? billType($row[3]) : '';
	$main_sub_system = $row[4] ?? '';
	$main_created_date = $row[5] ?? '';
	$main_created_by = $usercheck[$row[6] ?? ''] ?? '-';
	$main_approved_date = $row[7] ?? '';
...
// added by nirmal 24_11_2025 (to update qo main comment)
function updateQuotMainComment()
{
	include('config.php');
	global $message, $id;
	$out = true;
	if (isset($_POST['qo_main_no'])) {
		$id = $_POST['qo_main_no'];
	} else {
		$out = false;
		$message = "Error: Quotation main no is missing.";
	}

	if (isset($_POST['qo_main_comment'])) {
		$comment = $_POST['qo_main_comment'];
	} else {
		$out = false;
		$message = "Error: Quotation main comment is missing.";
	}

	if ($out) {
		$query = "UPDATE `quotation_main` SET `comment`='$comment' WHERE id='$id'";
		$result = mysqli_query($conn, $query);
		if (!$result) {
			$message = "Error: Quotation main comment update error";
		} else {
			$message = "Quotation main comment updated succesfully";
		}
	}
...
// updated by nirmal 19_12_2024 (added sub system filter)
function getOnGoing($sub_system)
{
	...
	include('config.php');
	$qry_subsys = $user_qry = '';
	$user_qry = " AND qm.created_by='$user_id'";
	...
	WHERE qm.id=qi.quot_no AND qm.`cust`=cu.id AND qm.store=st.id AND qm.`status`=1 $user_qry $qry_subsys
	GROUP BY qm.id";
...
function imageresize2($systemid, $image1)
{
	$out = false;
...
// added by nirmal 03_06_2024 (Edit old function into new one after with potential bug fixing)
function qoAddImage()
{
	global $message;
	$id = $_GET['id'];
		...
		if (isset($_FILES["fileToUpload1"]) && $_FILES["fileToUpload1"]["name"] != '') {
			...
			if ($uploadOk == 0) {
				...
				if (move_uploaded_file($_FILES["fileToUpload1"]["tmp_name"], $destination_file)) {
					if (imageresize2($systemid, $tmp_name)) {
						$image1 = 1;
...
// update by nirmal 2024_05_14 change create by when revise qo
function qoRevise()
{
	global $message, $quot_no;
	$quot_no = $_GET['id'];
	...
	if ($quo_status == 2 || $quo_status == 3 || $quo_status == 5) {
		$query1 = "UPDATE `quotation_main` SET `v`='$quo_v',`status`='1', `created_by`='$user_id' WHERE id='$quot_no'";
		$result1 = mysqli_query($conn, $query1);
	} else {
		$msg = "Error: Unauthorize Request";
	}
...
function qoAddNote()
{
	...
	$quot_no = $_GET['id'];
	$user_id = $_COOKIE['user_id'];
	$textnote = htmlspecialchars($_POST['textnote'], ENT_QUOTES);
...
function qoNote()
{
	global $qn_noid, $qn_timestamp, $qn_user, $qn_note, $edit_note, $edit_note0, $conn2;
	$quot_no = $_GET['id'] ?? '';
	$user_id = $_COOKIE['user_id'];
	$date_now = dateNow();
	$qn_noid = array();
	...
	while ($row = mysqli_fetch_array($result)) {
		if ($row[0] != $comid) {
			...
			$qn_note[] = $row[3];
			if ((substr($row[1], 0, 10) == $date_now) && ($row[4] == $user_id)) {
				$edit_note[] = true;
			} else {
				$edit_note[] = false;
			}
...
function qoUpdateSentWithTax()
{
	...
	$query1 = "UPDATE `quotation_main` SET `is_quotation_sent_with_tax`='$value' WHERE id='$quot_no'";
	$result1 = mysqli_query($conn, $query1);
	if ($result1) {
		if ($value) {
			$message = "Quotation was updated successfully! Marked as sent with tax";
		} else {
...
// added by nirmal 27_08_2025
function qoUpdateTaxFormat()
{
	global $quot_no, $message;
	$quot_no = isset($_GET['id']) ? $_GET['id'] : 0;
	...
	$query1 = "UPDATE `quotation_main` SET `tax_display_format`='$value' WHERE id='$quot_no'";
	$result1 = mysqli_query($conn, $query1);
	if ($result1) {
		if ($value) {
			$message = "Quotation was updated successfully! Marked as sent with tax format changed";
		} else {
			$message = "Quotation was updated successfully!";
		}
	} else {
...
{
	global $from_date, $to_date, $cust, $qo_no0, $store, $user, $status, $qn_qo_id, $qn_time, $qn_user, $qn_note, $qn_store, $qn_cust, $qn_heading, $qn_status_name,
	$qn_status_color, $qm_custname, $qm_mobile, $qm_tel, $conn2;
	...
	$query = "SELECT qm.id,qn.`timestamp`,up.username,qn.note,st.name,cu.name,qm.heading,qm.`status`,cu.cust_name,cu.mobile,cu.shop_tel
	FROM quotation_main qm, quotation_note qn, cust cu, stores st, userprofile up
	WHERE qm.id=qn.qo_no AND qm.`cust`=cu.id AND qn.`user`=up.id AND qm.store=st.id AND qm.`status`!=0 AND date(qn.timestamp) BETWEEN '$from_date' AND '$to_date'
	$qry_subsys $qry_cust $qry_qono $qry_store $qry_user $qry_status ORDER BY qn.`timestamp`";
	$result = mysqli_query($conn2, $query);
