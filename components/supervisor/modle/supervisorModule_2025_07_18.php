...
function dailySale($store)
{
	...
	if ($type_req != 3 && $type_req != 5) {
		if ($_GET['components'] == 'marketing') {
			$query = "SELECT bm.`invoice_no`,(bm.`invoice_+total` + bm.`invoice_-total`),di.name,up.username,cu.name,date(bm.billed_timestamp),time(bm.billed_timestamp),st.name,bm.tax
			...
			$query = "SELECT bm.`invoice_no`,SUM(bi.qty*bi.unit_price),di.name,up.username,cu.name,date(bm.billed_timestamp),time(bm.billed_timestamp),st.name,SUM(bi.discount),bm.tax
	...
	if ($out) {
		...
		if (isset($_GET['save_cust_credit_balance'])) {
			...
			for ($i = 0; $i < count($cust_id); $i++) {
				$cid = (int) $cust_id[$i];
				$balance = (float) $cust_cr_balance0[$i];

				$sql = "INSERT INTO cust_credit_balances (cust_id, as_of, balance) VALUES ($cid, '$as_of_esc', $balance) ON DUPLICATE KEY UPDATE balance = VALUES(balance)";
