...
function getCreditData($sub_system){
	...
	if($out){
		...
		if($systemid == 24){
			...
			$query="SELECT cu.id,cu.name,SUM(bm.`invoice_+total`)+SUM(bm.`invoice_-total`) FROM bill_main bm, cust cu WHERE bm.`cust`=cu.id AND bm.exclude=0 AND bm.`status` NOT IN ($exclude_status) AND bm.`lock`=1 AND cu.`status`!=0 AND date(bm.billed_timestamp)<='$as_of' $sub_system_qry $storefilter $townfilter $groupfilter $salemanfilter GROUP BY bm.`cust` ORDER BY cu.name";
		}
		$result=mysqli_query($conn2,$query);
		while($row=mysqli_fetch_array($result)){
			$cust_id[]=$row[0];
...
function generateQuot($case){
	...
	$query="SELECT date(qm.quo_timestamp),time(qm.quo_timestamp),$user_name_column,cu.id,cu.name,cu.nic,cu.mobile,cu.shop_address,st.shop_name,st.address,st.tel,qm.att,qm.warranty,qm.validity,qm.leadtime,qm.terms1,qm.terms2,qm.note,qm.image,qm.image_height,qm.address,qm.heading,qm.v,qm.discount FROM quotation_main qm, cust cu, stores st, userprofile up WHERE qm.`cust`=cu.id AND qm.created_by=up.id AND qm.mapped_inventory=st.id AND qm.id='$quot_no'";
