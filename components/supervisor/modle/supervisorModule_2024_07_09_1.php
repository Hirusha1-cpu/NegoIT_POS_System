...
function getCreditData($sub_system){
	...
	if($out){
		...
		$query="SELECT bm.`cust`,SUM(bm.`invoice_-total`) FROM bill_main bm WHERE bm.`status` NOT IN ($exclude_status) AND bm.exclude=0 AND bm.`lock`=1 AND date(bm.billed_timestamp)<='$as_of' $store_payment_filter GROUP BY bm.`cust`";
		$result=mysqli_query($conn2,$query);
		while($row=mysqli_fetch_array($result)){
			$cashback_bm_cust[]=$row[0];
			$cashback_bm_total[]=$row[1];
		...
		foreach ($days as $day) {
			...
			$query="SELECT bm.`cust`,SUM(bm.`invoice_+total`) FROM bill_main bm WHERE bm.`status` NOT IN ($exclude_status) AND bm.exclude=0 AND bm.`lock`=1 AND date(bm.billed_timestamp)<='$bkdate' $store_payment_filter GROUP BY bm.`cust`";
			$result=mysqli_query($conn2,$query);
			while($row=mysqli_fetch_array($result)){
				${'bm_cust' . $day}[] = $row[0];
        		${'bm_total' . $day}[] = $row[1];
