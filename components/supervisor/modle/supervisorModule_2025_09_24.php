...
function newQuot($cust0)
{
	...
	if ($out) {
		...
		if ($cust != '') {
			...
			$query = "INSERT INTO `quotation_main` (`id`,`billed_district`,`cust`,`created_by`,`quo_timestamp`,`type`,`v`,`store`,`mapped_inventory`,`sub_system`,`status`)
			VALUES ('$quot_no','$district','$cust','$salesman','$time_now','$qo_type','1','$store','$mapped_inventory','$sub_system','1')";
...
// update by nirmal 09_10_2023, 20_02_2024 tax calcualtion
function updateQuot()
{
	global $message, $quot_no, $salesman, $cust;
	$quot_itm_id = $_GET['id'];
...
// added by nirmal 24_10_2023, 20_02_2024 tax calcualtion
...
function generateQuot($case)
{
	...
	$query = "SELECT date(qm.`quo_timestamp`), time(qm.`quo_timestamp`), $user_name_column, cu.`id`, cu.`name`, cu.nic, cu.mobile, cu.shop_address, st.shop_name, st.address, st.tel, qm.att, qm.warranty, qm.validity, qm.leadtime, qm.terms1, qm.terms2, qm.note, qm.image, qm.image_height, qm.address, qm.heading, qm.v, qm.discount
	...
	$query = "SELECT itm.description,itm.code,qi.qty,qi.unit_price,qi.discount,qi.`comment`,qi.item,itm.`unit` FROM quotation qi, inventory_items itm WHERE qi.item=itm.id AND qi.quot_no='$quot_no'";
	$result = mysqli_query($conn2, $query);
