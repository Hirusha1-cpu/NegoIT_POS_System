...
function getCreditData($sub_system){
	...
	if($out){
		...
		for($i=0;$i<sizeof($cust_id);$i++){
			...
			if($key>-1) $return=$cashback_bm_total[$key]; else $return=0; // If the customer ID is found ($key > -1), $return is set to the corresponding cashback total from $cashback_bm_total.
			...
			if($key>-1) $pay0=$py_total[$key]; else $pay0=0; // If the customer ID is found ($key > -1), $pay0 is set to the corresponding payment total from $py_total
			// $cust_cr_balance0[]=$bm_total[$i]-$pay0;
...
function apendQuot(){
	...
	if(isset($_REQUEST['discount'])) $discount0=$_REQUEST['discount']; else $discount0='';
...
function generateQuot($case){
	...
	$qo_cust_nic=$row[5];
	$qo_cust_mob=$row[6];
	if(empty($cust_address)) $qo_cust_address=$row[7];
	else $qo_cust_address = $cust_address; // Ensure we set $qo_cust_address if $cust_address is not empty
	$qo_st_name=$row[8];
	$qo_st_add=$row[9];
	$qo_st_tel=$row[10];
	$qo_att=$row[11];
