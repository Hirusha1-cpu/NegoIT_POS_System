...
function getCreditReportBasedOnItemCategory($sub_system){
	...
	if($out){
		...
		$category_query="SELECT `id` FROM item_category";
		$category_result=mysqli_query($conn2,$category_query);
		while($category_row=mysqli_fetch_array($category_result)){
			$query ="SELECT cu.id, cu.name, COALESCE(SUM(bm.`invoice_+total`), 0) + COALESCE(SUM(bm.`invoice_-total`), 0), bm.items_category, bm.invoice_no
			FROM cust cu
			LEFT JOIN bill_main bm ON bm.`cust` = cu.id AND bm.exclude = 0 AND bm.`status` NOT IN ($exclude_status) AND bm.`lock` = 1 AND date(bm.billed_timestamp) <= '$as_of' $salemanfilter
			LEFT JOIN ( SELECT py.`cust`, SUM(py.amount) AS amount FROM payment py WHERE py.status = 0 $store_payment_filter AND py.chque_return = 0 AND date(py.payment_date) <= '$as_of' GROUP BY py.`cust`) py ON py.cust = cu.id
			WHERE cu.`status` != 0 AND bm.items_category='$category_row[0]' $sub_system_qry $storefilter $townfilter $groupfilter
			...
			while($row=mysqli_fetch_array($result)){
				...
				// $query="SELECT py.`cust`,SUM(py.amount) FROM payment py WHERE py.cust='$row[0]' AND py.invoice_no='' AND py.status=0 AND py.chque_return=0 AND date(py.payment_date)<='$as_of' GROUP BY py.`cust`";
				// $result=mysqli_query($conn2,$query);
				// // var_dump($query);
				// // exit;
				// while($row=mysqli_fetch_array($result)){
		...
		var_dump($bm_total);
		print '<hr>';
		var_dump($bm_category);
		print '<hr>';
		var_dump($bm_invoice_no);
		// print '<hr>';
		// var_dump($py_total);
		exit;
		...
		var_dump('third query '.$query.'<br><hr>');
		...
		var_dump('forth query '.$query.'<br><hr>');
...
function getQuotList($sub_system){
	...
	$query="SELECT qm.id,up.username,qm.quo_timestamp,qm.validity,st.name,cu.name,qm.heading,SUM(qi.qty * qi.unit_price),qm.`status`,qm.rejected_com,cu.cust_name,cu.mobile,cu.shop_tel,qm.discount FROM quotation_main qm, quotation qi, cust cu, stores st, userprofile up WHERE qm.id=qi.quot_no AND qm.`cust`=cu.id AND qm.created_by=up.id AND qm.store=st.id AND qm.`status`!=0 AND date(qm.quo_timestamp) BETWEEN '$from_date' AND '$to_date' $qry_subsys $qry_cust $qry_item $qry_store $qry_salesman $qry_status GROUP BY qm.id";
	$result=mysqli_query($conn2,$query);
	while($row=mysqli_fetch_array($result)){
		$qm_id[]=$row[0];
		$qm_created_by[]=$row[1];
