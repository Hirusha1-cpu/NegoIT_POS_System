...
function groupStores(){
		include('config.php');
	if(isset($_COOKIE['store'])&&($_GET['components']!='fin')){
...
function getFilter($sub_system){
	global $store_id,$store_name,$up_id,$up_name,$gp_id,$gp_name,$item_id,$item_desc;
	...
	$query="SELECT id,username FROM userprofile WHERE `status`=0 $storefilter3 $sub_system_qry1";
	$result=mysqli_query($conn2,$query);
	while($row=mysqli_fetch_array($result)){
		$up_id[]=$row[0];
		$up_name[]=$row[1];
...
function apendQuot(){
	...
	if($discount==='error'){ $out=false; $msg='Invalid Discount'; }
	...
	$result=mysqli_query($conn,$query);
	if(!$result){ $out=false; $msg='Error: Table Update Failed. Please Contact NegoIT'; }
	if($pr_sr==1){
	...
	if($out){
		...
		$result=mysqli_query($conn,$query);
		if(!$result){ $out=false; $msg='Error: Item Could not be Added to Quotation'; }
	}
...
function removeQuot(){
	...
	if(($salesman==$qm_created_by)&&($qm_status==1)){
		...
		$result=mysqli_query($conn,$query);
		if(!$result){ $out=false; $msg='Error: Item Could not be Removed'; }
	}else{ $out=false; $msg='Unauthorize Request'; }
...
function updateQuot(){
	...
	if(($salesman==$qm_created_by)&&($qm_status==1)){
		...
		$result=mysqli_query($conn,$query);
		if(!$result){ $out=false; $msg='Error: Item Could not be Updated'; }
	}else{ $out=false; $msg='Unauthorize Request'; }
...
global $message,$quot_no;
	...
	$msg='Error: Terms & Conditions Could not be Appended';
	$result1=false;
	include('config.php');
...
/* 1. Auto Approve 			*/		if($qm_status==1 && $new_status==2 && $approver) $query1="UPDATE `quotation_main` SET `status`='3',`quo_timestamp`='$datetime',`approved_by`='$salesman',`approve_timestamp`='$datetime' WHERE id='$quot_no'"; else
/* 2. Submit for Approval 	*/		if($qm_status==1 && $new_status==2 && $qo_owner) $query1="UPDATE `quotation_main` SET `status`='2',`quo_timestamp`='$datetime' WHERE id='$quot_no'"; else
/* 3. Approve the Quot	 	*/		if($qm_status==2 && $new_status==3 && $approver) $query1="UPDATE `quotation_main` SET `status`='3',`approved_by`='$salesman',`approve_timestamp`='$datetime' WHERE id='$quot_no'"; else
/* 4. Reject the Quot	 	*/		if($qm_status==2 && $new_status==4 && $approver) $query1="UPDATE `quotation_main` SET `status`='4',`approved_by`='$salesman',`approve_timestamp`='$datetime' WHERE id='$quot_no'"; else
/* 5. Delete the Quot	 	*/		if(($qm_status==1 || $qm_status==2 || $qm_status==3) && $new_status==0 && $qo_owner) $query1="UPDATE `quotation_main` SET `status`='0',`deleted_by`='$salesman',`deleted_timestamp`='$datetime' WHERE id='$quot_no'"; else
/* 6. Send to Cust	 		*/		if($qm_status==3 && $new_status==5) $query1="UPDATE `quotation_main` SET `status`='5',`submited_by`='$salesman',`submited_timestamp`='$datetime' WHERE id='$quot_no'"; else
/* 7. Customer Accepted		*/		if($qm_status==5 && $new_status==6) $query1="UPDATE `quotation_main` SET `status`='6',`custacc_by`='$salesman',`custacc_timestamp`='".$_GET['accdate']."',`cust_po`='".$_GET['custpo']."' WHERE id='$quot_no'"; else
/* 8. Customer Accepted		*/		if($qm_status==5 && $new_status==7) $query1="UPDATE `quotation_main` SET `status`='7',`custacc_by`='$salesman',`custacc_timestamp`='$datenow',`rejected_com`='".$_GET['custreject']."' WHERE id='$quot_no'"; else
/* 9. Order Completed		*/		if($qm_status==6 && $new_status==8) $query1="UPDATE `quotation_main` SET `status`='8',`completed_by`='$salesman',`completed_timestamp`='$datetime' WHERE id='$quot_no'"; else
									{ $out=false; $msg='Error: Undefine Action'; }
		if($out){ 
			$result1=mysqli_query($conn,$query1);
			if(!$result1){ $out=false; $msg='Error: Quotation Status Could not be Changed'; }
		}
...
global $quot_no,$qo_date,$qo_time,$qo_salesman,$qo_cust_id,$qo_cust_name,$qo_cust_nic,$qo_cust_mob,$qo_cust_address,$cu_details,$qo_st_name,$qo_st_add,$qo_st_tel,$qo_att,$qo_warranty,$qo_validity,$qo_leadtime,$qo_terms,$qo_note,$qo_image,$qo_image_hei,$qi_item_des,$qi_item_code,$qi_qty,$qi_uprice,$qi_discount,$qi_comment,$qi_filled_qty,$total,$systemid,$web,$email,$qo_heading,$qo_v;
	$quot_no=$_REQUEST['id'];
	$total=0;
	$qi_item_des=$qi_item_code=$qi_qty=$qi_uprice=$qi_discount=array();
...
function imageresize($systemid,$image1){
	$out=false;
	$dest_image1= 'images/'.$systemid.'/quotation/'.$image1; // make sure the directory is writeable
	$image1='images/'.$systemid.'/quotation/uploads/'.$image1;
...
function qoAddImage(){
	...
	if($quo_status==2 || $quo_status==3){
		$target_dir = "images/$systemid/quotation/uploads/";
		...
		if($out){
			...
			unlink('images/'.$systemid.'/quotation/'.str_pad($id, 10, "0", STR_PAD_LEFT).'.jpg');
			$message=$msg;
			return false;
		}
	}else{
