...
function getTemporaryBills(){
	...
	WHERE bi.`bm_no`=bm.`bm_no` AND bm.`billed_by`=up.`id` AND bm.`cust`=cu.`id` AND bm.`store`=st.`id` AND bm.`status` NOT IN (0,7) AND bm.`store`='$store' GROUP BY bm.`bm_no` ORDER BY st.`name`, bi.`date` DESC";
	$result=mysqli_query($conn2,$query);
	while($row=mysqli_fetch_array($result)){
		$invoice_no[]=$row[0];
		$billed_by[]=$row[1];
...
function getQuotationItems(){
	...
	if(isset($_REQUEST['id'])){
		...
		$query="SELECT qm.`discount` FROM quotation_main qm WHERE qm.`id`='$quot_no'";
		$result=mysqli_query($conn2,$query);
		$row = mysqli_fetch_assoc($result);
		$qo_discount=$row['discount'];
...
// update by nirmal 11_01_2024 (fixed error log bug, discount undifiend), 20_02_2024 (tax calculation)
function apendQuot(){
	global $message,$quot_no,$salesman,$cust;
	$quot_no=$_REQUEST['id'];
