...
<script>
	function showBillTypeAlertAndProceed() {
		var custOdr = '<?php echo $cust_odr; ?>';
		var systemId = '<?php echo $systemid; ?>';
		var allowedSystemIds = [1, 10, 13, 16, 17];

		if (allowedSystemIds.includes(parseInt(systemId))) {
			var billType = (custOdr === 'yes') ? 'Customer Order' : 'Sale Bill';
			var billMsg = (custOdr === 'yes')
				? 'You are going to create a <b>Customer Order</b>'
				: 'You are going to create a <b>Sale Bill</b>';
				...
				if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
					...
					if (returntext != '') {
						...
						if (myObj.msg == 'Done') {
							...
							document.getElementById('div_submit').innerHTML = '<input type="button" value="Submit" onclick="showBillTypeAlertAndProceed()" style="width:100px; height:50px;" />';
						}
					}
				}
			};
			...
			<?php if ($current_district != '') { ?>
				<table align="center" bgcolor="<?php print $main_tale_color; ?>" style="border-radius: 15px;" border="0">
					...
					if ($systemid == 1 || $systemid == 4 || $systemid == 10 || $systemid == 15)
						...
						print '<tr><td colspan="5"><input type="hidden" id="search-sm" name="bill_sm" /></td></tr>';
					?>
					<tr>
						<td width="50px"></td>
						...
						<td colspan="2">
							<table>
								<tr>
									<td>
										<div id="div_submit"><input type="button" value="Submit" onclick="showBillTypeAlertAndProceed()"
				...
				<table align="center" height="100%">
					<tr>
						<td style="font-size:12pt;">
							<form id="searchinv"
								action="index.php?components=bill2&action=search_bill&s=<?php print $bill_salesman; ?>&cust_odr=<?php print $_GET['cust_odr']; ?>"
								method="post" onsubmit="removeLeadingZeros()">
					...
					<tr>
						<td style="font-size:12pt;">
							<form action="index.php"><input type="hidden" name="components" value="bill2" /><input type="hidden"
									...
									value="<?php print $_GET['cust_odr']; ?>" /><input type="text" style="width:100px" name="searchcustid"
									placeholder="Customer ID" value="<?php if (isset($_GET['searchcustid']))
										print $_GET['searchcustid']; ?>" onclick="this.value=''" /><input type="Submit" value="Search" />
							</form>
						</td>
					</tr>
					<tr>
						<td style="font-size:12pt;">
							<form action="index.php"><input type="hidden" name="components" value="bill2" /><input type="hidden"
									...
									name="searchcustname" placeholder="Customer Name" value="<?php if (isset($_GET['searchcustname']))
										print $_GET['searchcustname']; ?>" onclick="this.value=''" /><input type="Submit" value="Search" />
							</form>
						</td>
					</tr>
					<tr>
						<td style="font-size:12pt;">
							<form action="index.php"><input type="hidden" name="components" value="bill2" /><input type="hidden"
									...
									placeholder="Mobile Number" value="<?php if (isset($_GET['searchmob']))
										print $_GET['searchmob']; ?>" onclick="this.value=''" /><input type="Submit" value="Search" />
							</form>
						</td>
					</tr>
					<tr>
						<td style="font-size:12pt;">
							<form action="index.php"><input type="hidden" name="components" value="bill2" /><input type="hidden"
									name="action" value="home" /><input type="hidden" name="s"
									value="<?php print $bill_salesman; ?>" /><input type="hidden" name="cust_odr"
									value="<?php print $_GET['cust_odr']; ?>" /><input type="text" style="width:100px" name="searchunic"
									placeholder="Unique ID" value="<?php if (isset($_GET['searchunic']))
										print $_GET['searchunic']; ?>" onclick="this.value=''" /><input type="Submit" value="Search" />
							</form>
						</td>
					</tr>
				</table>
...
if ($current_district == '') {
	if ($static_district != 0)
		print '<script type="text/javascript"> document.getElementById("district").value=' . $static_district . '; setDistrict2("bill2"); </script>';
}
