			...
			<td>
					...
					<?php
						for($i=0;$i<sizeof($bill_id);$i++){
							...
							if($bi_return_odr[$i]==0){
								...
								print '<tr style="background-color:'.$color1.'"><td style="padding-bottom: 5px;">'.$bi_code[$i].'</td>';
								if($dn=='no' && (!($picked))) print '<td align="right">'.$bi_price[$i].'</td>';
								print '<td align="right">'.$bi_qty[$i].'</td>';
								if($dn=='no' && (!($picked))) print '<td align="right">'.number_format($bi_qty[$i]*$bi_price[$i]).'&nbsp;</td>';
								if($dn=='no' && ($picked)) print '<td align="right">'.str_pad(str_replace("<br />","<BR>",$bi_drawer[$i]),5,' ',STR_PAD_LEFT).'&nbsp;</td>';
								if($dn=='yes') print '<td></td><td></td>';
								print '</tr>';
								if($dn=='no' && (!($picked))) print '<tr><td colspan="'.$colspan.'">&nbsp;</td></tr>';
							}
						}
						if($dn=='no'){
							if(!($picked)){
								...
								print '<tr style="font-size:10pt;"><td colspan="3" align="right" height="20px" style="padding-right:5px; border-right:1; border-bottom:0; border-top:0;">'.$advance.' Payment: Cash</td><td align="right">'.number_format($cash_amount).'&nbsp;&nbsp;</td></tr>';
								print '<tr style="font-size:10pt;"><td colspan="3" align="right" height="20px" style="padding-right:5px; border-right:1; border-bottom:0; border-top:0;">'.$advance.' Payment: Bank</td><td align="right">'.number_format($bank_amount).'&nbsp;&nbsp;</td></tr>';
				...
				if(sizeof($rt_pending_code)>0){
					...
					for($i=0;$i<sizeof($rt_pending_code);$i++){
						...
						print '<tr>
							<td>'.$rt_pending_code[$i].'</td>
							<td align="right">'.number_format($rt_pending_qty[$i]).'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>';
							if($picked) print '<td align="right">'.str_pad(str_replace("<br />","<BR>",$rt_pending_itm_drawer[$i]),5,' ',STR_PAD_LEFT).'</td>';
						print '</tr>';
						print '<tr><td colspan="'.$colspan.'"><br></td></tr>';
					}
					print '</table>';
				}
				if(sizeof($removed_code)>0){
