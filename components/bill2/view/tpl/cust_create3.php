		...
		function getCustData($case, $val) {
			...
			xmlhttp.onreadystatechange = function() {
				if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
					...
					myObj.forEach(item => {
						document.getElementById('shop_name').value = item.cu_name1 ? item.cu_name1 : "";
						document.getElementById('nic').value = item.cu_nic1 ? item.cu_nic1 : "";
						document.getElementById('mobile').value = item.cu_mobile1 ? item.cu_mobile1 : "";
						document.getElementById('cr_limit').value = item.cu_crlimit1 ? item.cu_crlimit1 : "";
						document.getElementById('customer').value = item.cu_custname1 ? item.cu_custname1 : "";
						document.getElementById('nickname').value = item.cu_nickname1 ? item.cu_nickname1 : "";
						document.getElementById('shop_address').value = item.cu_shop_add1 ? item.cu_shop_add1 : "";
						document.getElementById('shop_tel').value = item.cu_shop_tel1 ? item.cu_shop_tel1 : "";
						document.getElementById('home_address').value = item.cu_home_add1 ? item.cu_home_add1 : "";
						document.getElementById('home_tel').value = item.cu_home_tel1 ? item.cu_home_tel1 : "";
						document.getElementById('email_add').value = item.cu_email_add ? item.cu_email_add : "";
					...
					success: function(data) {
						...
						if (obj.user_create) {
							...
							if (ws == "wholesale_cust") {
								...
								toastr.options = toasterOptions;
								toastr.success(obj.user_create, 'Success!');
								window.location = 'index.php?components=<?php print $components ?>&action=new_tmp_bill2&cust_odr=<?php print $_GET["cust_odr"]; ?>&cust_id='+obj.cust_id+'&sm_id=<?php print $_GET["sm_id"]; ?>&message=Customer created successfully!&re=success';
							}
						}
...
<!-- ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------  -->
		...
		<table align="center" bgcolor="#E5E5E5" style="font-size:11pt;">
			...
			<tr>
				...
				<td>
					<input type="tel" name="mobile" id="mobile" value="<?php print $_GET['mob']; ?>" />
					<small>
			...
			<?php if ($_GET['action'] == 'onetime_cust') { ?>
				...
				<tr>
					<td colspan="4" align="center"><br />
						<div id="div_submit">
							<input type="button" id="submit" value="Add Customer" style="width:130px; height:50px" onclick="addCustomerAjax()" />
						</div>
						<br /><br />
					</td>
				</tr>
	...
	<?php if (!($_GET['action'] == 'onetime_cust')) { ?>
		...
		<form enctype="multipart/form-data" action="index.php?components=<?php print $components; ?>&action=add_cust_image1&s=<?php print $_GET['sm_id']; ?>&cust_odr=<?php print $_GET['cust_odr']; ?>" method="POST" id="image_form" name="image_form">


			<table align="center" bgcolor="#E5E5E5" style="font-size:11pt; border-radius: 0px 0px 15px 15px;">
				<tr>
					<td colspan="4"><br /></td>
				</tr>
				<tr>
					<td width="50px"></td>
					<td width="130px">Documents</td>
					<td width="250px">
				...
				<tr>
					<td width="48px"></td>
					<td><small><span style="color:red;">*</span> Required fields </small><br /><br /></td>
