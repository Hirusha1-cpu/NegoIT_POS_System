...
<table align="center" height="<?php print $page_height; ?>px" width="100%" border="1" cellspacing="0">
	...
	for ($i = 0; $i < sizeof($bill_id); $i++) {
		if ($bi_return_odr[$i] == 0) {
			...
			if ($dn == 'no') {
				...
				$unit_price = ($bi_price[$i] / ($tax + 100)) * 100;
				$line_total = $bi_qty[$i] * $unit_price;  // tax-excluded line total
				print '<td style="border-bottom:0; border-top:0;" align="right">' . number_format($unit_price, $decimal) . '&nbsp;&nbsp;
					</td>
					<td align="right" style="border-bottom:0; border-top:0;">
						' . number_format($line_total, $decimal) . '&nbsp;&nbsp;</td>';
			} else {
		...
		$total_discount += $bi_qty[$i] * $bi_discount[$i];
	}
	...
	if ($dn == 'no') {
		...
		$total = $sub_total - $bm_discount + $vat_amount;

		// discount
		if ($bm_discount > 0) {
			print '<tr style="font-family:Calibri; font-size:11pt; font-weight:900; color:white; background-color:black; -webkit-print-color-adjust: exact;">
						<td colspan="' . $colspan . '" align="right" height="20px" style="padding-right:5px; border-right:1; border-bottom:1; border-top:0; color:white;">Discount</td>
						<td align="right" style="padding: 0 5px;">' . number_format($bm_discount, $decimal) . '</td>
					</tr>';
		...
		print '<tr style="font-family:Calibri; font-size:11pt; font-weight:900; color:white; background-color:black; -webkit-print-color-adjust: exact;">
		<td colspan="' . $colspan . '" align="right" height="20px" style="padding-right:5px; border-right:1; border-bottom:1; border-top:0; color:white;">Total</td>
		<td align="right" style="padding: 0 5px;">' . number_format($sub_total, $decimal) . '</td>
		</tr>';
		...
		if (($tax != '') && ($tax != 0)) {
			// old $tax_added_value
			print '<tr style="font-family:Calibri; font-size:11pt; font-weight:900; color:white; background-color:black; -webkit-print-color-adjust: exact;">
				<td colspan="' . $colspan . '" align="right" height="20px" style="padding-right:5px; border-right:1; border-bottom:1; border-top:0; color:white;">
				Value Added Tax ' . $tax . '%
				</td>
				<td align="right" style="padding: 0 5px;">' . number_format($vat_amount, $decimal) . '</td>
			</tr>';
		}

		// total
		// $total = $sub_total - $tax_added_value; old
		print '<tr style="font-family:Calibri; font-size:11pt; font-weight:900; color:white; background-color:black; -webkit-print-color-adjust: exact;">
			<td colspan="' . $colspan . '" align="right" height="20px" style="padding-right:5px; border-right:1; border-bottom:1; border-top:0; color:white;">
				Payable Amount Including VAT
			</td>
			<td align="right" style="padding: 0 5px;">
				' . number_format($total, $decimal) . '
			</td>
		</tr>';
		...
		print '<tr style="font-family:Calibri; font-size:11pt; font-weight:900; color:white; background-color:black; -webkit-print-color-adjust: exact;">
				...
				<td align="right" style="padding: 0 5px;">
					' . number_format($cash_amount + $bank_amount + $chque_amount + $quotation_advance_payment, $decimal) . '
				</td>
			</tr>';
		...
		print '<tr
				...
				<td colspan="' . $colspan . '" align="right" height="20px"
					style="padding-right:5px; border-right:1; border-bottom:0; border-top:0; color:white;">Due Payment
				</td>
				<td align="right" style="padding: 0 5px;">' . number_format(($total - $cash_amount - $bank_amount - $chque_amount - $quotation_advance_payment), $decimal) . '</td>
			</tr>';
	}
	?>
</table>
