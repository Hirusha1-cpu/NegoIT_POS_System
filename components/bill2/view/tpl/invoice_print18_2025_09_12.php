<?php
	...
	if($bi_type==1 || $bi_type==2){
        if(($tax != '') && ($tax != 0) && ($sub_system == 0)) $bill_title='TAX INVOICE';
    ...
    <tbody class="report-content">
		...
		<table width="100%" border="0" style="font-family:Calibri; font-size:11pt;">
			...
			<tr>
				<td>
					<?php
                        print '<a href="../../../../index.php?components=bill2&action=cust_details&id='.$cu_id.'&action2=finish_bill&id2='.$_GET['id'].'" target="_parent" title="Customer: '.$cu_details.'" style="text-decoration:none; color:black" >Customer: '.ucfirst($bi_cust0).'</a>,<br />Address: '.wordwrap($bi_cust0_address, 50, "<br>");
                        if ($sub_system == 0) {
                            print '<br> TRN No: ' . $cust_tax_no;
                        }
                    ?>
        ...
        <table align="center" height="<?php print $page_height; ?>px" width="100%" border="1" cellspacing="0" >
            <tr style="font-family:Arial; font-size:10pt; color:black; background-color:white; -webkit-print-color-adjust: exact;">
                ...
                <?php if($dn=='no'){
                        ...
                        <?php } ?>
                    <th width="60px" >TOTAL (<?php print rtrim($currency,"."); ?>)</th>
                <?php }else{ ?>
			...
			<?php
				for($i=0;$i<sizeof($bill_id);$i++){
					if($bi_return_odr[$i]==0){
						print '<tr style="font-size:9pt; border-bottom: 1px solid #EEE;" height="20px">
                                    <td style="border-top:0;" align="center">'.sprintf('%02d',($i+1)).'</td>
									<td style="border-top:0; padding-left:10px;">'.$bi_desc[$i].'</td>
									<td style="border-top:0;" align="right">'.number_format($bi_qty[$i]).'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>';
						if($dn=='no'){
							print '<td style="border-top:0;" align="right">'.number_format($bi_price[$i],2).'&nbsp;&nbsp;</td>
							<td align="right" style="border-top:0;">'.number_format(($bi_qty[$i]*$bi_price[$i]),2).'&nbsp;&nbsp;</td>';
						}else{
							print '<td style="border-top:0;"></td>';
						}
						print '</tr>';
					}
				}
                print '<tr style="font-size:9pt; border-bottom: 1px solid #EEE;">
                ...
                if($dn=='no'){
                    if($sub_system==0)
                    print
                    '<td align="right" style="border-top:0;"></td>
                    <td align="right" style="border-top:0;"></td>';
                }else{
                    print '<td style="border-top:0;"></td>';
                }
				...
				if($dn=='no'){
                    ...
                    if(isCustomerTotalOutstandingShowInBill(2)){
                        print '<tr style="font-family:Calibri; font-size:11pt; font-weight:900; color:black; background-color:white; -webkit-print-color-adjust: exact;" id="total_outstanding_row"><td colspan="'.$colspan.'" align="right" height="20px" style="padding-right:5px; border-right:1; border-bottom:1; border-top:0; color:black;">Total Outstanding Amount</td><td align="right" id="credit_balance_cell"><span id="credit_balance_span">'.number_format($credit_balance, $decimal).'</span>&nbsp;&nbsp;</td></tr>';
                    }
				}
        	?>
        </table>
