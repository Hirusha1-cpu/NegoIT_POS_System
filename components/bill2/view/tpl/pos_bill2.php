...
for($i=0;$i<sizeof($bill_id);$i++){
	if($bi_discount[$i]!=0)
		$dis_print='@'.number_format(round(($bi_discount[$i]/($bi_price[$i]+$bi_discount[$i])*100),2),2).'%';
	else $dis_print='';
	print str_replace("<br />","<BR>",$bi_desc[$i]);
	print str_pad('[Dis '.$dis_print.']',13,' ',STR_PAD_RIGHT).'  '.str_pad(number_format($bi_price[$i]+$bi_discount[$i]),6,' ',STR_PAD_LEFT).'  '.str_pad(number_format($bi_price[$i]),7,' ',STR_PAD_LEFT).'  '.str_pad(number_format($bi_qty[$i]),4,' ',STR_PAD_LEFT).'  '.str_pad(number_format(($bi_qty[$i]*$bi_price[$i]),2),10,' ',STR_PAD_LEFT).'<br>';
	print '<br>';
}

$cash_amount = isset($cash_amount) ? $cash_amount : 0;
$card_amount = isset($card_amount) ? $card_amount : 0;
$bank_amount = isset($bank_amount) ? $bank_amount : 0;
$chque_amount = isset($chque_amount) ? $chque_amount : 0;
$pay_pro_fee = isset($pay_pro_fee) ? $pay_pro_fee : 0;
...
// print $advance.' Payment: Cheque '.str_pad(number_format($chque_amount,2),21,' ',STR_PAD_LEFT).'<br>';
