		...
		<td valign="top">
			<div style="margin:0 auto; background-color:#EEEEEF; border-radius: 15px; padding-left:10px; padding-right:10px; height:50px; " >
				<table height="100%" style="color:#0158C2; font-family:Calibri; font-size:16pt; vertical-align:middle">
					<tr><td><strong>Customer Payment</strong></td></tr>
				</table>
			</div>
			<form name="payForm" action="index.php?components=bill2&action=add_payment" onsubmit="return validatePayment()" method="post" >
				...
				<table align="center" bgcolor="#E5E5E5" style="border-radius: 15px;">
					...
					<tr>
						...
						<td><input type="text" name="invoice_no" id="invoice_no" onchange="invoiceNumberChange()" /></td>
					...
					<tr>
						...
						<td colspan="3">
							<div id="sw_card" style="display:none">
									...
									<tr><td style="font-size:12pt" width="100px">Card Terminal</td><td>
					...
					<tr>
						...
						<td colspan="3" height="10px">
							<div id="addpayment"><input type="submit" value="Add Payment" style="width:100px; height:50px" /></div>
						</td>
					</tr>
				<?php } ?>
		...
		<td valign="top">
			...
			if(isset($_GET['cust'])){
				print '<table align="center" height="100%" style="font-family:Calibri; font-size:11pt">';
					...
					for($i=0;$i<sizeof($payment_date);$i++){ if(($i%2)==0) $color='#FAFAFA' ; else $color='#DDDDDD' ;
						...
						<td title="'.$full_data[$i].'">&nbsp;&nbsp;<a style="color:'.$pay_color[$i].'"
								href="index.php?components=bill2&action=finish_payment&id='.$payment_id[$i].'">'.str_pad($payment_id[$i],
								7, "0", STR_PAD_LEFT).'</a>&nbsp;&nbsp;</td>
						<td align="right" title="'.$full_data[$i].'">&nbsp;&nbsp;'.number_format($payment_amount[$i],
							$decimal).'&nbsp;&nbsp;</td>
						</tr>';
						}
						print '</table>';
			...
			<table style="font-family:Calibri; font-size:11pt" align="center" width="300px">
				<tr>
					<td colspan="3">
						<form id="search_form1" method="post" action="index.php?components=bill2&action=search_pay"><input
								type="number" style="width:200px" name="search1" id="search1" placeholder="Payment Number" /><a
				...
				<tr>
					<td colspan="3">
						<form id="search_form2" method="post" action="#"><input type="number" style="width:200px" name="invoice_id"
								placeholder="Cheque Payments by Invoice No" /><a
				...
				for($i=0;$i<sizeof($sh_payid);$i++){
					...
					print '<tr style="color:'.$sh_color[$i].'; background-color:'.$color.'">
						...
						<td>&nbsp;&nbsp;<a style="color:'.$sh_color[$i].'"
								href="index.php?components=bill2&action=finish_payment&id='.$sh_payid[$i].'">'.str_pad($sh_payid[$i],
								7, "0", STR_PAD_LEFT).'</a>&nbsp;&nbsp;</td>
