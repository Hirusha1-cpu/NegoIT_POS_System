...
if (($tm_template == 3) || ($tm_template == 4))
	$execute = 'pos_bill2.php';
else
	$execute = 'pos_bill1.php';
if ($bm_status == 6) {
	$bill_title1 = 'CUST ORDER';
	$sub_title = 'INVOICE NO';
	if (($tax != '') && ($tax != 0)) {
		$sub_title = 'TAX INVOICE NO';
	}
} else {
	$bill_title1 = 'INVOICE';
	$sub_title = 'INVOICE NO';
	if (($tax != '') && ($tax != 0)) {
		$bill_title1 = 'TAX INVOICE';
		$sub_title = 'TAX INVOICE NO';
	}
}
if ($bi_type == 3) {
...
$decimal_places = getDecimalPlaces(1);
...
<form method="post">
	<div class="w3-container" style="margin-top:75px">
		...
		<div class="w3-row">
			...
			<div class="w3-col">
				<?php if (($main_sub_system_id == $sub_system) || ($main_refinvid == $_COOKIE['store']) || (($systemid == 24) && (isset($_COOKIE['top_manager'])))) { ?>
					<table width="90%">
						<!-- Invoice status -->
						<tr>
						...
						<tr>
							...
							<td style="vertical-align:top;">
										...
										<td>
											...
											<table width="90%" align="center">
												<tr>
													<td>
														<span
															style="font-family:'Arial Black'; font-size:20pt"><?php print $bill_title1; ?></span>
													</td>
												</tr>
												<tr>
													<td>
									...
									<tr>
										<td>
											...
											if (($systemid == 1) && ($bill_store == 9)) {
												$id_padded = str_pad(substr($_REQUEST['id'], 1), 6, "0", STR_PAD_LEFT);
											} else {
												$id_padded = str_pad($_REQUEST['id'], 7, "0", STR_PAD_LEFT);
											}
											?>
											<?php print $sub_title; ?>: [<?php print $id_padded; ?>]<br />
								...
								<table align="center" width="300px" border="0" cellspacing="0">
									...
									if (isCustomerTotalOutstandingShowInBill(1)) {
										print '<tr style="font-size:10pt; font-weight:900;" id="total_outstanding_row">
															<td colspan="3" align="right" height="20px" style="padding-right:5px; border-right:1; border-bottom:0;">Total Outstanding Amount</td>
															<td align="right"><span id="credit_balance_span">' . number_format($credit_balance, $decimal_places) . '</span>&nbsp;&nbsp;</td>
															</tr>';
									}
									print '	<tr><td colspan="4">------------------------------------------------------------------------</td></tr>';
									if ($chq0_fullNo != '') {
						...
						<table align="right">
							<tr>
								<td>
									<?php if (($bm_lock == 1) || ($bm_lock == 2)) { ?>
										<div
											style="background-color:#6699FF; border:medium; border-color:black; width:80px; text-align:center">
											<a class="shortcut-button"
												onclick="parent.location='printscheme://<?php print $url . '/' . $execute . '?id=' . $_REQUEST['id']; ?>'"
												href="#"><span style="text-decoration:none; font-family:Arial; color:navy;">
							...
							<tr>
								<td>
									<?php if (($bm_lock == 1) || ($bm_lock == 2)) { ?>
										<div
											style="background-color:#9966FF; border:medium; border-color:black; width:80px; text-align:center">
											<a class="shortcut-button"
												onclick="sendToQuickPrinterChrome(<?php print $_GET['id']; ?>)" href="#"><span
							...
							<?php
							$viewType = 'grouped';
							if (!empty($_GET['view']) && $_GET['view'] == 'ungrouped') {
								$viewType = 'ungrouped';
							}

							// Set toggle variables
							$toggleView = $viewType == 'grouped' ? 'ungrouped' : 'grouped';
							$toggleText = $viewType == 'grouped' ? 'Ungrouped' : 'Grouped';
							$isChecked = $viewType == 'grouped' ? 'checked' : '';
							...
							if ($systemid == 1 || $systemid == 17) {
								...
								<tr>
									<td>
										...
										<div style="">
											<label
												style="display: flex; align-items: center; justify-content: center; font-family: Arial; cursor: pointer;">
												<input type="checkbox" id="viewToggleCheckbox" <?php echo $isChecked; ?>
													style="margin-right: 5px;"
													onclick="window.location.href='index.php?components=<?php echo htmlspecialchars($_GET['components']) ?>&action=finish_bill&id=<?php echo $_GET['id']; ?>&view=<?php echo $toggleView ?>'">
												Show <?php echo $toggleText ?> Pending Return
											</label>
										</div>
									</td>
								</tr>
							...
							<tr>
								<td>
									...
									<?php if ($bm_status != 0) {
										if ($billpermission) { ?>
											<div
												style="background-color:#FF9191; border:medium; border-color:black; width:80px; text-align:center">
												<a class="shortcut-button" onclick="deleteInvoice('<?php print $_GET['id']; ?>')"
													href="#"><span style="text-decoration:none; font-family:Arial; color:navy;">
