...
<table align="center">
	<tr>
				...
				<table align="center" bgcolor="#E5E5E5" style=" border-radius: 15px;">
					...
					<?php } else { ?>
						...
						<tr>
							...
							<td colspan="2"><input type="text" name="rplace_code" id="tags6" value="<?php if ($unic_item_code != '')
								print $unic_item_code; ?>" onclick="this.value=''" /></td>
							<td width="50px"></td>
						</tr>
						<tr>
						...
						<?php if (!isQuickBooksActive(1)) { ?>
							<tr>
								...
								<td colspan="2"><input type="text" name="extra_pay" style="width:50px" onfocus="showCodeDesc()" /><input
										type="button" value="Get Cost Difference" onclick="getPriceDiff()" /></td>
								<td width="50px">
		...
		<td valign="top">
			<!-- ------------------Item List----------------------- -->
			<div
				style="background-color:#D1D8F1; border-radius: 5px; padding-left:20px; padding-right:20px; height:500px; width:400px"
				align="left">
				<br />
			...
			<input type="Button" value="Remove"  onclick="removeReturn(' . $rt_id[$i] . ',\'bill2\')" style="background-color:maroon; color:white"/>
				...
				<?php } else { ?>
					<form id="searchinv" action="index.php?components=bill2&action=search_return" method="post">
						<table height="100%">
							<tr>
								<td style="font-size:12pt;"><input type="text" style="width:200px" name="search1" id="search1"
										placeholder="Search Return" /><input type="Submit" value="Search" />
								</td>
							</tr>
						</table>
					</form>
