...
<script>
	...
	function getItemData($case, $val) {
		...
		if ($case == 'code') document.getElementById('hidden_code').value = $val;
		else document.getElementById('hidden_description').value = $val;
		...
		xhttp.onreadystatechange = function () {
			if (this.readyState == 4 && this.status == 200) {
				...
				if (myObj.error) {
					// Show error message to user (customize as needed)
					document.getElementById('av_qty').innerHTML =
						'<span style="color:red;">' + myObj.message + '</span>';
					...
					document.getElementById('discount_btn').innerHTML = '';
					return;
				}
				...
				if (returntext != '') {
					...
					for ($i = 0; $i < $items.length; $i++) {
						...
						$datalist +=
							...
							+ '<option value="r,' + $item[2] + '" <?php if (isset($_COOKIE['retail']) && ($_COOKIE['retail'] == 1)) {
								print 'selected';
							} ?>>Retail - ' + thousands_separators($item[2]) + '</option>'
							+ '<option value="w,' + $item[3] + '" <?php if (isset($_COOKIE['retail']) && ($_COOKIE['retail'] == 0)) {
								print 'selected';
							} ?>>Wholesale - ' + thousands_separators($item[3]) + '</option>'
							+ '</select>'
	...
	<?php if (($systemid == 13) && ($components != 'to')) { ?>
		function updateInvoiceDiscount(id) {
			var discountInput = document.getElementById("discount_for_invoice");
			var discount = discountInput.value;
			var originalDiscountElem = document.getElementById("original_invoice_discount");
			var submitBtn = document.getElementById("invoice_discount_btn"); // The sending button

			id = "<?php if (isset($_GET['bill_no'])) {
				echo addslashes($_GET['bill_no']);
			} else {
				echo '0';
			} ?>";
			...
			if (discount > 0) {
				if (
					confirm(
						"Are you sure you want to apply a discount of " +
						discount +
						" to this invoice?"
					)
				) {
					...
					xhr.open(
						"POST",
						"index.php?components=<?php echo $components; ?>&action=update_invoice_discount_ajax",
						true
					);
						...
						if (xhr.readyState === 4) {
							if (xhr.status === 200) {
								...
								if (response.status === "success") {
									...
									// Optionally update the original value if the change is valid
									originalDiscountElem.value = discount;
									submitBtn.style.display = "block";
								} else {
									// Revert the discount value back to the original on error
									discountInput.value = originalDiscountElem.value;
									alert(response.message || "Error applying discount.");
									// Show the sending button again
									submitBtn.style.display = "block";
								}
							} else {
								// Request failed; revert the discount input & show button
								discountInput.value = originalDiscountElem.value;
					...
					// Send the POST data with id and discount (both URL-encoded)
				...
				alert("Discount value cannot be a negative value");
				return false;
			}
		}
		function updateDueDate(id) {
			...
			var originalDueDateElem = document.getElementById("original_due_date");
			var submitBtn = document.getElementById("due_date_btn"); // The sending button

			if (dueDate) {
				if (
					confirm(
						"Are you sure you want to change the due date to " +
						dueDate +
						"?"
					)
				) {
					...
					xhr.open(
						"POST",
						"index.php?components=<?php echo $components; ?>&action=update_due_date_ajax",
						true
					);
					xhr.setRequestHeader(
						"Content-Type",
						...
						if (xhr.readyState === 4) {
							if (xhr.status === 200) {
								...
								if (response.status === "success") {
									alert(response.message);
									// Optionally update the original value if the change is valid
									originalDueDateElem.value = dueDate;
									submitBtn.style.display = "block";
								} else {
									// Revert the discount value back to the original on error
									dueDateInput.value = originalDueDateElem.value;
									alert(response.message || "Error changing due date.");
									// Show the sending button again
									submitBtn.style.display = "block";
								}
							} else {
...
<input type="hidden" id="hidden_description" value="" />
<input type="hidden" id="original_invoice_discount" value="<?php if (isset($invoice_discount))
	print $invoice_discount; ?>" />
<input type="hidden" id="original_due_date" value="<?php if (isset($due_date))
	print $due_date; ?>" />
	...
	<tr>
		...
		<td valign="top">
			<div style="background-color:#EEEEEF; border-radius: 15px; padding-left:10px; padding-right:10px">
				<table width="100%">
					<tr>
						<td>
							<h1 style="color:#2277DD">
								<?php if (isset($cust_odr) && $cust_odr == 'no')
									print 'Sales Billing';
								else
									print 'Cust Order'; ?>
							</h1>
						...
						<td align="right">
							<select name="district" id="district" onchange="setDistrict2('bill2')" <?php if ($static_district != 0)
								print 'disabled="disabled"'; ?>>
								<option>-SELECT-</option>
								<?php
								if (isset($district_id)) {
									for ($i = 0; $i < sizeof($district_id); $i++) {
			...
			<?php if (isset($current_district) && $current_district != '') { ?>
				<table align="center" bgcolor="<?php if (isset($main_tale_color))
					print $main_tale_color; ?>" style="border-radius: 15px;" border="0">
					<tr>
						<td colspan="5"><br /></td>
					</tr>
					<?php
					print '<tr><td width="50px"></td><td style="font-size:12pt">';
					if (isset($cust_mtype) && $cust_mtype == 'primary')
						print '<span style="color:red">Primary<br />Customer</span>';
					elseif (isset($cust_mtype) && $cust_mtype == 'secondary')
					...
					print '</td><td width="50px"></td></tr>';
					if (isset($systemid) && ($systemid == 1) || ($systemid == 13) || ($systemid == 15)) {
					...
					<tr>
						...
						<td colspan="2">
							...
							<?php if (isset($systemid) && ($systemid == 4) || ($systemid == 13) || ($systemid == 15) || ($systemid == 20) || ($systemid == 24)) { ?>
								<?php if ($bill_item_count == 0) { ?>
									<select name="salesman" id="salesman" onchange="changeSalesman(<?php if (isset($_GET['bill_no']))
										print $_GET['bill_no']; ?>,this.value)">
										<?php
										if (isset($sm_id)) {
											for ($i = 0; $i < sizeof($sm_id); $i++) {
												if ($sm_id[$i] == $salesman_id)
													$select = 'selected="selected"';
												else
													$select = '';
					...
					<!-- Recover Agent -->
					<?php if (isset($hire_purchase) && $hire_purchase) { ?>
						<tr>
							<td width="50px"></td>
					...
					<tr>
						...
						<td colspan="2">
							<div class="frmSearch">
								<input type="text" id="search-code" value="" onclick="this.value=''" placeholder="Item Code"
									autocomplete="off" />
					...
					<tr>
						...
						<td colspan="2">
							<div class="frmSearch">
								<input type="text" id="search-desc" value="" onclick="this.value=''" placeholder="Item Description"
									autocomplete="off" />
					...
					<tr>
								...
								<tr>
									...
									<td>
										<div id="discount_div" onclick="changeDiscount()" style="color:blue; cursor:pointer;">
											<?php if (isset($default_discount_type) && $default_discount_type == 'price')
												print $currency;
											else
												print '%'; ?>
										</div>
						...
						<td colspan="2">
							<table width="100%">
								<tr>
									<td><input type="number" name="discount" id="discount" style="width:50px" onclick="this.value=''" />
									</td>
									<td>
										<div id="discount_btn"></div>
									</td>
									<td><input type="button" value="Check" onclick="loadDiscount('<?php if (isset($cu_id))
										print $cu_id; ?>')" /></td>
									<td>
					...
					<?php if (($systemid == 13) && ($components != 'to')) { ?>
						...
						<tr>
							<td width="50px"></td>
							<td style=" font-size:12pt">Invoice Discount (<?php if (isset($currency))
								print $currency; ?>)</td>
							<td colspan="2">
								<table width="100%" style=" font-size:12pt">
									<tr>
										<td>
											<input type="number" name="discount_for_invoice" id="discount_for_invoice" style="width:70px" value="<?php if (isset($invoice_discount))
												print $invoice_discount; ?>" />
										</td>
										<td>
											<input type="Button" value="Update" onclick="updateInvoiceDiscount(<?php if (isset($_GET['bill_no']))
												print $_GET['bill_no']; ?>)" id="invoice_discount_btn" />
										</td>
									</tr>
								</table>
							</td>
						...
						<tr>
							<td width="50px"></td>
							<td style=" font-size:12pt">Due Date</td>
							<td colspan="2">
								<table width="100%" style=" font-size:12pt">
									<tr>
										<td>
											<input type="date" name="due_date" id="due_date" style="width:120px" value="<?php if (isset($due_date))
												print $due_date; ?>" />
										</td>
										<td>
											<input type="Button" value="Update" onclick="updateDueDate(<?php if (isset($_GET['bill_no'])) {
												print $_GET['bill_no'];
											} ?>)" id="due_date_btn" />
										</td>
									</tr>
					...
					<tr>
						...
						<td colspan="2" align="center">
							<br />
							<div id="div_submit">
								<?php if (isset($bill_item_count) && $bill_item_count > 0)
									print '<input type="button" value="To Payment" onclick="proceedToPayment()" style="width:100px; height:50px;" />'; ?>
							</div>
						</td>
						<td width="50px"></td>
					</tr>
			...
			<table align="center" style="font-size:12pt" width="100%">
				<tr style="height:35px">
					...
					<th bgcolor="#C5C5C5"
						title="Calculation of 'Up to Now'&#13;Invoice Total - Cash Payments - All Cheque Payments">Up to
						Now</th>
				</tr>
				<tr>
				...
				<tr>
					...
					<td bgcolor="#E5E5E5" align="right">
						<?php if (isset($remaining_cr_limit))
							print number_format($remaining_cr_limit, $decimal); ?>
					</td>
				</tr>
			</table>
		</td>
		...
		<td valign="top">
			<div style="background-color:#D1D8F1; border-radius: 5px; padding-left:20px; padding-right:20px; height:600px;">
						...
						<td>
							<div id="printorder_h">
								<div style="font-weight:bold; font-size:12pt; text-align:left;">
									Temp Bill No:
									&nbsp;&nbsp;&nbsp;T<?php if (isset($_GET['bill_no']))
										print str_pad($_GET['bill_no'], 7, "0", STR_PAD_LEFT); ?>
								</div>
							</div>
						</td>
						<td width="20px"></td>
						<td><input type="button" value="Print Order" onclick="printdiv('printorder_c','printorder_h')" /></td>
						<td><input type="button" value="Delete Bill" onclick="deleteTMPBill()"
								style="background-color:maroon; color:white" /></td>
						<td>
				...
				<?php
				if (isset($bill_item_count) && $bill_item_count == 0) {
					print '<br /><hr /><br />';
					...
					<table width="100%" style="font-size:10pt">
						<tr bgcolor="#F5F5FF">
							...
							<td align="right" style="padding-left:10px; padding-right:10px; color:red">
								<?php if (isset($chq2_retuned))
									print $chq2_retuned; ?>
							</td>
						</tr>
						<tr bgcolor="#F5F5FF">
							<td>Postponed Cheques</td>
							<td align="right" style="padding-left:10px; padding-right:10px; color:red">
								<?php if (isset($chq2_postpone))
									print $chq2_postpone; ?>
							</td>
						</tr>
						<tr bgcolor="#F5F5FF">
							<td>Deposited Cheques</td>
							<td align="right" style="padding-left:10px; padding-right:10px; color:blue">
								<?php if (isset($chq2_banked))
									print $chq2_banked; ?>
							</td>
						</tr>
					</table>
...
<script type="text/javascript">
	getBillItems();
	document.getElementById('search-sn').addEventListener('keydown', function (event) {
		if (event.key === 'Enter') {  // Check if the key pressed is Enter
			event.preventDefault();  // Prevent the default action of the Enter key
			var val = this.value;  // Get the value of the input field
			// Check if the value is empty
			if (!val) {
				alert('Input value is empty.');
				return;  // Exit the function if the value is empty
			}
			// Call the selectSN function with the value
			selectSN(val);
