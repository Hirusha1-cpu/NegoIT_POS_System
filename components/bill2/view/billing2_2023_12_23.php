	...
	function addToBill(){
		...
		if($itm_unic=='1'){
			...
			for($i=0;$i<$items.length;$i++){
				...
				if($discount_type=='percentage') $discount=((($price.slice(2))/100)*($discount)).toFixed(<?php print decimal_paces(); ?>);
			...
			if($discount_type=='percentage') $discount=((($uprice.slice(2))/100)*($discount)).toFixed(<?php print decimal_paces(); ?>);
		...
		<td valign="top">
			...
			<?php if($current_district!=''){ ?>
				<table align="center" bgcolor="<?php print $main_tale_color; ?>" style="border-radius: 15px;" border="0">
					...
					print '</td><td width="50px"></td></tr>';
					print '<tr><td width="50px"></td><td style="font-size:12pt">Ref</td><td colspan="2" style="font-size:12pt; color:maroon">'.$cu_asso_sman.'</td><td width="50px"></td></tr>';
					?>
					<!-- Salesman -->
					<tr>
						<td width="50px"></td>
						...
						<td style="font-size:12pt">
							<table border="0">
								<tr>
									<td>Unit Discount</td>
									<td>
										<div id="discount_div" onclick="changeDiscount()" style="color:blue; cursor:pointer;"><?php if($default_discount_type=='price') print 'Rs'; else print '%'; ?></div>
									</td>
								</tr>
							</table>
						</td>
