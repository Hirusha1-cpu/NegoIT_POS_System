...
switch ($_REQUEST['action']) {
   ...
   case "home":
      include_once  'components/bill2/modle/bill2Module.php';
      getDistrict();
      if ($systemid == 1 || $systemid == 4 || $systemid == 10 || $systemid == 15) getSalesman2();
      if (isMobile())
   ...
   case "bill_item":
      ...
      getReturnChque('all');
      getTechnicient();
      if($systemid == 1 || $systemid == 4 || $systemid == 10 || $systemid == 13 || $systemid == 15 || $systemid == 20) getSalesman3($sub_system);
      if($systemid ==10| $systemid ==15 || $systemid == 20) getRecoveryAgent($sub_system);
      if(isMobile())
   ...
   case "home2":
      ...
      if (isset($_COOKIE['district'])) {
         ...
         if (isset($_GET['cust'])) {
            if (validateBillNo()) header('Location: index.php?components=bill2&action=new_bill&cust_odr=' . $_REQUEST['cust_odr'] . '&salesman=' . $_GET['s'] . '&cust_id=' . $_GET['cust'] . '&quotation=0');
      ...
      if ($systemid == 1 || $systemid == 4 || $systemid == 10) getSalesman2();
      billTemplate();
      getReturnChque('all');
      getTechnicient();
  ...
  case "wholesale_cust":
   ...
   case "add_cust_image1":
      ...
      if (addCustImage($systemid))
          ...
          header('Location: index.php?components=bill2&action=wholesale_cust&s='.$_GET['s'].'&cust_odr='.$_GET['cust_odr'].'&message='.$message.'&re=fail');
      break;
   ...
   case "pay_bill":
      ...
      getBank();
      getBillMain($_GET['bill_no']);
      if($bm_type == 3) $remaining_cr_limit = getCreditStatus2($cu_id);
      else $remaining_cr_limit = getCreditStatus2($cu_id) - $bill_total;
   ...
   case "add_payment":
      ...
      if (addPayment('pay')) {
         if ($bm_type == 3)
            ...
            header('Location: index.php?components=bill2&action=finish_payment&id=' . $payment_id);
      } else
...
*/
   ...
   case "finalize_return":
      include_once  'components/bill2/modle/bill2Module.php';
      if (finalizeReturn())
         header('Location: index.php?components=bill2&action=finish_return&id=' . $_GET['id']);
      else
   ...
   case "quotation":
      ...
      if (isset($_COOKIE['district'])) {
         ...
         getCust(1);
         if (isset($_GET['cust'])) {
            if (validateQuotNo()) header('Location: index.php?components=bill2&action=new_quot&cust_id=' . $_GET['cust'] . '&att=');
         }
      }
      if(isMobile())
