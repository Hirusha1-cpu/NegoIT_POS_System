...
switch ($_REQUEST['action']) {
   ...
   case "bill_item":
      ...
      getCreditStatus2($cu_id);
      getCustomerCreditLimit($cu_id);
      getCustomerPendingChequeAmount($cu_id);
      getCreditData($sub_system, $cu_id);
      getReturnChque('all');
      getTechnicient();
   ...
   case "home2":
      ...
      if (isset($_COOKIE['district'])) {
         ...
         if (isset($_GET['cust'])) {
            ...
            getCreditData($sub_system, $_GET['cust']);
            getCustomerCreditLimit($cu_id);
            getCustomerPendingChequeAmount($cu_id);
         } else {
   ...
   case "payment_home":
      ...
      if (isset($_GET['cust'])) {
         ...
         // getCreditStatus($sub_system, $_GET['cust']);
         getCreditData($sub_system, $_GET['cust']);
         getCustomerCreditLimit($_GET['cust']);
         getCustomerPendingChequeAmount($_GET['cust']);
      }
      getBank();
      if ($_COOKIE['retail'] == 0) {
         getCust(1);
   ...
   case "payment_form":
      ...
      if (isset($_GET['cust'])) {
         getCustPayments();
         getCreditData($sub_system, $_GET['cust']);
         getCreditStatus2($_GET['cust']);
         getCustomerCreditLimit($_GET['cust']);
         getCustomerPendingChequeAmount($_GET['cust']);
         getCustQuotationNumbers($_GET['cust']);
      }
      getBank();
      getChequeNames();
      if ($_COOKIE['retail'] == 0)
   ...
   case "add_payment":
      ...
      if (addPayment('pay')) {
         if ($bm_type == 3)
            header('Location: index.php?components=bill2&action=finish_bill&id=' . $invoice_no . '&message=' . urlencode($message) . '&re=success');
         else
            header('Location: index.php?components=bill2&action=finish_payment&id=' . $payment_id . '&message=' . urlencode($message) . '&re=success');
      } else {
         header('Location: index.php?components=bill2&action=payment_form&cust=' . $cust . '&message=' . urlencode($message) . '&re=fail');
      }
      break;
   ...
   //--------------------------------------Item Return----------------------------//
   case "select_cust_item_return":
      include_once 'components/bill2/modle/bill2Module.php';
      if (isMobile())
         include_once 'components/bill2/view/m_select_cust_item_return.php';
      else
         include_once 'components/bill2/view/select_cust_item_return.php';
      break;
   ...
   case "apend_return":
      ...
      if (apendReturn()) {
         ...
         header('Location: index.php?components=bill2&action=item_return&id=' . $return_invoice_no . '&cust=' . $cust . '&message=' . urlencode($message) . '&re=success');
      } else {
         debugEnd($debug_id, 'fail');
         header('Location: index.php?components=bill2&action=item_return&id=' . $return_invoice_no . '&cust=' . $cust . '&message=' . urlencode($message) . '&re=fail');
      }
      break;
   ...
   case "search_return":
      ...
      if (searchReturn($_POST['search1']))
         ...
         header('Location: index.php?components=bill2&action=select_cust_item_return');
      break;
