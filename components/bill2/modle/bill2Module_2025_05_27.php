...
function addToBill($sub_system)
{
	...
	// ------------------------------------------------------------
	// Mixed Quantity Rules Implementation
	// Requirements:
	// 1. When the bill is empty, any new entry is allowed.
	// 2. Once at least one line exists:
	//    (a) If the first entry is positive the bill is in "positive mode":
	//         • Any positive entry from any item is allowed.
	//         • A negative entry is allowed ONLY if it cancels (i.e. exactly
	//           equals in absolute value) the aggregated positive quantity for
	//           that same item.
	//    (b) If the first entry is negative the bill is in "negative mode":
	//         • Only negative entries for the same item are allowed.
	//         • A positive entry is allowed ONLY if it exactly cancels the aggregated
	//           negative quantity for that same item.
	// 3. Cross–item check: if an entry is being added with a sign opposite
	//    to an entry from another item, then block.
	// ------------------------------------------------------------

	/*
																													if ($out && isQuickBooksActive(1)) {
																														$newQty = (float) $qty;          // Quantity the user is trying to add.
																														$itemId = (int) $itm_id;         // The item currently being added.

																														// -------------------------------------------
																														// 1) Determine the overall mode of the bill.
																														// -------------------------------------------
																														$sqlMode = "SELECT
																																				SUM(CASE WHEN qty > 0 THEN 1 ELSE 0 END) AS countPos,
																																				SUM(CASE WHEN qty < 0 THEN 1 ELSE 0 END) AS countNeg
																																			FROM bill_tmp
																																			WHERE bm_no='$bm_no'";
																														$resMode = mysqli_query($conn, $sqlMode);
																														if (!$resMode) {
																															$out = false;
																															$message = "Error: Failed to determine overall mode.";
																														} else {
																															$rowMode = mysqli_fetch_assoc($resMode);
																															$countPos = (int) $rowMode['countPos'];
																															$countNeg = (int) $rowMode['countNeg'];

																															if ($countPos > 0 && $countNeg == 0) {
																																$mode = "pos";   // Bill is in positive mode.
																															} elseif ($countNeg > 0 && $countPos == 0) {
																																$mode = "neg";   // Bill is in negative mode.
																															} elseif ($countPos == 0 && $countNeg == 0) {
																																$mode = null;    // Bill is empty; no mode set yet.
																															} else {
																																// Both positives and negatives exist (should not happen per rules).
																																$out = false;
																																$message = "Error: Cannot mix positive and negative entries.";
																															}
																														}

																														// -------------------------------------------
																														// 2) Cross–Item Check for Opposite Signs
																														// -------------------------------------------
																														if ($out) {
																															$sqlCross = "SELECT item, qty FROM bill_tmp WHERE bm_no='$bm_no'";
																															$resCross = mysqli_query($conn, $sqlCross);
																															if (!$resCross) {
																																$out = false;
																																$message = "Error: Failed to read existing bill lines.";
																															} else {
																																while ($row = mysqli_fetch_assoc($resCross)) {
																																	$existingId = (int) $row['item'];
																																	$existingQty = (float) $row['qty'];
																																	// Only check for items different from the current one.
																																	if ($existingId !== $itemId) {
																																		// If attempting to mix opposite-signed entries on different items, block.
																																		if (
																																			($newQty > 0 && $existingQty < 0) ||
																																			($newQty < 0 && $existingQty > 0)
																																		) {
																																			$out = false;
																																			$message = sprintf(
																																				"Error: Cannot mix opposite-signed quantities across different items. " .
																																				"Item #%d has %+.2f; you tried %+.2f on item #%d.",
																																				$existingId,
																																				$existingQty,
																																				$newQty,
																																				$itemId
																														...
																														// -------------------------------------------
																														// 3) Enforce Mode-Based Rules for the Same Item.
																														// -------------------------------------------
																														if ($out) {
																															// If the bill is empty, no mode is set—allow any entry.
																															if ($mode === null) {
																															...
																															elseif ($mode === "pos") {
																																if ($newQty >= 0) {
																																	// All positive entries are allowed in positive mode.
																																} else { // $newQty < 0
																																	// For a negative entry, the sum of the positive quantities for this item must exist.
																																	$sqlItem = "SELECT SUM(qty) AS sumForItem
																																									FROM bill_tmp
																																									WHERE bm_no='$bm_no' AND item='$itemId'";
																																	$resItem = mysqli_query($conn, $sqlItem);
																																	if (!$resItem) {
																																		$out = false;
																																		$message = "Error: Could not retrieve data for item #$itemId.";
																																	} else {
																																		$rowItem = mysqli_fetch_assoc($resItem);
																																		$sumForItem = (float) $rowItem['sumForItem'];
																																		if ($sumForItem <= 0) {
																																			// There is no positive total for this item.
																																			$out = false;
																																			$message = "Error: Cannot add a negative quantity for item #$itemId because no positive entry exists for it.";
																																		} elseif ($sumForItem == 0) {
																																			// Already balanced.
																																			$out = false;
																																			$message = "Error: Item #$itemId is already balanced; no further pairing is allowed.";
																																		} else {
																																			// The new negative entry must exactly cancel the positive aggregate.
																																			if (abs($newQty) !== $sumForItem) {
																																				$out = false;
																																				$message = sprintf(
																																					"Error: To cancel item #%d, you must add exactly %+.2f (current positive total is %+.2f).",
																																					$itemId,
																																					-$sumForItem,
																																					$sumForItem
																																				);
																															...
																															elseif ($mode === "neg") {
																																if ($newQty <= 0) {
																																	// Negative entries in negative mode: they must be for the same item.
																																	$sqlItem = "SELECT SUM(qty) AS sumForItem
																																									FROM bill_tmp
																																									WHERE bm_no='$bm_no' AND item='$itemId'";
																																	$resItem = mysqli_query($conn, $sqlItem);
																																	if (!$resItem) {
																																		...
																																		$sumForItem = (float) $rowItem['sumForItem'];
																																		if ($sumForItem == 0) {
																																			// The negative mode was set by another item.
																																			$out = false;
																																			$message = "Error: Cannot add a negative quantity for item #$itemId because it is not the item already in negative mode.";
																																		}
																																		// Otherwise, additional negative entries for the same item are allowed.
																																	}
																																} else { // $newQty > 0
																																	// A positive entry in negative mode must exactly cancel the negative total for this item.
																																	$sqlItem = "SELECT SUM(qty) AS sumForItem
																																									FROM bill_tmp
																																									WHERE bm_no='$bm_no' AND item='$itemId'";
																																	$resItem = mysqli_query($conn, $sqlItem);
																																	if (!$resItem) {
																																		...
																																		$rowItem = mysqli_fetch_assoc($resItem);
																																		$sumForItem = (float) $rowItem['sumForItem']; // This value is negative.
																																		if ($sumForItem >= 0) {
																																			$out = false;
																																			$message = "Error: Cannot add a positive quantity for item #$itemId because no negative entry exists for it.";
																																		} elseif ($sumForItem == 0) {
																																			$out = false;
																																			$message = "Error: Item #$itemId is already balanced; no further pairing is allowed.";
																																		} else {
																																			// Enforce that the positive quantity exactly cancels the negative total.
																																			if ($newQty !== abs($sumForItem)) {
																																				$out = false;
																																				$message = sprintf(
																																					"Error: To cancel item #%d, you must add exactly %+.2f (current negative total is %+.2f).",
																																					$itemId,
																																					abs($sumForItem),
																																					$sumForItem
																																				);
																														...
																														// -------------------------------
																														// 4) Final guard: Ensure that $qty is set.
																														// -------------------------------
																														if ($out && !isset($qty)) {
																															$out = false;
...
function createInvoice($sub_system)
{
	...
	if ($out) {
		try {
			...
			if ($out) {
				...
				while ($row = mysqli_fetch_array($result)) {
					...
					if ($out && $inventory_deduct) {
						if ($new_inv_ref == 0) {
							if (!($bm_type == 4 && $qty < 0)) {
								...
								if (!$result_update_qty || mysqli_affected_rows($conn) == 0) {
							...
							if (!($bm_type == 4 && $qty < 0)) {
								...
								if (!$result_update_new || mysqli_affected_rows($conn) == 0) {
									$out = false;
									$msg = 'Error: Failed to update the new inventory';
					...
					if ($out && ($itm_unic == 1)) {
						...
						if (!$result_update_unic || mysqli_affected_rows($conn) == 0) {
							$out = false;
							$msg = 'Error: Failed to update the unique items';
							throw new Exception($msg);
						}
			...
			if ($out) {
				if (isQuickBooksActive(1)) {
					$invoiceCostQuery = "SELECT SUM(b.qty * b.cost), c.name, bm.`type` FROM bill_main bm, bill b, cust c
					...
					if (!$invoiceCostRow) { // Check if fetch failed or query returned no rows
						throw new Exception("Error fetching invoice cost/type details for QB. Details: " . mysqli_error($conn));
					}
					$invoiceCost = $invoiceCostRow[0];
					$custName = $invoiceCostRow[1];
					$bill_type = $invoiceCostRow[2];
					...
					// NORMAL BILL (SALES BILL)
					if ($bill_type == 1) {
						if ($total <= 0) { // cash back invoice
							if ($total != 0) {
					...
					if ($bill_type == 4) {
						...
						$query_update_qb = "UPDATE bill_main SET qb_status = 0, qb_temp_cost = '$invoiceCost' WHERE invoice_no = '$invoice_no'";
	...
	try {
		...
		if ($proceed) {
			...
			if ($proceed && isQuickBooksActive(1)) {
				...
				$invoiceQuery = "SELECT SUM(b.`qty` * b.`cost`) as cost, SUM(bm.`invoice_+total`+bm.`invoice_-total`) as total, bm.`qb_value_id`, bm.`qb_cost_id`,
					c.`name`, c.`qb_cust_id`,bm.`status`
	...
	try {
		...
		if ($out) {
			if (isQuickBooksActive(1)) {
				...
				/*
																																																																																																																																																																																																																																														// Add input validation at the start
																																																																																																																																																																																																																																														if (!is_numeric($bill_total) || !is_numeric($invoiceCost)) {
																																																																																																																																																																																																																																															throw new Exception("Invalid amount values");
																																																																																																																																																																																																																																														}
																																																																																																																																																																																																																																														...
																																																																																																																																																																																																																																														if ($bill_type == 1) {
																																																																																																																																																																																																																																															if ($bill_total_minus <> 0) { // cash back invoice
																																																																																																																																																																																																																																																...
																																																																																																																																																																																																																																																$journalEntryForInvoiceTotal = buildJournalEntry($conn, abs($bill_total_minus), $debitAccountName, $creditAccountName, $description, $debitEntityType, $debitEntityID, $creditEntityType, $creditEntityID);
																																																																																																																																																																																																																																																if (isset($journalEntryForInvoiceTotal['error'])) {
																																																																																																																																																																																																																																																	$qb_msg = $journalEntryForInvoiceTotal['error'];
																																																																																																																																																																																																																																																	throw new Exception("QuickBooks error: " . $qb_msg);
																																																																																																																																																																																																																																																} else {
																																																																																																																																																																																																																																																...
																																																																																																																																																																																																																																																if (isset($journalEntryForInvoiceTotal['error'])) {
																																																																																																																																																																																																																																																	...
																																																																																																																																																																																																																																																	$batch_id = generateBatchID();
																																																																																																																																																																																																																																																	$action_name = 'normal_invoice_total_insert'; // corrected spelling
																																																																																																																																																																																																																																																	foreach ($journalEntryForInvoiceTotal as $entry) {
																																																																																																																																																																																																																																																		$posting_type = mysqli_real_escape_string($conn, $entry['posting_type']);
																																																																																																																																																																																																																																																		$account_id = mysqli_real_escape_string($conn, $entry['account_id']);
																																																																																																																																																																																																																																																		$account_name = mysqli_real_escape_string($conn, $entry['account_name']);
																																																																																																																																																																																																																																														...
																																																																																																																																																																																																																																														// CUST ODERS temp cost (becuase they can add more items to bill later in odr)
																																																																																																																																																																																																																																														if ($bill_type == 4) {
																																																																																																																																																																																																																																															if ($bill_total_minus <> 0) { // cash back invoice
																																																																																																																																																																																																																																																// invoice cost journal entry
																																																																																																																																																																																																																																																// $debitAccountName = "Inventory Asset";
																																																																																																																																																																																																																																																// $creditAccountName = "Cost of Goods Sold";
																																																																																																																																																																																																																																																// $description = "[TEMPORARY CASH BACK INVOICE] - Invoice No: $invoice_no, Customer : $custName";
																																																																																																																																																																																																																																																// $debitEntityType = "";
																																																																																																																																																																																																																																																// $debitEntityID = "";
																																																																																																																																																																																																																																																// $creditEntityType = "";
																																																																																																																																																																																																																																																// $creditEntityID = "";
																																																																																																																																																																																																																																															...
																																																																																																																																																																																																																																															// Update bill_main table for indetifying bill made after qb activated
																																																																																																																																																																																																																																															$query1 = "UPDATE bill_main SET qb_status = 0 WHERE invoice_no = '$invoice_no'";
																																																																																																																																																																																																																																															$result1 = mysqli_query($conn, $query1);
																																																																																																																																																																																																																																															if (!$result1) {
																																																																																																																																																																																																																																																$out = false;
																																																																																																																																																																																																																																															...
																																																																																																																																																																																																																																															// Update bill_main table for indetifying bill made after qb activated
																																																																																																																																																																																																																																															$query1 = "UPDATE bill_main SET qb_temp_cost = '$invoiceCost' WHERE invoice_no = '$invoice_no'";
																																																																																																																																																																																																																																															$result1 = mysqli_query($conn, $query1);
																																																																																																																																																																																																																																															if (!$result1) {
																																																																																																																																																																																																																																																$out = false;
																																																																																																																																																																																																																																																$msg = 'Error: failed to update qb status in bill main';
			...
			if ($emptyamount) {
				...
				if ((($payment_type == 2) || ($payment_type == 3)) && ($proceed)) {
					...
					if ($systemid == 1 || $systemid == 4 || $systemid == 17) {
						...
						$query = "SELECT COUNT(id) as payment_count FROM payment WHERE chque_bank='$chque_bank_id' AND chque_no='$chque_no' AND chque_branch='$chque_branch'";
						$row = mysqli_fetch_row(mysqli_query($conn, $query));
						$payment_count = $row[0];
