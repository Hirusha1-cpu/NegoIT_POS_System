...
// update by nirmal 02_08_2023 round and add decimal number to wholesale and retail dropdwon
				...
				if (($itm_unic == 1) && ($unic_cal)) {
					...
					if ($bi_qty > 0) {
						...
						if (($row1[0] !== null) && ($row1[0] >= -$bi_qty)) {
							...
							$out = false;
							$main_msg = $msg = 'Error: This Customer Did Not Purchase This Item OR Purchased Less Quantity';
						}
					}
				}
			}
...
function addToBill($sub_system)
{
	...
	// not allow add cashback invoice to normal
	if ($out && isQuickBooksActive(1)) {
		$firstItemQty = null; // To track the first item's quantity

		// Query to fetch existing items in the invoice
		$query = "SELECT qty FROM bill_tmp WHERE bm_no='$bm_no'";
		$result = mysqli_query($conn, $query);
		...
		if ($result && mysqli_num_rows($result) > 0) {
			...
			if (isset($row['qty'])) {
				$firstItemQty = (float) $row['qty']; // Set the first item's quantity
			}
		}

		// Assuming $qty is the new item's quantity (provided as input)
		if (isset($qty)) {
			$newItemQty = (float) $qty;

			// Enforce rules for subsequent items
			if ($firstItemQty !== null) {
				// Check if the new item violates the rules
				if (($firstItemQty > 0 && $newItemQty < 0) || ($firstItemQty < 0 && $newItemQty > 0)) {
					$out = false;
					$message = "Error: Positive and negative quantity items cannot be added together.";
				}
			}
		} else {
			$out = false;
			$message = "Error: New item quantity is not defined.";
		}
	}
...
function newTMPBill($sub_system, $cust_odr, $bill_sm, $qo_no)
{
	...
	if ($out) {
		...
		if (!$result3) {
			$out = false;
			$message = $query; /* $message='Error: Bill Could Not be Created!h'; */
		}
...
function getBillMain($bill_no)
{
	...
	$query = "SELECT btm.`type`, cu.`id`, cu.`name`, cu.`nic`, cu.`mobile`, btm.`billed_by`, btm.`recovery_agent`, user1.`username`, cu.`gps_x`, cu.`gps_y`, btm.`discount`,
	btm.`due_date`, user.`username`
	FROM bill_main_tmp AS btm
	INNER JOIN cust AS cu ON btm.`cust` = cu.id
	INNER JOIN userprofile AS user ON btm.`billed_by` = user.`id`
	INNER JOIN userprofile AS user1 ON cu.`associated_salesman` = user1.`id`
	WHERE btm.`status` != 0 AND btm.bm_no='$bill_no'";
	$row = mysqli_fetch_row(mysqli_query($conn, $query));
	$bm_type = $row[0];
...
function createInvoice($sub_system)
{
	...
	if ($out) {
		...
		if (!$result) {
			$out = false;
			$msg = 'Failed to Create the Invoice';
		}
	}
	if ($out) {
		if ($is_custom_invoice_no_active) {
		...
		if ($invoice_no == '') {
			$out = false;
			$msg = 'Invoice Number Error';
		}
	}
	if ($out) {
		$query = "SELECT bi.item,bi.qty,bi.unit_price,bi.discount,bi.`comment`,bi.repair_model,bi.repair_sn,bi.`date`,bi.new_inv_ref,bi.gift,itm.unic,itm.pr_sr,itm.default_cost,itm.category FROM bill_tmp bi, inventory_items itm WHERE bi.item=itm.id AND bi.bm_no='$bm_no'";
		$result = mysqli_query($conn, $query);
		while ($row = mysqli_fetch_array($result)) {
			if ($out) {
				$item = $row[0];
				...
				if ($out) {
					$query1 = "INSERT INTO `bill` (`invoice_no`,`item`,`qty`,`unit_price`,`cost`,`discount`,`w_price`,`r_price`,`comment`,`repair_model`,`repair_sn`,`date`,`gift`,`return_odr_replace`,`cross_trans`,`no_update`)
					...
					if (!$result1) {
						$out = false;
						$msg = 'Failed to Add Item to Invoice';
					}
	...
	if (inf_systemid(1) == 24) {
		if ($out) {
			$query = "UPDATE bill_main SET `items_category` = '$item_category' WHERE `invoice_no`='$invoice_no'";
			$result = mysqli_query($conn, $query);
			if (!$result) {
				$out = false;
				$msg = 'Error: Failed to update the bill category';
			}
		}
	}
	if ($out) {
...
function createInvoice($sub_system)
{
	...
	if ($out) {
		...
		if (!$result) {
			$out = false;
			$msg = 'Error: Failed to create the invoice';
		}
	}
	if ($out) {
		if ($is_custom_invoice_no_active) {
		...
		if ($invoice_no == '') {
			$out = false;
			$msg = 'Error: Invoice number selecting error';
		}
	}
	if ($out) {
...
function generateInvoice($order_by)
{
	...
	$cust_tax_no, $sub_total, $credit_balance, $logo, $bm_discount;
...
function generateReturnList()
{
	...
	$query = "SELECT DISTINCT itm.code,itm.description,rt.qty,itm.id,rm.invoice_no,rt.id FROM return_main rm, `return` rt, inventory_items itm, inventory_qty itq WHERE rm.invoice_no=rt.invoice_no AND rt.replace_item=itm.id AND itm.id=itq.item AND rm.`status`='2' AND rt.odr_packed='0' AND rm.`cust`='$rm_cust' ORDER BY $pending_return_items_order_by";
	$result = mysqli_query($conn, $query);
	while ($row = mysqli_fetch_array($result)) {
...
function deleteTmpBill()
{
	...
	if ($count == 0) {
		$proceed = false;
		$message = 'You dont have Permission to Delete this TMP Bill';
	}
	if ($proceed) {
		$query = "DELETE FROM `bill_tmp` WHERE `bm_no` = '$bm_no'";
		$result = mysqli_query($conn, $query);
...
function deleteInvoice($n, $force)
{
	...
	try {
		...
		if ($proceed) {
			...
			$query0 = "SELECT ivq.id,bi.qty,bi.id,(bi.unit_price+bi.discount),bi.cost,bi.w_price,bi.r_price,ivq.w_price,ivq.r_price,ivq.c_price,ivq.item,bi.no_update,ivq.location,itm.pr_sr,itm.unic,bm.`cust`
			FROM bill_main bm, bill bi, inventory_items itm, inventory_qty ivq
			WHERE bm.invoice_no=bi.invoice_no AND bi.item=itm.id AND ivq.item=bi.item AND ivq.location='$store' AND bi.`invoice_no`='$invoice_no' ORDER BY bi.id DESC";
			...
			if ($proceed) {
				...
				if (!$result) {
					$proceed = false;
					$message = 'Error: failed to change status of the invoice';
					throw new Exception($message);
				}
			}
			if ($proceed) {
				...
				if (!$result) {
					$proceed = false;
					$message = 'Error: Failed to delete the payment';
					throw new Exception($message);
				}
			}
			...
			if ($proceed) {
				...
				if (!$result) {
					$proceed = false;
					$message = 'Error: Failed to remove packed return items';
					throw new Exception($message);
				}
			}
...
function deleteInvoice($n, $force)
{
	...
	$query = "SELECT bi.`qty`, itm.`unic`, bm.`mapped_inventory` FROM bill_main bm, bill bi, inventory_items itm WHERE bm.`invoice_no`=bi.`invoice_no` AND bi.`item`=itm.`id` AND bm.`invoice_no`='$invoice_no'";
	$result = mysqli_query($conn, $query);
	while ($row = mysqli_fetch_array($result)) {
		$store = $row[2];
		if (($row[0] < 0) && ($row[1] == 1) && (!$unic_cal)) {
		...
		if ($proceed) {
			...
			if (!$resultReturnItems) {
				...
				while ($returnItem = mysqli_fetch_array($resultReturnItems)) {
					...
					$replace_item = $returnItem['replace_item'];
					$qty = $returnItem['total_qty'];

					if ($itm_pr_sr == 1) {
						if ((($itemUniqueStatus == 1) && (!$unic_cal)) || ($itemUniqueStatus == 0)) {
							$query = "SELECT COUNT(*) AS item_count FROM bill WHERE invoice_no = '$invoice_no' AND item = '$replace_item' AND qty < 0";
							$result = mysqli_query($conn, $query);
							if (!$result) {
								$message = 'Error: query error';
								$proceed = false;
								...
								$row = mysqli_fetch_assoc($result);
								$itemExists = $row['item_count'] > 0;

								if (!$itemExists) {
									$querySelectInventoryQty = "SELECT id FROM inventory_qty WHERE item='$replace_item' AND `location`='$store'";
									...
									$queryUpdateInventoryQty = "UPDATE inventory_qty SET qty = qty + $replaceItemQty WHERE id='$inventoryQtyId'";
	...
	if (isQuickBooksActive(1)) {
		if ($proceed) {
			...
			$journalEntryForDeletedPayment = $journal_payment_entry_result = [];
			$paymentQuery = "SELECT p.`id`, p.`payment_type`, p.`qb_id`, c.`name`, c.`qb_cust_id`, p.`amount`, p.`chque_clear`, p.`chque_return`, p.`chque_deposit_bank`, p.`bank_trans` FROM `payment` p, `cust` c WHERE c.`id` = p.`cust` AND p.`bill_pay`=1 AND p.`qb_id`!=0 AND p.`invoice_no`='$invoice_no'";
			...
			if ($paymentResult && mysqli_num_rows($paymentResult) > 0) {
				while ($paymentRow = mysqli_fetch_assoc($paymentResult)) {
					...
					$qb_status = $paymentRow['qb_status']; // status has to be 0:pending, 1:sent or 2:could not sent
					...
					if (($paymentRow['payment_type'] == 1) && ($qb_status !== null && $qb_status != '')) { // Cash
						...
						if (isset($journalEntryForDeletedPayment['error'])) {
							...
							foreach ($journalEntryForDeletedPayment as $entry) {
								...
								$query = "INSERT INTO qb_queue (`batch_id`, `action`, `payment_id`, `invoice_no`, `posting_type`, `account_id`, `account_name`, `invoice_total`, `description`, `entity_type`, `entity_id`)
									VALUES ('$batch_id','$action_name', '$payment_id', '$invoice_no', '$posting_type', '$account_id', '$account_name', '$amount', '$description',
						...
						// 			$action = "UPDATE PAYMENT DELETED QB ID : Payment Id: $payment_id";
						...
						// 		$action = "INVOICE PAYMENT, DELETE JOURNAL ENTRY INSERT (CASH): Payment Id: $payment_id";
						// 		$functionName = "deleteInvoice";

						// 		$error_log_query="INSERT INTO `qb_debug_log` (`action`,`function`,`amount`,`debit_account_name`,`credit_account_name`,`journal_entry_description`,`debit_entity_type`,`debit_entity_id`,`credit_entity_type`,`credit_entity_id`,`qb_message`,`action_time`)
						// 		VALUES ('$action','$functionName','$amount','$debitAccountName','$creditAccountName','$description','$debitEntityType','$debitEntityID','$creditEntityType','$creditEntityID','$qb_msg','$time_now')";
						// 		$result=mysqli_query($conn,$error_log_query);
						// 		$flag = false;
						// 	}
						// } catch (Exception $e) {
					...
					if (($paymentRow['payment_type'] == 2) && ($qb_status !== null && $qb_status != '')) { // Cheuqe
						if ($paymentRow['chque_clear'] == 1 && $paymentRow['chque_return'] != 0) {
							...
							if ($accountNameResult && mysqli_num_rows($accountNameResult) > 0) {
								...
								// 		$qb_msg = mysqli_real_escape_string($conn, $qb_msg);
								// 		$description = mysqli_real_escape_string($conn, $description);
								// 		$action = "INVOICE PAYMENT, DELETE JOURNAL ENTRY INSERT (CHEQUE): Payment Id: $payment_id";
								// 		$functionName = "deleteInvoice";
					...
					if (($paymentRow['payment_type'] == 3) && ($qb_status !== null && $qb_status != '')) { // Bank
						...
						if ($accountNameResult && mysqli_num_rows($accountNameResult) > 0) {
							...
							// 		$qb_msg = mysqli_real_escape_string($conn, $qb_msg);
							// 		$description = mysqli_real_escape_string($conn, $description);
							// 		$action = "INVOICE PAYMENT, DELETE JOURNAL ENTRY INSERT (BANK): Payment Id: $payment_id";
							// 		$functionName = "deleteInvoice";
					...
					if (($paymentRow['payment_type'] == 4) && ($qb_status !== null && $qb_status != '')) { // Card
						$accountNameQuery = "SELECT `name` FROM accounts WHERE id='$bank_trans'";
						$accountNameResult = mysqli_query($conn, $accountNameQuery);
						if ($accountNameResult && mysqli_num_rows($accountNameResult) > 0) {
							$accountNameRow = mysqli_fetch_row($accountNameResult);
							...
							// 		$qb_msg = mysqli_real_escape_string($conn, $qb_msg);
							// 		$description = mysqli_real_escape_string($conn, $description);
							// 		$action = "INVOICE PAYMENT, DELETE JOURNAL ENTRY INSERT (CARD): Payment Id: $payment_id";
							// 		$functionName = "deleteInvoice";
			...
			// Delete associated invoice (if exists in QuickBooks)
			$journalEntryForDeleteInvoiceValue = $journalEntryForDeleteInvoiceCost = $journal_entry_result = $journal_entry_result1 = [];
			$invoiceQuery = "SELECT SUM(b.`qty` * b.`cost`) as cost, SUM(bm.`invoice_+total`+bm.`invoice_-total`) as total, bm.`qb_value_id`, bm.`qb_cost_id`, c.`name` FROM `bill_main` bm, bill b, `cust` c WHERE bm.`invoice_no` = b.`invoice_no` AND c.`id` = bm.`cust` AND bm.`invoice_no`='$invoice_no'";
			...
			if ($invoiceResult && mysqli_num_rows($invoiceResult) > 0) {
				...
				if (!empty($invoiceRow['qb_cost_id'])) {
					...
					$journalEntryForDeleteInvoiceCost = buildJournalEntry($conn2, abs($invoiceCost), $debitAccountName, $creditAccountName, $description, $debitEntityType, $debitEntityID, $creditEntityType, $creditEntityID);
					if (isset($journalEntryForDeleteInvoiceCost['error'])) {
						$flag = false;
						$qb_msg = $journalEntryForDeleteInvoiceCost['error'];

						$qb_msg = mysqli_real_escape_string($conn, $qb_msg);
						$action = "INVOICE DELETE, JOURNAL ENTRY BUILD (Invoice Cost) : Invoice No: $invoice_no";
						$functionName = "deleteInvoice";

						$error_log_query = "INSERT INTO `qb_debug_log` (`action`,`function`,`amount`,`qb_message`,`action_time`)
						VALUES ('$action','$functionName','$invoiceCost','$qb_msg','$time_now')";
						$result = mysqli_query($conn, $error_log_query);
					}

					try {
						$journal_entry_result = QBAddJournalEntry($journalEntryForDeleteInvoiceCost);
						...
						if ((isset($journal_entry_result['status'])) && ($journal_entry_result['status'] == 'success')) {
							...
							$query = "UPDATE `bill_main` SET `qb_deleted_cost_id`='$qb_journal_entry_id' WHERE `invoice_no`='$invoice_no'";
							$result1 = mysqli_query($conn, $query);
							if (!$result1) {
								$flag = false;
								$qb_msg = 'Quickbooks bill main qb_deleted_cost_id update error.';

								$qb_msg = mysqli_real_escape_string($conn, $qb_msg);
								$action = "UPDATE BILL MAIN INVOICE DELETED COST ID (Invoice Cost) : Invoice No: $invoice_no";
								$functionName = "addBillPayment";
				...
				if (!empty($invoiceRow['qb_value_id'])) {
					...
					$journalEntryForDeleteInvoiceValue = buildJournalEntry($conn2, abs($invoiceTotal), $debitAccountName, $creditAccountName, $description, $debitEntityType, $debitEntityID, $creditEntityType, $creditEntityID);
					if (isset($journalEntryForDeleteInvoiceValue['error'])) {
						$flag = false;
						$qb_msg = $journalEntryForDeleteInvoiceValue['error'];

						$qb_msg = mysqli_real_escape_string($conn, $qb_msg);
						$action = "INVOICE DELETE, JOURNAL ENTRY BUILD (Invoice Total) : Invoice No: $invoice_no";
						$functionName = "deleteInvoice";

						$error_log_query = "INSERT INTO `qb_debug_log` (`action`,`function`,`amount`,`qb_message`,`action_time`)
						VALUES ('$action','$functionName','$invoiceTotal','$qb_msg','$time_now')";
						$result = mysqli_query($conn, $error_log_query);
					}

					try {
						$journal_entry_result1 = QBAddJournalEntry($journalEntryForDeleteInvoiceValue);
						$qb_msg = $journal_entry_result1['message'];

						if ((isset($journal_entry_result1['status'])) && ($journal_entry_result1['status'] == 'success')) {
							$qb_journal_entry_id1 = $journal_entry_result1['qb_journal_entry_id'];
							$query = "UPDATE `bill_main` SET `qb_deleted_value_id`='$qb_journal_entry_id1' WHERE `invoice_no`='$invoice_no'";
							$result1 = mysqli_query($conn, $query);
							if (!$result1) {
								$flag = false;
								$qb_msg = 'Quickbooks bill main qb_deleted_value_id update error.';

								$qb_msg = mysqli_real_escape_string($conn, $qb_msg);
								$action = "UPDATE BILL MAIN INVOICE DELETED VALUE ID (Invoice Total) : Invoice No: $invoice_no";
								$functionName = "addBillPayment";
			...
			if (!empty($delete_messages)) {
				foreach ($delete_messages as $delete_message) {
					$message .= $delete_message . '<br>';
				}
			} else {
				$qb_msg = 'Invoices and payments deleted successfully from QuickBooks.';
			}
		}
	}
	return $proceed;
...
// Function to delete a QuickBooks journal entry and handle status updates
function deleteJournalEntryAndUpdate($conn, $qb_id, $table, $invoice_no, $field = 'qb_status')
{
	$qb_msg = '';
	try {
		...
		if ($qb_result['status'] == 'success') {
			...
			$query = "UPDATE `$table` SET `$field`=0 WHERE `invoice_no`='$invoice_no'";
			$result = mysqli_query($conn, $query);
			if (!$result) {
				return 'QuickBooks error: Status update failed for table: ' . $table;
			}
		}
		return $qb_result['message'];
	} catch (Exception $e) {
		return 'QuickBooks error: ' . $e->getMessage();
	}
}
...
// update by nirmal 20_02_2024 new improved tax calculation code
// update by nirmal 12_03_2024 (added qucikbooks intergration)
...
function sms()
{
	...
	if (($sms_cust == 1) && ($sms_balance > 0) && ($_SERVER['SERVER_NAME'] == inf_url_primary()) && ($bill_type != 3) && ($sms_sent == 0) && ($systemid == 17)) {
		...
		$msg = 'SMS alredy sent OR bill is not issued on today';
	}
}
...
function addBillPayment($sub_system)
{
	...
	$out = true;
	$msg = 'Done';
	$lastid_cash = $lastid_card = $bill_total_minus = $lastid_chque = '';
	$tran_bnk_n = $tran_bnk_v = $bill_total = $qb_msg = $qb_result = '';
	$systemid = inf_systemid(1);
	include('config.php');
	...
	if (($out) && ($amount_bank <> 0)) {
		if (isSalesmanPaymentDepositActive()) {
			$query = "INSERT INTO `payment` (`invoice_no`,`bill_pay`,`cust`,`payment_type`,`amount`,`salesman`,`sys_user`,`payment_date`,`comment`,`store`,`gps_x`,`gps_y`,`sub_system`,`bank_trans`,`status`) VALUES ('$invoice_no','1','$cust','3','$amount_bank','$billed_by','$sys_user','$time_now','$comment','$store','0','0','$sub_system','$tr_bank','2')";
		} else {
	...
	if ($out) {
		if (isQuickBooksActive(1)) {
			...
			$invoiceCostQuery = "SELECT SUM(b.qty * b.cost), c.name FROM bill_main bm, bill b, cust c WHERE c.id = bm.cust AND bm.invoice_no = b.invoice_no AND bm.`invoice_no`='$invoice_no'";
			...
			if ($qb_cust_id) {
				...
				if ($bill_type == 1) {
					...
					// Journal Entry Build
					if ($bill_total_minus <> 0) { // cash back invoice
						...
						$journalEntryForInvoiceTotal = buildJournalEntry($conn2, abs($bill_total_minus), $debitAccountName, $creditAccountName, $description, $debitEntityType, $debitEntityID, $creditEntityType, $creditEntityID);
						if (isset($journalEntryForInvoiceTotal['error'])) {
							...
							$action = "RETURN INVOICE, JOURNAL ENTRY BUILD (Invoice Total)";
							$functionName = "addBillPayment";

							$error_log_query = "INSERT INTO `qb_debug_log` (`action`,`function`,`amount`,`debit_account_name`,`credit_account_name`,`journal_entry_description`,`debit_entity_type`,`debit_entity_id`,`credit_entity_type`,`credit_entity_id`,`qb_message`,`action_time`)
							VALUES ('$action','$functionName','$bill_total_minus','$debitAccountName','$creditAccountName','$description','$debitEntityType','$debitEntityID','$creditEntityType','$creditEntityID','$qb_msg','$time_now')";
							$result = mysqli_query($conn, $error_log_query);
							// throw new Exception($qb_msg);
						}
						...
						$journalEntryForInvoiceCost = buildJournalEntry($conn2, abs($invoiceCost), $debitAccountName, $creditAccountName, $description, $debitEntityType, $debitEntityID, $creditEntityType, $creditEntityID);
						if (isset($journalEntryForInvoiceCost['error'])) {
							...
							$action = "RETURN INVOICE, JOURNAL ENTRY BUILD (Invoice Cost)";
							$functionName = "addBillPayment";

							$error_log_query = "INSERT INTO `qb_debug_log` (`action`,`function`,`amount`,`debit_account_name`,`credit_account_name`,`journal_entry_description`,`debit_entity_type`,`debit_entity_id`,`credit_entity_type`,`credit_entity_id`,`qb_message`,`action_time`)
							VALUES ('$action','$functionName','$invoiceCost','$debitAccountName','$creditAccountName','$description','$debitEntityType','$debitEntityID','$creditEntityType','$creditEntityID','$qb_msg','$time_now')";
							$result = mysqli_query($conn, $error_log_query);
							// throw new Exception($qb_msg);
						}
						...
						$journalEntryForInvoiceTotal = buildJournalEntry($conn2, $bill_total, $debitAccountName, $creditAccountName, $description, $debitEntityType, $debitEntityID, $creditEntityType, $creditEntityID);
						if (isset($journalEntryForInvoiceTotal['error'])) {
							$flag = false;
							$qb_msg = $journalEntryForInvoiceTotal['error'];
							...
							$action = "NORMAL INVOICE, JOURNAL ENTRY BUILD (Invoice Total)";
							$functionName = "addBillPayment";

							$error_log_query = "INSERT INTO `qb_debug_log` (`action`,`function`,`amount`,`debit_account_name`,`credit_account_name`,`journal_entry_description`,`debit_entity_type`,`debit_entity_id`,`credit_entity_type`,`credit_entity_id`,`qb_message`,`action_time`)
							VALUES ('$action','$functionName','$bill_total','$debitAccountName','$creditAccountName','$description','$debitEntityType','$debitEntityID','$creditEntityType','$creditEntityID','$qb_msg','$time_now')";
							$result = mysqli_query($conn, $error_log_query);
							// throw new Exception($qb_msg);
						}
						...
						$journalEntryForInvoiceCost = buildJournalEntry($conn2, $invoiceCost, $debitAccountName, $creditAccountName, $description, $debitEntityType, $debitEntityID, $creditEntityType, $creditEntityID);
						if (isset($journalEntryForInvoiceCost['error'])) {
							$flag = false;
							$qb_msg = $journalEntryForInvoiceCost['error'];
							...
							$action = "NORMAL INVOICE, JOURNAL ENTRY BUILD (Invoice Cost)";
							$functionName = "addBillPayment";
							$error_log_query = "INSERT INTO `qb_debug_log` (`action`,`function`,`amount`,`debit_account_name`,`credit_account_name`,`journal_entry_description`,`debit_entity_type`,`debit_entity_id`,`credit_entity_type`,`credit_entity_id`,`qb_message`,`action_time`)
							VALUES ('$action','$functionName','$invoiceCost','$debitAccountName','$creditAccountName','$description','$debitEntityType','$debitEntityID','$creditEntityType','$creditEntityID','$qb_msg','$time_now')";
							$result = mysqli_query($conn, $error_log_query);
					...
					// journal entry bulid end

					// journal entry insert
					// invoice total journal entry inset
					try {
						...
						if (isset($journal_entry_result['status']) && ($journal_entry_result['status'] == 'success')) {
							...
							$result1 = mysqli_query($conn, $query);
							if (!$result1) {
								$qb_msg = 'Error: Quickbooks bill main qb_value_id update error.';

								$qb_msg = mysqli_real_escape_string($conn, $qb_msg);
								$action = "UPDATE BILL MAIN INVOICE VALUE ID (Invoice Total) : Invoice No: $invoice_no";
								$functionName = "addBillPayment";
							...
							$qb_msg = mysqli_real_escape_string($conn, $qb_msg);
							$action = "INVOICE, JOURNAL ENTRY INSERT (Invoice Total) : Invoice No: $invoice_no";
							$functionName = "addBillPayment";
							$error_log_query = "INSERT INTO `qb_debug_log` (`action`,`function`,`qb_message`,`action_time`)
							VALUES ('$action','$functionName','$qb_msg','$time_now')";
							$result = mysqli_query($conn, $error_log_query);
					...
					// invoice cost journal entry inset
					try {
						...
						if (isset($journal_entry_result1['status']) && ($journal_entry_result1['status'] == 'success')) {
							...
							$result1 = mysqli_query($conn, $query);
							if (!$result1) {
								$qb_msg = 'Error: Quickbooks bill main qb_cost_id update error.';
							...
							$result1 = mysqli_query($conn, $query);
							if (!$result1) {
								$qb_msg = 'Error: Quickbooks bill main qb status update error.';

								$qb_msg = mysqli_real_escape_string($conn, $qb_msg);
								$action = "UPDATE BILL MAIN INVOICE QB STATUS : Invoice No: $invoice_no";
								$functionName = "addBillPayment";
							...
							$qb_msg = mysqli_real_escape_string($conn, $qb_msg);
							$action = "INVOICE, JOURNAL ENTRY INSERT (Invoice Cost) : Invoice No: $invoice_no";
							$functionName = "addBillPayment";
							$error_log_query = "INSERT INTO `qb_debug_log` (`action`,`function`,`qb_message`,`action_time`)
							VALUES ('$action','$functionName','$qb_msg','$time_now')";
							$result = mysqli_query($conn, $error_log_query);
						...
						$qb_msg = "<br>QuickBooks error: " . $e->getMessage();
						$journal_entry_result1['status'] = 'error';
					...
					if ((($amount_cash != '') && ($amount_cash != '0') && ($amount_cash > 0))) { // Cash payment
						...
						$result_array = processQBPayment($creditAccountName, $debitAccountName, $conn, $amount_cash, $qb_cust_id, $description);
						if (is_array($result_array)) {
							try {
								...
								if ((isset($journal_entry_result['status'])) && ($journal_entry_result['status'] == 'success')) {
									$qb_journal_entry_id = $journal_entry_result['qb_journal_entry_id'];
									$query = "UPDATE `payment` SET `qb_id`='$qb_journal_entry_id', `qb_status`=1 WHERE `id`='$lastid_cash'";
									$result1 = mysqli_query($conn, $query);
									if (!$result1) {
										...
										$action = "UPDATE PAYMENT QB ID : Invoice No: $invoice_no, Payment Id: $lastid_cash";
										$functionName = "addBillPayment";
									...
									$qb_msg = mysqli_real_escape_string($conn, $qb_msg);
									$action = "INVOICE PAYMENT, JOURNAL ENTRY INSERT (CASH): Invoice No: $invoice_no, Payment Id: $lastid_cash";
									$functionName = "addBillPayment";
									$error_log_query = "INSERT INTO `qb_debug_log` (`action`,`function`,`qb_message`,`action_time`)
									VALUES ('$action','$functionName','$qb_msg','$time_now')";
									$result = mysqli_query($conn, $error_log_query);
							...
							$qb_msg = $result_array;
							$action = "INVOICE PAYMENT, JOURNAL ENTRY BUILD (CASH), Payment Id: $lastid_cash";
							$functionName = "addBillPayment";
							...
							$error_log_query = "INSERT INTO `qb_debug_log` (`action`,`function`,`amount`,`debit_account_name`,`credit_account_name`,`journal_entry_description`,`debit_entity_type`,`debit_entity_id`,`credit_entity_type`,`credit_entity_id`,`qb_message`,`action_time`)
							VALUES ('$action','$functionName','$amount_cash','$debitAccountName','$creditAccountName','$description','','','Customer','$qb_cust_id','$qb_msg','$time_now')";
							$result = mysqli_query($conn, $error_log_query);
							// throw new Exception($qb_msg);
						}
					}
							...
							if ($accountNameResult && mysqli_num_rows($accountNameResult) > 0) {
								...
								if (is_array($result_array)) {
									try {
										...
										if ((isset($journal_entry_result['status'])) && ($journal_entry_result['status'] == 'success')) {
											$qb_journal_entry_id = $journal_entry_result['qb_journal_entry_id'];
											$query = "UPDATE `payment` SET `qb_id`='$qb_journal_entry_id', `qb_status`=1 WHERE `id`='$lastid_bank'";
											$result1 = mysqli_query($conn, $query);
											if (!$result1) {
												...
												$action = "UPDATE PAYMENT QB ID : Invoice No: $invoice_no, Payment Id: $lastid_bank";
												$functionName = "addBillPayment";
											...
											$qb_msg = mysqli_real_escape_string($conn, $qb_msg);
											$action = "INVOICE PAYMENT, JOURNAL ENTRY INSERT (BANK) : Invoice No: $invoice_no, Payment Id: $lastid_bank";
											$functionName = "addBillPayment";
											$error_log_query = "INSERT INTO `qb_debug_log` (`action`,`function`,`qb_message`,`action_time`)
											VALUES ('$action','$functionName','$qb_msg','$time_now')";
											$result = mysqli_query($conn, $error_log_query);
									...
									$action = "INVOICE PAYMENT, JOURNAL ENTRY BUILD (BANK), Payment Id: $lastid_bank";
									$functionName = "addBillPayment";

									$error_log_query = "INSERT INTO `qb_debug_log` (`action`,`function`,`amount`,`debit_account_name`,`credit_account_name`,`journal_entry_description`,`debit_entity_type`,`debit_entity_id`,`credit_entity_type`,`credit_entity_id`,`qb_message`,`action_time`)
									VALUES ('$action','$functionName','$amount_bank','$debitAccountName','$creditAccountName','$description','','','Customer','$qb_cust_id','$qb_msg','$time_now')";
									$result = mysqli_query($conn, $error_log_query);
								}
							}
						}
					...
					if ((($amount_chque != '') && ($amount_chque != '0') && ($amount_chque > 0))) { // Cheque payment
						...
						$result_array = processQBPayment($creditAccountName, $debitAccountName, $conn, $amount_chque, $qb_cust_id, $description);
						if (is_array($result_array)) {
							try {
								$journal_entry_result = QBAddJournalEntry($result_array);
								$qb_msg = $journal_entry_result['message'];
								...
								if ((isset($journal_entry_result['status'])) && ($journal_entry_result['status'] == 'success')) {
									$qb_journal_entry_id = $journal_entry_result['qb_journal_entry_id'];
									$query = "UPDATE `payment` SET `qb_id`='$qb_journal_entry_id', `qb_status`=1 WHERE `id`='$lastid_chque'";
									$result1 = mysqli_query($conn, $query);
									if (!$result1) {
										$qb_msg = 'Error: QuickBooks payment ID update error.';
										...
										$action = "UPDATE PAYMENT QB ID : Invoice No: $invoice_no, Payment Id: $lastid_chque";
										$functionName = "addBillPayment";
									...
									$qb_msg = mysqli_real_escape_string($conn, $qb_msg);
									$action = "INVOICE PAYMENT, JOURNAL ENTRY INSERT (CHEQUE) : Invoice No: $invoice_no, Payment Id: $lastid_chque";
									$functionName = "addBillPayment";
									$error_log_query = "INSERT INTO `qb_debug_log` (`action`,`function`,`qb_message`,`action_time`)
									VALUES ('$action','$functionName','$qb_msg','$time_now')";
									$result = mysqli_query($conn, $error_log_query);
							...
							$action = "INVOICE PAYMENT, JOURNAL ENTRY BUILD (CHEQUE), Payment Id: $lastid_chque";
							$functionName = "addBillPayment";

							$error_log_query = "INSERT INTO `qb_debug_log` (`action`,`function`,`amount`,`debit_account_name`,`credit_account_name`,`journal_entry_description`,`debit_entity_type`,`debit_entity_id`,`credit_entity_type`,`credit_entity_id`,`qb_message`,`action_time`)
							VALUES ('$action','$functionName','$amount_chque','$debitAccountName','$creditAccountName','$description','','','Customer','$qb_cust_id','$qb_msg','$time_now')";
							$result = mysqli_query($conn, $error_log_query);
							// throw new Exception($qb_msg);
						}
					}
					...
					if ((($amount_card != '') && ($amount_card != '0') && ($amount_card > 0))) { // Card payment
						...
						if ($accountNameResult && mysqli_num_rows($accountNameResult) > 0) {
							...
							$result_array = processQBPayment($creditAccountName, $debitAccountName, $conn, $amount_card, $qb_cust_id, $description);
							if (is_array($result_array)) {
								try {
									$journal_entry_result = QBAddJournalEntry($result_array);
									$qb_msg = $journal_entry_result['message'];
									...
									if ((isset($journal_entry_result['status'])) && ($journal_entry_result['status'] == 'success')) {
										$qb_journal_entry_id = $journal_entry_result['qb_journal_entry_id'];
										$query = "UPDATE `payment` SET `qb_id`='$qb_journal_entry_id', `qb_status`=1 WHERE `id`='$lastid_card'";
										$result1 = mysqli_query($conn, $query);
										if (!$result1) {
											$qb_msg = 'Error: QuickBooks payment ID update error.';
											...
											$action = "UPDATE PAYMENT QB ID : Invoice No: $invoice_no, Payment Id: $lastid_card";
											$functionName = "addBillPayment";
										...
										$qb_msg = mysqli_real_escape_string($conn, $qb_msg);
										$action = "INVOICE PAYMENT, JOURNAL ENTRY INSERT (CARD) : Invoice No: $invoice_no, Payment Id: $lastid_card";
										$functionName = "addBillPayment";
										$error_log_query = "INSERT INTO `qb_debug_log` (`action`,`function`,`qb_message`,`action_time`)
										VALUES ('$action','$functionName','$qb_msg','$time_now')";
										$result = mysqli_query($conn, $error_log_query);
								...
								$action = "INVOICE PAYMENT, JOURNAL ENTRY BUILD (CARD), Payment Id: $lastid_card";
								$functionName = "addBillPayment";

								$error_log_query = "INSERT INTO `qb_debug_log` (`action`,`function`,`amount`,`debit_account_name`,`credit_account_name`,`journal_entry_description`,`debit_entity_type`,`debit_entity_id`,`credit_entity_type`,`credit_entity_id`,`qb_message`,`action_time`)
								VALUES ('$action','$functionName','$amount_card','$debitAccountName','$creditAccountName','$description','','','Customer','$qb_cust_id','$qb_msg','$time_now')";
								$result = mysqli_query($conn, $error_log_query);
								// throw new Exception($qb_msg);
							}
						}
				...
				// CUST ORDER
				if ($bill_type == 4) {
					if ($bill_total > 0) { // not cash back
						// insert cost
						//ALTER TABLE bill_main ADD COLUMN qb_temp_cost INT DEFAULT NULL COMMENT 'bill time total bill cost';
						// invoice cost journal entry
						// $debitAccountName = "Cost of Goods Sold";
						// $creditAccountName = "Inventory Asset";
						// $description = "[INVOICE] - Invoice No: $invoice_no, Customer : $custName";
						...
						// 	$action = "CUST ODR INVOICE, JOURNAL ENTRY BUILD (Invoice Cost)";
						// 	$functionName = "addBillPayment";
							...
							if (is_array($result_array)) {
								try {
									...
									if ((isset($journal_entry_result['status'])) && ($journal_entry_result['status'] == 'success')) {
										...
										$qb_msg = mysqli_real_escape_string($conn, $qb_msg);
										$action = "CUST ODR INVOICE, PAYMENT JOURNAL ENTRY INSERT (CASH): Invoice No: $invoice_no, Payment Id: $lastid_cash";
										$functionName = "addBillPayment";
										$error_log_query = "INSERT INTO `qb_debug_log` (`action`,`function`,`qb_message`,`action_time`)
										VALUES ('$action','$functionName','$qb_msg','$time_now')";
										$result = mysqli_query($conn, $error_log_query);
								...
								$qb_msg = $result_array;
								$action = "CUST ODR INVOICE, PAYMENT JOURNAL ENTRY BUILD (CASH), Payment Id: $lastid_cash";
								$functionName = "addBillPayment";
									...
									if (is_array($result_array)) {
										try {
											...
											if ((isset($journal_entry_result['status'])) && ($journal_entry_result['status'] == 'success')) {
												...
												$qb_msg = mysqli_real_escape_string($conn, $qb_msg);
												$action = "CUST ODR INVOICE, PAYMENT JOURNAL ENTRY INSERT (BANK) : Invoice No: $invoice_no, Payment Id: $lastid_bank";
												$functionName = "addBillPayment";
												$error_log_query = "INSERT INTO `qb_debug_log` (`action`,`function`,`qb_message`,`action_time`)
												VALUES ('$action','$functionName','$qb_msg','$time_now')";
												$result = mysqli_query($conn, $error_log_query);
										...
										$action = "CUST ODR INVOICE, PAYMENT JOURNAL ENTRY BUILD (BANK), Payment Id: $lastid_bank";
										$functionName = "addBillPayment";
							...
							if (is_array($result_array)) {
								try {
									...
									if ((isset($journal_entry_result['status'])) && ($journal_entry_result['status'] == 'success')) {
										...
										$qb_msg = mysqli_real_escape_string($conn, $qb_msg);
										$action = "CUST ODR INVOICE, PAYMENT JOURNAL ENTRY INSERT (CHEQUE) : Invoice No: $invoice_no, Payment Id: $lastid_chque";
										$functionName = "addBillPayment";
										$error_log_query = "INSERT INTO `qb_debug_log` (`action`,`function`,`qb_message`,`action_time`)
										VALUES ('$action','$functionName','$qb_msg','$time_now')";
										$result = mysqli_query($conn, $error_log_query);
								...
								$action = "CUST ODR INVOICE, PAYMENT JOURNAL ENTRY BUILD (CHEQUE), Payment Id: $lastid_chque";
								$functionName = "addBillPayment";
								...
								if (is_array($result_array)) {
									try {
										...
										if ((isset($journal_entry_result['status'])) && ($journal_entry_result['status'] == 'success')) {
											...
											$qb_msg = mysqli_real_escape_string($conn, $qb_msg);
											$action = "CUST ODR INVOICE, PAYMENT JOURNAL ENTRY INSERT (CARD) : Invoice No: $invoice_no, Payment Id: $lastid_card";
											$functionName = "addBillPayment";
											$error_log_query = "INSERT INTO `qb_debug_log` (`action`,`function`,`qb_message`,`action_time`)
											VALUES ('$action','$functionName','$qb_msg','$time_now')";
											$result = mysqli_query($conn, $error_log_query);
									...
									$action = "CUST ODR INVOICE, PAYMENT JOURNAL ENTRY BUILD (CARD), Payment Id: $lastid_card";
									$functionName = "addBillPayment";
...
function addPayment($ocation)
{
	...
	$sys_user = $_COOKIE['user_id'];
	$cust = $_REQUEST['cust'];
	$payment_type = $payment_type2 = $_REQUEST['payment_type'];
	...
	try {
		...
		if (isQuickBooksActive(1)) {
			$qb_cust_id = getCustomerQBId($cust);
			if ($qb_cust_id == '') {
				$qb_msg = 'Error: This customer is not registered in Quckbooks';
				throw new Exception($msg);
			}
		}
		if ($invoice_no == '') {
			...
			if ($emptyamount) {
				...
				if ((($payment_type == 1) || ($payment_type == 3)) && ($proceed)) {
					if ((isSalesmanPaymentDepositActive()) && ($cash_type == 3)) {
						$query = "INSERT INTO `payment` (`invoice_no`,`bill_pay`,`cust`,`payment_type`,`amount`,`salesman`,`sys_user`,`payment_date`,`comment`,`store`,`gps_x`,`gps_y`,`sub_system`,`status` $tran_bnk_n) VALUES ('$invoice_no','$bill_pay','$cust','$cash_type','$amount_cash','$salesman','$sys_user','$time_now','$comment','$store','$gps_x','$gps_y','$sub_system','2' $tran_bnk_v)";
					} else {
				...
				if ((($payment_type == 2) || ($payment_type == 3)) && ($proceed)) {
					...
					if ($chque_bank_id != '') {
						$query = "INSERT INTO `payment` (`invoice_no`,`bill_pay`,`cust`,`payment_type`,`amount`,`salesman`,`sys_user`,`payment_date`,`chque_no`,`chque_bank`,`chque_branch`,`chque_date`,`chque_name`,`comment`,`store`,`gps_x`,`gps_y`,`sub_system`) VALUES ('$invoice_no','$bill_pay','$cust','2','$amount_chque','$salesman','$sys_user','$time_now','$chque_no','$chque_bank_id','$chque_branch','$chque_date','$chque_name','$comment','$store','$gps_x','$gps_y','$sub_system')";
						$result2 = mysqli_query($conn, $query);
						if (!$result2) {
							$message = 'Error: Payment could not be added';
							$out = false;
				...
				if ($ocation == 'pay') {
					if (isQuickBooksActive(1)) {
						...
						if (($qb_cust_id != '')) { // already customer register in quickbooks
							$creditAccountName = 'Accounts Receivable (A/R)';
							...
							if ($payment_type2 == 4) { // card
								...
								if ($accountNameResult && mysqli_num_rows($accountNameResult) > 0) {
									...
									$description .= ", Invoice No: $invoice_no, Customer: $custName";
									$result_array = processQBPayment($creditAccountName, $debitAccountName, $conn, $amount_card, $qb_cust_id, $description);
									if (is_array($result_array)) {
										try {
											$journal_entry_result = QBAddJournalEntry($result_array);
							...
							$qb_msg = 'Quickbooks error: This user not registered in Quickbooks';
							throw new Exception($qb_msg);
						}
					}
				}
...
function deletePayment($n, $force)
{
	...
	try {
		...
		if ($out) {
			if (isQuickBooksActive(1)) {
				$journalEntryForDeletedPayment = $journal_payment_entry_result = [];
				$paymentQuery = "SELECT p.`id`, p.`payment_type`, p.`qb_id`, c.`name`, c.`qb_cust_id`, p.`amount`, p.`chque_clear`, p.`chque_return`, p.`chque_deposit_bank`, p.`bank_trans` FROM `payment` p, `cust` c WHERE c.`id` = p.`cust` AND p.`qb_id`!=0 AND p.`id`='$payment_id'";
				...
				if ($paymentResult && mysqli_num_rows($paymentResult) > 0) {
					while ($paymentRow = mysqli_fetch_assoc($paymentResult)) {
						...
						if (($flag) && ($paymentRow['payment_type'] == 1) && ($qb_payment_id != '')) { // Cash
							$debitAccountName = "Accounts Receivable (A/R)";
							$creditAccountName = "Cash on Hand";
							$description = "[DELETE PAYMENT] - Invoice No: $invoice_no, Customer : $custName";
							$debitEntityType = "Customer";
							...
							$journalEntryForDeletedPayment = buildJournalEntry($conn2, abs($amount), $debitAccountName, $creditAccountName, $description, $debitEntityType, $debitEntityID, $creditEntityType, $creditEntityID);
							if (isset($journalEntryForDeletedPayment['error'])) {
								$flag = false;
						...
						if (($flag) && ($paymentRow['payment_type'] == 2) && ($qb_payment_id != '')) { // Cheuqe
							if ($paymentRow['chque_clear'] == 1 && $paymentRow['chque_return'] != 0) {
								$accountNameQuery = "SELECT `name` FROM accounts WHERE id='$chque_deposit_bank'";
								$accountNameResult = mysqli_query($conn2, $accountNameQuery);
								if ($accountNameResult && mysqli_num_rows($accountNameResult) > 0) {
						...
						if (($flag) && ($paymentRow['payment_type'] == 3) && ($qb_payment_id != '')) { // Bank
							$accountNameQuery = "SELECT `name` FROM accounts WHERE id='$bank_trans'";
							$accountNameResult = mysqli_query($conn2, $accountNameQuery);
							if ($accountNameResult && mysqli_num_rows($accountNameResult) > 0) {
								$accountNameRow = mysqli_fetch_row($accountNameResult);
						...
						if (($flag) && ($paymentRow['payment_type'] == 4) && ($qb_payment_id != '')) { // Card
							$accountNameQuery = "SELECT `name` FROM accounts WHERE id='$bank_trans'";
							$accountNameResult = mysqli_query($conn2, $accountNameQuery);
							if ($accountNameResult && mysqli_num_rows($accountNameResult) > 0) {
								$accountNameRow = mysqli_fetch_row($accountNameResult);
								...
								try {
									...
									$flag = false;
									$qb_msg = "<br>QuickBooks error: " . $e->getMessage();
									$journal_payment_entry_result['status'] = 'error';
									throw new Exception($qb_msg);
								}
							}
						}
...
// updated by nirmal 27_03_2024 added quickbooks integration
...
function addCashPaymentDeposit($sub_system, $systemid)
{
	...
	if ($out && ($amount > $amount_to_settle)) {
		$out = false;
	...
	if ($out && $note != '') {
		if ($note > 255) {
			$message = 'Error: Note length exceeds the allowed limit of 255 characters.';
			$out = false;
		}
	}
...
function getBankDepositsReport($sub_system)
{
	...
	$fromdate, $todate;
	$payment_id = $invoice_no = $cust_name = $payment_amount = $bank_name = $comment = $payment_date = $payment_status = $payment_type = $payment_deposit_id = $payment_transfer_date = [];
	include('config.php');
	$user_id = $_COOKIE['user_id'];
	$user_qry = $bank_qry = $date_qry = $status_qry = $source_qry = '';
	...
	$query = "SELECT py.`id`, py.`invoice_no`, cu.`name`, py.`amount`, ac.`name`, py.`comment`, py.`payment_date`, pd.`status`, pd.`type`, pd.`id`, pd.`transfer_date`
				FROM payment_deposit pd
