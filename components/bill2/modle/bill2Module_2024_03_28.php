...
function generateReturnList(){
	...
	$query="SELECT DISTINCT itm.code,itm.description,rt.qty,itm.id,rm.invoice_no FROM return_main rm, `return` rt, inventory_items itm, inventory_qty itq WHERE rm.invoice_no=rt.invoice_no AND rt.replace_item=itm.id AND itm.id=itq.item AND rm.`status`='2' AND rt.odr_packed='0' AND rm.`cust`='$rm_cust' ORDER BY $pending_return_items_order_by";
	$result=mysqli_query($conn,$query);
	while($row=mysqli_fetch_array($result)){
		...
		if((isQuickBooksActive(1))){
			...
			if($qb_cust_id){ // create invoice
					if($bill_total_minus<>0){ // cash back invoice
						...
						$qb_msg = processQBRefund($bill_total_minus, $comment, $amount_cash, $qb_cust_id, $invoice_no);
					}
					try{
...
function processQBRefund($bill_total_minus, $comment, $amount_cash, $qb_cust_id, $invoice_no) {
    try {
		include('config.php');
        $qb_account = 188;
    ...
    try {
        ...
        if ($result3 && mysqli_num_rows($result3) > 0) {
			...
			if($invoice_no == 0){
				...
				$amount_cash,'qb_payment_method_id'=>$qb_payment_method_id, 'qb_payment_ref_number' => $qb_payment_ref_number);
			}else{
    ...
    try {
        ...
        if ($result3 && mysqli_num_rows($result3) > 0) {
            ...
            if ($result1 && mysqli_num_rows($result1) > 0) {
                ...
                if ($qb_account_id != '') {
					if($invoice_no == 0){
						$payment_array = array('qb_cust_id' => $qb_cust_id, 'amount'=> $amount_bank, 'total_amount' => $amount_bank, 'qb_bank_id'=> $qb_account_id,'qb_payment_method_id'=>$qb_payment_method_id, 'qb_payment_ref_number' => $qb_payment_ref_number);
					}else{
						$payment_array = array('qb_cust_id' => $qb_cust_id, 'amount'=> $amount_bank, 'total_amount' => $amount_bank, 'qb_invoice_id'=> $qb_txnid, 'qb_bank_id'=> $qb_account_id,'qb_payment_method_id'=>$qb_payment_method_id, 'qb_payment_ref_number' => $qb_payment_ref_number);
					}
	...
	try{
		...
		if ($result3 && mysqli_num_rows($result3) > 0) {
			...
			if (($result1) && (mysqli_num_rows($result1) > 0)) {
				...
				if ($qb_account_id != '') {
					if($invoice_no == 0){
						$payment_array = array('qb_cust_id' => $qb_cust_id, 'amount'=> $amount_card, 'total_amount' => $amount_card, 'qb_bank_id'=> $qb_account_id,'qb_payment_method_id'=>$qb_payment_method_id, 'qb_payment_ref_number' => $qb_payment_ref_number);
					}else{
						$payment_array = array('qb_cust_id' => $qb_cust_id, 'amount'=> $amount_card, 'total_amount' => $amount_card, 'qb_invoice_id'=> $qb_txnid, 'qb_bank_id'=> $qb_account_id,'qb_payment_method_id'=>$qb_payment_method_id, 'qb_payment_ref_number' => $qb_payment_ref_number);
					}
					try {
						$qb_bank_payment_result = QBAddPayment($payment_array);
						$qb_msg = $qb_bank_payment_result['message'];
