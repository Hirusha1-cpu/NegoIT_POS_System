...
{	
	...
	if ($_POST['keyword']) {
		...
		if ($cashback == 1) {
			$fn = 'selectSN2';
			$query = "SELECT itu.sn FROM bill_main bm, bill bi, inventory_unic_item itu
			WHERE bm.invoice_no=bi.invoice_no AND itu.bill_id=bi.id AND itu.invoice_no=bm.invoice_no
			AND bm.`status`!=0 AND bm.`lock`=1 AND itu.`status`=1 AND bi.item='$item_id' AND bm.`cust`='$cust_id' AND itu.sn LIKE '%$keyword%' LIMIT 20";
			$result = mysqli_query($conn, $query);
			while ($row = mysqli_fetch_array($result)) {
				$data_list[] = $row[0];
			}
...
function updateTMPBillItem($sub_system)
{
	...
	if ($qty < 0) {
		$query1 = "SELECT SUM(bi.qty) FROM bill_main bm, bill bi WHERE bm.invoice_no=bi.invoice_no AND bm.`cust`='$cust'
		AND bi.item='$itm_id' AND bm.`status`!='0' AND bm.`lock`='1'";
		$row1 = mysqli_fetch_row(mysqli_query($conn, $query1));
			...
			if ($pr_sr == 1) {
				if (($itm_unic == 1) && ($unic_cal)) {
					...
					if ($bi_qty > 0) {
						...
						$query1 = "SELECT SUM(bi.`qty`), inv.`description` FROM bill_main bm, bill bi, inventory_items inv WHERE bi.item=inv.id AND
						bm.invoice_no=bi.invoice_no AND bm.`cust`='$bm_cust' AND bi.item='$itm_id' AND bm.`status`!='0' AND bm.`lock`='1'";
						$row1 = mysqli_fetch_row(mysqli_query($conn, $query1));
						$purchased_qty = ($row1[0] !== null) ? (int) $row1[0] : 0;
...
function addToBill($sub_system)
{
	...
	if ($qty < 0) {
		...
		$query1 = "SELECT SUM(bi.qty) FROM bill_main bm, bill bi WHERE bm.invoice_no=bi.invoice_no AND bm.`cust`='$bill_main_cust'
		AND bi.item='$itm_id' AND bm.`status`!='0' AND bm.`lock`='1'";
		$row1 = mysqli_fetch_row(mysqli_query($conn, $query1));
		$purchased_qty = ($row1[0] !== null) ? (int) $row1[0] : 0;
		if ($systemid != 17 && $systemid != 1) {
		...
		try {
			...
			if ($out) {
				...
				while ($row = mysqli_fetch_array($result)) {
					...
					if ($out) {
						debugEnd($debug_id, 'success', $bill_id);
					} else {
						debugEnd($debug_id, 'fail', $bill_id);
					}
				}
			}
...
function removeBillitem()
{
	...
	$query = "SELECT ivq.id,ivq.qty,bi.qty,(bi.unit_price+bi.discount),bi.cost,ivq.w_price,ivq.r_price,ivq.c_price,
	ivq.item,bi.no_update,bm.`lock`,bm.`type`,bm.`status`,bm.invoice_no
...
function generateInvoice($order_by)
{
	...
	if (($bi_type == 1) || ($bi_type == 4)) {
		$query = "SELECT bi.id,itm.description,bi.qty,bi.unit_price,itm.id,bi.`comment`,itm.`code`,
		bi.discount,itm.unic,bi.repair_model,bi.repair_sn,bi.return_odr_replace,bi.cross_trans,itq.drawer_no
		FROM bill bi, inventory_items itm, inventory_qty itq WHERE bi.item=itm.id AND itm.id=itq.item
		AND itq.location='$bm_mapped_inventory' AND bi.invoice_no='$invoice_no' ORDER BY $order_by";
	} else {
		$query = "SELECT bi.id,itm.description,bi.qty,bi.unit_price,itm.id,bi.`comment`,itm.`code`,
		bi.discount,itm.unic,bi.repair_model,bi.repair_sn,bi.return_odr_replace,bi.cross_trans
		FROM bill bi, inventory_items itm WHERE  bi.item=itm.id AND bi.invoice_no='$invoice_no' ORDER BY bi.id";
	}
...
function billPermission()
{
	...
	switch ($bm_status) {
		...
		case 7:
			...
			break;
		case 10:
			$status_out = 'Cross Check (Pending)';
			$status_color = 'yellowgreen';
			break;
		case 11:
			$status_out = 'Cross Check Picked';
			$status_color = 'green';
			break;
		default:
			$status_out = 'Unknown Status';
			$status_color = 'grey';
			break;
	}
...
function today($systemid, $sub_system)
{
	...
	$tmp_bm_salesman, $tmp_bm_type, $tmp_bm_sys_user, $w_invno, $w_time, $w_cust_name, $w_no,$conn;
	...
	$query = "SELECT bi.invoice_no,time(bm.`billed_timestamp`),SUM(bi.qty*bi.unit_price),SUM((bi.qty*bi.unit_price)-(bi.qty*bi.cost)),cu.name,
	SUM(bi.discount),bm.`sys_user` FROM bill bi, bill_main bm, cust cu
	WHERE bi.invoice_no=bm.invoice_no AND cu.id=bm.`cust` AND bm.`status`!=0  $salesman_qry1 AND bm.`lock`!='0' AND date(bm.`billed_timestamp`)='$date'
	GROUP BY bi.invoice_no ORDER BY bm.`billed_timestamp`";
	$result = mysqli_query($conn, $query);
	while ($row = mysqli_fetch_array($result)) {
		$invoice_no[] = $row[0];
		$time[] = $row[1];
	...
	// $query = "SELECT bi.invoice_no,date(bi.date),SUM(bi.qty*bi.unit_price),SUM((bi.qty*bi.unit_price)-(bi.qty*bi.cost)),
	// cu.name,cu.id,bm.`status`,bm.billed_by,bm.`type`,bm.`sys_user`
	// FROM bill bi, bill_main bm, cust cu WHERE bi.invoice_no=bm.invoice_no AND cu.id=bm.`cust` AND bm.`status`!=0  $salesman_qry1 AND bm.`lock`='0'
	// GROUP BY bi.invoice_no ORDER BY bm.`billed_timestamp`";
	$query = "SELECT 
    bi.invoice_no,
    DATE(bi.date) as bill_date,
    SUM(bi.qty*bi.unit_price) as total_amount,
    SUM((bi.qty*bi.unit_price)-(bi.qty*bi.cost)) as profit,
    cu.name,
    cu.id,
    bm.status,
    bm.billed_by,
...
function deleteInvoice($n, $force)
{
	...
	try {
		...
		$result = mysqli_query($conn, "SELECT `value` FROM settings WHERE setting='uniq_item_cal'");
		$row = mysqli_fetch_assoc($result);
		if ($row['value'] == 'True') {
			$unic_cal = true;
		} else {
		...
		//----------------To Compatible With Backend Delete Requests----------------------//
		// don't use inf_system_id it cause error when delete using backend.
		$result = mysqli_query($conn, "SELECT `value` FROM settings WHERE setting='systemid'");
		$row = mysqli_fetch_assoc($result);
		$system_id = $row['value'];
...
{
	global $cust_id, $cust_name, $cust_nic, $cust_mobile, $cust_asso_sman, $gps_x, $gps_y,$conn2;
	$sub_system = $_COOKIE['sub_system'];
	$sub_system = mapToSubSystemId(1, $sub_system);
	$cust_qry = '';

	include('config.php');
	if (isset($_REQUEST['action']) && ($_REQUEST['action'] == 'item_return') && isset($_REQUEST['cust'])) {
		$cust = $_REQUEST['cust'];
		$cust_qry = ' AND cu.id=' . $cust;
	}

	$query = "SELECT cu.id,cu.name,cu.nic,cu.mobile,up.username,cu.gps_x,cu.gps_y
	FROM cust cu, userprofile up
	WHERE cu.associated_salesman=up.id AND cu.sub_system='$sub_system' AND cu.`status` IN ($status) $cust_qry";
	$result = mysqli_query($conn2, $query);
	while ($row = mysqli_fetch_array($result)) {
		$cust_id[] = $row[0];
		$cust_name[] = $row[1];
...
function getBank()
{
	global $bank_id, $bank_code, $bank_name, $ac_bank_id, $ac_bank_name,$conn2;
	include('config.php');
...
function apendReturn()
{
	...
	if ($proceed) {
		if ((($ok_item1 == 1) && ($ok_item2 == 1)) || (($unic_item1 == '') && ($unic_item2 == ''))) {
			$proceed = true;
		} else {
			$proceed = false;
...
{
	global $chq0_id, $chq0_invno, $chq0_cuname, $chq0_amount, $chq0_no, $chq0_bank, $chq0_branch, $chq0_date, $chq0_paymentdate, $chq0_returndate, $chq0_code, $chq0_salesman, $salesman_filter, $chq2_banked, $chq2_retuned, $chq2_postpone, $conn;
	$store = $_COOKIE['store'];
...
function searchCust()
{
	global $fromdate, $todate, $cu_id, $cu_name, $search_name, $bm_no, $bm_total, $status_out, $status_color, $bm_store, $conn;
