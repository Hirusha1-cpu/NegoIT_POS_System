		...
		if ($out) {
			if (isQuickBooksActive(1)) {
				...
				if ((($amount_cash != '') && ($amount_cash != '0') && ($amount_cash > 0))) { // Cash payment
					...
					$result_array = processQBPayment($creditAccountName, $debitAccountName, $conn, abs($amount_cash), $qb_cust_id, $description, $debitClassID, $creditClassID, $debitDepartmentID, $creditDepartmentID);
				...
				if ((($amount_card != '') && ($amount_card != '0') && ($amount_card > 0))) { // Card payment
					...
					if ($accountNameResult && mysqli_num_rows($accountNameResult) > 0) {
						...
						$result_array = processQBPayment($creditAccountName, $debitAccountName, $conn, $amount_card, $qb_cust_id, $description, $debitClassID, $creditClassID, $debitDepartmentID, $creditDepartmentID);
						if (is_array($result_array)) {
							$batch_id = generateBatchID();
							if (!$batch_id) {
								throw new Exception("Error: Failed to generate a valid batch ID.");
