...
// update by nirmal 01_11_2023 (get emp name instead username for systemid=13)
...
// update by nirmal 20_11_2023 (get drawer no to return pending items)
function generateReturnList(){
	...
	$query="SELECT itm.code,itm.description,rt.qty,itq.`drawer_no` FROM return_main rm, `return` rt, inventory_items itm, inventory_qty itq WHERE rm.invoice_no=rt.invoice_no AND rt.replace_item=itm.id AND itm.id=itq.item AND itq.location='$bm_mapped_inventory' AND rm.`status`='2' AND rt.odr_packed='0' AND rm.`cust`='$rm_cust' ORDER BY itm.description";
	$result=mysqli_query($conn,$query);
	while($row=mysqli_fetch_array($result)){
		$rt_pending_code[]=$row[0];
		$rt_pending_desc[]=$row[1];
		$rt_pending_qty[]=$row[2];
		$rt_pending_itm_drawer[]=$row[3];
	}
