...
function generateReturnList(){
	...
	$query="SELECT DISTINCT itm.code,itm.description,rt.qty,itm.id FROM return_main rm, `return` rt, inventory_items itm, inventory_qty itq WHERE rm.invoice_no=rt.invoice_no AND rt.replace_item=itm.id AND itm.id=itq.item AND rm.`status`='2' AND rt.odr_packed='0' AND rm.`cust`='$rm_cust' ORDER BY $pending_return_items_order_by";
	$result=mysqli_query($conn,$query);
	while($row=mysqli_fetch_array($result)){
