...
*/
// update by nirmal 01_01_2025 (added transaction codes)
function createInvoice($sub_system)
{
	$bm_no = $_GET['bm_no'];
	$user_id = $_COOKIE['user_id'];
...
function generateInvoice($order_by)
{
	...
	$cust_tax_no, $sub_total, $credit_balance, $logo;
		...
		if ($proceed) {
			...
			if ($paymentResult && mysqli_num_rows($paymentResult) > 0) {
				while ($paymentRow = mysqli_fetch_assoc($paymentResult)) {
					...
					if(($paymentRow['payment_type'] == 1) && ($qb_payment_id != '')){ // Cash
						...
						if (isset($journalEntryForDeletedPayment['error'])) {
							...
							$action = "INVOICE PAYMENT, DELETE JOURNAL ENTRY BUILD (CASH): Payment Id: $payment_id";
					...
					if(($paymentRow['payment_type'] == 2) && ($qb_payment_id != '')){ // Cheuqe
						if($paymentRow['chque_clear'] == 1 && $paymentRow['chque_return'] != 0){
							...
							if ($accountNameResult && mysqli_num_rows($accountNameResult) > 0) {
								...
								if (isset($journalEntryForDeletedPayment['error'])) {
									...
									$qb_msg = mysqli_real_escape_string($conn, $qb_msg);
									$description = mysqli_real_escape_string($conn, $description);
									$action = "INVOICE PAYMENT, DELETE JOURNAL ENTRY BUILD (CHEQUE): Payment Id: $payment_id";
									$functionName = "deleteInvoice";
					...
					if(($paymentRow['payment_type'] == 3) && ($qb_payment_id != '')){ // Bank
						...
						if ($accountNameResult && mysqli_num_rows($accountNameResult) > 0) {
							...
							if (isset($journalEntryForDeletedPayment['error'])) {
								...
								$qb_msg = mysqli_real_escape_string($conn, $qb_msg);
								$description = mysqli_real_escape_string($conn, $description);
								$action = "INVOICE PAYMENT, DELETE JOURNAL ENTRY BUILD (BANK): Payment Id: $payment_id";
								$functionName = "deleteInvoice";
					...
					if(($paymentRow['payment_type'] == 4) && ($qb_payment_id != '')){ // Card
						$accountNameQuery = "SELECT `name` FROM accounts WHERE id='$bank_trans'";
						$accountNameResult = mysqli_query($conn2, $accountNameQuery);
						if ($accountNameResult && mysqli_num_rows($accountNameResult) > 0) {
							$accountNameRow = mysqli_fetch_row($accountNameResult);
							...
							if (isset($journalEntryForDeletedPayment['error'])) {
								...
								$qb_msg = mysqli_real_escape_string($conn, $qb_msg);
								$description = mysqli_real_escape_string($conn, $description);
								$action = "INVOICE PAYMENT, DELETE JOURNAL ENTRY BUILD (CARD): Payment Id: $payment_id";
								$functionName = "deleteInvoice";
...
function paymentDeletePermission($payment_no)
{
	...
	if (($user == $salesman) && ($today == $date) && ($status == 0))
		$paymentpermission = true;
	else
		$paymentpermission = false;
	if (($systemid == 15) && (isset($_COOKIE['settings'])) && ($status == 0))
		$paymentpermission = true;
...
function getPaymentTotalToBeSettle($type){
	...
	if($type == 'cash'){
		...
		$query2 = "SELECT SUM(amount) FROM payment_deposit WHERE `type` = 1 AND `transfer_source` = 1 AND `user_id` = '$user_id' AND `status` NOT IN(0,1,3)";
		$result2 = mysqli_query($conn2, $query2);
		$row2 = mysqli_fetch_row($result2);
...
function getChequePaymentsCollectedBySalesman($sub_system){
	...
	WHERE payment_date > '$start_date' AND (p.salesman = $user_id AND p.payment_type = 2 AND p.status = 0 AND p.chque_return != 1 AND ct.payment_id IS NULL) OR (ct.to = $user_id AND (ct.status = 2 AND ct.latest = 1)) OR (ct.to = $user_id AND ct.latest = 1 AND ct.modify_cheque = 1 AND ct.status != 10) ORDER BY p.id ASC";
