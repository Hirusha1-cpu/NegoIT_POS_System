...
function deleteInvoice($n, $force)
{
	...
	if (isQuickBooksActive(1)) {
		if ($proceed) {
			...
			$paymentQuery = "SELECT `qb_id` FROM `payment` WHERE `bill_pay`=1 AND `qb_id`!=0 AND `invoice_no`='$invoice_no'";
			$paymentResult = mysqli_query($conn, $paymentQuery);

			if ($paymentResult && mysqli_num_rows($paymentResult) > 0) {
				while ($paymentRow = mysqli_fetch_assoc($paymentResult)) {
					$qb_id = $paymentRow['qb_id'];
					$qb_msg = deleteJournalEntryAndUpdate($conn, $qb_id, 'payment', $invoice_no);
					if (strpos($qb_msg, 'error') !== false) {
						$delete_messages[] = "Error deleting QB payment ID {$qb_id}: " . $qb_msg;
					}
				}
			}

			// Delete associated invoice (if exists in QuickBooks)
			$invoiceQuery = "SELECT `qb_value_id`, `qb_cost_id` FROM `bill_main` WHERE `invoice_no`='$invoice_no'";
			...
			if ($invoiceResult && mysqli_num_rows($invoiceResult) > 0) {
				...
				if (!empty($invoiceRow['qb_value_id'])) {
					$qb_msg = deleteJournalEntryAndUpdate($conn, $invoiceRow['qb_value_id'], 'bill_main', $invoice_no);
					if (strpos($qb_msg, 'error') !== false) {
						$delete_messages[] = $qb_msg;
					}
				} else {
					$delete_messages[] = 'QuickBooks error: Invoice value ID not recorded in QuickBooks.';
				}

				if (!empty($invoiceRow['qb_cost_id'])) {
					$qb_msg = deleteJournalEntryAndUpdate($conn, $invoiceRow['qb_cost_id'], 'bill_main', $invoice_no);
					if (strpos($qb_msg, 'error') !== false) {
						$delete_messages[] = $qb_msg;
					}
				} else {
					$delete_messages[] = 'QuickBooks error: Invoice cost ID not recorded in QuickBooks.';
				}
			}
...
function addBillPayment($sub_system)
{
	...
	if (isset($_REQUEST['amount_card'])) {
		if ($amount_card <> 0) {
			if ($card_bank == '') {
				$out = false;
				$msg = "Please Select the Card Terminal's Bank";
			}
			if (strlen($card_no) == 4) {
				$card_no = "**** **** **** " . $card_no;
	...
	if ($amount_bank <> 0) {
		if ($tr_bank == '') {
			$out = false;
			$msg = 'Please Select the Money Transferred Bank';
		}
	...
	if (($custom_bill_date != '') && ($systemid == 25)) {
		...
		$query1 = "UPDATE bill_main
				SET order_timestamp = CONCAT('$custom_bill_date ', TIME(order_timestamp)),
					billed_timestamp = CONCAT('$custom_bill_date ', TIME(billed_timestamp))
		...
		$query2 = "UPDATE payment
				SET payment_date = CONCAT('$custom_bill_date ', TIME(payment_date))
				WHERE invoice_no = '$invoice_no'";
		$result2 = mysqli_query($conn, $query2);
		if (!$result2) {
			$out = false;
		...
		// Update bill table
		$query3 = "UPDATE bill
				SET `date` = CONCAT('$custom_bill_date ', TIME(`date`))
				WHERE invoice_no = '$invoice_no'";
		$result3 = mysqli_query($conn, $query3);
		if (!$result3) {
			$out = false;
	...
	if ($out) {
		if ((isQuickBooksActive(1))) {
			...
			if($qb_cust_id){
				...
				if ($bill_type == 1) {
					...
					if (($flag)) {
						...
						if ($journal_entry_result['status'] == 'success') {
							...
							$result1 = mysqli_query($conn, $query);
							if (!$result1) {
								$qb_msg = 'Quickbooks bill main qb_value_id update error.';
							}
						}
						...
						if ($journal_entry_result['status'] == 'success') {
							...
							if ($journal_entry_result1['status'] == 'success') {
						...
						if ($journal_entry_result1['status'] == 'success') {
							$creditAccountName = 'Accounts Receivable (A/R)';
							if (($flag) && (($amount_cash != '') && ($amount_cash != '0') && ($amount_cash > 0))) { // Cash payment
								$debitAccountName = "Cash on Hand";
								$description = "[PAYMENT] - Method: $debitAccountName, Invoice No: $invoice_no, Customer: $custName";
							...
							if (($flag) && (($amount_card != '') && ($amount_card != '0') && ($amount_card > 0))) { // Card payment
								...
								if ($accountNameResult && mysqli_num_rows($accountNameResult) > 0) {
									...
									$description = "[PAYMENT] - Method: Card ($debitAccountName), Customer: $custName";
									if ($invoice_no != 0) {
										$description = "[PAYMENT] - Method: Card ($debitAccountName), Invoice No: $invoice_no, Customer: $custName";
									}
							...
							if (($flag) && (($amount_chque != '') && ($amount_chque != '0') && ($amount_chque > 0))) { // Cheque payment
								$debitAccountName = "Undeposited Funds";
								$description = "[PAYMENT] - Method: Cheque ($debitAccountName), Customer: $custName";
								if ($invoice_no != 0) {
									$description = "[PAYMENT] - Method: Cheque ($debitAccountName), Invoice No: $invoice_no, Customer: $custName";
								}
				...
				$qb_msg = "Customer not created in QB: Journal entry not saved in QB";
			}
		}
	}
...
function addPayment($ocation)
{
	...
	if (isset($_REQUEST['cash_bank_switch'])) {
		...
		if ($_REQUEST['cash_bank_switch'] == 2) {
			...
			if ($card_bank == '') {
				$out = false;
				$message = "Please Select the Card Terminal's Bank";
			}
			if (strlen($card_no) == 4) {
				$card_no = "**** **** **** " . $card_no;
			} else if ((strlen($card_no) == 16) || (strlen($card_no) == 19)) {
	...
	if ($out) {
		...
		if ($emptyamount) {
			...
			if ($ocation == 'pay') {
				if (isQuickBooksActive(1)) {
					$qb_cust_id = getCustomerQBId($cust);
					$query1 = "SELECT c.name FROM bill_main bm, cust c WHERE bm.cust = c.id AND bm.`invoice_no`='$invoice_no' || bm.cust = '$cust' LIMIT 1";
					$row1 = mysqli_fetch_row(mysqli_query($conn2, $query1));
					$custName = $row1[0];
...
function deletePayment($n, $force)
{
	...
	if ($out) {
		if (isQuickBooksActive(1)) {
			$query1 = "SELECT `qb_id` FROM payment WHERE id='$payment_id'";
			$row1 = mysqli_fetch_row(mysqli_query($conn, $query1));
			$qb_msg = '';
			...
			if (!empty($row1)) {
				...
				if ($qb_id != '' && $qb_id != 0) {
					...
					if ($qb_result['status'] == 'success') {
						$qb_result_id = $qb_result['qb_result_journal_entry_id'];
						$query2 = "UPDATE `payment` SET `qb_status`=0 WHERE `qb_id`='$qb_result_id' AND `id`='$payment_id'";
						$result2 = mysqli_query($conn, $query2);
					...
					$qb_msg = 'QuickBooks error: This payment is not recorded in QuickBooks.';
				}
			} else {// Handle the case where qb_id is not found
				$qb_msg = "QuickBooks error: No QuickBooks ID found for payment ID $payment_id.";
			}
		}
...
function getPaymentTotalToBeSettle($type){
	...
	if($type == 'cash'){
		$query = "SELECT SUM(amount) FROM payment WHERE `salesman` = '$user_id' AND `status` = 0 AND `payment_type` = 1 AND payment_date > date(2024-10-29)";
		$result = mysqli_query($conn2, $query);
		$row = mysqli_fetch_row($result);
...
function addCashPaymentDeposit($sub_system, $systemid)
{
	...
	if($out){
		$query = "SELECT SUM(amount) FROM payment WHERE `salesman` = '$placed_by' AND `status` = 0 AND `payment_type` = 1";
		$result = mysqli_query($conn2, $query);
		$row = mysqli_fetch_row($result);
		$payment_cash_total_sum = $row[0];
...
function getBankPaymentsCollectedBySalesman($sub_system){
	...
	$query = "SELECT py.`id`, py.`invoice_no`, cu.`name`, py.`amount`, ac.`name`, py.`comment`, py.`payment_date`
			...
			WHERE py.`salesman` = '$user_id' AND py.`status` = 2 AND py.`payment_type` = 3 AND NOT EXISTS (
...
function getChequePaymentsCollectedBySalesman($sub_system){
	...
	$query2 = "SELECT p.id AS payment_id, p.chque_no, p.chque_bank, p.chque_branch, p.chque_date, p.amount, c.name AS customer_name, p.payment_date, b.name AS bank_name, b.bank_code,  ct.`modify_cheque`
	...
	WHERE (p.salesman = $user_id AND p.payment_type = 2 AND p.status = 0 AND p.chque_return != 1 AND ct.payment_id IS NULL) OR (ct.to = $user_id AND (ct.status = 2 AND ct.latest = 1)) OR (ct.to = $user_id AND ct.latest = 1 AND ct.modify_cheque = 1 AND ct.status != 10) ORDER BY p.id ASC";
	$result2 = mysqli_query($conn2, $query2);
	while ($row2 = mysqli_fetch_array($result2)) {
		$payment_id[] = $row2[0];
		$chq_no = $row2[1];
...
function deleteChequeTransferAjax($sub_system){
    ...
    if ($id == '') {
		...
		if ($result) {
			$id_count = mysqli_num_rows($result);
			// Transaction Management: Added transaction handling to ensure both operations succeed or fail together
