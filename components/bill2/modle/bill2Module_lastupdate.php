...
function generateReturnList(){
	...
	$query="SELECT itm.code,itm.description,rt.qty FROM return_main rm, `return` rt, inventory_items itm, inventory_qty itq WHERE rm.invoice_no=rt.invoice_no AND rt.replace_item=itm.id AND rm.`status`='2' AND rt.odr_packed='1' AND rt.odr_no='$invoice_no' AND itm.id=itq.item ORDER BY $replacement_items_return_order_by";
	$result=mysqli_query($conn,$query);
	while($row=mysqli_fetch_array($result)){
		if($row[0]!='CREDIT RTN'){
			$rt_code[]=$row[0];
