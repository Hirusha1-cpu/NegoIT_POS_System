...
// update by nirmal 05_10_2023, quatation bill item updated logic added
...
// update by nirmal 05_10_2023, quatation bill item adding logic added
...
function newTMPBill($sub_system,$cust_odr,$bill_sm,$qo_no){
	...
	if($out){
		...
		// // Check if there are any items in the bill_tmp table for the given bm_no
		// $query = "SELECT `bm_no`,`billed_by` FROM `bill_main_tmp` ORDER BY `bm_no` DESC LIMIT 1";
		// $result = mysqli_query($conn, $query);
		// $row = mysqli_fetch_row($result);
		// $last_bm_no = $row[0];
		// $last_billed_by = $row[1];

		// $query = "SELECT `id` FROM bill_tmp WHERE `bm_no` = '$last_bm_no'";
		// $result = mysqli_query($conn, $query);
		// $count = mysqli_num_rows($result);

		// // If there are no items in bill_tmp, delete the last record from bill_main_tmp
		// if($system_user==$last_billed_by){
		// 	if ($count === 0) {
		// 		$query = "DELETE FROM bill_main_tmp WHERE bm_no = '$last_bm_no'";
