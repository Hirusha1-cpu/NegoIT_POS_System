...
function updateTMPBillItem($sub_system)
{
	...
	include('config.php');
	$query = "SELECT bm.sys_user,bm.billed_by,itm.unic,itm.pr_sr,bi.new_inv_ref,mapped_inventory,itm.id,bm.cust,bm.quotation_no,itm.`description`,bm.`invoice_no` FROM bill_main_tmp bm, bill_tmp bi, inventory_items itm WHERE bm.bm_no=bi.bm_no AND bi.item=itm.id AND bi.id='$bill_id'";
	$row = mysqli_fetch_row(mysqli_query($conn, $query));
	$sys_user = $row[0];
	$billed_by = $row[1];
	$unic = $row[2];
	...
	if ($qty < 0) {
		...
		// }
		// 1. Calculate the total quantity purchased by the customer for this item from FINALIZED bills.
		$query_final_purchased = "SELECT SUM(bi.qty) FROM bill_main bm JOIN bill bi ON bm.invoice_no=bi.invoice_no WHERE bm.`cust`='$cust' AND bi.item='$itm_id' AND bm.`status`!='0' AND bm.`lock`='1'";
		$result_final_purchased = mysqli_query($conn, $query_final_purchased);

		if ($result_final_purchased === false) {
			$out = false;
			$message = 'Database query error (final purchased quantity): ' . mysqli_error($conn);
		} else {
			$row_final_purchased = mysqli_fetch_row($result_final_purchased);
			$total_final_purchased_qty = ($row_final_purchased[0] !== null) ? (int) $row_final_purchased[0] : 0;

			// 2. Calculate the *existing* cumulative return quantity already in the current bill_tmp for this item.
			// This sum is for negative quantities (returns) already added to this SAME TEMPORARY BILL ($bm_no)
			// *before* the current $qty0 is considered.
			...
			if ($result_existing_tmp_returns === false) {
				$out = false;
				$message = 'Database query error (existing temporary returns quantity): ' . mysqli_error($conn);
			} else {
				...
				// 3. Calculate the *projected total* return quantity for this item in this temp bill,
				// INCLUDING the quantity currently being added ($qty).
				$projected_total_tmp_return_qty = $existing_tmp_return_qty + abs($qty);

				// 4. Compare the total purchased quantity with the projected total return quantity.
				if ($total_final_purchased_qty < $projected_total_tmp_return_qty) {
					$out = false; // Using $out as your variable in this context

					// --- Dynamic Error Message Generation ---
					if ($total_final_purchased_qty == 0) {
						// Scenario: Customer has purchased 0 items.
						$message = 'Error: This customer has not purchased item : ' . $inv_item_name . ' before, so it cannot be returned.';
					} elseif ($total_final_purchased_qty < $projected_total_tmp_return_qty && $existing_tmp_return_qty == 0) {
						// Scenario: Customer purchased some, but not enough for this single new return (first return for this item in temp bill).
						// e.g., Purchased: 5, Trying to return: 10 (abs($qty) is 10, existing is 0, projected is 10)
						$message = 'Error: Customer purchased ' . $total_final_purchased_qty . ' items, but you are trying to return ' . abs($qty) . '. This exceeds the purchased quantity.';
					} else {
						// Scenario: Customer purchased enough, but adding this return would exceed the limit due to existing returns in this temp bill.
						// e.g., Purchased: 10, Existing in temp: 5, Trying to add: 6 (abs($qty) is 6, projected is 11)
						$message = 'Error: Adding this quantity (' . abs($qty) . ') would exceed the return limit. Customer purchased: ' . $total_final_purchased_qty . '. Existing returns in this bill: ' . $existing_tmp_return_qty . '. Total projected returns: ' . $projected_total_tmp_return_qty . '.';
					}
				}
			}
		}
				...
				if (($itm_unic == 1) && ($unic_cal)) {
					...
					if ($bi_qty > 0) {
						...
						if (($row1[0] !== null) && ($row1[0] >= -$bi_qty)) {
							...
							$out = false;
							$main_msg = $msg = 'Error: This customer did not purchase this item or purchased less quantity (purchased qty : ' . $purchased_qty . ')';
						}
					}
				}
			}
...
function addToBill($sub_system)
{
	...
	if ($qty < 0) {
		...
		// }
		// 1. Calculate the total quantity purchased by the customer for this item from FINALIZED bills.
		$query_final_purchased = "SELECT SUM(bi.qty) FROM bill_main bm JOIN bill bi ON bm.invoice_no=bi.invoice_no WHERE bm.`cust`='$bill_main_cust' AND bi.item='$itm_id' AND bm.`status`!='0' AND bm.`lock`='1'";
		$result_final_purchased = mysqli_query($conn, $query_final_purchased);
...
function newTMPBill($sub_system, $cust_odr, $bill_sm, $qo_no)
{
	...
	if ((isQuickBooksActive(1))) {
		...
		if (!empty($row)) {
			if ($row['qb_cust_id'] == '') {
				$out = false;
				$qb_msg = 'Error: this customer is not registered in qb';
			}
		}
	}
...
function createInvoice($sub_system)
{
	...
	if ($out) {
		try {
			...
			if ($out) {
				if (isQuickBooksActive(1)) {
					$invoiceQuery = "SELECT bm.`type`, SUM(b.`qty` * b.`cost`), bm.`invoice_+total` + bm.`invoice_-total`,bm.`tax`, c.`name` FROM bill_main bm, bill b, cust c
					WHERE c.id = bm.cust AND bm.invoice_no = b.invoice_no AND bm.`invoice_no`='$invoice_no'";
					...
					if ($bill_type == 1) {
						...
						if ($total > 0) {
							...
							$journalEntryForInvoiceTotal = buildJournalEntry($conn, abs($total), $debitAccountName, $creditAccountName, $description, $debitEntityType, $debitEntityID, $creditEntityType, $creditEntityID);
						...
						if ($bill_tax > 0) {
							...
							$journalEntryForInvoiceTax = buildJournalEntry($conn, abs($bill_tax), $debitAccountName, $creditAccountName, $description, $debitEntityType, $debitEntityID, $creditEntityType, $creditEntityID);
...
function deleteInvoice($n, $force)
{
	...
	try {
		...
		$query = "SELECT bi.`qty`, itm.`unic`, bm.`mapped_inventory`, bm.`status` FROM bill_main bm, bill bi, inventory_items itm WHERE bm.`invoice_no`=bi.`invoice_no` AND bi.`item`=itm.`id` AND bm.`invoice_no`='$invoice_no'";
		$result = mysqli_query($conn, $query);
		while ($row = mysqli_fetch_array($result)) {
			$store = $row[2];
			$bill_status = $row[3];
		...
		if ($proceed) {
			...
			if ($proceed && isQuickBooksActive(1)) {
				...
				$paymentQuery = "SELECT p.`id`, p.`payment_type`, p.`qb_id`, c.`name`, c.`qb_cust_id`, p.`amount`, p.`chque_clear`, p.`chque_return`, p.`chque_deposit_bank`, p.`bank_trans`, p.status
								FROM `payment` p, `cust` c
								WHERE c.`id` = p.`cust`
								AND p.`bill_pay` = 1 AND p.`qb_status` IS NOT NULL AND p.`invoice_no` = '$invoice_no'";
				$paymentResult = mysqli_query($conn, $paymentQuery);
				if ($paymentResult && mysqli_num_rows($paymentResult) > 0) {
					while ($paymentRow = mysqli_fetch_assoc($paymentResult)) {
						$payment_id = $payment_id = $qb_status = $custName = $qb_cust_id = $amount = $chque_deposit_bank = $bank_trans = '';
				...
				$invoiceQuery = "SELECT SUM(b.`qty` * b.`cost`) as cost, bm.`invoice_+total` + bm.`invoice_-total` as total, bm.`qb_value_id`, bm.`qb_cost_id`,
				...
				if ($invoiceResult && mysqli_num_rows($invoiceResult) > 0) {
					...
					if ($bill_status < 3 && $invoiceCost < 0) {
						...
						if ($invoiceCost != 0) {
							...
							$journalEntryForDeleteInvoiceCost = buildJournalEntry($conn, abs($invoiceCost), $debitAccountName, $creditAccountName, $description, $debitEntityType, $debitEntityID, $creditEntityType, $creditEntityID);
				...
				if (mysqli_num_rows($result) > 0) {
					...
					if ($item_cost != 0) {
						...
						$journalEntryForReturnItems = buildJournalEntry($conn, abs($item_cost), $debitAccountName, $creditAccountName, $description, $debitEntityType, $debitEntityID, $creditEntityType, $creditEntityID);
	...
	try {
		...
		if ($out) {
			if (isQuickBooksActive(1)) {
				...
				$invoiceCostQuery = "SELECT SUM(b.qty * b.cost), c.name FROM bill_main bm, bill b, cust c
				WHERE c.id = bm.cust AND bm.invoice_no = b.invoice_no AND bm.`invoice_no`='$invoice_no'";
				...
				if ((($amount_cash != '') && ($amount_cash != '0') && ($amount_cash > 0))) { // Cash payment
					...
					$result_array = processQBPayment($creditAccountName, $debitAccountName, $conn, abs($amount_cash), $qb_cust_id, $description);
				...
				if ((($amount_bank != '') && ($amount_bank != '0') && ($amount_bank > 0))) { // Bank payment
					if (!isSalesmanPaymentDepositActive()) {
						...
						if ($accountNameResult && mysqli_num_rows($accountNameResult) > 0) {
							...
							$result_array = processQBPayment($creditAccountName, $debitAccountName, $conn, abs($amount_bank), $qb_cust_id, $description);
							if (is_array($result_array)) {
								$batch_id = generateBatchID();
								if (!$batch_id) {
									throw new Exception("Error: Failed to generate a valid batch ID.");
				...
				if ((($amount_chque != '') && ($amount_chque != '0') && ($amount_chque > 0))) { // Cheque payment
					...
					$result_array = processQBPayment($creditAccountName, $debitAccountName, $conn, abs($amount_chque), $qb_cust_id, $description);
					if (is_array($result_array)) {
						$batch_id = generateBatchID();
						if (!$batch_id) {
							throw new Exception("Error: Failed to generate a valid batch ID.");
...
function deletePayment($n, $force)
{
	...
	try {
		...
		if ($out) {
			if (isQuickBooksActive(1)) {
				$journalEntryForDeletedPayment = [];
				$paymentQuery = "SELECT p.`id`, p.`payment_type`, p.`qb_id`, c.`name`, c.`qb_cust_id`, p.`amount`, p.`chque_clear`, p.`chque_return`, p.`chque_deposit_bank`, p.`bank_trans`
				FROM `payment` p, `cust` c
				WHERE c.`id` = p.`cust` AND p.`id`='$payment_id' AND p.`qb_status` IS NOT NULL";
				$paymentResult = mysqli_query($conn, $paymentQuery);
				if (!$paymentResult) {
...
function apendReturn()
{
	...
	if ($proceed) {
		...
		$query_purchased = "SELECT SUM(bi.qty) FROM bill_main bm JOIN bill bi ON bm.invoice_no=bi.invoice_no WHERE bm.`cust`='$cust' AND bi.item='$return_itemid' AND bm.`status`>2 AND bm.`lock`='1'";
		...
		if ($result_purchased === false) {
			...
			// 2. Calculate the total quantity already returned by the customer for this item.
			// We now join `return_main` with `return` to link customer to the return item.
			// Assuming `return_main.status` '2' means 'finished' or completed returns that should count.
			// Please confirm the meaning of `return_main.status` for completed returns.
			// 0=deleted, 1=on-going, 2=finished
			$query_returned = "SELECT SUM(r.qty)
            FROM `return_main` rm
            JOIN `return` r ON rm.invoice_no = r.invoice_no
            WHERE rm.`cust`='$cust'
            AND r.return_item='$return_itemid'
           	AND r.`odr_packed`=1";
			// -- Assuming '1' (on-going) means the return is finalized and should be counted
			...
			if ($result_returned === false) {
				...
				$row_returned = mysqli_fetch_row($result_returned);
				$total_returned_qty = ($row_returned[0] !== null) ? (int) $row_returned[0] : 0;

				// 3. Calculate the net available quantity for return.
				$net_available_qty = $total_purchased_qty - $total_returned_qty;

				// The quantity the user is attempting to return in this current transaction.
				$current_return_qty_absolute = abs($qty);

				// 4. Check if the net available quantity is sufficient for the current return request.
				if ($net_available_qty < $current_return_qty_absolute) {
					$proceed = false;

					$message = 'Error: This customer has only ' . $net_available_qty . ' items available for return (purchased: ' . $total_purchased_qty;

					// Add details about previously returned only if there were actual returns
					if ($total_returned_qty > 0) {
						$message .= ', previously returned: ' . $total_returned_qty;
					}

					$message .= '). You are trying to return: ' . $current_return_qty_absolute . '.';
				}
			}
		}
	}
