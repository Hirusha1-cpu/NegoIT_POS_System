	...
	if(isset($_GET['case'])){
		...
		if($itm_pr_sr==2){
			...
			if($itm_unic==1){
				...
				$itm_uprice='<select id="uprice"><option value="w,'.$w_price.'">Wholesale - '.number_format($w_price).'</option><option value="r,'.$r_price.'" '.$retail_default.'>Retail - '.number_format($r_price).'</option></select>';
				$itm_qty=$row[3];
...
function removeTMPBillItem($sub_system){
	...
	if(($user_id==$row[0])||($user_id==$row[1])){
		$bm_no = $row[2];
		$query="UPDATE `bill_main_tmp` SET `type`='1',`status`='5' WHERE `bm_no`='$bm_no'";
		$result=mysqli_query($conn,$query);
		if(!$result){ 
			$out=false; $msg='Error: Failed to Update the Bill Main Temp'; 
		}
		if($out){
...
function updateTMPBillItem($sub_system){
	...
	include('config.php');
	$query="SELECT bm.sys_user,bm.billed_by,itm.unic,itm.pr_sr,bi.new_inv_ref,mapped_inventory,itm.id FROM bill_main_tmp bm, bill_tmp bi, inventory_items itm WHERE bm.bm_no=bi.bm_no AND bi.item=itm.id AND bi.id='$bill_id'";
	$row=mysqli_fetch_row(mysqli_query($conn,$query));
	$sys_user=$row[0];
	$billed_by=$row[1];
	$unic=$row[2];
...
function validateTMPBill($case){
	...
	include('config.php');
	$query="SELECT bi.id,bm.sys_user,bm.billed_by,bm.mapped_inventory,itm.id,itm.unic,bi.comment,bi.qty,bi.new_inv_ref,bm.cust,itm.pr_sr FROM bill_main_tmp bm, bill_tmp bi, inventory_items itm WHERE bm.bm_no=bi.bm_no AND bi.item=itm.id AND bm.bm_no='$bm_no'";
	$result=mysqli_query($conn,$query);
	while($row=mysqli_fetch_array($result)){
		$bi_id=$row[0];
...
// update by nirmal 18_01_2022, discound bug fixed
...
function newTMPBill($sub_system,$cust_odr,$bill_sm,$qo_no){
	...
	$out=true;
	$bm_type=1;
	$bm_no=$back_off_com_type=0;
	$message='Done';
	
	if($cust_id==''){ $out=false; $message='Please Select the Customer or Phone Number'; }
...
function createInvoice($sub_system){
	...
	if($out){
		...
		VALUES ((SELECT MAX(b2.invoice_no)+1 FROM bill_main b2),'$orderhash','$authorize_code','$bm_billed_district','$bm_quotation_no','2','$bm_cust','$bm_billed_by','$bm_sys_user' $qry_re2,'$bm_order_timestamp','$time_now','$bm_type','$bm_store','$bm_mapped_inventory','$lock','$bm_gps_x','$bm_gps_y','$bm_sub_system','$bm_status' $qry_pb2 $qry_pt2);";
		$result=mysqli_query($conn,$query);
		$lastid_temp=mysqli_insert_id($conn);
	...
	if($out){
		...
		$query="UPDATE bill_main SET `invoice_+total`='$up_total',`invoice_-total`='$down_total' WHERE invoice_no='$invoice_no'";
		$result=mysqli_query($conn,$query);
...
function generateInvoice($order_by){
	...
	$hire_purchase,$hp_type,$hp_amount,$hp_schedule,$hp_schedule_remark,$hp_pay_id,$card_amount,$pay_pro_fee,$card_full_data;
...
function searchBill($id){
	...
	$id=ltrim($id, '0');
	include('config.php');
	$query="SELECT count(invoice_no) FROM bill_main WHERE invoice_no!=0 And invoice_no='$id'";
	$row=mysqli_fetch_row(mysqli_query($conn,$query));
...
function addBillPayment($sub_system){
    ...
    if($out){
		$query="SELECT bm.billed_by,bm.`cust` FROM bill_main bm WHERE bm.invoice_no='$invoice_no'";
		$row=mysqli_fetch_row(mysqli_query($conn,$query));
		$billed_by=$row[0];
		$cust=$row[1];
    }
