...
function addToBill($sub_system){
	...
	include('config.php');
	$query="SELECT sys_user FROM bill_main_tmp WHERE bm_no='$bm_no'";
	$row=mysqli_fetch_row(mysqli_query($conn,$query));
...
function createInvoice($sub_system){
	...
	if($out){
		...
		$tax = (($up_total + $down_total) * ($tax_rate / 100));
		$query="UPDATE bill_main SET `invoice_+total`='$up_total',`invoice_-total`='$down_total', `tax` = '$tax' WHERE invoice_no='$invoice_no'";
		// ------------
		$result=mysqli_query($conn,$query);
...
// updated by nirmal 27_07_2023 (decimal get places and add tax amount to the bill)
function generateInvoice($order_by){
	...
	$hire_purchase,$hp_type,$hp_amount,$hp_schedule,$hp_schedule_remark,$hp_pay_id,$card_amount,$pay_pro_fee,$card_full_data, $decimal_places, $tax, $tax_amount;
	...
	if(($bi_type==1)||($bi_type==4)){
		...
		$query="SELECT bi.id,itm.description,bi.qty,bi.unit_price,itm.id,bi.`comment`,itm.`code`,bi.discount,itm.unic,bi.repair_model,bi.repair_sn,bi.return_odr_replace,bi.cross_trans FROM bill bi, inventory_items itm, bill_main bm WHERE  bi.item=itm.id AND bi.invoice_no='$invoice_no' ORDER BY bi.id";
	}
...
function calculateTotal(){
	...
	$tax = (($up_total + $down_total) * ($tax_rate / 100));
	$query3="UPDATE bill_main SET `invoice_+total`='$up_total', `invoice_-total`='$down_total', `tax` = '$tax' WHERE `invoice_no`='$invoice_no'";
	// ------------
...
function changeJobTotal(){
	...
	if($out){
		...
		if($row[0]==1){
			...
			$tax = (($new_total) * ($tax_rate / 100));
			$query="UPDATE `bill_main` SET `invoice_+total`='$new_total', `tax` = '$tax'  WHERE `invoice_no`='$invoice_no'";
			// ------------
...
function addPayment($ocation){
	...
	if($out){
		...
		if(($lock_status==0)&&($ocation=='bill')&&($result2)){
			...
			$tax = (($payment1 + $payment2) * ($tax_rate / 100));

			$query="UPDATE bill_main SET `lock`='$lock_new', `billed_timestamp`='$today', `invoice_+total`='$payment1', `invoice_-total`='$payment2', `tax` = '$tax' WHERE invoice_no='$invoice_no'";
			// ------------
