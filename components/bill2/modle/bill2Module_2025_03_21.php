...
function getBillMain($bill_no)
{
	...
	$query = "SELECT btm.`type`, cu.`id`, cu.`name`, cu.`nic`, cu.`mobile`, user.`username`, btm.`recovery_agent`, user1.`username`, cu.`gps_x`, cu.`gps_y`, btm.`discount`,
	btm.`due_date`
...
function addPayment($ocation)
{
	...
	$qb_msg = $qb_cust_id = $qb_payment_id = '';
	include('config.php');
