...
// update by nirmal 03_07_2024 (show bill main store address instead of mapped invetory address for system id = 17 & sus_system =1)
function generateInvoice($order_by){
	...
	$result = mysqli_query($conn2,"SELECT name,shop_name,address,tel FROM stores WHERE id='$bm_mapped_inventory'");
	$row = mysqli_fetch_assoc($result);
	$tm_shop=$row['name'];
...
function deleteInvoice($n,$force){
	...
	if(isQuickBooksActive(1)){
		if($proceed){
			...
			$query = "SELECT `qb_payment_id` FROM `payment` WHERE `bill_pay`=1  AND `qb_payment_id`!=0 AND `invoice_no`='$invoice_no'";
			$result = mysqli_query($conn, $query);
			if ($result) {
				if (mysqli_num_rows($result) > 0) {
					while ($row = mysqli_fetch_assoc($result)) {
						...
						try {
							$qb_payment_result =  QBPaymentDelete($qb_payment_id);
							if($qb_payment_result['status'] == 'success'){
								$qb_result_payment_id = $qb_payment_result['qb_result_payment_id'];
								$query1="UPDATE `payment` SET `qb_status`=0 WHERE `qb_payment_id`='$qb_result_payment_id'";
								$result1=mysqli_query($conn,$query1);
								if(!$result1){
			...
			// invoice delete second
			$query2="SELECT `qb_txnid` FROM `bill_main` WHERE `invoice_no`='$invoice_no'";
			$result2 = mysqli_query($conn,$query2);
			if($result2){
				if (mysqli_num_rows($result2) > 0) {
					$row2 = mysqli_fetch_assoc($result2);
					$qb_txnid = $row2['qb_txnid'];
					if($qb_txnid != ''){
						try {
							$qb_result = QBInvoiceDelete($qb_txnid);
							if($qb_result['status'] == 'success'){
								$query3="UPDATE `bill_main` SET `qb_status`=0 WHERE `invoice_no`='$invoice_no'";
								$result3=mysqli_query($conn,$query3);
								if(!$result3){
									$qb_msg = 'Quickbooks bill_main status update error.';
								}
							}
	...
	if($out){
		...
		if($row[0]==1){
			...
			if($result){
				...
				if((isQuickBooksActive(1))){
					$amount = $new_total;
					$result4 = mysqli_query($conn2,"SELECT `qb_txnid` FROM bill_main WHERE invoice_no='$invoice_no'");
					$row4 = mysqli_fetch_array($result4);
					if(!empty($row4)){
						$qb_txnid=$row4[0];
						if($qb_txnid){ // create invoice
							$bill_array = array('Amount'=>$amount, 'qb_txnid'=>$qb_txnid);
							try {
								$qb_result = QBInvoiceUpdate($bill_array);
								$qb_msg = $qb_result['message'];
							} catch (Exception $e) {
								$qb_msg = "<br>QuickBooks error: " . $e->getMessage();
							}
...
function addBillPayment($sub_system){
	...
	$custom_bill_date='';
	if(isset($_POST['custom_bill_date'])) $custom_bill_date=$_POST['custom_bill_date'];
		...
		if((isQuickBooksActive(1))){
			...
			if($qb_cust_id){ // create invoice
					if($bill_total_minus<>0){ // cash back invoice
						// Quickbooks dosen't allow add (-) minus value invoices or payment, then we have to use refund recipt option
						$qb_msg = processQBRefundInvoice($bill_total_minus, $comment, $amount_cash, $qb_cust_id, $invoice_no);
					}
					try{
						$bill_array = array('Amount'=>$amount, 'total_amount'=>$bill_total, 'bill_invoice_no'=>$invoice_no, 'qb_cust_id'=> $qb_cust_id);
					...
					if($qb_result['status'] == 'success'){
						...
						$query = "UPDATE `bill_main` SET `qb_doc_id`='$qb_doc_id', `qb_txnid`='$qb_txnid', `qb_status`=1  WHERE `invoice_no`='$invoice_no'";
						$result = mysqli_query($conn, $query);

						if (($result) && (($amount_cash != '') && ($amount_cash != '0') && ($amount_cash > 0))) { // Cash payment
							$qb_msg = processQBCashPayment($qb_cust_id, $amount_cash, $qb_txnid, $lastid_cash, $comment, $invoice_no);
						}
						if (($result) && (($amount_bank != '') && ($amount_bank != '0') && ($amount_bank > 0))) { // Bank payment
							$qb_msg = processQBBankPayment($tr_bank, $qb_cust_id, $amount_bank, $qb_txnid, $lastid_bank, $comment, $invoice_no);
						}
						if (($result) && (($amount_card != '') && ($amount_card != '0') && ($amount_card > 0))) { // Card payment
							$qb_msg = processQBCardPayment($card_bank, $comment, $qb_cust_id, $amount_card, $qb_txnid, $lastid_bank, $invoice_no);
						}
					}
					$qb_msg = $qb_result['message'];
				...
				if($qb_result['status'] == 'success'){
					...
					if($result){ // create new invoice
						...
						if($qb_result['status'] == 'success'){ // update qb invoice number and start payment adding in qb
							$qb_doc_id = $qb_result['qb_doc_id'];
							$qb_txnid = $qb_result['qb_txnid'];
							$query = "UPDATE `bill_main` SET `qb_doc_id`='$qb_doc_id', `qb_txnid`='$qb_txnid', `qb_status`=1 WHERE `invoice_no`='$invoice_no'";
							$result = mysqli_query($conn, $query);
							...
							if (($result) && (($amount_card != '') && ($amount_card != '0') && ($amount_card > 0))) { // Card payment
								$qb_msg = processQBCardPayment($conn2, $card_bank, $comment, $qb_cust_id, $amount_card, $qb_txnid, $lastid_bank, $invoice_no);
							}
						}
						$qb_msg = $qb_result['message'];
					}
...
function processQBCashPaymentReturnInvoice($qb_cust_id, $amount_cash, $comment, $invoice_no) {
    $qb_msg = $qb_account = $qb_payment_result = $qb_payment_method_id = "";
	$qb_payment_ref_number = $comment;
    try {
		include('config.php');
		$query1 = "SELECT `qb_account_id` FROM accounts WHERE `name` = 'Cash in Hand'";
        $result1 = mysqli_query($conn2, $query1);
		if ($result1 && mysqli_num_rows($result1) > 0) {
			$row1 = mysqli_fetch_assoc($result1);
            $qb_account = $row1['qb_account_id'];
		}
		if(!$qb_account == ''){
			$query3 = "SELECT `qb_id` FROM qb_payment_methods WHERE `name` = 'Cash'";
			$result3 = mysqli_query($conn2, $query3);

			if ($result3 && mysqli_num_rows($result3) > 0) {
				$row3 = mysqli_fetch_assoc($result3);
				$qb_payment_method_id = $row3['qb_id'];
				$payment_array = array('qb_cust_id' => $qb_cust_id, 'total_amount' => $amount_cash, 'qb_payment_method_id'=>$qb_payment_method_id, 'qb_payment_ref_number' => $qb_payment_ref_number, 'qb_account_id' => $qb_account);
				try {
					$qb_payment_result = QBAddPayment($payment_array);
					$qb_msg = $qb_payment_result['message'];
				} catch (Exception $e) {
					$qb_msg = "QuickBooks error: " . $e->getMessage();
					$qb_payment_result['status'] = 'error';
				}
				if ($qb_payment_result['status'] == 'success') {
					$qb_payment_id = $qb_payment_result['qb_payment_id'];
					$query4 = "UPDATE `return_main` SET `qb_id`='$qb_payment_id', `qb_status`=1 WHERE `invoice_no`='$invoice_no'";
					$result4 = mysqli_query($conn, $query4);
					if (!$result4) {
						$qb_msg = 'Quickbooks payment id update error.';
					}
        ...
        $qb_msg = "Error: " . $e->getMessage();
    }

    return $qb_msg;
}

function processQBRefundReturnInvoice($bill_total_minus, $comment, $qb_cust_id, $invoice_no) {
    try {
		include('config.php');
		$amount = abs($bill_total_minus);
		$qb_payment_ref_number = $comment;
		$qb_msg = $qb_account = "";
		...
		if(!$qb_account == ''){
				...
				if ($result3 && mysqli_num_rows($result3) > 0) {
					...
					$refund_array = array(
						...
						'qb_payment_ref_number' => $qb_payment_ref_number
					);
					$qb_result = QBRefundReceipt($refund_array);
					$qb_msg = $qb_result['message'];
				}
		}
    ...
    if ($qb_result['status'] == 'success') {
        $qb_refund_id = $qb_result['qb_refund_id'];
        $query4 = "UPDATE `return_main` SET `qb_id`='$qb_refund_id', `qb_status`=1 WHERE `invoice_no`='$invoice_no'";
        $result4 = mysqli_query($conn, $query4);
        if (!$result4) {
            $qb_msg = 'Quickbooks refund id update error.';
        }
    }
    return $qb_msg;
}

function processQBRefundInvoice($bill_total_minus, $comment, $amount_cash, $qb_cust_id, $invoice_no) {
    try {
		include('config.php');
        $qb_account = 188;
        $amount = abs($bill_total_minus);
    ...
    if ($qb_result['status'] == 'success') {
        $qb_refund_id = $qb_result['qb_refund_id'];
        $qb_doc_id = $qb_result['qb_refund_id'];
        $query4 = "UPDATE `bill_main` SET `qb_doc_id`='$qb_doc_id', `qb_txnid`='$qb_refund_id', `qb_status`=1 WHERE `invoice_no`='$invoice_no'";
        $result4 = mysqli_query($conn, $query4);
        if (!$result4) {
            $qb_msg = 'Quickbooks refund id update error.';
        }
...
function processQBCashPayment($qb_cust_id, $amount_cash, $qb_txnid, $paymnet_id, $comment, $invoice_no) {
    $qb_msg = $qb_account = "";
	$qb_payment_ref_number = $comment;
    try {
		include('config.php');
		...
		if($qb_account != ''){
			...
			if ($result3 && mysqli_num_rows($result3) > 0) {
				...
				if($invoice_no == 0){
					$payment_array = array('qb_cust_id' => $qb_cust_id, 'total_amount' =>
					$amount_cash,'qb_payment_method_id'=>$qb_payment_method_id, 'qb_payment_ref_number' => $qb_payment_ref_number, 'qb_account_id' => $qb_account);
				}else{
					$payment_array = array('qb_cust_id' => $qb_cust_id, 'amount'=> $amount_cash, 'total_amount' => $amount_cash,
					'qb_invoice_id'=> $qb_txnid, 'qb_payment_method_id'=>$qb_payment_method_id, 'qb_payment_ref_number' =>
					$qb_payment_ref_number, 'qb_account_id' => $qb_account);
				}
				...
				if ($qb_payment_result['status'] == 'success') {
					$qb_payment_id = $qb_payment_result['qb_payment_id'];
					$query5 = "UPDATE `payment` SET `qb_payment_id`='$qb_payment_id', `qb_status`=1 WHERE id='$paymnet_id'";
					$result5 = mysqli_query($conn, $query5);
					if (!$result5) {
						$qb_msg = 'Quickbooks payment id update error.';
					}
...
function processQBBankPayment($account_id, $qb_cust_id, $amount_bank, $qb_txnid, $paymnet_id, $comment, $invoice_no) {
    $qb_msg = "";
	$qb_payment_ref_number = $comment;
    try {
		include('config.php');
        $query3 = "SELECT `qb_id` FROM qb_payment_methods WHERE `name` = 'Bank Transfer'";
        $result3 = mysqli_query($conn2, $query3);
        ...
        if ($result3 && mysqli_num_rows($result3) > 0) {
            ...
            $query1 = "SELECT `qb_account_id` FROM accounts WHERE id='$account_id'";
            $result1 = mysqli_query($conn2, $query1);

            if ($result1 && mysqli_num_rows($result1) > 0) {
                $row1 = mysqli_fetch_row($result1);
                $qb_account_id = $row1[0];

                if ($qb_account_id != '') {
					if($invoice_no == 0){
						$payment_array = array('qb_cust_id' => $qb_cust_id, 'amount'=> $amount_bank, 'total_amount' => $amount_bank, 'qb_account_id'=> $qb_account_id,'qb_payment_method_id'=>$qb_payment_method_id, 'qb_payment_ref_number' => $qb_payment_ref_number);
					}else{
						$payment_array = array('qb_cust_id' => $qb_cust_id, 'amount'=> $amount_bank, 'total_amount' => $amount_bank, 'qb_invoice_id'=> $qb_txnid, 'qb_account_id'=> $qb_account_id,'qb_payment_method_id'=>$qb_payment_method_id, 'qb_payment_ref_number' => $qb_payment_ref_number);
					}

                    try {
                        $qb_bank_payment_result = QBAddPayment($payment_array);
                        $qb_msg = $qb_bank_payment_result['message'];
                    } catch (Exception $e) {
                        $qb_msg = "QuickBooks error: " . $e->getMessage();
                        $qb_bank_payment_result['status'] = 'error';
                    }

                    if ($qb_bank_payment_result['status'] == 'success') {
                        $qb_payment_id2 = $qb_bank_payment_result['qb_payment_id'];
                        $query6 = "UPDATE `payment` SET `qb_payment_id`='$qb_payment_id2', `qb_status`=1 WHERE id='$paymnet_id'";
                        $result6 = mysqli_query($conn, $query6);
                        if (!$result6) {
                            $qb_msg = 'Quickbooks payment id update error.';
                        }
...
function processQBCardPayment($card_bank, $comment, $qb_cust_id, $amount_card, $qb_txnid, $paymnet_id, $invoice_no) {
    $qb_msg = "";
	...
	try{
		include('config.php');
		$query3 = "SELECT `qb_id` FROM qb_payment_methods WHERE `name` = 'Credit Card'";
		$result3 = mysqli_query($conn2, $query3);
		...
		if ($result3 && mysqli_num_rows($result3) > 0) {
			...
			// Retrieve qb_account_id
			$query1 = "SELECT `qb_account_id` FROM accounts WHERE id='$card_bank'";
			$result1 = mysqli_query($conn2, $query1);
			...
			if (($result1) && (mysqli_num_rows($result1) > 0)) {
				...
				if ($qb_account_id != '') {
					if($invoice_no == 0){
						$payment_array = array('qb_cust_id' => $qb_cust_id, 'amount'=> $amount_card, 'total_amount' => $amount_card, 'qb_account_id'=> $qb_account_id,'qb_payment_method_id'=>$qb_payment_method_id, 'qb_payment_ref_number' => $qb_payment_ref_number);
					}else{
						$payment_array = array('qb_cust_id' => $qb_cust_id, 'amount'=> $amount_card, 'total_amount' => $amount_card, 'qb_invoice_id'=> $qb_txnid, 'qb_account_id'=> $qb_account_id,'qb_payment_method_id'=>$qb_payment_method_id, 'qb_payment_ref_number' => $qb_payment_ref_number);
					}
					try {
						$qb_bank_payment_result = QBAddPayment($payment_array);
						$qb_msg = $qb_bank_payment_result['message'];
	...
	if($out){
		...
		if($emptyamount){
			...
			if($ocation == 'pay'){
				if(isQuickBooksActive(1)){
					$qb_cust_id = getCustomerQBId($cust);
					$query1="SELECT  bm.`qb_txnid` FROM bill_main bm WHERE bm.`invoice_no`='$invoice_no'";
					$row1=mysqli_fetch_row(mysqli_query($conn2,$query1));
					$qb_txnid = $row1[0];

					if(($qb_cust_id != '')){ // already customer register in quickbooks
						if($payment_type2 == 1){ // cash
							$qb_msg = processQBCashPayment($qb_cust_id, $amount_cash, $qb_txnid, $payment_id, $comment, $invoice_no);
						}
						if($payment_type2 == 3){ // bank
							$amount_bank =  $amount_cash;
							$qb_msg = processQBBankPayment($tr_bank, $qb_cust_id, $amount_bank, $qb_txnid, $payment_id, $comment, $invoice_no);
						}
						if($payment_type2 == 4){ // card
							$amount_card = $amount_cash;
							$qb_msg =processQBCardPayment($card_bank, $comment, $qb_cust_id, $amount_card, $qb_txnid, $payment_id, $invoice_no);
						}
					}
				}
			}
...
function deletePayment($n,$force){
	...
	if($out){
		if(isQuickBooksActive(1)){
			$query1="SELECT `qb_payment_id` FROM payment WHERE id='$payment_id'";
			$row1=mysqli_fetch_row(mysqli_query($conn,$query1));
			if(!empty($row1)){
				$qb_payment_id=$row1[0];
				if(($qb_payment_id != '') && ($qb_payment_id != 0)){
					try{
					...
					if($qb_result['status'] == 'success'){
						$qb_result_payment_id = $qb_result['qb_result_payment_id'];
						$query2="UPDATE `payment` SET `qb_status`=0 WHERE `qb_payment_id`='$qb_result_payment_id'";
						$result2=mysqli_query($conn,$query2);
						if(!$result2){
...
function finalizeReturn(){
	...
	$result=mysqli_query($conn,$query);
	$qb_cust_id = $cust_id = $extra_pay = $qb_msg = '';

	if($result){
		if(isQuickBooksActive(1)){
			$query1 = "SELECT rm.cust, SUM(r.extra_pay) FROM return_main rm, `return` r WHERE rm.invoice_no = r.invoice_no AND r.invoice_no = '$id' AND r.extra_pay != 0";
			$row1 = mysqli_fetch_row(mysqli_query($conn, $query1));
			$cust_id = $row1[0];
			$extra_pay = $row1[1];
			...
			try{
				if($qb_cust_id != ''){
					if($extra_pay > 0){
						$comment = "Return extra pay";
						$qb_msg =  processQBCashPaymentReturnInvoice($qb_cust_id, $extra_pay, $comment, $id);
					}else{
						$comment = "Return refund";
						$qb_msg = processQBRefundReturnInvoice($extra_pay, $comment, $qb_cust_id, $id);
					}
				}
			}catch (Exception $e) {
				$qb_msg = "<br>QuickBooks error: " . $e->getMessage();
