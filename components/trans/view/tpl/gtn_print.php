...
<table width="100%">
	...
	<?php
	if (isset($gtn_comment) && !empty($gtn_comment)) { ?>
		<tr>
			<td>
				<?php
				print '<div style="text-decoration:none; color:black; display:block; width: max-content; max-width: 400px"><span style="font-weight: bold">Note : </span>' . $gtn_comment . '</div>';
				?>
			</td>
		</tr>
	<?php } ?>
...
<table align="center" height="<?php print $page_height; ?>px" width="100%" border="1" cellspacing="0">
	...
	for ($i = 0; $i < sizeof($gtn_item_id); $i++) {
		...
		print '<td style="' . $systemid13Styles . ' border-top:0; padding:0 10px; ">' . sprintf('%02d', ($i + 1)) . '. ' . $gtn_item_des[$i] . '</td>' . $table_u_cost . '
		<td style="' . $systemid13Styles . ' border-top:0; padding-right:10px" align="right">' . number_format($gtn_item_qty[$i]) . '&nbsp;' . $gtn_item_unit[$i] . '';
		if ($systemid == 13 && $sub_system == 1) {
			print '<td style="' . $systemid13Styles . ' border-top:0; padding:0 10px;" width="100px"></td>';
		}
		print '</td>' . $table_total_cost . '</td>
		</tr>';
	}
	print '<tr style="font-size:10pt; border-top:0;">';
