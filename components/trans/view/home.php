...
<script type="text/javascript">
	...
	function finalize1() {
		...
		window.location = 'index.php?components=trans&action=finish_gtn&id=<?php if (isset($_GET['id']))
			print $_GET['id']; ?>&approve_permission=0';
	}
</script>
		...
		<tr>
			<td valign="top">
				<table width="100%">
					<tr>
						<td>
							<h1 style="color:orange">Stores Transfer</h1>
						</td>
						<td align="right" style="font-weight:bold; font-size:12pt;">TR No:
							<?php print str_pad($id, 7, "0", STR_PAD_LEFT); ?>
						</td>
					</tr>
				</table>
				<table align="center" bgcolor="#E5E5E5">
					...
					<tr>
						...
						<td><input type="number" name="qty1" id="qty1" onfocus="getPrice()" style="width:50px" <?php if (sizeof($unic_item_list) > 0) { ?> value="1" disabled="disabled" <?php } ?> /></td>
						<td>
					...
					<tr>
						<td width="50px"></td>
						<td style="font-size:12pt">Remote Store</td>
						<td colspan="2">
							<?php
					...
					<tr>
						<td width="50px"></td>
						<td style="font-size:12pt">Drawer No</td>
						<td colspan="2">
					...
					<?php if ($systemid == 26) { ?>
						...
						<tr>
							<td></td>
							<td style="font-size:12pt">New Cost</td>
							<td colspan="2"><input type="number" id="new_c_price" name="new_c_price" value="" /></td>
							<td></td>
						</tr>
					<?php } ?>
					<tr>
					...
					<tr>
						<td colspan="2" align="right">
							<div id="addtogtn">
								<?php if (sizeof($gtn_itemid) < $apend_limit)
									print '<input type="submit" value="Add to GTN" style="width:100px; height:30px" />'; ?>&nbsp;
							</div>
						</td>
						<td colspan="3" align="left">
							<div id="finalize1"><?php if (sizeof($gtn_itemid) > 0) { ?><input type="Button" value="Finalize"
										style="width:100px; height:70px" onclick="finalize1()" /><?php } ?></div>
							<br /><br />
						</td>
					</tr>
					<tr>
					...
					<tr>
						<td colspan="5">
							<div style="display: flex; flex-direction: column; gap: 10px;">
								<textarea name="comment" id="comment" style="padding: 8px; font-family: Arial; font-size: 12px;"><?php if (isset($gtn_comment))
									echo $gtn_comment; ?></textarea>
								<button type="button" onclick="updateCommentForm()" id="updateBtn"
									style="padding: 10px; cursor: pointer; width: 100%;">Update
									Comment</button>
							</div>
						</td>
					</tr>
				...
				<table align="center" bgcolor="#E5E5E5" height="100%">
					...
					for ($i = 0; $i < sizeof($gtn_itemid); $i++) {
						...
						if ($tc > -1) {
							...
							$allow_remove = true;
						if ($gtn_no_update[$i] == 0)
							$update_button = '<input type="Button" value="Update"  onclick="updateGTN(' . $gtn_itemid[$i] . ')" />';
						else
							$update_button = '<input type="Button" value="Update" onclick="alert(' . "'Update is Restricted for this item'" . ')" />';
						if ($allow_remove)
							$remove_button = '<input type="Button" value="Remove"  onclick="removeGTN(' . $gtn_itemid[$i] . ')" style="background-color:maroon; color:white"/>';
						else
							$remove_button = '';
...
<script>
	function updateCommentForm() {
		...
		form.method = 'POST';
		form.action = 'index.php?components=trans&action=update_gtn_comment';

		form.innerHTML = `
		<input type="hidden" name="gtn_no" value="<?php if (isset($_GET['id']))
			print $_GET['id']; ?>">
		<input type="hidden" name="remotestore" value="<?php if (isset($_GET['remotestore']))
			print $_GET['remotestore']; ?>">
		<input type="hidden" name="comment" value="${document.getElementById('comment').value}">
	`;
