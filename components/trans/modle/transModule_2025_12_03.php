...
function generateGTN()
{
	...
	$gtn_to_user, $gtn_c_price, $gtn_cross_invoice, $total_cost, $gtn_status, $gtn_from_shop_name, $gtn_item_unit;
	$gtn_no = $_REQUEST['id'];
	$user_id = $_COOKIE['user_id'];
	$gtn_from = $gtn_to = '';
	$total_cost = 0;
	...
	$query = "SELECT tr.`id`, inv.`description`, tr.`qty`, itq.`drawer_no`, tm.`from_store`, tm.`to_store`, DATE(tm.`date`), $user_name, tm.`remote_user`,
	 $price_column_name, tm.`status`, tr.`sn_list`, inv.`unic`, tm.`invoice_no`, inv.`unit`
