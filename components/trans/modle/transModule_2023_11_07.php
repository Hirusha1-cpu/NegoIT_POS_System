...
function getStores($systemid,$sub_system){
	...
	if($systemid==13) $sub_sys_qry="AND sub_system ='$sub_system'";
...
global $gtn_no,$gtn_item_id,$gtn_item_des,$gtn_item_qty,$gtn_item_draw,$gtn_item_from,$gtn_item_to,$gtn_date,$gtn_from_user,$gtn_to_user,$gtn_c_price,$gtn_cross_invoice,$total_cost,$gtn_status;
	$gtn_no=$_REQUEST['id'];
	...
	if($isMobile)	include('config.php');	else include('../../../../config.php');
		...
		$query="SELECT tr.id,inv.description,tr.qty,itq.drawer_no,tm.from_store,tm.to_store,date(tm.date),up.username,tm.remote_user,tr.c_price,tm.`status`,tr.sn_list,inv.unic,tm.invoice_no FROM transfer_main tm, transfer tr, inventory_items inv, inventory_qty itq, userprofile up WHERE tm.gtn_no=tr.gtn_no AND tm.user=up.id AND tr.item=inv.id AND inv.id=itq.item AND itq.location=tm.from_store AND tm.gtn_no='$gtn_no'";
		$result=mysqli_query($conn,$query);
		while($row=mysqli_fetch_array($result)){
			$trn_id_tmp=$row[0];
			$gtn_item_id[]=$row[0];
