...
function getCustMore(){
	...
	$result = mysqli_query($conn2,"SELECT COUNT(id) AS `count` FROM payment WHERE payment_type=2 AND cust='$id' AND `status`=0 AND chque_return=1");
	$row = mysqli_fetch_assoc($result);
	$chq2_retuned=$row['count'];
	$result = mysqli_query($conn2,"SELECT COUNT(id) AS `count` FROM payment WHERE payment_type=2 AND cust='$id' AND `status`=0 AND chque_postpone=1");
	$row = mysqli_fetch_assoc($result);
	$chq2_postpone=$row['count'];
	$result = mysqli_query($conn2,"SELECT COUNT(id) AS `count` FROM payment WHERE payment_type=2 AND cust='$id' AND `status`=0 AND chque_clear=1");
	$row = mysqli_fetch_assoc($result);
