...
<div class="wrap">
	...
	<div>
		...
		<table align="center" class="styled-table" style="margin-top:0px" width="720px">
			<thead>
				<tr>
					<td colspan="5" style="color: black; background: #dddddd;" class="td-style"><strong>Today Invoices [Completed]</strong></td>
				</tr>
				<tr>
					<th width="20px">#</th>
			...
			<tbody>
				...
				$inv=0;
					for($i=0;$i<sizeof($invoice_no);$i++){
						if($bi_discount[$i]>0){ 
							$color1='style="color:red"'; $title='title="Discounted Invoice"'; 
						}else{ 
							$color1=''; $title=''; 
						}
						$key=array_search($invoice_no[$i],$py_invno);
						$color2='black';
						if($bm_sys_user[$i]==$user_id) $color='#F2F3F4'; else $color='';
						if($key>-1){ 
							if($py_type[$key]==1) $color2='#009900'; 
							if($py_type[$key]==2) $color2='blue';
							if($py_type[$key]==3) $color2='#00AAAA';
						}
						print '<tr id="owner" bgcolor="'.$color.'" '.$color1.' class="a">
								<td>'.($i+1).'</td>
								<td align="center"><a target="_blank" '.$title.' href="index.php?components=billing&action=finish_bill&id='.$invoice_no[$i].'">'.str_pad($invoice_no[$i], 7, "0", STR_PAD_LEFT).'</a></td>
	...
	<div>
		<table align="center" border="0"  class="styled-table" width="720px">
			<thead>
				<tr>
					<td colspan="5" style="color: black; background: #dddddd;" class="td-style"><strong style="padding-left: 10px">Pending Invoices</strong></td>
				</tr>
				<tr>
					<th width="20px">#</th>
					<th width="120px" align="center">Invoice No</th>
			...
			<tbody>
				...
				for($i=0;$i<sizeof($linvoice_no);$i++){
					...
					if(($i%2)==0) $class='active'; else $class='';
					...
					print '<tr>
							<td>'.($i+1).'</td>
							<td align="center"><a target="_blank" href="index.php?components=billing&action=finish_bill&id='.$linvoice_no[$i].'">'.str_pad($linvoice_no[$i], 7, "0", STR_PAD_LEFT).'</a>'.$edit.'</td>
	...
	<div>
		<table align="center" class="styled-table" width="720px">
			<thead>
				<tr>
					<td colspan="5" style="color: black; background: #dddddd;" class="td-style"><strong style="padding-left: 10px">Payment Invoices</strong></td>
				</tr>
				<tr>
					<th width="20px">#</th>
					<th width="120px" align="center">Invoice No</th>
			...
			<?php
				...
				for($i=0;$i<sizeof($py_id);$i++){
					if($py_bill_pay[$i]==2){
						if($py_sys_user[$i]==$user_id) $color='#F2F3F4'; else $color='';
						if($py_type[$i]==1) $color2='#009900'; 
						if($py_type[$i]==2) $color2='blue';
						if($py_type[$i]==3) $color2='#00AAAA';
						print '<tr bgcolor="'.$color.'">
								<td>'.($i+1).'</td>
								<td align="center"><a target="_blank" href="index.php?components=billing&action=finish_payment&id='.$py_id[$i].'">'.str_pad($py_id[$i], 7, "0", STR_PAD_LEFT).'</a></td>
								<td align="center">'.$py_time[$i].'</td>
	...
	<div>
		<table align="center" class="styled-table" width="720px">
			<thead>	
				<tr>
					<td colspan="4" style="color: black; background: #dddddd;" class="td-style"><strong style="padding-left: 10px">Return Invoices</strong></td>
				</tr>
				<tr>
					<th width="20px">#</th>
					<th width="120px" align="center">Invoice No</th>
			...
			<tbody>
				...
				$inv=0;
					for($i=0;$i<sizeof($rt_invno);$i++){
						...
						if($rt_return_by[$i]==$user_id) $color='#F2F3F4'; else $color='';

						print '<tr bgcolor="'.$color.'">
								<td>'.($i+1).'</td>
								<td align="center"><a target="_blank" href="index.php?components=billing&action=finish_return&id='.$rt_invno[$i].'">'.str_pad($rt_invno[$i], 7, "0", STR_PAD_LEFT).'</a>&nbsp;&nbsp;&nbsp;&nbsp;'.$edit.'</td>
								<td align="center">'.$rt_time[$i].'</td>
	...
	<div>
		<table align="center" class="styled-table" width="720px">
			<thead>
				<tr>
					<td colspan="5" style="color: black; background: #dddddd;" class="td-style"><strong style="padding-left: 10px">Warranty Invoices</strong></td>
				</tr>
				<tr>
					<th width="20px">#</th>
					<th width="120px" align="center">Invoice No</th>
			...
			<tbody>
				...
				for($i=0; $i<sizeof($w_invno); $i++){
					print '
						<tr>
							<td>'.($i+1).'</td>
							<td align="center"><a target="_blank" href="index.php?components=billing&action=finish_bill&id='.$w_invno[$i].'">'.str_pad($w_invno[$i], 7, "0", STR_PAD_LEFT).'</a></td>
							<td align="center"><a target="_blank" href="index.php?components=billing&action=warranty_show&id='.$w_no[$i].'">'.str_pad($w_no[$i], 7, "0", STR_PAD_LEFT).'</a></td>
							<td align="center">'.$w_time[$i].'</td>
							<td>'.$w_cust_name[$i].'</td>
						</tr>
					';
				}
