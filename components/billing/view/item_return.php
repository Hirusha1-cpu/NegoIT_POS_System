...
?>
	...
	function getReplacement(){
	  ...
	  document.getElementById('replace_itemid').value=$replace_itemid;
		if(replacesn==''){
			  ...
			  xhttp.open("GET", 'index.php?components=billing&action=get_replacementsn&replace_itemid='+$replace_itemid, true);
			  xhttp.send();
		}
	}
	...
	function getPriceDiff(){
	  ...
	  if(isNaN($qty)){
		  ...
		  xhttp.open("GET", 'index.php?components=billing&action=get_pricediff&return_itemid='+$return_itemid+'&replace_itemid='+$replace_itemid, true);
		  xhttp.send();
	  }
	}
	...
	function selectCust(){
		...
		if(custname!=''){
			...
			window.location = 'index.php?components=billing&action=new_return&cust='+custid_arr[a]+'&gps_x='+gps_x+'&gps_y='+gps_y;		
		}
	}
	...
	function getReturn(){
		...
		if(itemcode!=''){
			...
			if((unic[a]==1)&&(itemcode!=itm_tmp)){
				...
				window.location = 'index.php?components=billing&action=item_return&id='+invoice_no+'&cust='+invoice_cust+'&itemid='+itemid[a]+'&unic=yes';
			}
			if((unic[a]==0)&&(unic_list_size>0)){
				document.getElementById('addtoreturn').innerHTML='';
				window.location = 'index.php?components=billing&action=item_return&id='+invoice_no+'&cust='+invoice_cust+'&itemid='+itemid[a]+'&unic=no';
			}
		}else if(itemdesc!=''){
	...
	function finalize(){
		...
		window.location = 'index.php?components=billing&action=finalize_return&id=<?php if(isset($_GET['id'])) print $_GET['id']; ?>'
	}
...
<table align="center">
<tr><td valign="top">
	<form action="index.php?components=billing&action=apend_return" onsubmit="return validateReturn()" method="post" >
	...
	<table height="100%" style="color:#0158C2; font-family:Calibri; font-size:16pt; vertical-align:middle;"><tr><td><strong>Item Return</strong></td></tr></table>
	</div>
	...
	<?php }else{ ?>
	<tr><td width="50px"></td><td style="font-size:12pt">Return Item Code</td><td colspan="2"><input type="text" name="code" id="tags1" value="<?php if($unic_item_code!='') print $unic_item_code; ?>" onclick="this.value=''" /></td><td width="50px"></td></tr>
	<tr><td width="50px"></td><td style="font-size:12pt">Return Item Description</td><td colspan="2"><input type="text" name="description" id="tags2" /></td><td width="50px"></td></tr>
	<tr><td></td><td style="font-size:12pt">Quantity</td><td><input type="number" name="qty1" id="qty1" onfocus="getReturn()" style="width:50px" <?php if($unic_item_select){ ?> value="1" disabled="disabled" <?php }?> /></td><td><div style="font-size:10pt;" id="av_qty" align="right"></div><div style="font-size:10pt; color:#CC0000" id="tt_qty" align="right"></div></td><td></td></tr>
	...
	<tr><td width="50px"></td><td style="font-size:12pt">Replacement Item Code</td><td colspan="2"><input type="text" name="rplace_code" id="tags6" value="<?php if($unic_item_code!='') print $unic_item_code; ?>" onclick="this.value=''" /></td><td width="50px"></td></tr>
	<tr><td width="50px"></td><td style="font-size:12pt">Replacement Item Description</td><td colspan="2"><input type="text" name="rplace_description" id="tags7" /></td><td width="50px"></td></tr>
	<?php if($unic_item_select){
	...
	<tr><td colspan="5"><hr /></td></tr>
	<tr><td width="50px"></td><td style="font-size:12pt">Extra Payment</td><td colspan="2"><input type="text" name="extra_pay" style="width:50px" onfocus="showCodeDesc()" /><input type="button" value="Get Cost Difference" onclick="getPriceDiff()" /></td><td width="50px"><div id="price_diff" style="font-size:12pt; color:red"></div></td></tr>
	<tr><td colspan="2" align="right"><br /><div id="addtoreturn"><?php if(sizeof($rt_id)<16) print '<input type="submit" value="Add to Return" style="width:100px; height:30px" onclick="showCodeDesc()" />'; ?><br /><br /><br /><br /></div>
	<?php if(sizeof($rt_id)>0) { ?>
		</td><td colspan="3" align="left"><div id="finalize"><input type="Button" value="Finalize" style="width:100px; height:70px" onclick="finalize()" /></div>
	<?php } ?>
	<br />
	<br /></td></tr>
	<tr><td colspan="5" align="center" style="font-size:9pt; color:gray"><i>The Extra Payment Need to Be Collected As the Same Time of Issuewing this Invoice</i></td></tr>
	<?php } ?>
	</table>
...
<!-- ------------------Item List----------------------- -->
<div style="background-color:#D1D8F1; border-radius: 5px; padding-left:20px; padding-right:20px; height:500px; width:400px" align="left">
<br />
<?php if(isset($_REQUEST['id'])) { ?>
<span style="font-weight:bold; font-size:12pt; border-radius:5px;  background-color:#E5E5EA; padding-left:15px; padding-right:15px" >Return Invoice No: &nbsp;&nbsp;&nbsp;<?php print str_pad($_REQUEST['id'], 7, "0", STR_PAD_LEFT); ?></span>
<br /><br /><br />
	...
	<?php
		for($i=0;$i<sizeof($rt_id);$i++){
			...
			<input type="Button" value="Remove"  onclick="removeReturn('.$rt_id[$i].',\'billing\')" style="background-color:maroon; color:white"/> 
			</td></tr>';
		} ?>
	</table>
<?php }else{	?>	
	<form id="searchinv" action="index.php?components=billing&action=search_return" method="post">
	<table height="100%">
	<tr><td style="font-size:12pt;"><input type="text" style="width:200px" name="search1" id="search1" placeholder="Search Return" /><input type="Submit" value="Search" /></td></tr>
	</table>
	</form>
<?php } ?>
</div>
