...
<script type="text/javascript">
	<?php  if(isset($_COOKIE['district'])){ ?>
		$(function() {
			<?php  if($systemid==1 || $systemid==4 || $systemid==10 || $systemid==15 || $systemid==17){ ?>
			...
			if(itemcode!=''){
				...
				if(itemid[a]!=storecrossitm){
					...
					<?php if($_COOKIE['direct_mkt']==1){ ?>
						document.getElementById('priceshow').innerHTML='<select name="price"><option value="r_'+rprice[a]+'">Retail - '+rprice[a]+'</option></select>';
					<?php }else{ ?>
						<?php if(($_COOKIE['retail']==0)||(($cust_odr=='yes')&&($systemid==16))){ ?>
						document.getElementById('priceshow').innerHTML='<select name="price"><option value="w_'+wprice[a]+'"><?php print $pricename1; ?> - '+wprice[a]+'</option><option value="r_'+rprice[a]+'"><?php print $pricename2; ?>- '+rprice[a]+'</option></select>';
						<?php }else{ ?>
						document.getElementById('priceshow').innerHTML='<input type="hidden" name="price" value="r_'+rprice[a]+'" /><select disabled="disabled"><option value="r_'+rprice[a]+'">Retail - '+rprice[a]+'</option><option value="w_'+wprice[a]+'">Wholesale - '+wprice[a]+'</option></select>';
						document.getElementById('price_div').innerHTML='<select name="price"><option value="r_'+rprice[a]+'">Retail - '+rprice[a]+'</option><option value="w_'+wprice[a]+'">Wholesale - '+wprice[a]+'</option></select>';
						<?php }
					} ?>
					...
					if(pr_sr[a]==2 || pr_sr[a]==3){
						document.getElementById('priceshow').innerHTML='<input type="text" name="price" value="w_'+wprice[a]+'" />';
		...
		xhttp.onreadystatechange = function() {
			...
			var returntext=this.responseText;
				...
				if(isNaN($values[2])){
					...
					document.getElementById('price_div').innerHTML='<select name="price"><option value="r_'+$values[0]+'">Retail - '+$values[0]+'</option><option value="w_'+$values[1]+'">Wholesale - '+$values[1]+'</option></select>';
				}
			}
		};
...
<?php } ?>
	...
	<?php
		if(isset($_GET['cust'])){
			...
			if($systemid==1 || $systemid==4 || $systemid==10 || $systemid==15 || $systemid==17)
	...
	<tr><td width="50px"></td><td style="font-size:12pt"></td><td colspan="2">
		...
		<?php if(!isset($_GET['cust'])){
			if(($_COOKIE['retail']==0)||($_GET['cust_odr']=='yes')){ ?><input type="button" value="Create Cust" onclick="window.location = 'index.php?components=billing&action=wholesale_cust&s=<?php print $_GET['s']; ?>&cust_odr=<?php print $cust_odr; ?>&mob='"  style="width:100px; height:50px;" /> <?php }
			else{ ?><input type="button" value="Create Cust" onclick="window.location = 'index.php?components=billing&action=onetime_cust&s=<?php print $_GET['s']; ?>&cust_odr=<?php print $cust_odr; ?>&mob='+document.getElementById('mob').value"  style="width:100px; height:50px;" /> <?php }
		} ?>
	<br /><br /></td><td width="50px"></td></tr>
	<?php }else{ ?>
	<?php if(($systemid==4)||($systemid==15)||($systemid==17)){ ?>
	...
	<tr><td width="50px"></td><td style="font-size:12pt"><table border="0"><tr><td>Unit Discount</td><td><div id="c" onclick="changeDiscount()" style="color:blue; cursor:pointer;"><?php if($discount=='price') print 'Rs'; else print '%'; ?></div></td></tr></table></td><td colspan="2"><table width="100%"><tr><td><input type="text" name="discount" id="discount" value="0" style="width:50px" onclick="this.value=''" /><input type="button" value="Check" onclick="loadDiscount('<?php print $_GET['cust']; ?>')" /></td><td><div style="font-size:10pt; color:red" id="div_discount" align="right"></div></td></tr></table></td><td width="50px"></td></tr>
...
?><br /></div><?php
	...
	<tr><td style="font-size:12pt;"><form id="searchinv" action="index.php?components=billing&action=search_bill&s=<?php print $_GET['s']; ?>&cust_odr=<?php print $_GET['cust_odr']; ?>" method="post" onsubmit="removeLeadingZeros()"><input type="text" style="width:100px" name="search1" id="search1" placeholder="Invoice Number" /><input type="Submit" value="Search" /></form></td></tr>
	...
	<tr><td style="font-size:12pt;"><form action="index.php"><input type="hidden" name="components" value="billing" /><input type="hidden" name="action" value="home" /><input type="hidden" name="s" value="<?php print $_GET['s']; ?>" /><input type="hidden" name="cust_odr" value="<?php print $_GET['cust_odr']; ?>" /><input type="text" style="width:100px" name="searchmob" placeholder="Mobile Number" /><input type="Submit" value="Search" /></form></td></tr>
	...
	if(isset($_GET['searchcust'])){
		print '<iframe id="search_frm" width="260px" height="350px" src="components/billing/view/tpl/search_bill.php?searchcust='.$_GET['searchcust'].'"></iframe>';
	}else if(isset($_GET['searchmob'])){
		print '<iframe id="search_frm" width="260px" height="350px" src="components/billing/view/tpl/search_bill.php?searchmob='.$_GET['searchmob'].'"></iframe>';
	}else if(isset($_GET['searchunic'])){
		print '<iframe id="search_frm" width="260px" height="350px" src="components/billing/view/tpl/search_bill.php?searchunic='.$_GET['searchunic'].'"></iframe>';
	}
	?>
<?php } ?>
</td></tr>
