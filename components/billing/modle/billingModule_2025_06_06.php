...
// update by nirmal 12_02_2024 (added bill time chosen value if it is retail price or wholesale price (type))
// updated by nirmal 19_11_2024 (if quickbooks enable then cannot add item qty mix - qty with + qty)
// updated by nirmal 27_01_2025 (added if, cust order try to add -qty that time not update inventory)
...
function apendBill($case, $systemid, $storecrossitm, $storecrossst)
{
	...
	/*
				if ($out && isQuickBooksActive(1)) {
					...
					// Query to fetch existing items in the invoice
					$query = "SELECT qty FROM bill WHERE invoice_no='$invoice_no'";
					$result = mysqli_query($conn2, $query);
	...
	if ($out && isQuickBooksActive(1)) {
		...
		// -------------------------------------------
		if ($out) {
			$sqlCross = "SELECT item, qty FROM bill WHERE invoice_no='$bm_no'";
			...
			elseif ($mode === "pos") {
				if ($newQty >= 0) {
					...
					$sqlItem = "SELECT SUM(qty) AS sumForItem
													FROM bill
													WHERE invoice_no='$bm_no' AND item='$itemId'";
					...
					if ($result) {
						if ($qty != 0) {
							...
							if (($pr_sr == 1) && ($itemid == $storecrossitm))
								crossTransfer($invoice_no, $storecrossitm, $storecrossst, $qty);
...
// update by nirmal 07_02_2024 (null handling)
// updated by nirmal 19_11_2024 (if quickbooks enable then cannot update mix - qty with + qty)
// updated by nirmal 02_02_2025 (added if, cust order try to add -qty that time not update inventory)
	...
	/*
																						if (isQuickBooksActive(1)) {
																							...
																							if (isset($qty)) {
																								...
																								if (($newItemQty > 0 && $check_qty < 0) || ($newItemQty < 0 && $check_qty > 0)) {
																									// Conflict in signs (one positive, the other negative)
																									if (($newItemQty > 0 && $check_qty < 0)) {
																										$msg = "Error: please only enter negative QTY";
																										$out2 = false;
																									}
																									if (($newItemQty < 0 && $check_qty > 0)) {
																										$msg = "Error: please only enter positive QTY";
																										$out2 = false;
																									}
																									$out2 = false;
																								}
																								// Enforce rules for subsequent items
																								if (($out2) && ($firstItemQty !== null)) {
																									// Check if the new item violates the rules
																									if (($firstItemQty > 0 && $newItemQty < 0) || ($firstItemQty < 0 && $newItemQty > 0)) {
																										$out2 = false;
																										$msg = "Error: Positive and negative quantity items cannot be added together.";
																									}
																								}
																								if ($qty == 0 || $qty == "") {
																									$out2 = false;
																									$msg = 'Error: Quantity cannot be zero!';
																								}
																							} else {
																								$out2 = false;
																								$msg = "Error: New item quantity is not defined.";
																							}
																						}
																						*/
...
// ------------------------------------------------------------
	...
	if ($out2 && isQuickBooksActive(1)) {
		$bm_no = $invoice_no; // Bill number.
		$newQty = (float) $qty;          // Quantity the user is trying to add.
		$itemId = (int) $itm_id;         // The item currently being added.
...
// update by nirmal 06_02_2025 (added, if bill qty is - $new_ivq_qty calculate differently, because -qty not affect to stock)
function removeBillitem()
{
	...
	$query = "SELECT bm.`invoice_+total` + bm.`invoice_-total` AS total  FROM  bill_main bm  WHERE bm.invoice_no='$invoice_no'";
	$row = mysqli_fetch_row(mysqli_query($conn, $query));
	$total_of_bill = $row[0];

	$query = "SELECT ivq.id,ivq.qty,bi.qty,(bi.unit_price+bi.discount),bi.cost,ivq.w_price,ivq.r_price,ivq.c_price,ivq.item,bi.no_update,bm.`lock`,bm.`type`,bm.`status`,bm.invoice_no
	...
	if (isQuickBooksActive(1) && ($total_of_bill == 0 || $total_of_bill == '')) {
		$out = false;
		$message = 'Item cannot be remove when bill is 0';
	}
	...
	if (($bm_lock == 0) || ($force_permit)) {
		...
		if (isQuickBooksActive(1) && (!$out)) {
			$message = 'Item cannot be remove when bill total is zero (0)';
			return false;
		} else {
...
function billDeletePermission($invoice_no)
{
	...
	if ((isset($_COOKIE['report'])) && ($bm_store == $store) && ($systemid == 4))
		$billpermission = true;
...
function calculateTotal()
{
	...
	if ($down_total == '')
		$down_total = 0;
	$message = 'Quantity of item successfully updated. The invoice total amount has been recalculated.';
	$out = true;
	$qb_result = "";
...
// updated by nirmal 23_01_2024 (add common file, case : sometimes in tpl we only call this php only, then isCustomInvoiceNoActive will not call because it is located in another file (common.php file))
...
function changeJobTotal()
{
	...
	if ($out) {
		...
		if ($row[0] == 1) {
			...
			$message = "Error: Cannot update a job contains more than 1 item!";
	}
...
// update by nirmal 04_10_2024 (added system id 17 sms sending code)
function sms()
{
	$invoice_no = $_REQUEST['id'];
	$timenow = timeNow();
...
// update by nirmal 04_03_2024 (quickbooks backend payment delete added)
function deletePayment($n, $force)
{
	global $message, $cust;
	$payment_id = $_REQUEST['id'];
	...
	if ($proceed) {
		try {
			...
			if (!$result) {
				throw new Exception("Error: Failed to update payment record in the database.");
			}
			...
			$message = 'Payment was deleted successfully!' . ' ' . $qb_msg;
			return true;
		} catch (Exception $e) {
...
function apendReturn()
{
	...
	if ($proceed) {
		if ($rm_status == 1) {
			...
			$proceed = false;
			$msg = 'Error: Cannot Add items to a Locked Invoice';
		}
	}
	if ($proceed) {
...
function getReturnChque($user)
{
	...
	if ($_GET['components'] == 'supervisor')
		...
		$storefilter = '';
	if (isset($_GET['cust']))
		$custfilter = "AND cu.`id`='" . $_REQUEST['cust'] . "'";
	else
		$custfilter = '';
