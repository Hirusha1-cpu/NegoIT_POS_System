			...
			if (isQuickBooksActive(1)) {
				...
				if ($paymentResult && mysqli_num_rows($paymentResult) > 0) {
					while ($paymentRow = mysqli_fetch_assoc($paymentResult)) {
						...
						$qb_status = $paymentRow['qb_status'];
						...
						if (($flag) && ($paymentRow['payment_type'] == 1) && ($qb_status !== null && $qb_status != '')) { // Cash
							...
							if (isset($journalEntryForDeletedPayment['error'])) {
								...
								$action_name = 'delete_payment_insert_through_bill';
								foreach ($journalEntryForDeletedPayment as $entry) {
									...
									$query = "INSERT INTO qb_queue (`batch_id`, `action`, `payment_id`, `posting_type`, `account_id`, `account_name`, `invoice_total`, `description`, `entity_type`, `entity_id`)
						...
						if (($flag) && ($paymentRow['payment_type'] == 2) && ($qb_status !== null && $qb_status != '')) { // Cheuqe
						...
						if (($flag) && ($paymentRow['payment_type'] == 3) && ($qb_status !== null && $qb_status != '')) { // Bank
						...
						if (($flag) && ($paymentRow['payment_type'] == 4) && ($qb_status !== null && $qb_status != '')) { // Card
							$accountNameQuery = "SELECT `name` FROM accounts WHERE id='$bank_trans'";
							$accountNameResult = mysqli_query($conn, $accountNameQuery);
							if ($accountNameResult && mysqli_num_rows($accountNameResult) > 0) {
								$accountNameRow = mysqli_fetch_row($accountNameResult);
					...
					$qb_msg = " QuickBooks error: No payment found to delete.";
					throw new Exception($qb_msg);
				}
			}
