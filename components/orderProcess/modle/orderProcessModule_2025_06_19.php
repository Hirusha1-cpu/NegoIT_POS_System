...
function apendBillOdr($case, $systemid, $storecrossitm, $storecrossst)
{
	...
	if ($qty_cannot_update) {
		...
		$query_purchased = "SELECT SUM(bi.qty) FROM bill_main bm JOIN bill bi ON bm.invoice_no=bi.invoice_no WHERE bm.`cust`='$cust' AND bi.item='$itemid' AND bm.`status`>2 AND bm.`lock`='1'";
...
function updateBillitemOdr()
{
	...
	if ($out) {
		if ($qty < 0) {
			...
			// 1. Calculate the total quantity purchased by the customer for this item.
			// This query remains the same as it correctly identifies purchases.
			$query_purchased = "SELECT SUM(bi.qty) FROM bill_main bm JOIN bill bi ON bm.invoice_no=bi.invoice_no WHERE bm.`cust`='$cust' AND bi.item='$bill_item' AND bm.`status`>2 AND bm.`lock`='1'";
			$result_purchased = mysqli_query($conn, $query_purchased);
           	...
           	AND r.`odr_packed`=1";
				...
				if ($result_returned === false) {
					...
					if ($net_available_qty < $current_return_qty_absolute) {
						...
						$msg = 'Error: This customer has only ' . $net_available_qty . ' items available for return (purchased: ' . $total_purchased_qty;

						// Add details about previously returned only if there were actual returns
						if ($total_returned_qty > 0) {
							$msg .= ', previously returned: ' . $total_returned_qty;
						}

						$msg .= '). You are trying to return: ' . $current_return_qty_absolute . '.';
					}
				}
			}
		}
...
function setStatus($method)
{
	...
	$qb_msg = $qb_cust_id = $qb_status = $qb_temp_cost = $custName = '';
	...
	try {
		...
		if ($out && (isQuickBooksActive(1))) {
			...
			$userQry = "SELECT c.`qb_cust_id`, bm.`qb_status`, bm.`qb_temp_cost`, c.`name`
				...
				if ($qb_status !== null && $qb_status != '') {
					if ($delivered_updated) {
						...
						$invoiceTotalQuery = "SELECT (bm.`invoice_+total` + bm.`invoice_-total`)
								FROM bill_main bm, bill b
								WHERE  bm.`invoice_no` = b.`invoice_no` AND bm.`invoice_no`='$invoice_no' AND bm.`qb_status` IS NOT NULL";
						...
						// tax
						$invoiceTaxQuery = "SELECT bm.`tax` FROM bill_main bm
                    WHERE bm.`invoice_no`='$invoice_no' AND bm.`qb_status` IS NOT NULL";
								...
								if ($bill_tax != 0) {
									...
									if (isset($journalEntryForInvoiceTax['error'])) {
										...
										foreach ($journalEntryForInvoiceTax as $entry) {
											...
											$account_name = mysqli_real_escape_string($conn, $entry['account_name']);
											$amount = mysqli_real_escape_string($conn, $entry['amount']);
											// Use a unique variable name to avoid conflict with the outer $description
...
function processReturn()
{
	...
	try {
		...
		if (isQuickBooksActive(1) && $invrtn > 0) {
			...
			$journalEntry = buildJournalEntry($conn, abs($total), $debitAccountName, $creditAccountName, $description, $debitEntityType, $debitEntityID, $creditEntityType, $creditEntityID);
