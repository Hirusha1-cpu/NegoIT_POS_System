...
function getOrder($method){
	...
	if($method=='cust_odr'){
		$type='4,5'; $status='1,2'; $lock_qry="bm.`lock`='2'"; $menu_by='Picked By'; $menu_date='Picked Date'; $orderby='ORDER BY '.$orderby0;
	}
	if($method=='pending'){
		$type='1,2'; $status='1,2'; $lock_qry="bm.`lock`='1'";; $menu_by='Picked By'; $menu_date='Picked Date'; $orderby='ORDER BY '.$orderby0;
	}
	if($method=='picked'){
		$type='1,2,4,5'; $status='2'; $lock_qry="bm.`lock` IN (1,2)"; $menu_by='Picked By'; $menu_date='Picked Date'; $orderby="AND bm.seen_by='$user' ORDER BY bm.seen_timestamp DESC";
	}
	if($method=='packed'){
		$type='1,2,4,5'; $status='3'; $lock_qry="bm.`lock`='1'"; $menu_by='Packed By'; $menu_date='Packed Date'; $orderby="AND bm.seen_by!='' ORDER BY ".$orderby0;
	...
	if($method=='shipped'){
		$type='1,2,4,5'; $status='4'; $lock_qry="bm.`lock`='1'"; $menu_by='Shipped By'; $menu_date='Shipped Date'; $orderby="AND bm.seen_by!='' ORDER BY bm.seen_timestamp DESC";
	}
	if($method=='delivered'){
		$type='1,2,4,5'; $status='5'; $lock_qry="bm.`lock`='1'"; $menu_by='Delivered By'; $menu_date='Delivered Date'; $orderby="AND bm.seen_by!='' ORDER BY bm.seen_timestamp DESC";
		if($systemid == 13 && $sub_system == 1){
			$menu_by = 'Finished By';
...
function getOneOrder(){
	...
	$bm_status='';
	if($_GET['action']=='list_one_custodr' || $_GET['action'] == 'list_one') $order_by_qry='inq.drawer_no, bi.id'; else $order_by_qry='inv.description';
	$systemid = inf_systemid(1);
	$sub_system=$_COOKIE['sub_system'];
	include('config.php');
	...
	while($row=mysqli_fetch_array($result)){
		...
		$bm_store=$row[21];
		if($bm_status==1){
			$button='Pick'; $button_action='set_picked';
		}
		if($bm_status==2){
			$button='Packed'; $button_action='set_packed';
		}
		if($bm_status==3){
			$button='Shipped'; $button_action='set_shipped';
			if($systemid == 13 && $sub_system == 1){
				$button='Finished'; $button_action='set_delivered';
			}
		}
		if($bm_status==4){
			$button='Delivered'; $button_action='set_delivered';
			if($systemid == 13 && $sub_system == 1){
				$button='Finished'; $button_action='set_delivered';
			}
		}
...
// updated by nirmal 16_12_2024 (if quickBooksActive, settle the return qty from invoiced qty. eg: bill qty = -4, return qty = 4 then allow to pack)
function returnPacked(){
	...
	if(isQuickBooksActive(1)){
		$query1="SELECT `item`, `qty` FROM bill WHERE invoice_no='$odr_no'";
		$result1 = mysqli_query($conn2, $query1);
		if (!$result1) {
			print mysqli_error($conn2);
		}else{
			if (mysqli_num_rows($result1) > 0) {
				while ($itemRow = mysqli_fetch_array($result1)) {
					$query2="SELECT `cust` FROM bill_main WHERE invoice_no='$odr_no'";
					$row2=mysqli_fetch_row(mysqli_query($conn2,$query2));
					$bm_cust=$row2[0];

					$query3="SELECT rm.`cust`,rt.odr_packed,rt.replace_item,rt.qty,rm.mapped_inventory FROM return_main rm, `return` rt WHERE rm.invoice_no=rt.invoice_no AND rt.id='$id'";
					...
					$result4 = mysqli_query($conn2,"SELECT on_place_replace FROM stores WHERE id='$store'");
					$row4 = mysqli_fetch_assoc($result4);
					$on_place_replace=$row4['on_place_replace'];
					...
					if(($rt_packed == 0) && ($itemRow[0] == $replace_item) && ($itq_qty < $rt_qty)){
						if(abs($itemRow[1]) == $rt_qty){
							$query6="UPDATE `return` SET `odr_no`='$odr_no', `odr_packed`='1', `odr_packed_date`='$time_now', `odr_packed_by`='$user_id' WHERE id='$id'";
							$result6=mysqli_query($conn2,$query6);
							if($result6) print 'done'; else print 'error';
						}
					}else{
						if($on_place_replace==0){
							if(($itq_qty>=$rt_qty)&&($rt_packed==0)){
								$query8="UPDATE `inventory_qty` SET `qty`=qty-$rt_qty WHERE `id`='$itq_id'";
								$result8=mysqli_query($conn,$query8);
								if(!$result8) $on_place=false;
							}else{
								$on_place=false;
							}
						}
						if(($bm_cust==$rm_cust)&&($rt_packed==0)&&($on_place)){
							...
							if($result7) print 'done'; else print 'error';
						}else{
							print 'error';
						}
					}
		...
		$row=mysqli_fetch_row(mysqli_query($conn,$query));
		$bm_cust=$row[0];
		$query="SELECT rm.`cust`,rt.odr_packed,rt.replace_item,rt.qty,rm.mapped_inventory FROM return_main rm, `return` rt WHERE rm.invoice_no=rt.invoice_no AND rt.id='$id'";
		$row=mysqli_fetch_row(mysqli_query($conn,$query));
		$rm_cust=$row[0];
		$rt_packed=$row[1];
		$replace_item=$row[2];
...
function removeReturnPacked(){
	...
	if(($bm_cust==$rm_cust)&&($rt_packed==1)&&($on_place)&&($bm_lock==2)){
		...
		if($result5) print 'done'; else print 'error';
	}else{
		print 'error';
	}
}
...
function setStatus($method){
	...
	$result1 = mysqli_query($conn,"SELECT `status`,`type`,`mapped_inventory` FROM bill_main WHERE invoice_no='$invoice_no'");
	$row1 = mysqli_fetch_assoc($result1);
	$status=$row1['status'];
	$type=$row1['type'];
	$bm_mapped_inventory=$row1['mapped_inventory'];
	...
	if (isQuickBooksActive(1)) {
		...
		$userQry = "SELECT c.qb_cust_id FROM bill_main bm INNER JOIN cust c ON c.id = bm.cust WHERE bm.invoice_no = '$invoice_no'";
		$userResult = mysqli_query($conn2, $userQry);
	...
	if($out){
		...
		if(($method=='picked')&&($status==1)){
			$query="UPDATE `bill_main` SET `seen_by`='$salesman',`seen_timestamp`='$time_now',status='2' WHERE `invoice_no`='$invoice_no'"; $result=mysqli_query($conn,$query);
		}
		if(($method=='packed')&&($status==2)){
			$query="UPDATE `bill_main` SET $lock_qry `packed_by`='$salesman',`billed_timestamp`='$time_now',`packed_timestamp`='$time_now',status='3' WHERE `invoice_no`='$invoice_no'"; $result=mysqli_query($conn,$query);
		}
		if(($method=='shipped')&&($status==3)){
			$query="UPDATE `bill_main` SET `shipped_by`='$salesman',`shipped_timestamp`='$time_now',status='4' WHERE `invoice_no`='$invoice_no'"; $result=mysqli_query($conn,$query);
		}
		if($systemid == 13 && $sub_system == 1){
		...
		if(($method=='delivered')&&($status==4)){
			$query="UPDATE `bill_main` SET `deliverd_by`='$salesman',`deliverd_timestamp`='$time_now',status='5' WHERE `invoice_no`='$invoice_no'"; $result=mysqli_query($conn,$query);
		}
		...
		if($method=='packed' && $result){
			...
			if($type==4){
				$query3 = "SELECT bi.item,bi.qty,itm.pr_sr,itm.unic FROM bill bi, inventory_items itm WHERE bi.item=itm.id AND bi.invoice_no='$invoice_no'";
				...
				while ($row3 = mysqli_fetch_array($result3)) {
					...
					if ($itm_pr_sr == 1) {
						if ((($itm_unic == 1) && (!$unic_cal)) || ($itm_unic == 0)) {
							$query1 = "SELECT id FROM inventory_qty WHERE item='$item' AND `location`='$bm_mapped_inventory'";
							$row1 = mysqli_fetch_row(mysqli_query($conn, $query1));
							$itq_id = $row1[0];

							if($qty < 0){
								$query4 = "UPDATE `inventory_qty` SET `qty`=`qty`-$qty WHERE `id`='$itq_id'";
		...
		if(isQuickBooksActive(1)){
			...
			if($delivered_updated){
				$invoiceCostQuery="SELECT SUM(b.qty * b.cost), c.name, (bm.`invoice_+total` + bm.`invoice_-total`), bm.`qb_cost_id` FROM bill_main bm, bill b, cust c WHERE c.id = bm.cust AND bm.invoice_no = b.invoice_no AND bm.`invoice_no`='$invoice_no'";
				$invoiceCostRow=mysqli_fetch_row(mysqli_query($conn2,$invoiceCostQuery));
				if(!empty($invoiceCostRow)){
					$invoiceCost=$invoiceCostRow[0];
					$custName=$invoiceCostRow[1];
					$bill_total = $invoiceCostRow[2];
					$qb_cost_id = $invoiceCostRow[3];
					...
					if(($invoiceCost < 0)){
						...
						$journalEntryForInvoiceTotal = buildJournalEntry($conn2, abs($bill_total), $debitAccountName, $creditAccountName, $description, $debitEntityType, $debitEntityID, $creditEntityType, $creditEntityID);
						if (isset($journalEntryForInvoiceTotal['error'])) {
							...
							$action = "CUST ODR RETURN INVOICE, JOURNAL ENTRY BUILD (Invoice Total) : Invoice No: $invoice_no";
						...
						if (isset($journalEntryForInvoiceCost['error'])) {
							...
							$action = "CUST ODR RETURN INVOICE, JOURNAL ENTRY BUILD (Invoice Cost) : Invoice No: $invoice_no";
							$functionName = "setStatus";
					...
					if(($invoiceCost > 0)){
						...
						if (isset($journalEntryForInvoiceTotal['error'])) {
							...
							$action = "CUST ODR INVOICE, JOURNAL ENTRY BUILD (Invoice Total) : Invoice No: $invoice_no";
							$functionName = "setStatus";
					...
					if (mysqli_num_rows($result) > 0) {
						...
						$journalEntryForReturnItems = buildJournalEntry($conn2, $item_cost, $debitAccountName, $creditAccountName, $description, $debitEntityType, $debitEntityID, $creditEntityType, $creditEntityID);
						...
						if (isset($journalEntryForReturnItems['error'])) {
							...
							$action = "CUST ODR RETURN ITEMS, JOURNAL ENTRY BUILD (Item Cost) : Return Id : $return_id, Invoice No: $invoice_no";
							$functionName = "setStatus";

							$error_log_query="INSERT INTO `qb_debug_log` (`action`,`function`,`amount`,`debit_account_name`,`credit_account_name`,`journal_entry_description`,`debit_entity_type`,`debit_entity_id`,`credit_entity_type`,`credit_entity_id`,`qb_message`,`action_time`)
							VALUES ('$action','$functionName','$item_cost','$debitAccountName','$creditAccountName','$description','$debitEntityType','$debitEntityID','$creditEntityType','$creditEntityID','$qb_msg','$time_now')";
							$result=mysqli_query($conn,$error_log_query);
						}
					...
					$flag = false;
					$qb_msg = 'Error : SQL Error: ' . mysqli_error($conn2);

					$qb_msg = mysqli_real_escape_string($conn, $qb_msg);
					$action = "CUST ODR SQL ERROR : Invoice No: $invoice_no";
					$functionName = "setStatus";
...
function orderUnassign(){
	...
	if ((isset($_COOKIE['report']) && isset($_COOKIE['user_id'])) || (isset($_COOKIE['manager']) && isset($_COOKIE['user_id']))) {
		if ((isset($_COOKIE['report']) && $_COOKIE['report'] == $_COOKIE['user_id']) ||
