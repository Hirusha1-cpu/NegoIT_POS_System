...
function getOrder($method)
{
	...
	if ($method == 'list_cross_check') {
		$type = '1,2,4,5';
		$status = '10';
		$lock_qry = "bm.`lock` IN (1,2)";
		$menu_by = 'Picked By';
		$menu_date = 'Picked Date';
		$orderby = " ORDER BY bm.seen_timestamp DESC";
	}
	if ($method == 'list_my_cross_check') {
		$type = '1,2,4,5';
		$status = '11';
		$lock_qry = "bm.`lock` IN (1,2)";
		$menu_by = 'Picked By';
		$menu_date = 'Picked Date';
		$orderby = "AND bm.cross_checked_by='$user' ORDER BY bm.cross_check_timestamp DESC";
	}
	if ($method == 'packed') {
		$type = '1,2,4,5';
		$status = '3';
	...
	if (($method == 'picked')) {
		...
		bm.`invoice_no` = bi.`invoice_no` AND itm.`id`=bi.`item` AND bm.`lock` IN (1,2) AND bm.`mapped_inventory`='$store'
		AND bm.`status` IN (2) AND bm.`type` IN (1,2,4,5) AND bm.`seen_by`='$user' GROUP BY itm.`id`";
		$result = mysqli_query($conn2, $query);
		while ($row = mysqli_fetch_array($result)) {
			$bi_item_id[] = $row[0];
			$bi_item_desc[] = $row[1];
		...
		if ($row[0] == 0) {
			// get returned items
			$query = "SELECT itm.`id`,itm.`description`,SUM(rt.`qty`) FROM
			return_main rm, `return` rt, inventory_items itm WHERE
			rm.`invoice_no`=rt.`invoice_no` AND rt.`replace_item`=itm.`id` AND rm.`status`='2' AND rt.`odr_packed`='0'
			AND rm.`cust` IN (SELECT DISTINCT bm.`cust` FROM bill_main bm WHERE bm.`seen_by`='$user' AND bm.`status` IN ($status) AND bm.`type` IN ($type))
...
// updated by nirmal 25_09_2025 (added new cross check status)
function getOneOrder()
{
	global $button, $button_action, $odr_bill_id, $odr_bi_desc, $odr_bi_code, $odr_bi_qty, $odr_bi_price, $odr_total, $odr_ledc2, $odr_bi_drawer, $odr_bi_no_update,
	...
	bm.`status`,cu.id,bm.`type`,bm.`order_timestamp`,bm.billed_district,cu.id,bm.mapped_inventory,bm.store,bm.cross_checked_by
	,date(bm.cross_check_timestamp),time(bm.cross_check_timestamp)
	...
	while ($row = mysqli_fetch_array($result)) {
		...
		$bm_store = $row[21];
		if ($row[22] != '') {
			$bi_cross_checked_by = $salesman[$row[22]];
		}
		$bi_cross_checked_date = $row[23];
		$bi_cross_checked_time = $row[24];
		if ($bm_status == 1) {
			$button = 'Pick';
		...
		if (isOdrCrossCheckActive()) {
			if ($bm_status == 2) {
				$button = 'Move to Cross Check';
				$button_action = 'move_to_cross_check';
			}
		} else {
			if ($bm_status == 2) {
				$button = 'Packed';
		...
		if ($bm_status == 10) {
			$button = 'Cross Check Pick';
			$button_action = 'set_cross_check_start';
		}
		...
		$query2 = "SELECT bi.id,inv.description,bi.qty,bi.unit_price,inq.drawer_no,bi.no_update,bi.return_odr_replace,inv.code
...
function getPackedReturn()
{
	...
	include('config.php');
	$query = "SELECT rm.invoice_no,rt.id,date(rm.return_date),up.username,itm.code,itm.description,rt.qty,
	date(rt.process_date),rt.`status`
	FROM return_main rm, `return` rt, inventory_items itm, userprofile up
	WHERE rm.invoice_no=rt.invoice_no AND rt.replace_item=itm.id AND up.id=rm.return_by AND rt.odr_packed='1'
	AND rt.odr_no='$odr_id' AND rm.`status`='2' ORDER BY rm.invoice_no DESC";
	$result = mysqli_query($conn2, $query);
	while ($row = mysqli_fetch_array($result)) {
		$rtn2_inv[] = $row[0];
		$rtn2_id[] = $row[1];
...
function apendBillOdr($case, $systemid, $storecrossitm, $storecrossst)
{
	...
	if ($qty_cannot_update) {
		...
		$result = mysqli_query($conn, "SELECT `description` FROM inventory_items WHERE id='$itemid'");
		$row = mysqli_fetch_assoc($result);
		$inv_item_name = $row['description'];

		$query_purchased = "SELECT SUM(bi.qty) FROM bill_main bm JOIN bill bi ON bm.invoice_no=bi.invoice_no WHERE bm.`cust`='$cust' AND bi.item='$itemid' AND bm.`status`!=0 AND bm.`lock`='1'";

		$result_purchased = mysqli_query($conn, $query_purchased);
		if ($result_purchased === false) {
			$out = false;
			$message = 'Database query error (purchased quantity): ' . mysqli_error($conn);
		} else {
			$row_purchased = mysqli_fetch_row($result_purchased);
			// Data from the combined query
			$total_purchased_qty = $row_purchased && $row_purchased[0] !== null ? (int) $row_purchased[0] : 0;
        ...
        WHERE rm.cust = '$cust'
          ...
          AND rm.status IN (1, 2)";
			...
			if ($result_returns === false) {
				$out = false;
				$message = 'Database query error (returned quantity): ' . mysqli_error($conn);
			} else {
				$row_returns = mysqli_fetch_assoc($result_returns);
				// PHP 5 compatible check
				...
				// The quantity the user is attempting to return now.
				$current_return_qty_absolute = abs($qty);
				...
				// Now, check if this single return exceeds the net available quantity.
				else if ($current_return_qty_absolute > $net_available_qty) {
					$out = false;

					// Build the intelligent error message
					$message = 'Error: Cannot return ' . $current_return_qty_absolute . ' item(s). Max available for return: ' . $net_available_qty . '.';
				...
				if ((($new_qty >= 0) || ($cross_bill)) || ($pr_sr == 2 || $pr_sr == 3)) {
					...
					if ($result) {
						if ($qty != 0) {
							...
							$message = 'Error: Item could not be added1';
							return false;
						}
					} else {
						$message = 'Error: Item could not be added2';
						return false;
					}
				} else {
...
function removeBillitemOdr()
{
	...
	$sn_remove = $return_cash_back_qty = '';
	include('config.php');
	...
	if (($bm_type == 4 || $bm_type == 5) && (($bm_status == 2) || ($bm_status == 11)) && ($out)) {
		$force_permit = true;
	}
...
function updateBillitemOdr()
{
	...
	$out2 = true;
	$check_qty = '';
	$msg = 'Error: Item could not be updated!';
	include('config.php');
	$systemid = inf_systemid(1);
	...
	$query = "SELECT itm.pr_sr,bi.invoice_no,bi.`item` FROM inventory_items itm, bill bi WHERE itm.id=bi.item AND bi.id='$itemid'";
	$row = mysqli_fetch_row(mysqli_query($conn, $query));
	$pr_sr = $row[0];
	$invoice_no = $row[1];
	$bill_item = $row[2];
	...
	if ($out) {
		...
		if ($result && mysqli_num_rows($result) > 0) {
			...
			$return_cash_back_qty = 0;   // no matching row → treat as zero
		}
	...
	if ($out) {
		if ($qty < 0) {
			...
			if ($systemid != 17 & $systemid != 1) {
				$result = mysqli_query($conn, "SELECT `description` FROM inventory_items WHERE id='$bill_item'");
				$row = mysqli_fetch_assoc($result);
				$inv_item_name = $row['description'];

				$query_purchased = "SELECT SUM(bi.qty) FROM bill_main bm JOIN bill bi ON bm.invoice_no=bi.invoice_no WHERE bm.`cust`='$bm_cust' AND bi.item='$bill_item' AND bm.`status`!=0 AND bm.`lock`='1'";

				$result_purchased = mysqli_query($conn, $query_purchased);
				if ($result_purchased === false) {
					$out = false;
					$msg = 'Database query error (purchased quantity): ' . mysqli_error($conn);
				} else {
					$row_purchased = mysqli_fetch_row($result_purchased);
					$total_purchased_qty = $row_purchased && $row_purchased[0] !== null ? (int) $row_purchased[0] : 0;
        ...
        FROM `return_main` rm
        JOIN `return` r ON rm.invoice_no = r.invoice_no
        WHERE rm.cust = '$bm_cust'
          AND r.return_item = '$bill_item'
          AND rm.status IN (1, 2)";
					...
					if ($result_returns === false) {
						$out = false;
						$msg = 'Database query error (returned quantity): ' . mysqli_error($conn);
					} else {
						$row_returns = mysqli_fetch_assoc($result_returns);
						// PHP 5 compatible check
						$total_pending_returns = (int) (isset($row_returns['pending_qty']) ? $row_returns['pending_qty'] : 0);
						...
						// Now, check if this single return exceeds the net available quantity.
							...
							// Build the intelligent error message
							$msg = 'Error: Cannot return ' . $current_return_qty_absolute . ' item(s). Max available for return: ' . $net_available_qty . '.';
...
// updated by nirmal 25_09_2025 (added new status update)
...
function setStatus($method)
{
	...
	$qb_msg = $qb_cust_id = $qb_status = $qb_temp_cost = $custName = $storeQBID = '';
	...
	try {
		...
		if ($out && (isQuickBooksActive(1))) {
			...
			$userQry = "SELECT c.`qb_cust_id`, bm.`qb_status`, bm.`qb_temp_cost`, c.`name`, s.`qb_id` AS `store_qb_id`
					FROM bill_main bm INNER JOIN cust c ON c.`id` = bm.`cust` INNER JOIN stores s ON s.id = bm.`store` WHERE bm.`invoice_no` = '$invoice_no'";
		...
		$billMainQuery = "SELECT `status`,`type`,`mapped_inventory`,`cust` FROM bill_main WHERE invoice_no='$invoice_no'";
		...
		$bm_cust = $billMainQueryResultRow['cust'];
		...
		if ($out) {
			...
			if (($method == 'move_to_cross_check') && ($status == 2)) { // picked move to cross check status
				$query = "UPDATE `bill_main` SET `seen_by`='$salesman',`seen_timestamp`='$time_now', `status`='10' WHERE `invoice_no`='$invoice_no'";
				$result = mysqli_query($conn, $query);
			}
			if (($method == 'set_cross_check_start') && ($status == 10)) { // cross check status
				$query = "UPDATE `bill_main` SET `cross_checked_by`='$salesman',`cross_check_timestamp`='$time_now', `status`='11' WHERE `invoice_no`='$invoice_no'";
				$result = mysqli_query($conn, $query);
			}
			if (($method == 'packed') && ($status == 11)) { // picked after cross check
				if (isOdrCrossCheckActive()) {
					// --- HELPER FUNCTION TO ENSURE CONSISTENT KEYS ---
					// This function MUST match the logic of the JavaScript canonicalizeString()
					function canonicalizeString($str)
					{
						...
						return strtolower($singleSpaced);
					}

					// --- Step 1: Get and Validate Client Data ---
					if (!isset($_POST['checklistData'])) {
						throw new Exception("CRITICAL ERROR: Checklist data was not sent from the client.");
					}

					$clientData = json_decode($_POST['checklistData'], true);
					if (json_last_error() !== JSON_ERROR_NONE) {
						throw new Exception("CRITICAL ERROR: Invalid checklist data format received.");
					}
					$clientChecklist = isset($clientData['checkedItems']) ? $clientData['checkedItems'] : [];

					// --- Step 2: Build the Server's "Source of Truth" Aggregate List ---
					$serverSourceOfTruth = [];

					$processRow = function ($row) use (&$serverSourceOfTruth) {
						$code = canonicalizeString($row['code']);
						$desc = trim($row['description']);
						$qty = (float) $row['qty'];

						// We must have a valid key to aggregate. Code is preferred.
						if (empty($code)) {
							return;
						}

						if (!isset($serverSourceOfTruth[$code])) {
							$serverSourceOfTruth[$code] = ['qty' => 0, 'desc' => $desc];
						}
						$serverSourceOfTruth[$code]['qty'] += $qty;
					};

					// Escape the invoice number once for use in all queries
					$escaped_invoice_no = mysqli_real_escape_string($conn, $invoice_no);

					// 2a. Get all items from the main bill for this invoice
					$billItemsQuery = "SELECT b.qty, i.description, i.code
            FROM bill b
            JOIN inventory_items i ON b.item = i.id
            WHERE b.invoice_no = '{$escaped_invoice_no}' AND b.qty > 0";
					$billResult = mysqli_query($conn, $billItemsQuery);
					if (!$billResult) {
						throw new Exception("DB Error: Could not fetch required bill items.");
					}
					while ($row = mysqli_fetch_assoc($billResult)) {
						$processRow($row);
					}

					// 2b. Get all RETURN items that are marked as PACKED for THIS order
					$packedReturnsQuery = "SELECT rt.qty, itm.description, itm.code
            FROM `return` rt
            JOIN `inventory_items` itm ON rt.replace_item = itm.id
						JOIN `return_main` rm ON rt.invoice_no = rm.invoice_no
            WHERE rt.odr_packed = 1 AND rm.status = 2 AND rt.odr_no = '{$escaped_invoice_no}'";
					$returnsResult = mysqli_query($conn, $packedReturnsQuery);
					if (!$returnsResult) {
						throw new Exception("DB Error: Could not fetch packed return items.");
					}
					...
					// Check for quantity mismatches or extra items from the client
					foreach ($clientChecklist as $code => $clientItemData) {
						$clientQty = (float) $clientItemData['verifiedQty'];

						if (!isset($serverSourceOfTruth[$code])) {
							$clientDesc = htmlspecialchars($clientItemData['desc']);
							$errors[] = "Invalid item found in checklist: '{$clientDesc}'. It may not belong to this order.";
						} elseif (abs($serverSourceOfTruth[$code]['qty'] - $clientQty) > $epsilon) {
							$serverQty = $serverSourceOfTruth[$code]['qty'];
							$description = htmlspecialchars($serverSourceOfTruth[$code]['desc']);
							$errors[] = "Quantity mismatch for '{$description}'. Bill requires {$serverQty}, but you verified {$clientQty}.";
						}
					}

					// Check for items the server requires but the client did NOT check
					foreach ($serverSourceOfTruth as $code => $serverItemData) {
						if (!isset($clientChecklist[$code])) {
							$description = htmlspecialchars($serverItemData['desc']);
							$serverQty = $serverItemData['qty'];
							$errors[] = "Missing item from checklist: '{$description}' (Required Qty: {$serverQty}).";
						}
					}

					// --- Final Step: If any errors were found, throw a single exception ---
					if (!empty($errors)) {
						$error_message = "Verification Failed:<br><br> &bull; " . implode("<br> &bull; ", $errors);
			...
			if (($method == 'shipped') && ($status == 3)) { // packed
			...
			if (isQuickBooksActive(1)) {
				...
				if ($qb_status !== null && $qb_status != '') {
					if ($delivered_updated) {
						...
						// positive cost
						$invoiceTotalQueryPositive = "SELECT SUM(b.`qty` * b.`cost`) FROM bill_main bm, bill b
						WHERE bm.invoice_no = b.invoice_no AND bm.`invoice_no`='$invoice_no' AND b.qty > 0 AND bm.`qb_status` IS NOT NULL";
						$invoiceTotalResult = mysqli_query($conn, $invoiceTotalQueryPositive);
						if ($invoiceTotalResult === false) { // Check if fetch failed or query returned no rows
							throw new Exception("Error fetching invoice details for QB. Details: " . mysqli_error($conn));
						} else {
							$invoiceTotalRowPositive = mysqli_fetch_row($invoiceTotalResult);
							if (!empty($invoiceTotalRowPositive)) {
						...
						// negative cost
						$invoiceTotalQueryNegative = "SELECT SUM(b.`qty` * b.`cost`) FROM bill_main bm, bill b
						WHERE bm.invoice_no = b.invoice_no AND bm.`invoice_no`='$invoice_no' AND b.qty < 0 AND bm.`qb_status` IS NOT NULL";
						$invoiceTotalQueryResultNegative = mysqli_query($conn, $invoiceTotalQueryNegative);
						if ($invoiceTotalQueryResultNegative === false) { // Check if fetch failed or query returned no rows
							throw new Exception("Error fetching invoice details for QB. Details: " . mysqli_error($conn));
						} else {
							$invoiceTotalRowNegative = mysqli_fetch_row($invoiceTotalQueryResultNegative);
							if (!empty($invoiceTotalRowNegative)) {
								$invoiceCost = $invoiceTotalRowNegative[0];
								if ($invoiceCost < 0) {
						...
						// total
						$invoiceTotalQuery = "SELECT (bm.`invoice_+total` + bm.`invoice_-total`)
								FROM bill_main bm WHERE bm.`invoice_no`='$invoice_no' AND bm.`qb_status` IS NOT NULL";
						$invoiceTotalRowResult = mysqli_query($conn, $invoiceTotalQuery);
						if ($invoiceTotalRowResult === false) { // Check if fetch failed or query returned no rows
							throw new Exception("Error fetching invoice details for QB. Details: " . mysqli_error($conn));
						} else {
							$invoiceTotalRow = mysqli_fetch_row($invoiceTotalRowResult);
							if (!empty($invoiceTotalRow)) {
								$bill_total = $invoiceTotalRow[0];
						...
						// tax
						$invoiceTaxQuery = "SELECT bm.`tax` FROM bill_main bm WHERE bm.`invoice_no`='$invoice_no' AND bm.`qb_status` IS NOT NULL";
						$invoiceTaxRowResult = mysqli_query($conn, $invoiceTaxQuery);

						if ($invoiceTaxRowResult === false) {
							throw new Exception("Error fetching invoice details for QB. Details: " . mysqli_error($conn));
						} else {
							$invoiceTaxRow = mysqli_fetch_row($invoiceTaxRowResult);

							if (!empty($invoiceTaxRow)) {
								$bill_tax = (float) $invoiceTaxRow[0]; // Cast to float for numeric comparisons

								// Only proceed if tax is non-zero
								if ($bill_tax != 0) {
									$debitAccountName = "";
									$creditAccountName = "";
									$description = "";

									// Determine accounts and description based on tax sign
									if ($bill_tax > 0) {
										$debitAccountName = "Sales";
									...
									// These are common and can be defined once
									$debitEntityType = "";
									$debitEntityID = "";
									$creditEntityType = "";
									$creditEntityID = "";

									// Call buildJournalEntry once
									$journalEntryForInvoiceTax = buildJournalEntry(
										$conn,
										abs($bill_tax), // Use absolute value here
										$debitAccountName,
										$creditAccountName,
										$description,
										$debitEntityType,
									...
									if (isset($journalEntryForInvoiceTax['error'])) {
										...
										// Loop through journal entries and insert into qb_queue once
										foreach ($journalEntryForInvoiceTax as $entry) {
											$posting_type = mysqli_real_escape_string($conn, $entry['posting_type']);
											$account_id = mysqli_real_escape_string($conn, $entry['account_id']);
											$account_name = mysqli_real_escape_string($conn, $entry['account_name']);
											$amount = mysqli_real_escape_string($conn, $entry['amount']);
											$escaped_entry_description = mysqli_real_escape_string($conn, $entry['description']);
											$entity_type = isset($entry['entity_type']) ? mysqli_real_escape_string($conn, $entry['entity_type']) : null;
											$entity_id = isset($entry['entity_id']) ? mysqli_real_escape_string($conn, $entry['entity_id']) : null;
											$class_id = isset($entry['class_id']) ? mysqli_real_escape_string($conn, $entry['class_id']) : null;
											$department_id = isset($entry['department_id']) ? mysqli_real_escape_string($conn, $entry['department_id']) : null;
                    	...
                    	VALUES ('$batch_id','$action_name', '$invoice_no', '$posting_type', '$account_id', '$account_name', '$amount', '$escaped_entry_description', '$time_now',
                   			" . ($class_id !== null ? "'$class_id'" : "NULL") . ",
												" . ($department_id !== null ? "'$department_id'" : "NULL") . ",
												" . ($entity_type !== null ? "'$entity_type'" : "NULL") . ",
                        " . ($entity_id !== null ? "'$entity_id'" : "NULL") . ")";
						...
						$query = "SELECT rt.replace_item, rt.qty, rm.mapped_inventory, rt.extra_pay
									FROM return_main rm
						...
						if (mysqli_num_rows($result) > 0) {
							...
							if ($item_cost != 0) {
								...
								$journalEntryForReturnItems = buildJournalEntry(
									$conn,
									abs($item_cost),
									$debitAccountName,
									$creditAccountName,
...
function getAllCustomerItems()
{
	...
	$billMainQuery = "SELECT `status`,`cust` FROM bill_main WHERE invoice_no='$invoice_no'";
	$billMainQueryResult = mysqli_query($conn, $billMainQuery);
	$billMainQueryResultRow = mysqli_fetch_assoc($billMainQueryResult);
	$status = $billMainQueryResultRow['status'];
	$bm_cust = $billMainQueryResultRow['cust'];
	...
	if ($status == 11) {
		// 1. Fetch all REQUIRED bill items for this order
		$requiredQuery = "SELECT i.code, i.description
                          FROM bill b
                          JOIN inventory_items i ON b.item = i.id
                          WHERE b.invoice_no = '$invoice_no'";
		$requiredResult = mysqli_query($conn, $requiredQuery);
		if ($requiredResult) {
			while ($row = mysqli_fetch_assoc($requiredResult)) {
				$searchableList[] = [
					"code" => $row['code'],
		...
		// 2. Fetch all RELEVANT return items for this customer
		$allReturnsQuery = "SELECT itm.code, itm.description
                            FROM `return` rt
                            JOIN `return_main` rm ON rt.invoice_no = rm.invoice_no
                            JOIN `inventory_items` itm ON rt.return_item = itm.id
                            WHERE rm.cust = '$bm_cust'
                              AND (
                                rt.odr_packed = '0'
                                OR
                                (rt.odr_packed = '1' AND rt.odr_no = '$invoice_no')
                              )";
		$allReturnsResult = mysqli_query($conn, $allReturnsQuery);
		if ($allReturnsResult) {
			while ($row = mysqli_fetch_assoc($allReturnsResult)) {
				$searchableList[] = [
					"code" => $row['code'],
					"description" => $row['description'],
				];
	...
	return json_encode($searchableList, JSON_HEX_TAG | JSON_HEX_AMP);
}
...
function orderUnassign()
{
	...
	} else if ($bm_status == 11 && $authorization) {
		$query2 = "UPDATE bill_main SET cross_checked_by=null, cross_check_timestamp=null, `status`='10' WHERE invoice_no='$invoice_no'";
		$result2 = mysqli_query($conn, $query2);
		if (!$result2) {
			$out = false;
		}
...
{
	// This action is called by our autocomplete and is restricted to the current order
	include('config.php'); // Ensure you have the DB connection
	header('Content-Type: application/json');

	$term = mysqli_real_escape_string($conn2, $_GET['term']);
	$invoice_no = mysqli_real_escape_string($conn2, $_GET['id']);
	$customer_id = mysqli_real_escape_string($conn2, $_GET['cust_id']);
	$suggestions = [];

	if (strlen($term) < 2 || empty($invoice_no) || empty($customer_id)) {
	...
	// =======================================================================
	// FINAL FIX: Using the correct column name `code` instead of `barcode`
	// as per your provided table schema.
	// =======================================================================
	$query = "
        (SELECT
            inv.description as item_desc
        FROM bill bi
        JOIN inventory_items inv ON bi.item = inv.id
        WHERE bi.invoice_no = '$invoice_no'
          AND (inv.description LIKE '$searchTerm' OR inv.code LIKE '$searchTerm'))
        ...
        WHERE rm.cust = '$customer_id'
          AND (itm.description LIKE '$searchTerm' OR itm.code LIKE '$searchTerm'))
	...
	if (!$result) {
		// This will prevent a crash if there's ever another SQL error.
		error_log("SQL Error in search_order_items: " . mysqli_error($conn2));
		echo json_encode([]);
		exit();
	}
