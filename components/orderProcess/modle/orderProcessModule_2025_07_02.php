				...
				if ($qb_status !== null && $qb_status != '') {
					if ($delivered_updated) {
						if ($qb_temp_cost > 0) {
							...
							$journalEntryForInvoiceCost = buildJournalEntry($conn, abs($qb_temp_cost), $debitAccountName, $creditAccountName, $description, $debitEntityType, $debitEntityID, $creditEntityType, $creditEntityID, $debitDepartmentID, $creditDepartmentID, $debitClassID, $creditClassID);
						...
						if ($invoiceTotalRowResult === false) { // Check if fetch failed or query returned no rows
							...
							if (!empty($invoiceTotalRow)) {
								...
								if ($bill_total > 0) {
									...
									$creditEntityType = "";
									$creditEntityID = "";
									$journalEntryForInvoiceTotal = buildJournalEntry($conn, abs($bill_total), $debitAccountName, $creditAccountName, $description, $debitEntityType, $debitEntityID, $creditEntityType, $creditEntityID, $debitDepartmentID, $creditDepartmentID, $debitClassID, $creditClassID);
...
function processReturn()
{
	...
	try {
		...
		if (isQuickBooksActive(1) && $invrtn > 0) {
			...
			$journalEntry = buildJournalEntry($conn, abs($total), $debitAccountName, $creditAccountName, $description, $debitEntityType, $debitEntityID, $creditEntityType, $creditEntityID, $debitDepartmentID, $creditDepartmentID, $debitClassID, $creditClassID);
