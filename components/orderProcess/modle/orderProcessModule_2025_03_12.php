...
function setStatus($method)
{
	...
	// $result = mysqli_query($conn2, "SELECT `status`,`type`,`mapped_inventory` FROM bill_main WHERE invoice_no='$invoice_no'");
	// $row = mysqli_fetch_assoc($result);
	// $status = $row['status'];
	// $type = $row['type'];
	// $bm_mapped_inventory = $row['mapped_inventory'];


	// $query4 = "SELECT rt.`replace_item`, rt.`qty` FROM `return` rt JOIN inventory_items itm ON itm.id = rt.`replace_item`
	...
	// 			$query = "SELECT COALESCE(SUM(bi.`qty`), 0) AS total FROM bill bi WHERE bi.invoice_no = '$invoice_no' AND bi.`item` = '$item' AND bi.`qty` < 0";
	...
	// 			$query1 = "SELECT qty FROM inventory_qty WHERE item='$item' AND `location`='$bm_mapped_inventory'";
	// 			$result1 = mysqli_query($conn, $query1);

	// 			if ($result1 && mysqli_num_rows($result1) > 0) {
	// 				$row1 = mysqli_fetch_row($result1);
	// 				$itq_qty = $row1[0];
	// 				$itq_qty = $itq_qty - $bill_minus_qty;
	// 				if ($qty > $itq_qty) {
	// 					$qty_update_prevent = false;
	// 					$query5 = "UPDATE `return` SET `odr_no` = NULL, `odr_packed` = '0',`odr_packed_date` = NULL, `odr_packed_by` = NULL WHERE `odr_no` = '$invoice_no'";
	// 					$result5 = mysqli_query($conn, $query5);
	// 					if (!$result5) {
	// 						// Handle query error
	// 						$out = false;
	...
	$billMainQueryResult = mysqli_query($conn2, $billMainQuery);
	...
	WHERE rt.`odr_no` = '$invoice_no' AND rt.`odr_packed` = 1";
	$returnItemsQueryResult = mysqli_query($conn2, $returnItemsQuery);
	...
	if ($out) {
		...
		if ($method == 'packed' && $result) {
			...
			if ($type == 4) {
				...
				$query4 = "SELECT rt.`replace_item`, rt.`qty`, itm.pr_sr, itm.unic FROM `return` rt, inventory_items itm WHERE itm.id = rt.`replace_item` AND rt.`odr_no` = '$invoice_no' AND rt.`odr_packed` = 1";
				$result4 = mysqli_query($conn2, $query4);
				$unic_cal = unicCal();
				while ($row4 = mysqli_fetch_array($result4)) {
					$itq_id = '';
