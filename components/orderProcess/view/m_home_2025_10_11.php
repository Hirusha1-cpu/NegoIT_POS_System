		...
		<div class="w3-col">
			...
			<table align="center" style="font-size:10pt;" class="tbl">
				<tr style="background-color:#C0C0C0">
					<?php if($action=='list_shipped' || $action== 'list_delivered' || $action=='list_packed' || $action=='list_my' || $action=='list_custodr' || $action=='list_pending') print '<th width="60px;">#</th>'; ?>
					...
					<th style="padding-left:20px; padding-right:20px">
						Customer <input type="checkbox" onchange="setOrderBy()"
							<?php if($orderby=='cust') print 'checked="checked"'; ?> />
					</th>
					<th style="padding-left:20px; padding-right:20px">Billed Store</th>
					<th style="padding-left:20px; padding-right:20px">Salesman</th>
					<th style="padding-left:20px; padding-right:20px">
						Billed Date<input type="checkbox" onchange="setOrderBy()"
							<?php if($orderby=='date') print 'checked="checked"'; ?> />
					</th>
				...
				<?php
					...
					if($action=='list_my'){
						for($i=0;$i<sizeof($bi_invoice_no);$i++){
							if($bi_type[$i]==4 || $bi_type[$i]==5) $action0='list_one_custodr'; else $action0='list_one';
							if($currentst_name!=$bm_store[$i]) $color2='red'; else $color2='black';
							print '<tr style="background-color:#F0F0F0">
									<td align="center">'.($i+1).'</td>
