			...
			xhttp.onreadystatechange = function() {
			  if (this.readyState == 4 && this.status == 200) {
				  ...
				  if(returntext=='done'){
				  	...
				  	document.getElementById('return7_'+$id).innerHTML='<span style="color:red"><strong>Error</strong></span>';
				  }
			  }
			};
...
<div class="w3-row">
  ...
  <div class="w3-col">
				...
				<tr>
					<td>
						<?php if($button=='Packed' && $bi_seen_by==$_COOKIE['user']){
							if($button=='Packed' && $systemid == 13) $button1 = 'Started'; else $button1 = 'Packed';
						?>
							<div id="orderprocess">
								<input type="button" value="<?php print $button1; ?>" style="height:50px; width:80px; background-color:#CC5100; font-weight:bold; color:white" onclick="orderProcess()" />
							</div>
						<?php } ?>
					</td>
					<td width="20px"></td>
					<td>
						<?php if($button!='Pick'){ ?>
								<input type="button" value="Print" style="height:50px; width:80px; background-color:#007799; font-weight:bold; color:white" onclick="sendToQuickPrinterChrome('print_odr')" />
							<!-- <input type="button" value="Print" style="height:50px; width:80px; background-color:#007799; font-weight:bold; color:white" onclick="parent.location='printscheme://<?php print $url; ?>/pos_odr.php?id=<?php print $_REQUEST['id']; ?>&user=<?php print $_COOKIE['user']; ?>'" /> -->
						<?php } ?>
					</td>
					<td width="20px"></td>
					<td>
	...
	<tr><td colspan="5">
		<table>
		<tr style="background-color:#C0C0C0"><th></th><th>Item Description</th><th style="padding-left:10px; padding-right:10px">Item Qty</th><th style="padding-left:10px; padding-right:10px">Drawer No</th><th></th></tr>
		<?php
		for($i=0;$i<sizeof($odr_bill_id);$i++){
			if($odr_bi_order[$i]==0){
				if($odr_bi_no_update[$i]==0) $update_button='<input type="Button" value="Update"  onclick="updateItemODR('.$odr_bill_id[$i].')" />'; else $update_button='<input type="Button" value="Update" onclick="alert('."'Update is Restricted for this item'".')" />';
				print '<tr style="background-color:#F0F0F0"><td><input type="checkbox" /></td><td style="padding-left:10px; padding-right:10px">'.$odr_bi_desc[$i].'</td><td align="center"><input type="text" id="qty_'.$odr_bill_id[$i].'" value="'.$odr_bi_qty[$i].'" style="width:50px; text-align:right;" /></td><td style="padding-right:20px" align="right">'.$odr_bi_drawer[$i].'</td><td><div id="button_'.$odr_bill_id[$i].'" >'.$update_button.' <input type="button" value="Remove" onmouseup="removeItemODR('.$odr_bill_id[$i].')" style="background-color:maroon; color:white" /></div></td></tr>';
			}
		}
			print '<tr style="background-color:#F0F0F0"><td colspan="2"></td><td></td><td></td><td><input type="button" value="Add New" onmouseup="window.location = '."'index.php?components=order_process&action=setdistrict_custodr&bill_no=".$_GET['id']."&id=$bm_district&return=0'".'" /></td></tr>';
		?>
		</table>
		...
		<?php if($systemid==1 || $systemid==10 || $systemid==16 || $systemid==17){ ?>
			...
			<hr />
			<table align="center" width="100%"><tr style="background-color:#467898; color:white;"><th height="50px">RETURN ITEM PACKING</th></tr></table>
			<table>
			<tr style="background-color:#467898; color:white;"><td colspan="4" style="padding-left:10px">Credit Balance for Cancel Retuns : </td><td align="right" style="padding-right:10px"><?php print $currency; ?> <?php print number_format($return_cr_bal, $decimal); ?> </td></tr>
			<tr style="background-color:#C0C0C0"><th></th><th>Item Description</th><th style="padding-left:10px; padding-right:10px">Item Qty</th><th style="padding-left:10px; padding-right:10px">Drawer No</th><th></th></tr>
			<?php
			for($i=0;$i<sizeof($odr_bill_id);$i++){
				if($odr_bi_order[$i]==1){
					if($odr_bi_no_update[$i]==0) $update_button='<input type="Button" value="Update"  onclick="updateItemODR('.$bill_id[$i].')" />'; else $update_button='<input type="Button" value="Update" onclick="alert('."'Update is Restricted for this item'".')" />';
					print '<tr style="background-color:#F0F0F0"><td><input type="checkbox" /></td><td style="padding-left:10px; padding-right:10px">'.$odr_bi_desc[$i].'</td><td align="center"><input type="text" id="qty_'.$odr_bill_id[$i].'" value="'.$odr_bi_qty[$i].'" style="width:50px; text-align:right;" /></td><td style="padding-right:20px" align="right">'.$odr_bi_drawer[$i].'</td><td><div id="button_'.$bill_id[$i].'" >'.$update_button.' <input type="button" value="Remove" onmouseup="removeItemODR('.$odr_bill_id[$i].')" style="background-color:maroon; color:white" /></div></td></tr>';
				}
			}
				print '<tr style="background-color:#F0F0F0"><td colspan="2"></td><td></td><td></td><td><input type="button" value="Add New" onmouseup="window.location = '."'index.php?components=order_process&action=setdistrict_custodr&bill_no=".$_GET['id']."&id=$bm_district&return=1'".'" /></td></tr>';
			?>
			</table>

			<table align="center">
			<tr><th style="background-color:#467898;color :white;" colspan="7">Return Items | Pending</th></tr>
			<tr style="background-color:#C0C0C0"><th></th><th style="padding-left:20px; padding-right:20px">RT Invoice</th><th style="padding-left:20px; padding-right:20px">Return Date</th><th style="padding-left:20px; padding-right:20px">Item</th><th style="padding-left:20px; padding-right:20px">Return Qty</th><th style="padding-left:20px; padding-right:20px">Salesman</th><th style="padding-left:20px; padding-right:20px">Pack</th></tr>
			<?php
			for($i=0;$i<sizeof($rtn_id);$i++){
				print '<tr style="background-color:#F0F0F0"><td><div id="return1_'.$rtn_id[$i].'"><input type="checkbox" /></div></td><td align="center"><div id="return2_'.$rtn_id[$i].'"><a href="index.php?components=billing&action=finish_return&id='.$rtn_inv[$i].'">'.str_pad($rtn_inv[$i], 7, "0", STR_PAD_LEFT).'</a></div></td><td align="center"><div id="return3_'.$rtn_id[$i].'">'.$rtn_date[$i].'</div></td><td style="padding-left:20px; padding-right:20px"><div id="return4_'.$rtn_id[$i].'"><a style="text-decoration:none" href="index.php?components=order_process&action=show_one_return_item&odr_id='.$_GET['id'].'&rtn_id='.$rtn_id[$i].'">'.$rtn_itm_desc[$i].'</a></div></td><td style="padding-right:20px" align="right"><div id="return5_'.$rtn_id[$i].'">'.$rtn_qty[$i].'</div></td><td style="padding-right:20px; padding-left:20px;" align="right"><div id="return6_'.$rtn_id[$i].'">'.ucfirst($rtn_by[$i]).'</div></td><td align="center"><div id="return7_'.$rtn_id[$i].'" ><input type="button" value="Pack" onmouseup="returnPacked('.$rtn_id[$i].')" style="background-color:maroon; color:white" /></div></td></tr>';
			}
			for($i=0;$i<sizeof($rtn2_id);$i++){
				print '<tr style="background-color:#F0F0F0"><td><div id="return1_'.$rtn2_id[$i].'"></div></td><td align="center"><div id="return2_'.$rtn2_id[$i].'"></div></td><td align="center"><div id="return3_'.$rtn2_id[$i].'"></div></td><td style="padding-left:20px; padding-right:20px"><div id="return4_'.$rtn2_id[$i].'"></div></td><td style="padding-right:20px" align="right"><div id="return5_'.$rtn2_id[$i].'"></div></td><td style="padding-right:20px; padding-left:20px;" align="right"><div id="return6_'.$rtn2_id[$i].'"></div></td><td align="center"><div id="return7_'.$rtn2_id[$i].'" ></div></td></tr>';
			}
			?>
			<tr><th colspan="7"><br /></th></tr>
			<tr><th style="background-color:#787898;color :white;" colspan="7">Return Items | <?php if($systemid == 13) echo 'Started'; else print 'Packed';?></th></tr>
			<tr style="background-color:#C0C0C0"><th></th><th style="padding-left:20px; padding-right:20px">RT Invoice</th><th style="padding-left:20px; padding-right:20px">Return Date</th><th style="padding-left:20px; padding-right:20px">Item</th><th style="padding-left:20px; padding-right:20px">Return Qty</th><th style="padding-left:20px; padding-right:20px">Salesman</th><th style="padding-left:20px; padding-right:20px">Pack</th></tr>
			<?php
			for($i=0;$i<sizeof($rtn_id);$i++){
				print '<tr style="background-color:#F0F0F0"><td><div id="return1_done_'.$rtn_id[$i].'"></div></td><td align="center"><div id="return2_done_'.$rtn_id[$i].'"></div></td><td align="center"><div id="return3_done_'.$rtn_id[$i].'"></div></td><td style="padding-left:20px; padding-right:20px"><div id="return4_done_'.$rtn_id[$i].'"></div></td><td style="padding-right:20px" align="right"><div id="return5_done_'.$rtn_id[$i].'"></div></td><td style="padding-right:20px; padding-left:20px;" align="right"><div id="return6_done_'.$rtn_id[$i].'"></div></td><td align="center"><div id="return7_done_'.$rtn_id[$i].'" ></div></td></tr>';
			}
			for($i=0;$i<sizeof($rtn2_id);$i++){
				print '<tr style="background-color:#F0F0F0"><td><div id="return1_done_'.$rtn2_id[$i].'"><input type="checkbox" /></div></td><td align="center"><div id="return2_done_'.$rtn2_id[$i].'"><a href="index.php?components=billing&action=finish_return&id='.$rtn2_inv[$i].'">'.str_pad($rtn2_inv[$i], 7, "0", STR_PAD_LEFT).'</a></div></td><td align="center"><div id="return3_done_'.$rtn2_id[$i].'">'.$rtn2_date[$i].'</div></td><td style="padding-left:20px; padding-right:20px"><div id="return4_done_'.$rtn2_id[$i].'">'.$rtn2_itm_desc[$i].'</div></td><td style="padding-right:20px" align="right"><div id="return5_done_'.$rtn2_id[$i].'">'.$rtn2_qty[$i].'</div></td><td style="padding-right:20px; padding-left:20px;" align="right"><div id="return6_done_'.$rtn2_id[$i].'">'.ucfirst($rtn2_by[$i]).'</div></td><td align="center"><div id="return7_done_'.$rtn2_id[$i].'" ><input type="button" value="Remove" onmouseup="removeReturnPacked('.$rtn2_id[$i].')" style="background-color:maroon; color:white" /></div></td></tr>';
			}
			?>
			</table>
		<?php } ?>
