...
<?php if($button!='Pick'){
			...
			else include_once  'components/billing/view/tpl/'.$execute;
} 
...
<div class="w3-row">
  ...
  <div class="w3-col">
	...
	<tr><td style="background-color:#467898;color :white;"><strong>Order Date</strong></td><td bgcolor="#EEEEEE"><?php print  substr($odr_date,0,16); ?></td><td></td><td style="background-color:#467898;color :white;"><strong>Customer</strong></td><td bgcolor="#EEEEEE"><?php print '<a href="index.php?components=billing&action=cust_details&id='.$cu_id.'&action2=list_one_custodr&id2='.$_REQUEST['id'].'" >'.ucfirst($custname_0).'</a>'; ?></td></tr>
	<tr><td colspan="5" height="50px" align="center"><table align="center" style="font-size:10pt"><tr><td>
		<?php if($button!='' && $button!='Packed'){ ?><div id="orderprocess"><input type="button" value="<?php print $button; ?>" style="height:50px; width:80px; background-color:#CC5100; font-weight:bold; color:white" onclick="orderProcess()" /></div><?php } ?>
		<?php if($bm_status==1 && $bm_type==1){	print '</td><td><div id="moveodr"><button type="button" style="height:50px; width:70px; background-color:#336699; font-weight:bold; color:white" onclick="moveCustOdr('.$id.')">Move to<br />Cust Order</button></div>'; } ?>
	...
	<tr><td colspan="5">
		<table>
		<tr style="background-color:#C0C0C0"><th>Item Description</th><th style="padding-left:20px; padding-right:20px">Item Qty</th><th style="padding-left:20px; padding-right:20px">Drawer No</tr>
