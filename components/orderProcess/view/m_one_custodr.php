			...
			<table align="center" style="font-size:12pt" width="100%">
				<tr>
					<td style="background-color:#467898;color :white;">
						<strong><?php if($bm_status<3) print 'Order No'; else print 'Invoice No'; ?></strong></td>
							...
							<tr>
								<td>
									<?php if($button=='Packed' && $bi_seen_by==$_COOKIE['user']){
										if($button=='Packed' && $systemid == 13 && $sub_system == 1) $button1 = 'Started'; else $button1 = 'Packed';
									?>
										<div id="orderprocess">
									...
									<?php
										if((isset($_COOKIE['report']) || isset($_COOKIE['manager']))&& $button=='Packed'){
											print '<div id="orderprocess3"><input type="button" value="Unassign"
												style="height:50px; width:80px; background-color:orange; font-weight:bold; color:white"
					...
					<td colspan="5">
						<table>
							<tr style="background-color:#C0C0C0">
								...
								<th style="padding-left:10px; padding-right:10px">Item Qty</th>
								<th style="padding-left:10px; padding-right:10px">Drawer No</th>
								<th></th>
							</tr>
							<?php
								for($i=0;$i<sizeof($odr_bill_id);$i++){
									if($odr_bi_order[$i]==0){
										if($odr_bi_no_update[$i]==0) $update_button='<input type="Button" value="Update"
											onclick="updateItemODR('.$odr_bill_id[$i].')" />'; else $update_button='<input type="Button" value="Update"
											onclick="alert('."'Update is Restricted for this item'".')" />';
										print '<tr style="background-color:#F0F0F0">
											<td><input type="checkbox" /></td>
											<td style="padding-left:10px; padding-right:10px">'.$odr_bi_desc[$i].'</td>
											<td align="center"><input type="text" id="qty_'.$odr_bill_id[$i].'" value="'.$odr_bi_qty[$i].'"
						...
						<?php if($systemid==1 || $systemid==10 || $systemid==16 || $systemid==17){ ?>
							...
							<table>
								<tr style="background-color:#467898; color:white;">
									...
									<td align="right" style="padding-right:10px"><?php print $currency; ?>
										<?php print number_format($return_cr_bal, $decimal); ?> </td>
								</tr>
								<tr style="background-color:#C0C0C0">
									<th></th>
									<th>Item Description</th>
								...
								<?php
									for($i=0;$i<sizeof($odr_bill_id);$i++){
										if($odr_bi_order[$i]==1){
											if($odr_bi_no_update[$i]==0) $update_button='<input type="Button" value="Update"
												onclick="updateItemODR('.$bill_id[$i].')" />'; else $update_button='<input type="Button" value="Update"
												onclick="alert('."'Update is Restricted for this item'".')" />';
											print '<tr style="background-color:#F0F0F0">
												<td><input type="checkbox" /></td>
												<td style="padding-left:10px; padding-right:10px">'.$odr_bi_desc[$i].'</td>
												...
												<td style="padding-right:20px" align="right">'.$odr_bi_drawer[$i].'</td>
												<td>
													<div id="button_'.$bill_id[$i].'">'.$update_button.' <input type="button" value="Remove" onmouseup="removeItemODR('.$odr_bill_id[$i].')" style="background-color:maroon; color:white" />
													</div>
												</td>
											</tr>';
										}
							...
							<table align="center">
										...
										print '<tr style="background-color:#F0F0F0">
												...
												<td align="center">
													<div id="return7_'.$rtn_id[$i].'">
														<input type="button" value="Pack" onmouseup="returnPacked('.$rtn_id[$i].')" style="background-color:maroon; color:white" />
													</div>
												</td>
											</tr>';
										}
								...
								<tr>
									<th style="background-color:#787898;color :white;" colspan="7">Return Items |
										<?php if($systemid == 13 && $sub_system == 1) echo 'Started'; else print 'Packed';?></th>
								</tr>
								<tr style="background-color:#C0C0C0">
									<th></th>
									<th style="padding-left:20px; padding-right:20px">RT Invoice</th>
											...
											print '<tr style="background-color:#F0F0F0">
													...
													<td align="center">
														<div id="return7_done_'.$rtn2_id[$i].'">
															<input type="button" value="Remove" onmouseup="removeReturnPacked('.$rtn2_id[$i].')"
						...
						<table width="100%">
							<tr style="background-color:#C0C0C0">
								...
								<td style="padding-left:20px;">
									<?php if($systemid == 13 && $sub_system == 1) echo 'Started'; else print 'Packed';?> By</td>
									<?php if($systemid != 13 || $sub_system != 1) { ?><td style="padding-left:20px;">Shipped By</td><?php } ?>
								<td style="padding-left:20px;">
									<?php if($systemid == 13 && $sub_system == 1) echo 'Finished'; else print 'Delivered';?> By</td>
							</tr>
							<tr style="background-color:#F0F0F0">
								...
								<?php if($systemid != 13 || $sub_system != 1) { ?><td style="padding-left:20px;"><?php print  ucfirst($bi_shipped_by); ?></td><?php } ?>
							...
							<tr style="background-color:#F0F0F0">
								...
								<?php if($systemid != 13 || $sub_system != 1) { ?><td style="padding-left:20px;"><?php print  $bi_shipped_date.'<br/>'.$bi_shipped_time; ?></td><?php } ?>
