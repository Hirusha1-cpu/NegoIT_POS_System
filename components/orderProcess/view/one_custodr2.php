...
if (isset($_REQUEST['message'])) {
  if ($_REQUEST['re'] == 'success')
    ...
    $color = 'red';
  print '<table align="center" bgcolor="#E5E5E5">
					<tr>
					<td colspan="5"><span style="color:' . $color . '; font-weight:bold;font-size:12pt;">' . $_REQUEST['message'] . '</span></td>
					</tr>
					</table><br />';
}
?>
    ...
    <div class="order-content">
      <form action="#" method="post">
        <table style="font-size:12pt">
          <tr>
            <td style="background-color:#467898;color :white;">
              <strong><?php echo $bm_status < 3 ? 'Order No' : 'Invoice No'; ?></strong>
            </td>
            <td bgcolor="#EEEEEE" colspan="2">
              <?php print str_pad($_REQUEST['id'], 7, '0', STR_PAD_LEFT); ?>
            </td>
            <td style="background-color:#467898;color :white;">
            ...
            <td rowspan="2" style="text-align: right;">
              ...
              <?php if (
                $button == 'Packed' &&
                $bi_cross_checked_by == $_COOKIE['user']
              ) {
                $button1 =
                  $systemid == 13 && $sub_system == 1 ? 'Started' : 'PACKED'; ?>
                <div id="orderprocess">
                  <input type="button" value="<?php print $button1; ?>"
                    style="height:50px; width:150px; background-color:#CC5100; font-weight:bold; color:white"
                    onclick="orderProcess('<?php print $button1; ?>')" />
              ...
              <input type="button" value="Print"
                style="height:50px; width:150px; background-color:#007799; font-weight:bold; color:white" onclick="window.location = 'index.php?components=<?php print $bill_module; ?>&action=finish_bill&id=<?php print $_REQUEST[
                     'id'
                   ]; ?>'" />
              <?php if (
                (isset($_COOKIE['report']) || isset($_COOKIE['manager'])) &&
                $button == 'Cross Check'
              ) {
                print '<div id="orderprocess3"><input type="button" value="Unassign" style="height:50px; width:150px; background-color:orange; font-weight:bold; color:white" onclick="orderUnassign(' .
                  $bm_type .
                  ')" /></div>';
              } ?>
            </td>
          ...
          <tr>
            ...
            <td bgcolor="#EEEEEE" colspan="2">
              <?php print substr($odr_date, 0, 16); ?>
            </td>
            <td style="background-color:#467898;color :white;">
              <strong>Customer</strong>
            </td>
            <td bgcolor="#EEEEEE">
              <?php print '<a href="index.php?components=' .
                $bill_module .
                '&action=cust_details&id=' .
                $cu_id .
                '&action2=list_one_custodr&id2=' .
                $_REQUEST['id'] .
                '" >' .
                ucfirst($bi_cust) .
                '</a>'; ?>
        ...
        <?php if (
          ...
          <?php endif; ?>
          <table id="pending-returns-table" align="center">
            <thead>
              <tr>
          ...
          <table id="packed-returns-table" align="center">
            <thead>
              <tr>
  ...
  function orderProcess(button) {
    if (confirm(`Are you sure you want to ${button} this order?`)) {
      document.getElementById('orderprocess').innerHTML =
        document.getElementById('loading').innerHTML;
      console.log((button === 'PACKED' || button === 'Started') && <?php echo $bm_status == 11 ? 'true' : 'false'; ?>);
