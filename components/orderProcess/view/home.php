...
<script type="text/javascript">
	...
	function generateTag() {
		var order_arr = [<?php for ($x = 0; $x < sizeof($bi_invoice_no); $x++) {
			print '"' . $bi_invoice_no[$x] . '",';
		} ?>];
		var i;
		var tag_list = '';
...
if (($action == 'list_custodr') || ($action == 'list_pending')) {
	$menu_status = '<th style="padding: 0px 10px; padding-right:20px">Status</th>';
} else {
	$menu_status = '<th style="padding: 0px 10px; padding-right:20px">Order Type</th>';
}
?>
...
<table align="center" style="font-family:Calibri; font-size:12pt">
	<tr style="background-color:#C0C0C0">
		<?php if (
			$action == 'list_custodr' || $action == 'list_pending' || $action == 'list_my' || $action == 'list_packed' ||
			$action == 'list_shipped' || $action == 'list_delivered' || $action == 'list_cross_check' || $action == 'list_my_cross_check'
		)
		...
		<?php if ($action == 'list_shipped')
			print '<th style="padding: 0px 10px; padding-right:20px;">Deliver</th>'; ?>
		<th style="padding: 0px 10px; padding-right:20px">
			<?php if (in_array($_GET['action'], array("list_custodr", "list_pending", "list_my")))
				print 'Order No';
			else
				print 'Invoice No'; ?>
		</th>
		<th style="padding: 0px 10px; padding-right:20px">Billed District</th>
		<th style="padding: 0px 10px; padding-right:20px">
			Customer <input type="checkbox" onchange="setOrderBy()" <?php if ($orderby == 'cust')
				print 'checked="checked"'; ?> />
		</th>
		<th style="padding: 0px 10px; padding-right:20px">Billed Store</th>
		<th style="padding: 0px 10px; padding-right:20px">Salesman</th>
		<th style="padding: 0px 10px; padding-right:20px;">
			Billed Date<input type="checkbox" onchange="setOrderBy()" <?php if ($orderby == 'date')
				print 'checked="checked"'; ?> />
		</th>
		<th style="padding: 0px 10px; padding-right:20px"><?php print $menu_by; ?></th>
		<th style="padding: 0px 10px; padding-right:20px"><?php print $menu_date; ?></th>
	...
	if ($action == 'list_custodr' || $action == 'list_pending') {
		...
		for ($i = 0; $i < sizeof($bi_invoice_no); $i++) {
			...
			print '<tr style="background-color:#F0F0F0">
						...
						<td style="padding: 0px 10px;">' . ucfirst($bi_packed_by[$i]) . '</td>
						<td align="center">
		...
		for ($i = 0; $i < sizeof($bi_invoice_no); $i++) {
			...
			print '
					<tr style="background-color:#F0F0F0">
						...
						<td style="padding: 0px 10px;">' . ucfirst($bi_shipped_by[$i]) . '</td>
						<td align="center">
		...
		for ($i = 0; $i < sizeof($bi_invoice_no); $i++) {
			...
			if ($bi_deliverd_by[$i] != '')
				print '<tr style="background-color:#F0F0F0">
						<td align="center">' . sprintf('%02d', ($i + 1)) . '</td>
						<td align="center"><a href="index.php?components=order_process&action=list_one&id=' . $bi_invoice_no[$i] . '">' . str_pad($bi_invoice_no[$i], 7, "0", STR_PAD_LEFT) . '</a></td>
						<td style="padding: 0px 10px;">' . $bi_district[$i] . '</td>
						<td style="padding: 0px 10px;">' . $bi_cust[$i] . '</td>
						<td style="padding: 0px 10px; color:' . $color2 . '">' . $bm_store[$i] . '</td>
						<td style="padding: 0px 10px;">' . ucfirst($bi_billed_by[$i]) . '</td>
						<td align="center"><a style="cursor:pointer; color:blue;" title="Time: ' . substr($bi_billed_time[$i], 0, 5) . '">' . $bi_billed_date[$i] . '</a></td><td style="padding: 0px 10px;">' . ucfirst($bi_deliverd_by[$i]) . '</td>
						<td align="center"><a style="cursor:pointer; color:blue;" title="Time: ' . substr($bi_deliverd_time[$i], 0, 5) . '">' . $bi_deliverd_date[$i] . '</a>
						</td>' . $td_type . '</tr>';
		}
	}
	?>
