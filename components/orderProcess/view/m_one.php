...
$custname_0 = $bi_cust;
generateInvoiceOdr('itm.description');
generalPrintOdr();
generateReturnListOdr();
...
<div class="w3-container" style="margin-top:75px">
	...
	if ($button != 'Pick') {
		if ($bm_status < 3)
			...
			include_once 'components/' . $bill_module . '/view/tpl/' . $execute;
	}
		...
		<div class="w3-col">
			<table align="center" style="font-size:12pt" width="100%">
				...
				<tr>
					...
					<td bgcolor="#EEEEEE">
						<?php print '<a href="index.php?components=' . $bill_module . '&action=cust_details&id=' . $cu_id . '&action2=list_one_custodr&id2=' . $_REQUEST['id'] . '" >' . ucfirst($custname_0) . '</a>'; ?>
					</td>
				</tr>
				<tr>
							...
							<tr>
								<td>
									<?php if (isOdrCrossCheckActive()) { ?>
										<?php if (($button != '') && ($button != 'Packed') && ($button != 'Move to Cross Check')) { ?>
											<div id="orderprocess">
									...
									<?php } else { ?>
										<?php if ($button != '' && $button != 'Packed') { ?>
											<div id="orderprocess">
												<input type="button" value="<?php print $button; ?>"
													style="height:50px; width:80px; background-color:#CC5100; font-weight:bold; color:white"
													onclick="orderProcess()" />
											</div>
										<?php } ?>
									<?php } ?>
									...
									<td>
										<div id="moveodr">
											<button type="button" style="height:50px; width:150px; background-color:#336699; font-weight:bold; color:white" onclick="moveCustOdr(' . $id . ')">Move to<br />Cust Order</button>
										</div>';
									} ?>
								...
								<td>
									<?php if ($button != 'Pick') {
										if ($bm_status < 3) {
											print '<input type="button" value="Print" style="height:50px; width:150px; background-color:#007799; font-weight:bold; color:white" onclick="sendToQuickPrinterChrome(\'print_odr\')" />';
											...
											print '<input type="button" value="Print" style="height:50px; width:150px; background-color:#007799; font-weight:bold; color:white" onclick="sendToQuickPrinterChrome(\'print\')" />';
									...
									print '&nbsp;<input type="button" value="Print Tag" style="height:50px; width:150px; background-color:#007777; font-weight:bold; color:white" onclick="sendToQuickPrinterChrome(\'print_address\')" />';
								...
								<td>
									<?php if ((isset($_COOKIE['report']) || isset($_COOKIE['manager'])) && $button == 'Packed') {
										print '<div id="orderprocess3"><input type="button" value="Unassign" style="height:50px; width:150px; background-color:orange; font-weight:bold; color:white" onclick="orderUnassign(' . $bm_type . ')" /></div>';
									}
									?>
									<?php if (isOdrCrossCheckActive() && ($bm_status == 2) && (isset($_COOKIE['report']) || isset($_COOKIE['manager'])) && $button == 'Move to Cross Check') {
										print '<div id="orderprocess3"><input type="button" value="Unassign" style="height:50px; width:150px; background-color:orange; font-weight:bold; color:white" onclick="orderUnassign(' . $bm_type . ')" /></div>';
									}
									?>
								</td>
					...
					<td colspan="5">
						<table width="100%">
							...
							<tr style="background-color:#F0F0F0">
								...
								<?php if ($systemid != 13 || $sub_system != 1) { ?>
									<td style="padding-left:20px;"><?php print $bi_shipped_date . '<br/>' . $bi_shipped_time; ?></td>
								<?php } ?>
