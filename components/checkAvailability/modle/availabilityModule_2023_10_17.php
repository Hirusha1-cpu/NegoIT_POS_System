...
global $district_id,$district_name,$current_district;
	include('config.php');
...
function getItems($sub_system,$systemid){
global $id,$code,$description,$w_price,$r_price,$cost,$drawer,$qty,$cat_id,$cat_name,$sto_name,$code_uni,$description_uni,$decimal;
	...
	*/
	...
	if($mapped_inventory==0)
		$query="SELECT DISTINCT itm.id,itm.code,itm.description,itq.w_price,itq.r_price,itq.c_price,itq.drawer_no,itq.qty,ca.id,ca.name,st.name,itm.id,itm.unic,itq.id FROM inventory_items itm, inventory_qty itq, stores st, store_group gp, item_category ca WHERE itm.`id`=itq.`item` AND st.`id`=itq.location AND st.id=gp.store AND itm.`status`=1 AND gp.`group`='$group' AND $category_qry itm.`category`=ca.id";
	else
		$query="SELECT DISTINCT itm.id,itm.code,itm.description,itq.w_price,itq.r_price,itq.c_price,itq.drawer_no,itq.qty,ca.id,ca.name,st.name,itm.id,itm.unic,itq.id FROM inventory_items itm, inventory_qty itq, stores st, item_category ca WHERE itm.`id`=itq.`item` AND st.`id`=itq.location AND itq.location='$mapped_inventory' AND itm.`status`=1 AND $category_qry itm.`category`=ca.id";
		$result=mysqli_query($conn2,$query);
		while($row=mysqli_fetch_array($result)){
	...
	if($mapped_inventory==0)
		$query="SELECT itm.id,itm.code,itm.description,itn.w_price,itn.r_price,itn.c_price,itm.id,itn.qty,ca.id,ca.name,st.name,itm.id FROM inventory_items itm, inventory_new itn, stores st, store_group gp, item_category ca WHERE itm.`id`=itn.`item` AND st.`id`=itn.store AND st.id=gp.store AND itm.`status`=1 AND gp.`group`='$group' AND $category_qry itm.`category`=ca.id";
	else
		$query="SELECT itm.id,itm.code,itm.description,itn.w_price,itn.r_price,itn.c_price,itm.id,itn.qty,ca.id,ca.name,st.name,itm.id FROM inventory_items itm, inventory_new itn, stores st, item_category ca WHERE itm.`id`=itn.`item` AND st.`id`=itn.store AND itn.store='$mapped_inventory' AND itm.`status`=1 AND $category_qry itm.`category`=ca.id";
		$result=mysqli_query($conn2,$query);
		while($row=mysqli_fetch_array($result)){
			$no_specialrate=$no_catspecialrate=true;
			$id[]=$row[0];
...
global $store_name,$item_w_price,$item_r_price,$item_qty,$search_code,$search_description,$item_dr,$decimal;
	...
	*/
	...
	if($mapped_inventory==0)
		$query="SELECT st.name,itq.w_price,itq.r_price,itq.qty,itq.item,itm.`category`,itq.drawer_no,itq.id,itm.unic FROM inventory_items itm, inventory_qty itq, stores st, store_group gp WHERE itm.id=itq.item AND itq.location=st.id AND st.id=gp.store AND st.`status`=1 AND gp.`group`='$group' AND itm.code='$search_code'";
	else
...
function getCatalog(){
	global $decimal,$from_date,$to_date,$qty_order,$show_all,$direct_mkt,$sold_color,$today,$selected_cat,$filter_category_id,$filter_category_name,$district,$cust_id,$item_id,$item_desc,$cat_itemid,$cat_category,$cat_desc,$cat_min_w_rate,$cat_max_w_rate,$cat_max_r_rate,$w_price,$r_price,$cat_qty,$cat_last_rate,$cat_store;
...
function getStock(){
	global $mystore,$store_id,$store_name,$itm_id,$itm_des,$itm_qty;
	...
	$query="SELECT itm.id,itm.description,itq.location,itq.qty,itq.id,itm.unic,itm.pr_sr FROM inventory_items itm, inventory_qty itq, store_group sg WHERE itm.id=itq.item AND itq.location=sg.store AND itm.category='$cat_id' AND itm.pr_sr='1' AND sg.`group`='$group' ORDER BY itm.description";
	$result=mysqli_query($conn2,$query);
	while($row=mysqli_fetch_array($result)){
		$itq_qty=$row[3];
		$itq_id[]=$row[4];
	...
	$query="SELECT itm.id,itm.description,iqt.location,tr.qty,iqt.id,itm.unic,tr.id FROM inventory_items itm, inventory_qty iqt, stores st, store_group sg, transfer_main tm, transfer tr  WHERE itm.category='$cat_id' AND tm.gtn_no=tr.gtn_no AND tm.from_store=st.id AND tr.item=itm.id AND st.id=iqt.location AND st.id=sg.store AND itm.id=iqt.item AND itm.pr_sr='1' AND sg.`group`='$group' AND tm.`status` IN (0,4) AND itm.`status`=1 ORDER BY iqt.drawer_no";
	$result=mysqli_query($conn2,$query);
	while($row=mysqli_fetch_array($result)){
		$itm_id_tmp=$row[0];
		$itq_id_tmp=$row[4];
		...
		for($i=0;$i<sizeof($store_id);$i++){
			if($qty>0){
				if($store_id[$i]==$row[2]) $tr_qty[$row[0]][$i]+=$qty;
			}
		}
	} 
