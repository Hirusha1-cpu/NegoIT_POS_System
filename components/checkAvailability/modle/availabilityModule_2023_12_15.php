...
function getItems($sub_system,$systemid){
	...
	if($mapped_inventory==0)
		$query="SELECT DISTINCT itm.id,itm.code,itm.description,itq.w_price,itq.r_price,itq.c_price,itq.drawer_no,itq.qty,ca.id,ca.name,st.name,itm.id,itm.unic,itq.id FROM inventory_items itm, inventory_qty itq, stores st, store_group gp, item_category ca WHERE itm.`id`=itq.`item` AND st.`id`=itq.location AND st.id=gp.store AND itm.`status`=1 AND gp.`group`='$group' AND $category_qry itm.`category`=ca.id $subsystem_qry";
	else
