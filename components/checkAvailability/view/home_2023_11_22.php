...
<table align="center">
<tr><td>
<table width="100%"><tr><td><h1 style="color:orange">Check Item Availability</h1></td><td align="right">
...
?><br /></td></tr>
<tr><td width="50px"></td><td style="font-size:12pt">Item Code</td><td colspan="2"><input type="text" name="code" id="tags1" value="<?php print $search_code; ?>" onfocus="refresh('tags1')" /></td><td width="50px"></td></tr>
<tr><td width="50px"></td><td style="font-size:12pt">Item Description</td><td colspan="2"><input type="text" name="description" id="tags2" value="<?php print $search_description; ?>" onfocus="refresh('tags2')" onclick="clearBox()" /></td><td width="50px"></td></tr>
<tr><td width="50px"></td><td style="font-size:12pt" colspan="3"><input type="text" readonly="readonly" id="txt_result" style="background-color:#EEEEEE; border-color:#CCCCCC; text-align:center; width:100%" value="<?php print $search_description; ?>" /></td><td width="50px"></td></tr>
<tr><td colspan="5" align="center"><input type="submit" onfocus="getPrice()" value="Check Availability" style="width:130px; height:40px; font-weight:bold"  /><br /><br /></td></tr>
<tr><td colspan="5" style="font-size:12pt">
	<table border="1" cellspacing="0" width="90%" align="center" >
	<tr><th>DR</th><th>Store</th><th>W/Price</th><?php if($retail_shop==1) print '<th>R/Price</th>'; ?><th>Qty</th></tr>
<?php
	for($i=0;$i<sizeof($store_name);$i++){
		if($item_qty[$i]=='Available' || $item_qty[$i]=='-') $qty_print=$item_qty[$i]; else
		if($sub_system!=0){ if($item_qty[$i]>100) $qty_print='100+'; else $qty_print=$item_qty[$i]; }else{ $qty_print=number_format($item_qty[$i]); }
		print '<tr><td align="center">'.$item_dr[$i].'</td>
		...
		if($retail_shop==1) print '<td style="text-align:right; padding-right:10px">'.number_format($item_r_price[$i],$decimal).'</td>';
		print '<td style="text-align:right; padding-right:10px">'.$qty_print.'</td></tr>';
	}
	$qty_print='';
?>
</table>
...
<tr><td width="50px"></td><td style="font-size:12pt">List By Category</td><td colspan="2">
		<select id="category" onchange="window.location = 'index.php?components=availability&action=home&category='+document.getElementById('category').value">
...
<?php
	if(isset($_GET['category'])){
		...
		?>
			...
			<tr bgcolor="#CCCCCC"><th>Item Code</th><th>Description</th><th>Wholesale Price</th><th>Retail Price</th><th>Quantity</th><th>Location</th></tr>
			<?php 
