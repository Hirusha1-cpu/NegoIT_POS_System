...
<?php
	if(sizeof($cat_desc)==0) print '<input type="hidden" id="filter_cat" value="all" />';
	if($direct_mkt==0){
		$title_price='W/Price';
...
<?php if(isset($_REQUEST['cust_id'])||($direct_mkt==1)){ ?>
	<table align="center" style="font-family:Calibri; font-size:11pt" >
		<tr style="background-color:#467898;color :white;">
			...
			<th class="shipmentTB3">Category</th>
			<th class="shipmentTB3">Item</th>
			<th class="shipmentTB3">
				<table style="background-color:#467898;color :white;">
					...
					<tr>
						<td><input type="checkbox" id="order_act" <?php if($qty_order!='') print 'checked="checked"'; ?>
								onchange="activateOrder()" /></td>
						<td style="font-size:6pt;" valign="middle">Order</td>
					</tr>
				</table>
			</th>
			<th class="shipmentTB3"><?php print $title_price; ?></th>
			<th class="shipmentTB3"><?php print $title_discount; ?></th>
			<?php print $title_lastdiscount; ?><?php if($item_id!='') print '<th>Store</th>'; ?>
		</tr>
		<tr style="background-color:#467898;color :white;">
			<th></th>
			<th>
			...
			<th></th>
			<th></th>
			<th></th><?php print $title_filter; if($item_id!='') print '<th></th>'; ?>
		</tr>
	<?php
	for($i=0;$i<sizeof($cat_desc);$i++){
		if($direct_mkt==1) $price=$r_price[$i]; else $price=$w_price[$i];
		if($direct_mkt==1) $discount=$cat_max_r_rate[$i].'%'; else $discount=$cat_min_w_rate[$i].'% ~
		'.$cat_max_w_rate[$i].'%';
		if($direct_mkt==1) $lastdiscount=''; else $lastdiscount='<td class="shipmentTB3">
			<div id="div_discount'.$cat_itemid[$i].'"><input type="button" value="Last Discount"
					onclick="loadDiscount('.$cat_itemid[$i].','.$price.');" style="font-size:9pt;" /></div>
		</td>';
		if($sold_color[$i]==3){ if(($i%2)==0) $color='style="background-color:#FAFAFA"'; else
		...
		print '<tr '.$color.'>
			...
			<td class="shipmentTB3">'.$cat_category[$i].'</td>
			<td class="shipmentTB3">'.$cat_desc[$i].'</td>
			<td class="shipmentTB3" align="right">'.$cat_qty[$i].'</td>
			<td class="shipmentTB3" align="right">'.number_format($price,$decimal).'</td>
			<td class="shipmentTB3">'.$discount.'</td>'.$lastdiscount;
	...
	<div id="printheader" style="display:none">
		...
		<table style="font-size:12pt" border="1" cellspacing="0">
			<tr>
				<td width="150px" style="background-color:#C0C0C0; padding-left:10px">Category</td>
				<td width="200px" style="background-color:#EEEEEE; padding-left:10px"><?php print $selected_catname; ?>
				</td>
			</tr>
			<tr>
				<td width="150px" style="background-color:#C0C0C0; padding-left:10px">Generated Date&nbsp;&nbsp;&nbsp;</td>
				<td width="200px" style="background-color:#EEEEEE; padding-left:10px"><?php print $today; ?></td>
			</tr>
		</table>
		<br />
	</div>
	...
	<div id="print" style="display:none">
		<table align="center" style="font-family:Calibri; font-size:11pt" border="1" cellspacing="0">
			<tr style="background-color:#467898;color :white;">
				...
				<th>QTY</th>
				<th>W/Price</th>
				<th>Discount Range</th><?php if($item_id!='') print '<th>Store</th>'; ?>
			</tr>
			<?php
				for($i=0;$i<sizeof($cat_desc);$i++){
					print '<tr>
						<td align="center">&nbsp;&nbsp;&nbsp;'.sprintf('%02d',($i+1)).'&nbsp;&nbsp;&nbsp;</td>
						<td>&nbsp;&nbsp;&nbsp;'.$cat_category[$i].'&nbsp;&nbsp;&nbsp;</td>
						<td>&nbsp;&nbsp;&nbsp;'.$cat_desc[$i].'&nbsp;&nbsp;&nbsp;</td>
						<td align="right">&nbsp;&nbsp;&nbsp;'.$cat_qty[$i].'&nbsp;&nbsp;&nbsp;</td>
						<td align="right">&nbsp;&nbsp;&nbsp;'.number_format($w_price[$i],$decimal).'&nbsp;&nbsp;&nbsp;</td>
						<td>&nbsp;&nbsp;&nbsp;'.$cat_min_w_rate[$i].'% - '.$cat_max_w_rate[$i].'% &nbsp;&nbsp;&nbsp;</td>';
