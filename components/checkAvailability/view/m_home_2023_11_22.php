...
<?php if($current_district!=''){ ?>
	<table>
	<tr style="font-size:large; background-color:#EEEEEE"><td>Item Code</td><td colspan="2"><input type="text" name="code" id="tags1" value="<?php print $search_code; ?>" onfocus="this.value=''" /></td></tr>
	<tr style="font-size:large; background-color:#EEEEEE"><td>Item Description</td><td colspan="2"><input type="text" name="description" id="tags2" value="<?php print $search_description; ?>" onfocus="this.value=''" onclick="clearBox()" /></td></tr>
	<tr style="font-size:large; background-color:#EEEEEE"><td colspan="3" align="center"><input type="Submit" value="Check Availability" style="width:170px; height:60px" onclick="getPrice()" /><br /><br /></td></tr>
	<tr><td colspan="3" style="font-size:12pt">
		<table border="1" cellspacing="0" width="90%" align="center" >
		<tr><th>DR</th><th>Store</th><?php if($retail_shop==1) print '<th>R/Price</th>'; else print '<th>W/Price</th>'; ?><th>Qty</th></tr>
	<?php
	...
	<tr style="font-size:large; background-color:#EEEEEE"><td>List By Category</td><td colspan="2">
	...
	<?php } ?>
	<tr style="font-size:large; background-color:#EEEEEE"><td colspan="3"><br /><br /></td></tr>
	</table>
  </div>
</div>
<hr>
...
<?php if($current_district!=''){ ?>
		<?php
			...
			<tr><th>Item Code</th><th>Description</th><th>Wholesale Price</th><th>Retail Price</th><th>Quantity</th><th>Location</th></tr>
			<?php
				for($i=0;$i<sizeof($id);$i++){
					if($_GET['category']==$cat_id[$i])
						print '<tr><td>'.$code[$i].'</td><td>'.$description[$i].'</td><td align="right">'.number_format($w_price[$i]).'</td><td align="right">'.number_format($r_price[$i]).'</td><td>'.number_format($qty[$i]).'</td><td>'.$sto_name[$i].'</td></tr>';
				}
