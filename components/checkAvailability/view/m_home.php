												...
												<table border="1" cellspacing="0" width="100%" align="center">
													...
													<?php
														for($i=0;$i<sizeof($store_name);$i++){
															...
															print '<td style="text-align:right; padding-right:10px">'.$item_qty[$i].'</td>
															</tr>';
														}
													?>
												</table>
							...
							if(isset($_GET['category'])){
								if($_GET['category']!=='off'){
									if(($systemid!=1)&&($systemid!=4)){ ?>
										<div class="table-responsive">
											<table align="center" bgcolor="#EFEFEF" border="1" width="100%" cellspacing="0">
												<tr>
													<th width="60px">#</th>
													<th>Item Code</th>
													<th>Description</th>
												...
												<?php
													for($i=0;$i<sizeof($id);$i++){
														if($_GET['category']==$cat_id[$i])
															print '<tr>
															<td align="center" style="padding: 0px 5px;">' . sprintf('%02d', ($i + 1)) . '</td>
															<td style="padding: 0px 5px;">' .$code[$i].'</td> <td>
															'.$description[$i].'</td>
															<td align="right" style="padding: 0px 5px;">'.number_format($w_price[$i]).'</td>
															<td align="right" style="padding: 0px 5px;">'.number_format($r_price[$i]).'</td>
															<td style="padding: 0px 5px;">'.number_format($qty[$i]).'</td>
															<td style="padding: 0px 5px;">'.$sto_name[$i].'</td>
															</tr>';
													}
												?>
											</table>
