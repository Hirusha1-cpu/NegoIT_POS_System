...
<?php if ($current_district != '') { ?>
	<script type="text/javascript">
		$(function () {
			var availableTags1 = [<?php for ($x = 0; $x < sizeof($code_uni); $x++) {
				print '"' . $code_uni[$x] . '",';
			} ?>];
			$("#tags1").autocomplete({
				source: availableTags1
			});
			var availableTags2 = [<?php for ($x = 0; $x < sizeof($description_uni); $x++) {
				print '"' . $description_uni[$x] . '",';
			} ?>];
			$("#tags2").autocomplete({
		...
		function getPrice() {
			var code = [<?php for ($x = 0; $x < sizeof($code_uni); $x++) {
				print '"' . $code_uni[$x] . '",';
			} ?>];
			var description = [<?php for ($x = 0; $x < sizeof($description_uni); $x++) {
				print '"' . $description_uni[$x] . '",';
			} ?>];
					...
					<table align="center" bgcolor="#E5E5E5">
						...
						<tr>
							...
							<td colspan="2"><input type="text" name="description" id="tags2"
									value="<?php print $search_description; ?>" onfocus="refresh('tags2')"
									onclick="clearBox()" /></td>
							<td width="50px"></td>
						</tr>
						<tr>
						...
						<tr>
							<td colspan="5" align="center"><input type="submit" onfocus="getPrice()"
									value="Check Availability"
									style="width:130px; height:40px; font-weight:bold" /><br /><br /></td>
						</tr>
						<tr>
							<td colspan="5" style="font-size:12pt">
								<table border="1" cellspacing="0" width="90%" align="center" style="font-family:Calibri; font-size:10pt;">
									<tr>
										<th width="60px">#</th>
										<th>DR</th>
										<th>Store</th>
										<th>W/Price</th><?php if ($retail_shop == 1)
											print '<th>R/Price</th>'; ?>
										<th>Qty</th>
									</tr>
									<?php
									for ($i = 0; $i < sizeof($store_name); $i++) {
										if ($item_qty[$i] == 'Available' || $item_qty[$i] == '-')
											$qty_print = $item_qty[$i];
										else if ($sub_system != 0) {
											if ($item_qty[$i] > 100)
												$qty_print = '100+';
											else
												$qty_print = $item_qty[$i];
					...
					if (isset($_GET['category'])) {
						if ($_GET['category'] !== 'off') {
							if (($systemid != 1) && ($systemid != 4)) { ?>
								<table align="center" bgcolor="gray" style="font-family:Calibri; font-size:10pt;">
									<tr bgcolor="#CCCCCC">
										<th width="60px">#</th>
										<th>Item Code</th>
										<th>Description</th>
										...
										if ($_GET['category'] == $cat_id[$i]) {
											if ($qty[$i] == 'Available' || $qty[$i] == '-')
												...
												if ($sub_system != 0) {
													...
													$qty_print = number_format($qty[$i]);
												}
											print '<tr bgcolor="#EEEEEE">
													<td align="center">' . sprintf('%02d', ($i + 1)) . '</td>
													<td style="padding-left:10px">' . $code[$i] . '</td>
													<td style="padding-left:10px; padding-right:10px">' . $description[$i] . '</td>
													<td align="right" style="padding-right:10px">' . number_format($w_price[$i], $decimal) . '</td>
													<td align="right" style="padding-right:10px">' . number_format($r_price[$i], $decimal) . '</td>
													<td align="right" style="padding-right:10px">' . $qty_print . '</td>
													<td style="padding-left:10px; padding-right:10px">' . $sto_name[$i] . '</td>
												</tr>';
										}
									}
