...
$inv=0;
	for($i=0;$i<sizeof($invoice_no);$i++){
		if($bi_discount[$i]>0){ $color='style="color:red"'; $title='title="Discounted Invoice"'; }else{ $color=''; $title=''; }
			print '<tr '.$color.'><td>&nbsp;&nbsp;<a '.$title.' href="index.php?components=billing&action=finish_bill&id='.$invoice_no[$i].'">'.str_pad($invoice_no[$i], 7, "0", STR_PAD_LEFT).'</a>&nbsp;&nbsp;&nbsp;&nbsp;</td><td width="50px"  align="center">'.$time[$i].'</td><td width="50px" align="right">'.$invoice_total[$i].'&nbsp;&nbsp;</td><td>&nbsp;&nbsp;'.$cust[$i].'</td></tr>';
	}
?>	
	</table>
	<br>
...
$inv=0;
	for($i=0;$i<sizeof($linvoice_no);$i++){
			$edit='<input type="Button" value="Edit"  onclick="window.location = '."'".'index.php?components=billing&action=home&id='.$linvoice_no[$i].'&s=1&cust='.$lcust[$i]."'".'" />';
			print '<tr><td>&nbsp;&nbsp;<a href="index.php?components=billing&action=finish_bill&id='.$linvoice_no[$i].'">'.str_pad($linvoice_no[$i], 7, "0", STR_PAD_LEFT).'</a>&nbsp;&nbsp;&nbsp;&nbsp;'.$edit.'</td><td width="50px"  align="center">'.$ldate[$i].'</td><td width="50px" align="right">'.$linvoice_total[$i].'&nbsp;&nbsp;</td><td>&nbsp;&nbsp;'.$lcust[$i].'</td></tr>';
	}
?>	
	</table>
	<br>
...
$inv=0;
	for($i=0;$i<sizeof($py_invno);$i++){
			print '<tr><td align="center"><a href="index.php?components=billing&action=finish_payment&id='.$py_invno[$i].'">'.str_pad($py_invno[$i], 7, "0", STR_PAD_LEFT).'</a></td><td width="50px" align="center">'.$py_time[$i].'</td><td align="right">'.number_format($py_amount[$i]).'&nbsp;&nbsp;</td><td>&nbsp;&nbsp;'.$py_cust[$i].'</td></tr>';
	}
?>	
	</table>
	<br>
...
$inv=0;
	for($i=0;$i<sizeof($rt_invno);$i++){
			print '<tr><td align="center"><a href="index.php?components=billing&action=finish_return&id='.$rt_invno[$i].'">'.str_pad($rt_invno[$i], 7, "0", STR_PAD_LEFT).'</a></td><td width="50px" align="center">'.$rt_time[$i].'</td><td>&nbsp;&nbsp;'.$rt_cust[$i].'</td></tr>';
	}
?>	
	</table>
  </div>
