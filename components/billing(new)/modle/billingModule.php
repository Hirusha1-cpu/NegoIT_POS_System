...
global $discount,$unic_qty,$id,$code,$description,$w_price,$r_price,$cost,$drawer,$qty,$tt_item,$tt_qty,$unic,$pr_sr,$unic_item_code,$unic_item_list,$unic_item_list2,$is_unic_item;
	...
	if(isset($_GET['unic'])){
		...
		if($_GET['unic']=='yes'){
			...
			$is_unic_item=$row1[1]; 
			$query1="SELECT itu.sn,itq.qty FROM inventory_items itm, inventory_qty itq, inventory_unic_item itu WHERE itm.id=itq.item AND itu.itq_id=itq.id AND itq.`location`='$store' AND itu.`status`='$itu_status' AND itm.id='$unicitemid'";
			$result1=mysqli_query($conn,$query1);
			while($row1=mysqli_fetch_array($result1)){	
				$unic_item_list[]=$row1[0]; 
				$unic_qty=$row1[1]; 
			} 
			$query1="SELECT itu.sn FROM inventory_items itm, inventory_qty itq, inventory_unic_item itu WHERE itm.id=itq.item AND itu.itq_id=itq.id AND itu.`status`=1 AND itm.id='$unicitemid'";
			$result1=mysqli_query($conn,$query1);
			while($row1=mysqli_fetch_array($result1)){	
	...
	if($filter_product){
		$query="SELECT inv.id,inv.code,inv.description,inq.w_price,inq.r_price,inq.c_price,inq.drawer_no,inq.qty,inv.`category`,inv.`unic`,inv.pr_sr FROM inventory_items inv, inventory_qty inq WHERE inv.id=inq.item AND inq.location='$store' AND inv.`status`=1 AND inv.pr_sr=1 $qry_filter";
		$result=mysqli_query($conn,$query);
		while($row=mysqli_fetch_array($result)){
		...
		$query3="SELECT itm.id,itn.qty FROM inventory_new itn, inventory_items itm WHERE itn.item=itm.id AND store='$store'";
	...
	if($filter_service){
		if($item_filter==2) $qry_pr_sr="AND inv.pr_sr='2'"; else
		if($item_filter==3) $qry_pr_sr="AND inv.pr_sr='3'"; else
		$qry_pr_sr="AND inv.pr_sr IN (2,3)";
		$query="SELECT inv.id,inv.code,inv.description,inv.default_price,inv.pr_sr FROM inventory_items inv WHERE inv.`status`=1 $qry_pr_sr";
		$result=mysqli_query($conn,$query);
		while($row=mysqli_fetch_array($result)){
			$id[]=$row[0];
			$code[]=$row[1];
		...
		if($_COOKIE['district']!=0){
			...
			if($cust_odr=='yes'){ $bm_status=1; $bm_type=4; }
				...
				*/
				...
				//$query="INSERT INTO `bill_main` (`invoice_no`,`order_hash`,`authorize_code`,`billed_district`,`quotation_no`,`cust`,`billed_by`,`sys_user`,`order_timestamp`,`billed_timestamp`,`type`,`store`,`mapped_inventory`,`gps_x`,`gps_y`,`sub_system`,`status`) VALUES ('$invoice_no','$orderhash','$authorize_code','$district','$quot_no','$cust','$salesman','$sys_user','$time_now','$time_now','$bm_type','$store','$mapped_inventory','$gps_x','$gps_y','$sub_system','$bm_status')";
				$query="INSERT INTO `bill_main` (`invoice_no`,`order_hash`,`authorize_code`,`billed_district`,`quotation_no`,`cust`,`billed_by`,`sys_user`,`order_timestamp`,`billed_timestamp`,`type`,`store`,`mapped_inventory`,`gps_x`,`gps_y`,`sub_system`,`status`) VALUES ((SELECT MAX(b2.invoice_no)+1 FROM bill_main b2),'$orderhash','$authorize_code','$district','$quot_no','$cust','$salesman','$sys_user','$time_now','$time_now','$bm_type','$store','$mapped_inventory','$gps_x','$gps_y','$sub_system','$bm_status');";
...
function apendBill($case,$systemid,$storecrossitm,$storecrossst){
	...
	if(is_numeric($discount)){
		...
		if($proceed){ if(($ivq_id!='')||($pr_sr=='2')||($pr_sr=='3')){ $proceed=true; }else{ $proceed=false; $msg='Error: Store Mismatch'; }}
		if($proceed){ if($bm_lock==0){ $proceed=true; }else{ $proceed=false; $msg='You Canot Add Items to a Finalized Invoice'; }}
		if($proceed){ if(mismatch($ivq_id)){ $proceed=true; }else{ $proceed=false; $msg='Error 107. Please contact Support !'; }}
...
global $print_time,$key_dev_name,$tm_web,$tm_email;
	$key_dev_name='';
	$isMobile=isMobile();
...
global $tm_company,$tm_address,$tm_tel,$chq0_fullNo,$bill_id,$bi_desc,$bi_code,$bi_discount,$bi_qty,$bi_price,$total,$ledc2,$bi_drawer,$bi_type,$pay_id,$cash_amount,$chque_amount,$chq0_date,$bi_cust0,$bi_cust0_address,$bi_cust,$bi_salesman_id,$up_salesman,$bi_date,$bi_time,$cu_id,$cu_details,$up_mobile,$bm_status,$bm_quotation_no,$qm_warranty,$qm_terms,$qm_po,$bm_packed_by,$tm_shop,$bm_print_st,$bm_bocom_type,$bm_bocom,$bi_repair_sn,$up_packedby,$pay_type,$bi_return_odr,$return_odr,$bill_cross_tr;
	$invoice_no=$_REQUEST['id'];
...
//		$query="SELECT bi.id,itm.description,bi.qty,bi.unit_price,itm.id,bi.`comment`,itm.code,bi.discount,itm.unic,bi.repair_model,bi.repair_sn,bi.return_odr_replace,bi.cross_trans FROM bill_main bm ,bill bi, inventory_items itm, cust cu, userprofile up WHERE bm.invoice_no=bi.invoice_no AND up.id=bm.billed_by AND bm.`cust`=cu.id AND bi.item=itm.id AND bi.invoice_no='$invoice_no' ORDER BY $order_by";
		if(($bi_type==1)||($bi_type==4)){
			if($order_by=='itq.drawer_no, bi.id'){
				$query="SELECT bi.id,itm.description,bi.qty,bi.unit_price,itm.id,bi.`comment`,itm.`code`,bi.discount,itm.unic,bi.repair_model,bi.repair_sn,bi.return_odr_replace,bi.cross_trans,itq.drawer_no FROM bill_main bm ,bill bi, inventory_items itm, cust cu, userprofile up, inventory_qty itq WHERE bm.invoice_no=bi.invoice_no AND up.id=bm.billed_by AND bm.`cust`=cu.id AND bi.item=itm.id AND itm.id=itq.item AND itq.location=bm.mapped_inventory AND bi.invoice_no='$invoice_no' ORDER BY $order_by";
			}else{
				$query="SELECT bi.id,itm.description,bi.qty,bi.unit_price,itm.id,bi.`comment`,itm.`code`,bi.discount,itm.unic,bi.repair_model,bi.repair_sn,bi.return_odr_replace,bi.cross_trans,bm.mapped_inventory FROM bill_main bm ,bill bi, inventory_items itm, cust cu, userprofile up WHERE bm.invoice_no=bi.invoice_no AND up.id=bm.billed_by AND bm.`cust`=cu.id AND bi.item=itm.id AND bi.invoice_no='$invoice_no' ORDER BY $order_by";
			}
		}else{
			$query="SELECT bi.id,itm.description,bi.qty,bi.unit_price,itm.id,bi.`comment`,itm.`code`,bi.discount,itm.unic,bi.repair_model,bi.repair_sn,bi.return_odr_replace,bi.cross_trans,bm.mapped_inventory FROM bill_main bm ,bill bi, inventory_items itm, cust cu, userprofile up WHERE bm.invoice_no=bi.invoice_no AND up.id=bm.billed_by AND bm.`cust`=cu.id AND bi.item=itm.id AND bi.invoice_no='$invoice_no' ORDER BY bi.id";
		}
		$result=mysqli_query($conn,$query);
		while($row=mysqli_fetch_array($result)){
			$bill_id_tmp=$row[0];
			...
			$item_id=$row[4];
			$bm_mapped_inv=$row[13];
			
			if($order_by=='itq.drawer_no, bi.id'){
				if(($bi_type==1)||($bi_type==4)) $bi_drawer[]=$row[13]; else $bi_drawer[]='<br /><br /><br />';
			}else{
				$query2="SELECT itq.drawer_no FROM inventory_qty itq WHERE itq.item='$item_id' AND itq.location='$bm_mapped_inv'";
				$row2=mysqli_fetch_row(mysqli_query($conn,$query2));
				if(($bi_type==1)||($bi_type==4)) $bi_drawer[]=$row2[0]; else $bi_drawer[]='<br /><br /><br />';
			}
	...
	*/
		...
		while($row1=mysqli_fetch_array($result1)){
			...
			if($row1[1]==4) $cash_amount=$row1[2];
			$chq0_no=$row1[3];
			$chq0_bnk=$row1[4];
			$chq0_branch=$row1[5];
			$chq0_date=$row1[6];
...
global $invoice_no,$time,$invoice_total,$invoice_profit,$date,$lbm_type,$color,$cust,$py_id,$py_invno,$py_bill_pay,$py_type,$py_time,$py_amount,$py_cust,$linvoice_no,$ldate,$linvoice_total,$linvoice_profit,$lcust,$lcustid,$lbm_status,$bi_discount,$rt_invno,$rt_time,$rt_cust_id,$rt_cust_name,$rt_status,$lbm_salesman,$bm_sys_user,$lbm_sys_user,$py_sys_user,$rt_return_by;
	$salesman=$_COOKIE['user_id'];	
	$invoice_no=$linvoice_no=$py_id=$py_invno=$rt_invno=array();
...
function addPayment($ocation){
	...
	if($ocation=='bill'){
		...
		$amount_card=$_REQUEST['amount_card'];
		...
		if($amount_card<>0){
			...
			if(strlen($card_no) == 4){
				$card_no = "**** **** **** ". $card_no;
					...
					$card_no = "**** **** **** ". substr($card_no,15);
			}else{
				$out=false;
				$msg='Error: Please eneter a valid Payment Card Number!';
			}
			$payment_type=4;
			$tran_bnk_n1=',`bank_trans`,`card_no`';
			$tran_bnk_v1=",'$card_bank','$card_no'";
    	}
	}
	if($ocation=='pay'){
	...
	if($out){
		...
		if($emptyamount){
			...
			if(($payment_type==4) && ($proceed)){
				$query="INSERT INTO `payment` (`invoice_no`,`bill_pay`,`cust`,`payment_type`,`amount`,`salesman`,`sys_user`,`payment_date`,`comment`,`store`,`gps_x`,`gps_y`,`sub_system` $tran_bnk_n1) VALUES 
				('$invoice_no','$bill_pay','$cust','4','$amount_card','$salesman','$sys_user','$time_now','$comment','$store','$gps_x','$gps_y','$sub_system' $tran_bnk_v1)";
				$result2=mysqli_query($conn,$query);
				if($result2){
					$result2=true;
				}
		...
		if(($lock_status==0)&&($ocation=='bill')&&($result2)){
			...
			$query="UPDATE bill_main SET `lock`='$lock_new', `billed_timestamp`='$today', `invoice_+total`='$payment1', `invoice_-total`='$payment2' WHERE invoice_no='$invoice_no'";
			$result=mysqli_query($conn,$query);
...
global $message,$cust,$bm_type,$payment_id,$invoice_no;
	...
	if(isset($_REQUEST['comment'])) $comment=$_REQUEST['comment']; else $comment='';
	if(isset($_REQUEST['cash_bank_switch'])){ 
		if($_REQUEST['cash_bank_switch']==3){
			$payment_type=1;
			$tr_bank=$_POST['tr_bank'];
			$cash_type=3; 
			$tran_bnk_n=',`bank_trans`';
			$tran_bnk_v=",'$tr_bank'";
		}elseif($_REQUEST['cash_bank_switch']==4){
			$payment_type=1;
			$card_bank=$_REQUEST['card_bank'];
			$card_no=$_REQUEST['card_no'];
			$cash_type=4; 
	...
	if($result2){
		if($message=='') $message='Payment was Added Successfully!';
		return true;
	}else{
		if($message=='') $message='Payment could not be Added!';
		return false;
	}
}
