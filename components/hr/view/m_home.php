	...
	function validateCheckOut(){
		...
		if((gps_x==0)||(gps_y==0)){
			...
			return false;
	  		...
	  		document.getElementById('checkout_div').innerHTML=document.getElementById('loading').innerHTML;
...
<!-- ------------------Item List----------------------- -->
<?php if($checkout){ ?>
  	<form id="checkout_form" action="index.php?components=hr&action=set_check_out" method="post">
	  	<input type="hidden" id="gps_x" name="gps_x" value="0" />
		<input type="hidden" id="gps_y" name="gps_y" value="0" />
	  <table align="center">
	  	<tr><td align="center"><div id="checkout_div"><a onclick="validateCheckOut()" style="cursor:pointer"><img src="images/check_out.png" style="width:150px; vertical-align:middle" /><br />Check Out</a></div></td></tr>
	  </table>
	</form>
	<hr />
<?php } ?>
	<table align="center"><tr><td>
		...
		<table align="center" bgcolor="gray" style="border-radius: 10px; font-family:Calibri"><tr><td>
			...
			<table border="0" cellspacing="0" bgcolor="white" style="border-radius: 10px;" align="center" border="0"  style="font-size:12pt; font-family:Calibri">
				...
				</td><td></td><td><strong>User</strong></td><td>
					<select id="leave_user" name="leave_user" onchange="setLeaveUser()">
						<?php 
	...
	<!-- -----------------------------------LEAVE LOG------------------------------------------- -->
		<table align="center" bgcolor="gray" style="border-radius: 10px; font-family:Calibri"><tr><td>
			<table border="0" cellspacing="1" bgcolor="white" style="border-radius: 10px;" align="center" border="0"  style="font-size:12pt; font-family:Calibri">
				<tr bgcolor="#467898" style="color:#F5F5F5"><th class="shipmentTB4">Leave Type</th><th class="shipmentTB4">From Date</th><th class="shipmentTB4">To Date</th><th class="shipmentTB4">Duration</th><th class="shipmentTB4">Status</th></tr>
				<?php 
				...
				for($i=0;$i<sizeof($llog_id);$i++){
					...
					print '<tr bgcolor="'.$color.'" ><td class="shipmentTB4"><a href="'.$url.'" title="Applied on: '.substr($llog_apply_date[$i],0,16).'" style="text-decoration:none;">'.$llog_type[$i].'</a></td><td class="shipmentTB4" align="center">'.$llog_from_date[$i].'</td><td class="shipmentTB4" align="center">'.$llog_to_date[$i].'</td><td class="shipmentTB4" align="center">'.$llog_days[$i].'</td><td class="shipmentTB4" align="center" style="color:'.$llog_st_color[$i].'">'.$llog_st_name[$i].'</td></tr>';
				}
				?>
			</table>
		</td></tr></table>
