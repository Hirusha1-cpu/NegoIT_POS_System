...
function addExpense()
{
	...
	try {
		...
		if ($out) {
			$result = mysqli_query($conn2, "SELECT MAX(expense_id) as `maxid` FROM expense_main");
			if (!$result) {
				$out = false;
		...
		if ($out) {
			if (count($expense_array) > 0) {
				$result2 = mysqli_query($conn2, "SELECT `name` FROM expense_paymethod WHERE `id`='$method'");
				$row2 = mysqli_fetch_assoc($result2);
				$payment_method = $row2['name'];

				$result1 = mysqli_query($conn2, "SELECT `qb_account_id` FROM accounts WHERE `id`='$from_account'");
				$row = mysqli_fetch_assoc($result1);
				...
				if ($qb_payee_type == 'vendor') {
					$result1 = mysqli_query($conn2, "SELECT `qb_account_id` FROM accounts WHERE `name`='$payee'");
					$row = mysqli_fetch_assoc($result1);
					$payee_ref = $row['qb_account_id'];
				}

				if ($qb_payee_type == 'customer') {
					$result1 = mysqli_query($conn2, "SELECT `qb_cust_id` FROM cust WHERE `name`='$payee'");
					$row = mysqli_fetch_assoc($result1);
					$payee_ref = $row['qb_cust_id'];
				}

				if ($qb_payee_type == 'employee') {
					$result1 = mysqli_query($conn2, "SELECT `qb_id` FROM userprofile WHERE `username`='$payee'");
					$row = mysqli_fetch_assoc($result1);
					$payee_ref = $row['qb_id'];
				}
				if ($payee_ref != '') {
		...
		error_log("Error in addExpense(): " . $message); // Log the error
	}
...
function addJournal()
{
	...
	try {
						...
						if (($out > 0) && (isQuickBooksActive(1))) {
							...
							if ($account_name == 'Accounts Receivable (A/R)') {
								...
								$journal_entries_array[] = [
									...
									'posting_type' => $cr_dr,
									'account_id' => $qb_account_id,
									'account_name' => $account_name // Including the account name if necessary
								];
							}
						}
					}
			...
			if ($flag) {
				if (count($journal_entries_array) > 0) {
					...
					foreach ($journal_entries_array as $entry) {
						...
						$query = "INSERT INTO qb_queue (`action`,`batch_id`, `journal_id`, posting_type, account_id, account_name, amount, description, entity_type, entity_id)
								VALUES ('$action_name','$batch_id','$journal_id', '$posting_type', '$account_id', '$account_name', '$amount', '$description',
		...
		$message = $e->getMessage();
		$message = $message . ' ' . (isset($qb_msg) ? $qb_msg : '');
		error_log("Error in addJournal(): " . $message); // Log the error
		return false;
	}
}
...
function getCahrtOfAccounts()
{
	...
	include('config.php');
	$query = "SELECT ac.id,ac.name,ac.system_ac,ac.`status`,acat.category_level1,acat.category_level2,acat.category_level3,ac.parent_account_id FROM accounts ac, account_category acat $table WHERE ac.category=acat.id $filter_accounts ORDER BY acat.category_level1,acat.category_level2,acat.category_level3,ac.parent_account_id";
	$result = mysqli_query($conn, $query);
...
function addCahrtOfAccounts()
{
	...
	try {
		...
		mysqli_rollback($conn);
		$message = $e->getMessage();
		error_log("Error in addCahrtOfAccounts(): " . $message); // Log the error
	}
	$message = $message . ' ' . $qb_msg;
	return $out;
}
...
function editCahrtOfAccounts()
{
	...
	try {
		...
		mysqli_rollback($conn);
		$message = $e->getMessage();
		error_log("Error in editCahrtOfAccounts(): " . $message); // Log the error
	}
	$message = $message . ' ' . $qb_msg;
	return $out;
}
...
function setStCahrtOfAccounts()
{
	...
	try {
		...
		mysqli_rollback($conn);
		$message = $e->getMessage();
		error_log("Error in setStCahrtOfAccounts(): " . $message); // Log the error
	}
	$message = $message . ' ' . $qb_msg;
	return $out;
}
