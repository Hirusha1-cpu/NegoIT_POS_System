...
function getCahrtOfAccounts()
{
  ...
  ac.`system_ac`,
  ac.`status`,
  acat.`category_level1`,
  acat.`category_level2`,
  acat.`category_level3`,
  ac.`parent_account_id`
	FROM
		accounts AS ac
	JOIN
		account_category AS acat ON ac.`category` = acat.`id`
	LEFT JOIN
		supplier AS s ON ac.`name` = s.`name`
	WHERE
  s.`name` IS NULL -- hide supplier from the query
  $filter_accounts
	ORDER BY
		acat.`category_level1`,
		acat.`category_level2`,
