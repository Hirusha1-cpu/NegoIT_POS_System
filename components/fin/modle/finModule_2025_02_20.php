...
function addJournal(){
	...
	include('config.php');
	$result = mysqli_query($conn2,"SELECT MAX(journal_id) as `maxid` FROM journal_main");
	$row = mysqli_fetch_assoc($result);
	...
	for($i=1;$i<=10;$i++){
		...
		if($exp!=''){
			$payee=$_POST['payee'.$i];
			$account_name=substr($exp,0,strpos($exp,'-'));
			$result1 = mysqli_query($conn2,"SELECT id FROM accounts WHERE name='$account_name'");
			$row = mysqli_fetch_assoc($result1);
			$account_id=$row['id'];
			if($payee_type!=''){
				if($payee_type=='customer') $query="SELECT id FROM cust WHERE name='$payee'";
			...
			// 	$result1 = mysqli_query($conn2,"SELECT qb_account_id FROM accounts WHERE `id`='$account_id'");
			// 	$row = mysqli_fetch_assoc($result1);
			// 	$qb_account_id=$row['qb_account_id'];
			// 	if($cr_dr == 'cr') $posting_type = 'Credit'; else $posting_type = 'Debit';
			// 	$journal_entry_array = array('description'=>$des, 'amount'=>$amount, 'posting_type'=> $posting_type, 'account_id' => $qb_account_id);
			// 	$qb_result = QBAddJournalEntry($journal_entry_array);
			...
			// 			if($last_id != ''){
			// 				$query = "UPDATE `journal_item` SET `qb_id`='$qb_journal_entry_id' WHERE `id`='$last_id'";
			// 				$result = mysqli_query($conn, $query);
			// 			}
			// 		}
			// 	}
	...
	if($out>0){
		...
		$message='Journal could not be Added. '.$qb_msg;
		return false;
	}
}
