...
function getExpenseFormData(){
	...
	$query="SELECT ac.`id`,ac.`name`,acat.`category_level1` FROM accounts ac, account_category acat, account_category_order odr WHERE ac.`category`=acat.`id` AND acat.`category_level1`=odr.`categoryL1` AND ac.`status`='1' AND odr.`order`='1' ORDER BY odr.`order`, ac.`name`";
	$result=mysqli_query($conn,$query);
	while($row=mysqli_fetch_array($result)){
		$count=strlen($row[1]);
		$ac_id[]=$row[0];
...
function addExpense(){
	...
	include('config.php');
	if($payee_type=='customer') $query="SELECT id FROM cust WHERE name='$payee'";
	if($payee_type=='supplier') $query="SELECT id FROM supplier WHERE name='$payee'";
	if($payee_type=='employee') $query="SELECT id FROM userprofile WHERE username='$payee'";
	if($payee_type=='other') $query="SELECT id FROM other_payee WHERE name='$payee'";
	if($out){
		$result = mysqli_query($conn,$query);
	...
	if($out){
		...
		if(!$result){
			...
			if($expense_id=='') $expense_id=1; else $expense_id++;
		}	
	}
		...
		if($out){
			for($i=1;$i<=10;$i++){
				...
				if($exp!=''){
					...
					if(!$result2){
						$out=false;
						$message="Error: Expenses Itrm table is not updated!";
					}else{
						$out++;
					}
				}
...
function listExpense(){
	...
	while($row=mysqli_fetch_array($result)){
		...
		if($payee_type=='customer') $query1="SELECT name FROM cust WHERE id='$payee_id'";
		if($payee_type=='supplier') $query1="SELECT name FROM supplier WHERE id='$payee_id'";
		if($payee_type=='employee') $query1="SELECT username as `name` FROM userprofile WHERE id='$payee_id'";
		if($payee_type=='other') $query1="SELECT name FROM other_payee WHERE id='$payee_id'";
...
function addJournal(){
	...
	for($i=1;$i<=10;$i++){
		...
		if($cr!=''){ $cr_dr='cr'; $amount=-$cr; }
		if($dr!=''){ $cr_dr='dr'; $amount=$dr; }
	...
	if($out>0){
		...
		$message='Journal could not be Added';
		return false;
	}
}
	...
	include('config.php');
		...
		while($row=mysqli_fetch_array($result)){
			...
			while($row1=mysqli_fetch_array($result1)){
				...
				if($payee_type=='customer'){ $payee_array_id=$cust_id; $payee_array_name=$cust_name; }
				if($payee_type=='supplier'){ $payee_array_id=$sup_id; $payee_array_name=$sup_name; }
				if($payee_type=='employee'){ $payee_array_id=$user_id; $payee_array_name=$user_name; }
				if($payee_type=='other'){ $payee_array_id=$other_id; $payee_array_name=$other_name; }
				if($payee_type!=''){
...
global $message;
	...
	if($result1) $result2 = mysqli_query($conn,"DELETE FROM journal_main WHERE journal_id='$id'");
	if($result2){
		...
		while($row1=mysqli_fetch_array($result1)){
			...
			while($row2=mysqli_fetch_array($result2)){
				...
				if($category_l2_tmp!=''){
					...
					$query3="SELECT category_level3 FROM account_category WHERE category_level1='$category_l1_tmp' AND category_level2='$category_l2_tmp' ORDER BY category_level3";
...
function getCahrtOfAccounts(){
	global $ac_id,$ac_name,$ac_system,$ac_status,$category_L1,$category_L2,$category_L3;
	...
	include('config.php');
	$query="SELECT ac.id,ac.name,ac.system_ac,ac.`status`,acat.category_level1,acat.category_level2,acat.category_level3 FROM accounts ac, account_category acat $table WHERE ac.category=acat.id $filter_accounts ORDER BY acat.category_level1,ac.name";
	$result=mysqli_query($conn,$query);
...
function addCahrtOfAccounts(){
	...
	include('config.php');
	if(isset($_POST['payment_ac'])) $payment_ac=1; else $payment_ac=0;
	if(isset($_POST['bank_ac'])) $bank_ac=1; else $bank_ac=0;
	if(isset($_POST['category_l2'])) $category_l2=$_POST['category_l2']; else $category_l2='';
	if(isset($_POST['category_l3'])) $category_l3=$_POST['category_l3']; else $category_l3='';
	if($_POST['bank_fee'] == '') $bank_fee=0; else $bank_fee=$_POST['bank_fee']; 
	...
	if($out){
		$query="INSERT INTO `accounts` (`name`,`category`,`bank_ac`,`payment_ac`,`system_ac`,`processing_fee`,`status`) VALUES ('$ac_name','$account_category_id',' $bank_ac','$payment_ac','0','$bank_fee','1')";
		$result=mysqli_query($conn,$query);

		if(!$result){
			$out=false;
			$message='Acount could not be Created';
		}
	}
	return $out;
}
...
function editCahrtOfAccounts(){
	...
	if($out){
		if($bank_ac==0) $bank_fee = 0;
		$query="UPDATE `accounts` SET `name`='$ac_name',`category`='$account_category_id',`payment_ac`='$payment_ac',`bank_ac`='$bank_ac',`processing_fee`='$bank_fee' WHERE id='$id'";
		$result=mysqli_query($conn,$query);

		if(!$result){
			$message='Acount could not be Updated';
			$out = false;
		}
	}
...
function setStCahrtOfAccounts(){
	...
	$id=$_GET['id'];
	$status=$_GET['status'];
	if($status==0) $msg='Disabled'; else $msg='Enabled';
	include('config.php');
	...
	if($result){
		...
		$message="Acount could not be $msg !";
		return false;
	}
}
...
function getAccountHistory(){
	global $account_name,$date,$id_list,$type,$payee,$dr,$cr,$from_date,$to_date,$statting_balance;
	$id=$_GET['id'];
	if(isset($_GET['from_date'])) $from_date=$_GET['from_date']; else $from_date=date("Y-m-d",time()-(60*60*20*30));
	...
	while($row=mysqli_fetch_array($result)){
		...
		if($row[2]=='dr'){
			...
			if($row1[0]!='') $oo_payee[]=$row1[0]; 	else $oo_payee[]='';		
		}
		if($row[2]=='cr'){
			$oo_dr[]=null;
			$oo_cr[]=-$row[3];
	...
	if($account_bank_ac==1){
		$query="SELECT SUM(py.amount) FROM payment py, cust cu WHERE py.`cust`=cu.id AND py.`status`=0 AND py.payment_type=3 AND py.bank_trans='$ac_id_tmp' AND date(py.payment_date) < '$from_date'";
		$row=mysqli_fetch_row(mysqli_query($conn,$query));
		$statting_balance+=$row[0];

		$query="SELECT date(py.payment_date),py.id,cu.name,py.amount FROM payment py, cust cu WHERE py.`cust`=cu.id AND py.`status`=0 AND py.payment_type=3 AND py.bank_trans='$ac_id_tmp' AND date(py.payment_date) BETWEEN '$from_date' AND '$to_date'";
		$result=mysqli_query($conn,$query);
		while($row=mysqli_fetch_array($result)){
			$oo_date[]=$row[0];
			$oo_id[]=$row[1];
	...
	if($account_name=='Cash in Hand'){
		$query="SELECT SUM(py.amount) FROM payment py, cust cu WHERE py.`cust`=cu.id AND py.`status`=0 AND py.payment_type=1 AND date(py.payment_date) < '$from_date'";
		$row=mysqli_fetch_row(mysqli_query($conn,$query));
		$statting_balance+=$row[0];
		...
		$query="SELECT date(py.payment_date),py.id,cu.name,py.amount FROM payment py, cust cu WHERE py.`cust`=cu.id AND py.`status`=0 AND py.payment_type=1 AND date(py.payment_date) BETWEEN '$from_date' AND '$to_date'";
		$result=mysqli_query($conn,$query);
		while($row=mysqli_fetch_array($result)){
			$oo_date[]=$row[0];
			$oo_id[]=$row[1];
	...
	if($account_name=='Undeposited Cheques'){
		$query="SELECT SUM(py.amount) FROM payment py, cust cu WHERE py.`cust`=cu.id AND py.`status`=0 AND py.payment_type=2 AND py.chque_clear=0 AND date(py.payment_date) < '$from_date'";
		$row=mysqli_fetch_row(mysqli_query($conn,$query));
		$statting_balance+=$row[0];

		$query="SELECT date(py.payment_date),py.id,cu.name,py.amount FROM payment py, cust cu WHERE py.`cust`=cu.id AND py.`status`=0 AND py.payment_type=2 AND py.chque_clear=0 AND date(py.payment_date) BETWEEN '$from_date' AND '$to_date'";
		$result=mysqli_query($conn,$query);
		while($row=mysqli_fetch_array($result)){
			$oo_date[]=$row[0];
			$oo_id[]=$row[1];
	...
	if($account_name=='Account Receivable'){
		...
		$query="SELECT bm.`cust`,SUM(bm.`invoice_+total` + bm.`invoice_-total`) FROM bill_main bm WHERE bm.exclude=0 AND bm.`lock`=1 AND bm.`status` NOT IN (0,7) GROUP BY bm.`cust`";
		$result=mysqli_query($conn,$query);
		while($row=mysqli_fetch_array($result)){
			$key=array_search($row[0],$pay_cust1);
			if($key>-1) $balance=$row[1]-$pay_amount1[$key]; else $balance=$row[1];
			if($balance>0){
				$tttt+=$balance;
				...
				$oo_payee[]=$row1[1];
				if($balance2>0){	 
					$oo_dr[]=$balance2; $oo_cr[]=0;
				}else{ 
					$oo_cr[]=-$balance2; $oo_dr[]=0; 
				}
			}
		}
	}
	...
	if($account_name=='Sales'){
		$query="SELECT SUM(bi.qty*bi.unit_price) FROM bill_main bm, bill bi, cust cu WHERE bm.invoice_no=bi.invoice_no AND bm.`cust`=cu.id AND bm.exclude=0 AND bm.`status` NOT IN (0,7) AND bm.`lock`=1 AND date(bm.billed_timestamp) < '$from_date'";
		$row=mysqli_fetch_row(mysqli_query($conn,$query));
		$statting_balance-=$row[0];

		$query="SELECT date(bm.billed_timestamp),bm.invoice_no,cu.name,SUM(bi.qty*bi.unit_price) FROM bill_main bm, bill bi, cust cu WHERE bm.invoice_no=bi.invoice_no AND bm.`cust`=cu.id AND bm.exclude=0 AND bm.`status` NOT IN (0,7) AND bm.`lock`=1 AND date(bm.billed_timestamp) BETWEEN '$from_date' AND '$to_date' GROUP BY bm.invoice_no";
		$result=mysqli_query($conn,$query);
		while($row=mysqli_fetch_array($result)){
			$oo_date[]=$row[0];
			$oo_id[]=$row[1];
			$oo_type[]='Bill';
			$oo_payee[]=$row[2];
			if($row[3]>0){	$oo_cr[]=$row[3];	$oo_dr[]=0; }else{
				$oo_cr[]=0;	$oo_dr[]=-$row[3];
			}
		}
	}
	...
	$query="SELECT date(py.chque_deposit_date),py.id,cu.name,py.amount FROM payment py, accounts ac, cust cu WHERE py.chque_deposit_bank=ac.id AND py.`cust`=cu.id AND py.`status`=0 AND py.payment_type=2 AND py.chque_clear=1 AND ac.id='$id' AND date(py.chque_deposit_date) BETWEEN '$from_date' AND '$to_date'";
	$result=mysqli_query($conn,$query);
	while($row=mysqli_fetch_array($result)){
	if($row[0]!=NULL){
		$oo_date[]=$row[0];
	...
	$query="SELECT date(py.payment_date),py.id,cu.name,py.amount FROM payment py, accounts ac, cust cu WHERE py.bank_trans=ac.id AND py.`cust`=cu.id AND py.`status`='0' AND py.payment_type='4' AND ac.id='$id' AND date(py.payment_date) BETWEEN '$from_date' AND '$to_date'";
	$result=mysqli_query($conn,$query);
	while($row=mysqli_fetch_array($result)){
	if($row[0]!=NULL){
		$oo_date[]=$row[0];
		...
		while($row=mysqli_fetch_array($result)){
			...
			if($ac_name_tmp=='Account Receivable'){
				...
				while($row1=mysqli_fetch_array($result1)){
					...
					if($key>-1) $balance=$row1[1]-$pay_amount1[$key]; else $balance=$row1[1];
...
function getPayrollForm(){
	...
	while($row=mysqli_fetch_array($result)){
		...
		if($remaining<$loan_installment){
			...
			if($remaining<($loan_installment+10))	$payroll_installment=$loan_installment+$remaining; else $payroll_installment=$loan_installment;
		}
	...
	while($row=mysqli_fetch_array($result)){
		...
		$sa_type[]=$row[1];
		$sa_amount[]=$row[2];
		if($row[1]==1)	$sa_epf_employee[$row[0]]=$row[2]*$epf_employee_rate;
		if($row[1]==1)	$sa_tax_employee[$row[0]]=$row[2]*$sa_tax_rate;
	}
...
function getPayrollUserView(){
	global $etf_rate,$epf_employer_rate,$emp_id,$emp_name,$type_id,$type_name,$payroll_month,$payroll_arr,$generated_date,$etf_employer_payble,$epf_employer_payble,$emp_fullname,$emp_nic,$emp_bank,$emp_bankbranch,$emp_bankac,$emp_store;
	...
	$query="SELECT year(pm.`month`),monthname(pm.`month`),up.id,up.username,pd.`type`,pd.amount,date(pm.generated_date),pm.generated_by,pm.employer_etf,pm.employer_epf,up.emp_name,up.nic,bk.name,up.bank_branch,up.bank_ac,up.store FROM payroll_main pm, payroll_data pd, userprofile up, bank bk WHERE pm.payroll_no=pd.payroll_no AND pd.emp_id=up.id AND up.bank_id=bk.id AND pm.payroll_no='$payroll_no' AND pd.emp_id='$emp'";
	$result=mysqli_query($conn,$query);
	while($row=mysqli_fetch_array($result)){
		$payroll_month=$row[0].' - '.$row[1];
		$generated_date=$row[6];
	...
	$query="SELECT name FROM stores WHERE id='$emp_st'";
	$row=mysqli_fetch_row(mysqli_query($conn,$query));
	$emp_store=$row[0]; 			
}
...
$status_name='';
	switch ($status_id){
		case "0" : $status_name='Inactive';  break;
		case "1" : $status_name='Pending';  break;
		case "2" : $status_name='Approved';  break;
		case "3" : $status_name='Rejected';  break;
		case "4" : $status_name='Granted';  break;
	}
	return $status_name;
}
...
function getLoanOne(){
	...
	if($one_lastpaydate!='') $date1=$one_lastpaydate; else $date1=$one_start;
	...
	if($one_duration<1) $one_installment=0;
...
function payLoan(){
	...
	if($loan_lastpaydate!='') $date1=$loan_lastpaydate; else $date1=$loan_start;
	$payoff_duration=dateDiff($date1, $today)/30;
	$payoff_duration=ceil($payoff_duration);
	...
	*/
	...
	if($loan_status==0){ $out=false; $message='Error: This Loan is a inactive Loan'; }
	if($amount!=$loan_balance){ $out=false; $message='Error: Invalid Amount'; }
	if($amount==$loan_balance){ $message='The Loan is Fully Settled'; }
	
	if($out){
		$query="INSERT INTO `loan_pay` (`loan_id`,`date`,`capital_pay`,`interest_pay`) VALUES ('$id','$today','$remaining_capital','$interest_pay')";
		print $query;
		$result=mysqli_query($conn,$query);
		if(!$result){ $out=false; $message='Error: Amount could not be Added!'; }
	}
	if($out){
		$query="UPDATE `loan_main` SET `status`='0' WHERE id='$id'";
		$result=mysqli_query($conn,$query);
		if(!$result){ $out=false; $message='Error: Loan status could not be updated!'; }
	}
