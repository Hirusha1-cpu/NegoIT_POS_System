	...
	if($out){
		if((isQuickBooksActive(1))){
			...
			if(!$result2){
				...
				$account_array = array('account_name'=>$ac_name, 'account_type'=>$account_type, 'account_sub_type'=> $account_sub_type, 'account_classification' => $account_classification);
...
function setStCahrtOfAccounts(){
	...
	if($result){
		...
		$message="Account could not be $msg!";
		$out = false;
	}
	if($out){
		if((isQuickBooksActive(1))){
...
function getAccountHistory(){
	...
	if($account_bank_ac==1){
		$query="SELECT SUM(py.amount) FROM payment py, cust cu WHERE py.`cust`=cu.id AND py.`status`=0 AND py.payment_type=3 AND py.bank_trans='$ac_id_tmp' AND date(py.payment_date) < '$from_date' $groupsearch";
		$row=mysqli_fetch_row(mysqli_query($conn,$query));
		$statting_balance+=$row[0];

		$query="SELECT date(py.payment_date),py.id,cu.name,py.amount FROM payment py, cust cu WHERE py.`cust`=cu.id AND py.`status`=0 AND py.payment_type=3 AND py.bank_trans='$ac_id_tmp' AND date(py.payment_date) BETWEEN '$from_date' AND '$to_date' $groupsearch";
		$result=mysqli_query($conn,$query);
		while($row=mysqli_fetch_array($result)){
			$oo_date[]=$row[0];
			$oo_id[]=$row[1];
	...
	if($account_name=='Cash in Hand'){
		$query="SELECT SUM(py.amount) FROM payment py, cust cu WHERE py.`cust`=cu.id AND py.`status`=0 AND py.payment_type=1 AND date(py.payment_date) < '$from_date' $groupsearch";
		$row=mysqli_fetch_row(mysqli_query($conn,$query));
		$statting_balance+=$row[0];
		...
		$query="SELECT date(py.payment_date),py.id,cu.name,py.amount FROM payment py, cust cu WHERE py.`cust`=cu.id AND py.`status`=0 AND py.payment_type=1 AND date(py.payment_date) BETWEEN '$from_date' AND '$to_date' $groupsearch";
		$result=mysqli_query($conn,$query);
		while($row=mysqli_fetch_array($result)){
			$oo_date[]=$row[0];
			$oo_id[]=$row[1];
	...
	if($account_name=='Undeposited Cheques'){
		$query="SELECT SUM(py.amount) FROM payment py, cust cu WHERE py.`cust`=cu.id AND py.`status`=0 AND py.payment_type=2 AND py.chque_clear=0 AND date(py.payment_date) < '$from_date' $groupsearch";
		$row=mysqli_fetch_row(mysqli_query($conn,$query));
		$statting_balance+=$row[0];

		$query="SELECT date(py.payment_date),py.id,cu.name,py.amount FROM payment py, cust cu WHERE py.`cust`=cu.id AND py.`status`=0 AND py.payment_type=2 AND py.chque_clear=0 AND date(py.payment_date) BETWEEN '$from_date' AND '$to_date' $groupsearch";
		$result=mysqli_query($conn,$query);
		while($row=mysqli_fetch_array($result)){
			$oo_date[]=$row[0];
			$oo_id[]=$row[1];
	...
	if($account_name=='Account Receivable'){
		...
		$query="SELECT bm.`cust`, SUM(bm.`invoice_+total` + bm.`invoice_-total`) FROM bill_main bm, cust cu WHERE bm.exclude=0 AND bm.`lock`=1 AND bm.`status` NOT IN (0,7) AND bm.`cust`=cu.`id` $groupsearch GROUP BY bm.`cust`";
		$result=mysqli_query($conn,$query);
		while($row=mysqli_fetch_array($result)){
	...
	if($account_name=='Sales'){
		$query="SELECT SUM(bi.qty*bi.unit_price) FROM bill_main bm, bill bi, cust cu WHERE bm.invoice_no=bi.invoice_no AND bm.`cust`=cu.id AND bm.exclude=0 AND bm.`status` NOT IN (0,7) AND bm.`lock`=1 AND date(bm.billed_timestamp) < '$from_date' $groupsearch";
		$row=mysqli_fetch_row(mysqli_query($conn,$query));
		$statting_balance-=$row[0];

		$query="SELECT date(bm.billed_timestamp),bm.invoice_no,cu.name,SUM(bi.qty*bi.unit_price) FROM bill_main bm, bill bi, cust cu WHERE bm.invoice_no=bi.invoice_no AND bm.`cust`=cu.id AND bm.exclude=0 AND bm.`status` NOT IN (0,7) AND bm.`lock`=1 AND date(bm.billed_timestamp) BETWEEN '$from_date' AND '$to_date' $groupsearch GROUP BY bm.invoice_no";
		$result=mysqli_query($conn,$query);
		while($row=mysqli_fetch_array($result)){
			$oo_date[]=$row[0];
			$oo_id[]=$row[1];
	...
	$query="SELECT date(py.chque_deposit_date),py.id,cu.name,py.amount FROM payment py, accounts ac, cust cu WHERE py.chque_deposit_bank=ac.id AND py.`cust`=cu.id AND py.`status`=0 AND py.payment_type=2 AND py.chque_clear=1 AND ac.id='$id' AND date(py.chque_deposit_date) BETWEEN '$from_date' AND '$to_date' $groupsearch";
	$result=mysqli_query($conn,$query);
	while($row=mysqli_fetch_array($result)){
	if($row[0]!=NULL){
		$oo_date[]=$row[0];
	...
	$query="SELECT date(py.payment_date),py.id,cu.name,py.amount FROM payment py, accounts ac, cust cu WHERE py.bank_trans=ac.id AND py.`cust`=cu.id AND py.`status`='0' AND py.payment_type='4' AND ac.id='$id' AND date(py.payment_date) BETWEEN '$from_date' AND '$to_date' $groupsearch";
	$result=mysqli_query($conn,$query);
	while($row=mysqli_fetch_array($result)){
	if($row[0]!=NULL){
		$oo_date[]=$row[0];
