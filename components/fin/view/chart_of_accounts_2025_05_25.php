...
<script type="text/javascript">
	...
	<?php for($i=0;$i<sizeof($category_l1);$i++){ ?>
		var <?php print $category_l1[$i]; ?> = [ <?php for ($x=0;$x<sizeof($category_l2[$category_l1[$i]]);$x++){ print '"'.$category_l2[$category_l1[$i]][$x].'",'; } ?>	];
	...
	<?php for($i=0;$i<sizeof($category_l2_list);$i++){ ?>
		var <?php print $category_l2_list[$i]; ?> = [ <?php for ($x=0;$x<sizeof($category_l3[$category_l2_list[$i]]);$x++){ print '"'.$category_l3[$category_l2_list[$i]][$x].'",'; } ?>	];
		...
		<td valign="top">
			<table align="center" border="0" width="450px" style="font-family:Calibri">
				...
				for($i=0;$i<sizeof($ac_id);$i++){
					...
					else $delete_ac='<span style="font-style:italic; color:gray">system account</span>';
					
					if(($ac_status[$i]==0) && ($components == 'fin')){ $color='silver'; $delete_ac=''; }else $color='black';
