...
function getItems($sub_system)
{
	...
	if (isset($_GET['category'])) {
		...
		if ($category_req == 'all')
			...
			$category_qry = 'ca.id=' . $category_req . ' AND';
	}
	if (isset($_GET['store'])) {
	...
	if (($type == 1) || ($type == 5)) {
		...
		if ($systemid != 4) {
			...
			$query1 = "SELECT inv.code, inv.description, iqt.drawer_no, ca.id, ca.name, st.name, st.id, iui.w_price, iui.r_price, iui.c_price, COUNT(iui.id) AS unique_item_count, inv.unic, inv.id AS inventory_item_id, iqt.id AS inventory_qty_id, iqt.qty
						...
						INNER JOIN stores st ON iqt.location = st.id
						LEFT JOIN inventory_unic_item iui ON iqt.id = iui.itq_id AND iui.status = '0'
						WHERE $category_qry $store_qry $tags_qry st.`status` != 0 AND inv.pr_sr = '1' AND inv.status = '1' AND inv.unic = '1'
		...
		// build filtered inventory_qty query with safe WHERE clause
		$conds = [];
		if ($tmp = $clean($category_qry)) $conds[] = $tmp;
		if ($tmp = $clean($store_qry)) $conds[] = $tmp;
		if ($tmp = $clean($tags_qry)) $conds[] = $tmp;
		$conds[] = "st.id=iqt.location";
		$where = '';
		if (!empty($conds)) {
			$where = 'WHERE ' . implode(' AND ', $conds) . ' AND ';
		}
		$query = "SELECT inv.id,inv.code,inv.description,iqt.w_price,iqt.r_price,iqt.c_price,iqt.drawer_no,iqt.qty,ca.id,ca.name,st.name,st.id,iqt.id,inv.unic " .
			"FROM inventory_items inv, inventory_qty iqt, item_category ca, stores st $where" .
		...
		// build inventory_new query with safe WHERE clause
		$conds = [];
		if ($tmp = $clean($category_qry)) $conds[] = $tmp;
		if ($tmp = $clean($store_qry)) $conds[] = $tmp;
		if ($tmp = $clean($tags_qry)) $conds[] = $tmp;
		$conds[] = "st.id = itn.store";
		$where = '';
		if (!empty($conds)) {
			$where = 'WHERE ' . implode(' AND ', $conds) . ' AND ';
		}
		$query = "SELECT inv.id,inv.code,inv.description,itn.w_price,itn.r_price,itn.c_price,itn.qty,ca.id,ca.name,st.name " .
			"FROM inventory_items inv, inventory_new itn, item_category ca, stores st $where" .
			"inv.id=itn.item AND inv.`category`=ca.id AND inv.pr_sr='1' AND inv.`status`='1' $unic_qry";
		$result = mysqli_query($conn2, $query);
		while ($row = mysqli_fetch_array($result)) {
			$id[] = $row[0];
			$code[] = $row[1] . ' (New)';
...
function getShipmentList($systemid, $sub_system)
{
	...
	$ship_invoice_no, $total, $status, $invoice_date, $totalOfDeletedShipments, $unicItemDeletedCountInShipment, $invoice_date;
	$user = $_COOKIE['user_id'];
	$month = $_GET['month'];
	$date_type = '';
