		...
		if ($out) {
			if (isQuickBooksActive(1)) {
				...
				if ($inventory_cost_for_item_before > 0) {
					...
					$journalEntryForEditItemQty = buildJournalEntry($conn, $inventory_cost_for_item_before, $debitAccountName, $creditAccountName, $description, $debitEntityType, $debitEntityID, $creditEntityType, $creditEntityID);
				...
				if ($inventory_cost_for_item_after > 0) {
					...
					$journalEntryForEditItemQty = buildJournalEntry($conn, $inventory_cost_for_item_after, $debitAccountName, $creditAccountName, $description, $debitEntityType, $debitEntityID, $creditEntityType, $creditEntityID);
					if (isset($journalEntryForEditItemQty['error'])) {
						$qb_msg = $journalEntryForEditItemQty['error'];
						throw new Exception("QuickBooks error: " . $qb_msg);
					} else {
...
function getShipmentList($systemid, $sub_system)
{
	...
	$ship_invoice_no, $total, $status,
	$totalOfDeletedShipments, $unicItemDeletedCountInShipment;
	$user = $_COOKIE['user_id'];
	$month = $_GET['month'];
	$store0 = 'all';
	$qry_st = "";
	$total = 0;
	$time_now = timeNow();
	$shipment_no = $store_name = $ship_invoice_no = $ship_total_cost = $status = $unicItemDeletedCountInShipment = array();
	...
	if ($month != '') {
		...
		$query = "SELECT ins.`shipment_no`, ins.`added_by`, DATE(ins.`time`), TIME(ins.`time`), ins.`time`, sm.`unic`, ins.`location`, sm.`invoice_no`, SUM(ins.`cost` * ins.`added_qty`) AS total_cost, sm.`status`
		FROM inventory_shipment ins LEFT JOIN shipment_main sm ON ins.`shipment_no`=sm.`id`
		WHERE $sub_sys_qry DATE(ins.`time`) LIKE '$month%' $qry_st GROUP BY ins.`shipment_no` ORDER BY ins.`shipment_no` DESC";
