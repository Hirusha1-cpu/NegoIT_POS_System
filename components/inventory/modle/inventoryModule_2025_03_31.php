...
function addQtyFinal($shipment_num,$unic,$code,$qty,$c_price,$w_price,$r_price,$c_price2,$w_price2,$r_price2,$sn){
	...
	if($out){
		...
		if($out){
			...
			if($duplicate>0) $message='Alert: Duplicated Unic ID Detected'; else $message='Inventory updated successfully';
		}else{
			mysqli_query($conn,"DELETE FROM `inventory_shipment` WHERE `id`='$lastid'");
			...
			while($row=mysqli_fetch_array($result)){
				...
				if($out){
					if(addQtyFinal($main_shipment_no,$unic,$inv_code,0,$c_price,$w_price,$r_price,$c_price2,$w_price2,$r_price2,$sn_arr)){
						...
						if($out){
							...
							if($count == 0) {
								...
								if(isQuickBooksActive(1)){
									// if shipment main dose'nt have any journal entry id
									...
									if($qbID == '' && $vendorID != ''){
										...
										if($journal_entry_result['status'] == 'success'){
											...
											$query = "UPDATE `shipment_main` SET `qb_id`='$qb_journal_entry_id' WHERE `id`='$main_shipment_no'";
										...
										$qb_msg = "Quickbooks error: supplier is not registered in QuickBooks through the system or this shipment already recoded in Quickbooks";
									}
						...
						$result2 = mysqli_query($conn,$query2);
						if(!$result2){ $message='Error: The Unic Item Status could not be Updated'; }
					}
				}
			}
		}else{
...
function addQty($shipment_num,$unic,$code,$qty,$c_price,$w_price,$r_price,$c_price2,$w_price2,$r_price2){
	...
	if(((($unic0==0)||($unic=='yes'))) &&($proceed)){
		...
		if($result2){
			...
			$result2=mysqli_query($conn,$query2);
			debugEnd($debug_id,'success');
			if($duplicate>0)$message='Alert: Duplicated edit Item1 Unic ID Detected'; else $message='Inventory updated successfully';
			return true;
		}else{
			mysqli_query($conn,"DELETE FROM `inventory_shipment` WHERE `id`='$lastid'");
			debugEnd($debug_id,'fail');
...
function getShipmentList($systemid,$sub_system){
	global $shipment_no,$salesman,$current_user,$date,$time,$active,$unic,$ship_store,$month,$store0,$shipment_edit_time,$ship_total_cost,$ship_invoice_no,$total,$status,
