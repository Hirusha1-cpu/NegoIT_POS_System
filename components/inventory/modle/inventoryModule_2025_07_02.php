									...
									if ($vendorID != '') { // if shipment main doesn't have any journal entry id
										if ($qbID == '') {
											...
											foreach ($journal_entries as $entry) {
												...
												$query = "INSERT INTO qb_queue (`batch_id`, `action`, `shipment_no`, `posting_type`, `account_id`, `account_name`, `amount`, `description`, `created_at`, `entity_type`, `entity_id`, `entity_name`)
							...
							if (isQuickBooksActive(1)) {
								...
								if ($vendorID != '') {
									if ($qbID == '') {
										...
										foreach ($journal_entries as $entry) {
											$shipment_no = $main_shipment_no;
											$posting_type = $entry['posting_type'];
											$account_id = $entry['account_id'];
											$account_name = $entry['account_name'];
											$amount = $entry['amount'];
											$description = $entry['description'];
											$entity_type = isset($entry['entity_type']) ? $entry['entity_type'] : null;
											$entity_id = isset($entry['entity_id']) ? $entry['entity_id'] : null;
											$entity_name = isset($entry['entity_name']) ? $entry['entity_name'] : null;
		...
		if ($out) {
			if (isQuickBooksActive(1)) {
				...
				if ($inventory_cost_for_item_before > 0) {
					...
					$journalEntryForEditItemQty = buildJournalEntry($conn, abs($inventory_cost_for_item_before), $debitAccountName, $creditAccountName, $description, $debitEntityType, $debitEntityID, $creditEntityType, $creditEntityID);
				...
				if ($inventory_cost_for_item_after > 0) {
					...
					$journalEntryForEditItemQty = buildJournalEntry($conn, abs($inventory_cost_for_item_after), $debitAccountName, $creditAccountName, $description, $debitEntityType, $debitEntityID, $creditEntityType, $creditEntityID);
					if (isset($journalEntryForEditItemQty['error'])) {
						$qb_msg = $journalEntryForEditItemQty['error'];
						throw new Exception("QuickBooks error: " . $qb_msg);
					} else {
