...
function getItems($sub_system){
	...
	if(($type==1)||($type==5)){
		$query="SELECT inv.id,inv.code,inv.description,iqt.w_price,iqt.r_price,iqt.c_price,iqt.drawer_no,iqt.qty,ca.id,ca.name,st.name,st.id,iqt.id,inv.unic FROM inventory_items inv, inventory_qty iqt, item_category ca, stores st  WHERE $category_qry $store_qry $tags_qry st.id=iqt.location AND inv.id=iqt.item AND inv.`category`=ca.id AND inv.pr_sr='1' AND inv.`status`=1 ORDER BY $sort_by";
		$result=mysqli_query($conn2,$query);
		while($row=mysqli_fetch_array($result)){
			$id[]=$row[0];
			$code[]=$row[1];
