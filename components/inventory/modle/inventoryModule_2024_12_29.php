...
function getShipmentList($systemid,$sub_system){
	global $shipment_no,$salesman,$current_user,$date,$time,$active,$unic,$ship_store,$month,$store0,$shipment_edit_time,$ship_total_cost,$ship_invoice_no,$total;
	$user=$_COOKIE['user_id'];
	$month=$_GET['month'];
	$store0='all';
	$qry_st="";
	$total=0;
	$ship_total_cost = array();
	if($systemid==15){
	...
	if($month!=''){
		...
		$query="SELECT ins.shipment_no,ins.added_by,date(ins.time),time(ins.time),ins.time,sm.unic,ins.location,sm.`invoice_no`,SUM(ins.`cost` * ins.`added_qty`) as total_cost FROM inventory_shipment ins LEFT JOIN shipment_main sm ON ins.shipment_no=sm.id WHERE sm.`status`!='3' AND sub_system='$sub_system' AND sm.shipment_date LIKE '$month%' $qry_st GROUP BY ins.shipment_no ORDER BY ins.shipment_no DESC";
		$result=mysqli_query($conn2,$query);
		while($row=mysqli_fetch_array($result)){
			$shipment_no[]=$row[0];
			$salesman[]=$row[1];
