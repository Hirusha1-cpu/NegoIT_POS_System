...
function addQtyFinal($shipment_num,$unic,$code,$qty,$c_price,$w_price,$r_price,$c_price2,$w_price2,$r_price2,$sn){
	...
	if($out){
		...
		if(!$result){ $out=false; $message='Error: Items cannot be added to the Database!'; }
...
function getTmpShipmentPendingList($sub_system){
	global $shipment_no_tmp,$salesman_tmp,$date_tmp,$unic_tmp,$ship_store_tmp,$edit_ship_tmp,$month_tmp;
	$user_id=$_COOKIE['user_id'];
	...
	if($month_tmp!=''){
		...
		$query="SELECT smt.`id`,up.id,up.username,smt.shipment_date,smt.`unic`,st.`name` FROM shipment_main_tmp smt, userprofile up, stores st  WHERE smt.added_by=up.id AND smt.store=st.id AND smt.sub_system='$sub_system' AND smt.`shipment_date` LIKE '$month_tmp%' ORDER BY smt.`id` DESC";
		$result=mysqli_query($conn2,$query);
		while($row=mysqli_fetch_array($result)){
