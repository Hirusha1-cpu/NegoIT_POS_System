...
function addQtyFinal($shipment_num, $unic, $code, $qty, $c_price, $w_price, $r_price, $c_price2, $w_price2, $r_price2, $sn)
{
	...
	if ($out) {
				...
				if ($temptable1) {
					if ($temptable2) {
						if ($qty_old == 0) {
							...
							if (!$result) {
								$out = false;
								$message = 'Error: Failed to update the Inventory!';
							}
						} else {
							$newtmp_id = '';
							...
							if ($newtmp_id != '') {
								...
								if (!$result) {
									$out = false;
									$message = 'Error: Failed to Update the New Inventory!';
								}
								...
								if (!$result) {
									$out = false;
									$message = 'Error: Failed to Add the New Inventory!';
								}
					...
					if ($iqt_id == '') {
						...
						if (!$result) {
							$out = false;
							$message = 'Error: Failed to Add the Inventory!';
						}
		...
		if ($out) {
				...
				for ($i = 0; $i <= 9; $i++) {
					if ($sn[$i] != '') {
						...
						if ($unic_count == 0) {
							...
							if (!$result0) {
								$out = false;
								$message = 'Error: Failed to Add the Inventory Unique Database!';
							}
						}
					}
				}
			...
			if (!$result) {
				$out = false;
				$message = 'Error: Failed to update the Shipment Item!';
			}
...
function oneShipmentTmpFinalize($shipment_no, $sub_req, $sub_system)
{
	...
	$count = $row[0];
	if ($count > 0) {
		$message = "Please check your shipment Items before finalize the shipment";
		$out = false;
	}
	...
	if ($out) {
		...
		if ($one_item_saved) {
			...
			if (!$out) {
				$message = 'Error: The Shipment main could not be Inserted';
				$out = false;
			}
			...
			while ($row = mysqli_fetch_array($result)) {
				...
				if ($out) {
					if (addQtyFinal($main_shipment_no, $unic, $inv_code, 0, $c_price, $w_price, $r_price, $c_price2, $w_price2, $r_price2, $sn_arr)) {
						...
						$result2 = mysqli_query($conn, $query2);
						if (!$result2) {
							$message = 'Error: The SN temp Status could not be Updated';
							$out = false;
						}
						if ($out) {
						...
						$result2 = mysqli_query($conn, $query2);
						if (!$result2) {
							$message = 'Error: The shipment item status could not be updated';
						}

						$query2 = "UPDATE `shipment_unic_tmp` SET `status`= '0' WHERE  shipment_tmp_itm_id = '$shipment_tmp_item_id'";
						$result2 = mysqli_query($conn, $query2);
						if (!$result2) {
							$message = 'Error: The unic item status could not be updated';
						}
					}
				}
			}
