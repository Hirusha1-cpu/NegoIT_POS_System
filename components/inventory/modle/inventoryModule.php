...
// updated by nirmal 11_07_2024 (if request is inventory and user has top manager, get category all, without consider subsystem)
function getCategory55($sub_system)
{
	global $category_id, $category_name, $conn2;
	$category_id = $category_name = array();
	include('config.php');
	$sub_sys_qry = " WHERE `sub_system`='all' OR `sub_system`='$sub_system'";
	include('config.php');

	if (isset($_COOKIE['top_manager']) && isset($_REQUEST['components']) && $_REQUEST['components'] == 'inventory') {
		$sub_sys_qry = "";
	}
	$query = "SELECT id,name FROM item_category $sub_sys_qry ORDER BY name";
...
// update by nirmal 08_08_2024 (order stores by sub system & store id)
// updated by nirmal 27_11_2024 (if logged user top-mgr then show data for all sub-systems)
...
// 	if ($systemid == 13)
// 		$sub_system_qry = "AND sub_system='$sub_system'";
// 	// if top manager no subsystem filter
// 	if (isset($_COOKIE['top_manager'])) {
// 		$sub_system_qry = "";
// 	}
// 	$order_by = " ORDER BY `sub_system`, `id`";
// 	include('config.php');
// 	$query = "SELECT id,name FROM stores WHERE status='1' $sub_system_qry $order_by";
// 	$result = mysqli_query($conn2, $query);
// 	while ($row = mysqli_fetch_array($result)) {
// 		$stores_id[] = $row[0];
// 		$stores_name[] = $row[1];
...
function getStores($sub_system)
{
	global $stores_id, $stores_name, $conn2;

	$stores_id = $stores_name = array();
	$systemid = inf_systemid(1);
...
function getSupplier()
{
	global $su_id, $su_name, $su_email, $su_tel1, $su_tel2, $su_address, $su_country, $conn2;

	$su_id = $su_name = $su_email = $su_tel1 = $su_tel2 = $su_address = $su_country = array();
...
// update by nirmal 28_11_2023 (added empty function to check query is empty, case : some db doesn't have some settings rows)
...
function getDefaultPrices()
{
	global $conn2, $default_max_r_rate, $default_min_w_rate, $default_max_w_rate, $unic_no, $default_commision;
...
// 	if ($_REQUEST['sub'] == 'show_add_unic_tmp')
// 		$unic = 1;
// 	else
// 		$unic = 0;
// 	include('config.php');
...
function addShipmentTmp($sub_system)
{
	...
	if ($store === null || $store === '' || !is_numeric($store)) {
	...
	$query1 = "INSERT INTO `shipment_main_tmp`
	(`id`,`added_by`,`shipment_date`,`supplier`,`invoice_no`,`invoice_date`,`invoice_due`,`unic`,`store`,`sub_system`)
	VALUES
	('$shipment_no','$user','$ship_date','$suplier','$ship_inv_no','$ship_inv_date','$ship_inv_dudate','$unic','$store','$sub_system')";
...
// updated by nirmal 17_11_2025 (excluded current editing item from sql)
function findDuplicateItem($type, $data, $excludeId = null)
{
	global $conn2;
	include('config.php');
	...
	// FIX: Escape the raw data ONLY, before putting it in the query
	...
	switch ($type) {
		case 'code':
			...
			break;
		case 'description':
			$qry = "`description`='$data'";
			break;
		case 'po_description':
			$qry = "`po_description`='$data'";
			break;
		default:
			return true;
	}
	...
	if ($excludeId !== null) {
		$excludeId = mysqli_real_escape_string($conn2, $excludeId);
		// Append to the existing $qry string
		$qry .= " AND `id` != '$excludeId'";
	}

	$base_query = "SELECT COUNT(id) AS `count` FROM `inventory_items` WHERE $qry";

	$result = mysqli_query($conn2, $base_query);

	if (!$result) {
		// Optional: good for debugging if it fails again
		error_log("MySQL Error: " . mysqli_error($conn2));
		return true;
	}
...
// edit by nirmal 28_11_2023 (added capability to save item description po-description with => / and &)
function addItem($sub_system)
{
	global $message, $type, $conn;
	$code = trim(preg_replace('/[^A-Za-z0-9\-+.\ _]/', '', strtoupper($_REQUEST['code'])));
	$description = trim(preg_replace('/[^A-Za-z0-9,&()\"\':;\/\-\+. _]/', '', $_REQUEST['description']));
	$po_description = trim(preg_replace('/[^A-Za-z0-9,&()\"\':;\/\-\+. _]/', '', $_REQUEST['po_description']));
	if ($po_description == '') {
	...
	if (($code == '') || ($description == '')) {
		$out = false;
		$message = 'Description or Code cannot be Empty !';
	}
	if ((!findDuplicateItem('code', $code)) || (!findDuplicateItem('description', $description))) {
		$out = false;
		$message = 'Duplicate Item Description or Code detected. !';
	}
	...
	for ($i = 0; $i < sizeof($tags_arr); $i++) {
		...
		if ($result !== false) {
			...
			if ($row !== null) {
				...
				// You can either skip this iteration or throw an exception
				continue;
			}
		}
	}
	if ($count > 1) {
		$out = false;
		$message = 'Error: You can only add one price tag to an item.';
	}
		...
		if ($out) {
			if ($type == 1) {
				...
				while ($row = mysqli_fetch_array($result)) {
					...
					if (!$result2) {
						$out = false;
						$message = 'Quantity could not be Added!';
					}
				}
			}
		}
...
function getItems1($itmstatus)
{
	...
	$store = $_COOKIE['store'];
	if ($itmstatus == 'on') {
		$itemst_qry = 'WHERE itm.`status`=1';
	}
	if ($itmstatus == 'nounic') {
		$itemst_qry = 'WHERE itm.`unic`=0 AND itm.`status`=1';
	}
	if ($itmstatus == 'unic') {
		$itemst_qry = 'WHERE itm.`unic`=1 AND itm.`status`=1';
	}
	if ($itmstatus == 'off') {
		$itemst_qry = '';
	}
	include('config.php');
	$query = "SELECT itm.id,itm.code,itm.description FROM inventory_items itm
	LEFT JOIN (SELECT * FROM inventory_qty WHERE location='$store') as itq ON itm.id=itq.item $itemst_qry";
	$result = mysqli_query($conn, $query);
	while ($row = mysqli_fetch_array($result)) {
		$item_id = $row[0];
		$code[] = $row[1];
...
// 	if ($itmstatus == 'on')
// 		$itemst_qry = 'WHERE `status`=1';
// 	if ($itmstatus == 'nounic')
// 		$itemst_qry = 'WHERE `unic`=0 AND `status`=1';
// 	if ($itmstatus == 'unic')
// 		$itemst_qry = 'WHERE `unic`=1 AND `status`=1';
// 	if ($itmstatus == 'off') {
// 		$itemst_qry = '';
// 		$sub_system_qry = '';
// 	} else {
...
function getItems2($itmstatus)
{
	global $code, $description, $item_id, $conn2;

	$item_id = $code = $description = array();
	...
	if ($itmstatus == 'off') {
		...
		if ($sub_system !== '') {
			$sub_system_qry = "AND `sub_system`=" . (int)$sub_system;
		}
	}
...
// update by nirmal 03_11_2023
// update by nirmal 11_01_2024 (product item sort by item code)
// update by nirmal 25_07_2024 (show different view of data to unic items based on qty and prices of each store)
// update by nirmal 21_11_2024 (type error log bug fix)

function getItems($sub_system)
{
	global $id, $code, $description, $w_price, $r_price, $cost, $drawer, $qty, $reorder_level, $reorder_qty, $cat_id, $cat_name, $st_name, $color, $mismatch, $rowClass;
	$id = $itq_id = $code = $description = $w_price = $r_price = $cost = $drawer = $qty = $cat_id = $cat_name = $st_name = $color = $rowClass = array();
	$unic_cal = unicCal();
	...
	if ($category_req == 'all') {
		...
		$category_qry = 'ca.id=' . (int)$category_req . ' AND';
	}
} else {
	$category_qry = '';
}
	...
	if ($store_req == 'all') {
		$store_qry = '';
	} elseif ($store_req == 'all-sub') {
		if (isset($_COOKIE['top_manager'])) {
			$store_qry = 'st.`status`!= 0 AND';
		} else {
			$store_qry = 'st.`sub_system`=' . (int)$sub_system . ' AND st.`status`!= 0 AND';
		}
	} else {
		$store_qry = 'st.id=' . (int)$store_req . ' AND';
	}
} else {
	$store_qry = '';
}
	...
	if (isset($_GET['tags'])) {
		...
		if ($tags_req != '') {
			...
			if ($tag_selection == 1)
				$query = "SELECT ta.item FROM tag_name tn, tag_assignment ta WHERE tn.id=ta.tag AND $tags_list";
			if ($tag_selection == 2)
				$query = "SELECT ta.item,count(ta.tag) FROM tag_assignment ta, tag_name tn WHERE ta.tag=tn.id AND $tags_list GROUP BY ta.item HAVING count(ta.tag)>=$match_count";
			$result = mysqli_query($conn2, $query);
			while ($row = mysqli_fetch_array($result)) {
				$tags_list2 .= $row[0] . ',';
			}
	...
	if (($type == 1) || ($type == 5)) {
		// added by nirmal 25_07_2024
		if ($systemid != 4) {
			// $query1 = "SELECT inv.code, inv.description, iqt.drawer_no, ca.id, ca.name, st.name, st.id,
			// iui.w_price, iui.r_price, iui.c_price, COUNT(iui.id), inv.unic, inv.id, iqt.id, iqt.qty
			// FROM inventory_items inv
			// INNER JOIN inventory_qty iqt ON inv.id = iqt.item
			// INNER JOIN item_category ca ON inv.category = ca.id
			// INNER JOIN stores st ON iqt.location = st.id
			// LEFT JOIN inventory_unic_item iui ON iqt.id = iui.itq_id AND iui.status = '0'
			// WHERE $category_qry $store_qry $tags_qry inv.pr_sr = '1' AND inv.status = '1' AND inv.unic = '1'
			// GROUP BY iui.w_price, iui.r_price, iui.c_price, st.id
			// ORDER BY $sort_by, ca.id";
			// $conditions = array();
			// if ($category_qry) $conditions[] = rtrim($category_qry, ' AND');
			// if ($store_qry) $conditions[] = rtrim($store_qry, ' AND');
			// if ($tags_qry) $conditions[] = rtrim($tags_qry, ' AND');
			// $conditions[] = "st.`status` != 0";
			// $conditions[] = "inv.pr_sr = '1'";
			// $conditions[] = "inv.status = '1'";
			// $conditions[] = "inv.unic = '1'";
			// $where = 'WHERE ' . implode(' AND ', $conditions);
			$clean = function ($s) {
	$s = trim((string)$s);
	$s = preg_replace('/^\s*AND\s+/i', '', $s);   // remove leading AND
	$s = preg_replace('/\s+AND\s*$/i', '', $s);   // remove trailing AND
	...
	$tmp = $clean($category_qry);
	if ($tmp !== '') $conditions[] = $tmp;
	...
	$where = 'WHERE ' . implode(' AND ', $conditions);
			// $query1 = "SELECT inv.code, inv.description, iqt.drawer_no, ca.id, ca.name, st.name, st.id, iui.w_price, iui.r_price, iui.c_price, COUNT(iui.id) AS unique_item_count, inv.unic, inv.id AS inventory_item_id, iqt.id AS inventory_qty_id, iqt.qty
			// 			FROM inventory_items inv
			// 			INNER JOIN inventory_qty iqt ON inv.id = iqt.item
			// 			INNER JOIN item_category ca ON inv.category = ca.id
			// 			INNER JOIN stores st ON iqt.location = st.id
			// 			LEFT JOIN inventory_unic_item iui ON iqt.id = iui.itq_id AND iui.status = '0'
			// 			$where
			// 			GROUP BY inv.code, inv.description, iqt.drawer_no, ca.id, ca.name, st.name, st.id, iui.w_price, iui.r_price, iui.c_price, inv.unic, inv.id, iqt.id, iqt.qty
			// 			ORDER BY iqt.drawer_no, ca.id";
			$query1 = "SELECT inv.code, inv.description, iqt.drawer_no, ca.id, ca.name, st.name, st.id, iui.w_price, iui.r_price, iui.c_price, COUNT(iui.id) AS unique_item_count, inv.unic, inv.id AS inventory_item_id, iqt.id AS inventory_qty_id, iqt.qty
			FROM inventory_items inv
			INNER JOIN inventory_qty iqt ON inv.id = iqt.item
			INNER JOIN item_category ca ON inv.category = ca.id
			...
			$currentItem = null;
			$previousPrices = [];
			// die('<pre>' . htmlspecialchars($query1) . '</pre>');
			...
			while ($row1 = mysqli_fetch_array($result1)) {
				...
				$itemKey = $row1[0] . '-' . $row1[5]; // Combine code and store name for uniqueness
				// Check if it's a new store or item
				if ($currentStore !== $row1[6] || $currentItem !== $itemKey) {
					$currentStore = $row1[6];
					$currentItem = $itemKey;
					$previousPrices = []; // Reset previous prices for the new store/item combination
				}

				// Apply different styling for rows with the same item and store but different prices
				if (in_array($row1[7], $previousPrices) || in_array($row1[8], $previousPrices) || in_array($row1[9], $previousPrices)) {
					$rowClass[] = 'highlighted-row';
				} else {
					$rowClass[] = 'normal-row';
				}
				// Store the current prices to check against future rows
				$previousPrices[] = $row1[7];
				$previousPrices[] = $row1[8];
				$previousPrices[] = $row1[9];
			}
		}

		if ($systemid != 4) {
			$unic_qry = 'AND inv.unic != 1';
		}

		$query = "SELECT inv.id,inv.code,inv.description,iqt.w_price,iqt.r_price,iqt.c_price,iqt.drawer_no,iqt.qty,ca.id,ca.name,st.name,st.id,iqt.id,inv.unic
		FROM inventory_items inv, inventory_qty iqt, item_category ca, stores st  WHERE $category_qry $store_qry $tags_qry st.id=iqt.location AND
		inv.id=iqt.item AND inv.`category`=ca.id AND inv.pr_sr='1' AND inv.`status`=1 $unic_qry ORDER BY $sort_by";
		$result = mysqli_query($conn2, $query);
		while ($row = mysqli_fetch_array($result)) {
			$id[] = $row[0];
			$code[] = $row[1];
			...
			$rowClass[] = 'normal-row';
			if (!$unic_cal) {
				if (checkMismatch($row[13], $row[0], $row[11], $row[12], $row[7])) {
					$color[] = 'black';
				} else {
					$color[] = 'orange';
					$mismatch++;
		...
		$query = "SELECT inv.id,inv.code,inv.description,itn.w_price,itn.r_price,itn.c_price,itn.qty,ca.id,ca.name,st.name FROM
		inventory_items inv, inventory_new itn, item_category ca, stores st
		WHERE $category_qry $store_qry $tags_qry st.id=itn.store AND inv.id=itn.item AND inv.`category`=ca.id AND inv.pr_sr='1' AND inv.`status`='1' $unic_qry";
		$result = mysqli_query($conn2, $query);
		while ($row = mysqli_fetch_array($result)) {
			$id[] = $row[0];
			$code[] = $row[1] . ' (New)';
...
// updated by nirmal 09_01_2024 (added existing excel sheet to new page (Inventory Uniq Item Cost) items)
...
function getUnicItems($sub_system)
{
	...
	$systemid = inf_systemid(1);
	$today = dateNow();
	$category_qry = $store_qry = $tags_qry = $item_store = '';
	...
	if (isset($_GET['category'])) {
		...
		if ($category_req == 'all')
			...
			$category_qry = 'itc.id=' . $category_req . ' AND';
	}
	...
	if (isset($_GET['store'])) {
		...
		if ($store_req == 'all') {
			...
			if ($store_req == 'all-sub') {
				if ((isset($_COOKIE['top_manager']))) {
					$store_qry = 'st.`status` != 0 AND';
				} else {
					$store_qry = 'st.`sub_system` = ' . $sub_system . ' AND st.`status` != 0 AND';
				}
			} else {
				$store_qry = 'st.id = ' . $store_req . ' AND';
			}
		}
	}
	...
	// excel sheet next page (Inventory Uniq Item Cost) items. (category, item description, qty, cost, value (qty * cost)), and value total
	$query1 = "SELECT itc.name AS category_name, itm.description AS item_description, ROUND(AVG(itu.c_price),0) AS cost, COUNT(itu.sn) AS qty, AVG(itu.c_price) * COUNT(itu.sn) AS value FROM inventory_items itm JOIN inventory_qty itq ON itm.id = itq.item JOIN item_category itc ON itm.category = itc.id JOIN stores st ON itq.location = st.id JOIN inventory_unic_item itu ON itq.id = itu.itq_id WHERE $category_qry $store_qry $tags_qry itm.unic = 1 AND itu.`status` = 0 GROUP BY itc.name, itm.description ORDER BY itc.name, itm.description";
	$result1 = mysqli_query($conn2, $query1);
	while ($row1 = mysqli_fetch_array($result1)) {
		$item_category_1[] = $row1[0];
		$item_desc_1[] = $row1[1];
		$item_cost_1[] = $row1[2];
		$item_qty_1[] = $row1[3];
		$items_values_1[] = $row1[4];
	}
...
{
	global $ship_inv_id, $ship_itm_desc, $ship_item_qty, $ship_itm_st_name, $ship_itm_st_color, $editable, $conn2, $conn;
...
function updateOneUnicPrice()
{
	...
	if (($c_price != '') && ($w_price != '') && ($r_price != '')) {
		...
		if ($out) {
			if ($status == 0)
				$query = "UPDATE `inventory_unic_item` SET `w_price`='$w_price',`r_price`='$r_price',`c_price`='$c_price' WHERE `id`='$itu_id'";
			if ($status == 1)
				$query = "UPDATE `inventory_unic_item` SET `c_price`='$c_price' WHERE `id`='$itu_id'";
			if ($query != "") {
				$result = mysqli_query($conn, $query);
				if ($result) {
					$message = 'Done';
		...
		if ($out) {
			...
			if ($status == 1 && $count != 0) {
				...
				$result = mysqli_query($conn, $query);
				if (!$result) {
					$message = 'Error: Invoice update failed';
					$out = false;
				}
			}
		}
...
function updateBulkUnicPrice()
{
	...
	if (($c_price != '') || ($w_price != '') || ($r_price != '')) {
		...
		$qry0 = '';
		if ($w_price != '')
			$qry0 .= "`w_price`='$w_price',";
		if ($r_price != '')
			$qry0 .= "`r_price`='$r_price',";
		if ($c_price != '')
			$qry0 .= "`c_price`='$c_price'";
		...
		while ($row0 = mysqli_fetch_array($result0)) {
			...
			if ($out) {
				if ($status == 0)
					$query = "UPDATE `inventory_unic_item` SET $qry0 WHERE `id`='$itu_id'";
				if (($status == 1) && ($c_price > 0))
					$query = "UPDATE `inventory_unic_item` SET `c_price`='$c_price' WHERE `id`='$itu_id'";
				if ($query != "") {
					$result = mysqli_query($conn, $query);
...
function deleteUnic($ins_id0, $sn)
{
	...
	if ($proceed) {
		if ($itq_status == 0) {
			...
			$proceed = false;
			$message = 'Error: Item Status was Modified !';
		}
	}
	...
	if ($proceed) {
		...
		if (!$result) {
			$proceed = false;
			$message = 'Error: Unique Item could not be deleted.';
		}
	}
	...
	if ($proceed) {
		...
		if ($result2) {
			...
			$result = mysqli_query($conn, $query);
			if ($result) {
				$message = 'Unic ID was Deleted Successfully';
			} else {
				$proceed = false;
				$message = 'Error: Unic ID Could Not Be Deleted';
			}
		}
	}
	return $proceed;
...
function removeSnTmp($sn_id)
{
	...
	if ($out) {
		...
		if ($added_by != $user) {
			$message = "Sorry! You don't have no authority to perform this action!";
			$out = false;
		}
	}
	...
	if ($out) {
		...
		$result = mysqli_query($conn, $query);
		if (!$result) {
			$message = "Error: SN could not be Deleted";
			$out = false;
		}
	}
	...
	if ($out) {
		...
		if ($count1 == 0) {
			...
			$result = mysqli_query($conn, $query);
			if (!$result) {
				$message = "Error: Temp Item could not be Deleted";
				$out = false;
			}
		} else {
			...
			if ($count2 == 0) {
				...
				$result = mysqli_query($conn, $query);
				if (!$result) {
					$message = "Error: Temp Item Status could not be Updated";
					$out = false;
				}
			} else {
				$i = 0;
				...
				while ($row = mysqli_fetch_assoc($result)) {
					...
					if ($out) {
						...
						$result2 = mysqli_query($conn, $query2);
						if (!$result) {
							$message = "Error: SN Status could not be Updated";
							$out = false;
						}
						$st_arr[$i] = $status;
						$i += 1;
				...
				if ($out) {
					if (array_search(3, $st_arr) > -1)
						$checked_status3 = true;
					else
						$checked_status3 = false;
					if (array_search(4, $st_arr) > -1)
						$checked_status4 = true;
					else
						$checked_status4 = false;
					...
					$result = mysqli_query($conn, $query);
					if (!$result) {
						$message = 'Error: The Item Status could not be Updated';
						$out = false;
					}
				}
			}
...
{
	global $message, $action, $shipment_no, $conn;
	$shipment_no = $shipment;
	if ($unic == 'yes')
		$action = 'show_add_unic_tmp';
	else
		$action = 'show_add_qty_tmp';
	...
	if ((!isNumeric($c_price) || !isNumeric($w_price) || !isNumeric($r_price) || !isNumeric($c_price2) || !isNumeric($w_price2) || !isNumeric($r_price2))) {
		$message = "Sorry! Your input values must be integers. Please check your input numbers again";
		$out = false;
	}
	...
	$itm_code = $row['code'];
	$itm_unic = $row['unic'];
	if (($unic == 'yes') && ($itm_unic == 0)) {
		$message = 'You cannot add qty item from here!';
		$out = false;
	}
	if (($unic == 'no') && ($itm_unic == 1)) {
		$message = 'You cannot add unic item from here!';
		$out = false;
	}

	// check the owner of shipment to continue add more items to the shipment
	if ($out) {
		$query = "SELECT `added_by` FROM `shipment_main_tmp` WHERE `id`='$shipment_no'";
		$result = mysqli_query($conn, $query);
		$row = mysqli_fetch_array($result);
	...
	if ($out) {
		...
		if (!$result) {
			$message = 'Error: The Item could not be added to DB';
			$out = false;
		}
	}
	if ($itm_unic == 1) {
			...
			for ($i = 1; $i <= 10; $i++) {
				if ($_REQUEST['sn' . $i] != '') {
					if ($out) {
						...
						$result0 = mysqli_query($conn, $query0);
						if (!$result0) {
							$message = 'Error: The SN could not be added to the DB';
							$out = false;
						}
					}
				}
...
function addQtyFinal($shipment_num, $unic, $code, $qty, $c_price, $w_price, $r_price, $c_price2, $w_price2, $r_price2, $sn)
{
	...
	$out = true;
	if (($c_price1 == $c_price2) && ($w_price1 == $w_price2) && ($r_price1 == $r_price2))
		$temptable1 = false;
	else
		$temptable1 = true;
	if ((($w_price1 != 0) && ($w_price2 == 0)) || (($r_price1 != 0) && ($r_price2 == 0)))
		$temptable2 = false;
	else
		$temptable2 = true;
	include('config.php');
	...
	if ($unic == 'yes') {
		if (!validateDeleteShipment($shipment_no, 1)) {
			$out = false;
			$message = 'Error: The shipment is LOCKED. Cannot apend any items.';
		}
	}
	if ($out) {
		if (!(($unic0 == 0) || ($unic == 'yes'))) {
			$out = false;
			$message = 'Error: for this item, Qty cannot be added from here.';
		}
	}
	...
	if ($out) {
				...
				if ($temptable1) {
					if ($temptable2) {
						if ($qty_old == 0) {
							...
							if (!$result) {
								$out = false;
								$message = 'Error: Failed to update the inventory qty.';
							}
						} else {
							$newtmp_id = '';
							...
							if ($newtmp_id != '') {
								...
								if (!$result) {
									$out = false;
									$message = 'Error: Failed to update the inventory new.';
								}
								...
								if (!$result) {
									$out = false;
									$message = 'Error: Failed to add the inventory new.';
								}
					...
					if ($iqt_id == '') {
						...
						if (!$result) {
							$out = false;
							$message = 'Error: Failed to add the inventory qty.';
						}
		...
		if ($out) {
				...
				for ($i = 0; $i <= 9; $i++) {
					if ($sn[$i] != '') {
						...
						if ($unic_count == 0) {
							...
							if (!$result0) {
								$out = false;
								$message = 'Error: Failed to add the inventory unic item.';
							}
						}
					}
				}
			...
			if (!$result) {
				$out = false;
				$message = 'Error: Failed to update the inventory shipment item.';
			}
			debugEnd($debug_id, 'success');

			if ($duplicate > 0)
				$message = 'Alert: Duplicated unic ID detected.';
			else
...
// update by nirmal 27_08_2024
// updated by nirmal 19_06_2025 (qb features added)
...
function oneShipmentTmpFinalize($shipment_no, $sub_req, $sub_system)
{
	...
	$ship_tmp_item_id = $unic = $inv_code = $add_qty = $c_price = $w_price = $r_price = $c_price2 = $w_price2 = $r_price2 = $ship_date = $suplier =
		$ship_inv_no = $ship_inv_date = $ship_inv_dudate = "";
	$action = $sub_req;
	$out = true;
	$journal_entry_result = $qb_msg = '';
	$main_shipment_no = 0;
	...
	$count = $row[0];
	if ($count > 0) {
		$message = "Error: Please check your shipment Items before finalize the shipment.";
		$out = false;
	}
	...
	if ($out) {
		...
		if ($one_item_saved) {
			...
			if (!$out) {
				$message = 'Error: The shipment main could not be inserted. Details: ' . $message; // Capture potential error from addShipmentToMain
				$out = false;
			}
			...
			if ($out) {
				...
				$result = mysqli_query($conn, $query);
				if (!$result) {
					$message = 'Error: The Item main temp could not be updated.';
					$out = false;
				}
			}
		}
	...
	if ($out) {
		if ($action == "show_add_unic_tmp") {
			...
			$query = "SELECT `id`,inv_code,unic,added_qty,c_price,w_price,r_price,c_price2,w_price2,r_price2
			FROM `shipment_item_tmp` WHERE `status` != '2' AND shipment_tmp_no ='$shipment_no'";
			$result = mysqli_query($conn, $query);
			while ($row = mysqli_fetch_array($result)) {
				$i = $j = 0;
				$st_arr = array();
				...
				while ($row1 = mysqli_fetch_assoc($result1)) {
					...
					if ($status == 1) {
						...
						if ($result2) {
							$sn_duplicate++;
							$out = false;
							$message = "Error: Please check your SNs  before finalize the shipment.";
						} else {
							$out = false;
							$message = "Error: SN status could not be updated in temp table for SN ID {$unic_itm_id}.";
						}
					}
				}
				...
				if ($sn_duplicate > 0) {
					...
					$result1 = mysqli_query($conn, $query1);
					if (!$result1) {
						$message .= " Error: The item Status could not be updated for item tmp ID {$shipment_tmp_item_id}."; // Append to existing message
						$out = false;
					}
				}
				...
				if ($out) {
					$addQtyFinalSuccess = addQtyFinal($main_shipment_no, $unic, $inv_code, 0, $c_price, $w_price, $r_price, $c_price2, $w_price2, $r_price2, $sn_arr);
					if ($addQtyFinalSuccess) {
						$query2 = "UPDATE `shipment_unic_tmp` SET `status`= '2' WHERE shipment_tmp_itm_id = '$shipment_tmp_item_id'";
						$result2 = mysqli_query($conn, $query2);
						if (!$result2) {
							$message = "Error: The SN temp status could not be updated for item tmp ID {$shipment_tmp_item_id}."; // Be specific
							$out = false;
						}
						if ($out) {
							$query2 = "SELECT COUNT(sut.`id`) FROM shipment_item_tmp sit, shipment_unic_tmp sut
							WHERE sit.`id`=sut.`shipment_tmp_itm_id` AND sit.`shipment_tmp_no`='$shipment_no' AND sut.`status` != '2'";
							...
							if ($count == 0) {
								...
								if (isQuickBooksActive(1)) {
									$shipmentQuery = "SELECT sm.`qb_id`, a.`qb_account_id`, s.`name` FROM `shipment_main` sm
														...
														WHERE sm.`id` = '$main_shipment_no' AND sm.`status` = 0";
									$shipmentRow = mysqli_fetch_row(mysqli_query($conn2, $shipmentQuery));
									$qbID = $shipmentRow[0];
									$vendorID = $shipmentRow[1];
									$vendorName = $shipmentRow[2];
									$amount = 0;
									$action_name = "shipment_insert";
									$batch_id = generateBatchID();
									if ($vendorID != '') { // if shipment main doesn't have any journal entry id
										if ($qbID == '') {
											$accountQuery = "SELECT `qb_account_id` FROM `accounts` WHERE `name` = 'Inventory Asset' AND `status` = 1";
											$accountResult = mysqli_fetch_row(mysqli_query($conn2, $accountQuery));
											$inventoryAssetAccountID = $accountResult[0];
											...
											$accountQuery3 = "SELECT SUM(`cost` * `added_qty`) FROM `inventory_shipment` WHERE `shipment_no` = '$main_shipment_no'";
											$accountResult3 = mysqli_fetch_row(mysqli_query($conn2, $accountQuery3));
											$amount = $accountResult3[0];

											$accountQuery4 = "SELECT `qb_id` FROM `stores` WHERE `id` = '$user_store'";
											$accountResult4 = mysqli_fetch_row(mysqli_query($conn2, $accountQuery4));
											$storeQBID = $accountResult4[0];
											...
											$journal_entries = [
												[
													...
													"amount" => abs($amount),
													"description" => "[SHIPMENT] - Shipment No: $main_shipment_no",
													"department_id" => $debitDepartmentID,
													"class_id" => $debitClassID
												],
												[
													"posting_type" => "Credit",
													...
													"amount" => abs($amount),
													"description" => "Supplier : $vendorName, Shipment No: $main_shipment_no",
													"entity_type" => "Vendor",
													"entity_id" => $vendorID,
													"entity_name" => $vendorName,
													"department_id" => $creditDepartmentID,
											...
											foreach ($journal_entries as $entry) {
												...
												$query = "INSERT INTO qb_queue (`batch_id`, `action`, `shipment_no`, `posting_type`, `account_id`, `account_name`, `amount`, `description`, `created_at`, `class_id`, `department_id`, `entity_type`, `entity_id`, `entity_name`)
														VALUES ('$batch_id','$action_name', '$main_shipment_no', '$posting_type', '$account_id', '$account_name', '$amount', '$description','$time_now',
											...
											$query2 = "UPDATE `shipment_main` SET `qb_status`= '0' WHERE `id` = '$main_shipment_no'";
											$result2 = mysqli_query($conn, $query2);
										} else {
											$qb_msg = "Quickbooks error: shipment already saved in QuickBooks.";
											$query = "INSERT INTO qb_queue_error_log (action, error, doc_number) VALUES ('$action_name', '$qb_msg', '$main_shipment_no')";
											mysqli_query($conn, $query);
										}
									} else {
										$qb_msg = "Quickbooks error: supplier is not registered in QuickBooks.";
										$query = "INSERT INTO qb_queue_error_log (action, error, doc_number) VALUES ('$action_name', '$qb_msg', '$main_shipment_no')";
										mysqli_query($conn, $query);
									}
								}
						...
						$out = false;
						$addQtyFinalErrorMessage = $message;
						$message = 'Error: The shipment unique item could not be updated. Details: ' . $addQtyFinalErrorMessage;

						$query2 = "UPDATE `shipment_item_tmp` SET `status`= '0' WHERE `id` = '$shipment_tmp_item_id'";
						$result2 = mysqli_query($conn, $query2);
						if (!$result2) {
							$message .= " AND Error: The shipment item status could not be updated for ID {$shipment_tmp_item_id} after addQtyFinal failure.";
						}

						$query2 = "UPDATE `shipment_unic_tmp` SET `status`= '0' WHERE  shipment_tmp_itm_id = '$shipment_tmp_item_id'";
						$result2 = mysqli_query($conn, $query2);
						if (!$result2) {
							$message .= " AND Error: The unic item status could not be updated for item tmp ID {$shipment_tmp_item_id} after addQtyFinal failure.";
						}
					}
				}
			}
			...
			while ($row = mysqli_fetch_array($result)) {
				...
				$addQtyFinalSuccess = addQtyFinal($main_shipment_no, $unic, $inv_code, $add_qty, $c_price, $w_price, $r_price, $c_price2, $w_price2, $r_price2, "");
				if ($addQtyFinalSuccess) {
					$query1 = "UPDATE `shipment_item_tmp` SET `status`= '2' WHERE `id`='$ship_tmp_item_id'";
					$result1 = mysqli_query($conn, $query1);
					if (!$result1) {
						$out = false;
						$message = "Error: Unable to update shipment temp status";
					}

					if ($out) {
						$query2 = "SELECT COUNT(id) FROM `shipment_item_tmp` WHERE shipment_tmp_no = '$shipment_no'";
						$row2 = mysqli_fetch_row(mysqli_query($conn, $query2));
						$shipment_item_count = $row2[0];
						$query3 = "SELECT COUNT(id) FROM `shipment_item_tmp` WHERE `shipment_tmp_no` = '$shipment_no' AND `status` ='2'";
						$row3 = mysqli_fetch_row(mysqli_query($conn, $query3));
						$shipment_added_item_count = $row3[0];
							...
							if (isQuickBooksActive(1)) {
								...
								if ($vendorID != '') {
									if ($qbID == '') {
										...
										foreach ($journal_entries as $entry) {
											$shipment_no = mysqli_real_escape_string($conn, $main_shipment_no);
											$posting_type = mysqli_real_escape_string($conn, $entry['posting_type']);
											$account_id = mysqli_real_escape_string($conn, $entry['account_id']);
											$account_name = mysqli_real_escape_string($conn, $entry['account_name']);
											$amount = mysqli_real_escape_string($conn, $entry['amount']);
											...
											$query = "INSERT INTO qb_queue (`batch_id`, `action`, `shipment_no`, `posting_type`, `account_id`, `account_name`, `amount`, `description`, `created_at`, `class_id`, `department_id`,  `entity_type`, `entity_id`, `entity_name`)
													VALUES ('$batch_id', '$action_name', '$shipment_no', '$posting_type', '$account_id', '$account_name', '$amount', '$description','$time_now',
														" . ($class_id !== null ? "'$class_id'" : "NULL") . ",
														" . ($department_id !== null ? "'$department_id'" : "NULL") . ",
														" . ($entity_type !== null ? "'$entity_type'" : "NULL") . ",
														" . ($entity_id !== null ? "'$entity_id'" : "NULL") . ",
					...
					$out = false;
					$addQtyFinalErrorMessage = $message;
					$message = "Error: Unable to save to the original tables. Details: " . $addQtyFinalErrorMessage;
					$query1 = "UPDATE `shipment_item_tmp` SET `status`= '0' WHERE `id`='$ship_tmp_item_id'";
					if (!$result1) {
						$message .= " AND Error: The shipment item status could not be updated for ID {$ship_tmp_item_id} after addQtyFinal failure.";
					}
				}
			}
		}
...
{
	global $shipment_no, $supplier_id, $ship_date, $invoice_no, $invoice_date, $invoice_due, $editable, $conn;
	$user_id = $_COOKIE['user_id'];
...
{
	global $message, $shipment_no, $action, $conn;
...
function addQty($shipment_num, $unic, $code, $qty, $c_price, $w_price, $r_price, $c_price2, $w_price2, $r_price2)
{
	...
	if ($unic == 'yes') {
		if (validateDeleteShipment($shipment_no, 1))
			...
			$proceed = false;
			$msg = 'Error: The Shipment is LOCKED. Cannot Apend any items!';
		}
	} else {
		$proceed = true;
	}
	...
	if (((($unic0 == 0) || ($unic == 'yes'))) && ($proceed)) {
		...
		if ($qty > 0) {
			...
			$result2 = false;
			if ($duplicate > 0)
				$msg = 'Error: Duplicated Unic ID Detected';
			else
				$msg = 'Error: Item Quantity is 0';
		}
		...
		if ($result2) {
			...
			debugEnd($debug_id, 'success');
			if ($duplicate > 0)
				$message = 'Alert: Duplicated edit Item1 Unic ID Detected';
			else
				$message = 'Inventory updated successfully';
			return true;
		} else {
		...
		if (!$proceed)
			...
			$message = 'For this Item, Qty Cannot be Added from Here';
		return false;
	}
}
...
function removeUnicShipmentItemTmp()
{
	...
	if ($out) {
		...
		$result = mysqli_query($conn, $query);
		if (!$result) {
			$message = "Error: SNs could not be Deleted";
			$out = false;
		}
	}
	if ($out) {
...
// update by nirmal 11_07_2024 (if user has top manager permission, show all subsystems' item)
// update by nirmal 12_07_2024 (added unic cal true, unic items prices get)
function getOneItem($sub_system)
{
	...
	$sub_sys_qry1 = "AND s.`sub_system`='$sub_system'";
	include('config.php');
	...
	if (($unic_cal) && ($itm_unic == 1)) {
		...
		while ($row = mysqli_fetch_array($result)) {
			...
			if ($row[9] !== $currentStore) {
				$currentStore = $row[9];
				$isFirstRow = true; // Reset isFirstRow for the new store
			}

			// Apply different styling for subsequent rows within the same store
			$rowClass[] = $isFirstRow ? 'normal-row' : 'highlighted-row';
			$isFirstRow = false; // Set isFirstRow to false after the first row
		}
	} else {
		...
		while ($row = mysqli_fetch_array($result)) {
			...
			if ($row1) {
				...
				// Set default values when no rows are found
				$itq_id_tmp = '';
				$tb_id[$i] = '';
				$w_price[$i] = 0;
				$r_price[$i] = 0;
...
function editItem1($sub_system)
{
	...
	// $description=preg_replace('/[^A-Za-z0-9\-+.\ _]/', '',$_REQUEST['description']); // legacy code
	// $po_description=preg_replace('/[^A-Za-z0-9\-+.\ _]/', '',$_REQUEST['po_description']); // legacy code

	$id = mysqli_real_escape_string($conn, $_REQUEST['id']);
	$code = mysqli_real_escape_string($conn, trim(preg_replace('/[^A-Za-z0-9\-+.\ _]/', '', strtoupper($_REQUEST['code']))));
	$description = mysqli_real_escape_string($conn, trim(preg_replace('/[^A-Za-z0-9,&()\"\':;\/\-\+. _]/', '', $_REQUEST['description'])));
	$po_description = mysqli_real_escape_string($conn, trim(preg_replace('/[^A-Za-z0-9,&()\"\':;\/\-\+. _]/', '', $_REQUEST['po_description'])));
	$min_w_rate = mysqli_real_escape_string($conn, $_REQUEST['min_w_rate']);
	$max_w_rate = mysqli_real_escape_string($conn, $_REQUEST['max_w_rate']);
	$max_r_rate = mysqli_real_escape_string($conn, $_REQUEST['max_r_rate']);
	$def_cost = mysqli_real_escape_string($conn, $_REQUEST['c_price']);
	$def_price = mysqli_real_escape_string($conn, $_REQUEST['d_price']);
	$commision = mysqli_real_escape_string($conn, $_REQUEST['commision']);
	$supplier = mysqli_real_escape_string($conn, $_REQUEST['supplier']);
	$category = mysqli_real_escape_string($conn, $_REQUEST['category']);

	if (isset($_REQUEST['item_st']) && $_REQUEST['item_st'] == "on")
		$item_status = 1;
	if (isset($_REQUEST['item_st']) && $_REQUEST['item_st'] == "off")
		$item_status = 0;
	if (isset($_REQUEST['item_unic']) && $_REQUEST['item_unic'] == "yes")
		$item_unic = 1;
	if (isset($_REQUEST['item_unic']) && $_REQUEST['item_unic'] == "no")
		$item_unic = 0;
	$unit = '';
	if (isset($_REQUEST['unit_type'])) {
		$unit = $_REQUEST['unit_type'];
	...
	if ($out) {
		...
		if ($current_code != $code) {
			if (findDuplicateItem('code', $code, $id)) {
				$query = "UPDATE `inventory_items` SET `code`='$code' WHERE `id`='$id'";
				$result1 = mysqli_query($conn, $query);
			} else {
				$out = false;
				$message = 'Error: Duplicate item code detected';
			}
		}
		if ($current_desc != $description) {
			if (findDuplicateItem('description', $description, $id)) {
				$query = "UPDATE `inventory_items` SET `description`='$description' WHERE `id`='$id'";
				$result1 = mysqli_query($conn, $query);
			} else {
				$out = false;
				$message = 'Error: Duplicate item description detected';
			}
		}
		if ($current_podesc != $po_description) {
			if (findDuplicateItem('po_description', $po_description, $id)) {
				$query = "UPDATE `inventory_items` SET `po_description`='$po_description' WHERE `id`='$id'";
				$result1 = mysqli_query($conn, $query);
			} else {
				$out = false;
				$message = 'Error: Duplicate item PO description detected';
			}
		}
...
// edit by nirmal 28_11_2023 (restricted item edit with saving 0 values to w_price and r_price)
// edit by nirmal 09_02_2024 (restricted edit item qty or price only master password 01 is entered and valid)
// updated by nirmal 19_05_2025 (added qb integration)
...
function editItem2($sub_system)
{
	...
	$drawer = $_GET['drawer'];
	$out = true;
	$qty_qry = $item_code = $item_description = $debug_id = "";
	$qty1 = $inventory_cost_for_item_before = $inventory_cost_for_item_after = 0;
	$time_now = timeNow();
	$user_store = $_COOKIE['store'];
	...
	try {
		...
		$master_pw = mysqli_real_escape_string($conn, $master_pw);
		$w_price = mysqli_real_escape_string($conn, $w_price);
		$r_price = mysqli_real_escape_string($conn, $r_price);
		$c_price = mysqli_real_escape_string($conn, $c_price);
		$qty = mysqli_real_escape_string($conn, $qty);
		$drawer = trim(mysqli_real_escape_string($conn, $drawer));
		$item_id = mysqli_real_escape_string($conn, $item_id);
		$stores_id = mysqli_real_escape_string($conn, $stores_id);
		$tb_id = mysqli_real_escape_string($conn, $tb_id);

		if ($qty < 0) {
			$message = 'Error: QTY cannot be less than zero';
			throw new Exception($message);
		}

		$query1 = "SELECT `value` FROM settings WHERE setting='master_pw'";
		$result1 = mysqli_query($conn, $query1);
		if (!$result1) {
		...
		if ($master_pw != '') {
			...
			$out = false;
			$message = 'Error : Master password 01 is required';
			throw new Exception($message);
		}

		// added by nirmal 28_11_2023
		if (($out)) {
			if ((!($w_price > 0)) || (!($r_price > 0))) {
				$out = false;
				$message = 'Wholesale or Retail price cannot be zero or empty!';
		...
		if ($out) {
			$query = '';
			if ($inv_type == 'new') {
				$query1 = "SELECT c_price * qty  FROM inventory_new WHERE id='$tb_id'";
				$result1 = mysqli_query($conn, $query1);
				if (!$result1) {
					throw new Exception('Database error: ' . mysqli_error($conn));
				}
				$row1 = mysqli_fetch_row($result1);
				$inventory_cost_for_item_before = $row1[0];
				...
				if ($tb_id == '') { // if store don't have any qty
					...
					if ($row1[0] == 1) {
						...
						$inventory_cost_for_item_before = 0;
					...
					$query1 = "SELECT c_price * qty  FROM inventory_qty WHERE id='$tb_id'";
					$result1 = mysqli_query($conn, $query1);
					if (!$result1) {
						throw new Exception('Database error: ' . mysqli_error($conn));
					}
		...
		if ($out) {
			if (isQuickBooksActive(1)) {
				...
				$accountQuery4 = "SELECT `qb_id` FROM `stores` WHERE `id` = '$user_store'";
				$accountResult4 = mysqli_fetch_row(mysqli_query($conn, $accountQuery4));
				$storeQBID = $accountResult4[0];
				...
				if ($inv_type == 'new') {
					...
					$row1 = mysqli_fetch_row($result1);
					$inventory_cost_for_item_after = $row1[0];
				} else {
					$query1 = "SELECT c_price * qty  FROM inventory_qty WHERE id='$tb_id'";
					$result1 = mysqli_query($conn, $query1);
					if (!$result1) {
				...
				$query = "SELECT `code`, `description` FROM inventory_items WHERE id='$item_id'";
				$result = mysqli_query($conn, $query);
				if (!$result) {
					throw new Exception('Database error: ' . mysqli_error($conn));
				}
				...
				// Existing (OLD) cost
				if ($inventory_cost_for_item_before > 0) {
					$debitAccountName = "Stock Written Off";
					$creditAccountName = "Inventory Asset";
					$description = "[EDIT ITEM QTY] - $item_code ($item_description)";
					$debitEntityType = "";
					$debitEntityID = "";
					$creditEntityType = "";
					$creditEntityID = "";

					$journalEntryForEditItemQty = buildJournalEntry(
						$conn,
						abs($inventory_cost_for_item_before),
						$debitAccountName,
						$creditAccountName,
						$description,
						$debitEntityType,
					...
					if (isset($journalEntryForEditItemQty['error'])) {
						$qb_msg = $journalEntryForEditItemQty['error'];
						...
						$action_name = 'edit_item_qty';
						foreach ($journalEntryForEditItemQty as $entry) {
							$posting_type = mysqli_real_escape_string($conn, $entry['posting_type']);
							$account_id = mysqli_real_escape_string($conn, $entry['account_id']);
							$account_name = mysqli_real_escape_string($conn, $entry['account_name']);
							$amount = mysqli_real_escape_string($conn, $entry['amount']);
							...
							$query = "INSERT INTO qb_queue (`batch_id`, `action`, `posting_type`, `account_id`, `account_name`, `amount`, `description`, `created_at`, `class_id`, `department_id`, `entity_type`, `entity_id`)
				...
				// New cost
				if ($inventory_cost_for_item_after > 0) {
					...
					$journalEntryForEditItemQty = buildJournalEntry(
						$conn,
						abs($inventory_cost_for_item_after),
						$debitAccountName,
						$creditAccountName,
						$description,
						$debitEntityType,
...
function editUnicsPrice($sub_system)
{
	...
	$old_w_price = $_GET['old_w_price'];
	$old_r_price = $_GET['old_r_price'];
	$old_c_price = $_GET['old_c_price'];
	$new_w_price = $_GET['new_w_price'];
	$new_r_price = $_GET['new_r_price'];
	$new_c_price = $_GET['new_c_price'];
	$drawer = $_GET['drawer'];
	$out = true;
	...
	// drawer number update
	if ($out) {
		$query = "UPDATE `inventory_qty` SET `drawer_no`='$drawer' WHERE `item`='$item_id' AND `location`='$store_id' AND `id`='$itq_id'";
		$result = mysqli_query($conn, $query);
		if (!$result) {
			$message = 'Error: Could not update the drawer number';
			$status = 'error';
			$out = false;
		}
	}
	...
	if (($out)) {
		if ((!($new_w_price > 0)) || (!($new_r_price > 0))) {
			$out = false;
			$message = 'Error: New Wholesale or Retail price cannot be zero or empty!';
			$status = 'error';
		}
	}

	if ($out) {
		if ((!($old_w_price > 0)) || (!($old_r_price > 0))) {
			$out = false;
			$message = 'Error: Existing Wholesale or Retail price cannot be zero or empty!';
			$status = 'error';
		}
	}
	...
	// price update
	if ($out) {
		$query = "SELECT `id` FROM inventory_unic_item WHERE `w_price`='$old_w_price' AND `r_price`='$old_r_price' AND `c_price`='$old_c_price' AND `itq_id`='$itq_id'";
		$result = mysqli_query($conn2, $query);
		if (!$result) {
			$message = 'Error: Could not execute the initial query - ' . mysqli_error($conn2);
			$out = false;
			$status = 'error';
		}
		if ($out) {
			while ($row = mysqli_fetch_array($result)) {
				$id = $row[0];
				$query1 = "UPDATE `inventory_unic_item` SET `w_price`='$new_w_price', `r_price`='$new_r_price', `c_price`='$new_c_price' WHERE `id`='$id' AND `status`!='1'";
				$result1 = mysqli_query($conn, $query1);
				if (!$result1) {
					$message = 'Error: Could not update the price for item ID ' . $id . ' - ' . mysqli_error($conn);
					$out = false;
					$status = 'error';
				}
			}
...
// update by nirmal 30_11_2023
// updated by nirmal 27_11_2024 (if logged user top-mgr then show data for all sub-systems)
// updated by nirmal 05_03_2025 (filter shipment to shipment entered time not shipment date)

function getShipmentList($systemid, $sub_system)
{
	global $shipment_no, $salesman, $current_user, $date, $time, $active, $unic, $ship_store, $month, $store0, $shipment_edit_time, $ship_total_cost,
	$ship_invoice_no, $total, $status, $invoice_date, $totalOfDeletedShipments, $unicItemDeletedCountInShipment, $invoice_date,$conn2;
	$user = $_COOKIE['user_id'];
	$month = $_GET['month'];
	$date_type = '';
	...
	$total = 0;
	$time_now = timeNow();
	$shipment_no = $store_name = $ship_invoice_no = $ship_total_cost = $status = $unicItemDeletedCountInShipment = $invoice_date = array();
	$sub_sys_qry = " sub_system='$sub_system'  AND ";
	include('config.php');
	...
	if (isset($_GET['store'])) {
		$store0 = $_GET['store'];
		if ($store0 != 'all' && $store0 != "")
			$qry_st = "AND ins.location='$store0'";
	}
	...
	if ($month != '') {
		...
		// $query = "SELECT ins.`shipment_no`, ins.`added_by`, date(ins.`time`), time(ins.`time`), ins.`time`, sm.`unic`, ins.`location`, sm.`invoice_no`, SUM(ins.`cost` * ins.`added_qty`) AS total_cost, sm.`status`
		// FROM inventory_shipment ins LEFT JOIN shipment_main sm ON ins.`shipment_no`=sm.`id`
		// WHERE $sub_sys_qry sm.`shipment_date` LIKE '$month%' $qry_st GROUP BY ins.`shipment_no` ORDER BY ins.`shipment_no` DESC";

		if ($date_type == 'invoice_month') {
			$date_filter_clause = "DATE_FORMAT(sm.`invoice_date`, '%Y-%m') LIKE '$month'";
		} else {
			// Default or 'entered_month'
			$date_filter_clause = "DATE(ins.`time`) LIKE '$month%'";
		}

		// $query = "SELECT ins.`shipment_no`, ins.`added_by`, DATE(ins.`time`), TIME(ins.`time`), ins.`time`, sm.`unic`, ins.`location`, sm.`invoice_no`, SUM(ins.`cost` * ins.`added_qty`) AS total_cost, sm.`status`, sm.`invoice_date`
		// FROM inventory_shipment ins LEFT JOIN shipment_main sm ON ins.`shipment_no`=sm.`id`
		// WHERE $sub_sys_qry $date_filter_clause $qry_st GROUP BY ins.`shipment_no` ORDER BY ins.`shipment_no` DESC";
		$query = "SELECT 
			ins.`shipment_no`,
			MAX(ins.`added_by`) AS `added_by`,
			DATE(MAX(ins.`time`)) AS `shipment_date`,
			TIME(MAX(ins.`time`)) AS `shipment_time`,
			MAX(ins.`time`) AS `shipment_timestamp`,
			MAX(sm.`unic`) AS `unic`,
			MAX(ins.`location`) AS `location`,
			MAX(sm.`invoice_no`) AS `invoice_no`,
			SUM(ins.`cost` * ins.`added_qty`) AS total_cost,
			MAX(sm.`status`) AS `status`,
			MAX(sm.`invoice_date`) AS `invoice_date`
		FROM inventory_shipment ins
		LEFT JOIN shipment_main sm ON ins.`shipment_no` = sm.`id`
		WHERE $sub_sys_qry $date_filter_clause $qry_st
		GROUP BY ins.`shipment_no`
		ORDER BY ins.`shipment_no` DESC";
		$result = mysqli_query($conn2, $query);
		if (!$result) {
			$message = 'Error: Could not execute the query - ' . mysqli_error($conn2);
			$out = false;
			$status = 'error';
		} else {
			while ($row = mysqli_fetch_array($result)) {

				$query1 = "SELECT COUNT(`id`) FROM inventory_unic_item WHERE shipment_no='$row[0]' AND `status` = 2";
				$row1 = mysqli_fetch_row(mysqli_query($conn2, $query1));
				$unicItemDeletedCountInShipment[] = $row1[0];
...
{
	global $shipment_no_tmp, $salesman_tmp, $date_tmp, $unic_tmp, $ship_store_tmp, $edit_ship_tmp, $month_tmp, $invoice_no_tmp,$conn2;
	$user_id = $_COOKIE['user_id'];
	$month_tmp = $_GET['month'];
	$shipment_no_tmp = $invoice_no_tmp = array();
	$sub_sys_qry = " AND smt.`sub_system`='$sub_system' ";
	...
	if ($month_tmp != '') {
		include('config.php');
		$query = "SELECT smt.`id`,up.`id`,up.`username`,smt.`shipment_date`,smt.`unic`,st.`name`,smt.`invoice_no`
		FROM shipment_main_tmp smt, userprofile up, stores st
		WHERE smt.`added_by`=up.`id` AND smt.`store`=st.`id` $sub_sys_qry AND smt.`shipment_date` LIKE '$month_tmp%' ORDER BY smt.`id` DESC";
		$result = mysqli_query($conn2, $query);
		while ($row = mysqli_fetch_array($result)) {
...
function oneShipment()
{
	...
	$query = "SELECT ins.id,itm.description,ins.old_qty,ins.added_qty,ins.added_by,date(ins.time),time(ins.time),st.name,
	ins.cost,itm.unic,itm.code
	FROM inventory_shipment ins, inventory_items itm, stores st
	WHERE ins.inv_item=itm.id AND ins.location=st.id AND ins.shipment_no='$shipment_no' ORDER BY ins.id";
	$result = mysqli_query($conn2, $query);
...
function oneShipmentExp()
{
	...
	$today = dateNow();
	include('config.php');
	$query = "SELECT itm.description,ins.added_qty,ins.cost,inu.sn
	FROM inventory_shipment ins, inventory_items itm, inventory_unic_item inu
	WHERE ins.inv_item=itm.id AND ins.id=inu.inv_ship_id AND ins.shipment_no='$shipment_no' ORDER BY itm.description";
	$result = mysqli_query($conn2, $query);
	while ($row = mysqli_fetch_array($result)) {
		$sh_itm_des[] = $row[0];
		$sh_qty[] = $row[1];
...
function reorderRepairParts()
{
	...
	$today = dateNow();
	include('config.php');
	$query = "SELECT rp.name,ri.reorder_level-ri.qty,ri.reorder_qty
	FROM repair_parts_inventory ri, repair_parts rp
	WHERE rp.id=ri.part AND ri.qty<ri.reorder_level AND rp.`status`=1 AND ri.location='$store'";
	$result = mysqli_query($conn2, $query);
	while ($row = mysqli_fetch_array($result)) {
		$rp_name[] = $row[0];
		if ($row[2] > $row[1])
			$ri_reorder_qty[] = $row[2];
		else
			$ri_reorder_qty[] = $row[1];
	}
...
// 	$sr_id = $cr_id = array();
// 	include('config.php');
// 	$query = "SELECT sr.id,itm.description,sr.`district`,sr.increment FROM special_rate sr, inventory_items itm WHERE sr.item=itm.id AND sr.sub_system='$sub_system' ORDER BY sr.increment DESC";
// 	$result = mysqli_query($conn2, $query);
// 	while ($row = mysqli_fetch_array($result)) {
// 		$sr_id[] = $row[0];
// 		$sr_item[] = $row[1];
...
// 		}
// 	}
// 	$query = "SELECT cr.id,ic.name,cr.`district`,cr.increment FROM category_rate cr, item_category ic WHERE cr.category=ic.id AND cr.sub_system='$sub_system' ORDER BY cr.increment DESC";
// 	$result = mysqli_query($conn2, $query);
// 	while ($row = mysqli_fetch_array($result)) {
// 		$cr_id[] = $row[0];
// 		$cr_category[] = $row[1];
...
function getSpecialPrice($sub_system)
{
	global $sr_id, $sr_item, $sr_district, $sr_increment, $cr_id, $cr_category, $cr_district, $cr_increment, $conn2;

	$sr_id = $sr_item = $sr_district = $sr_increment = array();
	$cr_id = $cr_category = $cr_district = $cr_increment = array();
...
function getDistrict()
{
	global $district_id, $district_name, $conn2;

	include_once('config.php');

	$district_id = $district_name = array();
...
function addSpecialPrice($sub_system)
{
	...
	if ($district == 0) {
		$result1 = mysqli_query($conn, "SELECT count(id) as `count` FROM special_rate WHERE item='$item'");
		$row1 = mysqli_fetch_assoc($result1);
		$count = $row1['count'];
		if ($count != 0) {
			$message = 'This item already has a district rate.';
			return false;
		}
	}
	...
	if ($out) {
		...
		if ($count == 0) {
			...
			$message = 'A special rate already exists for the specified item and district.';
			return false;
		}
	}
}
...
function getDistrictPrice($sub_system)
{
	global $di_id, $di_name, $di_increment, $conn2;

	$di_id = $di_name = $di_increment = array();
...
{
	global $itt_id, $itt_itm, $itt_wprice, $itt_rprice, $itt_cprice, $itt_qty, $itt_store, $itt_shipment, $conn2;

	$itt_id = $itt_itm = $itt_wprice = $itt_rprice = $itt_cprice = $itt_qty = $itt_store = $itt_shipment = array();

	include_once('config.php');

	$query = "SELECT itt.id,itm.description,itt.w_price,itt.r_price,itt.c_price,itt.qty,st.name,itt.shipment_no
			  FROM inventory_new itt, inventory_items itm, stores st
			  WHERE itt.item=itm.id AND itt.store=st.id
			  ORDER BY itt.id";
...
function addRepairInv()
{
	...
	if ($out) {
		...
		if (!$result) {
			$msg = 'Error: Inventory Could Not Be Updated !';
			$out = false;
		}
	}
	if ($out) {
		...
		$result = mysqli_query($conn, $query);
		if (!$result) {
			$msg = 'Error: Inventory Updated But Shipment record was Failed !';
			$out = false;
		}
	}

	if ($out) {
		$message = 'Repair Inventory updated successfully';
		return true;
	} else {
		$message = $msg;
		return false;
...
function getParts()
{
	global $part_id, $part_name, $part_drawer,$conn2;
...
function addRepairPart()
{
	...
	if ($count == 0) {
		...
		if ($result) {
			...
			$result = mysqli_query($conn, $query);
			if (!$result) {
				$msg = 'Error: Part Name Could Not Be Added !';
				$out = false;
			}
		} else {
			$msg = 'Error: Part Name Could Not Be Added !';
		...
		$msg = 'Error: Duplicated Name Found !';
		$out = false;
	}
	if ($out) {
...
function updateRepairPart()
{
	...
	if ($count == 0) {
		...
		$result = mysqli_query($conn, $query);
		if (!$result) {
			$msg = 'Error: Part Name Could Not Be Updated !';
			$out = false;
		}
	} else {
		$msg = 'Error: Duplicated Name Found !';
...
function enableRepairPart()
{
	...
	$result = mysqli_query($conn, $query);
	if (!$result) {
		$msg = 'Error: Part Could Not Be Enabled !';
		$out = false;
	}
	if ($out) {
...
function getTags2()
{
	global $tag_id, $tag_name, $tag_profit, $tag_licount, $conn;

	$tag_id = $tag_name = $tag_profit = $tag_licount = array();
...
function addTag($item_id, $tag_name)
{
	...
	if ($out) {
		if ($row[1] > 0) {
			...
			if ($row[0] > 0) {
				$out = false;
				$msg = 'Error: You can only add one price tag to an item';
			}
		}
	}
	if ($out) {
		...
		if ($count == 1) {
			$out = false;
			$msg = 'Error: Tag is already assigned';
		}
		if ($count > 1) {
	...
	if ($out) {
		...
		if (!$result) {
			$out = false;
			$msg = 'Error: Tag Could Not be Added';
		}
	}
	return $msg;
}
...
function createTag($tag_name, $tag_profit)
{
	...
	if ($tag_name == '') {
		$out = false;
		$msg = 'Error: Tag Name cannot be Empty';
	}
	...
	if ($row[0] != 0) {
		$out = false;
		$msg = 'Error: Tag is already exists';
	}
	if ($out) {
		...
		if (!$result) {
			$out = false;
			$msg = 'Error: Tag Could Not be Created';
		}
	}
...
function removeTag($item_id, $tag_id)
{
	...
	if (!$result) {
		$out = false;
		$msg = 'Error: Tag Could Not be Removed';
	}
...
function deleteTag($tag_id)
{
	...
	if ($row[0] > 0) {
		$out = false;
		$message = 'Please remove all associated items from this Tag before deletion';
	}
...
function updateTag($tag_id, $tag_name, $tag_profit)
{
	...
	if ($out) {
		...
		if (!$result) {
			$out = false;
			$msg = 'Error: Tag Could Not be Updated';
		}
	}
	return $msg;
}
...
function getOneItem2($sub_system, $systemid)
{
	...
	include('config.php');
	$query = "SELECT itm.`id`, itm.`code`, itm.`description`, itq.`c_price`, itq.`w_price`, itq.`r_price`, itq.`drawer_no` FROM inventory_items itm, inventory_qty itq WHERE itm.id=itq.item AND itm.`$case`='$value' AND itm.`sub_system`='$sub_system'";
	$row = mysqli_fetch_row(mysqli_query($conn2, $query));
	if (!empty($row)) {
		$id = $row[0];
		$code = $row[1];
		...
		$message = "Error: " . mysqli_error($conn2);
		$out = false;
	}
}
?>