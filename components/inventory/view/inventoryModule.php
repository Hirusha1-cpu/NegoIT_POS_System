...
function listCodeData($itmstatus){
	...
	if ($_POST['keyword']) {
		...
		if($itmstatus=='nounic'){
			...
			$fn = 'selectItemCode'; 
			$qry = "AND itm.`code` LIKE '%$keyword%' LIMIT 20";
		}
		if($itmstatus=='unic'){
		...
		$store=$_COOKIE['store'];
		include('config.php');
		$query="SELECT itm.code FROM inventory_items itm LEFT JOIN (SELECT * FROM inventory_qty WHERE location='$store') as itq ON itm.id=itq.item $itemst_qry $qry";
		$result=mysqli_query($conn,$query);
		while($row=mysqli_fetch_array($result)){
			$data_list[]=$row[0];
		} 
...
function moreItemData(){
	...
	if (isset($_POST['case'])) {
		...
		$val = $_POST['val'];
		$jasonArray = array();
		if ($case == 'code' && $type == 'nounic'){
			$itemst_qry='WHERE itm.`unic`=0 AND itm.`status`=1';
			$qry = "AND itm.`code`='$val'";
		}
		if ($case == 'code' && $type == 'unic'){
			$itemst_qry='WHERE itm.`unic`=1 AND itm.`status`=1';
			$qry = "AND itm.`code`='$val'";
		}
		...
		$query="SELECT itm.id,itm.code,itm.description FROM inventory_items itm LEFT JOIN (SELECT * FROM inventory_qty WHERE location='$store') as itq ON itm.id=itq.item $itemst_qry $qry";

		$row = mysqli_fetch_row(mysqli_query($conn, $query));
		$jasonArray["item_id"] = $row[0];
		$jasonArray["item_code"] = $row[1];
		$jasonArray["item_desc"] = $row[2];
...
function getItemData(){
	...
	include('config.php');
	$query="SELECT itm.id,itm.`code`,itm.description,itq.c_price,itq.w_price,itq.r_price,itq.qty,itq.drawer_no,itq.id FROM inventory_items itm LEFT JOIN inventory_qty itq ON itm.id=itq.item AND itq.`location`='$store' WHERE $qry";
...
function getItems2($itmstatus){
	...
	if($itmstatus=='off') $itemst_qry='';
	include('config.php');
	$query="SELECT id,code,description FROM inventory_items $itemst_qry ORDER BY description";
	$result=mysqli_query($conn2,$query);
	while($row=mysqli_fetch_array($result)){
		$item_id[]=$row[0];
		$code[]=$row[1];
...
function getItems($sub_system){
	...
	if(isset($_GET['store'])){
		...
		elseif($store_req=='all-sub') $store_qry='st.`sub_system`='.$sub_system.' AND'; 
...
function getShipmentItemsTmp($shipment_no){
	global $ship_inv_id,$ship_itm_desc,$ship_item_qty,$ship_item_status;
	$ship_inv_id=$ship_itm_desc=$ship_item_qty=$ship_item_status=array();
...
function getShipmentUnicItemsTmp($shipment_no){
	...
	if($shipment_no != ""){
		include('config.php');
		$query="SELECT sut.`inv_item`, itm.`description`,COUNT(sut.`inv_item`),sut.`status` FROM shipment_unic_tmp sut, inventory_items itm WHERE sut.`inv_item`=itm.`id` AND sut.`shipment_tmp_no`='$shipment_no' GROUP BY sut.`inv_item`";
		$result=mysqli_query($conn,$query);
		while($row=mysqli_fetch_array($result)){
			$ship_inv_id[]=$row[0];
			$ship_itm_desc[]=$row[1];
...
// added by nirmal 21_08_30, update by nirmal 21_9_2
function getSnList(){
	include('config.php');

	$shipment_no = $_POST['shipment_no'];
	$ship_inv_id = $_POST['ship_inv_id'];

	$query = "SELECT sut.`id`,sut.`sn` FROM shipment_unic_tmp sut, inventory_items it WHERE it.`id` = sut.`inv_item` AND sut.`shipment_tmp_no`='$shipment_no' AND sut.`inv_item`='$ship_inv_id'";
	$result=mysqli_query($conn,$query);

	while($row=mysqli_fetch_array($result)){
		$jsonArray["unic_tmp_id"][] = $row[0];
		$jsonArray["unic_tmp_sn"][] = $row[1];
	}
...
// add by nirmal 21_9_1
function removeSn($shipment_num, $id){
	...
	if($shipment_no != ""){
		...
		$result = mysqli_query($conn,"SELECT smt.`added_by` FROM `shipment_main_tmp` smt WHERE smt.`id` = '$shipment_no'");
		$row = mysqli_fetch_assoc($result);
		$added_by=$row['added_by'];
		...
		if($added_by != $user){
			...
			$result1=mysqli_query($conn,"DELETE FROM `shipment_unic_tmp` WHERE `status` = '1' AND `id`='$id'");
			if($result1){
				$message = "Item was removed successfully";
				return true;
			}else{
				$message = "Sorry! inventory could not be updated!";
				return false;
			}
		}	
	}
...
// added by nirmal 21_08_17, updated by nirmal 21_09_2
function addQtyTmp($shipment,$unic,$code,$qty,$c_price,$w_price,$r_price,$c_price2,$w_price2,$r_price2){
	...
	$unic0 = 0;
	$user=$_COOKIE['user'];
	$result = mysqli_query($conn,"SELECT id FROM inventory_items WHERE code='$code'");
	$row = mysqli_fetch_assoc($result);
	$inv_id=$row['id'];
	$time_now=timeNow();
	$result0 = false;
	...
	if($unic0 == 1){
		for($i=1;$i<=10;$i++){
			if($_REQUEST['sn'.$i]!=''){
				$sn=$_REQUEST['sn'.$i];
				$result = mysqli_query($conn,"SELECT count(id) as `count` FROM shipment_unic_tmp WHERE sn='$sn' AND `status`!=2");
				$row = mysqli_fetch_assoc($result);
				$unic_count=$row['count'];
				if($unic_count==0){
					$query0="INSERT INTO `shipment_unic_tmp` (`shipment_tmp_no`,`inv_item`,`sn`,`w_price`,`r_price`,`c_price`,`w_price2`,`r_price2`,`c_price2`,`status`) VALUES ('$shipment_no','$inv_id','$sn','$w_price','$r_price','$c_price','$w_price2','$r_price2','$c_price2','1') ";
					$result0=mysqli_query($conn,$query0);
					if($result0) $out = true;
					else $out = false;
				}
		...
		$query="INSERT INTO `shipment_item_tmp` (`shipment_tmp_no`,`inv_item`,`inv_code`,`added_qty`,`added_by`,`c_price`,`w_price`,`r_price`,`c_price2`,`w_price2`,`r_price2`,`unic`,`time`,`status`) VALUES ('$shipment','$inv_id','$code','$qty','$user','$c_price','$w_price','$r_price','$c_price2','$w_price2','$r_price2','$unic0','$time_now','1')";
		$result=mysqli_query($conn,$query);
...
// added by nirmal 21_08_17
function oneShipmentTmpFinalize($shipment_no, $action = null, $sub_system){
	include('config.php');
	global $main_shipment_no;
	$id = $unic = $inv_code = $add_qty = $c_price = $w_price = $r_price = $c_price2 = $w_price2 = $r_price2 = "";
	$ship_date = $suplier = $ship_inv_no = $ship_inv_date = $ship_inv_dudate = "";
	$sub = "";
	$out = true;
	$main_shipment_no = 0;
	...
	if($action == "show_add_unic_tmp"){
		...
		$query="SELECT `shipment_date`,`supplier`,`invoice_no`,`invoice_date`,`invoice_due`,`unic` FROM shipment_main_tmp WHERE id='$shipment_no'";
		$result=mysqli_query($conn,$query);

		$row1 = mysqli_fetch_assoc($result);
		$ship_date = $row1['shipment_date'];
		$suplier = $row1['supplier'];
		$ship_inv_no = $row1['invoice_no'];
		$ship_inv_date = $row1['invoice_date'];
		$ship_inv_dudate = $row1['invoice_due'];
		$unic = $row1['unic'];

		if($unic == 1) $sub = 'show_add_unic';

		// var_dump($query);
		// exit;
		$query1 = "SELECT count(*) AS `count` FROM shipment_unic_tmp WHERE shipment_tmp_no='$shipment_no' AND `status` IN('2')";
		$result1=mysqli_query($conn,$query1);
		$row = mysqli_fetch_assoc($result2);
		$saved_count = $row['count'];
		// if there is already item saved for this tmp shipment no, not save new record again in shipment_main table
		if($saved_count <= 0){
			if(addShipment($sub_system,$ship_date,$suplier,$ship_inv_no,$ship_inv_date,$ship_inv_dudate,$sub)) $out = true;
			else $out = false;
		}
		// select original table (shipment_main) inserted id
		$result2 = mysqli_query($conn,"SELECT id FROM shipment_main ORDER BY id DESC LIMIT 1");
		$row2 = mysqli_fetch_assoc($result2);
		$main_shipment_no = $row2['id'];

		// get inv_item ids from shipment_unic_tmp table 
		$query3 = "SELECT sut.`inv_item`, itm.`code` FROM inventory_items itm, shipment_unic_tmp sut WHERE sut.`shipment_tmp_no`='$shipment_no' AND itm.`id` = sut.`inv_item` AND sut.`status` IN('0','1') GROUP BY sut.`inv_item`";
		$result3=mysqli_query($conn,$query3);
		while($row3 = mysqli_fetch_assoc($result3)){
			$item_id = $row3['inv_item'];
			$inv_code = $row3['code'];
			$sn = array();
			$query4 = "SELECT sut.`sn`, sut.`c_price`, sut.`w_price`, sut.`r_price`, sut.`c_price2`, sut.`w_price2`, sut.`r_price2` FROM shipment_unic_tmp sut WHERE sut.`inv_item`='$item_id' AND sut.`status` IN('0','1')";
			$result4=mysqli_query($conn,$query4);
			$add_qty=mysqli_num_rows($result4);

			while($row4 = mysqli_fetch_assoc($result4)){
				$sn[] = $row4['sn'];
				$c_price = $row4['c_price'];
				$w_price = $row4['w_price'];
				$r_price = $row4['r_price'];
				$c_price2 = $row4['c_price2'];
				$w_price2 = $row4['w_price2'];
				$r_price2 = $row4['r_price2'];
 			}
			// send data to real function to save
			if(addQtyFinal($main_shipment_no,$unic,$inv_code,$add_qty,$c_price,$w_price,$r_price,$c_price2,$w_price2,$r_price2,$sn)){
			
				$query="UPDATE `shipment_unic_tmp` SET `status`= '2' WHERE `shipment_tmp_no` = '$shipment_no' AND `inv_item` = '$item_id'";
				mysqli_query($conn,$query);

				$query1 = "SELECT count(*) AS `count` FROM shipment_unic_tmp WHERE shipment_tmp_no='$shipment_no' AND `status` IN('0','1')";
				$result1=mysqli_query($conn,$query1);
				$count=mysqli_num_rows($result1);
				if($count == 0) {
					mysqli_query($conn,"DELETE FROM `shipment_unic_tmp` WHERE `shipment_tmp_no`='$shipment_no'");
					mysqli_query($conn,"DELETE FROM `shipment_main_tmp` WHERE `id`='$shipment_no'");
				}
				$out = true;
			}else{
				$out = false;
				$query="UPDATE `shipment_unic_tmp` SET `status`= '0' WHERE `shipment_tmp_no` = '$shipment_no' AND `inv_item` = '$item_id'";
				mysqli_query($conn,$query);
			}
		}
				// get summery of shipment to display in action=one_shipment_tmp
		if($out){
			oneShipmentTmp($main_shipment_no);
			return true;
		}else{
			return false;
		}
		...
		$query = "SELECT `status` FROM `shipment_item_tmp` WHERE shipment_tmp_no ='$shipment_no' AND `status` = '2'";
		$row=mysqli_fetch_row(mysqli_query($conn,$query));
		...
		$query5 = "SELECT * FROM `shipment_item_tmp` WHERE `status` IN('0','1') AND shipment_tmp_no ='$shipment_no'";
		$result=mysqli_query($conn,$query5);

		while($row5 = mysqli_fetch_assoc($result)){
			if($out){
				$id = $row5['id'];
				$inv_code = $row5['inv_code'];
				$add_qty = $row5['added_qty'];	
				$c_price = $row5['c_price'];
				$w_price = $row5['w_price'];
				$r_price = $row5['r_price'];
				$c_price2 = $row5['c_price2'];
				$w_price2 = $row5['w_price2'];
				$r_price2 = $row5['r_price2'];
				$unic = $row5['unic'];

				// updated nirmal - 21_08_26
				// update status of tmp shipment according to saved status of original inventory tables.
				if(addQty($main_shipment_no,$unic,$inv_code,$add_qty,$c_price,$w_price,$r_price,$c_price2,$w_price2,$r_price2)){
					if($unic == 1){
						$query="UPDATE `shipment_unic_tmp` SET `status`= '2' WHERE `shipment_tmp_no` = '$shipment_no' AND `sn` = '$sn'";
					}else{
						$query="UPDATE `shipment_item_tmp` SET `status`= '2' WHERE `id`='$id'";
					}
					mysqli_query($conn,$query);
					$out = true;
					...
					$query2 = "SELECT COUNT(id) FROM `shipment_item_tmp` WHERE `shipment_tmp_no` = '$shipment_no' AND `status` ='2'";
					...
					$out = false;
					if($unic == 1){
						$query="UPDATE `shipment_unic_tmp` SET `status`= '0' WHERE `shipment_tmp_no` = '$shipment_no' AND `sn` = '$sn'";
					}else{
						$query="UPDATE `shipment_item_tmp` SET `status`= '0' WHERE `id`='$id'";
						mysqli_query($conn,$query);
					}
				}
...
function addQtyFinal($shipment_num,$unic,$code,$qty,$c_price,$w_price,$r_price,$c_price2,$w_price2,$r_price2,$sn){
	...
	if(((($unic0==0)||($unic=='yes'))) &&($proceed)){
		...
		if($unic=='yes'){
			...
			for($i=0;$i<=9;$i++){
				if($sn!=''){
					$result = mysqli_query($conn,"SELECT count(id) as `count` FROM inventory_unic_item WHERE sn='$sn[$i]' AND `status`!=2");
					$row = mysqli_fetch_assoc($result);
					$unic_count=$row['count'];
					if($unic_count==0){
						$qty0++;
			...
			if($unic=='yes'){
				for($i=0;$i<=9;$i++){
					if($sn !=''){
						...
						if($unic_count==0){
							if($unic_cal)
								$query0="INSERT INTO `inventory_unic_item` (`itq_id`,`shipment_no`,`inv_ship_id`,`sn`,`w_price`,`r_price`,`c_price`,`status`) VALUES ('$iqt_id','$shipment_no','$lastid','$sn[$i]','$w_price1','$r_price1','$c_price1','0') ";
							else
								$query0="INSERT INTO `inventory_unic_item` (`itq_id`,`shipment_no`,`inv_ship_id`,`sn[$i]`,`status`) VALUES ('$iqt_id','$shipment_no','$lastid','$sn','0') ";
							$result0=mysqli_query($conn,$query0);
						}
					}
				}
...
function getShipmentHeaderDetails($shipment_num){
	global $shipment_no,$supplier_id,$ship_date,$invoice_no,$invoice_date,$invoice_due;
	$user=$_COOKIE['user'];	

	if(isset($shipment_num)){
		include('config.php');

		$query = "SELECT id FROM userprofile WHERE username = '$user'";
		$result=mysqli_query($conn,$query);
		$row = mysqli_fetch_assoc($result);
		$user_id = $row['id'];

		$query="SELECT smt.`id`,smt.`shipment_date`,smt.`supplier`,smt.`invoice_no`,smt.`invoice_date`,smt.`invoice_due` FROM shipment_main_tmp smt WHERE smt.`id`= '$shipment_num' AND smt.`added_by`='$user_id'";
		$result=mysqli_query($conn,$query);
...
function updateShipmentHeaderDetails(){
	...
	global $message,$shipment_no,$action;
	$message = "Shipment header details was Updated Successfully!";
	$shipment_no = $_POST['shipment_no'];
	$action = $_POST['sub'];
...
function addQty($shipment_num,$unic,$code,$qty,$c_price,$w_price,$r_price,$c_price2,$w_price2,$r_price2){
	...
	$w_price1= preg_replace("/[^0-9.]/",'',$w_price);
	$r_price1= preg_replace("/[^0-9.]/",'',$r_price);
	// $c_price2=$_REQUEST['c_price2'];
	// $w_price2=$_REQUEST['w_price2'];
	// $r_price2=$_REQUEST['r_price2'];
	$unic_cal=unicCal();
	$qty_old=$new_stock_id=$iqt_id='';
	$unic_count=$duplicate=$qty_old=$qty_old_total=$qty0=0;
	$action='show_add_qty';
	...
	if(((($unic0==0)||($unic=='yes'))) &&($proceed)){	
		...
		if($result2){
			...
			$result2=mysqli_query($conn,$query2);
			debugEnd($debug_id,'success');
			if($duplicate>0)$message='Alert: DuplicatededitItem1 Unic ID Detected'; else $message='Inventory was Updated Successfully!';
			return true;
		}else{
			mysqli_query($conn,"DELETE FROM `inventory_shipment` WHERE `id`='$lastid'");
			debugEnd($debug_id,'fail');
...
// added by nirmal 21_08_23
function removeShipmentItemTmp(){
	global $message,$shipment_no;
	$itemid=$_GET['id'];
...
function removeUnicShipmentItemTmp(){
	...
	$query="SELECT `shipment_tmp_no`  FROM shipment_unic_tmp WHERE `inv_item`='$itemid'";
	$result=mysqli_query($conn,$query);
	...
	if(($itemid != '') && ($shipment_no != '')){
		$query="DELETE FROM `shipment_unic_tmp` WHERE `inv_item`='$itemid'";
		$result1=mysqli_query($conn,$query);
	}
...
function getOneItem($sub_system){
	global $unic_cal,$id,$code,$description,$po_description,$w_price,$r_price,$c_price,$min_w_rate,$max_w_rate,$max_r_rate,$itm_def_cost,$itm_def_price,$itm_pr_sr,$drawer,$qty,$commision,$category,$supplier,$stores_id,$itm_status,$itm_unic,$stores_name,$inv_type,$tb_id,$color1;
	$code0=$_REQUEST['code0'];
	$unic_cal=unicCal();
	$w_price=$r_price=$c_price=$drawer=$qty=$qty_new=array();
	$i=0;
	include('config.php');
		$query="SELECT inv.id,inv.`status`,inv.category,inv.code,inv.description,inv.po_description,inv.min_w_rate,inv.max_w_rate,inv.max_r_rate,inv.commision,inv.`unic`,inv.`default_cost`,inv.`default_price`,inv.`pr_sr`,inv.supplier FROM inventory_items inv WHERE `inv`.code='$code0'";
		$row=mysqli_fetch_row(mysqli_query($conn2,$query));
			$id=$row[0];
			$itm_status=$row[1];
			$category=$row[2];
...
function editItem1($sub_system){
	...
	if($out){
		...
		if($out){
				$query="UPDATE `inventory_items` SET `min_w_rate`='$min_w_rate',`max_w_rate`='$max_w_rate',`max_r_rate`='$max_r_rate',`default_cost`='$def_cost',`default_price`='$def_price',`commision`='$commision',`category`='$category',`supplier`='$supplier',`unic`='$item_unic',`status`='$item_status' WHERE `id`='$id'";
				$result1=mysqli_query($conn,$query);
				if($result1){
...
function getShipmentList($systemid,$sub_system){
	global $shipment_no,$salesman,$current_user,$date,$time,$active,$unic,$ship_store,$month,$store0,$shipment_edit_time;
	$user=$_COOKIE['user_id'];
	$month=$_GET['month'];
	$store0='all';
	$qry_st="";
...
function getTmpShipmentPendingList($systemid,$sub_system){
	global $shipment_no_tmp,$salesman_tmp,$current_user_tmp,$date_tmp,$time_tmp,$unic_tmp,$month_tmp;
	$user=$_COOKIE['user_id'];
	$month_tmp=$_GET['month'];
	$shipment_no_tmp=array();

	if($month_tmp!=''){
		include('config.php');

		$query="SELECT sit.`shipment_tmp_no`,sit.`added_by`,date(sit.`time`),time(sit.`time`),smt.`unic` FROM shipment_item_tmp sit LEFT JOIN shipment_main_tmp smt ON sit.`shipment_tmp_no`=smt.`id` WHERE sit.`status`!='2' AND sub_system='$sub_system' AND smt.`shipment_date` LIKE '$month_tmp%' GROUP BY sit.`shipment_tmp_no` ORDER BY sit.`shipment_tmp_no` DESC";
...
// added by nirmal 21_08_17
function oneShipmentTmp($main_shipment_no){
...
function getTags2(){
	global $tag_id,$tag_name,$tag_licount;
	$tag_id=$tag_name=array();
	include('config.php');
	$query="SELECT tn.id,tn.tag,COUNT(ta.id) FROM tag_name tn LEFT JOIN tag_assignment ta ON tn.id=ta.tag GROUP BY tn.id ORDER BY tn.tag";
	$result=mysqli_query($conn,$query);
	while($row=mysqli_fetch_array($result)){
		$tag_id[]=$row[0];
		$tag_name[]=$row[1];
...
function addTag($item_id,$tag_name){
	...
	$msg='Done';
	include('config.php');
	$query="SELECT id,COUNT(id) FROM tag_name WHERE `tag`='$tag_name'";
	$row=mysqli_fetch_row(mysqli_query($conn2,$query));
	$tag_id=$row[0];
	if($row[1]!=1){ $out=false; $msg='Error: Tag was not found'; }
	if($out){
...
function createTag($tag_name){
	...
	if($out){
		$query="INSERT INTO `tag_name` (`tag`) VALUES ('$tag_name')";
		$result=mysqli_query($conn,$query);
...
function updateTag($tag_id,$tag_name){
	$out=true;
	$msg='Done';
	include('config.php');
	$query="SELECT COUNT(id) FROM tag_name WHERE `tag`='$tag_name'";
	...
	if($row[0]!=0){ $out=false; $msg='Error: Tag is already exists'; }
	if($out){
		$query="UPDATE `tag_name` SET `tag`='$tag_name' WHERE id='$tag_id'";
		$result=mysqli_query($conn,$query);
