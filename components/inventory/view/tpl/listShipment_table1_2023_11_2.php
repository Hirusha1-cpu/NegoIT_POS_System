	<div style="display:inline-block; margin-right: 40px">
		...
		<tr><td style="font-size:16pt; font-weight:bold; color:navy" align="center">List Of Pending Shipments</td></tr>
		</table>
		<p align="center" style="font-family:Calibri; font-size:10pt; color:gray">Temporarily saved shipments</p>
			...
			<tr bgcolor="#BBBBBB"><th>Shipment Tmp No</th><th width="100px">Entered Date</th><th width="100px">Placed By</th><th class="shipmentTB4">Store</th></tr>
			</th></tr>
		...
		$inv=0;
			for($i=0;$i<sizeof($shipment_no_tmp);$i++){
				...
				if($edit_ship_tmp[$i]){
					...
					print '<tr bgcolor="#DEDEDE"><td align="left" class="shipmentTB3"><a style="color:'.$color1.'; text-decoration:none;" href="index.php?components=inventory&action='.$sub.'&shipment_no='.$shipment_no_tmp[$i].'">'.str_pad($shipment_no_tmp[$i], 7, "0", STR_PAD_LEFT).'</a>&nbsp;&nbsp;&nbsp;&nbsp;'.$edit.'</td>
					<td width="50px" align="center">'.$date_tmp[$i].'</td>
					<td class="shipmentTB4" >'.ucfirst($salesman_tmp[$i]).'</td>
					<td class="shipmentTB4" >'.$ship_store_tmp[$i].'</td></tr>';
			}
		?>	
		</table>
	</div>
