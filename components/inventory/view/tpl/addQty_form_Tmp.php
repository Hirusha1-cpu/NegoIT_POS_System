...
<script type="text/javascript">
	...
	function editShipmentHeaderTmp(){
		...
		window.location = 'index.php?components=<?php print $components; ?>&action=edit_shipment_header_tmp&sub='+sub+'&shipment_no='+shipment_no;
	}
	...
	function finalyzeOneShipmentTemp(){
		...
		document.getElementById('div_submit').innerHTML=document.getElementById('loading').innerHTML;
		window.location = 'index.php?components=<?php print $components; ?>&action=finalyze_one_shipment_tmp&shipment_no='+shipment_no+'&sub=show_add_qty_tmp';
	}
</script>
<div id="loading" style="display:none"><img src="images/loading.gif" style="width:60px" /></div>
<form action="index.php?components=<?php print $components; ?>&action=add_qty_tmp" method="post" id="add_qty_tmp_from">
