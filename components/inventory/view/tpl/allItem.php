<style type="text/css">
    ...
    .text-cell {
        width: 1%; /* Set a minimal width to avoid extra space */
...
<?php
	...
	$selected_category='';
	if($type==4) $report_title='Repair Item Inventory'; else $report_title='Inventory Report';
...
<table align="center" border="0" style="font-family:Calibri; font-size:11pt" width="80%" bgcolor="#F5F5F5">
	<tr>
		<td colspan="2" align="center" style="font-weight:bold; color:navy;">
			<?php
...
if ($type == 4) {
    ...
    $store_get = $_GET['store'] ?? '';

    print '<td width="70px" rowspan="2"><input style="width:100px; height:30px" type="button" value="Parts List"
        onclick="window.location = \'index.php?components=inventory&action=repair_parts_list&category=' . $category_get . '&store=' . $store_get . '&type=4\'" /></td>';
}
?>
	</tr>
	<tr>
		<td>
			<table width="100%" class="legend-table">
				<tbody>
					<tr>
						<?php
							if($type==5){
								print '<td bgcolor="blue" class="color-cell"></td><td>Items in Unlocked Invoices or Pending/Transferring Transfers</td>';
							}
							if($type==1 || $type==5){
								print '<td bgcolor="orange" class="color-cell"></td><td>Quantity mismatch in unique items</td>';
								print '<td bgcolor="#ddefb1" class="color-cell"></td><td>Same store with different unic prices</td>';
							}
	...
	<tr>
		<td style="font-weight:bold; color:navy;" align="left">
...
<br />
<?php if($type==4){ ?>
		<form action="index.php?components=inventory&action=add_repair_inv&type=<?php print $_GET['type']; ?>&category=<?php print $_GET['category']; ?>&store=<?php print $_GET['store']; ?>"
		method="post">
		<table align="center" border="0" bgcolor="#EEEEEE" style="font-family:Calibri; font-size:11pt">
			<tr>
				...
				<td>
					<input type="text" id="part" name="part" value="" placeholder="Repair Part" /><strong>
					<input type="number" id="qty" name="qty" value="" style="width:50px" placeholder="Qty" /></strong>
	...
	<table align="center" border="0" style="font-family:Calibri">
		...
		<?php
			...
			for ($i = 0; $i < sizeof($id); $i++) {
				...
				if ($type == 3) {
					$link1 = '<a href="index.php?components=inventory&action=show_repair_map&item=' . $id[$i] . '" >';
					$link2 = '</a>';
				} else {
					$link1 = $link2 = '';
				}

				$w_price_i = $w_price[$i] ?? 0;
				$r_price_i = $r_price[$i] ?? 0;
				$cost_i    = $cost[$i] ?? 0;
				$qty_i     = $qty[$i] ?? 0;
				$drawer_i  = $drawer[$i] ?? '';
				$st_name_i = $st_name[$i] ?? '';
				$cat_name_i = $cat_name[$i] ?? '';
				$code_i = $code[$i] ?? '';
				$description_i = $description[$i] ?? '';
				$row_class_i = $rowClass[$i] ?? '';
				$color_i = $color[$i] ?? 'black';

				if (($qtymore0 == 'n') || (($qtymore0 == 'y') && ($qty_i > 0) && (($type == 1) || ($type == 5)))) {
					print '<tr bgcolor="' . $color7 . '" style="color:' . $color_i . '">
							<td align="center" class="' . $row_class_i . '">' . $cat_name_i . '</td>
							<td class="' . $row_class_i . '">&nbsp;' . $code_i . '</td>
							<td class="' . $row_class_i . '">&nbsp;' . $link1 . $description_i . $link2 . '&nbsp;</td>
							<td align="right" class="' . $row_class_i . '">' . number_format($w_price_i, $decimal) . '&nbsp;&nbsp;</td>';

					if ($type == 2 || $type == 3) {
						print '<td align="right" class="' . $row_class_i . '">' . number_format($cost_i, $decimal) . '&nbsp;&nbsp;</td>
								<td align="center" class="' . $row_class_i . '">' . $st_name_i . '</td>
							</tr>';
					} else {
						print '<td align="right" class="' . $row_class_i . '">' . number_format($r_price_i, $decimal) . '&nbsp;&nbsp;</td>
								<td align="right" class="' . $row_class_i . '">' . number_format($cost_i, $decimal) . '&nbsp;&nbsp;</td>
								<td align="center" class="' . $row_class_i . '">' . $drawer_i . '</td>
								<td align="right" class="' . $row_class_i . '">' . number_format($qty_i) . '&nbsp;&nbsp;</td>
								<td align="right" class="' . $row_class_i . '">' . number_format(($cost_i * $qty_i), $decimal) . '&nbsp;&nbsp;</td>
								<td align="center" class="' . $row_class_i . '">' . $st_name_i . '</td>
								</tr>';

						$total_c_price += $cost_i * $qty_i;
					}
				}
			}
			// for ($i = 0; $i < sizeof($id); $i++) {
			...
			// 		print '<tr bgcolor="' . $color7 . '" style="color:' . $color[$i] . '">
			// 				<td align="center" class="'.$rowClass[$i].'">' . $cat_name[$i] . '</td>
			// 				<td class="'.$rowClass[$i].'">&nbsp;' . $code[$i] . '</td>
			// 				<td class="'.$rowClass[$i].'">&nbsp;' . $link1 . $description[$i] . $link2 . '&nbsp;</td>
			// 				<td align="right" class="'.$rowClass[$i].'">' . number_format($w_price[$i], $decimal) . '&nbsp;&nbsp;</td>';
			// 		if ($type == 2 || $type == 3) {
			// 			print '<td align="right" class="'.$rowClass[$i].'">' . number_format($cost[$i], $decimal) . '&nbsp;&nbsp;</td>
			// 					<td align="center" class="'.$rowClass[$i].'">' . $st_name[$i] . '</td>
			// 				</tr>';
			// 		} else {
			// 			print '<td align="right" class="'.$rowClass[$i].'">' . number_format($r_price[$i], $decimal) . '&nbsp;&nbsp;</td>
			// 					<td align="right" class="'.$rowClass[$i].'">' . number_format($cost[$i], $decimal) . '&nbsp;&nbsp;</td>
			// 					<td align="center" class="'.$rowClass[$i].'">' . $drawer[$i] . '</td>
			// 					<td align="right" class="'.$rowClass[$i].'">' . number_format($qty[$i]) . '&nbsp;&nbsp;</td>
			// 					<td align="right" class="'.$rowClass[$i].'">' . number_format(($cost[$i] * $qty[$i]), $decimal) . '&nbsp;&nbsp;</td>
			// 					<td align="center" class="'.$rowClass[$i].'">' . $st_name[$i] . '</td>
			// 					</tr>';
			// 			$total_c_price += $cost[$i] * $qty[$i];
			// 		}
