...
<script type="text/javascript">
	function updateQtyTB($i,$inv_type,$tb_id,$stores_id){
		...
		if($master_pw != ''){
			...
			alert('Please enter Master Password 01');
		}
	}

	function updatePriceOfUnic($i, $itq_id, $item_id, $store_id, $old_w_price, $old_r_price, $old_c_price){
		var $master_pw=document.getElementById('master_pw').value;
		var $drawer=document.getElementById('drawer'+$i).value;
		var $w_price=document.getElementById('w_price'+$i).value;
		var $r_price=document.getElementById('r_price'+$i).value;
		var $c_price=document.getElementById('c_price'+$i).value;
		if($master_pw != ''){
			if (confirm('Are you sure you want to update the prices?')) {
				document.getElementById('div_btn_'+$i).innerHTML=document.getElementById('loading').innerHTML;
				var xhttp = new XMLHttpRequest();
				xhttp.onreadystatechange = function() {
					if (this.readyState == 4 && this.status == 200) {
						try {
							...
							if (response.status === 'success') {
								document.getElementById('div_btn_'+$i).innerHTML = '<span style="color: green;">' + response.message + '</span>';
							} else {
								document.getElementById('div_btn_'+$i).innerHTML = '<span style="color: maroon;">' + response.message + '</span>';
							}
						} catch (e) {
							document.getElementById('div_btn_'+$i).innerHTML = '<span style="color: maroon;">Invalid JSON response</span>';
						}
					}
				};
				xhttp.open("GET", 'index.php?components=inventory&action=edit_unics_price&master_pw='+$master_pw+'&itq_id='+$itq_id+'&item_id='+$item_id+'&store_id='+$store_id+'&old_w_price='+$old_w_price+'&old_r_price='+$old_r_price+'&old_c_price='+$old_c_price+'&new_w_price='+$w_price+'&new_r_price='+$r_price+'&new_c_price='+$c_price+'&drawer='+$drawer, true);
				xhttp.send();
			}
		}else{
			alert('Please enter Master Password 01');
	...
	<table align="center" bgcolor="#E5E5E5">
		...
		<?php if($itm_pr_sr==1){ ?>
			<tr>
				<td></td>
				<td colspan="2">
					<?php if(($unic_cal) && ($itm_unic==1)) { ?>
						<table width="100%" style="margin-bottom: 10px;">
							<tr>
								<td width="20px" bgcolor="wheat"></td>
								<td> - Same store with different prices</td>
							</tr>
						</table>
					<?php } ?>
					<table width="100%" border="1" cellspacing="0">
						<tr>
							<th align="left" class="shipmentTB4">Store</th>
							<th style="background-color:#AAAAAA" class="shipmentTB4">Wholesale<br/>Price</th>
							<th style="background-color:#AAAAAA" class="shipmentTB4">Retail<br />Price</th>
							<th style="background-color:#AAAAAA" class="shipmentTB4">Cost</th>
							<th class="shipmentTB4">Inventory<br/>Quantity</th>
							<th class="shipmentTB4">Drawer No.</th>
							<th class="shipmentTB4"></th>
						</tr>
						<?php
							...
							if(($unic_cal)&& ($itm_unic==1)){
								$style1='disabled="disabled" style="width:50px; background-color:silver; text-align:right;"';
								$style2='disabled="disabled"';

								for($i=0;$i<sizeof($inv_ship_id);$i++){
									print '	<tr><td class="shipmentTB4 '.$rowClass[$i].'">'.$stores_name[$i].'</td>
									<td align="center" class="'.$rowClass[$i].' text"><input type="number" id="w_price'.$i.'" style="text-align: right; width:100px" value="'.$w_price[$i].'" /></td>
									<td align="center" class="'.$rowClass[$i].' text"><input type="number" id="r_price'.$i.'" style="text-align: right; width:100px" value="'.$r_price[$i].'" /></td>
									<td align="center" class="'.$rowClass[$i].' text"><input type="number" id="c_price'.$i.'" style="text-align: right; width:100px" value="'.$c_price[$i].'" /></td>
									<td align="center" class="'.$rowClass[$i].' text"><input type="number" id="qty'.$i.'" value="'.$qty[$i].'" '.$style1.' /></td>
									<td align="center" class="'.$rowClass[$i].' text"><input type="text" id="drawer'.$i.'" style="text-align: left; width:50px;" value="'.$drawer[$i].'"/></td>
									<td align="center" class="'.$rowClass[$i].' text"><div id="div_btn_'.$i.'"><input type="button" value="Update" onclick="updatePriceOfUnic('."'$i','$itq_id[$i]','$item_id[$i]','$store_id[$i]','$w_price[$i]','$r_price[$i]','$c_price[$i]'".')" /></div></td>
									</tr>';
								}
							}else{
								for($i=0;$i<sizeof($stores_id);$i++){
									...
									print '	<tr style="background-color:'.$color1[$i].'" class="shipmentTB4"><td>'.$stores_name[$i].'</td>
									<td style="background-color:#AAAAAA" align="center"><input type="number" id="w_price'.$i.'" style="width:70px" value="'.$w_price[$i].'" '.$style3.' /></td>
									<td style="background-color:#AAAAAA" align="center"><input type="number" id="r_price'.$i.'" style="width:70px" value="'.$r_price[$i].'" '.$style3.' /></td>
									<td style="background-color:#AAAAAA" align="center"><input type="number" id="c_price'.$i.'" style="width:70px" value="'.$c_price[$i].'" '.$style3.' /></td>
			...
			<tr>
				<td></td>
				<td style="font-size:8pt; color:red;"><b>Important : to update <em>quantity or price</em>, please enter Master Password (01)</b></td>
				<td>
					<input type="password" id="master_pw" value="" placeholder="Type Master Password 01" style="width:96%;"/>
				</td>
				<td></td>
			</tr>
			<tr>
