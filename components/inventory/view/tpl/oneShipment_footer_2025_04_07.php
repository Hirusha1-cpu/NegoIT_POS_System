	...
	<table align="center" bgcolor="#E5E5E5" border="1" cellspacing="0" style="font-family:Calibri">
		...
		<?php
			...
			for($i=0;$i<sizeof($ins_id);$i++){
				if($itm_unic[$i]==1) $url='index.php?components='.$components.'&action=show_unic&shipment_no='.$_GET['shipment_no'].'&ins_id='.$ins_id[$i]; else $url='';
				$totalcost=0;
				if($unicCal){
					$unitcost='-';
				...
				print '<tr>
					...
					<td align="right" class="shipmentTB3">'.$unitcost.'</td>
					<td align="right" class="shipmentTB3">'.$totalcost.'</td>
				</tr>';
			}
			print '<tr>
