...
function updateShipment($id){
	...
	window.location = 'index.php?components=<?php print $components; ?>&action=shipment_item_update_tmp&id='+$id+'&qty_new='+qty_new;
}

function removeShipment($id){
	document.getElementById('div-update'+$id).innerHTML = document.getElementById('loading').innerHTML
	window.location = 'index.php?components=<?php print $components; ?>&action=shipment_qty_item_remove_tmp&id='+$id;
}
</script>
		...
		<?php
			for($i=0;$i<sizeof($ship_inv_id);$i++){
				print '<tr style="font-size:12pt">
						<td width="30px" style="color:blue"><strong>'.($i+1).'</strong></td>
						<td>'.$ship_itm_desc[$i].'</td><td width="5px"></td>
						<td align="center">
							<div class="st-div" style="background-color:'.$ship_itm_st_color[$i].'">'.$ship_itm_st_name[$i].'</div>
						</td>
						<td width="5px"></td>
						<td align="right">';
