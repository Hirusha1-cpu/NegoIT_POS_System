...
<script>
    ...
    function generateBarcode(){
        ...
        store_text_fs = document.getElementById('store_text_fs').value;
        store_text_color = document.getElementById('store_text_color').value;
        out=true;
        if((out) && (line_color != '')){
        ...
        if ((out) && (!isNaN(store_text_fs)) && (store_text_fs % 1 !== 0)) {
            out=false;
            alert('Store Name font size cannot be a decimal/float number');
        }
        if((out) && (store_text_color != '')){
            if(!isValidHexColor(store_text_color)){
                out=false;
                alert('Store text color must be a valid hex color code.');
            }else{
                store_text_color = encodeURIComponent(store_text_color);
            }
        }
        ...
        const checkbox1 = document.getElementById('print_store').checked;
        print_store = 'false';
        if(checkbox1){
            print_store = 'true';
        }
        if(out){
            url = 'index.php?components=inventory&action=barcode_print&code='+code+'&q='+quantity+'&w='+width+'&h='+height+'&color='+line_color+'&print_val='+print_val+'&ml='+margin_left+'&mr='+margin_right+'&mt='+margin_top+'&mb='+margin_bottom+'&fs='+font_size+'&print_store='+print_store+'&store_text_fs='+store_text_fs+'&store_text_color='+store_text_color;
...
<?php if(isset($id) && $id != ''){ ?>
    ...
    <table align="center" border="0" style="margin-top:20px; font-family:Calibri;">
        <tr  bgcolor="#CCCCCC">
            ...
            <td width="80px" align="center" style="">Print Store</td>
            <td width="60px" align="center" style="">Store Name Font Size</td>
            <td width="120px" align="center" style="">Store Name Color (Hex)</td>
        ...
        <tr>
            ...
            <td align="right"><input type="checkbox" id="print_store" class="input" checked/></td>
            <td align="right"><input type="number" value="10" id="store_text_fs" class="input" placeholder=""/></td>
            <td align="right"><input type="text" id="store_text_color" class="input"/></td>
