...
if(passwordExpire()) header('Location:
index.php?components=authenticate&action=change_pw&message=Your%20Password%20Has%20Expired.%20Please%20Change%20it%20NOW&re=fail');
if(!(($systemid!=1)||($systemid==1 && $sub_system==0 )||($systemid==1 && $sub_system==1 )) &&
!(($action=='show_all_item')||($action=='export_shipment')||($action=='export_unic_list'))){ header('Location:
index.php?components=inventory&action=show_all_item&category=1&store='.$_COOKIE['store'].'&type=5'); }else{
if(($systemid==1 && ($sub_system!=0 && $sub_system!=1)) && $action=='show_all_item' &&
($_GET['store']!=$_COOKIE['store'])) header('Location:
index.php?components=inventory&action=show_all_item&category=1&store='.$_COOKIE['store'].'&type=5');
}
switch ($action){
   case "show_add_item" :
  ...
  break;
   ...
   case "edit_item1" :
      ...
      if(editItem1($sub_system))
      		header('Location: index.php?components=inventory&action=show_one_item&code0='.$code.'&message='.$message.'&re=success');
      	else
