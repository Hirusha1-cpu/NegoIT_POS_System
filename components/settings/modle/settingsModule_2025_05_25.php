            ...
            if (isQuickBooksActive(1)) {
                ...
                if ($qb_id != '') {
                    ...
                    if ($qb_result['status'] == 'success') {
                        ...
                        throw new Exception("QuickBooks error for employee '$username': " . $qb_result['message']);
                    }
                } else {
...
function addCategory(){
	...
	if(($category=='')||($sub_sys=='')){
		$out=false; $message='Category name and sub system must be filled!';
	}
	...
	if($out){
		...
		$row = mysqli_fetch_row($result);
		if($row[0]>0){
			$out=false; $message='This category is already exist!';
		}
	}
	...
	if($out){
		...
		if($result){
			...
			$out=false; $message='Category could not be added!';
		}
	}
	return $out;
}
...
function updateTime(){
	...
	if($update){
		...
		if((0<$time_to)&&($time_to<24)) $update=true; else $update=false;
