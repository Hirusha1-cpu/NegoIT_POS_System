...
function getUsers()
{
	...
	$query = "SELECT up.id, up.username, up.store, up.device_restrict, up.time_restrict, up.mobile_rep, up.`status`, up.`sub_system`, ss.name, up.mapped_inventory
	FROM `userprofile` up INNER JOIN sub_system ss ON up.`sub_system` = ss.id $user_qry
	ORDER BY up.`sub_system`, up.username";
	$result = mysqli_query($conn, $query);
	while ($row2 = mysqli_fetch_array($result)) {
		$uprof_id[] = $row2[0];
		$uprof_name[] = $row2[1];
...
// updated by nirmal 04_02_2025 added more edit fields
function getOneUser()
{
	...
	$bank_id, $bank_name, $uprof_bank_account_no, $usr_branch_code, $uprof_branch_name, $usr_employee_no, $usr_employee_designation, $usr_bank_id;
	$usr_id1 = $_REQUEST['id'];

	include('config.php');

	$query = "SELECT up.`username`, up.`emp_name`, up.`nic`, up.`mobile`, bk.`name`, up.`bank_branch`, up.`bank_ac`, up.`status`, up.`direct_mkt`,
	up.`branch_code`, up.`employee_no`, up.`designation`, up.`bank_id`
	FROM userprofile up LEFT JOIN bank bk ON up.bank_id=bk.id WHERE up.id='$usr_id1'";
	$result = mysqli_query($conn, $query);
	while ($row = mysqli_fetch_array($result)) {
		$usr_uname1 = $row[0];
		$usr_emp_name = $row[1];
		...
		$usr_bank_ac = $row[6];
		$usr_status1 = $row[7];
		$usr_branch_code = $row[9];
		$usr_employee_no = $row[10];
		$usr_employee_designation = $row[11];
		$usr_bank_id = $row[12];
		if ($row[8] == 1)
			$usr_direct_mkt = 'checked="checked"';
		else
			$usr_direct_mkt = '';
	}
	$query = "SELECT `id`, `name` FROM bank WHERE `status`=1";
...
function addUser(){
	...
	if($id_count==0){
		$query="INSERT INTO `userprofile` (`username`,`password`,`change_pw`,`direct_mkt`,`emp_name`,`nic`,`mobile`,`bank_id`,`bank_branch`,`bank_ac`,`sub_system`,`status`)
		VALUES ('$username','$password','$change_pw','$direct_mkt','$emp_name','$nic','$mobile','$bank_id','$bank_branch','$bank_ac','$sub_system','0')";
		$result=mysqli_query($conn,$query);
		if($result){
...
// update by nirmal 11_09_2024 added quickbooks integration
// updated by nirmal 04_02_2025 (added new user data filed update logics)
function addUser()
{
	global $message;
	$out = true;
	...
	if (isset($_POST['change_pw']) && $_POST['change_pw'] != "")
		$change_pw = 1;
	else
		$change_pw = 0;
	if (isset($_POST['direct_mkt']) && $_POST['direct_mkt'] != "")
		$direct_mkt = 1;
	else
		$direct_mkt = 0;
	...
	try {
		...
		// Get bank ID
		$query = "SELECT id FROM bank WHERE `id`='$bank'";
		$row = mysqli_fetch_row(mysqli_query($conn, $query));
		$bank_id = $row[0];
		if ($bank_id == '')
			$bank_id = 0;
		...
		if ($id_count == 0) {
            ...
            (`username`, `password`, `change_pw`, `direct_mkt`, `emp_name`, `nic`, `mobile`, `bank_id`, `bank_branch`, `bank_ac`, `sub_system`, `status`, `branch_code`, `employee_no`, `designation`)
            VALUES
            ('$username', '$password', '$change_pw', '$direct_mkt', '$emp_name', '$nic', '$mobile', '$bank_id', '$bank_branch', '$bank_ac', '$sub_system', '0', '$branch_code', '$employee_no', '$designation')";
			$result = mysqli_query($conn, $query);
			...
			if ($result) {
				...
				if (isQuickBooksActive(1)) {
					$employeeArray = [
						'given_name' => $username,
						'family_name' => $emp_name,
						'mobile' => $mobile
					];

					$qb_result = QBAddEmployee($employeeArray);
					if ($qb_result['status'] == 'success') {
						$messages[] = $qb_result['message'];
						$qb_employee_id = $qb_result['qb_employee_id'];
						$query = "UPDATE `userprofile` SET `qb_id` = '$qb_employee_id' WHERE id = '$lastid'";
						$result = mysqli_query($conn, $query);
					} else {
						$out = false;
						$message = "QuickBooks error for employee $username: " . $qb_result['message'];
						$messages[] = $message;
				...
				throw new Exception('Error: User could not be added to the database.');
			}
		} else {
			throw new Exception('Error: This user already exists.');
		}
	} catch (Exception $th) {
...
function updateUser()
{
	...
	try {
		...
		if ($id_count == 0) {
			...
			// If QuickBooks is active, update the employee record in QuickBooks
			if (isQuickBooksActive(1)) {
				$query = "SELECT `qb_id`, `username`, `emp_name`, `mobile` FROM `userprofile` WHERE `id`='$user_id'";
				$result = mysqli_query($conn, $query);
				$row = mysqli_fetch_row($result);
				$qb_id = $row[0];
				$username = $row[1];
				...
				if ($qb_id != '') {
					...
					$qb_result = QBUpdateEmployee($employeeArray);
					if ($qb_result['status'] == 'success') {
						$messages[] = $qb_result['message'];
						$result = mysqli_query($conn, $query);
					} else {
						throw new Exception(
							"QuickBooks error for employee " .
							urlencode($username) .
							": " .
							$qb_result['message']
						);
					...
					throw new Exception('Error: User is not registered in QuickBooks!');
				}
			}
	...
	// Combine QuickBooks error messages with the main message
	if (!empty($messages)) {
		$message .= ' ' . implode(' ', $messages);
	}
...
function updateStoreaso()
{
	...
	$store = $_GET['store'];
	if ($store == 0)
		$message = 'User was Removed from All Stores!';
...
function updateMapInv()
{
	...
	if ($mapped_inventory == 0)
		$message = 'User was Set to Default Inventory!';
...
function addCategory()
{
	...
	$disable_in_bill = 1;
	if (isset($_POST["disable_in_bill"])) {
		$disable_in_bill = $_POST["disable_in_bill"];
		if ($disable_in_bill != 0) {
			$disable_in_bill = 1;
		}
	}
	$out = true;
	...
	if (($category == '') || ($sub_sys == '')) {
		$out = false;
		$message = 'Category name and sub system must be filled!';
	}
	...
	if ($out) {
		$query = "INSERT INTO `item_category` (`name`,`sub_system`,`status`) VALUES ('$category','$sub_sys','$disable_in_bill')";
		$result = mysqli_query($conn, $query);
		if ($result) {
...
function updateTime()
{
	...
	if (is_numeric($time_to))
		...
		$update = false;
	if ($update) {
		if ((0 < $time_from) && ($time_from < 24))
			$update = true;
		else
			$update = false;
		if ((0 < $time_to) && ($time_to < 24))
			$update = true;
		else
			$update = false;
...
function addGroupAllocation()
{
	...
	if (($user_id == '') || ($group_id == '')) {
		$message = 'Please Select Both a User and a Group';
		$out = false;
	}
	...
	if ($out) {
		...
		$result = mysqli_query($conn, $query);
		if (!$result) {
			$message = 'Error: The Group Could Not be Allocated';
			$out = false;
		}
	}
	return $out;
...
function removeGroupAllocation()
{
	...
	if ($out) {
		...
		$result = mysqli_query($conn, $query);
		if (!$result) {
			$message = 'Error: The Group Allocation Could Not be Removed';
			$out = false;
		}
	}
	return $out;
