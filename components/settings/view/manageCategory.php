...
<form action="index.php?components=settings&action=add_category" method="post" onsubmit="return validateAddCategory()" >
	<table align="center" bgcolor="#E5E5E5" style="font-family:Calibri; border-radius: 15px; font-size:12pt;">
		<tr>
			<td colspan="4">
				<?php
		...
		<tr>
			...
			<td width="50px"></td>
			<td>Disable in Bill</td>
			<td>
				<input type="checkbox" id="disable_in_bill" name="disable_in_bill" value="0" />
			</td>
			<td width="50px"></td>
		</tr>
		<tr>
			<td colspan="9" align="center"><br />
				<input type="submit" value="Add Category" style="width:130px; height:50px" /><br /><br />
			</td>
		</tr>
	</table>
</form>
...
<table align="center" bgcolor="#EFEFEF" border="0" style="font-family:Calibri">
	...
	<?php
		for($i=0;$i<sizeof($category_id);$i++){
			...
			$disabled = '';
			if($disable_in_bill[$i]==0){
				$disabled = 'Disabled';
			}
			print '<tr style="background-color:'.$color.'">
					<td style="padding-left:20px; padding-right:20px;">'.$category_name[$i].'</td>
					<td style="padding-left:20px; padding-right:20px;">'.$category_sub[$i].'</td>
					<td style="padding-left:20px; padding-right:20px;">'.$disabled.'</td>
					<td><input type="button" onclick="window.location = '."'".'index.php?components=settings&action=delete_category&id='.$category_id[$i]."'".'" value="Delete" />
					</td>
				</tr>';
		}
	?>
