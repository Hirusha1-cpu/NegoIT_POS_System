	...
	<form autocomplete="off" action="index.php?components=<?php print $components; ?>&action=add_user" onsubmit="return validateUser()" method="post" >
		...
		<table align="center" bgcolor="#E5E5E5" style="font-size:12pt; font-family:Calibri; font-weight:bold; border-radius: 15px;">
			...
			<tr>
				...
				<td>NIC</td>
				<td>
					<input type="text" name="nic" id="nic" /></td><td width="50px">
				</td>
			</tr>
			<!-- Username, Employee Bank -->
			<tr>
				<td width="50px"></td>
				<td>Username</td>
				<td>
			...
			<!-- Passowrd, Bank Branch -->
			<tr>
				<td width="50px"></td>
				<td>Passowrd</td>
				<td><input type="password" id="user_pass1" autocomplete="off" /></td>
				<td width="100px"></td>
				<td>Bank Branch</td>
				<td><input type="text" name="bank_branch" id="bank_branch" />
				<td width="50px"></td>
			</tr>
			<!-- Confirm Password, Bank AC -->
			<tr>
				<td width="50px"></td>
				<td>Confirm Password</td>
				<td><input type="password" id="user_pass2" autocomplete="off" /></td>
				<td width="100px"></td>
				<td>Bank AC</td>
				<td><input type="text" name="bank_ac" id="bank_ac" /><td width="50px"></td>
			</tr>
			<!-- Change Password on Next Login, Mobile No  -->
			<tr>
				<td width="50px"></td>
				<td colspan="2">Change Password on Next Login &nbsp;<input type="checkbox" name="change_pw" checked="checked" value="y"/></td>
				<td width="100px"></td>
				<td>Mobile No</td>
			...
			<?php if($systemid==13){ ?>
				<tr>
					<td width="50px"></td>
					<td colspan="2">Direct Marketing &nbsp;<input type="checkbox" name="direct_mkt" value="y"/></td>
					<td width="100px"></td>
					<td></td>
					<td></td>
				</tr>
...
<?php } if($_REQUEST['action']=='edit_user'){ ?>
	<form action="index.php?components=<?php print $components; ?>&action=update_user" onsubmit="return validateUser()" method="post" >
		...
		<tr>
			<td width="50px"></td><td>Employee Name</td><td><input type="text" name="emp_name" id="emp_name" value="<?php print $usr_emp_name; ?>" /></td>
			<td width="100px"></td><td>NIC</td><td><input type="text" name="nic" id="nic" value="<?php print $usr_nic; ?>" /></td><td width="50px"></td>
		</tr>
		<tr>
			<td width="50px"></td><td>Username</td><td><input type="text" name="user_name" id="user_name" value="<?php print $usr_uname1; ?>" /></td>
			<td width="100px"></td><td>Employee Bank</td><td><input type="text" name="bank_name" id="bank_name" value="<?php print $usr_bank_name; ?>" /><td width="50px"></td>
		</tr>
		<tr>
			<td width="50px"></td><td>Passowrd</td><td><input type="password" name="user_pass1" id="user_pass1" /></td>
			<td width="100px"></td><td>Bank Branch</td><td><input type="text" name="bank_branch" id="bank_branch" value="<?php print $usr_bank_branch; ?>" /><td width="50px"></td>
		</tr>
		<tr>
			<td width="50px"></td><td>Confirm Password</td><td><input type="password" name="user_pass2" id="user_pass2" /></td>
			<td width="100px"></td><td>Bank AC</td><td><input type="text" name="bank_ac" id="bank_ac" value="<?php print $usr_bank_ac; ?>" /><td width="50px"></td>
		</tr>
		<tr>
			<td width="50px"></td><td colspan="2">Change Password on Next Login &nbsp;<input type="checkbox" name="change_pw" checked="checked" value="y" /></td>
			<td width="100px"></td><td>Mobile No</td><td><input type="text" name="mobile" id="mobile" value="<?php print $usr_mobile; ?>" /><td width="50px"></td>
		</tr>
		<?php if($systemid==13){ ?>
			<tr>
				<td width="50px"></td><td colspan="2">Direct Marketing &nbsp;<input type="checkbox" name="direct_mkt" value="y" <?php print $usr_direct_mkt; ?> /></td>
				<td width="100px"></td><td></td><td><td width="50px"></td>
			</tr>
		<?php } ?>
		<tr>
			<td colspan="7" align="center">
				...
				<?php if($usr_status1==0){ ?>
					<input type="submit" value="Update Employee" style="width:150px; height:50px" /><input type="button" onclick="disableUser(<?php print $usr_id1; ?>)" value="Deactivate" style="width:80px; height:50px; background-color:maroon; color:white; font-weight:bold" />
				<?php } ?>
				<?php if($usr_status1==1){ ?>
					<input type="button" onclick="enableUser(<?php print $usr_id1; ?>)" value="Activate" style="width:80px; height:50px; background-color:green; color:white; font-weight:bold" />
				<?php } ?>
				<br />
			</td>
		</tr>
...
<div class="headerscrol" id="myHeader"  >
	<table id="myHeaderTB" align="center" style="font-family:Calibri; font-size:11pt;" border="1" bordercolor="white">
		<tr bgcolor="#467898" style="color:white; height:130px">
			<th width="150px"></th>
			<th class="rotate"><div>Device Check</div></th>
			<th class="rotate"><div>Time Check</div></th>
			<th class="rotate"><div>Mobile Rep</div></th>
			<?php
				$list_sub_system=0;
			...
			for($i=0;$i<sizeof($uprof_id);$i++){
				if(($_GET['action']=='manage_user')||(($_GET['action']=='edit_user')&&($uprof_id[$i]==$usr_id1))){
					...
					for($j=0;$j<sizeof($function_id);$j++){
						...
						if($emptybox) print '<td align="center" bgcolor="'.$color3.'" class="rotate"><input style="cursor:pointer" '.$disable.' type="checkbox" id="'.$uprof_id[$i].'|'.$function_id[$j].'" onchange="updatePermission('."'".$uprof_id[$i]."','".$function_id[$j]."','".'update_permission'."'".')" /></td>';
					}
