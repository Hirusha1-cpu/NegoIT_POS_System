...
{
    try {
        // Get the start date from the request or default to the first day of the current month
        $startDate = isset($_REQUEST['start_date']) && !empty($_REQUEST['start_date'])
            ? $_REQUEST['start_date']
            : date("Y-m-01");

        // Get the end date from the request or default to the current date
        $endDate = isset($_REQUEST['end_date']) && !empty($_REQUEST['end_date'])
            ? $_REQUEST['end_date']
            : date("Y-m-d");

        // Validate the date format
        if (!validateDateQB($startDate) || !validateDateQB($endDate)) {
        ...
        // Provide default dates to ensure the program continues
        return [date("Y-m-01"), date("Y-m-d")];
    }
}

function validateDateQB($date, $format = 'Y-m-d')
{
    $d = DateTime::createFromFormat($format, $date);
    return $d && $d->format($format) === $date;
}

function getQBRegisteredSubsystems()
{
    global $sub_system_name, $sub_system_qb_id, $sub_system_id;
    $sub_system_name = $sub_system_qb_id = $sub_system_id = array();
    include('config.php');

    $subSystemQuery = "SELECT `id`, `name`, `qb_id` FROM `sub_system` WHERE `qb_id` IS NOT NULL";
    $subSystemResult = mysqli_query($conn2, $subSystemQuery);
    if ($subSystemResult) {
        if (mysqli_num_rows($subSystemResult) > 0) {
            while ($subSystemRow = mysqli_fetch_assoc($subSystemResult)) {
                $sub_system_name[] = htmlspecialchars($subSystemRow['name'], ENT_QUOTES, 'UTF-8');
                $sub_system_qb_id[] = $subSystemRow['qb_id'];
                $sub_system_id[] = $subSystemRow['id'];
            }
        }
    }
}
...
function getProfitAndLossReport()
{
    ...
    if (isset($_COOKIE['sub_system'])) {
        ...
        $message = "Sub system is not available to query";
        $out = false;
        return $out;  // Exit early
    }

    $subSystem = '';  // Default for top managers (all sub systems)
    ...
    if (isset($_COOKIE['top_manager']) && !empty($_COOKIE['top_manager'])) {
        // Top manager: Allow filtering
        if (isset($_REQUEST['department'])) {
            $subSystem = $_REQUEST['department'];  // Use selected department
            // Optionally, validate $subSystem here if needed
        }
        // If no department, $subSystem remains '', meaning all
    } else {
        // Non-top manager: Only their sub system
        $subSystemQuery = "SELECT `qb_id` FROM `sub_system` WHERE `id` = $sub_system";
        $subSystemResult = mysqli_query($conn2, $subSystemQuery);
        if ($subSystemResult && mysqli_num_rows($subSystemResult) > 0) {
            $subSystemRow = mysqli_fetch_assoc($subSystemResult);
            $subSystem = $subSystemRow['qb_id'];
        } else {
            $message = "Error: Could not retrieve sub system.";
            $out = false;
            return $out;
        }
    }

    $result = QBgetProfitAndLoss($startDate, $endDate, $subSystem);
    if ($result['status'] == 'error') {
        $message = htmlspecialchars($result['message'], ENT_QUOTES, 'UTF-8');
        $out = false;
    }
...
function getTrialBalanceReport()
{
    ...
    if (isset($_REQUEST['department'])) {
        $subSystem = (int) $_REQUEST['department'];
    } else {
        $subSystem = '';
    }


    $result = QBgetTrialBalance($startDate, $endDate, $subSystem);
    if ($result['status'] == 'error') {
        $message = htmlspecialchars($result['message'], ENT_QUOTES, 'UTF-8');
        $out = false;
    }
...
function getBalanceSheetReport()
{
    ...
    $decimal = getDecimalPlaces(1);
    $out = true;
    $sub_system = $_COOKIE['sub_system'] ? $_COOKIE['sub_system'] : null;
    $sub_system = '';
    include('config.php');
    ...
    if (isset($_COOKIE['top_manager']) && !empty($_COOKIE['top_manager'])) {
        ...
        if ($subSystemResult && mysqli_num_rows($subSystemResult) > 0) {
            ...
            $message = "Error: Could not retrieve sub system.";
            $out = false;
            return $out;  // Exit if query fails
        }
    }

    $result = QBgetBalanceSheet($startDate, $endDate, $subSystem);
    if ($result['status'] == 'error') {
        $message = htmlspecialchars($result['message'], ENT_QUOTES, 'UTF-8');
        $out = false;
    }
...
{
    global $decimal, $startDate, $endDate, $out, $message, $departmentReport;
    $decimal = getDecimalPlaces(1);
    $out = true;
    ...
    $result = QBgetSalesByDepartmentReport($startDate, $endDate, 7);
    if ($result['status'] == 'error') {
        $message = htmlspecialchars($result['message'], ENT_QUOTES, 'UTF-8');
        $out = false;
    }
...
{
    include('config.php');
    global $accountList; // Assuming $db is your database connection
    $query = "SELECT qb_account_id, name FROM accounts WHERE status = 1"; // Adjust query as needed
    $result = mysqli_query($conn2, $query);

    $accountList = [];
    while ($row = mysqli_fetch_assoc($result)) {
        $accountList[$row['qb_account_id']] = $row['name'];
    }
...
{
    include('config.php');
    global $departmentList; // Assuming $db is your database connection
    $query = "SELECT id, qb_id, name FROM stores WHERE status = 1"; // Adjust query as needed
    $result = mysqli_query($conn2, $query);

    $departmentList = [];
    while ($row = mysqli_fetch_assoc($result)) {
        // Store both id and qb_id in the departmentList
        $departmentList[$row['qb_id']] = [
            'id' => $row['id'], // Internal store ID
...
function getJournalEntriesByDepartments()
{
    include('config.php');
    global $entries, $startDate, $endDate, $selectedStore, $currentPage, $maxResults, $totalEntries, $totalPages;

    // Default number of results per page
    $recordsPerPage = isset($_REQUEST['records_per_page']) ? $_REQUEST['records_per_page'] : 50;

    // Handle "All Records" option
    if ($recordsPerPage === 'All Records') {
        $maxResults = null; // No limit on the number of records
        $currentPage = 1; // Ignore pagination
    } else {
        $maxResults = (int) $recordsPerPage;
    ...
    $startPosition = ($maxResults && $currentPage > 1) ? (($currentPage - 1) * $maxResults) + 1 : 1;

    // Determine the selected store
    if (isset($_COOKIE['top_manager']) && !empty($_COOKIE['top_manager'])) {
        // Top manager can see all stores
        $selectedStore = isset($_REQUEST['store']) ? $_REQUEST['store'] : null;
    } else {
        // Non-top manager can only see their assigned store
        if (isset($_COOKIE['store']) && !empty($_COOKIE['store'])) {
            $selectedStore = $_COOKIE['store']; // Enforce restriction to assigned store
            $query = "SELECT qb_id, name FROM stores WHERE status = 1 AND id = '$selectedStore'"; // Adjust query as needed
            $row = mysqli_fetch_row(mysqli_query($conn2, $query));
            $selectedStore = $row[0];
        } else {
            $selectedStore = null; // No store assigned
        }
    }

    // Determine the date range
    $startDate = isset($_REQUEST['start_date']) ? $_REQUEST['start_date'] : date("Y-m-01");
    $endDate = isset($_REQUEST['end_date']) ? $_REQUEST['end_date'] : date("Y-m-d");

    // Get the total number of filtered journal entries
    $totalEntries = getTotalJournalEntries($selectedStore, $startDate, $endDate);

    // Calculate the total number of pages (only if pagination is enabled)
    $totalPages = $maxResults ? ceil($totalEntries / $maxResults) : 1;

    // Get journal entries with pagination or fetch all records
    $entries = getJournalEntriesGroupedByDepartment($selectedStore, $startDate, $endDate, $startPosition, $maxResults);
}
...
function getJournalReport()
{
    ...
    if (isset($_REQUEST['department'])) {
        $subSystem = $_REQUEST['department'];
    } else {
        $subSystem = '';
    }

    $result = QBgetJournalReport($startDate, $endDate, $subSystem);
    if ($result['status'] == 'error') {
        $message = htmlspecialchars($result['message'], ENT_QUOTES, 'UTF-8');
        $out = false;
    }
    ...
    if ($out) {
        foreach ($sortedAccounts as $account) {
            ...
            if ($out) {
                ...
                $checkQuery = "SELECT id, qb_account_id FROM accounts WHERE `name`='$name'";
                ...
                if (mysqli_num_rows($checkResult) == 0) {
                    ...
                    if ($row['qb_account_id'] !== $qb_account_id) {
                        $updateQuery = " UPDATE `accounts` SET qb_account_id = '$qb_account_id', qb_balance = '$currentBalance', qb_status=1 WHERE id = '$accountId'";
                        if (!mysqli_query($conn, $updateQuery)) {
                            $errorMessages[] = "Failed to update account '$name': " . mysqli_error($conn);
                            $out = false;
                        }
                    }
                }
...
function getAccountActivity()
{
    ...
    if (isset($_COOKIE['top_manager']) && !empty($_COOKIE['top_manager'])) {
        ...
        if (isset($_REQUEST['department']) && !empty($_REQUEST['department'])) {
            ...
            $subSystem = '';  // Default to all sub systems
        }
    } else {
        // Non-top manager: Only their sub system
        $subSystemQuery = "SELECT `qb_id` FROM `sub_system` WHERE `id` = $subSystem";
        $subSystemResult = mysqli_query($conn2, $subSystemQuery);
        if ($subSystemResult && mysqli_num_rows($subSystemResult) > 0) {
            $subSystemRow = mysqli_fetch_assoc($subSystemResult);
            $subSystem = $subSystemRow['qb_id'];
    ...
    if (empty($accountId) || !ctype_digit($accountId) || intval($accountId) <= 0) {
    ...
    $response = QBGetAccountGeneralLedger($startDate, $endDate, $accountId, $subSystem);
...
// Fetch counts for registered and unregistered customers
function getCustomerCounts()
{
    include('config.php');
    global $customerCounts;
    ...
    $query = "SELECT COUNT(*) AS total FROM `cust`";
    $result = mysqli_query($conn2, $query);
    if ($result) {
        $row = mysqli_fetch_assoc($result);
        $customerCounts['total'] = $row['total'];
    }

    // Query to get registered customers
    $query = "SELECT COUNT(*) AS registered FROM `cust` WHERE `qb_cust_id` IS NOT NULL";
    $result = mysqli_query($conn2, $query);
    if ($result) {
        $row = mysqli_fetch_assoc($result);
        $customerCounts['registered'] = $row['registered'];
    }

    // Calculate unregistered customers
    $customerCounts['unregistered'] = $customerCounts['total'] - $customerCounts['registered'];
}

// Fetch counts for registered and unregistered employees
function getEmployeeCounts()
{
    include('config.php');
    global $employeeCounts;
    ...
    $query = "SELECT COUNT(*) AS total FROM `userprofile` WHERE `status` = '0'";
    $result = mysqli_query($conn2, $query);
    if ($result) {
        $row = mysqli_fetch_assoc($result);
        $employeeCounts['total'] = $row['total'];
    }

    // Query to get registered employees
    $query = "SELECT COUNT(*) AS registered FROM `userprofile` WHERE `qb_id` IS NOT NULL AND `status` = '0'";
    $result = mysqli_query($conn2, $query);
    if ($result) {
        $row = mysqli_fetch_assoc($result);
        $employeeCounts['registered'] = $row['registered'];
    }

    // Calculate unregistered employees
    $employeeCounts['unregistered'] = $employeeCounts['total'] - $employeeCounts['registered'];
}

function getEmployees()
{
    global $employees, $message;
    $response = QBGetAllEmployees();
    $employees = [];
