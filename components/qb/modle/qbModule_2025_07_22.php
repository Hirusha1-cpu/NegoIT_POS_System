...
function getProfitAndLossReport()
{
    ...
    if (isset($_COOKIE['top_manager']) && !empty($_COOKIE['top_manager'])) {
        ...
        $subSystemQuery = "SELECT `qb_id` FROM `sub_system` WHERE `id` = ?";
        $stmt = mysqli_prepare($conn, $subSystemQuery);
        if ($stmt) {
            mysqli_stmt_bind_param($stmt, "i", $sub_system);
            mysqli_stmt_execute($stmt);
            $subSystemResult = mysqli_stmt_get_result($stmt);
