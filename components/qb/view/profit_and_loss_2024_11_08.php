        ...
        <table class="styled-table" align="center" border="0">
            ...
            <tbody>
                ...
                // var_dump($profitAndLossReport);
                    ...
                    // function displayRow($section, $row, &$rowIndex, $decimal)
                    // {
                    ...
                    //         echo '<td>' . htmlspecialchars(formatSectionName($section), ENT_QUOTES, 'UTF-8') . '</td>';
                    //         echo '<td>' . htmlspecialchars(isset($row->ColData[0]->value) ? $row->ColData[0]->value : '', ENT_QUOTES, 'UTF-8') . '</td>';
                    //         echo '<td align="right">' . htmlspecialchars(number_format((float) $row->ColData[1]->value, $decimal), ENT_QUOTES, 'UTF-8') . '</td>';
                    //         echo '</tr>';
                    //     }
                    // }
                    ...
                    //             echo '<tr><td colspan="4"><strong>' . htmlspecialchars($section->Header->ColData[0]->value, ENT_QUOTES, 'UTF-8') . '</strong></td></tr>';
                    //         }

                    //         if (isset($section->Rows->Row) && is_array($section->Rows->Row)) {
                    //             foreach ($section->Rows->Row as $row) {
                    //                 displayRow($section->group, $row, $rowIndex, $decimal);
                    //             }
                    //         }

                    //         if (isset($section->Summary)) {
                    //             // Check if the summary has a total value, otherwise skip the row
                    //             if (isset($section->Summary->ColData[1]->value) && (float) $section->Summary->ColData[1]->value != 0) {
                    //                 echo '<tr>';
                    //                 echo '<td>' . sprintf('%02d', $rowIndex++) . '</td>';
                    //                 echo '<td>' . htmlspecialchars(formatSectionName($section->group), ENT_QUOTES, 'UTF-8') . '</td>';
                    //                 echo '<td>' . htmlspecialchars(isset($section->Summary->ColData[0]->value) ? $section->Summary->ColData[0]->value : '', ENT_QUOTES, 'UTF-8') . '</td>';
                    //                 echo '<td align="right">' . htmlspecialchars(number_format((float) $section->Summary->ColData[1]->value, $decimal), ENT_QUOTES, 'UTF-8') . '</td>';
                    //                 echo '</tr>';
                    //             }
                    //         }
                    //     }
