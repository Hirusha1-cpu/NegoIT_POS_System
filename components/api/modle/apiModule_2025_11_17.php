...
function fetchProductsData()
{
	...
	try {
		...
		$query = "SELECT
					...
					CASE
						WHEN ii.description = '$exactMatch' THEN 1
						WHEN ii.description LIKE '$startsWithMatch' THEN 2
						WHEN ii.description LIKE '$containsMatch' THEN 3
						WHEN ii.description LIKE '$wordsMatch' THEN 4
						ELSE 5
					END as relevance
				FROM inventory_items ii
				INNER JOIN inventory_qty iq ON ii.id = iq.item
				WHERE (
					ii.category = '$productCategory' AND
					ii.description = '$exactMatch' OR
					ii.description LIKE '$startsWithMatch' OR
					ii.description LIKE '$containsMatch' OR
					ii.description LIKE '$wordsMatch'
				)
				AND ii.status = 1
				GROUP BY ii.id
				ORDER BY
					relevance ASC,
					LENGTH(ii.description) ASC,
					ii.description ASC";
