	  ...
	  if(($qty>0)&&($suplier!="")){
		  ...
		  xhttp.onreadystatechange = function() {
		    ...
		    var returntext=this.responseText;
				...
				if(returntext == 'Done') document.getElementById("div|"+$itemid).style.color = "green";
		    }
		  };
...
?></td></tr></table>
<form action="index.php?components=purchase_order&action=home" method="post">
	...
	</td><td width="100px"></td><td>Store</td><td>
	<select name="store" id="store">
		<option value="">-SELECT-</option>
	<?php for($i=0;$i<sizeof($store_id);$i++){
		if(isset($_POST['store'])){
			if($_POST['store']==$store_id[$i]) $select='selected="selected"'; else $select='';
		 }else $select='';
	...
	<br><br>
<?php if(((isset($_POST['category']))&&(isset($_POST['store']))&&(isset($_POST['from_date']))&&(isset($_POST['to_date'])))||(isset($_GET['item_list'])&&isset($_GET['qty_list']))){ ?>
	...
	<tr bgcolor="#DDDDDD"><th>Item</th><th>Selected Store<br>Inventory Qty</th><th>All Store<br>Inventory Qty</th><th>Selected Store<br>Sold Qty</th><th>All Store<br>Sold Qty</th><th>Required Qty<br>for New PO</th><th>PO Supplier</th><th width="100px">Action</th></tr>
	<?php
