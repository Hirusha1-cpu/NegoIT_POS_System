...
function addSupplier(){
	...
	if($out){
		if((isQuickBooksActive(1))){
			if($last_account_id != ''){
				$vendor_array = array('given_name'=>$sup_name, 'company_name' => $sup_name, 'display_name' => $sup_name, 'address' => $address, 'email' => $email, 'primary_phone' => $tel1, 'mobile'=> $tel2);
				try{
			...
			if($last_dis_account_id != ''){
				...
				$account_array = array('account_name'=> 'Dis-'.$sup_name, 'account_type'=>$account_type, 'account_sub_type'=> $account_sub_type, 'account_classification' => $account_classification);
				try{
					$qb_result = QBAddAccount($account_array);
					$qb_msg .= ' Discount account ';
					$qb_msg .= $qb_result['message'];
				}catch (Exception $e) {
					$qb_msg = "<br>QuickBooks error: " . $e->getMessage();
					$qb_result['status'] = 'error';
			...
			if ($last_account_id != '') {
				...
				try {
					...
					if ($qb_result['status'] == 'success') {
						...
						throw new Exception('Failed to add vendor to QuickBooks: ' . $qb_result['message']);
					}
				} catch (Exception $e) {
			...
			if ($last_dis_account_id != '') {
				...
				try {
					...
					if ($qb_result['status'] == 'success') {
						...
						throw new Exception('Failed to add discount account to QuickBooks: ' . $qb_result['message']);
					}
				} catch (Exception $e) {
					throw new Exception('QuickBooks error: ' . $e->getMessage());
				}
