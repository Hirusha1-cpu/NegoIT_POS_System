...
// updated by nirmal 09_07_2025 (added if query fail cases from time_restrict)
...
// updated by nirmal 09_07_2025 (added if query fail cases from device_restrict)
...
{
	$store = isset($_COOKIE['store']) ? $_COOKIE['store'] : 0;
	include('config.php');
	global $conn2;
...
{
	global $unit_type_name, $unit_type_id, $conn2;
	if ($method == 1)
		include('config.php');
	if ($method == 2)
		include('../../../../config.php');
...
function QBGetAccountGeneralLedger($startDate, $endDate, $accountId, $departmentQbId = null)
{
	getQuickBooksRefreshToken(1);
	$dataService = getQuickBooksDataServiceObj();
	$status = 'error';
	...
	if ($departmentQbId) {
		$reportService->setDepartment($departmentQbId);
	}
...
function QBAddJournalEntry($journal_entries_array)
{
	...
	foreach ($journal_entries_array as $entry) {
		...
		// ***** ADD DEPARTMENT REFERENCE HERE *****
		if (!empty($entry['department_id'])) {
			$lineDetail["DepartmentRef"] = [
				"value" => $entry['department_id'],
			];
			// Optionally include the department name for readability
			if (!empty($entry['department_name'])) {
				$lineDetail["DepartmentRef"]["name"] = $entry['department_name'];
			}
		}
...
// added by nirmal 06_08_2024
function QBgetProfitAndLoss($startDate, $endDate, $departmentQbId = null)
{
...
function QBgetTrialBalance($startDate, $endDate, $departmentQbId = null)
{
	getQuickBooksRefreshToken(1); // Ensure you have a valid OAuth token
	$dataService = getQuickBooksDataServiceObj();
	$status = 'error';
	...
	$reportService->setEndDate($endDate);
	$reportService->setAccountingMethod("Accrual");
	$reportService->setSummarizeColumnBy("Departments");
...
// added by nirmal 07_08_2024
function QBgetBalanceSheet($startDate, $endDate, $departmentQbId = null)
{
	getQuickBooksRefreshToken(1); // Ensure you have a valid OAuth token
	$dataService = getQuickBooksDataServiceObj();
	$status = 'error';
...
function QBgetSalesByDepartmentReport($startDate, $endDate, $departmentQbId = null)
{
	getQuickBooksRefreshToken(1); // Ensure you have a valid OAuth token
	$dataService = getQuickBooksDataServiceObj();
	$status = 'error';
	...
	$reportService->setStartDate($startDate);
	$reportService->setEndDate($endDate);
	$reportService->setAccountingMethod("Accrual"); // Or "Cash", depending on your needs

	if ($departmentQbId) {
		$reportService->setDepartment($departmentQbId);
	}

	// Use the DEPARTMENTSALES report name
	$salesByDepartmentReport = $reportService->executeReport(ReportName::DEPARTMENTSALES);

	if (!$salesByDepartmentReport) {
		$message .= 'Sales by Department report is null or an error occurred while fetching.';
		// Get the last error from DataService for more details
		$error = $dataService->getLastError();
		if ($error) {
			$message .= " QuickBooks Error: " . $error->getHttpStatusCode() . " " . $error->getOAuthHelperError() . " " . $error->getResponseBody();
		}
		return ['status' => $status, 'message' => $message];
	} else {
		$status = 'success';
		return ['status' => $status, 'data' => $salesByDepartmentReport];
	}
}

// added by nirmal 07_08_2024
function QBgetJournalReport($startDate, $endDate, $departmentQbId = null)
{
	getQuickBooksRefreshToken(1);
	$dataService = getQuickBooksDataServiceObj();
	$status = 'error';
...
// This version returns an associative array of department-wise journal entries.
// You can call this function in your new view.
function getJournalEntriesGroupedByDepartment($selectedStore, $startDate, $endDate, $startPosition = 1, $maxResults = 50)
{
	// Refresh token and get the data service object
	getQuickBooksRefreshToken(1);
	$dataService = getQuickBooksDataServiceObj();

	// Optionally get the service context
	$dataService->getServiceContext();

	// Base query to fetch all journal entries within the date range
	$query = "SELECT * FROM JournalEntry WHERE TxnDate >= '" . $startDate . "' AND TxnDate <= '" . $endDate . "'";

	// Initialize an array to store all entries
	$allEntries = array();

	// If maxResults is null or "All Records," fetch all records using pagination
	if ($maxResults === null || $maxResults === 'All Records') {
		$startPosition = 1; // Start from the first record
		$chunkSize = 100; // QBO API limit per query
		do {
			// Fetch a chunk of records
			$entries = $dataService->Query($query, $startPosition, $chunkSize);

			// Add the fetched entries to the allEntries array
			$allEntries = array_merge($allEntries, $entries);

			// Increment the start position for the next chunk
			$startPosition += $chunkSize;

			// Stop if fewer than chunkSize records are returned (end of data)
		} while (count($entries) === $chunkSize);

		// If no specific store is selected, return all entries
		if (empty($selectedStore)) {
			return $allEntries;
		}

		// Filter entries by DepartmentRef in the journal entry lines
		$filteredEntries = array();
		foreach ($allEntries as $entry) {
			if (isset($entry->Line) && is_array($entry->Line)) {
				foreach ($entry->Line as $line) {
					if (
						isset($line->JournalEntryLineDetail->DepartmentRef) &&
						$line->JournalEntryLineDetail->DepartmentRef == $selectedStore
					) {
						$filteredEntries[] = $entry;
						break; // Add the entry once and move to the next
					}
				}
			}
		}
	...
	// If maxResults is not "All Records," fetch records with pagination
	$entries = $dataService->Query($query, $startPosition, $maxResults);
...
function getTotalJournalEntries($selectedStore, $startDate, $endDate)
{
	// Refresh token and get the data service object
	getQuickBooksRefreshToken(1);
	$dataService = getQuickBooksDataServiceObj();
	...
	// Execute the query
	$entries = $dataService->Query($query);

	// If no specific store is selected, return the total count of all entries
	if (empty($selectedStore)) {
		...
		if (isset($entry->Line) && is_array($entry->Line)) {
			foreach ($entry->Line as $line) {
				if (
					...
					$filteredCount++;
					break; // Count the entry once and move to the next
				}
			}
		}
	}
...
function buildJournalEntry(
	...
	$creditEntityType = "",
	$creditEntityID = null,
	$debitDepartmentID = null,
	$creditDepartmentID = null,
	$debitClassID = null,
	$creditClassID = null
) {
...
function processQBPayment($creditAccountName, $debitAccountName, $conn, $paymentAmount, $qb_cust_id, $description, $debitClassID = null, $creditClassID = null, $debitDepartmentID = null, $creditDepartmentID = null)
{
...
// added by nirmal 22_07_2025
function QBGetAllEmployees()
{
	getQuickBooksRefreshToken(1);
	$dataService = getQuickBooksDataServiceObj();

	$allEmployees = []; // Array to store all fetched employees
	$startPosition = 1;
	$maxResults = 100; // Number of records to fetch per page
	...
	try {
		do {
			// Construct the query with pagination
			$query = "SELECT * FROM Employee STARTPOSITION $startPosition MAXRESULTS $maxResults";

			// Execute the query for the current page
			$employeesPage = $dataService->Query($query);
			...
			// If results were returned for this page
			if (!empty($employeesPage) && is_array($employeesPage)) {
				// Add the employees from the current page to the main array
				$allEmployees = array_merge($allEmployees, $employeesPage);

				// If the number of results in this page is less than MAXRESULTS,
				// it means we've reached the last page
				if (count($employeesPage) < $maxResults) {
					break; // Exit the do-while loop
				} else {
					// Otherwise, update the start position for the next page
					$startPosition += $maxResults;
				...
				// If no employees were returned on the first page or a subsequent page, stop
				// This handles the case of 0 employees or the last page having exactly maxResults records
				break; // Exit the do-while loop
			}

		} while (true); // Loop continues until explicitly broken

		// If we successfully exited the loop (fetched all data or found no employees)
		$status = 'success';
		return ['status' => $status, 'data' => $allEmployees];
...
// added by nirmal 16_05_2025
function QBAddDepartment($departmentData)
{
	getQuickBooksRefreshToken(1);
	$dataService = getQuickBooksDataServiceObj();
	$status = 'error';
	...
	// 2. Create a Department entity object from the array
	$newDepartment = Department::create($departmentData);

	try {
		// Add the department to QuickBooks Online
		$resultingDepartment = $dataService->add($newDepartment);

		// Check for errors returned by the SDK
		$error = $dataService->getLastError();
		if ($error) {
			$message =
				"QBO Error: " .
			...
			// BUG FIX 2: Update status and message on success.
			$status = 'success';
			$message = 'Department created successfully in QuickBooks.';
			$qb_department_id = $resultingDepartment->Id;
		}
	} catch (Exception $e) {
		// Catch any other exceptions during the process
		$message = "An unexpected error occurred: " . $e->getMessage();
	}
	return array('status' => $status, 'message' => $message, 'qb_department_id' => $qb_department_id);
}

function getQBStore($conn2, $store)
{
	// --- Input Validation ---
	if (!is_numeric($store)) {
		throw new Exception("Invalid store ID provided to getQBStore. Store ID must be a positive integer.");
	}
	$store = (int) $store; // Explicitly cast to integer after validation

	// --- 1. Check Session Cache First ---
	if (isset($_SESSION['qb_store_id'][$store]) && !empty($_SESSION['qb_store_id'][$store])) {
		return $_SESSION['qb_store_id'][$store];
	}

	// --- 2. If not in session, query the database ---
	$query = "SELECT `qb_id` FROM `stores` WHERE `id` = $store";
	$result = mysqli_query($conn2, $query);

	if (!$result) {
		throw new Exception("Database error: Could not execute query in getQBStore. " . mysqli_error($conn2));
	}

	$row = mysqli_fetch_assoc($result);

	if (empty($row)) {
		throw new Exception("Error: Store with ID '{$store}' not found in the database. Please verify the store ID.");
	} else {
		if (empty($row['qb_id'])) {
			throw new Exception("Error: Store '{$store}' exists, but its QuickBooks ID (qb_id) is missing or empty. Please ensure the store is properly registered with QuickBooks.");
		}
		$qb_id = $row['qb_id'];

		// --- 3. Store in session for future use ---
		if (session_id() == '') {
			session_start();
		}
		$_SESSION['qb_store_id'][$store] = $qb_id;
...
function isOdrCrossCheckActive()
{
	...
	if (isset($_SESSION['is_odr_cross_check_active'])) {
		...
		include('config.php');
		$result = mysqli_query($conn2, "SELECT `value` FROM settings WHERE `setting`='odr_cross_check'");
		$row = mysqli_fetch_assoc($result);
		if (!empty($row)) {
			if ($row['value'] == 1) {
				$_SESSION["is_odr_cross_check_active"] = 1;
			} else {
				$_SESSION["is_odr_cross_check_active"] = 0;
			}
		} else {
			$_SESSION["is_odr_cross_check_active"] = 0;
		}
