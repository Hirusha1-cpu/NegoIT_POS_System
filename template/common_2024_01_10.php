...
function listItem($sub_system){
	...
	if ($_POST['keyword']) {
		...
		if ($_GET['action'] == 'code-list') {
			if(($inf_systemid == 13) && ($components == 'bill2')) $qry = "ii.`code` LIKE '%$keyword%'";
			else $qry = "`code` LIKE '%$keyword%'";
			$fn = 'selectCode';
		}
		if ($_GET['action'] == 'desc-list') {
			if(($inf_systemid == 13) && ($components == 'bill2')) $qry = "ii.`description` LIKE '%$keyword%'";
			else $qry = "`description` LIKE '%$keyword%'";
			$fn = 'selectDesc';
		}
		...
		if ($item_filter == 1) {
			if(($inf_systemid == 13) && ($components == 'bill2')) $item_filter_qry = "AND ii.`pr_sr` IN (1,2)";
			else $item_filter_qry = "AND pr_sr IN (1,2)";
		}
		if ($item_filter == 2) {
			if(($inf_systemid == 13) && ($components == 'bill2')) $item_filter_qry = "AND ii.`pr_sr` IN (1,2)";
			else $item_filter_qry = "AND pr_sr IN (1,2)";
		...
		if ($item_filter == 3) {
			if(($inf_systemid == 13) && ($components == 'bill2')) $item_filter_qry = "AND ii.`pr_sr`='3'";
			else $item_filter_qry = "AND pr_sr='3'";
		}

		if ($item_type == 'all') {
			$item_type_qry = "";
		} else {
			if(($inf_systemid == 13) && ($components == 'bill2')) "AND ii.`unic`='$item_type'";
			else $item_type_qry = "AND `unic`='$item_type'";
		}
...
function bill_module($method){
	if(isset($_SESSION['bill_module'])){
		...
		$row = mysqli_fetch_assoc($result);
		if($row['status']==0) $md='billing'; else $md='bill2';
...
function getCustArray($cust){
	...
	'Line1'=>$shop_address,
	'PrimaryEmailAddr'=>$email_add, 'PrimaryPhone'=>$mobile, 'Country'=>$country, 'qb_cust_id'=>$qb_cust_id);
	return $custArray;
}
...
function QBInvoiceCreate($bill_array){
	...
	$apiEndpoint = 'https://'.$baseUrl.'/v3/company/'.$companyId.'/invoice?minorversion='.$minorVersion.'';
	$requestData = [
		"Line" => [
			[
				"Amount" => $amount,
	...
	if (curl_errno($curl)) {
		...
		if ($decodedResponse !== null) {
			if (isset($decodedResponse['fault']) && isset($decodedResponse['fault']['error'])) {
				$errorMessage = isset($decodedResponse['fault']['error'][0]['message']) ? $decodedResponse['fault']['error'][0]['message'] : null;
				$errorCode = isset($decodedResponse['fault']['error'][0]['code']) ? $decodedResponse['fault']['error'][0]['code'] : null;

				if ($errorMessage !== null && $errorCode !== null) {
					$message = "Intuit API Authentication Error: Message - $errorMessage, Code - $errorCode\n";
				} else {
					$message = 'Incomplete authentication error details in the response.';
				}
			} elseif (isset($decodedResponse['Fault']) && isset($decodedResponse['Fault']['error'])) {
				$errorMessage = isset($decodedResponse['Fault']['error'][0]['message']) ? $decodedResponse['Fault']['error'][0]['message'] : null;
				$errorCode = isset($decodedResponse['Fault']['error'][0]['code']) ? $decodedResponse['Fault']['error'][0]['code'] : null;

				if ($errorMessage !== null && $errorCode !== null) {
					$message = "Intuit API Validation Error: Message - $errorMessage, Code - $errorCode\n";
				} else {
					$message = 'Incomplete validation error details in the response.';
				}
			} elseif (isset($decodedResponse['Invoice'])) {
				$qb_doc_id = $decodedResponse['Invoice']['DocNumber'];
				$qb_txnid = $decodedResponse['Invoice']['Id'];
				$status = 'success';
				$message = 'Quickbooks updated!';
			} else {
	...
	return array('status' => $status,'message' => $message, 'qb_doc_id' => $qb_doc_id, 'qb_txnid' => $qb_txnid);
}

// added by nirmal 07_12_2023
function QBAddPaymentForInvoice($payment_array){
	getQuickBooksRefreshToken(1);
	$dataService = getQuickBooksDataServiceObj(); // Prep Data Services (common file function)
	$status='error';
	$message='Error: ';
...
// added by nirmal 12_12_2023
function QBAddPaymentWithoutInvoice($payment_array){
	getQuickBooksRefreshToken(1);
	$dataService = getQuickBooksDataServiceObj(); // Prep Data Services (common file function)
	$status='error';
	$message='Error: ';
...
function getCustomerQBId($cust){
	...
	$row = mysqli_fetch_assoc($result);
	if($row['qb_cust_id'] != '') return $row['qb_cust_id'];
	else return false;
}
