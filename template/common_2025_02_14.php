...
function QBgetAccounts($sortOrder = 'positive_first') {
    ...
    usort($sortedAccounts, function ($a, $b) {
        ...
        if ($a['current_balance'] > 0) return -1;
        if ($b['current_balance'] > 0) return 1;
...
function fetchAccountID($conn2, $accountName) {
...
function buildJournalEntry($conn2, $amount, $debitAccountName, $creditAccountName, $description, $debitEntityType = "", $debitEntityID = null, $creditEntityType = "", $creditEntityID = null) {
    // Fetch Debit Account ID
    $debitAccountID = fetchAccountID($conn2, $debitAccountName);
    if (strpos($debitAccountID, "Error") !== false) {
        return array("error" => $debitAccountID);
    }

    // Fetch Credit Account ID
    $creditAccountID = fetchAccountID($conn2, $creditAccountName);
