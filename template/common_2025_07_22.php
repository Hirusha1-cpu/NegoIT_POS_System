...
function getQBStore($conn, $store)
{
	// --- Input Validation ---
	// Perform type checking manually since scalar type hints are not available
	if (!is_numeric($store)) { // Check for numeric, then cast
	...
	$stmt = mysqli_prepare($conn, "SELECT `qb_id` FROM `stores` WHERE `id` = ?");
	if (!$stmt) {
		throw new Exception("Database error: Could not prepare statement in getQBStore. " . mysqli_error($conn));
	}

	// Bind Parameters and Execute
	mysqli_stmt_bind_param($stmt, "i", $store); // 'i' for integer type is fine
	...
	if (empty($row)) {
		...
		if (session_id() == '') { // Check if session is NOT active
			session_start();
		}
