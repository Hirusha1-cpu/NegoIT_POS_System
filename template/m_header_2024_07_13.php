...
<?php
	...
	if($systemid == 13 && $sub_system == 1) $shipment = 'Cost of Operations'; else $shipment = 'Shipments';
?>
<title><?php print $inf_company; ?> <?php print $apptitle; ?></title>
	<?php
	if ($systemid == 14) print '<link rel="manifest" href="js/manifest14.json">';
	elseif ($systemid == 15) print '<link rel="manifest" href="js/manifest15.json">';
	else print '<link rel="manifest" href="js/manifest.json">';
	?>
...
<style type="text/css">
	...
	?>.w3-theme {
		color: #fff !important;
		background-color: <?php print $json_array->{"theme_color_m1"}; ?> !important
	}

	.w3-theme-d2 {
		color: #fff !important;
		background-color: <?php print $json_array->{"theme_color_m2"}; ?> !important
	}
...
<body>
	<nav class="w3-sidenav w3-card-2 w3-white w3-top" style="width:30%;display:none;z-index:2" id="mySidenav">
		...
		<?php
			...
			if (checkPendingCust($sub_system)) $localstyle1 = 'style="color:red"';
			if (deleteAck()) $localstyle11 = 'style="color:red"';
			...
			if (isset($_COOKIE['billing'])) { ?>
				<a href="index.php?components=billing&action=home&s=<?php print $_COOKIE['user_id']; ?>&cust_odr=no"><strong>Billing</strong></a>
			<?php }

			// Bill2
			if (isset($_COOKIE['bill2'])) { ?>
				<a href="index.php?components=bill2&action=home&s=<?php print $_COOKIE['user_id']; ?>&cust_odr=no"><strong>Bill2</strong></a>
			<?php }
			...
			?>
				<a href="index.php?components=inventory&action=show_all_item&category=<?php print $menu_cat; ?>&store=<?php print $_COOKIE['store']; ?>&type=1"><strong>Inventory</strong></a>
			<?php }

			if (isset($_COOKIE['stores_transfer'])) {
				if (checkPendingGTN()) $localstyle2 = 'style="color:red"';
				else $localstyle2 = '';
			?>
			...
			if (isset($_COOKIE['supervisor'])) { ?>
				<a href="index.php?components=supervisor&action=sale&store=<?php print $_COOKIE['store']; ?>&group=all&salesman=all&processby=all&lock=1&cashback=no&type=" <?php print $localstyle1; ?>><strong>Supervisor</strong></a>
			<?php }

			if (isset($_COOKIE['hire_purchase'])) { ?>
				<a href="index.php?components=hire_purchase&action=home" <?php print $localstyle1; ?>><strong>Hire Purchase</strong></a>
			<?php } if (isset($_COOKIE['hr'])) {
				if (checkPendingHR($sub_system)) $localstyle9 = 'style="color:red"';
				else $localstyle9 = ''; ?>
			...
			if (isset($_COOKIE['manager'])) {
				...
				<?php } else { ?>
					<a href="index.php?components=manager&action=daily_sale&store=all&group=all&salesman=all&processby=all&lock=1&type=&cashback=no" <?php print $localstyle1; ?>><strong>Manager</strong></a>
				<?php } ?>

			<!-- Top Manager -->
			<?php } if (isset($_COOKIE['top_manager'])) { ?>
				<a href="index.php?components=topmanager&action=daily_sale&store=all&group=all&salesman=all&processby=all&lock=1&type=&cashback=no" <?php print $localstyle3; ?>><strong>Top Manager</strong></a>

			<!-- Report -->
			<?php } if (isset($_COOKIE['report'])) {
				if (requestApproval()) $localstyle7 = 'style="color:red"';
				else $localstyle7 = '';
				$localstyle_rep = '';
				if ($localstyle7 != '') $localstyle_rep = $localstyle7;
				if ($localstyle11 != '') $localstyle_rep = $localstyle11; ?>
			...
			<?php if ($_REQUEST['components'] == 'availability') { ?>
				<?php if ($_COOKIE['direct_mkt'] == 0) print '<a href="index.php?components=availability&action=home&category=all">Check Availability</a>'; ?>
				<?php if (($systemid == 14) || ($systemid == 15) || ($systemid == 16) || ($systemid == 17) || ($systemid == 13 && $sub_system == 0)) print '<a href="index.php?components=availability&action=catalog">Catalog</a>'; ?>
				<?php if (($systemid == 10) || ($systemid == 13) || ($systemid == 14) || ($systemid == 15) || ($systemid == 16)) print '<a href="index.php?components=availability&action=stock">Stock</a>'; ?>
			<?php } ?>

			<!-- Billing Menu Items -->
			<?php if ($_REQUEST['components'] == 'billing') { ?>
				<a href="index.php?components=billing&action=home&s=<?php print $_COOKIE['user_id']; ?>&cust_odr=no">Sales Billing</a>
				<?php if (($systemid == 1) || ($systemid == 10) || ($systemid == 13) || ($systemid == 16) || ($systemid == 17)) { ?>
					<a href="index.php?components=billing&action=home&s=<?php print $_COOKIE['user_id']; ?>&cust_odr=yes">Cust Order</a><?php } ?>
				...
				<a href="index.php?components=billing&action=credit&st=<?php print $_COOKIE['store']; ?>&gp=&display=2">Credit Report</a>
				...
				<a href="index.php?components=billing&action=unvisited&type=unvisited&asso_salesman=<?php print $_COOKIE['user_id']; ?>">Unvisited Customers</a>
				<?php
				if ($systemid != 1 && $systemid != 4) print '<a href="index.php?components=billing&action=sale&store=all&group=all&salesman=' . $_COOKIE['user_id'] . '&processby=all&lock=1&type=">My Sales</a>';
				if (($systemid != 15) && ($systemid != 14) && ($systemid != 24)) print '<a href="index.php?components=billing&action=item_return">Item Return</a>';
				?>
				...
				<a href="index.php?components=billing&action=salesman_commission_incomplete_one">Incomplete Salesman Commission Report</a>
				<?php } ?>
			<?php } ?>

			<!-- Bill2 Menu Items -->
			<?php if ($_REQUEST['components'] == 'bill2') { ?>
				<a href="index.php?components=bill2&action=home&s=<?php print $_COOKIE['user_id']; ?>&cust_odr=no">Sales Billing</a>
				<?php if (($systemid == 1) || ($systemid == 10) || ($systemid == 13) || ($systemid == 16) || ($systemid == 17)) { ?>
					<a href="index.php?components=bill2&action=home&s=<?php print $_COOKIE['user_id']; ?>&cust_odr=yes">Cust Order</a>
				<?php } ?>
				...
				<a href="index.php?components=bill2&action=credit&st=<?php print $_COOKIE['store']; ?>&gp=&display=2">Credit Report</a>
				...
				<a href="index.php?components=bill2&action=unvisited&type=unvisited&asso_salesman=<?php print $_COOKIE['user_id']; ?>">Unvisited Customers</a>
				<?php
					if($systemid != 1 && $systemid != 4) print '<a href="index.php?components=bill2&action=sale&store=all&group=all&salesman='.$_COOKIE['user_id'].'&processby=all&lock=1&type=">My Sales</a>';
					if($systemid != 15) print '<a href="index.php?components=bill2&action=item_return">Item Return</a>';
				?>
				...
				<a href="index.php?components=bill2&action=salesman_commission_incomplete_one">Incomplete Salesman Commission Report</a>
				<?php } ?>
			<?php } ?>
			...
			<?php if ($_REQUEST['components'] == 'order_process') { ?>
				...
				<a href="index.php?components=order_process&action=list_packed"><?php if($systemid == 13 && $sub_system == 1) echo 'Started'; else print 'Packed';?></a>
				<?php if($systemid !=  13 || $sub_system != 1) { ?> <a href="index.php?components=order_process&action=list_shipped">Shipped</a>  <?php } ?>
				<a href="index.php?components=order_process&action=list_delivered"><?php if($systemid == 13 && $sub_system == 1) echo 'Finished'; else print 'Delivered';?></a>
				<a href="index.php?components=order_process&action=list_return">Return</a>
				<?php if ($approver2) print '<a href="index.php?components=order_process&action=report_commision">Report</a>'; ?>
			<?php } ?>
			...
			<?php if ($_REQUEST['components'] == 'repair') { ?>
				...
				<?php
					if (isset($_COOKIE['manager'])) print '<a href="index.php?components=repair&action=change_st">MGR</a>';
				?>
			<?php }  ?>
			...
			if ($_REQUEST['components'] == 'supervisor') { ?>
				<a href="index.php?components=supervisor&action=sale&store=<?php print $_COOKIE['store']; ?>&group=all&salesman=all&processby=all&lock=1&type=">Sales Report</a>
				<a href="index.php?components=supervisor&action=repair_income">Repair Income</a>
				<a href="index.php?components=supervisor&action=credit&st=<?php print $_COOKIE['store']; ?>&gp=&display=2">Credit Report</a>
				<a href="index.php?components=supervisor&action=sales_byrep">Sales By Rep</a>
				<?php if ($_COOKIE['user'] == 'nimesha') print '<a href="index.php?components=supervisor&action=deleted" ' . $localstyle11 . '>Deleted</a>'; ?>
				...
				<?php if(bill_module(1) == 'bill2') { ?> <a href="index.php?components=supervisor&action=temporary_bills">Temporary Bills</a> <?php } ?>
				...
				<?php if($systemid == 13){ ?>
					...
					<a href="index.php?components=supervisor&action=quotation_ongoing">On-Going Quotationn</a>
					<a href="index.php?components=supervisor&action=quotation_list&cust=&item=&st=&sm=&status=all">Quotation List</a>
				<?php } ?>
			<?php }
			...
			<?php if ($_REQUEST['components'] == 'manager') { ?>
				...
				<a href="index.php?components=manager&action=daily_sale&store=all&group=all&salesman=all&processby=all&lock=1&type=&cashback=no">Daily Sales</a>
				...
				<a href="index.php?components=manager&action=chque_pending_finalyze">Cheque Pending Finalyze</a>
				...
				<?php if(bill_module(1) == 'bill2') { ?> <a href="index.php?components=manager&action=temporary_bills">Temporary Bills</a> <?php } ?>
				...
				<?php if($systemid!=15) print '<a href="index.php?components=manager&action=authorize_code">Authorize Code</a>'; ?>
			<?php } ?>
			...
			<?php if ($_REQUEST['components'] == 'topmanager') { ?>
				...
				<a href="index.php?components=topmanager&action=daily_sale&store=all&group=all&salesman=all&processby=all&lock=1&type=&cashback=no">Daily Sales</a>
				...
				<a href="index.php?components=topmanager&action=credit&st=<?php print $_COOKIE['store']; ?>&display=2&sub_system=0">Credit Report</a>
				...
				<?php if(bill_module(1) == 'bill2') { ?> <a href="index.php?components=topmanager&action=temporary_bills">Temporary Bills</a> <?php } ?>
				...
				<?php if ($systemid == 13) { ?>
					...
					<a href="index.php?components=topmanager&action=quotation_list&cust=&item=&st=&sm=&status=all">Quotation List</a>
			...
			if ($_REQUEST['components'] == 'report') { ?>
				...
				<?php if(isProfitReportActive()){ ?><a href="index.php?components=report&action=profit_report" >Profit Report</a> <?php } ?>
				...
				<a href="index.php?components=report&action=clear_chque_list&year=<?php print date("Y", time()); ?>">Cheque Report</a>
				...
				<a href="index.php?components=report&action=salesman_commission_incomplete">Incomplete Salesman Commission Report</a>
			...
			<br />
			<br />
			<hr /><a href="index.php?components=authenticate&action=change_pw" style="text-decoration:none; color:blue; font-weight:bold;"><?php if (isset($_COOKIE['user'])) print ucfirst($_COOKIE['user']); ?></a>
				...
				<tr>
					...
					<?php
						switch ($_REQUEST['components']) {
							...
							case "billing":
								if ($_REQUEST['action'] == 'home') {
									if ($_GET['cust_odr'] == 'no') print '<td>Sales Billing</td><td align="right">';
									else print '<td>Cust Order</td><td align="right">';
									include_once  'components/billing/view/tpl/district.php';
									print '</td>';
								} else if ($_REQUEST['action'] == 'pay_bill') {
									if ($_GET['cust_odr'] == 'no') print '<td>Bill Payment</td><td align="right">';
									else print '<td>Advance Payment</td><td align="right">';
									...
									print '<td>Billing</td><td align="right">Incompleted Salesman Commission Report</td>';
							...
							case "bill2":
								if ($_REQUEST['action'] == 'home') {
									...
									if ($_GET['cust_odr'] == 'no') print '<td>Bill Payment</td><td align="right">';
									else print '<td>Advance Payment</td><td align="right">Bill2 ';
							...
							case "order_process":
								if ($_REQUEST['action'] == 'list_custodr') {
									...
									if($systemid == 13 && $sub_system == 1) $packed = 'Started'; else $packed = 'Packed';
									...
									if($systemid == 13 && $sub_system == 1) $delivered = 'Finished'; else $delivered = 'Delivered';
							...
							case "inventory":
								...
								?>
									<button onclick="billingMenu22()" style="background-color:maroon; color:white" class="dropbtn2">All Items</button>
									<div id="myDropdown" class="dropdown-content">
										...
										<?php if ($sub_system == 0) { ?> <a onmouseover="billingMenu2()" onmouseleave="billingMenu2()" style="font-size:11pt" href="index.php?components=inventory&action=show_all_item&category=1&store=<?php print $_COOKIE['store']; ?>&type=2">Service INV</a><?php } ?>
										<?php if ($sub_system == 0) { ?> <a onmouseover="billingMenu2()" onmouseleave="billingMenu2()" style="font-size:11pt" href="index.php?components=inventory&action=show_all_item&category=1&store=<?php print $_COOKIE['store']; ?>&type=3">Repair INV</a><?php } ?>
										<?php if ($sub_system == 0) { ?> <a onmouseover="billingMenu2()" onmouseleave="billingMenu2()" style="font-size:11pt" href="index.php?components=inventory&action=show_all_item&category=1&store=<?php print $_COOKIE['store']; ?>&type=4">Repair Parts INV</a><?php } ?>
									</div>
								<?php
									print '</td>';
									...
									if ($_REQUEST['sub'] == 'show_add_qty')	print '<td>Inventory</td><td align="right">Add Qty</td>';
									if ($_REQUEST['sub'] == 'show_add_unic')	print '<td>Inventory</td><td align="right">Add Unique</td>';
								}
								break;
