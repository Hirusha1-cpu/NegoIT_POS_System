...
function QBgetJournalEntryReportByQuery()
{
	// Refresh token and get the data service object
	getQuickBooksRefreshToken(1);
	$dataService = getQuickBooksDataServiceObj();

	// Optionally initialize service context
	$dataService->getServiceContext();

	// Retrieve all JournalEntry objects
	$entries = $dataService->Query("SELECT * FROM JournalEntry");

	// Debug: display details for each entry
	foreach ($entries as $entry) {
		echo "Journal Entry DocNumber: " .
			(isset($entry->DocNumber) ? $entry->DocNumber : 'N/A') . "\n";
		echo "Txn Date: " . $entry->TxnDate . "\n";
		if (isset($entry->Line) && is_array($entry->Line)) {
			foreach ($entry->Line as $line) {
				echo "Line Description: " . $line->Description . "\n";
				echo "JournalEntryLineDetail properties:\n";
				var_dump($line->JournalEntryLineDetail);
				echo "\n";
			}
		}
		echo "---------------------------\n";
