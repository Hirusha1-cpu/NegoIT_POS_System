...
function setCustOrderMain()
{
	...
	try {
		...
		if (isQuickBooksActive(1)) {
			$custQuery = "SELECT c.qb_cust_id, c.name, s.`qb_id` FROM cust c JOIN bill_main bm ON bm.cust = c.id JOIN stores s ON bm.store = s.id WHERE bm.invoice_no = '$invoice_no' LIMIT 1";
			...
			// $invoiceQuery = "SELECT SUM(b.`qty` * b.`cost`) as cost, SUM(bm.`invoice_+total`+bm.`invoice_-total`) as total, bm.`qb_value_id`, bm.`qb_cost_id`, c.`name`, c.`qb_cust_id`, bm.`tax`
			...
			// Query 1: Get invoice-level details
			$billMainQuery = "SELECT (bm.`invoice_+total` + bm.`invoice_-total`) AS total, bm.tax, bm.qb_value_id, bm.qb_cost_id, c.name, c.qb_cust_id
                  FROM `bill_main` bm
                  JOIN `cust` c ON bm.cust = c.id
                  WHERE bm.invoice_no = '$invoice_no' AND bm.qb_status IS NOT NULL";
			$billMainResult = mysqli_query($conn, $billMainQuery);
			...
			if ($billMainResult && mysqli_num_rows($billMainResult) > 0) {
				$invoiceDetails = mysqli_fetch_assoc($billMainResult);

				$invoiceTotal = $invoiceDetails['total'];
				$bill_tax = $invoiceDetails['tax'];
				$custName = $invoiceDetails['name'];
				$qb_cust_id = $invoiceDetails['qb_cust_id'];


				// Query 2: Get the sum of item costs
				$itemCostQuery = "SELECT SUM(b.qty * b.cost) AS cost
                  FROM `bill` b
                  WHERE b.invoice_no = '$invoice_no'";

				$itemCostResult = mysqli_query($conn, $itemCostQuery);
				// Robustly check the result
				if (!$itemCostResult) {
					throw new Exception(
						"Database Error: " . mysqli_error($conn)
				...
				$costRow = mysqli_fetch_assoc($itemCostResult);
				$invoiceCost = $costRow['cost'];
				...
				if ($bill_tax != 0) {
					...
					$description = "[INVOICE VAT DELETE] - Invoice No: $invoice_no, Customer : $custName";
					$debitEntityType = "";
					$debitEntityID = "";
					$creditEntityType = "";
					$creditEntityID = "";
				...
				$query = "SELECT rt.replace_item, rt.qty, rm.mapped_inventory, rt.extra_pay, rt.id, s.`qb_id`
									...
									JOIN bill_main bm ON bm.invoice_no = rt.odr_no
				...
				if (mysqli_num_rows($result) > 0) {
					$item_cost = 0;
					$debitDepartmentID = $creditDepartmentID = $storeQBID = null;
					$debitClassID = $creditClassID = null;
					...
					$qb_message = ". QuickBooks is not updated: This invoice was not recorded with qb before";
				}
			} else {
				$qb_message = ". QuickBooks is not updated: This invoice was not recorded with qb before";
			}
		...
		mysqli_commit($conn);
		$message = 'Invoice status updated successfully' . $qb_message;
		return true;
	} catch (Exception $ex) {
		mysqli_rollback($conn);
		$message = $ex->getMessage() . $qb_message;
		return false;
	}
}
		...
		if (isQuickBooksActive(1)) {
			if ($old_type != $type) {
				...
				if ($old_type == 1 && $type == 4) {
					...
					// Query 1: Get invoice-level details
					$billMainQuery = "SELECT (bm.`invoice_+total` + bm.`invoice_-total`) AS total, bm.qb_value_id, bm.qb_cost_id, c.name, c.qb_cust_id, s.`qb_id` as `store_qb_id`
                  FROM `bill_main` bm
                  JOIN `cust` c ON bm.cust = c.id
									JOIN stores s ON bm.store = s.id
                  WHERE bm.invoice_no = '$bill_no'
                    AND bm.qb_status IS NOT NULL
                    AND c.qb_cust_id IS NOT NULL";
					$billMainResult = mysqli_query($conn, $billMainQuery);
					if (!$billMainResult) {
						throw new Exception(
							"Database Error: " . mysqli_error($conn)
					...
					// Robustly check the result
					if (mysqli_num_rows($billMainResult) > 0) {
						$invoiceDetails = mysqli_fetch_assoc($billMainResult);
						// Now you have the correct, non-duplicated values
						$invoiceTotal = $invoiceDetails['total'];
						$custName = $invoiceDetails['name'];
						$qb_cust_id = $invoiceDetails['qb_cust_id'];
						$storeQBID = $invoiceDetails['store_qb_id'];

						if ($qb_cust_id === null || $qb_cust_id === '') {
							throw new Exception("This customer is not registered in QB");
						}
						...
						// Query 2: Get the sum of item costs
						$itemCostQuery = "SELECT SUM(b.qty * b.cost) AS cost FROM `bill` b WHERE b.invoice_no = '$bill_no'";
						$itemCostResult = mysqli_query($conn, $itemCostQuery);
						// Robustly check the result
						if (!$itemCostResult) {
							throw new Exception("Could not calculate item costs for invoice #$bill_no.");
						}
						$costRow = mysqli_fetch_assoc($itemCostResult);
						$invoiceCost = $costRow['cost'];
						...
						if (isset($journalEntryForDeleteInvoiceValue['error'])) {
							$qb_msg = mysqli_real_escape_string($conn, $journalEntryForDeleteInvoiceValue['error']);
							throw new Exception($qb_msg);
						} else {
							$batch_id = generateBatchID();
				...
				if ($old_type == 4 && $type == 1) {
					$query = "SELECT bm.`invoice_+total`, bm.`invoice_-total`,c.`qb_cust_id`, c.`name`, s.`qb_id` FROM bill_main bm, `cust` c, stores s
                    WHERE c.`id` = bm.`cust` AND s.`id` = bm.`store` AND bm.`qb_status` IS NOT NULL AND c.`qb_cust_id` IS NOT NULL AND bm.`invoice_no`='$bill_no'";
					$result = mysqli_query($conn, $query);
					if (!$result) {
	...
	try {
		...
		if ($type == 'cheque') {
			...
			if ($original_payment_type == 3 && $new_payment_type == 1) { // bank to cash
				...
				$query = "UPDATE payment SET payment_date = '$date', invoice_no = '$invoice_no', amount = '$new_payment_amount', salesman = '$salesman', store = '$store', payment_type = '$new_payment_type', bank_trans = '$bank_tr' $status_query WHERE id = '$payment_id'";
			}
			$result = mysqli_query($conn, $query);
			if (!$result) {
		...
		if (!empty($original_qb_status) && $paymentTypeChanged && isQuickBooksActive(1)) {
			...
			$custQuery = "SELECT c.qb_cust_id, c.name, s.qb_id as store_qb_id FROM cust c JOIN payment p ON p.cust = c.id JOIN stores s ON p.store = s.id WHERE p.id = '$payment_id' LIMIT 1";
			$custResult = mysqli_query($conn, $custQuery);
			if (!$custResult || mysqli_num_rows($custResult) == 0) {
				throw new Exception("Failed to fetch customer details for QB reversal.");
			}
			...
			$custName = $custRow['name'];
			$storeQBID = isset($custRow['store_qb_id']) ? $custRow['store_qb_id'] : '';
		...
		if (isQuickBooksActive(1) && !empty($original_qb_id) && !$paymentTypeChanged && floatval($new_payment_amount) != floatval($original_amount)) {
			try {
...
function changeInvoiceCust()
{
	...
	try {
		...
		if (!isset($_POST['invoice_id']) || empty($_POST['invoice_id'])) {
			throw new Exception("Error: Invoice ID is empty.");
		}
		if (!isset($_POST['new_invoice_cust_id']) || empty($_POST['new_invoice_cust_id'])) {
			throw new Exception("Error: New customer ID is empty.");
		}
...
function changePaymentCust()
{
	...
	try {
		...
		if (!isset($_POST['payment_id']) || empty($_POST['payment_id'])) {
			throw new Exception("Error: Payment ID is empty.");
		}
		if (!isset($_POST['new_payment_cust_id']) || empty($_POST['new_payment_cust_id'])) {
			throw new Exception("Error: Customer ID is empty.");
		}
...
function deleteCashPaymentDeposit()
{
	...
	try {
		...
		if (isQuickBooksActive(1)) {
			...
			$query = "SELECT pd.`transfer_source`, pd.`bank_id`, up.`username`, pd.`amount`, s.`qb_id` FROM payment_deposit pd, userprofile up, stores s
			WHERE up.`id` = pd.`placed_by` AND s.`id` = up.`store` AND pd.`status` = 2 AND pd.`qb_id` IS NOT NULL AND pd.`id`='$id'";
			$result = mysqli_query($conn, $query);
			if (!$result) {
			...
			if ($row && !empty($row['transfer_source'])) {
				...
				$qb_message = " QuickBooks is not updated: This payment was not recorded with qb before";
			}
		}
		...
		$message = "Cash payment deposit deleted successfully." . $qb_message;
		return true;
	} catch (Exception $e) {
		mysqli_rollback($conn);
		$message = $e->getMessage() . $qb_message;
		return false;
	} finally {
		// Close the database connection
		if (isset($conn)) {
