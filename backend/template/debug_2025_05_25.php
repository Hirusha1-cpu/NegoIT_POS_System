...
function debugStart($debug_input1,$debug_input2){
	...
	if($_REQUEST['components']=='billing'){
		if($_REQUEST['action']=='apend_bill'){
			...
			$result = mysql_query("SELECT id,qty FROM inventory_qty WHERE item='$debug_item' AND location='$debug_store'");
		...
		if($_REQUEST['action']=='bill_item_gpdate'){
			...
			$debug_store=$_COOKIE['store'];
			$result = mysql_query("SELECT item FROM bill WHERE id='$debug_actionid'");
			$row = mysql_fetch_assoc($result);
			$debug_item=$row['item'];
			$debug_actionqty=-$_REQUEST['qty'];
			$result = mysql_query("SELECT id,qty FROM inventory_qty WHERE item='$debug_item' AND location='$debug_store'");
		...
		if($_REQUEST['action']=='bill_item_remove'){
			...
			$debug_actionid=$_REQUEST['id'];
			$debug_store=$_COOKIE['store'];
			$result = mysql_query("SELECT item,qty FROM bill WHERE id='$debug_actionid'");
			$row = mysql_fetch_assoc($result);
			$debug_item=$row['item'];
		...
		if($_REQUEST['action']=='return_item_gpdate'){
			...
			$debug_actionid=$_REQUEST['id'];
			$debug_store=$_COOKIE['store'];
			$result = mysql_query("SELECT item FROM `return` WHERE id='$debug_actionid'");
			$row = mysql_fetch_assoc($result);
			$debug_item=$row['item'];
			$debug_actionqty=$_REQUEST['qty'];
			$result = mysql_query("SELECT id,qty FROM inventory_qty WHERE item='$debug_item' AND location='$debug_store'");
		...
		if($_REQUEST['action']=='return_item_remove'){
			...
			$debug_actionid=$_REQUEST['id'];
			$debug_store=$_COOKIE['store'];
			$result = mysql_query("SELECT return_item,qty FROM `return` WHERE id='$debug_actionid'");
			$row = mysql_fetch_assoc($result);
	...
	if($_REQUEST['components']=='order_process'){
		...
		if($_REQUEST['action']=='process_return'){
			...
			$debug_store=$_COOKIE['store'];
			$result = mysql_query("SELECT item,qty FROM inventory_qty WHERE id='$debug_itqid'");
			$row = mysql_fetch_assoc($result);
			$debug_item=$row['item'];
			$debug_startqty=$row['qty'];
	...
	if($_REQUEST['components']=='trans'){
		...
		if($_REQUEST['action']=='gtn_item_gpdate'){
			...
			$debug_actionid=$_REQUEST['id'];
			$debug_store=$_COOKIE['store'];
			$result = mysql_query("SELECT item FROM `transfer` WHERE id='$debug_actionid'");
			$row = mysql_fetch_assoc($result);
			$debug_item=$row['item'];
			$debug_actionqty=-$_REQUEST['qty'];
			$result = mysql_query("SELECT id,qty FROM inventory_qty WHERE item='$debug_item' AND location='$debug_store'");
		...
		if($_REQUEST['action']=='gtn_item_remove'){
			...
			$debug_actionid=$_REQUEST['id'];
			$debug_store=$_COOKIE['store'];
			$result = mysql_query("SELECT item,qty FROM `transfer` WHERE id='$debug_actionid'");
			$row = mysql_fetch_assoc($result);
			$debug_item=$row['item'];
			$debug_actionqty=$row['qty'];
			$result = mysql_query("SELECT id,qty FROM inventory_qty WHERE item='$debug_item' AND location='$debug_store'");
	...
	if($_REQUEST['components']=='inventory'){
		...
		if($_REQUEST['action']=='shipment_item_gpdate'){
			...
			$debug_actionid=$_REQUEST['id'];
			$debug_store=$_COOKIE['store'];
			$result = mysql_query("SELECT inv_item FROM `inventory_shipment` WHERE id='$debug_actionid'");
			$row = mysql_fetch_assoc($result);
		...
		if($_REQUEST['action']=='shipment_item_remove'){
			...
			$debug_actionid=$_REQUEST['id'];
			$debug_store=$_COOKIE['store'];
			$result = mysql_query("SELECT inv_item,added_qty FROM `inventory_shipment` WHERE id='$debug_actionid'");
			$row = mysql_fetch_assoc($result);
			$debug_item=$row['inv_item'];
		...
		if($_REQUEST['action']=='delete_unic'){
			...
			$debug_sn=$_GET['sn'];
			$debug_store=$_COOKIE['store'];
			$result = mysql_query("SELECT id,itq_id FROM inventory_unic_item WHERE `status`!='2' AND sn='$debug_sn' LIMIT 1");
			$row = mysql_fetch_assoc($result);
	...
	if($debug_proceed){
		...
		$result = mysql_query("SELECT sum(qty) as `qty` FROM inventory_new WHERE store='$debug_store' AND item='$debug_item'");
		$row = mysql_fetch_assoc($result);
		$debug_startqty+=$row['qty'];

		$result = mysql_query("SELECT unic FROM inventory_items WHERE id='$debug_item'");
		$row = mysql_fetch_assoc($result);
		$debug_unic=$row['unic'];
		if($debug_unic==1){
			$result = mysql_query("SELECT count(id) as `qty` FROM inventory_unic_item WHERE itq_id='$debug_itqid' AND `status`=0");
			$row = mysql_fetch_assoc($result);
...
// update by nirmal 04_03_2024 (store cookie is not use in backend users, hence store is not set)
function debugEnd($debug_id,$debug_status){
	...
	if($debug_id!=0){
		...
		if($_REQUEST['components']=='billing'){
			$debug_proceed=true;
			if($_REQUEST['action']=='apend_bill'){
				$result = mysql_query("SELECT MAX(id) as `lastid` FROM bill");
				$row = mysql_fetch_assoc($result);
				$debug_actionid=$row['lastid'];
				$debug_actionqry=",`action_id`='$debug_actionid'";
			}
			if($_REQUEST['action']=='apend_return'){
				$result = mysql_query("SELECT MAX(id) as `lastid` FROM `return`");
				$row = mysql_fetch_assoc($result);
				$debug_actionid=$row['lastid'];
				$debug_actionqry=",`action_id`='$debug_actionid'";
			}
		...
		if($_REQUEST['components']=='trans'){
			$debug_proceed=true;
			if($_REQUEST['action']=='apend_gtn'){
				$result = mysql_query("SELECT MAX(id) as `lastid` FROM `transfer`");
				$row = mysql_fetch_assoc($result);
				$debug_actionid=$row['lastid'];
				$debug_actionqry=",`action_id`='$debug_actionid'";
			}
			if($_REQUEST['action']=='reject'){
				$result = mysql_query("SELECT tr.from_stores FROM debug dg, transfer tr WHERE tr.id=dg.action_id AND dg.id='$debug_id'");
				$row = mysql_fetch_assoc($result);
		...
		if($_REQUEST['components']=='inventory'){
			$debug_proceed=true;
			if($_REQUEST['action']=='add_qty'){
				$result = mysql_query("SELECT MAX(id) as `lastid` FROM `inventory_shipment`");
				$row = mysql_fetch_assoc($result);
				$debug_actionid=$row['lastid'];
				$debug_actionqry=",`action_id`='$debug_actionid'";
			}
			if($_REQUEST['action']=='edit_item'){
				$result = mysql_query("SELECT itq.location FROM debug dg, inventory_qty itq WHERE dg.itq_id=itq.id AND dg.id='$debug_id'");
				$row = mysql_fetch_assoc($result);
		...
		if($debug_proceed){
			$result = mysql_query("SELECT itq.qty,dg.unic,itq.id FROM debug dg, inventory_qty itq WHERE itq.item=dg.item AND itq.location='$debug_store' AND dg.id='$debug_id'");
			$row = mysql_fetch_assoc($result);
			$debug_endqty=$row['qty'];
			$debug_unic=$row['unic'];
			$debug_itqid=$row['id'];

			$result = mysql_query("SELECT sum(itn.qty) as `qty` FROM debug dg ,inventory_new itn WHERE dg.item=itn.item AND itn.store='$debug_store' AND dg.id='$debug_id'");
			$row = mysql_fetch_assoc($result);
			$debug_endqty+=$row['qty'];
